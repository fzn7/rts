...
CVFSHandler::AddArchiveWithDeps(const std::string& archiveName,
                                ...
                                const std::string& type)
{
    const std::vector<std::string>& ars =
      archiveScanner->GetAllArchivesUsedBy(archiveName);
    ...
    for (it = ars.begin(); it != ars.end(); ++it) {
        if (!AddArchive(*it, override, type))
            throw content_error("Failed loading archive '" + *it +
...
CVFSHandler::RemoveArchive(const std::string& archiveName)
{
    ...
    for (std::map<std::string, FileData>::iterator f = files.begin();
         f != files.end();) {
        if (f->second.ar == ar) {
...
CVFSHandler::~CVFSHandler()
{
    LOG_L(L_INFO,
          "[%s] #archives=%lu",
          __FUNCTION__,
          (long unsigned)archives.size());

    for (std::map<std::string, IArchive*>::iterator i = archives.begin();
         i != archives.end();
         ++i) {
...
CVFSHandler::GetFileData(const std::string& normalizedFilePath)
{
    ...
    const std::map<std::string, FileData>::const_iterator fi =
      files.find(normalizedFilePath);
    if (fi != files.end()) {
        fileData = &(fi->second);
    }
                      ...
                      std::vector<boost::uint8_t>& buffer)
{
    ...
    if (fileData == NULL) {
        LOG_L(L_DEBUG,
              "LoadFile: File '%s' does not exist in VFS.",
              filePath.c_str());
        return false;
    }
    ...
    if (!fileData->ar->GetFile(normalizedPath, buffer)) {
        LOG_L(L_DEBUG,
              "LoadFile: File '%s' does not exist in archive.",
              filePath.c_str());
        return false;
    }
    return true;
