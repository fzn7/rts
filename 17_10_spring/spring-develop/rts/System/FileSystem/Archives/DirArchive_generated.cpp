...
IArchive*
CDirArchiveFactory::DoCreateArchive(const std::string& filePath) const
{
  ...
  , dirName(archiveName + '/')
{
    const std::vector<std::string>& found =
      dataDirsAccess.FindFiles(dirName, "*", FileQueryFlags::RECURSE);
...
CDirArchive::GetFile(unsigned int fid, std::vector<boost::uint8_t>& buffer)
{
    ...
    const std::string rawpath =
      dataDirsAccess.LocateFile(dirName + searchFiles[fid]);
