...
ReadOnlyConfigSource::GetString(const string& key) const
{
    ...
    if (pos == data.end()) {
        throw std::runtime_error("ConfigSource: Error: Key does not exist: " +
                                 key);
    }
    return pos->second;
}
  ...
  : filename(filename)
{
    ...
    if ((file = fopen(filename.c_str(), "r"))) {
        ...
        LOG_L(L_ERROR,
              "FileConfigSource: Error: Could not write to config file \"%s\"",
              filename.c_str());
    }
}
...
{
    ReadModifyWrite(
      boost::bind(&FileConfigSource::SetStringInternal, this, key, value));
}
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (*line_stripped == '\0' || *line_stripped == '#') {
            ...
            if (eq) {
                ...
                LOG_L(
                  L_ERROR, "ConfigSource: Error: Can not parse line: %s", line);
            }
        }
    }
...
FileConfigSource::Write(FILE* file)
{
    ...
    if (err != 0) {
        LOG_L(L_ERROR,
              "FileConfigSource: Error: Failed truncating config file.");
    }
...
DefaultConfigSource::DefaultConfigSource()
{
    ...
    for (ConfigVariable::MetaDataMap::const_iterator it = vars.begin();
         it != vars.end();
         ++it) {
