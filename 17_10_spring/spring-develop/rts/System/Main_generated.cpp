...
Run(int argc, char* argv[])
{
    ...
    if (err != NULL) {
        ErrorMessageBox(
          " error: " + err->message, err->caption, err->flags, true);
    }
...
SetNvOptimusProfile(const std::string& processFileName)
{
    ...
    const bool profileChanged =
      (SOP_SetProfile("Spring", processFileName) == SOP_RESULT_CHANGE);
...
// HEADLESS run mostly in parallel for testing purposes, 100% omp threads
// wouldn't help then
    ...
    if (SetNvOptimusProfile(FileSystem::GetFilename(argv[0]))) {
        ...
        // ExecProc normally does not return; if it does the retval is an
        // error-string
        ErrorMessageBox(Platform::ExecuteProcess(argv[0], args),
                        "Execv error:",
                        MBF_OK | MBF_EXCL);
    }
#endif
