    ...
    if (!taskGroups.empty()) {
        if (lk.try_lock()) {
            ...
            if (foundEmpty) {
                ...
                boost::unique_lock<boost::shared_mutex> ulk(taskMutex,
                                                            boost::defer_lock);
                while (!ulk.try_lock()) {
                }
...
WorkerLoop(int id)
{
    ...
    while (!exitThread) {
        const auto spinlockStart = boost::chrono::high_resolution_clock::now() +
                                   boost::chrono::milliseconds(spinlockMs);
        ...
        while (!DoTask(lk) && !exitThread) {
            if (spinlockStart < boost::chrono::high_resolution_clock::now()) {
                ...
                const boost::system_time timeout =
                  boost::get_system_time() + boost::posix_time::microseconds(1);
...
SetThreadCount(int num)
{
    ...
    LOG("[ThreadPool::%s][1] #wanted=%d #current=%d #max=%d",
        __FUNCTION__,
        num,
        curThreads,
    ...
    if (curThreads < num) {
            ...
            try {
                for (int i = curThreads; i < num; ++i) {
                    thread_group.push_back(
                      new COffscreenGLThread(boost::bind(&WorkerLoop, i)));
                }
        ...
        if (!hasOGLthreads) {
            for (int i = curThreads; i < num; ++i) {
                thread_group.push_back(
                  new boost::thread(boost::bind(&WorkerLoop, i)));
            }
        }
    } else {
        for (int i = curThreads; i > num && i > 1; --i) {
            ...
            auto taskgroup = std::make_shared<
              ParallelTaskGroup<const std::function<void()>>>();
            taskgroup->enqueue_unique(GetNumThreads() - 1,
                                      [] { exitThread = true; });
            ThreadPool::PushTaskGroup(taskgroup);
#ifndef UNITSYNC
            if (hasOGLthreads) {
                auto th =
                  reinterpret_cast<COffscreenGLThread*>(thread_group.back());
                th->join();
                delete th;
            } else
#endif
