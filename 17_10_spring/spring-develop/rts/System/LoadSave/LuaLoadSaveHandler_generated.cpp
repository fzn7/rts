...
CLuaLoadSaveHandler::SaveGame(const std::string& file)
{
    const std::string realname =
      dataDirsAccess.LocateFile(file, FileQueryFlags::WRITE).c_str();
    ...
    try {
        ...
        // Open the zip
        if (realname.empty() ||
            (savefile = zipOpen(realname.c_str(), APPEND_STATUS_CREATE)) ==
              NULL) {
        ...
        if (Z_OK !=
            zipClose(
              savefile,
              "Spring save file, visit https://springrts.com/ for details.")) {
            LOG_L(
              L_ERROR, "Unable to close save file \"%s\"", filename.c_str());
        }
        return; // Success
...
CLuaLoadSaveHandler::SaveGameStartInfo()
{
    ...
    SaveEntireFile(
      FILE_STARTSCRIPT, "game setup", scriptText.data(), scriptText.size());
}
...
CLuaLoadSaveHandler::SaveHeightmap()
{
    ...
    const int* currHeightmap =
      (const int*)(const char*)readMap->GetCornerHeightMapSynced();
    const int* origHeightmap =
      (const int*)(const char*)readMap->GetOriginalHeightMapSynced();
                                    ...
                                    bool throwOnError)
{
    ...
    if (Z_OK != zipOpenNewFileInZip(savefile,
                                    file,
                                    NULL,
                                    NULL,
                                    0,
...
{
    // FIXME: need some way to 'chroot' them into a single directory?
    // absolute-ify & check path-prefix?
...
CLuaLoadSaveHandler::LoadHeightmap()
{
    ...
    if (loadfile->GetFile(FILE_HEIGHTMAP, buf)) {
        ...
        LOG_L(L_ERROR,
              "Unable to load heightmap from save file \"%s\"",
              filename.c_str());
    }
}
