            ...
            bool forced)
{
    ...
    if (forced) {
        LOG_L(L_ERROR,
              "[%s] failed to shutdown normally, exit forced",
              __FUNCTION__);
    }
                ...
                bool fromMain)
{
...
#else
    LOG_L(L_ERROR,
          "[%s][1] msg=\"%s\" IsMainThread()=%d fromMain=%d",
          __FUNCTION__,
          msg.c_str(),
          Threading::IsMainThread(),
          fromMain);

    // SpringApp::Shutdown is extremely likely to deadlock or end up waiting
    // indefinitely if any MT thread has crashed or deviated from its normal
    // execution path by throwing an exception
    boost::thread* forcedExitThread =
      new boost::thread(boost::bind(&ForcedExit, msg, caption, flags));
    ...
    if (!Threading::IsMainThread()) {
        ...
        // terminate thread
        // FIXME: only the (separate) loading thread can catch
        // thread_interrupted
