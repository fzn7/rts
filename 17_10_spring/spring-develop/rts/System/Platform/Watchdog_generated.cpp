...
static inline void
UpdateActiveThreads(Threading::NativeThreadId num)
{
    ...
    for (unsigned int i = 0; i < WDT_COUNT; ++i) {
        ...
        if (threadInfo->numreg != 0 &&
            Threading::NativeThreadIdsEqual(num, threadInfo->threadid)) {
    ...
    while (!hangDetectorThreadInterrupted) {
        ...
        for (unsigned int i = 0; i < WDT_COUNT; ++i) {
            ...
            if (spring_istime(curwdt) && (curtime - curwdt) > hangTimeout) {
                if (!hangDetected) {
                    LOG_L(L_WARNING,
                          "[Watchdog] Hang detection triggered for Spring %s.",
...
#ifdef WIN32
                CrashHandler::Stacktrace(registeredThreads[i]->thread,
                                         threadNames[i],
                                         LOG_LEVEL_WARNING);
#else
                CrashHandler::SuspendedStacktrace(
                  registeredThreads[i]->ctls.get(),
                  std::string(threadNames[i]));
#endif
            }
...
RegisterThread(WatchdogThreadnum num, bool primary)
{
    ...
    if (num >= WDT_COUNT || registeredThreads[num]->numreg != 0) {
        LOG_L(L_ERROR,
              "[Watchdog::%s] Invalid thread number %u",
              __FUNCTION__,
              num);
        return;
    }
    ...
    for (i = 0; i < WDT_COUNT; ++i) {
        ...
        else if (Threading::NativeThreadIdsEqual(threadInfo->threadid,
                                                 threadId))
            break;
    }
...
DeregisterThread(WatchdogThreadnum num)
{
    ...
    if (num >= WDT_COUNT || registeredThreads[num] == nullptr ||
    ...
    if (num == WDT_MAIN || !Threading::IsMainThread()) {
        LOG("[WatchDog] deregistering controls for thread [%s]",
            threadNames[num]);
...
ClearTimer(bool disable, Threading::NativeThreadId* _threadId)
{
    ...
    for (; num < WDT_COUNT; ++num) {
        if (Threading::NativeThreadIdsEqual(registeredThreads[num]->threadid,
                                            threadId))
            break;
    }
    ...
    if (num >= WDT_COUNT ||
        (threadInfo = registeredThreads[num])->numreg == 0) {
        LOG_L(L_ERROR,
              "[Watchdog::%s(id)] Invalid thread %d (_threadId=%p)",
              __FUNCTION__,
              num,
              _threadId);
        return;
...
ClearTimer(WatchdogThreadnum num, bool disable)
{
    ...
    if (num >= WDT_COUNT ||
        (threadInfo = registeredThreads[num])->numreg == 0) {
        LOG_L(
          L_ERROR, "[Watchdog::%s(num)] Invalid thread %d", __FUNCTION__, num);
        return;
    }
...
ClearTimer(const std::string& name, bool disable)
{
    ...
    if (i == threadNameToNum.end() || (num = i->second) >= WDT_COUNT ||
        (threadInfo = registeredThreads[num])->numreg == 0) {
        LOG_L(L_ERROR,
              "[Watchdog::%s(name)] Invalid thread name \"%s\"",
              __FUNCTION__,
              name.c_str());
        return;
    }
...
Install()
{
    ...
    LOG("[WatchDog%s] Installed (HangTimeout: %isec)",
        __FUNCTION__,
        hangTimeoutSecs);
}
...
{
    LOG_L(L_INFO,
          "[WatchDog::%s][1] hangDetectorThread=%p",
          __FUNCTION__,
          hangDetectorThread);
