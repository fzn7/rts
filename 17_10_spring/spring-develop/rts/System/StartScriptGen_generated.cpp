...
CreateMinimalSetup(const std::string& game, const std::string& map)
{
    ...
    modopts->AddPair("MinimalSetup",
                     1); // use for ingame detecting this type of start
                   ...
                   const std::string& playername)
{
    ...
    const bool isSkirmishAITestScript =
      CAIScriptHandler::Instance().IsSkirmishAITestScript(ai);
    if (isSkirmishAITestScript) {
        SkirmishAIData aiData =
          CAIScriptHandler::Instance().GetSkirmishAIData(ai);
