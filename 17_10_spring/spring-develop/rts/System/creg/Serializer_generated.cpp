...
struct PackageHeader
{
    ...
    int objTableOffset;
    int numObjects;
    int objClassRefOffset; // a class ref is: zero-term class string + checksum
                           // DWORD
    int numObjClassRefs;
...
WriteVarSizeUInt(std::ostream* stream, unsigned int val)
{
    if (val < 0x80) {
        ...
        throw "Cannot save varible-size int";
}
                                       ...
                                       bool isEmbedded)
{
    ...
    for (std::vector<ObjectRef*>::iterator i = refs->begin(); i != refs->end();
         ++i) {
...
COutputStreamSerializer::SerializeObject(Class* c, void* ptr, ObjectRef* objr)
{
    ...
    for (uint a = 0; a < c->members.size(); a++) {
        ...
        LOG_SL(LOG_SECTION_CREG_SERIALIZER,
               L_DEBUG,
               "Serialized %s::%s type:%s",
               c->name.c_str(),
               m->name,
...
COutputStreamSerializer::SerializeObjectInstance(void* inst,
                                                 creg::Class* objClass)
{
    // register the object, and mark it as embedded if a pointer was already
    // referencing it
    ...
    if (!obj) {
        ...
        std::vector<ObjectRef*>::iterator it =
          std::find(pendingObjects.begin(), pendingObjects.end(), obj);
...
COutputStreamSerializer::SerializeInt(void* data, int byteSize)
{
    ...
    switch (byteSize) {
        ...
        case 2: {
            x = *(boost::int16_t*)data;
            break;
        }
        case 4: {
            x = *(boost::int32_t*)data;
            break;
        }
        case 8: {
            x = *(boost::int64_t*)data;
            break;
        }
        default: {
...
void
COutputStreamSerializer::SavePackage(std::ostream* s,
                                     void* rootObj,
                                     Class* rootObjClass)
{
    PackageHeader ph;
    ...
    while (!pendingObjects.empty()) {
        ...
        for (std::vector<ObjectRef*>::const_iterator i = po.begin();
             i != po.end();
             ++i) {
            ObjectRef* obj = *i;
            SerializeObject(obj->class_, obj->ptr, obj);
            // LOG_SL(LOG_SECTION_CREG_SERIALIZER, L_DEBUG, "Serialized %s
            // size:%i", obj->class_->name.c_str(), sz);
        }
    }
    ...
    for (ObjectRef& oRef : objects) {
        ...
        std::map<creg::Class*, ClassRef>::iterator cr =
          classMap.find(oRef.class_);
    ...
    for (ObjectRef& oRef : objects) {
        ...
        for (j = oRef.memberGroups.begin(); j != oRef.memberGroups.end(); ++j) {
            std::map<creg::Class*, ClassRef>::iterator cr =
              classMap.find(j->membersClass);
            if (cr == classMap.end())
                throw "Cannot find member class ref";
            ...
            for (k = j->members.begin(); k != j->members.end(); ++k, ++midx) {
                if ((k->memberId != midx) &&
                    (!hasSerializerMember || k != (j->members.end() - 1))) {
...
CInputStreamSerializer::SerializeObject(Class* c, void* ptr)
{
    ...
    for (uint a = 0; a < c->members.size(); a++) {
        ...
        LOG_SL(LOG_SECTION_CREG_SERIALIZER,
               L_DEBUG,
               "Deserialized %s::%s type:%s size:%u",
               c->name.c_str(),
               m->name,
               m->type->GetName().c_str(),
               unsigned(stream->tellg()) - oldPos);
    }
...
CInputStreamSerializer::SerializeInt(void* data, int byteSize)
{
    ...
    switch (byteSize) {
        ...
        case 2: {
            *(boost::int16_t*)data = x;
            break;
        }
        case 4: {
            *(boost::int32_t*)data = x;
            break;
        }
        case 8: {
            *(boost::int64_t*)data = x;
            break;
        }
        default: {
            throw "Unknown int type";
                                    ...
                                    creg::Class*& rootCls)
{
    ...
    for (int a = 0; a < ph.numObjClassRefs; a++) {
        ...
        if (!class_)
            throw std::runtime_error(
              "Package file contains reference to unknown class " + className);
        classRefs[a] = class_;
    }
    ...
    if (checksum != ph.metadataChecksum)
        throw std::runtime_error("Metadata checksum error: Package file was "
                                 "saved with a different version");
