...
void
ISound::Initialize(bool forceNullSound)
{
    if (singleton == NULL) {
#ifndef NO_SOUND
...
{
    if (IsNullAudio()) {
        // FIXME: on reload, sound-ids change (depends on order when they are
        // requested, see GetSoundId()/GetSoundItem()
        LOG_L(L_ERROR,
              "re-enabling sound isn't supported yet, expect problems!");
    }
    Shutdown();
