...
void
AudioChannel::SetVolume(float newVolume)
{
    ...
    for (std::map<CSoundSource*, bool>::iterator it = cur_sources.begin();
         it != cur_sources.end();
         ++it) {
                                ...
                                bool relative)
{
    ...
    if (cur_sources.size() >= maxConcurrentSources) {
        ...
        if (src && prio <= sndItem->GetPriority()) {
            ...
            LOG_L(L_DEBUG,
                  "CSound::PlaySample: Max concurrent sounds in channel "
                  "reached! Dropping playback!");
            return;
        }
    }
    ...
    if (!sndSource ||
        (sndSource->GetCurrentPriority() >= sndItem->GetPriority())) {
        LOG_L(L_DEBUG,
              "CSound::PlaySample: Max sounds reached! Dropping playback!");
        return;
    }
                         ...
                         bool enqueue)
{
    ...
    if (!curStreamSrc)
        curStreamSrc =
          sound->GetNextBestSource(); //! may return 0 if no sources available

    if (curStreamSrc) {
        cur_sources[curStreamSrc] = true; //! This one first, PlayStream may
                                          //! invoke Stop immediately thus
                                          //! setting curStreamSrc to NULL
