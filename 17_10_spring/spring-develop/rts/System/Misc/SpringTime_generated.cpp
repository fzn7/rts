...
GetTicksNative()
{
    ...
    if (highResMode) {
        ...
        if (tickFreq.QuadPart >= boost::int64_t(1e9))
            return (FromNanoSecs<boost::uint64_t>(
              std::max(0.0, currTick.QuadPart / (tickFreq.QuadPart * 1e-9))));
        if (tickFreq.QuadPart >= boost::int64_t(1e6))
            return (FromMicroSecs<boost::uint64_t>(
              std::max(0.0, currTick.QuadPart / (tickFreq.QuadPart * 1e-6))));
        if (tickFreq.QuadPart >= boost::int64_t(1e3))
            return (FromMilliSecs<boost::uint64_t>(
              std::max(0.0, currTick.QuadPart / (tickFreq.QuadPart * 1e-3))));
...
thread_yield()
{
    ...
    if (t1 >= t0) {
        ...
        avgThreadYieldTimeMilliSecs =
          mix(avgThreadYieldTimeMilliSecs, dt.toMilliSecsf(), 0.1f);
    }
}
...
spring_time::sleep()
{
    // for very short time intervals use a yielding loop (yield is ~5x more
    // accurate than sleep(), check the UnitTest)
    if (toMilliSecsf() <
        (avgThreadSleepTimeMilliSecs + avgThreadYieldTimeMilliSecs * 5.0f)) {
    ...
    boost::this_thread::sleep(
      boost::posix_time::microseconds(std::ceil(toNanoSecsf() * 1e-3)));
#endif
...
#if defined(SPRINGTIME_USING_STD_SLEEP)
    auto tp =
      chrono::time_point<chrono::high_resolution_clock, chrono::nanoseconds>(
        chrono::nanoseconds(toNanoSecsi()));
