...
void
CSyncDebugger::Initialize(bool useBacktrace, unsigned numPlayers)
{
    delete[] history;
    history = NULL;
    delete[] historybt;
    ...
    if (useBacktrace) {
        ...
        memset(historybt,
               0,
               HISTORY_SIZE * BLOCK_SIZE * sizeof(HistItemWithBacktrace));
    } else
#endif
    {
...
void
CSyncDebugger::Sync(const void* p, unsigned size, const char* op)
{
...
#ifdef HAVE_BACKTRACE
    if (historybt) {
        // HACK to skip the uppermost 2 or 3 (32 resp. 64 bit) frames without
        // memcpy'ing the whole backtrace
        const int frameskip = (8 + sizeof(void*)) / sizeof(void*);
        historybt[historyIndex].bt_size =
          backtrace(historybt[historyIndex].bt - frameskip,
                    MAX_STACK + frameskip) -
          frameskip;
    ...
    if (size == 4) {
        ...
        // > XOR seems dangerous in that every bit is independent of any other,
        // this is bad. This isn't the case here, however, because typically we
        // checksum only 1-8 bytes of data at a time, so most of it fits in the
        // checksum anyway. (see SyncedPrimitiveBase / SyncedPrimitive, the main
...
void
CSyncDebugger::Backtrace(int index, const char* prefix) const
{
    if (historybt) {
...
unsigned
CSyncDebugger::GetBacktraceChecksum(int index) const
{
    return HsiehHash((char*)historybt[index].bt,
                     sizeof(void*) * historybt[index].bt_size,
                     0xf00dcafe);
}

bool
CSyncDebugger::ServerReceived(const unsigned char* inbuf)
{
    bool syncDebugPacket = false;
    switch (inbuf[0]) {
        case NETMSG_SD_CHKRESPONSE:
            if (*(short*)&inbuf[1] != HISTORY_SIZE * sizeof(unsigned) + 12) {
                logger.AddLine("Server: received checksum response of %d "
                               "instead of %d bytes",
                               *(short*)&inbuf[1],
                               HISTORY_SIZE * 4 + 12);
            } else {
                ...
                if (!playerHandler->IsValidPlayer(player)) {
                    logger.AddLine(
                      "Server: got invalid playernum %d in checksum response",
                      player);
                } else {
                    logger.AddLine("Server: got checksum response from %d",
                                   player);
                    ...
                    std::copy(
                      begin, end, players[player].checksumResponses.begin());
                    ...
                    while (i < playerHandler->ActivePlayers() &&
                           (!players[i].checksumResponses.empty() ||
                            !playerHandler->Player(i)->active))
                        ++i;
                    if (i == playerHandler->ActivePlayers()) {
                        ServerQueueBlockRequests();
                        logger.AddLine("Server: checksum responses received; "
                                       "%d block requests queued",
        ...
        case NETMSG_SD_BLKRESPONSE:
            if (*(short*)&inbuf[1] != BLOCK_SIZE * sizeof(unsigned) + 4) {
                logger.AddLine(
                  "Server: received block response of %d instead of %d bytes",
                  *(short*)&inbuf[1],
                  BLOCK_SIZE * 4 + 4);
            } else {
                int player = inbuf[3];
                if (!playerHandler->IsValidPlayer(player)) {
                    logger.AddLine(
                      "Server: got invalid playernum %d in block response",
                      player);
                } else {
                    ...
                    std::copy(
                      begin, end, players[player].remoteHistory.begin() + size);
                    int i = 0;
                    size += BLOCK_SIZE;
                    while (i < playerHandler->ActivePlayers() &&
...
bool
CSyncDebugger::ClientReceived(const unsigned char* inbuf)
{
    bool syncDebugPacket = false;
    switch (inbuf[0]) {
        case NETMSG_SD_CHKREQUEST:
            if (gs->frameNum != *(int*)&inbuf[1]) {
                logger.AddLine("Client: received checksum request for frame %d "
                               "instead of %d",
                               *(int*)&inbuf[1],
                               gs->frameNum);
            } else {
                disable_history =
                  true; // no more additions to the history until we're done
                may_enable_history = false;
        ...
        case NETMSG_SD_BLKREQUEST:
            if (*(unsigned short*)&inbuf[1] >= HISTORY_SIZE) {
                logger.AddLine(
                  "Client: invalid block number %d in block request",
                  *(unsigned short*)&inbuf[1]);
            } else {
                ClientSendBlockResponse(*(unsigned short*)&inbuf[1]);
                logger.AddLine("Client: block response sent for block %d",
                               *(unsigned short*)&inbuf[1]);
                // simple progress indication
                LOG("Client: %d / %d",
                    *(unsigned short*)&inbuf[3],
                    *(unsigned short*)&inbuf[5]);
            }
            syncDebugPacket = true;
            break;
...
void
CSyncDebugger::ServerTriggerSyncErrorHandling(int serverframenum)
{
    if (!disable_history) {
        // this will set disable_history = true once received so only one sync
        // errors is handled at a time.
    }
}

void
CSyncDebugger::ClientSendChecksumResponse()
{
    ...
    for (unsigned i = 0; i < HISTORY_SIZE; ++i) {
        ...
        for (unsigned j = 0; j < BLOCK_SIZE; ++j) {
            if (historybt) {
                checksum = HsiehHash((char*)&historybt[BLOCK_SIZE * i + j].data,
                                     sizeof(historybt[0].data),
                                     checksum);
            } else {
                checksum = HsiehHash((char*)&history[BLOCK_SIZE * i + j].data,
                                     sizeof(history[0].data),
                                     checksum);
            }
        }
        checksums.push_back(checksum);
    }
    clientNet->Send(CBaseNetProtocol::Get().SendSdCheckresponse(
      gu->myPlayerNum, flop, checksums));
}

void
CSyncDebugger::ServerQueueBlockRequests()
{
    ...
    unsigned i =
      ((unsigned)(correctFlop % (HISTORY_SIZE * BLOCK_SIZE)) / BLOCK_SIZE) + 1;
    for (unsigned c = 0; c < HISTORY_SIZE; ++i, ++c) {
        ...
        for (int j = 0; j < playerHandler->ActivePlayers(); ++j) {
            if (players[j].checksumResponses.empty())
                continue;
            if (correctChecksum &&
                players[j].checksumResponses[i] != correctChecksum) {
    ...
    if (!pendingBlocksToRequest.empty()) {
        logger.AddLine("Server: blocks: %u equal, %u not equal",
                       HISTORY_SIZE - pendingBlocksToRequest.size(),
                       pendingBlocksToRequest.size());
        requestedBlocks = pendingBlocksToRequest;
        // we know the first FPU bug occured in block # ii, so we send out a
        // block request for it.
        // 		serverNet->SendData<unsigned> (NETMSG_SD_BLKREQUEST,
        // ii);
    } else {
...
CSyncDebugger::ServerHandlePendingBlockRequests()
{
    if (!pendingBlocksToRequest.empty() && !waitingForBlockResponse) {
        ...
        clientNet->Send(CBaseNetProtocol::Get().SendSdBlockrequest(
          pendingBlocksToRequest.front(),
          requestedBlocks.size() - pendingBlocksToRequest.size() + 1,
...
void
CSyncDebugger::ClientSendBlockResponse(int block)
{
    std::vector<unsigned> checksums;
#ifdef TRACE_SYNC
...
#ifdef TRACE_SYNC
        if (historybt) {
            tracefile << historybt[BLOCK_SIZE * block + i].data << " "
                      << historybt[BLOCK_SIZE * block + i].data << "\n";
        } else {
            tracefile << history[BLOCK_SIZE * block + i].data << " "
                      << history[BLOCK_SIZE * block + i].data << "\n";
        }
#endif
    }
#ifdef TRACE_SYNC
...
#endif
    clientNet->Send(
      CBaseNetProtocol::Get().SendSdBlockresponse(gu->myPlayerNum, checksums));
}

void
CSyncDebugger::ServerReceivedBlockResponses()
{
...
void
CSyncDebugger::ServerDumpStack()
{
    // first calculate start iterator...
    unsigned posInHistory =
      (unsigned)(players[0].remoteFlop % (HISTORY_SIZE * BLOCK_SIZE));
    logger.AddLine("Server: position in history: %u", posInHistory);
    unsigned blockNr = posInHistory / BLOCK_SIZE;
    unsigned virtualBlockNr =
      0; // block nr in remoteHistory (which skips unchanged blocks)
    for (; virtualBlockNr < requestedBlocks.size() &&
           requestedBlocks[virtualBlockNr] != blockNr;
         ++virtualBlockNr) {
    }
    unsigned virtualPosInHistory =
      (virtualBlockNr * BLOCK_SIZE) + (posInHistory % BLOCK_SIZE) + 1;
    ...
    // then loop from virtualPosInHistory to virtualHistorySize and from 0 to
    // virtualPosInHistory.
    for (unsigned i = virtualPosInHistory, c = 0; c < virtualHistorySize;
         ++i, ++c) {
        unsigned correctChecksum = 0;
        ...
        for (int j = 0; j < playerHandler->ActivePlayers(); ++j) {
            if (!playerHandler->Player(j)->active)
                continue;
            if (correctChecksum &&
                players[j].remoteHistory[i] != correctChecksum) {
                if (historybt) {
                    ...
                    std::map<unsigned, unsigned>::iterator it =
                      checksumToIndex.find(checksum);
                    if (it == checksumToIndex.end()) {
                        ++curBacktrace;
                        checksumToIndex[checksum] = curBacktrace;
                        indexToHistPos[curBacktrace] = histPos;
                    }
                    logger.AddLine("Server: 0x%08X/%15.8e instead of "
                                   "0x%08X/%15.8e, frame %06u, backtrace %u in "
                                   "\"%s\"",
                                   players[j].remoteHistory[i],
                                   *(float*)(char*)&players[j].remoteHistory[i],
                                   correctChecksum,
                                   *(float*)(char*)&correctChecksum,
                                   historybt[histPos].frameNum,
                                   checksumToIndex[checksum],
                                   historybt[histPos].op);
                } else {
                    logger.AddLine(
                      "Server: 0x%08X/%15.8e instead of 0x%08X/%15.8e",
                      players[j].remoteHistory[i],
                      *(float*)(char*)&players[j].remoteHistory[i],
                      correctChecksum,
                      *(float*)(char*)&correctChecksum);
    ...
    if (ndif)
        logger.AddLine("Server: chks: %d equal, %d not equal",
                       virtualHistorySize - ndif,
                       ndif);
    else
        // This is impossible (internal error).
        // Server first finds there are differing blocks, then all checksums
        // equal?? Turns out this can happen if the checksum function is weak.
    ...
    if (historybt) {
        ...
        for (std::map<unsigned, unsigned>::iterator it = indexToHistPos.begin();
             it != indexToHistPos.end();
             ++it) {
