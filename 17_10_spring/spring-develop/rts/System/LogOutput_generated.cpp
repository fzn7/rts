    ...
    for (size_t n = 1; n < enabledSections.size();) {
        ...
        if (k != std::string::npos) {
            ...
            if (!sub.empty()) {
                ...
                const std::string& logSec =
                  (sepChr != std::string::npos) ? sub.substr(0, sepChr) : sub;
                const std::string& logLvl =
                  (sepChr != std::string::npos)
                    ? sub.substr(sepChr + 1, std::string::npos)
                    : "";
...
InitializeLogSections()
{
    ...
    const std::set<const char*>& registeredSections =
    ...
    for (auto si = registeredSections.begin(); si != registeredSections.end();
         ++si) {
    ...
    LOG("  or the SPRING_LOG_SECTIONS environment variable (both comma "
        "separated).");
...
CLogOutput::RotateLogFile() const
{
    ...
    const std::string logArchiveDir =
      filePath.substr(0, filePath.find_last_of("/\\") + 1) + "log" +
      FileSystem::GetNativePathSeparator();
    const std::string archivedLogFile =
      logArchiveDir + FileSystem::GetFileModificationDate(filePath) + "_" +
      fileName;
...
CLogOutput::Initialize()
{
    ...
    log_file_addLogFile(filePath.c_str(),
                        NULL,
                        LOG_LEVEL_ALL,
                        configHandler->GetInt("LogFlushLevel"));
    InitializeLogSections();
