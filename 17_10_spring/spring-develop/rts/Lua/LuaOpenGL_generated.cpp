...
LuaOpenGL::Init()
{
    ...
    canUseShaders =
      (globalRendering->haveGLSL && configHandler->GetBool("LuaShaders"));
}
...
LuaOpenGL::Free()
{
    ...
    for (auto it = occlusionQueries.begin(); it != occlusionQueries.end();
         ++it) {
...
LuaOpenGL::SetupScreenMatrices()
{
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    const int winPosY_bl = globalRendering->screenSizeY -
                           globalRendering->winSizeY -
                           globalRendering->winPosY; //! origin BOTTOMLEFT
    const float dist = screenDistance;               // eye-to-screen (meters)
    const float width = screenWidth;                 // screen width (meters)
    const float vppx = float(globalRendering->winPosX +
                             globalRendering->viewPosX); // view pixel pos x
    const float vppy =
      float(winPosY_bl + globalRendering->viewPosY);      // view pixel pos y
    ...
    const float spsx =
      float(globalRendering->screenSizeX); // screen pixel size x
    const float spsy =
      float(globalRendering->screenSizeY); // screen pixel size x
...
LuaOpenGL::DrawMiniMap(lua_State* L)
{
    ...
    if (drawMode != DRAW_SCREEN) {
        luaL_error(
          L, "gl.DrawMiniMap() can only be used within DrawScreenItems()");
    }
...
GLObjectDrawWithLuaMat(lua_State* L, CSolidObject* obj, LuaObjType objType)
{
    ...
    if (!lua_isnumber(L, 3)) {
        ...
        lmd->UpdateCurrentLOD(objType,
                              camera->ProjectedDistance(obj->pos),
    ...
    if (lua_toint(L, 3) >= 0) {
        lmd->SetCurrentLOD(std::min(
          lmd->GetLODCount() - 1, static_cast<unsigned int>(lua_toint(L, 3))));
        return true;
    }
...
GLObjectShape(lua_State* L, const SolidObjectDef* def)
{
    ...
    if (luaL_optboolean(L, 5, true)) {
        CUnitDrawer::DrawIndividualDefOpaque(
          def, luaL_checkint(L, 2), rawState, toScreen);
    } else {
...
LuaOpenGL::UnitCommon(lua_State* L, bool applyTransform, bool callDrawUnit)
{
    ...
    typedef void (CUnitDrawer::*RawDrawMemFunc)(
      const CUnit*, unsigned int, unsigned int, bool, bool);
    typedef void (CUnitDrawer::*MatDrawMemFunc)(const CUnit*, bool);

    const RawDrawMemFunc rawDrawFuncs[2] = { &CUnitDrawer::DrawUnitNoTrans,
                                             &CUnitDrawer::DrawUnit };
    const MatDrawMemFunc matDrawFuncs[2] = {
        &CUnitDrawer::DrawIndividualNoTrans, &CUnitDrawer::DrawIndividual
    };
    ...
    if (doRawDraw) {
        ...
        (unitDrawer->*rawDrawFuncs[applyTransform])(
          unit, 0, 0, fullModel, noLuaCall);
    } else {
...
{
    return (UnitCommon(L, true, true));
}
int
LuaOpenGL::UnitRaw(lua_State* L)
{
    return (UnitCommon(L, false, false));
}
...
{
    CheckDrawingEnabled(L, __FUNCTION__);
    GLObjectShapeTextures(L,
                          unitDefHandler->GetUnitDefByID(luaL_checkint(L, 1)));
    return 0;
}
                         ...
                         bool callDrawFeature)
{
    ...
    typedef void (CFeatureDrawer::*RawDrawMemFunc)(
      const CFeature*, unsigned int, unsigned int, bool, bool);
    typedef void (CFeatureDrawer::*MatDrawMemFunc)(const CFeature*, bool);

    const RawDrawMemFunc rawDrawFuncs[2] = {
        &CFeatureDrawer::DrawFeatureNoTrans, &CFeatureDrawer::DrawFeature
    };
    const MatDrawMemFunc matDrawFuncs[2] = {
        &CFeatureDrawer::DrawIndividualNoTrans, &CFeatureDrawer::DrawIndividual
    };
    ...
    if (doRawDraw) {
        // draw with void material state
        (featureDrawer->*rawDrawFuncs[applyTransform])(
          feature, 0, 0, false, noLuaCall);
    } else {
        // draw with full material state
...
{
    return (FeatureCommon(L, true, true));
}
int
LuaOpenGL::FeatureRaw(lua_State* L)
{
    return (FeatureCommon(L, false, false));
}
...
{
    CheckDrawingEnabled(L, __FUNCTION__);
    GLObjectShapeTextures(
      L, featureDefHandler->GetFeatureDefByID(luaL_checkint(L, 1)));
    return 0;
}
...
{
    return (FeaturePieceMultMatrix(L));
}
int
...
LuaOpenGL::DrawGroundQuad(lua_State* L)
{
    ...
    */
    ...
    const int xis =
      std::max(0, std::min(mapxi, int((xs + 0.5f) / SQUARE_SIZE)));
    const int xie =
      std::max(0, std::min(mapxi, int((xe + 0.5f) / SQUARE_SIZE)));
    const int zis =
      std::max(0, std::min(mapzi, int((zs + 0.5f) / SQUARE_SIZE)));
    const int zie =
      std::max(0, std::min(mapzi, int((ze + 0.5f) / SQUARE_SIZE)));
    if ((xis >= xie) || (zis >= zie)) {
        return 0;
    }
...
static bool
ParseVertexData(lua_State* L, VertexData& vd)
{
...
LuaOpenGL::Shape(lua_State* L)
{
    ...
    const int table = 2;
    int i = 1;
    for (lua_rawgeti(L, table, i); lua_istable(L, -1);
...
LuaOpenGL::BeginEnd(lua_State* L)
{
    ...
    if (error != 0) {
        LOG_L(
          L_ERROR, "gl.BeginEnd: error(%i) = %s", error, lua_tostring(L, -1));
        lua_error(L);
    }
    return 0;
}
...
LuaOpenGL::Material(lua_State* L)
{
    ...
    for (lua_pushnil(L); lua_next(L, table) != 0; lua_pop(L, 1)) {
        ...
        if (key == "ambidiff") {
            ...
            LOG_L(
              L_WARNING, "gl.Material: unknown material type: %s", key.c_str());
        }
    }
    return 0;
}
...
LuaOpenGL::Scissor(lua_State* L)
{
    ...
    if (args == 1) {
        ...
        glScissor(
          x + globalRendering->viewPosX, y + globalRendering->viewPosY, w, h);
    } else {
...
LuaOpenGL::ColorMask(lua_State* L)
{
    ...
    if (args == 1) {
        ...
        glColorMask(luaL_checkboolean(L, 1),
...
LuaOpenGL::Smoothing(lua_State* L)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (type == LUA_TBOOLEAN) {
            ...
            if ((hint == GL_FASTEST) || (hint == GL_NICEST) ||
                (hint == GL_DONT_CARE)) {
                glHint(hintEnum, hint);
                glEnable(enableEnum);
            } else {
                luaL_error(
                  L, "Bad %s hint in gl.Smoothing()", smoothTypes[i].name);
            }
        }
    }
    return 0;
...
LuaOpenGL::PolygonOffset(lua_State* L)
{
    ...
    if (args == 1) {
        ...
        glEnable(GL_POLYGON_OFFSET_LINE);
        glEnable(GL_POLYGON_OFFSET_POINT);
        glPolygonOffset((GLfloat)luaL_checkfloat(L, 1),
                        (GLfloat)luaL_checkfloat(L, 2));
    } else {
...
LuaOpenGL::PointSprite(lua_State* L)
{
    ...
    if ((args >= 3) && lua_isboolean(L, 3)) {
        if (lua_toboolean(L, 3)) {
            glTexEnvi(
              GL_POINT_SPRITE, GL_POINT_SPRITE_COORD_ORIGIN, GL_UPPER_LEFT);
        } else {
            glTexEnvi(
              GL_POINT_SPRITE, GL_POINT_SPRITE_COORD_ORIGIN, GL_LOWER_LEFT);
        }
    }
    return 0;
}
...
LuaOpenGL::Texture(lua_State* L)
{
    ...
    LuaMatTexture tex;
    const bool loaded =
      LuaOpenGLUtils::ParseTextureImage(L, tex, lua_tostring(L, nextArg));
    if (loaded) {
        tex.enable = true;
        tex.Bind();
    }
...
LuaOpenGL::ActiveTexture(lua_State* L)
{
    ...
    if ((args < 2) || !lua_isnumber(L, 1) || !lua_isfunction(L, 2)) {
        luaL_error(
          L, "Incorrect arguments to gl.ActiveTexture(number, func, ...)");
    }
...
LuaOpenGL::LoadMatrix(lua_State* L)
{
    ...
    if (luaType == LUA_TSTRING) {
        const CMatrix44f* matptr =
          LuaOpenGLUtils::GetNamedMatrix(lua_tostring(L, 1));
        if (matptr != NULL) {
...
LuaOpenGL::CreateList(lua_State* L)
{
    ...
    SMatrixStateData prevMSD =
      GetLuaContextData(L)->glMatrixTracker.PushMatrixState(true);
    const int error = lua_pcall(L, (args - 1), 0, 0);
    SMatrixStateData matData =
      GetLuaContextData(L)->glMatrixTracker.GetMatrixState();
    ...
    if (error != 0) {
        ...
        LOG_L(
          L_ERROR, "gl.CreateList: error(%i) = %s", error, lua_tostring(L, -1));
        lua_pushnumber(L, 0);
    } else {
...
LuaOpenGL::CallList(lua_State* L)
{
    ...
    const unsigned int dlist = displayLists.GetDList(listIndex);
    if (dlist) {
        SMatrixStateData matrixStateData =
          displayLists.GetMatrixState(listIndex);
        int error = GetLuaContextData(L)->glMatrixTracker.ApplyMatrixState(
          matrixStateData);
        if (error == 0) {
            glCallList(dlist);
            return 0;
        ...
        luaL_error(L,
                   "Matrix stack %sflow in gl.CallList",
                   (error > 0) ? "over" : "under");
    }
    return 0;
}
...
LuaOpenGL::SaveImage(lua_State* L)
{
    ...
    if (!LuaIO::SafeWritePath(filename) || !LuaIO::IsSimplePath(filename)) {
        LOG_L(L_WARNING,
              "gl.SaveImage: tried to write to illegal path localtion");
        return 0;
    }
    if ((width <= 0) || (height <= 0)) {
    ...
    if (!gray16b) {
        glReadPixels(
          x, y, width, height, GL_RGBA, GL_UNSIGNED_BYTE, &bitmap.mem[0]);
        if (!yflip)
            bitmap.ReverseYAxis();
        ...
        // single channel only!
        glReadPixels(
          x, y, width, height, GL_LUMINANCE, GL_FLOAT, &bitmap.mem[0]);
        if (!yflip)
            bitmap.ReverseYAxis();
...
LuaOpenGL::DeleteQuery(lua_State* L)
{
    ...
    const GLuint* ptr =
      reinterpret_cast<const unsigned int*>(lua_topointer(L, 1));
...
LuaOpenGL::RunQuery(lua_State* L)
{
    ...
    if (error != 0) {
        LOG_L(
          L_ERROR, "gl.RunQuery: error(%i) = %s", error, lua_tostring(L, -1));
        lua_error(L);
    }
...
LuaOpenGL::GetSun(lua_State* L)
{
    ...
    const bool unitMode =
      lua_israwstring(L, 2) && (strcmp(lua_tostring(L, 2), "unit") == 0);
