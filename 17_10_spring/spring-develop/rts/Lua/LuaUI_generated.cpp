  ...
  : CLuaHandle("LuaUI", LUA_HANDLE_ORDER_UI, true, false)
{
    ...
    const bool luaLockedAccess =
      CFileHandler::FileExists("gamedata/LockLuaUI.txt", SPRING_VFS_MOD);
    const bool luaSocketEnabled = configHandler->GetBool("LuaSocketEnabled");

    const std::string mode = GetVFSMode(luaLockedAccess);
    const std::string file =
      (CFileHandler::FileExists("luaui.lua", mode) ? "luaui.lua"
                                                   : "LuaUI/main.lua");
    const std::string code = LoadFile(file, mode);

    LOG("LuaUI Entry Point: \"%s\"", file.c_str());
    LOG("LuaUI Access Lock: %s",
        (luaLockedAccess
           ? ((!CLuaHandle::GetDevMode()) ? "enabled" : "bypassed")
           : "disabled"));
...
void
CLuaUI::InitLuaSocket(lua_State* L)
{
    std::string code;
...
string
CLuaUI::LoadFile(const string& name, const std::string& mode) const
{
...
static bool
IsDisallowedCallIn(const string& name)
{
    return (name == "Explosion") || (name == "DrawUnit") ||
           (name == "DrawFeature") || (name == "DrawShield") ||
           (name == "DrawProjectile");
}

bool
CLuaUI::HasCallIn(lua_State* L, const string& name)
{
...
CLuaUI::UpdateTeams()
{
    if (luaUI) {
        ...
        luaUI->SetReadAllyTeam(luaUI->GetFullRead() ? AllAccessTeam
                                                    : gu->myAllyTeam);
        luaUI->SetSelectTeam(gu->spectatingFullSelect ? AllAccessTeam
                                                      : gu->myTeam);
    }
}
...
bool
CLuaUI::LoadCFunctions(lua_State* L)
{
    lua_newtable(L);
...
bool
CLuaUI::ConfigCommand(const string& command) // FIXME rename to fit event name
{
...
void
CLuaUI::ShockFront(const float3& pos,
                   float power,
                   float areaOfEffect,
                   const float* distMod)
{
    ...
    if (!cmdStr.GetGlobalFunc(L)) {
        haveShockFront = false; // FIXME improve in GameHelper.cpp and pipe
                                // instead through eventHandler?
...
bool
CLuaUI::LayoutButtons(int& xButtons,
                      int& yButtons,
                      ...
                      string& menuName)
{
    ...
    if (lua_israwstring(L, -1) && (string(lua_tostring(L, -1)) == "disabled")) {
        ...
        LOG_L(
          L_WARNING, "LayoutButtons() bad number of return values (%i)", args);
        lua_settop(L, top);
        return false;
    }
...
bool
CLuaUI::BuildCmdDescTable(lua_State* L, const vector<SCommandDescription>& cmds)
{
    lua_newtable(L);
...
bool
CLuaUI::GetLuaIntMap(lua_State* L, int index, map<int, int>& intMap)
{
...
bool
CLuaUI::GetLuaIntList(lua_State* L, int index, vector<int>& intList)
{
    const int table = index;
    if (!lua_istable(L, table)) {
        return false;
...
bool
CLuaUI::GetLuaReStringList(lua_State* L,
                           int index,
...
bool
CLuaUI::GetLuaReParamsList(lua_State* L,
                           int index,
                           vector<ReParamsPair>& reParamsCmds)
{
    const int table = index;
    ...
    for (lua_pushnil(L); lua_next(L, table) != 0; lua_pop(L, 1)) {
        ...
        for (lua_pushnil(L); lua_next(L, paramTable) != 0; lua_pop(L, 1)) {
            ...
            const string paramValue =
              string(lua_tostring(L, -1), lua_strlen(L, -1));
...
bool
CLuaUI::GetLuaCmdDescList(lua_State* L,
                          int index,
                          vector<SCommandDescription>& cmdDescs)
{
    ...
    for (lua_rawgeti(L, table, paramIndex); lua_istable(L, -1);
        ...
        for (lua_pushnil(L); lua_next(L, cmdDescTable) != 0; lua_pop(L, 1)) {
            if (lua_israwstring(L, -2) && lua_isstring(L, -1)) {
                const string key =
                  StringToLower(string(lua_tostring(L, -2), lua_strlen(L, -2)));
                ...
                if (key == "id") {
                    ...
                    LOG_L(L_WARNING,
                          "GetLuaCmdDescList() unknown key (%s)",
                          key.c_str());
                }
                ...
                if (key != "actions") {
                    LOG_L(L_WARNING,
                          "GetLuaCmdDescList() non \"actions\" table: %s",
                          key.c_str());
                    continue;
                }
                const int actionsTable = lua_gettop(L);
                for (lua_pushnil(L); lua_next(L, actionsTable) != 0;
                     lua_pop(L, 1)) {
                    if (lua_isstring(L, -1)) {
    ...
    if (!lua_isnil(L, -1)) {
        LOG_L(
          L_WARNING, "GetLuaCmdDescList() entry %i is not a table", paramIndex);
        lua_pop(L, 1);
        return false;
    }
    lua_pop(L, 1);
...
int
CLuaUI::SetShockFrontFactors(lua_State* L)
{
