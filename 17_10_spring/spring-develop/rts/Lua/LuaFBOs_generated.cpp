                      ...
                      GLenum attachLevel)
{
    if (lua_isnil(L, index)) {
        ...
        glFramebufferRenderbufferEXT(
          fbo->target, attachID, GL_RENDERBUFFER_EXT, 0);
        return true;
        ...
        glFramebufferTexture2DEXT(
          fbo->target, attachID, attachTarget, tex->id, attachLevel);
        fbo->xsize = tex->xsize;
        fbo->ysize = tex->ysize;
        return true;
    } else {
        ...
        glFramebufferRenderbufferEXT(
          fbo->target, attachID, attachTarget, rbo->id);
        fbo->xsize = rbo->xsize;
        fbo->ysize = rbo->ysize;
        return true;
    }
...
LuaFBOs::ActiveFBO(lua_State* L)
{
    ...
    if (error != 0) {
        LOG_L(
          L_ERROR, "gl.ActiveFBO: error(%i) = %s", error, lua_tostring(L, -1));
        lua_error(L);
    }
...
LuaFBOs::BlitFBO(lua_State* L)
{
    if (lua_israwnumber(L, 1)) {
        ...
        const GLbitfield mask =
          (GLbitfield)luaL_optint(L, 9, GL_COLOR_BUFFER_BIT);
        const GLenum filter = (GLenum)luaL_optint(L, 10, GL_NEAREST);

        glBlitFramebufferEXT(
          x0Src, y0Src, x1Src, y1Src, x0Dst, y0Dst, x1Dst, y1Dst, mask, filter);
    } else {
        ...
        const GLbitfield mask =
          (GLbitfield)luaL_optint(L, 11, GL_COLOR_BUFFER_BIT);
