...
LuaVFS::Include(lua_State* L, bool synced)
{
    ...
    if (!LoadFileWithModes(filename, code, modes)) {
        char buf[1024];
        SNPRINTF(
          buf, sizeof(buf), "Include() could not load '%s'", filename.c_str());
...
LuaVFS::MapArchive(lua_State* L)
{
    ...
    const std::string filename =
      archiveScanner->ArchiveFromName(luaL_checkstring(L, 1));
    ...
    if (args >= 2) {
        ...
        // lua numbers are float and so limited to 2^24, while the checksum is
        // an int32. const unsigned int argChecksum = lua_tonumber(L, 2);
        const unsigned int argChecksum = StringToInt(lua_tostring(L, 2));
        const unsigned int realChecksum =
          archiveScanner->GetSingleArchiveChecksum(filename);
...
LuaVFS::UnmapArchive(lua_State* L)
{
    ...
    if (!vfsHandler->RemoveArchive(filename)) {
        std::ostringstream buf;
        buf << "[" << __FUNCTION__
            << "] failed to remove archive: " << filename;
...
LuaVFS::CompressFolder(lua_State* L)
{
    ...
    const string compressedFilePath =
      luaL_optstring(L, 3, (folderPath + ".sdz").c_str());
    ...
    if (archiveType == "zip") {
        LuaZipFolder::ZipFolder(
          L, folderPath, compressedFilePath, includeFolder, modes);
...
LuaVFS::ZlibCompress(lua_State* L)
{
    ...
    const int error =
      compress(&compressed[0], &bufsize, (const boost::uint8_t*)inData, inLen);
    if (error == Z_OK) {
...
LuaVFS::ZlibDecompress(lua_State* L)
{
    ...
    const int error = uncompress(
      &uncompressed[0], &bufsize, (const boost::uint8_t*)inData, inLen);
    if (error == Z_OK) {
...
PackType(lua_State* L)
{
    ...
    if (lua_istable(L, 1)) {
        for (int i = 1; lua_rawgeti(L, 1, i), lua_isnumber(L, -1);
             lua_pop(L, 1), i++) {
...
{
    return PackType<boost::uint8_t>(L);
}
int
LuaVFS::PackU16(lua_State* L)
{
    return PackType<boost::uint16_t>(L);
}
int
LuaVFS::PackU32(lua_State* L)
{
    return PackType<boost::uint32_t>(L);
}
int
LuaVFS::PackS8(lua_State* L)
{
    return PackType<boost::int8_t>(L);
}
int
LuaVFS::PackS16(lua_State* L)
{
    return PackType<boost::int16_t>(L);
}
int
LuaVFS::PackS32(lua_State* L)
{
    return PackType<boost::int32_t>(L);
}
int
LuaVFS::PackF32(lua_State* L)
{
    return PackType<float>(L);
}
...
{
    return UnpackType<boost::uint8_t>(L);
}
int
LuaVFS::UnpackU16(lua_State* L)
{
    return UnpackType<boost::uint16_t>(L);
}
int
LuaVFS::UnpackU32(lua_State* L)
{
    return UnpackType<boost::uint32_t>(L);
}
int
LuaVFS::UnpackS8(lua_State* L)
{
    return UnpackType<boost::int8_t>(L);
}
int
LuaVFS::UnpackS16(lua_State* L)
{
    return UnpackType<boost::int16_t>(L);
}
int
LuaVFS::UnpackS32(lua_State* L)
{
    return UnpackType<boost::int32_t>(L);
}
int
LuaVFS::UnpackF32(lua_State* L)
{
    return UnpackType<float>(L);
}
