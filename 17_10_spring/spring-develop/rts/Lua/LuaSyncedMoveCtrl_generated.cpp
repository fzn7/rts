...
LuaSyncedMoveCtrl::SetProgressState(lua_State* L)
{
    ...
    if (lua_isnumber(L, 2)) {
        ...
        if (state == "done") {
            ...
            luaL_error(
              L, "SetProgressState(): bad state value (%s)", state.c_str());
        }
    } else {
        luaL_error(L, "Incorrect arguments to SetProgressState()");
    }
...
LuaSyncedMoveCtrl::SetPhysics(lua_State* L)
{
    ...
    const float3 rot(
      luaL_checkfloat(L, 8), luaL_checkfloat(L, 9), luaL_checkfloat(L, 10));
    ASSERT_SYNCED(pos);
    ASSERT_SYNCED(vel);
    ASSERT_SYNCED(rot);
...
SetMoveTypeValue(lua_State* L, AMoveType* moveType, int keyIdx, int valIdx)
{
    if (lua_isnumber(L, valIdx))
        return (SetMoveTypeValue<float>(
          moveType, lua_tostring(L, keyIdx), lua_tofloat(L, valIdx)));

    if (lua_isboolean(L, valIdx))
        return (SetMoveTypeValue<bool>(
          moveType, lua_tostring(L, keyIdx), lua_toboolean(L, valIdx)));
...
SetMoveTypeData(lua_State* L, AMoveType* moveType, const char* caller)
{
    ...
    if (moveType == NULL) {
        luaL_error(L,
                   "[%s] unit %d has incompatible movetype for %s",
                   __FUNCTION__,
                   lua_tonumber(L, 1),
                   caller);
    ...
    switch (lua_gettop(L)) {
        case 2: {
            ...
            if (lua_istable(L, tableIdx)) {
                for (lua_pushnil(L); lua_next(L, tableIdx) != 0;
                     lua_pop(L, 1)) {
                    if (lua_israwstring(L, -2) &&
                        SetMoveTypeValue(L, moveType, -2, -1)) {
                        numAssignedValues++;
                    } else {
                        LOG_L(L_WARNING,
                              "[%s] incompatible movetype key for %s",
                              __FUNCTION__,
                              caller);
                    }
                }
...
{
    return (SetMoveTypeData(
      L,
      ParseDerivedMoveType<CHoverAirMoveType>(L, __FUNCTION__, 1),
      __FUNCTION__));
}
...
LuaSyncedMoveCtrl::SetAirMoveTypeData(lua_State* L)
{
    return (SetMoveTypeData(
      L,
      ParseDerivedMoveType<CStrafeAirMoveType>(L, __FUNCTION__, 1),
      __FUNCTION__));
}
...
LuaSyncedMoveCtrl::SetGroundMoveTypeData(lua_State* L)
{
    return (
      SetMoveTypeData(L,
                      ParseDerivedMoveType<CGroundMoveType>(L, __FUNCTION__, 1),
                      __FUNCTION__));
}
...
LuaSyncedMoveCtrl::SetMoveDef(lua_State* L)
{
    ...
    if (lua_isnumber(L, 2))
        moveDef = moveDefHandler->GetMoveDefByPathType(Clamp(
          luaL_checkint(L, 2), 0, int(moveDefHandler->GetNumMoveDefs()) - 1));
    if (lua_isstring(L, 2))
    ...
    if (moveDef->udRefCount == 0) {
        ...
        LOG_L(L_ERROR,
              "SetMoveDef: Tried to use an unreferenced (:=disabled) MoveDef!");
        lua_pushboolean(L, false);
        return 1;
    }
