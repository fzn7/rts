...
LuaConstGame::PushEntries(lua_State* L)
{
    ...
    LuaPushNamedString(
      L,
      "versionFull",
      (!CLuaHandle::GetHandleSynced(L)) ? SpringVersion::GetFull() : "");
    LuaPushNamedString(
      L,
      "versionPatchSet",
      (!CLuaHandle::GetHandleSynced(L)) ? SpringVersion::GetPatchSet() : "");
    LuaPushNamedString(
      L,
      "buildFlags",
      (!CLuaHandle::GetHandleSynced(L)) ? SpringVersion::GetAdditional() : "");
    ...
    LuaPushNamedNumber(
      L, "gravity", -mapInfo->map.gravity * GAME_SPEED * GAME_SPEED);
    ...
    LuaPushNamedString(
      L, "mapHumanName", mapInfo->map.description); //! deprecated
    ...
    LuaPushNamedNumber(
      L, "waterReflectionDistortion", mapInfo->water.reflDistortion);
    ...
    LuaPushNamedNumber(
      L, "constructionDecayTime", modInfo.constructionDecayTime);
    LuaPushNamedNumber(
      L, "constructionDecaySpeed", modInfo.constructionDecaySpeed);
    ...
    LuaPushNamedNumber(
      L, "reclaimUnitEnergyCostFactor", modInfo.reclaimUnitEnergyCostFactor);
    LuaPushNamedNumber(
      L, "reclaimUnitEfficiency", modInfo.reclaimUnitEfficiency);
    LuaPushNamedNumber(L,
                       "reclaimFeatureEnergyCostFactor",
                       modInfo.reclaimFeatureEnergyCostFactor);
    ...
    LuaPushNamedNumber(
      L, "repairEnergyCostFactor", modInfo.repairEnergyCostFactor);
    LuaPushNamedNumber(
      L, "resurrectEnergyCostFactor", modInfo.resurrectEnergyCostFactor);
    LuaPushNamedNumber(
      L, "captureEnergyCostFactor", modInfo.captureEnergyCostFactor);
    ...
    LuaPushNamedNumber(
      L, "requireSonarUnderWater", modInfo.requireSonarUnderWater);
    ...
    if (damageArrayHandler != NULL) {
        ...
        const std::vector<std::string>& typeList =
          damageArrayHandler->GetTypeList();
