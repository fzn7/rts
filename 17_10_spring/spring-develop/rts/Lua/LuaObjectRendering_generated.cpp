...
{
    return (
      static_cast<CUnit*>(ParseSolidObject(L, caller, index, LUAOBJ_UNIT)));
}

static inline CFeature*
ParseFeature(lua_State* L, const char* caller, int index)
{
    return (static_cast<CFeature*>(
      ParseSolidObject(L, caller, index, LUAOBJ_FEATURE)));
}
...
LuaObjectRenderingImpl::GetLODCount(lua_State* L)
{
    const CSolidObject* obj =
      ParseSolidObject(L, __FUNCTION__, 1, GetObjectType());
...
SetLODLengthCommon(lua_State* L, CSolidObject* obj, float scale)
{
    ...
    lmd->SetLODLength(luaL_checknumber(L, 2) - 1,
                      luaL_checkfloat(L, 3) * scale);
    return 0;
}
...
LuaObjectRenderingImpl::SetLODLength(lua_State* L)
{
    ...
    return (SetLODLengthCommon(
      L, ParseSolidObject(L, __FUNCTION__, 1, GetObjectType()), 1.0f));
}
...
LuaObjectRenderingImpl::SetLODDistance(lua_State* L)
{
    ...
    return (
      SetLODLengthCommon(L,
                         ParseSolidObject(L, __FUNCTION__, 1, GetObjectType()),
                         0.0010786811520132682f));
}
...
ParseShader(lua_State* L, int index, LuaMatShader& shader)
{
    ...
    switch (lua_type(L, index)) {
        case LUA_TNUMBER: {
            shader.SetTypeFromID(
              shaders.GetProgramName(luaL_checknumber(L, index)));
        } break;
...
LuaObjectRenderingImpl::SetMaterial(lua_State* L)
{
    ...
    LuaObjectLODMaterial* lodMat =
      objMat->GetMaterial(luaL_checknumber(L, 2) - 1);
...
{
    return (
      SetObjectLuaDraw(L, featureHandler->GetFeature(luaL_checkint(L, 1))));
}
...
LuaObjectRenderingImpl::Debug(lua_State* L)
{
    ...
    LOG_L(
      L_DEBUG, "LUAMAT_OPAQUE         lastLOD = %i", opaqueMat.GetLastLOD());
    LOG_L(
      L_DEBUG, "LUAMAT_ALPHA_REFLECT  lastLOD = %i", alphaReflMat.GetLastLOD());
    LOG_L(L_DEBUG,
          "LUAMAT_OPAQUE_REFLECT lastLOD = %i",
          opaqueReflMat.GetLastLOD());
    LOG_L(
      L_DEBUG, "LUAMAT_SHADOW         lastLOD = %i", shadowMat.GetLastLOD());
