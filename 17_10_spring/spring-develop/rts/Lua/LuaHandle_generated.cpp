...
CLuaHandle::PushTracebackFuncToRegistry(lua_State* L)
{
    ...
    // We only need the debug.traceback function, the others are unsafe for
    // syncing. Later CLuaHandle implementations decide themselves if they want
    // to reload the lib or not (LuaUI does).
...
CLuaHandle::KillLua(bool inFreeHandler)
{
    ...
    // FIXME when multithreaded lua is enabled, wait for all running events to
    // finish (possible via a mutex?)
...
CLuaHandle::XCall(lua_State* srcState, const string& funcName)
{
    ...
    if (!funcHash.GetGlobalFunc(L)) {
        LOG_L(L_WARNING,
              "Tried to call non-linked Script.%s.%s()",
              GetName().c_str(),
              funcName.c_str());
        return 0;
    }
    ...
    if (srcState == L) {
        ...
        if (retCount > 0) {
            LuaUtils::CopyData(
              srcState,
              L,
              retCount); // push the new returned arguments on caller stack
        }
                               ...
                               bool popErrorFunc)
{
    ...
    struct ScopedLuaCall
    {
      public:
          ...
          , popErrFunc(_popErrFunc)
        {
            ...
            GLMatrixStateTracker& matTracker =
              GetLuaContextData(state)->glMatrixTracker;
            if (canDraw) {
            ...
            // note1: disable GC outside of this scope to prevent sync errors
            // and similar note2: we collect garbage now in its own callin
            // "CollectGarbage" lua_gc(L, LUA_GCRESTART, 0);
            error = lua_pcall(state, nInArgs, nOutArgs, errFuncIdx);
            // only run GC inside of "SetHandleRunning(L, true) ...
        ...
        void CheckFixStack(std::string& trace)
        {
            ...
            const int outArgs =
              (lua_gettop(luaState) - (GetTop() - 1)) + nInArgs;
            ...
            if (GetError() == 0) {
                if (nOutArgs != LUA_MULTRET) {
                    if (outArgs != nOutArgs) {
                        LOG_L(
                          L_ERROR,
                          "Internal Lua error: %d return values, %d expected",
                          outArgs,
                          nOutArgs);
                        if (outArgs > nOutArgs)
                ...
                if (outArgs > dbgOutArgs) {
                    LOG_L(
                      L_ERROR,
                      "Internal Lua error: %i too many elements on the stack",
                      outArgs - dbgOutArgs);
                    lua_pop(
                      luaState,
                      outArgs -
                        dbgOutArgs); // only leave traceback str on the stack
                ...
                trace +=
                  luaL_optstring(luaState, -1, "[No traceback returned]");
    ...
    ScopedLuaCall call(
      this, L, hs, inArgs, outArgs, errFuncIndex, popErrorFunc);
                               ...
                               bool popErrFunc)
{
    ...
    const int error = RunCallInTraceback(
      L, &hs, inArgs, outArgs, errFuncIndex, traceback, popErrFunc);

    if (error != 0) {
        LOG_L(L_ERROR,
              "%s::RunCallIn: error = %i, %s, %s",
              GetName().c_str(),
              error,
              hs.GetString().c_str(),
              traceback.c_str());
...
CLuaHandle::LoadCode(lua_State* L, const string& code, const string& debug)
{
    ...
    const int loadError =
      luaL_loadbuffer(L, code.c_str(), code.size(), debug.c_str());
    bool ret = true;
    ...
    if (loadError == 0) {
        ...
        LOG_L(L_ERROR,
              "Lua LoadCode loadbuffer error = %i, %s, %s",
              loadError,
              debug.c_str(),
              lua_tostring(L, -1));
        lua_pop(L, 1);
...
CLuaHandle::HasCallIn(lua_State* L, const string& name)
{
    ...
    // FIXME should be equal to below, but somehow it isn't and doesn't work as
    // expected!?
...
CLuaHandle::UnitDestroyed(const CUnit* unit, const CUnit* attacker)
{
    ...
    // call the routine
    RunCallInTraceback(
      L, cmdStr, argCount, 0, traceBack.GetErrFuncIdx(), false);
}
...
CLuaHandle::ProjectileCreated(const CProjectile* p)
{
    ...
    const CWeaponProjectile* wp =
      p->weapon ? static_cast<const CWeaponProjectile*>(p) : NULL;
...
CLuaHandle::KeyPress(int key, bool isRepeat)
{
    ...
    // FIXME we should never had started using directly SDL consts, somaeday we
    // should weakly force lua-devs to fix their code
    ...
    lua_pushinteger(L, 0); // FIXME remove, was deprecated utf32 char (now uses
                           // TextInput for that)
...
{
    lua_lock(L_GC);
    // SCOPED_MT_TIMER("CollectGarbage"); // this func doesn't run in parallel
    // yet, cause of problems with IsHandleRunning()
    ...
    static const float maxLuaGarbageCollectTime =
      configHandler->GetFloat("MaxLuaGarbageCollectionTime");

    float maxRunTime =
      smoothstep(10, 100, luaMemFootPrintKB / 1024) * maxLuaGarbageCollectTime;
    ...
    while (spring_gettime() < endTime) {
        ...
        const int luaMemFootPrintChange =
          luaMemFootPrintNow - luaMemFootPrintKB;
    ...
    lua_gc(
      L_GC, LUA_GCSTOP, 0); // don't collect garbage outside of this function
    ...
    if (gcsteps > 1 && numLuaGarbageCollectIters > 0) {
        ...
        const float avgTimePerLoopIter =
          (finishTime - startTime).toMilliSecsf() / numLuaGarbageCollectIters;

        if (avgTimePerLoopIter > (maxLuaGarbageCollectTime * 0.150f))
            gcsteps--;
        if (avgTimePerLoopIter < (maxLuaGarbageCollectTime * 0.075f))
            gcsteps++;
    }
