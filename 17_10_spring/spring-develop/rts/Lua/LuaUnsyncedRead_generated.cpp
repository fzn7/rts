...
LuaUnsyncedRead::GetReplayLength(lua_State* L)
{
    if (gameServer && gameServer->GetDemoReader()) {
        lua_pushnumber(L,
                       gameServer->GetDemoReader()->GetFileHeader().gameTime);
        return 1;
    }
    return 0;
}
...
LuaUnsyncedRead::GetWindowGeometry(lua_State* L)
{
    ...
    const int winPosY_bl = globalRendering->screenSizeY -
                           globalRendering->winSizeY - globalRendering->winPosY;
...
{
    lua_pushboolean(
      L, (game != NULL) ? game->IsLagging(luaL_optfloat(L, 1, 500.0f)) : false);
    return 1;
}
...
LuaUnsyncedRead::IsUnitAllied(lua_State* L)
{
    ...
    if (CLuaHandle::GetHandleReadAllyTeam(L) < 0) {
        ...
        lua_pushboolean(L,
                        unit->allyteam == CLuaHandle::GetHandleReadAllyTeam(L));
    }
...
LuaUnsyncedRead::GetUnitViewPosition(lua_State* L)
{
    ...
    const float3 unitPos =
      (luaL_optboolean(L, 2, false)) ? unit->GetObjDrawMidPos() : unit->drawPos;
    const float3 errorVec = unit->GetLuaErrorVector(
...
template<class T>
class CWorldObjectQuadDrawer : public CReadMap::IQuadDrawer
{
  public:
...
int
LuaUnsyncedRead::GetVisibleUnits(lua_State* L)
{
    // arg 1 - teamID
    ...
    readMap->GridVisibility(
      nullptr, &unitQuadIter, 1e9, CQuadField::BASE_QUAD_SIZE / SQUARE_SIZE);

    // Even though we're in unsynced it's ok to use gs->tempNum since its exact
    ...
    for (auto visUnitList : unitQuadIter.GetObjectLists()) {
        for (CUnit* u : *visUnitList) {
            ...
            if (!camera->InView(u->drawMidPos,
                                testRadius + (u->GetDrawRadius() * radiusMult)))
                continue;
...
LuaUnsyncedRead::GetVisibleFeatures(lua_State* L)
{
    ...
    readMap->GridVisibility(
      nullptr, &featureQuadIter, 1e9, CQuadField::BASE_QUAD_SIZE / SQUARE_SIZE);
    ...
    for (auto visFeatureList : featureQuadIter.GetObjectLists()) {
        for (CFeature* f : *visFeatureList) {
            ...
            if (!camera->InView(f->drawMidPos,
                                testRadius + (f->GetDrawRadius() * radiusMult)))
                continue;
...
LuaUnsyncedRead::GetVisibleProjectiles(lua_State* L)
{
    ...
    readMap->GridVisibility(
      nullptr, &projQuadIter, 1e9, CQuadField::BASE_QUAD_SIZE / SQUARE_SIZE);
...
LuaUnsyncedRead::GetMapSquareTexture(lua_State* L)
{
    ...
    lua_pushboolean(L,
                    groundTextures->GetSquareLuaTexture(
                      texSquareX, texSquareY, tid, txs, tys, texMipLevel));
    return 1;
}
...
#define PACK_CAMERA_VECTOR(s, n)                                               \
    ...
    PACK_CAMERA_VECTOR(topFrustumPlane,
                       frustumPlanes[CCamera::FRUSTUM_PLANE_TOP]);
    PACK_CAMERA_VECTOR(botFrustumPlane,
                       frustumPlanes[CCamera::FRUSTUM_PLANE_BOT]);
    PACK_CAMERA_VECTOR(lftFrustumPlane,
                       frustumPlanes[CCamera::FRUSTUM_PLANE_LFT]);
    PACK_CAMERA_VECTOR(rgtFrustumPlane,
                       frustumPlanes[CCamera::FRUSTUM_PLANE_RGT]);
...
LuaUnsyncedRead::TraceScreenRay(lua_State* L)
{
    ...
    const int wy =
      globalRendering->viewSizeY - 1 - my - globalRendering->viewPosY;
    ...
    if (useMiniMap && (minimap != nullptr) && !minimap->GetMinimized()) {
        const int px =
          minimap->GetPosX() - globalRendering->viewPosX; // for left dualscreen
    ...
    const float traceDist = TraceRay::GuiTraceRay(camPos,
                                                  pxlDir,
                                                  rawRange,
                                                  nullptr,
                                                  unit,
                                                  ...
                                                  onlyCoords,
                                                  ignoreWater);
    const float planeDist = CGround::LinePlaneCol(
      camPos, pxlDir, rawRange, luaL_optnumber(L, newArgIdx, 0.0f));

    const float3 tracePos = camPos + (pxlDir * traceDist);
    const float3 planePos =
      camPos + (pxlDir * planeDist); // backup (for includeSky and onlyCoords)

    if ((traceDist < 0.0f || traceDist > badRange) && unit == nullptr &&
        feature == nullptr) {
...
LuaUnsyncedRead::GetSoundEffectParams(lua_State* L)
{
    ...
    lua_rawset(L, -3);
    for (std::map<ALuint, ALfloat>::iterator it =
           efxprops->filter_properties_f.begin();
         it != efxprops->filter_properties_f.end();
         ++it) {
        const ALuint param = it->first;
        std::map<ALuint, std::string>::iterator fit =
          alFilterParamToName.find(param);
    ...
    n = efxprops->properties_v.size() + efxprops->properties_f.size() +
        efxprops->properties_i.size();
    lua_pushliteral(L, "reverb");
    lua_createtable(L, 0, n);
    lua_rawset(L, -3);
    for (std::map<ALuint, ALfloat>::iterator it =
           efxprops->properties_f.begin();
         it != efxprops->properties_f.end();
         ++it) {
        const ALuint param = it->first;
    ...
    for (std::map<ALuint, float3>::iterator it = efxprops->properties_v.begin();
         it != efxprops->properties_v.end();
         ++it) {
        const ALuint param = it->first;
        std::map<ALuint, std::string>::iterator fit = alParamToName.find(param);
        if (fit != alParamToName.end()) {
    ...
    for (std::map<ALuint, ALint>::iterator it = efxprops->properties_i.begin();
         it != efxprops->properties_i.end();
         ++it) {
        const ALuint param = it->first;
        std::map<ALuint, std::string>::iterator fit = alParamToName.find(param);
        if (fit != alParamToName.end()) {
...
LuaUnsyncedRead::GetDefaultCommand(lua_State* L)
{
    ...
    const int defCmd =
      guihandler->GetDefaultCommand(mouse->lastx, mouse->lasty);
...
LuaUnsyncedRead::GetKeySymbol(lua_State* L)
{
    ...
    lua_pushsstring(
      L, (keyCodes != NULL) ? keyCodes->GetDefaultName(keycode) : "");
    return 2;
}
...
LuaUnsyncedRead::GetActionHotKeys(lua_State* L)
{
    ...
    const CKeyBindings::HotkeyList& hotkeys =
      keyBindings->GetHotkeys(luaL_checksstring(L, 1));
...
LuaUnsyncedRead::GetPlayerRoster(lua_State* L)
{
    ...
    if (!lua_isnone(L, 1)) {
        playerRoster.SetSortTypeByCode(
          (PlayerRoster::SortType)luaL_checkint(L, 1));
    }
    ...
    const std::vector<int>& players =
      playerRoster.GetIndices(&count, includePathingFlag);
...
LuaUnsyncedRead::GetPlayerTraffic(lua_State* L)
{
    ...
    if ((playerID != -1) &&              // all system counts can be read
        ...
        (packetID != -1) && (packetID != NETMSG_CHAT) &&
        (packetID != NETMSG_PAUSE) && (packetID != NETMSG_LUAMSG) &&
...
LuaUnsyncedRead::GetConfigParams(lua_State* L)
{
    ...
    int i = 1;
    for (ConfigVariable::MetaDataMap::const_iterator it = cfgmap.begin();
         it != cfgmap.end();
         ++it) {
...
LuaUnsyncedRead::GetDecalOwner(lua_State* L)
{
    ...
    // XXX: I know, not very fast, but you cannot dynamic_cast a void* back to a
    // CUnit*
