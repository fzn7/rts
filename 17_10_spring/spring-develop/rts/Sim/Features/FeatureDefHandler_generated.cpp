...
CFeatureDefHandler::CFeatureDefHandler(LuaParser* defsParser)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++) {
        ...
        AddFeatureDef(
          nameLowerCase, CreateFeatureDef(fdTable, nameLowerCase), false);
    }
    for (unsigned int i = 0; i < keys.size(); i++) {
        const std::string& nameMixedCase = keys[i];
        const std::string& nameLowerCase = StringToLower(nameMixedCase);
        ...
        const FeatureDef* dfd =
          GetFeatureDef(fdTable.GetString("featureDead", ""));
                                  ...
                                  bool isDefaultFeature)
{
    ...
    fd->collisionVolume.SetDefaultToPieceTree(
      fd->collisionVolume.DefaultToPieceTree() && !isDefaultFeature);
                                     ...
                                     const std::string& mixedCase)
{
    ...
    fd.reclaimTime = std::max(
      1.0f, fdTable.GetFloat("reclaimTime", (fd.metal + fd.energy) * 6.0f));
    ...
    fd.xsize =
      std::max(1 * SPRING_FOOTPRINT_SCALE,
               fdTable.GetInt("footprintX", 1) * SPRING_FOOTPRINT_SCALE);
    fd.zsize =
      std::max(1 * SPRING_FOOTPRINT_SCALE,
               fdTable.GetInt("footprintZ", 1) * SPRING_FOOTPRINT_SCALE);
