...
void
QTPFS::NodeLayer::InitStatic()
{
    NUM_SPEEDMOD_BINS =
      std::max(1u, mapInfo->pfs.qtpfs_constants.numSpeedModBins);
    MIN_SPEEDMOD_VALUE =
      std::max(0.0f, mapInfo->pfs.qtpfs_constants.minSpeedModVal);
    MAX_SPEEDMOD_VALUE =
      std::min(8.0f, mapInfo->pfs.qtpfs_constants.maxSpeedModVal);
}
...
QTPFS::NodeLayer::QueueUpdate(const SRectangle& r, const MoveDef* md)
{
    ...
    for (unsigned int hmz = r.z1; hmz < r.z2; hmz++) {
        for (unsigned int hmx = r.x1; hmx < r.x2; hmx++) {
            const unsigned int recIdx =
              (hmz - r.z1) * r.GetWidth() + (hmx - r.x1);

            const unsigned int chmx =
              Clamp(int(hmx), md->xsizeh, r.x2 - md->xsizeh - 1);
            const unsigned int chmz =
              Clamp(int(hmz), md->zsizeh, r.z2 - md->zsizeh - 1);

            layerUpdate->speedMods[recIdx] =
              CMoveMath::GetPosSpeedMod(*md, hmx, hmz);
            layerUpdate->blockBits[recIdx] =
              CMoveMath::IsBlockedNoSpeedModCheck(*md, chmx, chmz, NULL);
            // layerUpdate->blockBits[recIdx] = CMoveMath::SquareIsBlocked(*md,
            // hmx, hmz, NULL);
        }
    }
}
...
QTPFS::NodeLayer::ExecQueuedUpdate()
{
    ...
    return (Update(rectangle,
                   moveDefHandler->GetMoveDefByPathType(layerNumber),
                   speedMods,
                   blockBits));
}
#endif
...
{
    assert((luSpeedMods == NULL && luBlockBits == NULL) ||
           (luSpeedMods != NULL && luBlockBits != NULL));

    unsigned int numNewBinSquares = 0;
    unsigned int numClosedSquares = 0;

    const bool globalUpdate = ((r.x1 == 0 && r.x2 == mapDims.mapx) &&
    ...
    for (unsigned int hmz = r.z1; hmz < r.z2; hmz++) {
        for (unsigned int hmx = r.x1; hmx < r.x2; hmx++) {
            ...
            // don't tesselate map edges when footprint extends across them in
            // IsBlocked*
            const unsigned int chmx =
              Clamp(int(hmx), md->xsizeh, r.x2 - md->xsizeh - 1);
            const unsigned int chmz =
            ...
            const float minSpeedMod =
              (luSpeedMods == NULL) ? CMoveMath::GetPosSpeedMod(*md, hmx, hmz)
                                    : (*luSpeedMods)[recIdx];
            const int maxBlockBit =
              (luBlockBits == NULL)
                ? CMoveMath::IsBlockedNoSpeedModCheck(*md, chmx, chmz, NULL)
                : (*luBlockBits)[recIdx];
            // NOTE:
            //   movetype code checks ONLY the *CENTER* square of a unit's
            //   footprint to get the current speedmod affecting it, and the
            //   default pathfinder only takes the entire footprint into account
            //   for STRUCTURE-blocking tests --> do the same here because
            //   full-footprint checking for both structures AND terrain is much
            //   slower (and if not handled correctly units will get stuck
            //   everywhere)
            // NOTE:
            //   IsBlockedNoSpeedModCheck works at HALF-heightmap resolution (as
            //   does the default pathfinder for DETAILED_DISTANCE searches!),
            //   so this can generate false negatives!
            //
            // const int maxBlockBit = (luBlockBits == NULL)?
            // CMoveMath::SquareIsBlocked(*md, hmx, hmz, NULL):
            // (*luBlockBits)[recIdx];

#define NL QTPFS::NodeLayer
            const float tmpAbsSpeedMod = Clamp(
              minSpeedMod, NL::MIN_SPEEDMOD_VALUE, NL::MAX_SPEEDMOD_VALUE);
            const float newAbsSpeedMod =
              tmpAbsSpeedMod *
              ((maxBlockBit & CMoveMath::BLOCK_STRUCTURE) == 0);
            const float newRelSpeedMod =
              Clamp((newAbsSpeedMod - NL::MIN_SPEEDMOD_VALUE) /
                      (NL::MAX_SPEEDMOD_VALUE - NL::MIN_SPEEDMOD_VALUE),
                    0.0f,
                    1.0f);
            const float curRelSpeedMod = Clamp(
              curSpeedMods[sqrIdx] / float(MaxSpeedModTypeValue()), 0.0f, 1.0f);
#undef NL

            const SpeedBinType newSpeedModBin =
              GetSpeedModBin(newAbsSpeedMod, newRelSpeedMod);
            const SpeedBinType curSpeedModBin = curSpeedBins[sqrIdx];

            numNewBinSquares += int(newSpeedModBin != curSpeedModBin);
            numClosedSquares +=
              int(newSpeedModBin == QTPFS::NodeLayer::NUM_SPEEDMOD_BINS);

            // need to keep track of these for Tesselate
            oldSpeedMods[sqrIdx] =
              curRelSpeedMod * float(MaxSpeedModTypeValue());
            curSpeedMods[sqrIdx] =
              newRelSpeedMod * float(MaxSpeedModTypeValue());
...
QTPFS::NodeLayer::GetSpeedModBin(float absSpeedMod, float relSpeedMod) const
{
    ...
    SpeedBinType speedModBin =
      Clamp(defBin, static_cast<SpeedBinType>(0), maxBin);
                                              ...
                                              unsigned int currMagicNum)
{
    ...
    const int xoff =
      (currFrameNum % ((mapDims.mapx >> 1) / SQUARE_SIZE)) * SQUARE_SIZE;
    const int zoff =
      (currFrameNum / ((mapDims.mapy >> 1) / SQUARE_SIZE)) * SQUARE_SIZE;
...

    {
        ...
        const int xmax = std::min(xmin + SQUARE_SIZE, mapDims.mapx >> 1),
                  zmax = std::min(zmin + SQUARE_SIZE, mapDims.mapy >> 1);
    ...
    {
        // top-right quadrant: [mapDims.mapx >> 1, mapDims.mapx) x [0,
        // mapDims.mapy >> 1)
        const int xmin = (xoff + (mapDims.mapx >> 1)), zmin = (zoff + 0);
        const int xmax = std::min(xmin + SQUARE_SIZE, mapDims.mapx),
                  zmax = std::min(zmin + SQUARE_SIZE, mapDims.mapy >> 1);
    ...
    {
        // bottom-right quadrant: [mapDims.mapx >> 1, mapDims.mapx) x
        // [mapDims.mapy >> 1, mapDims.mapy)
        const int xmin = (xoff + (mapDims.mapx >> 1)),
                  zmin = (zoff + (mapDims.mapy >> 1));
        const int xmax = std::min(xmin + SQUARE_SIZE, mapDims.mapx),
                  zmax = std::min(zmin + SQUARE_SIZE, mapDims.mapy);
    ...
    {
        // bottom-left quadrant: [0, mapDims.mapx >> 1) x [mapDims.mapy >> 1,
        // mapDims.mapy)
                                               ...
                                               unsigned int currMagicNum)
{
    ...
    const int xmin = std::max(ur.x1 - 1, 0),
              xmax = std::min(ur.x2 + 1, mapDims.mapx);
    const int zmin = std::max(ur.z1 - 1, 0),
              zmax = std::min(ur.z2 + 1, mapDims.mapy);
    ...
    for (int z = zmin; z < zmax;) {
        ...
        for (int x = xmin; x < xmax;) {
            ...
            // NOTE:
            //   during initialization, currMagicNum == 0 which nodes start with
            //   already (does not matter because prevMagicNum == -1, so updates
            //   are not no-ops)
            n->SetMagicNumber(currMagicNum);
