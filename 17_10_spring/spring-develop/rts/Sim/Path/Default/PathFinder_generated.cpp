...
{
    return (&PF_DIRECTION_VECTORS_2D[0]);
}
const float3*
CPathFinder::GetDirectionVectorsTable3D()
{
    return (&PF_DIRECTION_VECTORS_3D[0]);
}
                      ...
                      const CSolidObject* owner)
{
    ...
    // could not reach goal from this starting position if nothing to left to
    // explore
    if (openBlocks.empty())
        return IPath::GoalOutOfRange;
                                 ...
                                 const CSolidObject* owner)
{
    ...
    {
        SqState()
          : blockedState(CMoveMath::BLOCK_NONE)
          , speedMod(0.0f)
          , inSearch(false)
        {}
    ...
    for (unsigned int dir = 0; dir < PATH_DIRECTIONS; dir++) {
        const int2 ngbSquareCoors =
          square->nodePos + PF_DIRECTION_VECTORS_2D[PathDir2PathOpt(dir)];
        const int ngbSquareIdx = BlockPosToIdx(ngbSquareCoors);

        if ((unsigned)ngbSquareCoors.x >= nbrOfBlocks.x ||
            (unsigned)ngbSquareCoors.y >= nbrOfBlocks.y)
            continue;

        if (blockStates.nodeMask[ngbSquareIdx] &
            (PATHOPT_CLOSED | PATHOPT_BLOCKED)) // FIXME
            continue;

        // very time expensive call
        SqState& sqState = ngbStates[dir];
        sqState.blockedState = CMoveMath::IsBlockedNoSpeedModCheck(
          moveDef, ngbSquareCoors.x, ngbSquareCoors.y, owner);
        ...
        // hint: use posSpeedMod for PE! cause it assumes path costs are
        // bidirectional and so it only saves one `cost` for left & right
        // movement hint2: not worth to put in front of the above code, it only
        // has a ~2% chance (heavily depending on the map) to early-out
        if (!pfDef.dirIndependent) {
            sqState.speedMod = CMoveMath::GetPosSpeedMod(
              moveDef,
              ngbSquareCoors.x,
              ngbSquareCoors.y,
              PF_DIRECTION_VECTORS_3D[PathDir2PathOpt(dir)]);
        } else {
            sqState.speedMod = CMoveMath::GetPosSpeedMod(
              moveDef, ngbSquareCoors.x, ngbSquareCoors.y);
            // only close node if we're dirIndependent
            // otherwise, it's possible we'll be able to enter it from another
            // direction.
        ...
        if (sqState.speedMod != 0.0f) {
            sqState.inSearch =
              pfDef.WithinConstraints(ngbSquareCoors.x, ngbSquareCoors.y);
        }
    }

    const auto CAN_TEST_SQUARE_SM = [&](const int dir) {
        return (ngbStates[dir].speedMod != 0.0f);
    };
    const auto CAN_TEST_SQUARE_IS = [&](const int dir) {
        return (ngbStates[dir].inSearch);
    };
    ...
    //
    const auto TEST_DIAG_SQUARE =
      [&](const int BASE_DIR_X, const int BASE_DIR_Y, const int BASE_DIR_XY) {
          if (CAN_TEST_SQUARE_SM(BASE_DIR_XY) &&
              (CAN_TEST_SQUARE_SM(BASE_DIR_X) &&
               CAN_TEST_SQUARE_SM(BASE_DIR_Y))) {
              if (CAN_TEST_SQUARE_IS(BASE_DIR_XY) ||
                  (CAN_TEST_SQUARE_IS(BASE_DIR_X) &&
                   CAN_TEST_SQUARE_IS(BASE_DIR_Y))) {
                       ...
                       float speedMod)
{
    ...
    const int2 square =
      parentSquare->nodePos + PF_DIRECTION_VECTORS_2D[pathOptDir];
    ...
    assert(
      (blockStates.nodeMask[sqrIdx] & (PATHOPT_CLOSED | PATHOPT_BLOCKED)) == 0);
    assert((blockStatus & CMoveMath::BLOCK_STRUCTURE) == 0);
    assert(speedMod != 0.0f);

    if (pfDef.testMobile && moveDef.avoidMobilesOnPath &&
        (blockStatus & squareMobileBlockBits)) {
        if (blockStatus & CMoveMath::BLOCK_MOBILE_BUSY) {
            speedMod *=
              moveDef.speedModMults[MoveDef::SPEEDMOD_MOBILE_BUSY_MULT];
        } else if (blockStatus & CMoveMath::BLOCK_MOBILE) {
            speedMod *=
              moveDef.speedModMults[MoveDef::SPEEDMOD_MOBILE_IDLE_MULT];
        } else { // (blockStatus & CMoveMath::BLOCK_MOVING)
            speedMod *=
              moveDef.speedModMults[MoveDef::SPEEDMOD_MOBILE_MOVE_MULT];
        }
    }

    const float heatCost =
      (pfDef.testMobile)
        ? (PathHeatMap::GetInstance())
            ->GetHeatCost(
              square.x, square.y, moveDef, ((owner != NULL) ? owner->id : -1U))
        : 0.0f;
    // const float flowCost  = (pfDef.testMobile) ?
    // (PathFlowMap::GetInstance())->GetFlowCost(square.x, square.y, moveDef,
    // pathOptDir) : 0.0f;
    const float extraCost =
      blockStates.GetNodeExtraCost(square.x, square.y, pfDef.synced);

    const float dirMoveCost =
      (1.0f + heatCost) * PF_DIRECTION_COSTS[pathOptDir];
    ...
    // if heuristic says this node is closer to goal than previous h-estimate,
    // keep it
    ...
    // store and mark this square as open (expanded, but not yet pulled from
    // pqueue)
                          ...
                          IPath::Path& foundPath) const
{
    ...
    if (pfDef.needPath) {
        ...
        while (true) {
            ...
            square -= PF_DIRECTION_VECTORS_2D[blockStates.nodeMask[blockIdx] &
                                              PATHOPT_CARDINALS];
              ...
              const float3 p3)
{
    ...
    geometricObjects->AddLine(
      old + float3(0, 10, 0), p2 + float3(0, 10, 0), 5, 10, 600, 0);
#endif
}
                               ...
                               const float3 nextPoint) const
{
    ...
    if (((blockStates.nodeMask[tstsqr] & PATHOPT_BLOCKED) == 0) &&
                             ...
                             int2 curquare) const
{
    ...
    for (unsigned pathDir = PATHDIR_LEFT; pathDir < PATH_DIRECTIONS;
         ++pathDir) {
        // find the pathDir
        ...
        if ((dirPrv == PE_DIRECTION_VECTORS[(pathDir - 1) % PATH_DIRECTIONS]) ||
            (dirPrv == PE_DIRECTION_VECTORS[(pathDir + 1) % PATH_DIRECTIONS])) {
            SmoothMidWaypoint(curquare + (dirPrv * PATH_NODE_SPACING),
                              p2,
                              moveDef,
                              foundPath,
                              nextPoint);
