                             ...
                             const LuaTable& weaponTable)
{
    *this = UnitDefWeapon();
    ...
    this->maxMainDirAngleDif = math::cos(
      (weaponTable.GetFloat("maxAngleDif", 360.0f) * 0.5f) * (PI / 180.0f));
    ...
    this->onlyTargetCat =
      (otcString.empty())
        ? 0xffffffff
        : CCategoryHandler::Instance()->GetCategories(otcString);
...
UnitDef::UnitDef(const LuaTable& udTable, const std::string& unitName, int id)
{
    ...
    *this = UnitDef();
    ...
    harvestMetalStorage = udTable.GetFloat(
      "harvestMetalStorage", udTable.GetFloat("harvestStorage", 0.0f));
    ...
    health = std::max(
      0.1f,
      udTable.GetFloat("maxDamage", 0.0f)); // avoid some nasty divide by 0
    autoHeal = udTable.GetFloat("autoHeal", 0.0f) *
               (UNIT_SLOWUPDATE_RATE / float(GAME_SPEED));
    idleAutoHeal = udTable.GetFloat("idleAutoHeal", 10.0f) *
                   (UNIT_SLOWUPDATE_RATE / float(GAME_SPEED));
    ...
    mass = Clamp(udTable.GetFloat("mass", metal),
    ...
    buildTime = std::max(
      0.1f,
      udTable.GetFloat("buildTime", 0.0f)); // avoid some nasty divide by 0
    ...
    canCloak = udTable.GetBool("canCloak",
                               (udTable.GetFloat("cloakCost", 0.0f) != 0.0f));
    ...
    canRestore =
      udTable.GetBool("canRestore", builder) && (terraformSpeed > 0.0f);
    canRepair = udTable.GetBool("canRepair", builder) && (repairSpeed > 0.0f);
    canReclaim =
      udTable.GetBool("canReclaim", builder) && (reclaimSpeed > 0.0f);
    canCapture = udTable.GetBool("canCapture", false) && (captureSpeed > 0.0f);
    canResurrect =
      udTable.GetBool("canResurrect", false) && (resurrectSpeed > 0.0f);
    ...
    canManualFire =
      udTable.GetBool("canManualFire", udTable.GetBool("canDGun", false));
    ...
    const float maxSlopeDeg =
      Clamp(udTable.GetFloat("maxSlope", 0.0f), 0.0f, 89.0f);
    ...
    fireState = udTable.GetInt(
      "fireState", canFireControl ? FIRESTATE_NONE : FIRESTATE_FIREATWILL);
    fireState = std::min(fireState, int(FIRESTATE_FIREATWILL));
    moveState = udTable.GetInt("moveState",
                               (canmove && speed > 0.0f) ? MOVESTATE_NONE
                                                         : MOVESTATE_MANEUVER);
    ...
    maxAcc =
      math::fabs(udTable.GetFloat("acceleration", 0.5f)); // no negative values
    maxDec =
      math::fabs(udTable.GetFloat("brakeRate", maxAcc)); // no negative values
    ...
    turnInPlaceSpeedLimit = udTable.GetFloat(
      "turnInPlaceSpeedLimit", std::min(speed, turnInPlaceSpeedLimit));
    ...
    wingAngle = udTable.GetFloat(
      "wingAngle", 0.08f); // angle between front and the wing plane
    frontToSpeed = udTable.GetFloat(
      "frontToSpeed",
      0.1f); // fudge factor for lining up speed and front of plane
    speedToFront = udTable.GetFloat(
      "speedToFront",
      0.07f); // fudge factor for lining up speed and front of plane
    myGravity = udTable.GetFloat("myGravity", 0.4f); // planes are slower than
                                                     // real airplanes so lower
                                                     // gravity to compensate
    crashDrag =
      udTable.GetFloat("crashDrag", 0.005f); // drag used when crashing
    crashDrag = Clamp(crashDrag, 0.0f, 1.0f);

    maxBank = udTable.GetFloat("maxBank", 0.8f); // max roll
    maxPitch =
      udTable.GetFloat("maxPitch", 0.45f); // max pitch this plane tries to keep
    turnRadius = udTable.GetFloat(
      "turnRadius",
      500.0f); // hint to CStrafeAirMoveType about required turn-radius
    verticalSpeed = udTable.GetFloat(
      "verticalSpeed",
      3.0f); // speed of takeoff and landing, at least for gunships

    maxAileron =
      udTable.GetFloat("maxAileron", 0.015f); // turn speed around roll axis
    maxElevator =
      udTable.GetFloat("maxElevator", 0.01f); // turn speed around pitch axis
    maxRudder =
      udTable.GetFloat("maxRudder", 0.004f); // turn speed around yaw axis

    maxThisUnit = udTable.GetInt("unitRestricted", MAX_UNITS);
    maxThisUnit =
      std::min(maxThisUnit, gameSetup->GetRestrictedUnitLimit(name, MAX_UNITS));

    categoryString = udTable.GetString("category", "");

    category = CCategoryHandler::Instance()->GetCategories(
      udTable.GetString("category", ""));
    noChaseCategory = CCategoryHandler::Instance()->GetCategories(
      udTable.GetString("noChaseCategory", ""));

    iconType =
      icon::iconHandler->GetIcon(udTable.GetString("iconType", "default"));
...

    {
        ...
        if (RequireMoveDef()) {
            const std::string& moveClass =
              StringToLower(udTable.GetString("movementClass", ""));
            const std::string errMsg =
              "WARNING: Couldn't find a MoveClass named " + moveClass +
              " (used in UnitDef: " + unitName + ")";

            if ((moveDef = moveDefHandler->GetMoveDefByName(moveClass)) ==
                NULL) {
                throw content_error(errMsg); //! invalidate unitDef (this gets
                                             //! catched in ParseUnitDef!)
            }
        ...
        if (moveDef == NULL) {
            ...
            // we have no MoveDef, so pathType == -1 and IsAirUnit() MIGHT be
            // true
            ...
            // we have a MoveDef, so pathType != -1 and IsGroundUnit() MUST be
            // true
            cantBeTransported |= (!modInfo.transportGround &&
                                  moveDef->speedModClass == MoveDef::Tank);
            cantBeTransported |= (!modInfo.transportGround &&
                                  moveDef->speedModClass == MoveDef::KBot);
            cantBeTransported |= (!modInfo.transportShip &&
                                  moveDef->speedModClass == MoveDef::Ship);
            cantBeTransported |= (!modInfo.transportHover &&
                                  moveDef->speedModClass == MoveDef::Hover);
        }

        if (seismicSignature == -1.0f) {
            const bool isTank =
              (moveDef != NULL && moveDef->speedModClass == MoveDef::Tank);
            const bool isKBot =
              (moveDef != NULL && moveDef->speedModClass == MoveDef::KBot);

            // seismic signatures only make sense for certain mobile ground
            // units
            if (isTank || isKBot) {
    ...
    deathExpWeaponDef =
      weaponDefHandler->GetWeaponDef(udTable.GetString("explodeAs", ""));
    selfdExpWeaponDef = weaponDefHandler->GetWeaponDef(
      udTable.GetString("selfDestructAs", udTable.GetString("explodeAs", "")));
    if (deathExpWeaponDef == NULL) {
        deathExpWeaponDef = weaponDefHandler->GetWeaponDef("NOWEAPON");
        if (deathExpWeaponDef == NULL) {
            LOG_L(L_ERROR,
                  "Couldn't find WeaponDef NOWEAPON and explodeAs for %s is "
                  "missing!",
                  unitName.c_str());
        }
    }
    if (selfdExpWeaponDef == NULL) {
        ...
        if (selfdExpWeaponDef == NULL) {
            LOG_L(L_ERROR,
                  "Couldn't find WeaponDef NOWEAPON and selfDestructAs for %s "
                  "is missing!",
                  unitName.c_str());
        }
    }
    ...
    if (power < 1.0e-3f) {
        LOG_L(
          L_WARNING, "Unit %s is really cheap? %f", humanName.c_str(), power);
        LOG_L(L_WARNING,
              "This can cause a division by zero in experience calculations.");
        power = 1.0e-3f;
    }
    ...
    xsize =
      std::max(1 * SPRING_FOOTPRINT_SCALE,
               (udTable.GetInt("footprintX", 1) * SPRING_FOOTPRINT_SCALE));
    zsize =
      std::max(1 * SPRING_FOOTPRINT_SCALE,
               (udTable.GetInt("footprintZ", 1) * SPRING_FOOTPRINT_SCALE));

    buildingMask = (boost::uint16_t)udTable.GetInt(
      "buildingMask", 1); // 1st bit set to 1 constitutes for "normal building"
    if (IsImmobileUnit()) {
    ...
    const LuaTable& pieceCEGTable =
      sfxTable.SubTable("pieceExplosionGenerators");

    std::vector<int> modelCEGKeys;
    modelCEGTable.GetKeys(modelCEGKeys);
    std::vector<int> pieceCEGKeys;
    pieceCEGTable.GetKeys(pieceCEGKeys);
...
UnitDef::ParseWeaponsTable(const LuaTable& weaponsTable)
{
    ...
    for (int w = 0; w < MAX_WEAPONS_PER_UNIT; w++) {
        ...
        while (weapons.size() < w) {
            if (!noWeaponDef) {
                LOG_L(L_ERROR,
                      "Spring requires a NOWEAPON weapon type "
        ...
        if (wd->stockpile) {
            ...
            if (wd->interceptor || !stockpileWeaponDef ||
...
UnitDef::CreateYardMap(std::string yardMapStr)
{
    ...
    if (ymCopyIdx > defYardMap.size())
        LOG_L(L_WARNING,
              "%s: Given yardmap contains " _STPF_ " excess char(s)!",
              name.c_str(),
              ymCopyIdx - defYardMap.size());

    if (ymCopyIdx > 0 && ymCopyIdx < defYardMap.size())
        LOG_L(L_WARNING,
              "%s: Given yardmap requires " _STPF_ " extra char(s)!",
              name.c_str(),
              defYardMap.size() - ymCopyIdx);

    if (!unknownChars.empty())
        LOG_L(L_WARNING,
              "%s: Given yardmap contains unknown char(s) \"%s\"!",
              name.c_str(),
              unknownChars.c_str());
    ...
    for (unsigned int bmz = 0; bmz < zsize; bmz++) {
        for (unsigned int bmx = 0; bmx < xsize; bmx++) {
            const unsigned int yardMapIdx =
              (bmx >> (1 - highResMap)) + ((bmz >> (1 - highResMap)) * hxsize);
...
{
    // can fail if LuaMoveCtrl has changed a unit's MoveDef (UnitDef::pathType
    // is not updated) assert(pathType == -1u || moveDef ==
    // moveDefHandler->GetMoveDefByPathType(pathType));
...
UnitDef::HasBomberWeapon() const
{
    ...
    if (weapons[0].def == NULL) {
        return false;
    }
    return weapons[0].def->type == "AircraftBomb" ||
