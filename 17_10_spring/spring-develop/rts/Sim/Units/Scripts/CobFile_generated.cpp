...
CCobFile::CCobFile(CFileHandler& in, string name)
{
    ...
    scriptLengths.push_back(ch.TotalScriptLen -
                            scriptOffsets[ch.NumberOfScripts - 1]);
    ...
    scriptIndex.resize(COBFN_Last + (MAX_WEAPONS_PER_UNIT * COBFN_Weapon_Funcs),
                       -1);
    for (std::map<string, int>::const_iterator it = nameMap.begin();
         it != nameMap.end();
         ++it) {
