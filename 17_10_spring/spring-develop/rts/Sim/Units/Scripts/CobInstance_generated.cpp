...
CCobInstance::MapScriptToModelPieces(LocalModel* lmodel)
{
    std::vector<std::string>& pieceNames =
      script->pieceNames; // already in lowercase!
    ...
    for (size_t lmodelPieceNum = 0; lmodelPieceNum < lmodelPieces.size();
         lmodelPieceNum++) {
        lmodelPieces[lmodelPieceNum].SetScriptPieceIndex(-1);
    }
    for (size_t scriptPieceNum = 0; scriptPieceNum < pieceNames.size();
         scriptPieceNum++) {
        unsigned int lmodelPieceNum;

        // Map this piecename to an index in the script's pieceinfo
        for (lmodelPieceNum = 0; lmodelPieceNum < lmodelPieces.size();
             lmodelPieceNum++) {
            if (lmodelPieces[lmodelPieceNum].original->name.compare(
                  pieceNames[scriptPieceNum]) == 0) {
                break;
            }
        }
        ...
        if (lmodelPieceNum == lmodelPieces.size()) {
            for (lmodelPieceNum = 0; lmodelPieceNum < lmodelPieces.size();
                 lmodelPieceNum++) {
                if (StringToLower(lmodelPieces[lmodelPieceNum].original->name)
                      .compare(pieceNames[scriptPieceNum]) == 0) {
                    break;
                }
            }
        }
        ...
        if (lmodelPieceNum < lmodelPieces.size()) {
            ...
            const char* fmtString = "[%s] could not find piece named \"%s\" "
                                    "(referenced by COB script \"%s\")";
...
CCobInstance::Create()
{
    ...
    for (vector<CWeapon*>::iterator i = unit->weapons.begin();
         i != unit->weapons.end();
         ++i) {
...
{
    // yes, COB is silly, while it handles integers fine it uses model height to
    // identify units
...
CCobInstance::AimWeapon(int weaponNum, float heading, float pitch)
{
    ...
    args.push_back(short(heading * RAD2TAANG));
    args.push_back(short(pitch * RAD2TAANG));
    Call(COBFN_AimPrimary + COBFN_Weapon_Funcs * weaponNum,
         args,
         CBAimWeapon,
         weaponNum,
         nullptr);
                       ...
                       int* retCode)
{
    ...
    // LOG_L(L_DEBUG, "Calling %s:%s", script->name.c_str(),
    // script->scriptNames[functionId].c_str());
    ...
    if (!res) {
        ...
        //   prevent error-spam
        unsigned int i = 0,
                     argc = thread->CheckStack(
                       args.size(),
                       functionId != script->scriptIndex[COBFN_StartMoving]);
...
CCobInstance::Call(const std::string& fname,
                   ...
                   int* retCode)
{
    // TODO: Check that new behaviour of actually calling cb when the function
    // is not defined is right?
    //      (the callback has always been called [when the function is not
    //      defined]
...
CCobInstance::ThreadCallback(ThreadCallbackType type, int retCode, int cbParam)
{
    switch (type) {
        ...
        case CBAimWeapon:
            ...
            break;
        case CBAimShield:
            static_cast<CPlasmaRepulser*>(unit->weapons[cbParam])
              ->SetEnabled(retCode != 0);
            break;
        default:
            assert(false);
    }
...
CCobInstance::Signal(int signal)
{
    for (CCobThread* t : threads) {
        if ((signal & t->signalMask) != 0) {
            t->state = CCobThread::Dead;
            // LOG_L(L_DEBUG, "Killing a thread %d %d", signal,
            // (*i)->signalMask);
        }
    }
}
...
CCobInstance::PlayUnitSound(int snr, int attr)
{
    Channels::UnitReply->PlaySample(
      script->sounds[snr], unit->pos, unit->speed, attr);
}
