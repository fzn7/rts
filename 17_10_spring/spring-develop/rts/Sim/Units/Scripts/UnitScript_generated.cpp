...
CUnitScript::TurnToward(float& cur, float dest, float speed)
{
    ...
    // clamp: -pi .. 0 .. +pi (remainder(x,TWOPI) would do the same but is
    // slower due to streflop)
    if (delta > PI) {
        delta -= TWOPI;
    } else if (delta <= -PI) {
        delta += TWOPI;
    ...
    switch (type) {
        case AMove: {
            ...
            while (i < anims[type].size()) {
                ...
                if (MoveToward(
                      pos[ai.axis], ai.dest, ai.speed / (1000 / deltaTime))) {
                    ai.done = true;
                    if (ai.hasWaiting)
        ...
        case ATurn: {
            ...
            while (i < anims[type].size()) {
                ...
                if (TurnToward(
                      rot[ai.axis], ai.dest, ai.speed / (1000 / deltaTime))) {
                    ai.done = true;
                    if (ai.hasWaiting)
                        doneAnims.push_back(ai);
...
CUnitScript::Tick(int deltaTime)
{
    ...
    // Tell listeners to unblock, and remove finished animations from the
    // unit/script.
    for (int animType = ATurn; animType <= AMove; animType++) {
...
CUnitScript::RemoveAnim(AnimType type, const AnimContainerTypeIt& animInfoIt)
{
    ...
    //! We need to unblock threads waiting on this animation, otherwise they
    //! will be lost in the void NOTE: AnimFinished might result in new anims
    //! being added
    if (ai.hasWaiting)
                     ...
                     float accel)
{
    ...
    if (animInfoIt == anims[type].end()) {
        // If we were not animating before, inform the engine of this so it can
        // schedule us
        // FIXME: this could be done in a cleaner way
        if (!HaveAnimations()) {
...
CUnitScript::EmitSfx(int sfxType, int piece)
{
    ...
    switch (sfxType) {
        ...
        case SFX_BUBBLE: { // submarine bubble. does not provide direction
                           // through piece vertices..
            float3 pspeed = gu->RandVector() * 0.1f;
            pspeed.y += 0.2f;

            new CBubbleProjectile(unit,
                                  pos + gu->RandVector() * 2.0f,
                                  pspeed,
        ...
        case SFX_WHITE_SMOKE: // damaged unit smoke
            new CSmokeProjectile(unit,
                                 pos,
                                 gu->RandVector() * 0.5f + UpVector * 1.1f,
                                 60,
                                 4,
                                 0.5f,
                                 0.5f);
        ...
        default: {
            if (sfxType & SFX_CEG) {
                // emit defined explosion-generator (can only be custom, not
                // standard) index is made valid by callee, an ID of -1 means
                // CEG failed to load
                explGenHandler->GenExplosion(
                  ud->GetModelExplosionGeneratorID(sfxType - SFX_CEG),
                  pos,
                  dir,
                  unit->cegDamage,
                  1.0f,
...
CUnitScript::Explode(int piece, int flags)
{
    ...
    tracefile << absPos.x << " " << absPos.y << " " << absPos.z << " " << piece
              << " " << flags << "\n";
#endif
    ...
    if (unit->pos.y - CGround::GetApproximateHeight(unit->pos.x, unit->pos.z) >
        15)
    ...
    new CPieceProjectile(
      unit, pieces[piece], absPos, explSpeed, newFlags, 0.5f);
#endif
}
...
CUnitScript::Shatter(int piece, const float3& pos, const float3& speed)
{
    ...
    const float pieceChance =
      1.0f - (projectileHandler->GetCurrentParticles() -
              (projectileHandler->maxParticles - 2000)) /
               2000.0f;
    if (pieceChance > 0.0f) {
        const CMatrix44f m =
          unit->GetTransformMatrix() * lmp->GetModelSpaceMatrix();
        omp->Shatter(pieceChance,
                     unit->model->type,
                     unit->model->textureType,
                     unit->team,
                     pos,
                     speed,
                     m);
...
{
    // may happen in case one uses Spring.GetUnitCOBValue (Lua) on a unit with
    // CNullUnitScript
    if (!unit) {
    ...
    switch (val) {
        ...
        case XZ_ATAN:
            return int(RAD2TAANG *
                         math::atan2((float)UNPACKX(p1), (float)UNPACKZ(p1)) +
                       32768 - unit->heading);
        case XZ_HYPOT:
            return int(math::hypot((float)UNPACKX(p1), (float)UNPACKZ(p1)) *
                       COBSCALE);
        case ATAN:
        ...
        case GROUND_HEIGHT:
            return int(CGround::GetHeightAboveWater(UNPACKX(p1), UNPACKZ(p1)) *
                       COBSCALE);
        case GROUND_WATER_HEIGHT:
            return int(CGround::GetHeightReal(UNPACKX(p1), UNPACKZ(p1)) *
                       COBSCALE);
        ...
        case POW:
            return int(
              math::pow(((float)p1) / COBSCALE, ((float)p2) / COBSCALE) *
              COBSCALE);
        case PRINT:
        ...
        case SHIELD_POWER: {
            ...
            return int(
              static_cast<const CPlasmaRepulser*>(shield)->GetCurPower() *
              float(COBSCALE));
        }
        ...
        case PLAY_SOUND: {
            // FIXME: this can currently only work for CCobInstance, because Lua
            // can not get sound IDs (however, for Lua scripts there is already
            // LuaUnsyncedCtrl::PlaySoundFile)
            ...
            if ((p1 < 0) ||
                (static_cast<size_t>(p1) >= script->sounds.size())) {
                return 1;
            }
            switch (p3) { // who hears the sound
                ...
                case 2: // ALOS or radar
                    if (!(losHandler->InAirLos(unit, gu->myAllyTeam) ||
                          unit->losStatus[gu->myAllyTeam] & (LOS_INRADAR))) {
                        return 0;
                    }
                    break;
                case 3: // LOS or radar
                    if (!(unit->losStatus[gu->myAllyTeam] &
            ...
            if (p4 == 0) {
                Channels::General->PlaySample(script->sounds[p1],
                                              unit->pos,
                                              unit->speed,
                                              float(p2) / COBSCALE);
            } else {
        ...
        case SET_WEAPON_UNIT_TARGET: {
            ...
            CUnit* target =
              (targetID > 0) ? unitHandler->GetUnit(targetID) : NULL;
        ...
        case SET_WEAPON_GROUND_TARGET: {
            ...
            if ((weaponID < 0) ||
                (static_cast<size_t>(weaponID) >= unit->weapons.size())) {
                return 0;
            }
            CWeapon* weapon = unit->weapons[weaponID];
            if (weapon == NULL) {
        ...
        case FLANK_B_DIR:
            switch (p1) {
                case 1:
                    return int(unit->flankingBonusDir.x * COBSCALE);
                case 2:
                    return int(unit->flankingBonusDir.y * COBSCALE);
                case 3:
                    return int(unit->flankingBonusDir.z * COBSCALE);
                case 4:
                    unit->flankingBonusDir.x = (p2 / (float)COBSCALE);
                    return 0;
                case 5:
                    unit->flankingBonusDir.y = (p2 / (float)COBSCALE);
                    return 0;
                case 6:
                    unit->flankingBonusDir.z = (p2 / (float)COBSCALE);
                    return 0;
                case 7:
                    unit->flankingBonusDir = float3(p2 / (float)COBSCALE,
                                                    p3 / (float)COBSCALE,
                                                    p4 / (float)COBSCALE)
                                               .Normalize();
                    return 0;
        ...
        default:
            if ((val >= GLOBAL_VAR_START) && (val <= GLOBAL_VAR_END)) {
                ...
                ShowUnitScriptError(
                  "CobError: Unknown get constant " + IntToString(val) +
                  " (params = " + IntToString(p1) + " " + IntToString(p2) +
                  " " + IntToString(p3) + " " + IntToString(p4) + ")");
            }
    }
#endif
...
{
    // may happen in case one uses Spring.SetUnitCOBValue (Lua) on a unit with
    // CNullUnitScript
    if (!unit) {
    ...
    switch (val) {
        ...
        case BUGGER_OFF: {
            if (param != 0) {
                CGameHelper::BuggerOff(unit->pos +
                                         unit->frontdir * unit->radius,
                                       unit->radius * 1.5f,
                                       true,
                                       false,
                                       unit->team,
        ...
        case MAX_SPEED: {
            ...
            unit->commandAI->SetScriptMaxSpeed(
              std::max(param, -param) / float(COBSCALE), (param >= 0));
            break;
        }
        case CLOAKED: {
        ...
        case SHIELD_POWER: {
            if (unit->shieldWeapon != NULL) {
                CPlasmaRepulser* shield =
                  static_cast<CPlasmaRepulser*>(unit->shieldWeapon);
                shield->SetCurPower(
                  std::max(0.0f, float(param) / float(COBSCALE)));
            }
            break;
        }
        case STEALTH: {
        ...
        case FLANK_B_MAX_DAMAGE: {
            ...
            unit->flankingBonusAvgDamage =
              (param / (float)COBSCALE + mindamage) * 0.5f;
        ...
        case FLANK_B_MIN_DAMAGE: {
            float maxdamage =
              unit->flankingBonusAvgDamage + unit->flankingBonusDifDamage;
            unit->flankingBonusAvgDamage =
              (maxdamage + param / (float)COBSCALE) * 0.5f;
            unit->flankingBonusDifDamage =
              (maxdamage - param / (float)COBSCALE) * 0.5f;
            break;
        }
        default: {
            if ((val >= GLOBAL_VAR_START) && (val <= GLOBAL_VAR_END)) {
                ...
                ShowUnitScriptError("CobError: Unknown set constant " +
                                    IntToString(val));
            }
        }
    }
...
{
    ShowScriptError(unit
                      ? error + std::string(" ") + IntToString(unit->id) +
                          std::string(" of type ") + unit->unitDef->name
