...
int CUnitScript::GetUnitVal(int val, int p1, int p2, int p3, int p4)
{
	...
	case ABS:
		return abs(p1);
	case KSIN:
                return int(1024*sinf(TAANG2RAD*(float)p1));
	case KCOS:
                return int(1024*cosf(TAANG2RAD*(float)p1));
	case KTAN:
                return int(1024*tanf(TAANG2RAD*(float)p1));
	case SQRT:
                return int(sqrt((float)p1));
	case FLANK_B_MODE:
