  ...
  wantedHeading(0)
{
    ...
    turnAccel =
      turnRate *
      mix(0.333f, 0.033f, owner->moveDef->speedModClass == MoveDef::Ship);

    accRate = std::max(0.01f, owner->unitDef->maxAcc);
    decRate = std::max(0.01f, owner->unitDef->maxDec);

    // unit-gravity must always be negative
    myGravity = mix(-math::fabs(owner->unitDef->myGravity),
                    mapInfo->map.gravity,
                    owner->unitDef->myGravity == 0.0f);
}
...
CGroundMoveType::PostLoad()
{
    ...
    if (pathID != 0) {
        pathID = pathManager->RequestPath(
          owner, owner->moveDef, owner->pos, goalPos, goalRadius, true);
    }
}
                            ...
                            const float3& cmpEps)
{
    ...
    //   idling &= (Square(currWayPointDist - prevWayPointDist) <= (accRate *
    //   accRate));
    // too many false positives: waypoint-distance delta and speed vary too much
    //   idling = (Square(currWayPointDist - prevWayPointDist) <
    //   Square(owner->speed.w));
...
CGroundMoveType::Update()
{
    ...
    owner->UpdatePhysicalStateBit(CSolidObject::PSTATE_BIT_SKIDDING,
                                  owner->IsSkidding() || OnSlope(1.0f));
    ...
    return (OwnerMoved(
      heading,
      owner->pos - oldPos,
      float3(float3::cmp_eps(), float3::cmp_eps() * 1e-2f, float3::cmp_eps())));
}
...
CGroundMoveType::SlowUpdate()
{
    if (owner->GetTransporter() != NULL) {
        ...
        if (progressState == Active) {
            if (pathID != 0) {
                if (idling) {
                    numIdlingSlowUpdates = std::min(
                      MAX_IDLING_SLOWUPDATES, int(numIdlingSlowUpdates + 1));
                } else {
                    numIdlingSlowUpdates =
                      std::max(0, int(numIdlingSlowUpdates - 1));
                }
...
CGroundMoveType::StartMoving(float3 moveGoalPos, float moveGoalRadius)
{
    ...
    tracefile << owner->pos.x << " " << owner->pos.y << " " << owner->pos.z
              << " " << owner->id << "\n";
#endif
    ...
    if (owner->team == gu->myTeam) {
        Channels::General->PlayRandomSample(owner->unitDef->sounds.activate,
                                            owner);
    }
}
...
CGroundMoveType::FollowPath()
{
    ...
    if (WantToStop()) {
        ...
        {
            // NOTE:
            //   uses owner->pos instead of currWayPoint (ie. not the same as
            //   atEndOfPath)
            //
            //   if our first command is a build-order, then goalRadius is set
            //   to our build-range and we cannot increase tolerance safely
            //   (otherwise the unit might stop when still outside its range and
            //   fail to start construction)
            const float curGoalDistSq = (owner->pos - goalPos).SqLength2D();
            const float minGoalDistSq =
              (UNIT_HAS_MOVE_CMD(owner))
        ...
        if (!atGoal) {
            if (!idling) {
                ...
                numIdlingUpdates =
                  std::min(SHORTINT_MAXVALUE, int(numIdlingUpdates + 1));
            }
        }
        ...
        // set direction to waypoint AFTER requesting it; should not be a
        // null-vector
        if (currWayPoint != owner->pos)
            waypointDir =
              ((currWayPoint - owner->pos) * XZVector).SafeNormalize();
                             ...
                             bool fpsMode)
{
...
#endif
    {
        if (wantedSpeed > 0.0f) {
            ...
            float groundSpeedMod =
              CMoveMath::GetPosSpeedMod(*md, owner->pos, flatFrontDir);

            // the pathfinders don't check the speedmod of the square our unit
            // is currently on so if we got stuck on a nonpassable square and
            // can't move try to see if we're trying to release ourselves
            // towards a passable square
            if (groundSpeedMod == 0.0f)
                groundSpeedMod = CMoveMath::GetPosSpeedMod(
                  *md, owner->pos + flatFrontDir * SQUARE_SIZE, flatFrontDir);

            const float curGoalDistSq = (owner->pos - goalPos).SqLength2D();
            const float minGoalDistSq =
              Square(BrakingDistance(currentSpeed, decRate));

            const float3& waypointDifFwd = waypointDir;
            const float3 waypointDifRev = -waypointDifFwd;

            const float3& waypointDif =
              mix(waypointDifFwd, waypointDifRev, reversing);
            const short turnDeltaHeading =
              owner->heading -
              GetHeadingFromVector(waypointDif.x, waypointDif.z);

            // NOTE: <= 2 because every CMD_MOVE has a trailing
            // CMD_SET_WANTED_MAX_SPEED
            const bool startBraking =
              (UNIT_CMD_QUE_SIZE(owner) <= 2 &&
               curGoalDistSq <= minGoalDistSq && !fpsMode);

            if (!fpsMode && turnDeltaHeading != 0) {
                // only auto-adjust speed for turns when not in FPS mode
                const float reqTurnAngle =
                  math::fabs(180.0f * short(owner->heading - wantedHeading) /
                             SHORTINT_MAXVALUE);
                const float maxTurnAngle =
                  (turnRate / SPRING_CIRCLE_DIVS) * 360.0f;

                const float turnMaxSpeed =
                  mix(maxSpeed, maxReverseSpeed, reversing);
                float turnModSpeed = turnMaxSpeed;

                if (reqTurnAngle != 0.0f)
                    turnModSpeed *=
                      Clamp(maxTurnAngle / reqTurnAngle, 0.1f, 1.0f);

                if (waypointDir.SqLength() > 0.1f) {
                    if (!ud->turnInPlace) {
                        // never let speed drop below TIPSL, but limit TIPSL
                        // itself to turnMaxSpeed
                        targetSpeed = Clamp(
                          turnModSpeed,
                          std::min(ud->turnInPlaceSpeedLimit, turnMaxSpeed),
                          turnMaxSpeed);
                    } else {
                ...
                if (atEndOfPath) {
                    // at this point, Update() will no longer call
                    // GetNextWayPoint() and we must slow down to prevent
                    // entering an infinite circle
                    targetSpeed = std::min(targetSpeed,
                                           (currWayPointDist * PI) /
                                             (SPRING_CIRCLE_DIVS / turnRate));
                }
            }
    ...
    deltaSpeed = pathController->GetDeltaSpeed(pathID,
                                               targetSpeed,
                                               currentSpeed,
                                               accRate,
                                               decRate,
...
CGroundMoveType::ChangeHeading(short newHeading)
{
    ...
    owner->heading +=
      pathController->GetDeltaHeading(pathID,
                                      (wantedHeading = newHeading),
                                      owner->heading,
                                      turnRate,
                                      turnAccel,
                                      BrakingDistance(turnSpeed, turnAccel),
                                      &turnSpeed);
#endif
...
CGroundMoveType::CanApplyImpulse(const float3& impulse)
{
    ...
    const bool startFlying =
      StartFlying(newSpeed, CGround::GetNormal(owner->pos.x, owner->pos.z));
...
CGroundMoveType::UpdateSkid()
{
    ...
    owner->SetVelocity(spd + owner->GetDragAccelerationVec(
                               float4(mapInfo->atmosphere.fluidDensity,
    ...
    if (owner->IsFlying()) {
        ...
        const float impactDamageMult =
          impactSpeed * owner->mass * COLLISION_DAMAGE_MULT;
        const bool doColliderDamage = (modInfo.allowUnitCollisionDamage &&
                                       impactSpeed > ud->minCollisionSpeed &&
        ...
        if (groundHeight > pos.y) {
            ...
            if (doColliderDamage) {
                owner->DoDamage(DamageArray(impactDamageMult),
                                ZeroVector,
                                NULL,
                                -CSolidObject::DAMAGE_COLLISION_GROUND,
                                -1);
            }
        ...
        if (!onSlope && StopSkidding(spd, owner->frontdir)) {
            ...
            if (onSlope) {
                ...
                const float3 normalForce =
                  normalVector *
                  normalVector.dot(UpVector * mapInfo->map.gravity);
                const float3 newForce =
                  UpVector * mapInfo->map.gravity - normalForce;
                ...
                owner->SetVelocity(
                  spd * (1.0f - std::min(1.0f, speedReduction / speedScale)));
            }

            skidRotSpd = math::floor(skidRotSpeed +
                                     skidRotAccel * (remTime - 1.0f) + 0.5f);
            skidRotAccel = (skidRotSpd - skidRotSpeed) / remTime;
            skidRotAccel *= (PI / 180.0f);

            if (math::floor(skidRotSpeed) !=
                math::floor(skidRotSpeed + skidRotAccel)) {
                skidRotSpeed = 0.0f;
                skidRotAccel = 0.0f;
            }
        }
        ...
        if ((groundHeight - pos.y) < (spd.y + mapInfo->map.gravity)) {
            ...
            // in one frame
            const float3& normal =
              (pos.IsInBounds()) ? CGround::GetNormal(pos.x, pos.z) : UpVector;
            ...
            if (dot > 0.0f) {
                ...
                owner->SetVelocity(
                  spd + (normal * (math::fabs(spd.dot(normal)) + 0.1f)));
...
CGroundMoveType::UpdateControlledDrop()
{
    ...
    const float4& spd = owner->speed;
    const float3 acc =
      UpVector * std::min(mapInfo->map.gravity * owner->fallSpeed, 0.0f);
...
CGroundMoveType::CheckCollisionSkid()
{
    ...
    const vector<CUnit*> nearUnits =
      quadField->GetUnitsExact(pos, collider->radius);
    const vector<CFeature*> nearFeatures =
      quadField->GetFeaturesExact(pos, collider->radius);
    ...
    for (ui = nearUnits.begin(); ui != nearUnits.end(); ++ui) {
        ...
        if (!collidee->HasCollidableStateBit(
              CSolidObject::CSTATE_BIT_SOLIDOBJECTS))
            continue;
        ...
        if (collidee->unitDef->IsImmobileUnit()) {
            ...
            const float impactDamageMult =
              std::min(impactSpeed * collider->mass * COLLISION_DAMAGE_MULT,
                       MAX_UNIT_SPEED);

            const bool doColliderDamage =
              (modInfo.allowUnitCollisionDamage &&
               impactSpeed > colliderUD->minCollisionSpeed &&
               colliderUD->minCollisionSpeed >= 0.0f);
            const bool doCollideeDamage =
              (modInfo.allowUnitCollisionDamage &&
               impactSpeed > collideeUD->minCollisionSpeed &&
               collideeUD->minCollisionSpeed >= 0.0f);
            ...
            if (doColliderDamage) {
                collider->DoDamage(DamageArray(impactDamageMult),
                                   ZeroVector,
                                   NULL,
                                   -CSolidObject::DAMAGE_COLLISION_OBJECT,
                                   -1);
            }
            // damage the (static) collidee based on collider's mass, no added
            // impulse
            if (doCollideeDamage) {
                collidee->DoDamage(DamageArray(impactDamageMult),
                                   ZeroVector,
                                   NULL,
                                   -CSolidObject::DAMAGE_COLLISION_OBJECT,
                                   -1);
            ...
            collider->SetVelocity(collider->speed +
                                  ((dif * impactSpeed) * 1.8f));
        } else {
            assert(collider->mass > 0.0f && collidee->mass > 0.0f);

            // don't conserve momentum (impact speed is halved, so impulses are
            // too)
            // --> collisions are neither truly elastic nor truly inelastic to
            // prevent the simulation from blowing up from impulses applied to
            // tight groups of units
            const float impactSpeed =
              (collidee->speed - collider->speed).dot(dif) * 0.5f;
            const float colliderRelMass =
              (collider->mass / (collider->mass + collidee->mass));
            const float colliderRelImpactSpeed =
              impactSpeed * (1.0f - colliderRelMass);
            const float collideeRelImpactSpeed =
              impactSpeed * (colliderRelMass);

            const float colliderImpactDmgMult = std::min(
              colliderRelImpactSpeed * collider->mass * COLLISION_DAMAGE_MULT,
              MAX_UNIT_SPEED);
            const float collideeImpactDmgMult = std::min(
              collideeRelImpactSpeed * collider->mass * COLLISION_DAMAGE_MULT,
              MAX_UNIT_SPEED);
            ...
            // damage the collider
            if (doColliderDamage) {
                collider->DoDamage(DamageArray(colliderImpactDmgMult),
                                   dif * colliderImpactDmgMult,
                                   NULL,
                                   -CSolidObject::DAMAGE_COLLISION_OBJECT,
                                   -1);
            }
            // damage the collidee
            if (doCollideeDamage) {
                collidee->DoDamage(DamageArray(collideeImpactDmgMult),
                                   dif * -collideeImpactDmgMult,
                                   NULL,
                                   -CSolidObject::DAMAGE_COLLISION_OBJECT,
                                   -1);
            }
            ...
            collidee->SetVelocityAndSpeed(collidee->speed -
                                          collideeImpactImpulse);
        }
    }
    ...
    for (fi = nearFeatures.begin(); fi != nearFeatures.end(); ++fi) {
        ...
        const float3 dif = (pos - collidee->pos).SafeNormalize();
        const float impactSpeed = -collider->speed.dot(dif);
        const float impactDamageMult = std::min(
          impactSpeed * collider->mass * COLLISION_DAMAGE_MULT, MAX_UNIT_SPEED);
...
CGroundMoveType::GetObstacleAvoidanceDir(const float3& desiredDir)
{
    ...
    const float avoidanceRadius =
      std::max(currentSpeed, 1.0f) * (avoider->radius * 2.0f);
    const float avoiderRadius =
      FOOTPRINT_RADIUS(avoiderMD->xsize, avoiderMD->zsize, 1.0f);

    const vector<CSolidObject*>& objects =
      quadField->GetSolidsExact(avoider->pos,
                                avoidanceRadius,
                                0xFFFFFFFF,
                                CSolidObject::CSTATE_BIT_SOLIDOBJECTS);

    for (vector<CSolidObject*>::const_iterator oi = objects.begin();
         oi != objects.end();
         ++oi) {
        const CSolidObject* avoidee = *oi;
        const MoveDef* avoideeMD = avoidee->moveDef;
        const UnitDef* avoideeUD =
          dynamic_cast<const UnitDef*>(avoidee->GetDef());
        ...
        const bool avoideeMovable =
          (avoideeUD != NULL && !avoideeUD->pushResistant);

        const float3 avoideeVector =
          (avoider->pos + avoider->speed) - (avoidee->pos + avoidee->speed);
        ...
        const float avoideeMassScale =
          avoideeMobile ? (avoidee->mass / avoidanceMassSum) : 1.0f;
        ...
        if (avoideeMobile && avoideeMovable) {
            if (!avoiderMD->avoidMobilesOnPath ||
                (!avoidee->IsMoving() &&
                 avoidee->allyteam == avoider->allyteam)) {
                continue;
            }
        }

        // ignore objects that are more than this many degrees off-center from
        // us NOTE:
        ...
        //   avoidance vector to oscillate --> units with turnInPlace = true
        //   will slow to a crawl as a result
        if (avoider->frontdir.dot(-(avoideeVector / avoideeDist)) <
            MAX_AVOIDEE_COSINE)
            continue;

        if (avoideeDistSq >= Square(std::max(currentSpeed, 1.0f) * GAME_SPEED +
                                    avoidanceRadiusSum))
            continue;
        ...
        if (DEBUG_DRAWING_ENABLED) {
            if (selectedUnitsHandler.selectedUnits.find(owner) !=
                selectedUnitsHandler.selectedUnits.end()) {
                geometricObjects->AddLine(avoider->pos + (UpVector * 20.0f),
                                          avoidee->pos + (UpVector * 20.0f),
                                          3,
                                          1,
                                          4);
            }
        ...
        float avoiderTurnSign = -Sign(avoidee->pos.dot(avoider->rightdir) -
                                      avoider->pos.dot(avoider->rightdir));
        float avoideeTurnSign = -Sign(avoider->pos.dot(avoidee->rightdir) -
        ...
        const float avoidanceCosAngle =
          Clamp(avoider->frontdir.dot(avoidee->frontdir), -1.0f, 1.0f);
        const float avoidanceResponse =
          (1.0f - avoidanceCosAngle * int(avoideeMobile)) + 0.1f;
        const float avoidanceFallOff =
          (1.0f - std::min(1.0f, avoideeDist / (5.0f * avoidanceRadiusSum)));
        ...
        avoidanceVec += (avoidanceDir * avoidanceResponse * avoidanceFallOff *
                         avoideeMassScale);
    }
    ...
    avoidanceDir =
      (mix(desiredDir, avoidanceVec, DESIRED_DIR_WEIGHT)).SafeNormalize();
    avoidanceDir =
      (mix(avoidanceDir, lastAvoidanceDir, LAST_DIR_MIX_ALPHA)).SafeNormalize();
    ...
    if (DEBUG_DRAWING_ENABLED) {
        if (selectedUnitsHandler.selectedUnits.find(owner) !=
            ...
            const int avFigGroupID =
              geometricObjects->AddLine(p0, p1, 8.0f, 1, 4);
            const int adFigGroupID =
              geometricObjects->AddLine(p0, p2, 8.0f, 1, 4);
...
CGroundMoveType::GetNewPath()
{
    ...
    if ((newPathID = pathManager->RequestPath(
        ...
        currWayPoint = pathManager->NextWayPoint(
          owner, newPathID, 0, owner->pos, 1.25f * SQUARE_SIZE, true);
        nextWayPoint = pathManager->NextWayPoint(
          owner, newPathID, 0, currWayPoint, 1.25f * SQUARE_SIZE, true);
...
CGroundMoveType::CanGetNextWayPoint()
{
    ...
    if (currWayPoint.y != -1.0f && nextWayPoint.y != -1.0f) {
        ...
        if (pathManager->PathUpdated(pathID)) {
            ...
            //
            cwp = pathManager->NextWayPoint(
              owner, pathID, 0, pos, 1.25f * SQUARE_SIZE, true);
            nwp = pathManager->NextWayPoint(
              owner, pathID, 0, cwp, 1.25f * SQUARE_SIZE, true);
        }
        ...
        const float waypointDot =
          Clamp(waypointDir.dot(flatFrontDir * dirSign), -1.0f, 1.0f);
...
#else
        if ((currWayPointDist >
             std::max(turnRadius * 2.0f, 1.0f * SQUARE_SIZE)) &&
            (waypointDot >= 0.0f)) {
            return false;
        }
        if ((currWayPointDist >
             std::max(turnRadius * 1.0f, 1.0f * SQUARE_SIZE)) &&
            (waypointDot < 0.0f)) {
            return false;
        }
        if (math::acosf(waypointDot) <
            ((turnRate / SPRING_CIRCLE_DIVS) * (PI + PI))) {
            return false;
        }
#endif

        {
            ...
            const int xmin = std::min(cwp.x / SQUARE_SIZE, pos.x / SQUARE_SIZE),
                      xmax = std::max(cwp.x / SQUARE_SIZE, pos.x / SQUARE_SIZE);
            const int zmin = std::min(cwp.z / SQUARE_SIZE, pos.z / SQUARE_SIZE),
                      zmax = std::max(cwp.z / SQUARE_SIZE, pos.z / SQUARE_SIZE);

            const MoveDef* ownerMD = owner->moveDef;

            for (int x = xmin; x < xmax; x++) {
                for (int z = zmin; z < zmax; z++) {
                    if (ownerMD->TestMoveSquare(
                          owner, x, z, owner->speed, true, true, true)) {
                        continue;
                    }
                    // if still further than SS elmos from waypoint, disallow
                    // skipping note: can somehow cause units to move in circles
                    // near obstacles (mantis3718) if rectangle is too generous
                    // in size
                    if ((pos - cwp).SqLength() > Square(SQUARE_SIZE) &&
                        (pos - cwp).dot(flatFrontDir) >= 0.0f) {
                        return false;
                    }
                }
            }
...

        {
            ...
            // trigger Arrived on the next Update (only if we have non-temporary
            // waypoints) note:
            //   coldet can (very rarely) interfere with this, causing it to
            //   remain false a unit would then keep moving along its final
            //   waypoint-direction forever if atGoal, so we require waypointDir
            //   to always be updated in FollowPath (checking curr == next is
            //   not perfect, becomes true a waypoint too early)
            //
...
CGroundMoveType::GetNextWayPoint()
{
    ...
    if (CanGetNextWayPoint()) {
        ...
        nextWayPoint = pathManager->NextWayPoint(
          owner, pathID, 0, currWayPoint, 1.25f * SQUARE_SIZE, true);
    }

    if (nextWayPoint.x == -1.0f && nextWayPoint.z == -1.0f) {
        Fail(false);
    } else {
        const auto CWP_BLOCK_MASK =
          CMoveMath::SquareIsBlocked(*owner->moveDef, currWayPoint, owner);
        const auto NWP_BLOCK_MASK =
          CMoveMath::SquareIsBlocked(*owner->moveDef, nextWayPoint, owner);

        if ((CWP_BLOCK_MASK & CMoveMath::BLOCK_STRUCTURE) != 0 ||
            (NWP_BLOCK_MASK & CMoveMath::BLOCK_STRUCTURE) != 0) {
...
CGroundMoveType::Arrived(bool callScript)
{
    ...
    if (progressState == Active) {
        ...
        if (owner->team == gu->myTeam) {
            Channels::General->PlayRandomSample(owner->unitDef->sounds.arrived,
                                                owner);
        }
        ...
        if (!owner->commandAI->HasMoreMoveCommands()) {
            ...
            static_cast<CMobileCAI*>(owner->commandAI)
              ->SetFrontMoveCommandPos(owner->pos);
        }
...
CGroundMoveType::HandleObjectCollisions()
{
    ...
    // handle collisions for even-numbered objects on even-numbered frames and
    // vv. (temporal resolution is still high enough to not compromise accuracy
    // much?) if ((collider->id & 1) == (gs->frameNum & 1)) {
    {
        const UnitDef* colliderUD = collider->unitDef;
        ...
        HandleUnitCollisions(
          collider, colliderSpeed, colliderRadius, colliderUD, colliderMD);
        HandleFeatureCollisions(
          collider, colliderSpeed, colliderRadius, colliderUD, colliderMD);

        // blocked square collision (very performance hungry, process only every
        // 2nd game frame) dangerous: reduces effective square-size from 8 to 4,
        // but many ground units can move at speeds greater than half the
        // effective square-size per frame so this risks getting stuck on
        // impassable squares
        const bool squareChange =
          (CGround::GetSquare(owner->pos + owner->speed) !=
           CGround::GetSquare(owner->pos));
        const bool checkAllowed = ((collider->id & 1) == (gs->frameNum & 1));

        if (squareChange || checkAllowed) {
            HandleStaticObjectCollision(owner,
                                        owner,
                                        owner->moveDef,
                                        colliderRadius,
                                        0.0f,
...
// for factories, check if collidee's position is behind us (which means we are
// likely exiting)
//
// NOTE:
//   allow units to move _through_ idle open factories by extending the
//   collidee's footprint such that insideYardMap is true in a larger area
//   (otherwise pathfinder and coldet would disagree) the transition from
//   radius- to footprint-based handling is discontinuous --> cannot mix them
// TODO:
//   increase cost of squares inside open factories so PFS is less likely to
//   path through them
//
#if 0
    ...
    if (checkYardMap || checkTerrain) {
        ...
        // mantis{3614,4217}
        //   we cannot nicely bounce off terrain when checking only the center
        //   square however, testing more squares means CD can (sometimes)
        //   disagree with PFS in narrow passages --> still possible, but have
        //   to ensure we allow only lateral (non-obstructing) bounces
        const int xsh =
          colliderMD->xsizeh *
          (checkYardMap || (checkTerrain * colliderMD->allowTerrainCollisions));
        const int zsh =
          colliderMD->zsizeh *
          (checkYardMap || (checkTerrain * colliderMD->allowTerrainCollisions));
        ...
        if (DEBUG_DRAWING_ENABLED) {
            geometricObjects->AddLine(collider->pos + (UpVector * 25.0f),
                                      collider->pos + (UpVector * 100.0f),
                                      3,
                                      1,
                                      4);
        }
        ...
        for (int z = zmin; z <= zmax; z++) {
            for (int x = xmin; x <= xmax; x++) {
                ...
                if (checkTerrain) {
                    if (CMoveMath::GetPosSpeedMod(
                          *colliderMD, xabs, zabs, speed2D) > 0.01f)
                        continue;
                } else {
                    if ((CMoveMath::SquareIsBlocked(
                           *colliderMD, xabs, zabs, collider) &
                         CMoveMath::BLOCK_STRUCTURE) == 0)
                        continue;
                }

                const float3 squarePos =
                  float3(xabs * SQUARE_SIZE + (SQUARE_SIZE >> 1),
                         collider->pos.y,
                         zabs * SQUARE_SIZE + (SQUARE_SIZE >> 1));
                ...
                // RHS magic constant is the radius of a square
                // (sqrt(2*(SQUARE_SIZE>>1)*(SQUARE_SIZE>>1)))
                const float squareColRadiusSum =
                  colliderRadius + 5.656854249492381f;
                const float squareSepDistance = squareVec.Length2D() + 0.1f;
                const float squarePenDistance =
                  std::min(squareSepDistance - squareColRadiusSum, 0.0f);
                // const float  squareColSlideSign =
                // -Sign(squarePos.dot(collider->rightdir) -
                // (collider->pos).dot(collider->rightdir));

                // this tends to cancel out too much on average
                // strafeVec += (collider->rightdir * sqColSlideSign);
                bounceVec +=
                  (collider->rightdir *
                   (collider->rightdir.dot(squareVec / squareSepDistance)));
        ...
        if (sqrPenDistance.y > 0.0f) {
            ...
            const float strafeSign =
              -Sign(sqrSumPosition.dot(collider->rightdir) -
                    (collider->pos).dot(collider->rightdir));
            const float strafeScale =
              std::min(std::max(currentSpeed * 0.0f, maxSpeedDef),
                       std::max(0.1f, -sqrPenDistance.x * 0.5f));
            const float bounceScale =
              std::min(std::max(currentSpeed * 0.0f, maxSpeedDef),
                       std::max(0.1f, -sqrPenDistance.x * 0.5f));

            // in FPS mode, normalize {strafe,bounce}Scale and multiply by
            // maxSpeedDef (otherwise it would be possible to slide along map
            // edges at above-normal speeds, etc.)
            const float fpsStrafeScale =
              (strafeScale / (strafeScale + bounceScale)) * maxSpeedDef;
            const float fpsBounceScale =
              (bounceScale / (strafeScale + bounceScale)) * maxSpeedDef;

            strafeVec = collider->rightdir * strafeSign;
            strafeVec = strafeVec.SafeNormalize2D() *
                        mix(strafeScale,
                            fpsStrafeScale,
                            owner->UnderFirstPersonControl());
            ...
            if (colliderMD->TestMoveSquare(collider,
                                           collider->pos + strafeVec +
                                             bounceVec,
                                           collider->speed,
                                           checkTerrain,
                                           checkYardMap,
        ...
        const float colSlideSign = -Sign(collidee->pos.dot(collider->rightdir) -
                                         collider->pos.dot(collider->rightdir));

        const float strafeScale =
          std::min(currentSpeed, std::max(0.0f, -penDistance * 0.5f)) *
          (1 - checkYardMap * false);
        const float bounceScale =
          std::min(currentSpeed, std::max(0.0f, -penDistance)) *
          (1 - checkYardMap * true);

        const float3 strafeVec =
          (collider->rightdir * colSlideSign) * strafeScale;
        const float3 bounceVec = (separationVector / sepDistance) * bounceScale;

        if (colliderMD->TestMoveSquare(collider,
                                       collider->pos + strafeVec + bounceVec,
                                       collider->speed,
                                       true,
                                       true,
                                       true)) {
            ...
            // ChangeSpeed calculates speedMod without checking squares for
            // *structure* blockage (so that a unit can free itself if it ends
            // up within the footprint of a structure) this means deltaSpeed
            // will be non-zero if stuck on an impassable square and hence the
            // new speedvector which is constructed from deltaSpeed --> we would
            // simply keep moving forward through obstacles if not counteracted
            // by this
                                      ...
                                      const MoveDef* colliderMD)
{
    ...
    const std::vector<CUnit*> nearUnits =
      quadField->GetUnitsExact(collider->pos, searchRadius);
    ...
    for (CUnit* collidee : nearUnits) {
        ...
        const bool unloadingCollidee =
          (collidee->unloadingTransportId == collider->id);
        const bool unloadingCollider =
        ...
        // don't push/crush either party if the collidee does not block the
        // collider (or vv.)
        if (colliderMobile &&
            CMoveMath::IsNonBlocking(*colliderMD, collidee, collider))
            continue;
        if (collideeMobile &&
            CMoveMath::IsNonBlocking(*collideeMD, collider, collidee))
            continue;
        ...
        if (collider->GetTransporter() == collidee)
            continue;
        if (collidee->GetTransporter() != NULL)
            continue;
        ...
        const float separationMinDistSq =
          (colliderRadius + collideeRadius) * (colliderRadius + collideeRadius);
        ...
        if (collideeMobile) {
            const CGroundMoveType* gmt =
              static_cast<CGroundMoveType*>(collidee->moveType);
            if (collider->moveType->goalPos.SqDistance2D(
                  collidee->moveType->goalPos) < (PI * PI)) {
                if (collider->IsMoving() &&
                    collider->moveType->progressState == AMoveType::Active) {
                    if (collidee->moveType->progressState == AMoveType::Done) {
                        if (!collidee->IsMoving() &&
                            UNIT_CMD_QUE_SIZE(collidee) == 0) {
                            atEndOfPath = true;
                            atGoal = true;
                        }
                        // We're in a traffic jam so ignore current way point
                        // and go directly to the next one
                    } else if (collidee->moveType->progressState ==
                                 AMoveType::Active &&
                               gmt->currWayPoint == nextWayPoint) {
                        currWayPoint.y = -1.0f;
                    }
                }
            }
        ...
        // FIXME:
        //   allowPushingEnemyUnits is (now) useless because alliances are
        //   bi-directional ie. if !alliedCollision, pushCollider and
        //   pushCollidee BOTH become false and the collision is treated
        //   normally --> not what we want here, but the desired behavior
        //   (making each party stop and block the other) has many corner-cases
        //   this also happens when both parties are pushResistant --> make each
        //   respond to the other as a static obstacle so the tags still have
        //   some effect
        pushCollider =
          pushCollider && (alliedCollision || modInfo.allowPushingEnemyUnits ||
                           !collider->blockEnemyPushing);
        pushCollidee =
          pushCollidee && (alliedCollision || modInfo.allowPushingEnemyUnits ||
                           !collidee->blockEnemyPushing);
        pushCollider = pushCollider && (!collider->beingBuilt &&
                                        !collider->UsingScriptMoveType() &&
                                        !colliderUD->pushResistant);
        pushCollidee = pushCollidee && (!collidee->beingBuilt &&
                                        !collidee->UsingScriptMoveType() &&
        ...
        if ((!collideeMobile && !collideeUD->IsAirUnit()) ||
            ...
            HandleStaticObjectCollision(collider,
                                        collidee,
                                        colliderMD,
                                        colliderRadius,
                                        collideeRadius,
        ...
        const float colliderRelRadius =
          colliderRadius / (colliderRadius + collideeRadius);
        const float collideeRelRadius =
          collideeRadius / (colliderRadius + collideeRadius);
        const float collisionRadiusSum =
          modInfo.allowUnitCollisionOverlap
            ? (colliderRadius * colliderRelRadius +
               collideeRadius * collideeRelRadius)
            : (colliderRadius + collideeRadius);

        const float sepDistance = separationVector.Length() + 0.1f;
        const float penDistance =
          std::max(collisionRadiusSum - sepDistance, 1.0f);
        const float sepResponse =
          std::min(SQUARE_SIZE * 2.0f, penDistance * 0.5f);

        const float3 sepDirection = (separationVector / sepDistance);
        const float3 colResponseVec = sepDirection * XZVector * sepResponse;

        const float m1 = collider->mass, m2 = collidee->mass,
                    v1 = std::max(1.0f, colliderSpeed),
                    v2 = std::max(1.0f, collideeSpeed),
                    c1 = 1.0f + (1.0f - math::fabs(collider->frontdir.dot(
                                          -sepDirection))) *
                                  5.0f,
        ...
        const float colliderMassScale =
          Clamp(1.0f - r1, 0.01f, 0.99f) * (modInfo.allowUnitCollisionOverlap
                                              ? (1.0f / colliderRelRadius)
                                              : 1.0f);
        const float collideeMassScale =
          Clamp(1.0f - r2, 0.01f, 0.99f) * (modInfo.allowUnitCollisionOverlap
                                              ? (1.0f / collideeRelRadius)
                                              : 1.0f);
        ...
        // avoid deadlocks
        const float colliderSlideSign =
          Sign(separationVector.dot(collider->rightdir));
        const float collideeSlideSign =
          Sign(-separationVector.dot(collidee->rightdir));

        const float3 colliderPushVec =
          colResponseVec * colliderMassScale * int(!ignoreCollidee);
        const float3 collideePushVec = -colResponseVec * collideeMassScale;
        const float3 colliderSlideVec =
          collider->rightdir * colliderSlideSign * (1.0f / penDistance) * r2;
        const float3 collideeSlideVec =
          collidee->rightdir * collideeSlideSign * (1.0f / penDistance) * r1;
        ...
        if ((pushCollider || !pushCollidee) && colliderMobile) {
            if (colliderMD->TestMoveSquare(
                  collider, collider->pos + colliderMoveVec, colliderMoveVec)) {
                collider->Move(colliderMoveVec, true);
            }
        }

        if ((pushCollidee || !pushCollider) && collideeMobile) {
            if (collideeMD->TestMoveSquare(
                  collidee, collidee->pos + collideeMoveVec, collideeMoveVec)) {
                                         ...
                                         const MoveDef* colliderMD)
{
    ...
    const std::vector<CFeature*> nearFeatures =
      quadField->GetFeaturesExact(collider->pos, searchRadius);
    ...
    for (fit = nearFeatures.begin(); fit != nearFeatures.end(); ++fit) {
        ...
        // const float collideeRadius = FOOTPRINT_RADIUS(collideeFD->xsize,
        // collideeFD->zsize, 0.75f);
        const float collideeRadius =
          FOOTPRINT_RADIUS(collidee->xsize, collidee->zsize, 0.75f);
        ...
        const float3 separationVector = collider->pos - collidee->pos;
        const float separationMinDistSq =
          collisionRadiusSum * collisionRadiusSum;
        ...
        const float m1 = collider->mass, m2 = collidee->mass * 10000.0f,
                    v1 = std::max(1.0f, colliderSpeed), v2 = 1.0f,
                    c1 = (1.0f -
                          math::fabs(collider->frontdir.dot(-sepDirection))) *
                         5.0f,
                    c2 = (1.0f -
                          math::fabs(-collider->frontdir.dot(sepDirection))) *
                         5.0f,
...
CGroundMoveType::KeepPointingTo(float3 pos, float distance, bool aggressive)
{
    ...
    const short heading = GetHeadingFromVector(dir2.x, dir2.z) -
    ...
    if (!frontWeapon->TestRange(mainHeadingPos,
                                SWeaponTarget(mainHeadingPos, true))) {
        progressState = Active;
    }
}
...
CGroundMoveType::SetMainHeading()
{
    ...
    const float3 dir2 =
      ((mainHeadingPos - owner->pos) * XZVector).SafeNormalize();
    ...
    short newHeading = GetHeadingFromVector(dir2.x, dir2.z) -
                       GetHeadingFromVector(dir1.x, dir1.z);
...
CGroundMoveType::OnSlope(float minSlideTolerance)
{
    ...
    // if minSlideTolerance is LEQ 0, do not multiply maxSlope by
    // ud->slideTolerance (otherwise the unit could stop on an invalid path
    // location, and be teleported back)
    const float slopeMul =
      mix(ud->slideTolerance, 1.0f, (minSlideTolerance <= 0.0f));
...
CGroundMoveType::AdjustPosToWaterLine()
{
    ...
    if (modInfo.allowGroundUnitGravity) {
        if (owner->unitDef->floatOnWater) {
            owner->Move(UpVector * (std::max(CGround::GetHeightReal(
                                               owner->pos.x, owner->pos.z),
        ...
        owner->Move(UpVector * (GetGroundHeight(owner->pos) - owner->pos.y),
                    true);
    }
}
...
CGroundMoveType::UpdateDirectControl()
{
    ...
    currWayPoint =
      owner->frontdir * XZVector * mix(100.0f, -100.0f, wantReverse);
    currWayPoint = (owner->pos + currWayPoint).cClampInBounds();

    if (unitCon.forward || unitCon.back) {
        ChangeSpeed((maxSpeed * unitCon.forward) +
                      (maxReverseSpeed * unitCon.back),
                    wantReverse,
                    true);
    } else {
...
CGroundMoveType::GetNewSpeedVector(const float hAcc, const float vAcc) const
{
    ...
    if (modInfo.allowGroundUnitGravity) {
        ...
        if (owner->moveDef->speedModClass != MoveDef::Hover ||
            !modInfo.allowHoverUnitStrafing) {
            const float3 accelVec = (gndTangVec * hAcc) + (UpVector * vAcc);
            const float3 speedVec = (horSpeed + verSpeed) + accelVec;

            speedVector +=
              (flatFrontDir * speedVec.dot(flatFrontDir)) * dragCoeff;
            speedVector += (UpVector * speedVec.dot(UpVector));
        } else {
            // TODO: also apply to non-hovercraft on low-gravity maps?
            speedVector +=
              (gndTangVec *
               (std::max(0.0f, owner->speed.dot(gndTangVec) + hAcc * 1.0f))) *
              dragCoeff;
            speedVector +=
              (horSpeed -
               gndTangVec * (/*std::max(0.0f,*/ owner->speed.dot(gndTangVec) -
                             hAcc * 0.0f)) *
              slipCoeff;
            speedVector +=
              (UpVector * (owner->speed + UpVector * vAcc).dot(UpVector));
        }
        ...
        if ((owner->pos.y + speedVector.y) <= newGroundHeight) {
            speedVector.y =
              std::min(newGroundHeight - owner->pos.y,
                       math::fabs(newGroundHeight - oldGroundHeight));
        }
    } else {
                                ...
                                const float3& newSpeedVector)
{
    ...
    owner->UpdatePhysicalStateBit(CSolidObject::PSTATE_BIT_MOVING,
                                  newSpeed > 0.01f);
    ...
    if (!newSpeedVector.same(ZeroVector)) {
        ...
        //
        if (!pathController->IgnoreTerrain(*owner->moveDef, owner->pos) &&
            !owner->moveDef->TestMoveSquare(
              owner, owner->pos, owner->speed, true, false, true)) {
            bool updatePos = false;

            for (unsigned int n = 1; n <= SQUARE_SIZE; n++) {
                if (!updatePos && (updatePos = owner->moveDef->TestMoveSquare(
                                     owner,
                                     owner->pos + owner->rightdir * n,
                                     owner->speed,
                                     true,
                                     false,
                                     true))) {
        ...
        // assert(owner->moveDef->TestMoveSquare(owner, owner->pos,
        // owner->speed, true, false, true));
    }
...
CGroundMoveType::WantReverse(const float3& wpDir, const float3& ffDir) const
{
    ...
    const float goalDist =
      (goalPos - owner->pos)
        .Length2D(); // use *final* WP for ETA calcs; in elmos
    const float goalFwdETA = (goalDist / maxSpeed); // in frames (simplistic)
    const float goalRevETA =
      (goalDist / maxReverseSpeed); // in frames (simplistic)

    const float waypointAngle = Clamp(wpDir.dot(owner->frontdir),
                                      -1.0f,
                                      0.0f); // clamp to prevent NaN's; [-1, 0]
    const float turnAngleDeg =
      math::acosf(waypointAngle) * (180.0f / PI); // in degrees; [90.0, 180.0]
    const float fwdTurnAngle =
      (turnAngleDeg / 360.0f) * SPRING_CIRCLE_DIVS; // in "headings"
    const float revTurnAngle =
      SHORTINT_MAXVALUE - fwdTurnAngle; // 180 deg - angle

    // values <= 0 preserve default behavior
    if (maxReverseDist > 0.0f && minReverseAngle > 0.0f)
        return (currWayPointDist <= maxReverseDist &&
                turnAngleDeg >= minReverseAngle);

    // units start accelerating before finishing the turn, so subtract something
    const float turnTimeMod = 5.0f;
    const float fwdTurnAngleTime =
      std::max(0.0f, (fwdTurnAngle / turnRate) - turnTimeMod); // in frames
    const float revTurnAngleTime =
      std::max(0.0f, (revTurnAngle / turnRate) - turnTimeMod);

    const float apxFwdSpdAfterTurn =
      std::max(0.0f, currentSpeed - 0.125f * (fwdTurnAngleTime * decRate));
    const float apxRevSpdAfterTurn =
      std::max(0.0f, currentSpeed - 0.125f * (revTurnAngleTime * decRate));

    const float fwdDecTime = (reversing * apxFwdSpdAfterTurn) / decRate;
    const float revDecTime = (!reversing * apxRevSpdAfterTurn) / decRate;
    const float fwdAccTime =
      (maxSpeed - !reversing * apxFwdSpdAfterTurn) / accRate;
    const float revAccTime =
      (maxReverseSpeed - reversing * apxRevSpdAfterTurn) / accRate;

    const float fwdETA =
      goalFwdETA + fwdTurnAngleTime + fwdAccTime + fwdDecTime;
    const float revETA =
      goalRevETA + revTurnAngleTime + revDecTime + revAccTime;
...
#undef MEMBER_LITERAL_HASH
    ...
    float* floatMemberPtrs[] = {
        ...
        &maxReverseDist, &minReverseAngle, &maxReverseSpeed,
    };

    // special cases
    if (memberHash == floatMemberHashes[MAXREVERSESPEED_MEMBER_IDX]) {
        *(floatMemberPtrs[MAXREVERSESPEED_MEMBER_IDX]) =
    ...
    for (unsigned int n = 0;
         n < sizeof(boolMemberPtrs) / sizeof(boolMemberPtrs[0]);
         n++) {
    ...
    for (unsigned int n = 0;
         n < sizeof(floatMemberPtrs) / sizeof(floatMemberPtrs[0]);
         n++) {
