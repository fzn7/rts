...
bool
AAirMoveType::UseSmoothMesh() const
{
    if (!useSmoothMesh)
        return false;
    ...
    const bool onTransportMission =
      ...
      ((owner->commandAI->commandQue.front().GetID() == CMD_LOAD_UNITS) ||
       (owner->commandAI->commandQue.front().GetID() == CMD_UNLOAD_UNIT));
    const bool forceDisableSmooth =
      onTransportMission || (aircraftState != AIRCRAFT_FLYING);
    return !forceDisableSmooth;
}

void
AAirMoveType::DependentDied(CObject* o)
{
    if (o == lastColWarning) {
        lastColWarning = NULL;
        lastColWarningType = 0;
...
AAirMoveType::UpdateLanded()
{
    ...
    owner->SetVelocityAndSpeed(
      owner->speed +
      owner->GetDragAccelerationVec(float4(0.0f, 0.0f, 0.0f, 0.1f)));
    owner->Move(UpVector * (std::max(curHeight, minHeight) - owner->pos.y),
                true);
...
void
AAirMoveType::LandAt(float3 pos, float distanceSq)
{
    if (distanceSq < 0.0f)
    ...
    const float gh =
      CGround::GetHeightReal(reservedLandingPos.x, reservedLandingPos.z);
    wantedHeight = reservedLandingPos.y -
                   (owner->unitDef->canSubmerge ? gh : std::max(0.0f, gh));
}

void
AAirMoveType::UpdateLandingHeight()
{
    const float gh =
      CGround::GetHeightReal(reservedLandingPos.x, reservedLandingPos.z);
    reservedLandingPos.y =
      wantedHeight + (owner->unitDef->canSubmerge ? gh : std::max(0.0f, gh));
}
...
AAirMoveType::UpdateLanding()
{
    ...
    if (distSq <= radiusSq ||
        (distSq < landRadiusSq && localAltitude < wantedHeight + radius)) {
...
void
AAirMoveType::SetWantedAltitude(float altitude)
{
    if (altitude == 0.0f) {
...
void
AAirMoveType::SetDefaultAltitude(float altitude)
{
    wantedHeight = altitude;
...
void
AAirMoveType::CheckForCollision()
{
    if (!collide)
        return;
    ...
    const std::vector<CUnit*>& others =
      quadField->GetUnitsExact(midTestPos, 115.0f);
    ...
    for (CUnit* unit : others) {
        ...
        const float minOrtoDif =
          (unit->radius + owner->radius) * 2.0f + frontLength * 0.1f + 10;
