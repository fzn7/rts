...
{
    // queued move-commands (or active build/repair/etc-commands) mean unit has
    // to stay airborne
...
CHoverAirMoveType::SetState(AircraftState newState)
{
    ...
    // Cruise as default
    // FIXME: RHS overrides FLY_ATTACKING and FLY_CIRCLING (via
    // UpdateFlying-->ExecuteStop)
...
#define NOZERO(x) std::max(x, 0.0001f)
    ...
    wantedSpeed =
      (randomWind * math::fabs(owner->unitDef->dlHoverFactor) * 0.5f);
    wantedSpeed += (smoothstep(0.0f, 20.0f * 20.0f, (goalPos - owner->pos)) *
                    (goalPos - owner->pos));
...
CHoverAirMoveType::UpdateFlying()
{
    ...
    // don't change direction for waypoints we just flew over and missed
    // slightly
    if (flyState != FLY_LANDING && owner->commandAI->HasMoreMoveCommands()) {
        float3 goalDir = goalVec;

        if ((goalDir != ZeroVector) &&
            (goalVec.dot(goalDir.UnsafeANormalize()) < 1.0f)) {
    ...
    const float gHeight =
      UseSmoothMesh() ? std::max(smoothGround->GetHeight(pos.x, pos.z),
                                 CGround::GetApproximateHeight(pos.x, pos.z))
                      : CGround::GetHeightAboveWater(pos.x, pos.z);

    const bool closeToGoal =
      (flyState == FLY_ATTACKING)
        ? (goalDistSq2D < (400.0f))
        : (goalDistSq2D < (maxDrift * maxDrift)) &&
            (math::fabs(gHeight + wantedHeight - pos.y) < maxDrift);
    ...
    if (closeToGoal) {
        switch (flyState) {
            case FLY_CRUISING: {
                ...
                const int topCmdID =
                  cmdQue.empty() ? 0 : cmdQue.front().GetID();

                // NOTE: should CMD_LOAD_ONTO be here?
                const bool isTransporter = owner->unitDef->IsTransportUnit();
                const bool hasLoadCmds =
                  isTransporter &&
                  (topCmdID == CMD_LOAD_ONTO || topCmdID == CMD_LOAD_UNITS);
                // [?] transport aircraft need some time to detect that they can
                // pickup
                const bool canLoad =
                  isTransporter && (++waitCounter < ((GAME_SPEED << 1) - 5));
            ...
            case FLY_CIRCLING: {
                if ((++waitCounter) > ((GAME_SPEED * 3) + 10)) {
                    if (airStrafe) {
                        ...
                        static const CMatrix44f rot(
                          0.0f, fastmath::PI / 4.0f, 0.0f);

                        // make sure the point is on the circle, go there in a
                        // straight line
                        goalPos = circlingPos + (rot.Mul(relPos.Normalize2D()) *
                                                 goalDistance);
                    }
                    waitCounter = 0;
                }
            ...
            case FLY_ATTACKING: {
                if (airStrafe) {
                    ...
                    goalPos = circlingPos +
                              (rot.Mul(relPos.Normalize2D()) * goalDistance);
                }
            } break;
    ...
    const float goalSpeed = (maxSpeed) * (goalDist > brakeDist) +
    ...
    // Point toward goal or forward - unless we just passed it to get to another
    // goal
    if ((flyState == FLY_ATTACKING) || (flyState == FLY_CIRCLING)) {
        goalVec = circlingPos - pos;
    } else {
        const bool b0 = (flyState != FLY_LANDING &&
...
CHoverAirMoveType::UpdateLanding()
{
    ...
    if (!HaveLandingPos()) {
        if (CanLandAt(pos)) {
            ...
            if (goalPos.SqDistance2D(pos) < (30.0f * 30.0f)) {
                ...
                // exact landing pos failed, make sure finishcommand is called
                // anyway
    ...
    if (distSq2D > landRadiusSq) {
        ...
        wantedHeight = std::min(
          (orgWantedHeight - wantedHeight) * distSq2D / altitude + wantedHeight,
          orgWantedHeight);
        UpdateFlying();
...
CHoverAirMoveType::UpdateHeading()
{
    if (aircraftState == AIRCRAFT_TAKEOFF &&
        !owner->unitDef->factoryHeadingTakeoff)
        return;
    ...
    const short deltaHeading =
      forceHeading ? (forceHeadingTo - heading) : (wantedHeading - heading);
...
CHoverAirMoveType::UpdateBanking(bool noBanking)
{
    ...
    if (!owner->upright) {
        ...
        if (aircraftState == AIRCRAFT_FLYING && flyState == FLY_ATTACKING &&
            circlingPos.y < owner->pos.y) {
            wantedPitch =
              (circlingPos.y - owner->pos.y) / circlingPos.distance(owner->pos);
        }
    ...
    // always positive
    const float bankLimit =
      std::min(1.0f, goalPos.SqDistance2D(owner->pos) * Square(0.15f));
    ...
    upDir =
      upDir * math::cos(currentBank) + rightDir2D * math::sin(currentBank);
                                       ...
                                       float curVertSpeed) const
{
    ...
    if (curRelHeight < wh) {
        ...
        if ((spd.y > 0.0001f) &&
            (((wh - curRelHeight) / spd.y) * accRate * 1.5f) < spd.y) {
            ws = 0.0f;
        }
    } else {
        ws = -altitudeRate;
        ...
        if ((spd.y < -0.0001f) &&
            (((wh - curRelHeight) / spd.y) * accRate * 0.7f) < -spd.y) {
            ws = 0.0f;
        }
    }
    ...
    if (math::fabs(wh - curRelHeight) > 2.0f) {
        if (spd.y > ws) {
            owner->SetVelocity(
              (spd * XZVector) +
              (UpVector * std::max(ws, spd.y - accRate * 1.5f)));
        } else {
            ...
            owner->SetVelocity(
              ...
              (UpVector *
               std::min(ws,
                        spd.y +
                          accRate * ((curRelHeight < 20.0f) ? 2.0f : 0.7f))));
        }
    } else {
    ...
    if (deltaDotSpeed >= 0.0f) {
        ...
        if (deltaSpeedSq < Square(accRate)) {
            ...
            if (deltaSpeedSq > 0.0f) {
                owner->SetVelocity(
                  spd + (deltaSpeed / math::sqrt(deltaSpeedSq) * accRate));
            }
        }
    } else {
        // deccelerate
        if (deltaSpeedSq < Square(decRate)) {
            ...
            if (deltaSpeedSq > 0.0f) {
                owner->SetVelocity(
                  spd + (deltaSpeed / math::sqrt(deltaSpeedSq) * decRate));
            }
        }
    }
...
CHoverAirMoveType::UpdateMoveRate()
{
    ...
    // currentspeed is not used correctly for vertical movement, so compensate
    // with this hax
    if ((aircraftState == AIRCRAFT_LANDING) ||
        (aircraftState == AIRCRAFT_TAKEOFF)) {
        curRate = 1;
...
#define SPIN_DIR(o) ((o->id & 1) * 2 - 1)
                wantedHeading =
                  GetHeadingFromVector(owner->rightdir.x * SPIN_DIR(owner),
                                       owner->rightdir.z * SPIN_DIR(owner));
                wantedHeight = 0.0f;
#undef SPIN_DIR
            }
...
CHoverAirMoveType::Land()
{
    if (aircraftState == AAirMoveType::AIRCRAFT_HOVERING) {
        SetState(AAirMoveType::AIRCRAFT_FLYING); // switch to flying, it
                                                 // performs necessary checks to
                                                 // prepare for landing
    }
}
...
CHoverAirMoveType::HandleCollisions(bool checkCollisions)
{
    ...
    if (pos != oldPos) {
        ...
        // includes an extra condition for transports, which are exempt while
        // loading
        if (!forceHeading && checkCollisions) {
            ...
            for (CUnit* unit : nearUnits) {
                ...
                if (unit->mass >= CSolidObject::DEFAULT_MASS ||
                    unit->immobile) {
...
#define WANTEDHEIGHT_MEMBER_IDX 0
    ...
    static const unsigned int floatMemberHashes[] = {
        MEMBER_LITERAL_HASH("wantedHeight"), MEMBER_LITERAL_HASH("accRate"),
        MEMBER_LITERAL_HASH("decRate"),      MEMBER_LITERAL_HASH("turnRate"),
        MEMBER_LITERAL_HASH("altitudeRate"), MEMBER_LITERAL_HASH("currentBank"),
        MEMBER_LITERAL_HASH("currentPitch"), MEMBER_LITERAL_HASH("maxDrift"),
    };
