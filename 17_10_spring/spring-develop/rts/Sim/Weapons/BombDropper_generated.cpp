...
CBombDropper::GetPredictedImpactTime(float3 impactPos) const
{
    ...
    const float g = (weaponDef->myGravity == 0) ? mapInfo->map.gravity
                                                : -weaponDef->myGravity;
...
CBombDropper::TestRange(const float3 pos, const SWeaponTarget& trg) const
{
    ...
    const float dropDist =
      std::max(1, salvoSize - 1) * salvoDelay * owner->speed.Length2D() * 0.5f;

    // torpedoes especially should not be dropped if the
    // target position is already behind owner's position
    const float torpDist =
      torpMoveRange * (owner->frontdir.dot(pos - aimFromPos) > 0.0f);

    return (pos.SqDistance2D(aimFromPos + owner->speed * fallTime) <
            Square(dropDist + torpDist));
}
...
CBombDropper::FireImpl(const bool scriptCall)
{
    ...
    if (dropTorpedoes) {
        ...
        params.ttl = (weaponDef->flighttime == 0)
                       ? ((range / projectileSpeed) + 15 + predict)
        ...
        // fudge a bit better lateral aim to compensate for imprecise aircraft
        // steering
        ...
        params.ttl = 1000;
        params.gravity = (weaponDef->myGravity == 0) ? mapInfo->map.gravity
                                                     : -weaponDef->myGravity;
