...
{
    if (CACHE_BASE.empty()) {
        CACHE_BASE = dataDirsAccess.LocateDir(
          FileSystem::GetCacheDir() + "/analyzedResourceMaps/",
    ...
    minIncomeForSpot = 50;
    // if more spots than that are found the map is considered a resource-map
    // (eg. speed-metal), tweak this as needed
    maxSpots = 10000;

    const CResourceDescription* resource =
      resourceHandler->GetResource(resourceId);
                                     ...
                                     const UnitDef* extractor) const
{
    ...
    for (size_t i = 0; i < vectoredSpots.size(); ++i) {
        if (extractor != NULL) {
            spotCoords = CGameHelper::ClosestBuildSite(
              team, extractor, vectoredSpots[i], maxDivergence, 2);
        } else {
        ...
        if (spotCoords.x >= 0.0f) {
            ...
            float spotScore =
              vectoredSpots[i].y / distance /* / (myThreat + 10)*/;
            // int numEnemies = ai->cheat->GetEnemyUnits(&ai->unitIDs[0],
            // vectoredSpots[i], xtractorRadius * 2);
            ...
            // 			bool b3 = (myThreat <= (ai->tm->GetAverageThreat()
            // * 1.5)); 			bool b4 =
            // (ai->unitHandler->TaskPlanExist(spotCoords, extractor));
...
CResourceMapAnalyzer::GetResourcePoints()
{
    ...
    const unsigned char* resourceMapArray =
      resourceHandler->GetResourceMap(resourceId);
    ...
    for (int y = 0; y < mapHeight; y++) {
        for (int x = 0; x < mapWidth; x++) {
            ...
            if (x == 0 && y == 0)
                for (int sy = y - xtractorRadius, a = 0;
                     sy <= y + xtractorRadius;
                     sy++, a++) {
                    if (sy >= 0 && sy < mapHeight) {
                        for (int sx = x - xend[a]; sx <= x + xend[a]; sx++) {
                            if (sx >= 0 && sx < mapWidth) {
                                // get the resources from all pixels around the
                                // extractor radius
            ...
            if (maxResource < totalResources) {
                // find the spot with the highest resource value to set as the
                // map's max
    ...
    for (int a = 0; a < maxSpots; a++) {
        ...
        if (tempResources < minIncomeForSpot) {
            // if the spots get too crappy it will stop running the loops to
            // speed it all up
            stopMe = 1;
        }
        ...
        if (!stopMe) {
            ...
            const CResourceDescription* resource =
              resourceHandler->GetResource(resourceId);
            bufferSpot.y =
              tempResources * (resource->maxWorth) * maxResource / 255;
            ...
            // small speedup of "wipes the resources around the spot so it is
            // not counted twice"
            for (int sy = coordZ - xtractorRadius, a = 0;
                 sy <= coordZ + xtractorRadius;
                 sy++, a++) {
                if (sy >= 0 && sy < mapHeight) {
                    ...
                    for (int xClear = clearXStart; xClear <= clearXEnd;
                         xClear++) {
                        // wipes the resources around the spot so it is not
                        // counted twice
            ...
            // redo the whole averaging process around the picked spot so other
            // spots can be found around it
            for (int y = coordZ - doubleRadius; y <= coordZ + doubleRadius;
                 y++) {
                if (y >= 0 && y < mapHeight) {
                    for (int x = coordX - doubleRadius;
                         x <= coordX + doubleRadius;
                         x++) {
                        if (x >= 0 && x < mapWidth) {
                            totalResources = 0;
                                ...
                                for (int sy = y - xtractorRadius, a = 0;
                                    ...
                                    if (sy >= 0 && sy < mapHeight) {
                                        for (int sx = x - xend[a];
                                             ...
                                             sx++) {
                                            if (sx >= 0 && sx < mapWidth) {
                                                // get the resources from all
                                                // pixels around the extractor
                                                // radius
                                                totalResources +=
                                                  rexArrayA[sy * mapWidth + sx];
                                            }
                                        }
                                    }
                                }
                            ...
                            if (x > 0) {
                                ...
                                for (int sy = y - xtractorRadius, a = 0;
                                    ...
                                    if (sy >= 0 && sy < mapHeight) {
                                        ...
                                        if (addX < mapWidth) {
                                            totalResources +=
                                              rexArrayA[sy * mapWidth + addX];
                                        }
                                        if (remX >= 0) {
                                            totalResources -=
                                              rexArrayA[sy * mapWidth + remX];
                                        }
                                    }
                                }
                            } else if (y > 0) {
                                // x == 0 here
                                totalResources =
                                  tempAverage[(y - 1) * mapWidth];
                                // remove the top half
                                int a = xtractorRadius;

                                for (int sx = 0; sx <= xtractorRadius;
                                     sx++, a++) {
                                    if (sx < mapWidth) {
                                        int remY = y - xend[a] - 1;
                            ...
                            rexArrayB[y * mapWidth + x] =
                              totalResources * 255 / maxResource;
                        }
                    }
                }
            }
...
CResourceMapAnalyzer::LoadResourceMap()
{
    ...
    if (cacheFile != NULL) {
        try {
            ...
            for (int i = 0; i < numSpotsFound; i++) {
                fileReadChecked(
                  &vectoredSpots[i], sizeof(float3), 1, cacheFile);
            }
            loaded = true;
        } catch (const std::runtime_error& err) {
            LOG_L(L_WARNING,
