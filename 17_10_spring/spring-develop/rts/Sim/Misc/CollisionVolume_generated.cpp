...
CollisionVolume::InitSphere(float radius)
{
    ...
    InitShape(OnesVector * radius * 2.0f,
              ZeroVector,
              COLVOL_TYPE_SPHERE,
              COLVOL_HITTEST_CONT,
              COLVOL_AXIS_Z);
...
{
    InitShape(
      scales, offsets, COLVOL_TYPE_BOX, COLVOL_HITTEST_CONT, COLVOL_AXIS_Z);
}
...
CollisionVolume::SetBoundingRadius()
{
    ...
    switch (volumeType) {
        case COLVOL_TYPE_BOX: {
            ...
            volumeBoundingRadiusSq =
              halfAxisScalesSqr.x + halfAxisScalesSqr.y + halfAxisScalesSqr.z;
            volumeBoundingRadius = math::sqrt(volumeBoundingRadiusSq);
        } break;
        case COLVOL_TYPE_CYLINDER: {
            const float prhs =
              halfAxisScales[volumeAxes[0]]; // primary axis half-scale
            const float sahs =
              halfAxisScales[volumeAxes[1]]; // 1st secondary axis half-scale
            const float sbhs =
              halfAxisScales[volumeAxes[2]]; // 2nd secondary axis half-scale
            const float mshs =
              std::max(sahs, sbhs); // max. secondary axis half-scale
        ...
        case COLVOL_TYPE_ELLIPSOID: {
            volumeBoundingRadius = std::max(
              halfAxisScales.x, std::max(halfAxisScales.y, halfAxisScales.z));
            volumeBoundingRadiusSq =
              volumeBoundingRadius * volumeBoundingRadius;
        } break;
    }
...
CollisionVolume::FixTypeAndScale(float3& scales)
{
    ...
    if (volumeType == COLVOL_TYPE_ELLIPSOID) {
        if (scales.x == scales.y && scales.y == scales.z) {
            ...
            const float minValue =
              std::fmax(scales.x, std::max(scales.y, scales.z)) * 0.02f;
    ...
    if (volumeType == COLVOL_TYPE_CYLINDER) {
        scales[volumeAxes[1]] =
          std::max(scales[volumeAxes[1]], scales[volumeAxes[2]]);
...
{
    return (
      GetPointSurfaceDistance(f, lmp, f->GetTransformMatrixRef(true), pos));
}
                                         ...
                                         const float3& p) const
{
    ...
    switch (volumeType) {
        case COLVOL_TYPE_BOX: {
            ...
            pv.x = ((int(pv.x >= 0.0f) * 2) - 1) *
                   std::max(math::fabs(pv.x), halfAxisScales.x);
            pv.y = ((int(pv.y >= 0.0f) * 2) - 1) *
                   std::max(math::fabs(pv.y), halfAxisScales.y);
            pv.z = ((int(pv.z >= 0.0f) * 2) - 1) *
                   std::max(math::fabs(pv.z), halfAxisScales.z);
            ...
            // float l = std::min(pv.x - pt.x, std::min(pv.y - pt.y, pv.z -
            // pt.z));
            d = pv.distance(pt);
        } break;
        ...
        case COLVOL_TYPE_CYLINDER: {
            ...
            assert(halfAxisScales[volumeAxes[1]] ==
                   halfAxisScales[volumeAxes[2]]);
            assert(halfAxisScalesSqr[volumeAxes[1]] ==
                   halfAxisScalesSqr[volumeAxes[2]]);
                                     ...
                                     size_t axisC) const
{
    ...
    const float rSq =
      (halfAxisScalesSqr[axisB] + halfAxisScalesSqr[axisC]) * 0.5f;

    float d = 0.0f;

    if (pv[axisA] >= -halfAxisScales[axisA] &&
        pv[axisA] <= halfAxisScales[axisA]) {
        ...
        if (pSq <= rSq) {
            ...
            const float l =
              Square(math::fabs(pv[axisA]) - halfAxisScales[axisA]);
...
CollisionVolume::GetEllipsoidDistance(const float3& pv) const
{
    ...
    const float3 xyz2abc2 =
      (xyz1 * xyz1) /
      abc2; // {x2_a2, y2_b2, z2_c2}, same as xyzSq * Square(halfAxisScalesInv)
    ...
    float phi =
      math::atan2(xyz1.z, abc1.z * math::sqrt(xyz2abc2.x + xyz2abc2.y));
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        const float a2cos2t_b2sin2t_c2 =
          abc2.x * cost * cost + abc2.y * sin2t - abc2.z;

        const float d1 = a2b2costsint * cosp - xasint_ybcost;
        const float d2 = a2cos2t_b2sin2t_c2 * sinp * cosp -
                         sinp * xacost_ybsint + xyz1abc1.z * cosp;

        // Derivative matrix
        const float a11 =
          (abc2.x - abc2.y) * (1 - 2 * sin2t) * cosp - xacost_ybsint;
        const float a12 = -a2b2costsint * sinp;
        const float a21 = 2 * a12 * cosp + sinp * xasint_ybcost;
        const float a22 = a2cos2t_b2sin2t_c2 * (1 - 2 * sinp * sinp) -
                          cosp * xacost_ybsint - xyz1abc1.z;
