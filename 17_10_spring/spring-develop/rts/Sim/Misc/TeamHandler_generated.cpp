...
CTeamHandler::LoadFromSetup(const CGameSetup* setup)
{
    ...
    const int maxUnitsPerTeam =
      std::min(setup->maxUnitsPerTeam, int(MAX_UNITS / numTeams));

    for (size_t i = 0; i < teams.size(); ++i) {
        teams[i].teamNum = i;
        teams[i] = setup->GetTeamStartingDataCont()[i];

        // all non-Gaia teams (within one allyteam) get and maintain the same
        // unit-limit (because of this it would be better treated as a pool
        // owned by class AllyTeam)
    ...
    if (gs->useLuaGaia) {
        ...
        gaia.color[3] = 255;
        gaia.gaia = true;
        gaia.SetMaxUnits(MAX_UNITS -
                         ((teams.size() - 1) * teams[0].GetMaxUnits()));
        gaia.SetStartPos(ZeroVector);
        gaia.teamAllyteam = gaiaAllyTeamID;

        assert((((teams.size() - 1) * teams[0].GetMaxUnits()) +
                gaia.GetMaxUnits()) == MAX_UNITS);

        for (std::vector<::AllyTeam>::iterator it = allyTeams.begin();
             it != allyTeams.end();
             ++it) {
                                    ...
                                    bool countDeadTeams) const
{
    ...
    for (unsigned int teamNum = 0; teamNum < teamHandler->ActiveTeams();
         teamNum++) {
...
CTeamHandler::UpdateTeamUnitLimitsPreSpawn(int liveTeamNum)
{
    ...
    // count the number of remaining (non-dead) teams in <liveTeam>'s allyteam,
    // EXCLUDING us
    const unsigned int numRemainingActiveTeams =
      GetNumTeamsInAllyTeam(AllyTeam(liveTeamNum), false);

    if (numRemainingActiveTeams == 0) {
        // set default since we are the only team in our allyteam now
        liveTeam->SetMaxUnits(
          std::min(gameSetup->maxUnitsPerTeam, int(MAX_UNITS / teams.size())));
        return;
    }

    // reduce the limit of each team in our allyteam uniformly
    for (unsigned int tempTeamNum = 0; tempTeamNum < teams.size();
         tempTeamNum++) {
        ...
        tempTeam->SetMaxUnits(
          (tempTeam->GetMaxUnits() * numRemainingActiveTeams) /
...
CTeamHandler::UpdateTeamUnitLimitsPreDeath(int deadTeamNum)
{
    ...
    // INCLUDING us
    const unsigned int numRemainingActiveTeams =
      GetNumTeamsInAllyTeam(AllyTeam(deadTeamNum), false);
