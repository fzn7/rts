...
#define DEFAULT_ARMORDEF_NAME "default"
    ...
    try {
        ...
        LOG("[%s] number of ArmorDefs: " _STPF_,
            __FUNCTION__,
            armorDefKeys.size());

        // expects the following structure, subtables must be in array-format:
        //
        // {"tanks" = {[1] = "supertank", [2] = "megatank"}, "infantry" = {[1] =
        // "dude"}, ...}
        //
        // the old (pre-95.0) <key, value> subtable definitions are no longer
        // supported!
        //
        for (unsigned int armorDefIdx = 1; armorDefIdx < armorDefKeys.size();
             armorDefIdx++) {
            const std::string armorDefName =
              StringToLower(armorDefKeys[armorDefIdx]);

            if (armorDefName == DEFAULT_ARMORDEF_NAME) {
                // ignore, no need to clear entire table
                LOG_L(L_WARNING,
                      "[%s] ArmorDefs: tried to define the \"%s\" armor type!",
                      __FUNCTION__,
            ...
            const LuaTable armorDefTable =
              rootTable.SubTable(armorDefKeys[armorDefIdx]);
            ...
            if (SpringVersion::GetMajor()[1] >= '4') {
                ...
                if (!armorDefTableKeys.empty()) {
                    LOG_L(
                      L_WARNING,
                      "[%s] ArmorDefs contains sub-table \"%s\" in <key, "
                      "value> "
                      "format which is deprecated as of Spring 95.0 and will "
                      "not "
            ...
            for (unsigned int armorDefEntryIdx = 0;
                 armorDefEntryIdx < numArmorDefEntries;
                 armorDefEntryIdx++) {
                const std::string unitDefName = StringToLower(
                  armorDefTable.GetString(armorDefEntryIdx + 1, ""));
                const std::map<std::string, int>::const_iterator
                  armorDefTableIt = armorDefNameIdxMap.find(unitDefName);
                ...
                LOG_L(L_WARNING,
                      "[%s] UnitDef \"%s\" in ArmorDef \"%s\" already belongs "
                      "to ArmorDef category %d!",
                      __FUNCTION__,
                      unitDefName.c_str(),
                      armorDefName.c_str(),
                      armorDefTableIt->second);
            }
        }
...
CDamageArrayHandler::GetTypeFromName(const std::string& name) const
{
    const std::map<std::string, int>::const_iterator it =
      armorDefNameIdxMap.find(StringToLower(name));
