...
{
    LOG("[CCollisionHandler] dis-/continuous tests: %i/%i",
        numDiscTests,
        numContTests);
}
                             ...
                             CollisionQuery* q)
{
    ...
    const float rminx = std::min(pi0.x, pi1.x), rminy = std::min(pi0.y, pi1.y),
                rminz = std::min(pi0.z, pi1.z);
    const float rmaxx = std::max(pi0.x, pi1.x), rmaxy = std::max(pi0.y, pi1.y),
                rmaxz = std::max(pi0.z, pi1.z);

    // minimum and maximum (x, y, z) coordinates of (bounding box around) volume
    const float vminx = -v->GetHScales().x, vminy = -v->GetHScales().y,
                vminz = -v->GetHScales().z;
    const float vmaxx = v->GetHScales().x, vmaxy = v->GetHScales().y,
                vmaxz = v->GetHScales().z;
    ...
    switch (v->GetVolumeType()) {
        ...
        case CollisionVolume::COLVOL_TYPE_BOX: {
            // also covers footprints, but without taking the blocking-map into
            // account
            // TODO: this would require stepping ray across non-blocking yardmap
            // squares?
            //
            // intersect = CCollisionHandler::IntersectFootPrint(v, pi0, pi1,
            // q);
    ...
    if (q != nullptr) {
        ...
        if (q->b0 == CQ_POINT_ON_RAY) {
            q->p0 = m.Mul(q->p0);
        }
        if (q->b1 == CQ_POINT_ON_RAY) {
            q->p1 = m.Mul(q->p1);
        }
    }
...
CCollisionHandler::IntersectEllipsoid(const CollisionVolume* v,
                                      ...
                                      CollisionQuery* q)
{
    // transform the volume-space points into (unit) sphere-space (requires
    // fewer float-ops than solving the surface equation for arbitrary ellipsoid
    // volumes)
    const float3 pii0 = float3(pi0.x * v->GetHIScales().x,
                               pi0.y * v->GetHIScales().y,
                               pi0.z * v->GetHIScales().z);
    const float3 pii1 = float3(pi1.x * v->GetHIScales().x,
                               pi1.y * v->GetHIScales().y,
                               pi1.z * v->GetHIScales().z);
                                     ...
                                     CollisionQuery* q)
{
    ...
    const float ratio = ((pi0[sAx0] * pi0[sAx0]) / ahsq[sAx0]) +
    ...
    if (d >= -COLLISION_VOLUME_EPS) {
        if (a != 0.0f) {
            ...
            if (d < COLLISION_VOLUME_EPS) {
                ...
                p0 = (pii0 + (diir * t0)) * inv;
                s0 = (p0 - pi0).SqLength();
                b0 = (s0 < segLenSq && math::fabs(p0[pAx]) < ahs[pAx]) *
                     CQ_POINT_ON_RAY;
            } else {
                ...
                b0 = (s0 < segLenSq && math::fabs(p0[pAx]) < ahs[pAx]) *
                     CQ_POINT_ON_RAY;
                b1 = (s1 < segLenSq && math::fabs(p1[pAx]) < ahs[pAx]) *
                     CQ_POINT_ON_RAY;
            }
        } else {
            if (b != 0.0f) {
