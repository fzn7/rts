...
CCustomExplosionGenerator::GetFlagsFromTable(const LuaTable& table)
{
    ...
    if (table.GetBool("ground", false)) {
        flags |= CCustomExplosionGenerator::SPW_GROUND;
    }
    if (table.GetBool("water", false)) {
        flags |= CCustomExplosionGenerator::SPW_WATER;
    }
    if (table.GetBool("air", false)) {
        flags |= CCustomExplosionGenerator::SPW_AIR;
    }
    if (table.GetBool("underwater", false)) {
        flags |= CCustomExplosionGenerator::SPW_UNDERWATER;
    }
    if (table.GetBool("unit", false)) {
        flags |= CCustomExplosionGenerator::SPW_UNIT;
    }
    if (table.GetBool("nounit", false)) {
        flags |= CCustomExplosionGenerator::SPW_NO_UNIT;
    }
...
CCustomExplosionGenerator::GetFlagsFromHeight(float height, float groundHeight)
{
    ...
    } else if (height >= 5.0f) { // above water
        if (altitude >= 20.0f) {
            flags |= CCustomExplosionGenerator::SPW_AIR; // air
        } else {
            flags |= CCustomExplosionGenerator::SPW_GROUND; // ground
        }
    } else if (waterDist < 5.0f) { // water surface
        if (groundHeight > -2.0f) {
            flags |= CCustomExplosionGenerator::SPW_GROUND; // shallow water
                                                            // (use ground fx)
        } else {
...
{
    for (map<string, string>::const_iterator i = aliases.begin();
         i != aliases.end();
         ++i) {
...
CExplosionGeneratorHandler::ParseExplosionTables()
{
    ...
    exploParser = new LuaParser(
      "gamedata/explosions.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    aliasParser = new LuaParser(
      "gamedata/explosion_alias.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    explTblRoot = NULL;

    if (!aliasParser->Execute()) {
        LOG_L(L_ERROR,
              "Failed to parse explosion aliases: %s",
...
CExplosionGeneratorHandler::ReloadGenerators(const std::string& tag)
{
    ...
    if (tag.empty()) {
        ...
        if (it == expGenTagIdentMap.end()) {
            LOG_L(L_WARNING,
                  "[%s] no CEG named \"%s\" (forgot the \"%s\" prefix?)",
                  __FUNCTION__,
                  tag.c_str(),
                  CEG_PREFIX_STRING);
            return;
                                         ...
                                         CUnit* hit)
{
    ...
    return explosionGenerators[expGenID]->Explosion(
      pos, dir, damage, radius, gfxMod, owner, hit);
}
                                  ...
                                  CUnit* hit)
{
    ...
    const unsigned int flags =
      CCustomExplosionGenerator::GetFlagsFromHeight(pos.y, groundHeight);
    const bool airExplosion =
      ((flags & CCustomExplosionGenerator::SPW_AIR) != 0);
    const bool groundExplosion =
      ((flags & CCustomExplosionGenerator::SPW_GROUND) != 0);
    const bool waterExplosion =
      ((flags & CCustomExplosionGenerator::SPW_WATER) != 0);
    const bool uwExplosion =
      ((flags & CCustomExplosionGenerator::SPW_UNDERWATER) != 0);
    ...
    new CHeatCloudProjectile(owner,
                             npos,
    ...
    if (projectileHandler->GetParticleSaturation() < 1.0f) {
        // turn off lots of graphic only particles when we have more particles
        // than we want
        ...
        for (int a = 0; a < smokeDamage * 0.6f; ++a) {
            const float3 speed((-0.1f + gu->RandFloat() * 0.2f),
                               (0.1f + gu->RandFloat() * 0.3f) *
                                 smokeDamageISQRT,
                               (-0.1f + gu->RandFloat() * 0.2f));

            const float time = (40.0f + smokeDamageSQRT * 15.0f) *
                               (0.8f + gu->RandFloat() * 0.7f);

            float3 dir = gu->RandVector() * smokeDamage;
            dir.y = std::abs(dir.y);
            const float3 npos = pos + dir;

            new CSmokeProjectile2(owner,
                                  pos,
                                  npos,
                                  speed,
                                  time,
                                  smokeDamageSQRT * 4.0f,
                                  0.4f,
                                  0.6f);
        }
        ...
        if (groundExplosion) {
            ...
            const float explSpeedMod =
              0.7f + std::min(30.0f, damage) / GAME_SPEED;
            const float3 color(0.15f, 0.1f, 0.05f);

            for (int a = 0; a < numDirt; ++a) {
                const float3 explSpeed =
                  float3((0.5f - gu->RandFloat()) * 1.5f,
                ...
                new CDirtProjectile(owner,
                                    npos,
                                    explSpeed * explSpeedMod,
                                    90.0f + damage * 2.0f,
                                    2.0f + sqrtDmg * 1.5f,
                                    0.4f,
                                    0.999f,
        ...
        if (!airExplosion && !uwExplosion && waterExplosion) {
            ...
            for (int a = 0; a < numDirt; ++a) {
                const float3 speed((0.5f - gu->RandFloat()) * 0.2f,
        ...
        if (damage >= 20.0f && !uwExplosion && !airExplosion) {
            ...
            for (int a = 0; a < numDebris; ++a) {
                const float3 explSpeed =
                  (altitude < 4.0f) ? float3((0.5f - gu->RandFloat()) * 2.0f,
                                             1.8f + gu->RandFloat() * 1.8f,
                                             (0.5f - gu->RandFloat()) * 2.0f)
                ...
                new CWreckProjectile(
                  owner, npos, explSpeed * explSpeedMod, 90.0f + damage * 2.0f);
            }
        }
        if (uwExplosion) {
        ...
        if (radius > 10.0f && damage > 4.0f) {
            ...
            const float explSpeedMod =
              (8 + damage * 3.0f) / (9 + sqrtDmg * 0.7f) * 0.35f;

            for (int a = 0; a < numSpike; ++a) {
                float3 explSpeed =
                  (gu->RandVector()).SafeNormalize() * explSpeedMod;

                if (!airExplosion && !waterExplosion && (explSpeed.y < 0.0f)) {
                    explSpeed.y = -explSpeed.y;
                }

                new CExploSpikeProjectile(owner,
                                          pos + explSpeed,
                                          explSpeed *
                                            (0.9f + gu->RandFloat() * 0.4f),
                                          radius * 0.1f,
                                          radius * 0.1f,
                                          0.6f,
                                          0.8f / (8.0f + sqrtDmg));
    ...
    if (radius > 20.0f && damage > 6.0f && altitude < (radius * 0.7f)) {
        ...
        if (flashSize > 5.f && ttl > 15.f) {
            ...
            if (radius > 40.0f && damage > 12.0f) {
                ...
                circleGrowth =
                  (8.0f + damage * 2.5f) / (9.0f + sqrtDmg * 0.7f) * 0.55f;
            }

            new CStandardGroundFlash(
              pos, circleAlpha, flashAlpha, flashSize, circleGrowth, ttl);
        }
    }

    if (radius > 40.0f && damage > 12.0f) {
        CSpherePartProjectile::CreateSphere(owner,
                                            5.0f + int(sqrtDmg * 0.7f),
                                            std::min(0.7f, damage * 0.02f),
                                            (8.0f + damage * 2.5f) /
                                              (9.0f + sqrtDmg * 0.7f) * 0.5f,
                                            pos);
    }
                                                ...
                                                const float3& dir)
{
    ...
    float buffer[16];
    for (int i = 0; i < 16; i++)
        buffer[i] = 0.0f; // initialize buffer
    ...
    for (;;) {
        switch (*(code++)) {
            ...
            case OP_STOREI: {
                boost::uint8_t size = *(boost::uint8_t*)code;
                code++;
                boost::uint16_t offset = *(boost::uint16_t*)code;
                code += 2;
                switch (size) {
                    case 1: {
                        *(boost::int8_t*)(instance + offset) = (int)val;
                    } break;
                    case 2: {
                        *(boost::int16_t*)(instance + offset) = (int)val;
                    } break;
                    case 4: {
                        *(boost::int32_t*)(instance + offset) = (int)val;
                    } break;
                    case 8: {
                        *(boost::int64_t*)(instance + offset) = (int)val;
                    } break;
                    default: { /*no op*/
                    } break;
                }
            ...
            case OP_DISCRETE: {
                val = (*(float*)code) *
                      math::floor(SafeDivide(val, (*(float*)code)));
                code += 4;
                break;
            }
            case OP_SINE: {
  ...
  string& code)
{
    ...
    const bool isFloat =
      memberInfo.type == SExpGenSpawnableMemberInfo::TYPE_FLOAT;

    if (content == "dir") { // first see if we can match any keywords
        // if the user uses a keyword assume he knows that it is put on the
        // right datatype for now
        if (memberInfo.length < 3 || !isFloat) // dir has to be float3
            throw content_error(
              "[CCEG::ParseExplosionCode] incorrect use of \"dir\" (" + script +
              ")");
    ...
    if (memberInfo.length > 1) {
        ...
        for (unsigned int i = 0;
             i < memberInfo.length && start < script.length();
             ++i) {
            string::size_type subEnd = script.find(',', start + 1);
            ParseExplosionCode(
              psi, script.substr(start, subEnd - start), subInfo, code);
            start = subEnd + 1;
    ...
    if (isFloat) {
        if (allowedSizeFlt.find(memberInfo.size) == allowedSizeFlt.end())
            throw content_error(
              "[CCEG::ParseExplosionCode] incompatible float size \"" +
              IntToString(memberInfo.size) + "\" (" + script + ")");
    } else {
        if (allowedSizeInt.find(memberInfo.size) == allowedSizeInt.end())
            throw content_error(
              "[CCEG::ParseExplosionCode] incompatible integer size \"" +
              IntToString(memberInfo.size) + "\" (" + script + ")");
    }
    ...
    while (p < script.length()) {
        ...
        if (c == 'i')
            ...
            const char* fmt = "[CCEG::ParseExplosionCode] unknown op-code "
                              "\"%c\" in \"%s\" at index %d";
        ...
        if (!useInt) {
            ...
            const int v = std::max(
              0, std::min(16, (int)strtol(&script.c_str()[p], &endp, 10)));
                                ...
                                const string& tag)
{
    ...
    const LuaTable& expTable =
      (root != NULL) ? root->SubTable(tag) : LuaTable();

    if (!expTable.IsValid()) {
        // not a fatal error: any calls to Explosion will just return early
        LOG_L(L_WARNING,
              "[CCEG::%s] table for CEG \"%s\" invalid (parse errors?)",
              __FUNCTION__,
              tag.c_str());
        return false;
    }
    ...
    for (unsigned int n = 0; n < spawns.size(); n++) {
        ...
        const string& className = handler->GetProjectileClasses().ResolveAlias(
          spawnTable.GetString("class", spawnName));
        psi.spawnableID = CExpGenSpawnable::GetSpawnableID(className);
        if (psi.spawnableID == -1u) {
            LOG_L(L_WARNING,
                  "[CCEG::%s] %s: Unknown class \"%s\"",
                  __FUNCTION__,
                  tag.c_str(),
                  className.c_str());
            continue;
        ...
        for (const auto& propIt : props) {
            SExpGenSpawnableMemberInfo memberInfo = {
                ...
                0,
                0,
                STRING_HASH(std::move(StringToLower(propIt.first))),
                SExpGenSpawnableMemberInfo::TYPE_INT,
                nullptr
            };

            if (CExpGenSpawnable::GetSpawnableMemberInfo(className,
                                                         memberInfo)) {
                ParseExplosionCode(&psi, propIt.second, memberInfo, code);
            } else {
                LOG_L(L_WARNING,
                      "[CCEG::%s] %s: Unknown tag %s::%s",
                      __FUNCTION__,
                      tag.c_str(),
                      className.c_str(),
                      propIt.first.c_str());
    ...
    if (ttl > 0) {
        expGenParams.groundFlash.circleAlpha =
          gndTable.GetFloat("circleAlpha", 0.0f);
        expGenParams.groundFlash.flashSize =
          gndTable.GetFloat("flashSize", 0.0f);
        expGenParams.groundFlash.flashAlpha =
          gndTable.GetFloat("flashAlpha", 0.0f);
        expGenParams.groundFlash.circleGrowth =
          gndTable.GetFloat("circleGrowth", 0.0f);
        expGenParams.groundFlash.color =
          gndTable.GetFloat3("color", float3(1.0f, 1.0f, 0.8f));

        expGenParams.groundFlash.flags =
          SPW_GROUND | GetFlagsFromTable(gndTable);
        expGenParams.groundFlash.ttl = ttl;
    }

    expGenParams.useDefaultExplosions =
      expTable.GetBool("useDefaultExplosions", false);
    return true;
}
                                     ...
                                     CUnit* hit)
{
    ...
    const std::vector<ProjectileSpawnInfo>& spawnInfo =
      expGenParams.projectiles;
    ...
    for (int a = 0; a < spawnInfo.size(); a++) {
        ...
        for (unsigned int c = 0; c < psi.count; c++) {
            CExpGenSpawnable* projectile =
              CExpGenSpawnable::CreateSpawnable(psi.spawnableID);
            ExecuteExplosionCode(
              &psi.code[0], damage, (char*)projectile, c, dir);
            projectile->Init(owner, pos);
        }
    }

    if (groundExplosion && (groundFlash.ttl > 0) &&
        (groundFlash.flashSize > 1)) {
...
CCustomExplosionGenerator::OutputProjectileClassInfo()
{
    ...
    vfsHandler->AddArchiveWithDeps(
      archiveScanner->ArchiveFromName("Spring content v1"), false);
    LOG_ENABLE();
    ...
    for (vector<creg::Class*>::const_iterator ci = classes.begin();
         ci != classes.end();
         ++ci) {
        creg::Class* c = *ci;

        if (!c->IsSubclassOf(CExpGenSpawnable::StaticClass()) ||
            c == CExpGenSpawnable::StaticClass())
            continue;
        ...
        std::cout << "    \"alias\": \""
                  << (egh.GetProjectileClasses()).FindAlias(c->name) << "\"";
    ...
    return true;
#else // USING_CREG
    std::cout << "Unable to output CEG info since creg is disabled"
              << std::endl;
    return false;
#endif
}
