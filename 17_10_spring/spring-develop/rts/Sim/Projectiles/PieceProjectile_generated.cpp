    ...
    if (owner != NULL) {
        if ((explFlags & PF_NoCEGTrail) == 0) {
            explGenHandler->GenExplosion(
              (cegID =
                 owner->unitDef->GetPieceExplosionGeneratorID(gs->randInt())),
              pos,
              speed,
              100,
              0.0f,
...
CPieceProjectile::Collision(CUnit* unit, CFeature* feature)
{
    ...
    if ((explFlags & PF_Explode) && (unit || feature)) {
        ...
        const CExplosionParams params = { pos,
                                          ZeroVector,
                                          damageArray,
                                          NULL, // weaponDef
                                          owner(),
...
CPieceProjectile::Update()
{
    ...
    if ((explFlags & PF_NoCEGTrail) == 0) {
        ...
        explGenHandler->GenExplosion(
          cegID, pos, speed, 100, 0.0f, 0.0f, NULL, NULL);
        return;
    }
...
CPieceProjectile::Draw()
{
    if (explFlags & PF_Fire) {
        ...
        for (unsigned int age = 0; age < NUM_TRAIL_PARTS; ++age) {
            ...
            va->AddVertexQTC(interPos - camera->GetRight() * drawsize -
