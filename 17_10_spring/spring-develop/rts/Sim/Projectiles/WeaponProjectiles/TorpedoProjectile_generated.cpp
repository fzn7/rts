  ...
  , texy(0.0f)
{
    ...
    texx = projectileDrawer->torpedotex->xstart -
           (projectileDrawer->torpedotex->xend -
            projectileDrawer->torpedotex->xstart) *
             0.5f;
    texy = projectileDrawer->torpedotex->ystart -
           (projectileDrawer->torpedotex->yend -
            projectileDrawer->torpedotex->ystart) *
             0.5f;
...
CTorpedoProjectile::Update()
{
    ...
    if (!weaponDef->submissile && pos.y > 0.0f) {
        ...
        if (--ttl > 0) {
            if (!luaMoveCtrl) {
                ...
                const float3 targetLeadVec =
                  targetVel * (pos.distance(targetPos) / maxSpeed) * 0.7f;
                ...
                if (targetDirDif.Length() < tracking) {
                    ...
                    targetDirDif =
                      (targetDirDif - (dir * targetDirDif.dot(dir)))
                        .SafeNormalize();
    ...
    if (pos.y < -2.0f) {
        ...
        if (nextBubble == 0) {
            ...
            const float3 pspeed =
              (gu->RandVector() * 0.1f) + float3(0.0f, 0.2f, 0.0f);

            // Spawn unsynced bubble projectile
            new CBubbleProjectile(owner(),
                                  pos + gu->RandVector(),
                                  pspeed,
                                  40 + gu->RandFloat() * GAME_SPEED,
                                  1 + gu->RandFloat() * 2,
                                  0.01f,
...
int
CTorpedoProjectile::GetProjectilesCount() const
{
    return 8;
}
