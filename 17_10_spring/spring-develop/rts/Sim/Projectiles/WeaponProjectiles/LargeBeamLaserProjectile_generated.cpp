...
void
CLargeBeamLaserProjectile::Update()
{
    if ((--ttl) <= 0) {
        deleteMe = true;
    } else {
        ...
        explGenHandler->GenExplosion(cegID,
...
void
CLargeBeamLaserProjectile::Draw()
{
    inArray = true;
    ...
    const float startTex = 1.0f - ((gu->modGameTime * scrollspeed) -
                                   int(gu->modGameTime * scrollspeed));
    ...
    // note: beamTileMaxDst can be negative, in which case we want numBeamTiles
    // to equal zero
    const float numBeamTiles =
      std::floor(((std::max(beamTileMinDst, beamTileMaxDst) - beamTileMinDst) /
                  tilelength) +
                 0.5f);
    ...
    va->EnlargeArrays(
      64 + (8 * (int((beamTileMaxDst - beamTileMinDst) / tilelength) + 2)),
      0,
      VA_SIZE_TC);
...
#define WT4 weaponDef->visuals.texture4
    ...
    if (beamTileMinDst > beamLength) {
        ...
        for (float i = beamTileMinDst; i < beamTileMaxDst; i += tilelength) {
            ...
            va->AddVertexQTC(pos1 - (xdir * beamEdgeSize),
                             tex.xstart,
                             tex.ystart,
                             edgeColStart);
            va->AddVertexQTC(
    ...
    va->AddVertexQTC(pos2 + (xdir * beamEdgeSize) + (ydir * beamEdgeSize),
                     WT2->xend,
                     WT2->yend,
                     edgeColStart);
    va->AddVertexQTC(pos2 - (xdir * beamEdgeSize) + (ydir * beamEdgeSize),
    ...
    va->AddVertexQTC(pos2 + (xdir * beamCoreSize) + (ydir * beamCoreSize),
                     WT2->xend,
                     WT2->yend,
                     coreColStart);
    va->AddVertexQTC(pos2 - (xdir * beamCoreSize) + (ydir * beamCoreSize),
    ...
    float pulseStartTime =
      (gu->modGameTime * pulseSpeed) - int(gu->modGameTime * pulseSpeed);
...

    {
        ...
        va->AddVertexQTC(pos1 + (ydir * muzzleEdgeSize),
                         sidetex.xstart,
                         sidetex.ystart,
                         edgeColor);
        va->AddVertexQTC(pos1 + (ydir * muzzleEdgeSize) +
        ...
        va->AddVertexQTC(pos1 + (ydir * muzzleCoreSize),
                         sidetex.xstart,
                         sidetex.ystart,
                         coreColor);
        va->AddVertexQTC(pos1 + (ydir * muzzleCoreSize) +
...

    {
        ...
        va->AddVertexQTC(pos1 - (camera->GetRight() * flareEdgeSize) -
                           (camera->GetUp() * flareEdgeSize),
                         WT4->xstart,
                         WT4->ystart,
                         edgeColStart);
        va->AddVertexQTC(pos1 + (camera->GetRight() * flareEdgeSize) -
        ...
        va->AddVertexQTC(pos1 - (camera->GetRight() * flareCoreSize) -
                           (camera->GetUp() * flareCoreSize),
                         WT4->xstart,
                         WT4->ystart,
                         coreColStart);
        va->AddVertexQTC(pos1 + (camera->GetRight() * flareCoreSize) -
...
void
CLargeBeamLaserProjectile::DrawOnMinimap(CVertexArray& lines,
                                         CVertexArray& points)
{
    const unsigned char color[4] = {
        edgeColStart[0], edgeColStart[1], edgeColStart[2], 255
    };
...
int
CLargeBeamLaserProjectile::GetProjectilesCount() const
{
