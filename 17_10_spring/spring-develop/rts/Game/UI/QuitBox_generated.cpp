...
CQuitBox::Draw()
{
    ...
    const ContainerBox boxes[] = { resignBox, saveBox, giveAwayBox,
                                   cancelBox, menuBox, quitBox };
    ...
    font->glPrint(box.x1 + 0.045f,
                  box.y1 + 0.58f,
                  0.7f,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Do you want to ...");
    glColor4f(1.0f, 1.0f, 1.0f, 0.8f);

    font->glPrint(box.x1 + resignBox.x1 + 0.025f,
                  box.y1 + (resignBox.y1 + resignBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Resign");
    font->glPrint(box.x1 + saveBox.x1 + 0.025f,
                  box.y1 + (saveBox.y1 + saveBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Save");
    font->glPrint(box.x1 + giveAwayBox.x1 + 0.025f,
                  box.y1 + (giveAwayBox.y1 + giveAwayBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Give everything to ...");
    font->glPrint(box.x1 + cancelBox.x1 + 0.025f,
                  box.y1 + (cancelBox.y1 + cancelBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Cancel");
    font->glPrint(box.x1 + menuBox.x1 + 0.025f,
                  box.y1 + (menuBox.y1 + menuBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Quit To Menu");
    font->glPrint(box.x1 + quitBox.x1 + 0.025f,
                  box.y1 + (quitBox.y1 + quitBox.y2) / 2,
                  1,
                  FONT_VCENTER | FONT_SCALE | FONT_NORM,
                  "Quit To System");

    int teamPos = 0;
    for (int team = startTeam; team < MAX_QUIT_TEAMS && teamPos < numTeamsDisp;
         ++team, ++teamPos) {
        int actualTeam = team;
...
CQuitBox::GetTooltip(int x, int y)
{
    ...
    const ContainerBox boxes[] = { resignBox, saveBox,      giveAwayBox,
                                   scrollBox, scrollbarBox, teamBox,
                                   cancelBox, menuBox,      quitBox };
    const char* toolTips[] = {
...
CQuitBox::MousePress(int x, int y, int button)
{
    ...
    const ContainerBox boxes[] = { resignBox, saveBox,      giveAwayBox,
                                   teamBox,   cancelBox,    menuBox,
                                   quitBox,   scrollbarBox, scrollBox };
    ...
    if (hasScroll && InBox(mx, my, box + scrollBox)) {
        ...
        if (my < (box + scrollBox).y1)
            *(volatile int*)&startTeam =
              startTeam +
              std::min(MAX_QUIT_TEAMS - numTeamsDisp - startTeam, numTeamsDisp);
        ...
        if (teamHandler->IsValidTeam(team) &&
            ...
            // we don't want to give everything to the enemy if there are allies
            // left
            if (noAlliesLeft ||
                (!noAlliesLeft &&
                 teamHandler->Ally(gu->myAllyTeam,
...
CQuitBox::MouseRelease(int x, int y, int button)
{
    ...
    if (InBox(mx, my, box + resignBox) ||
        (InBox(mx, my, box + saveBox) &&
         !teamHandler->Team(gu->myTeam)->isDead) ||
        (InBox(mx, my, box + giveAwayBox) &&
         !teamHandler->Team(shareTeam)->isDead &&
         !teamHandler->Team(gu->myTeam)->isDead)) {

        // give away all units (and resources)
        if (InBox(mx, my, box + giveAwayBox) &&
            !playerHandler->Player(gu->myPlayerNum)->spectator) {
            clientNet->Send(CBaseNetProtocol::Get().SendGiveAwayEverything(
              gu->myPlayerNum,
              shareTeam,
              playerHandler->Player(gu->myPlayerNum)->team));
        }
        // resign, so self-d all units
        if (InBox(mx, my, box + resignBox) &&
            !playerHandler->Player(gu->myPlayerNum)->spectator) {
            clientNet->Send(
              CBaseNetProtocol::Get().SendResign(gu->myPlayerNum));
        }
        // save current game state
        if (InBox(mx, my, box + saveBox)) {
            if (FileSystem::CreateDirectory("Saves")) {
                std::string timeStr = CTimeUtil::GetCurrentTimeStr();
                std::string saveFileName(timeStr + "_" + modInfo.filename +
                                         "_" + gameSetup->mapName);
                ...
                if (!FileSystem::FileExists(saveFileName)) {
                    ...
                    LOG_L(L_ERROR,
                          "File %s already exists, game NOT saved!",
                          saveFileName.c_str());
                }
            }
        }
...
CQuitBox::MouseMove(int x, int y, int dx, int dy, int button)
{
    ...
    if (scrolling) {
        ...
        *(volatile int*)&startTeam = std::max(
          0, std::min((int)(scr / tsz + 0.5), MAX_QUIT_TEAMS - numTeamsDisp));
        return;
    }
    ...
    if (teamHandler->IsValidTeam(team) && !teamHandler->Team(team)->isDead) {
        ...
        if (noAlliesLeft ||
            (!noAlliesLeft &&
             teamHandler->Ally(gu->myAllyTeam, teamHandler->AllyTeam(team)))) {
            shareTeam = team;
        }
    }
}
