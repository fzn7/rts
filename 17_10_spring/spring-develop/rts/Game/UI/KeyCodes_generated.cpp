...
CKeyCodes::Reset()
{
    ...
    // I doubt these can be used correctly anyway (without special support in
    // other parts of the spring code...)
...
CKeyCodes::PrintNameToCode() const
{
    for (const auto& p : nameToCode) {
        LOG("KEYNAME: %13s = 0x%03X (SDL1 = 0x%03X)",
            p.first.c_str(),
            p.second,
            SDL21_keysyms(p.second));
    }
}
...
CKeyCodes::PrintCodeToName() const
{
    for (const auto& p : codeToName) {
        LOG("KEYCODE: 0x%03X = '%s' (SDL1 = 0x%03X)",
            p.first,
            p.second.c_str(),
            SDL21_keysyms(p.first));
    }
}
