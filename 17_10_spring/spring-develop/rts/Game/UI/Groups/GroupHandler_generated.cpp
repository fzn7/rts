...
{
    {
        for (std::vector<CGroup*>::iterator gi = groups.begin();
             gi != groups.end();
             ++gi) {
            if ((*gi) != NULL) {
                // Update may invoke RemoveGroup, but this will only NULL the
                // element, so there will be no iterator invalidation here
                (*gi)->Update();
            }
        }
    }
...
CGroupHandler::GroupCommand(int num, const std::string& cmd)
{
    ...
    if ((cmd == "set") || (cmd == "add")) {
        ...
        // do not select the group, just add its members to the current
        // selection
        CUnitSet::const_iterator ui;
        ...
        // do not select the group, just remove its members from the current
        // selection
        CUnitSet::const_iterator ui;
        for (ui = group->units.begin(); ui != group->units.end(); ++ui) {
        ...
        // do not select the group, just toggle its members with the current
        // selection
        const CUnitSet& selUnits = selectedUnitsHandler.selectedUnits;
        CUnitSet::const_iterator ui;
        for (ui = group->units.begin(); ui != group->units.end(); ++ui) {
