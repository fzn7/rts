...
std::string
CSelectionKeyHandler::ReadToken(std::string& str)
{
    std::string ret;

    size_t index = 0;
    while ((index < str.length()) && (str[index] != '_') &&
           (str[index] != '+')) {
        index++;
    }
...
#define DECLARE_FILTER_EX(name, args, condition, extra, init)                  \
    struct name##_Filter                                                       \
      : public Filter{ name##_Filter() : Filter(#name, args){ init;            \
    }                                                                          \
...
DECLARE_FILTER(Radar,
               unit->radarRadius || unit->sonarRadius || unit->jammerRadius)
DECLARE_FILTER(ManualFireUnit, unit->unitDef->canManualFire)

DECLARE_FILTER_EX(
  WeaponRange, 1, unit->maxRange > minRange, float minRange;
...
DECLARE_FILTER_EX(
  AbsoluteHealth, 1, unit->health > minHealth, float minHealth;
  void SetParam(int index, const std::string& value) {
...
DECLARE_FILTER_EX(
  RelativeHealth,
  1,
  unit->health / unit->maxHealth > minHealth,
  float minHealth;
  void SetParam(int index, const std::string& value) {
...
DECLARE_FILTER_EX(InPrevSel,
                  0,
                  prevTypes.find(unit->unitDef->id) != prevTypes.end(),
                  std::set<int> prevTypes;
                  void Prepare() {
                      prevTypes.clear();
                      const CUnitSet& tu = selectedUnitsHandler.selectedUnits;
                      for (CUnitSet::const_iterator si = tu.begin();
                           si != tu.end();
                           ++si) {
...
DECLARE_FILTER_EX(NameContain,
                  1,
                  unit->unitDef->humanName.find(name) != std::string::npos,
                  std::string name;
                  void SetParam(int index, const std::string& value) {
                      name = value;
                  }, )
...
DECLARE_FILTER_EX(
  Category, 1, unit->category == cat, unsigned int cat;
  void SetParam(int index, const std::string& value) {
...
DECLARE_FILTER_EX(
  RulesParamEquals,
  2,
  unit->modParams.find(param) != unit->modParams.end() &&
    ((wantedValueStr.empty())
       ? unit->modParams.find(param)->second.valueInt == wantedValue
...
CSelectionKeyHandler::DoSelection(std::string selectString)
{
    ...
    if (s == "AllMap") {
        ...
        float dist = CGround::LineGroundCol(
          camera->GetPos(), camera->GetPos() + mouse->dir * 8000, false);
    ...
    if (s == "SelectAll") {
        for (std::list<CUnit*>::iterator ui = selection.begin();
             ui != selection.end();
             ++ui)
        ...
        int a = 0;
        for (std::list<CUnit*>::iterator ui = selection.begin();
             ui != selection.end() && a <= selectNumber;
             ++ui, ++a)
            sel = *ui;
        ...
        if (camHandler->GetCurrentControllerNum() !=
            CCameraHandler::CAMERA_MODE_FIRSTPERSON) {
        ...
        ReadDelimiter(selectString);
        float part =
          atof(ReadToken(selectString).c_str()) * 0.01f; // convert from percent
