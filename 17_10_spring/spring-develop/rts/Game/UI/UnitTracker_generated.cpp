...
const char* CUnitTracker::modeNames[TrackModeCount] = { "Single",
                                                        "Average",
                                                        "Extents" };
...
int
CUnitTracker::GetMode() const
{
    return trackMode;
}

void
CUnitTracker::IncMode()
{
...
float3
CUnitTracker::CalcAveragePos() const
{
    float3 p(ZeroVector);
...
float3
CUnitTracker::CalcExtentsPos() const
{
...
CUnitTracker::SetCam()
{
    ...
    if (lastFollowUnit != 0 &&
        unitHandler->GetUnitUnsafe(lastFollowUnit) == 0) {
        timeOut = 1;
        lastFollowUnit = 0;
    }
    ...
    if (timeOut > 0) {
    ...
    } else if (camHandler->GetCurrentControllerNum() !=
               CCameraHandler::CAMERA_MODE_FIRSTPERSON) {
        ...
        camHandler->GetCurrentController().SetTrackingInfo(
          pos, u->radius * 2.7182818f);
        camHandler->UpdateTransition();

    } else {
        // FPS Camera
        const float deltaTime =
          gs->frameNum + globalRendering->timeOffset - lastUpdateTime;
        lastUpdateTime = gs->frameNum + globalRendering->timeOffset;

        float3 modPlanePos(u->drawMidPos - (u->frontdir * u->radius * 3));
        const float minHeight =
          CGround::GetHeightReal(modPlanePos.x, modPlanePos.z, false) +
          (u->radius * 2);
        modPlanePos.y = std::max(modPlanePos.y, minHeight);

        trackPos +=
          (modPlanePos - trackPos) * (1 - math::pow(0.95f, deltaTime));
        trackDir +=
          (u->frontdir - trackDir) * (1 - math::pow(0.90f, deltaTime));
        trackDir.ANormalize();
        ...
        CFPSController& fpsCamera =
          static_cast<CFPSController&>(camHandler->GetCurrentController());
        fpsCamera.SetDir(camera->GetDir());
        fpsCamera.SetPos(trackPos);

        const float3 right =
          mix<float3>(smoothedRight,
                      mix<float3>(u->rightdir, RgtVector, 0.75f),
                      deltaTime * 0.05f)
            .ANormalize();
