...
class CNotificationPeeper : public CEventClient
{
  public:
    ...
    virtual bool MapDrawCmd(int playerID,
                            int type,
                            const float3* pos0,
                            const float3* pos1,
...
void
CInMapDraw::MousePress(int x, int y, int button)
{
...
void
CInMapDraw::MouseRelease(int x, int y, int button)
{
    // TODO implement CInMapDraw::MouseRelease
}

void
CInMapDraw::MouseMove(int x, int y, int dx, int dy, int button)
{
    float3 pos = GetMouseMapPos();
    if (pos.x < 0) {
        return;
    ...
    if (mouse->buttons[SDL_BUTTON_LEFT].pressed &&
        (lastDrawTime < (gu->gameTime - 0.05f))) {
...
float3
CInMapDraw::GetMouseMapPos() // TODO move to some more global place?
{
    const float dist = CGround::LineGroundCol(
      camera->GetPos(),
      camera->GetPos() + (mouse->dir * globalRendering->viewRange * 1.4f),
      false);
    if (dist < 0) {
...
int
CInMapDraw::GotNetMsg(boost::shared_ptr<const netcode::RawPacket>& packet)
{
    int playerID = -1;
...
void
CInMapDraw::SetSpecMapDrawingAllowed(bool state)
{
    allowSpecMapDrawing = state;
    LOG("[%s] spectator map-drawing is %s",
        __FUNCTION__,
        allowSpecMapDrawing ? "enabled" : "disabled");
}

void
CInMapDraw::SetLuaMapDrawingAllowed(bool state)
{
    allowLuaMapDrawing = state;
    LOG("[%s] Lua map-drawing is %s",
        __FUNCTION__,
        allowLuaMapDrawing ? "enabled" : "disabled");
}

void
CInMapDraw::SendErase(const float3& pos)
{
    if (!gu->spectating || allowSpecMapDrawing)
        clientNet->Send(CBaseNetProtocol::Get().SendMapErase(
          gu->myPlayerNum, (short)pos.x, (short)pos.z));
}

void
CInMapDraw::SendPoint(const float3& pos, const std::string& label, bool fromLua)
{
    if (!gu->spectating || allowSpecMapDrawing)
        clientNet->Send(CBaseNetProtocol::Get().SendMapDrawPoint(
          gu->myPlayerNum, (short)pos.x, (short)pos.z, label, fromLua));
}

void
CInMapDraw::SendLine(const float3& pos, const float3& pos2, bool fromLua)
{
    if (!gu->spectating || allowSpecMapDrawing)
        clientNet->Send(CBaseNetProtocol::Get().SendMapDrawLine(gu->myPlayerNum,
                                                                (short)pos.x,
                                                                (short)pos.z,
                                                                (short)pos2.x,
                                                                (short)pos2.z,
...
void
CInMapDraw::SendWaitingInput(const std::string& label)
{
...
void
CInMapDraw::PromptLabel(const float3& pos)
{
    waitingPoint = pos;
...
void
CInMapDraw::GetPoints(std::vector<PointMarker>& points,
                      int pointsSizeMax,
                      const std::list<int>& teamIDs)
{
    ...
    for (size_t y = 0; (y < inMapDrawerModel->GetDrawQuadY()) &&
                       ((int)points.size() < pointsSizeMax);
         y++) {
        for (size_t x = 0; (x < inMapDrawerModel->GetDrawQuadX()) &&
                           ((int)points.size() < pointsSizeMax);
             x++) {
            pointsInt = &(inMapDrawerModel->GetDrawQuad(x, y)->points);

            for (auto point = pointsInt->cbegin();
                 (point != pointsInt->cend()) &&
                 ((int)points.size() < pointsSizeMax);
                 ++point) {
...
void
CInMapDraw::GetLines(std::vector<LineMarker>& lines,
                     int linesSizeMax,
                     const std::list<int>& teamIDs)
{
