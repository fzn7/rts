...
{
  public:
    AliasActionExecutor(IUnsyncedActionExecutor* innerExecutor,
                        const std::string& commandAlias)
      : IUnsyncedActionExecutor(commandAlias,
                                "Alias for command \"" + commandAlias + "\"")
      , innerExecutor(innerExecutor)
    {
        assert(innerExecutor != NULL);
    }
    virtual ~AliasActionExecutor() { delete innerExecutor; }
  ...
  public:
    SequentialActionExecutor(const std::string& command)
      : IUnsyncedActionExecutor(command,
                                "Executes the following commands in order:")
    {}
...
class SelectActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        selectionKeys->DoSelection(
          action.GetArgs()); // TODO give it a return argument?
        return true;
    }
};
...
class SelectUnitsActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SelectUnitsActionExecutor()
      : IUnsyncedActionExecutor("SelectUnits",
                                "<chat command description: SelectUnits>")
    {} // TODO

    bool Execute(const UnsyncedAction& action) const
    {
        selectedUnitsHandler.SelectUnits(
          action.GetArgs()); // TODO give it a return argument?
        return true;
    }
};
...
class SelectCycleActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SelectCycleActionExecutor()
      : IUnsyncedActionExecutor("SelectCycle",
                                "<chat command description: SelectUnits>")
    {} // TODO

    bool Execute(const UnsyncedAction& action) const
    {
        selectedUnitsHandler.SelectCycle(
          action.GetArgs()); // TODO give it a return argument?
        return true;
    }
};
...
class DeselectActionExecutor : public IUnsyncedActionExecutor
{
  public:
    DeselectActionExecutor()
      : IUnsyncedActionExecutor("Deselect",
...
class RoamActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        CSMFGroundDrawer* smfGD =
          dynamic_cast<CSMFGroundDrawer*>(readMap->GetGroundDrawer());
        if (!smfGD)
            return false;
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        if (shadowHandler->shadowConfig < 0) {
            LOG_L(L_WARNING,
                  "Shadows are disabled; change your configuration and restart "
                  "to use them");
            return true;
        }
        if (!CShadowHandler::ShadowsSupported()) {
            LOG_L(L_WARNING,
                  "Your hardware/driver setup does not support shadows");
            return true;
        }

        shadowHandler->Reload(
          ((action.GetArgs()).empty()) ? NULL : (action.GetArgs()).c_str());
        LOG("Set \"shadows\" config-parameter to %i",
            shadowHandler->shadowConfig);
        return true;
    }
};
...
{
  public:
    AdvModelShadingActionExecutor()
      : IUnsyncedActionExecutor("AdvModelShading",
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (pos != std::string::npos) {
            ...
            LOG_L(L_WARNING,
                  "/WByNum: wrong syntax (which is '/WByNum %%playerid')");
        }
        return true;
    }
};
...
class SetActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SetActionExecutor()
      : IUnsyncedActionExecutor("Set", "Set a config key=value pair")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (pos != std::string::npos) {
            ...
            LOG_L(L_WARNING,
                  "/set: wrong syntax (which is '/set %%cfgtag %%cfgvalue')");
        }
        return true;
    }
};
...
class SetOverlayActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (pos != std::string::npos) {
            const std::string varName = action.GetArgs().substr(0, pos);
            configHandler->SetString(
              varName, action.GetArgs().substr(pos + 1), true);
        } else {
            LOG_L(L_WARNING,
                  "/tset: wrong syntax (which is '/tset %%cfgtag %%cfgvalue')");
        }
        return true;
    }
};
  ...
  public:
    MouseActionExecutor(int button)
      : IUnsyncedActionExecutor("Mouse" + IntToString(button),
                                "Simulates a mouse button press of button " +
                                  IntToString(button))
      , button(button)
    {}
...
class ViewSelectionActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        // XXX this code is duplicated in CGroup::CalculateCenter(), move to
        // CUnitSet maybe
        float3 pos;
...
{
  public:
    CameraMoveActionExecutor(int moveStateIdx,
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        const std::vector<std::string>& args =
          _local_strSpaceTokenize(action.GetArgs());
        ...
        if (!args.empty()) {
            ...
            CTeam* teamToKill = (teamHandler->IsActiveTeam(teamToKillId))
                                  ? teamHandler->Team(teamToKillId)
                                  : NULL;
            const CTeam* teamToReceiveUnits =
              (teamHandler->IsActiveTeam(teamToReceiveUnitsId))
                ? teamHandler->Team(teamToReceiveUnitsId)
                : NULL;

            if (teamToKill == NULL) {
                LOG_L(L_WARNING,
                      "Team to %s: not a valid team number: \"%s\"",
                      actionName.c_str(),
                      args[0].c_str());
                badArgs = true;
            }
            if (share && teamToReceiveUnits == NULL) {
                LOG_L(L_WARNING,
                      "Team to receive units: not a valid team number: \"%s\"",
                      args[1].c_str());
                badArgs = true;
            }
            if (!badArgs &&
                (skirmishAIHandler.GetSkirmishAIsInTeam(teamToKillId).size() ==
                 0)) {
                LOG_L(L_WARNING,
                      "Team to %s: not a Skirmish AI team: %i",
                      actionName.c_str(),
                      teamToKillId);
                badArgs = true;
            } else {
                const CSkirmishAIHandler::ids_t skirmishAIIds =
                  skirmishAIHandler.GetSkirmishAIsInTeam(teamToKillId,
                                                         gu->myPlayerNum);
                if (!skirmishAIIds.empty()) {
                    skirmishAIId = skirmishAIIds[0];
                } else {
                    LOG_L(L_WARNING,
                          "Team to %s: not a local Skirmish AI team: %i",
                          actionName.c_str(),
                          teamToKillId);
                    badArgs = true;
                }
            ...
            if (!badArgs &&
                skirmishAIHandler.GetSkirmishAI(skirmishAIId)->isLuaAI) {
                LOG_L(L_WARNING,
                      "Team to %s: it is not yet supported to %s Lua AIs",
                      actionName.c_str(),
                      actionName.c_str());
                badArgs = true;
            }
            if (!badArgs) {
                const bool weAreAllied =
                  teamHandler->AlliedTeams(fromTeamId, teamToKillId);
                const bool weAreAIHost =
                  (skirmishAIHandler.GetSkirmishAI(skirmishAIId)->hostPlayer ==
                   gu->myPlayerNum);
                const bool weAreLeader =
                  (teamToKill->GetLeader() == gu->myPlayerNum);

                if (!(weAreAIHost || weAreLeader || singlePlayer ||
                      (weAreAllied && cheating))) {
                    LOG_L(L_WARNING,
                          "Team to %s: player %s is not allowed to %s Skirmish "
                          "AI controlling team %i (try with /cheat)",
                          actionName.c_str(),
                          fromPlayer->name.c_str(),
                          actionName.c_str(),
                          teamToKillId);
                    badArgs = true;
                }
            ...
            if (!badArgs && teamToKill->isDead) {
                LOG_L(L_WARNING,
                      "Team to %s: is a dead team already: %i",
                      actionName.c_str(),
                      teamToKillId);
                badArgs = true;
            }
            ...
            if (!badArgs) {
                if (kill) {
                    if (share) {
                        clientNet->Send(
                          CBaseNetProtocol::Get().SendGiveAwayEverything(
                            gu->myPlayerNum,
                            teamToReceiveUnitsId,
                            teamToKillId));
                        // when the AIs team has no units left,
                        // the AI will be destroyed automatically
                    } else {
                        const SkirmishAIData* sai =
                          skirmishAIHandler.GetSkirmishAI(skirmishAIId);
                        const bool isLocalSkirmishAI =
                          (sai->hostPlayer == gu->myPlayerNum);
                        if (isLocalSkirmishAI) {
                            skirmishAIHandler.SetLocalSkirmishAIDieing(
                              skirmishAIId, 3 /* = AI killed */);
                        }
                    }
                } else {
                    // reload
                    clientNet->Send(CBaseNetProtocol::Get().SendAIStateChanged(
                      gu->myPlayerNum, skirmishAIId, SKIRMAISTATE_RELOADING));
                }

                LOG("Skirmish AI controlling team %i is beeing %sed ...",
                    teamToKillId,
                    actionName.c_str());
            }
        } else {
            LOG_L(L_WARNING,
                  "/%s: missing mandatory argument \"teamTo%s\"",
                  GetCommand().c_str(),
                  actionName.c_str());
            badArgs = true;
        }
        ...
        if (badArgs) {
            if (kill) {
                LOG("description: "
                    "Kill a Skirmish AI controlling a team. The team itsself "
                    "will remain alive, "
                    "unless a second argument is given, which specifies an "
                    "active team "
                    "that will receive all the units of the AI team.");
                LOG("usage:   /%s teamToKill [teamToReceiveUnits]",
                    GetCommand().c_str());
            } else {
                // reload
                LOG("description: "
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (!args.empty()) {
            ...
            const CTeam* teamToControl =
              teamHandler->IsActiveTeam(teamToControlId)
                ? teamHandler->Team(teamToControlId)
                : NULL;
            ...
            if (args.size() >= 2) {
                ...
                LOG_L(L_WARNING,
                      "/%s: missing mandatory argument \"aiShortName\"",
                      GetCommand().c_str());
            }
            if (args.size() >= 3) {
                aiVersion = args[2];
            ...
            if (teamToControl == NULL) {
                LOG_L(L_WARNING,
                      "Team to control: not a valid team number: \"%s\"",
                      args[0].c_str());
                badArgs = true;
            }
            if (!badArgs) {
                const bool weAreAllied =
                  teamHandler->AlliedTeams(fromTeamId, teamToControlId);
                const bool weAreLeader =
                  (teamToControl->GetLeader() == gu->myPlayerNum);
                const bool noLeader = (!teamToControl->HasLeader());

                if (!(weAreLeader || singlePlayer ||
                      (weAreAllied && (cheating || noLeader)))) {
                    LOG_L(L_WARNING,
                          "Team to control: player %s is not allowed to let a "
                          "Skirmish AI take over control of team %i (try with "
                          "/cheat)",
                          fromPlayer->name.c_str(),
                          teamToControlId);
                    badArgs = true;
            ...
            if (!badArgs && teamToControl->isDead) {
                LOG_L(L_WARNING,
                      "Team to control: is a dead team: %i",
                      teamToControlId);
                badArgs = true;
            }
            // TODO remove this, if support for multiple Skirmish AIs per team
            // is in place
            if (!badArgs &&
                (!skirmishAIHandler.GetSkirmishAIsInTeam(teamToControlId)
                    .empty())) {
                LOG_L(L_WARNING,
                      "Team to control: there is already an AI controlling "
                      "this team: %i",
                      teamToControlId);
                badArgs = true;
            }
            if (!badArgs && (skirmishAIHandler.GetLocalSkirmishAIInCreation(
                               teamToControlId) != NULL)) {
                LOG_L(L_WARNING,
                      "Team to control: there is already an AI beeing created "
                      "for team: %i",
                      teamToControlId);
                badArgs = true;
            }
            if (!badArgs) {
                const std::set<std::string>& luaAIImplShortNames =
                  skirmishAIHandler.GetLuaAIImplShortNames();
                if (luaAIImplShortNames.find(aiShortName) !=
                    luaAIImplShortNames.end()) {
                    LOG_L(L_WARNING,
                          "Team to control: it is currently not supported to "
                          "initialize Lua AIs mid-game");
                    badArgs = true;
                }
            }
            ...
            if (!badArgs) {
                ...
                if (aiKey.IsUnspecified()) {
                    LOG_L(L_WARNING,
                          "Skirmish AI: not a valid Skirmish AI: %s %s",
                          aiShortName.c_str(),
                          aiVersion.c_str());
                    badArgs = true;
                } else {
                    const CSkirmishAILibraryInfo& aiLibInfo =
                      aiLibManager->GetSkirmishAIInfos().find(aiKey)->second;
                    ...
                    for (auto o = aiOptions.cbegin(); o != aiOptions.cend();
                         ++o)
            ...
            LOG_L(L_WARNING,
                  "/%s: missing mandatory arguments \"teamToControl\" and "
                  "\"aiShortName\"",
                  GetCommand().c_str());
            badArgs = true;
        }
        ...
        if (badArgs) {
            ...
            LOG("usage:   /%s teamToControl aiShortName [aiVersion] [name] "
                "[options...]",
                GetCommand().c_str());
            LOG("example: /%s 1 RAI 0.601 my_RAI_Friend difficulty=2 "
                "aggressiveness=3",
                GetCommand().c_str());
        }
...
class AIListActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        const CSkirmishAIHandler::id_ai_t& ais =
          skirmishAIHandler.GetAllSkirmishAIs();
        ...
        if (!ais.empty()) {
            ...
            for (ai = ais.begin(); ai != ais.end(); ++ai) {
                ...
                if (isLocal) {
                    lastPart = "(Key:)  " + ai->second.shortName + " " +
                               ai->second.version;
                } else {
                    lastPart =
                      "(Host:) " + playerHandler->Player(gu->myPlayerNum)->name;
                }
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (teamHandler->IsValidTeam(teamId)) {
            clientNet->Send(
              CBaseNetProtocol::Get().SendJoinTeam(gu->myPlayerNum, teamId));
        } else {
...
class SpectatorActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SpectatorActionExecutor()
      : IUnsyncedActionExecutor("Spectator",
                                "Lets the local user give up controll over a "
                                "team, and start spectating")
    {}
...
class SpecTeamActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SpecTeamActionExecutor()
      : IUnsyncedActionExecutor("SpecTeam",
                                "Lets the local user specify the team to "
                                "follow, if he is a spectator")
    {}
...
class AllyActionExecutor : public IUnsyncedActionExecutor
{
  public:
    AllyActionExecutor()
      : IUnsyncedActionExecutor("Ally",
                                "Starts/Ends alliance of the local players "
                                "ally-team with another ally-team")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (action.GetArgs().size() > 0) {
            if (!gameSetup->fixedAllies) {
                ...
                int state = -1;
                is >> state;
                if (state >= 0 && state < 2 && otherAllyTeam >= 0 &&
                    otherAllyTeam != gu->myAllyTeam)
                    clientNet->Send(CBaseNetProtocol::Get().SendSetAllied(
                      gu->myPlayerNum, otherAllyTeam, state));
                else
                    LOG_L(
                      L_WARNING,
                      "/%s: wrong parameters (usage: /%s <other team> [0|1])",
                      GetCommand().c_str(),
                      GetCommand().c_str());
            } else {
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if ((firstChar >= '0') && (firstChar <= '9')) {
            ...
            size_t firstCmdChar =
              action.GetArgs().find_first_not_of(" \t\n\r", 1);
            if (firstCmdChar != std::string::npos) {
                const std::string command =
                  action.GetArgs().substr(firstCmdChar);
  ...
  public:
    GroupIDActionExecutor(int groupId)
      : IUnsyncedActionExecutor("Group" + IntToString(groupId),
                                "Allows modifying the members of group " +
                                  IntToString(groupId))
      , groupId(groupId)
    {}
...
class LastMessagePositionActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        camHandler->CameraTransition(0.6f);
        camHandler->GetCurrentController().SetPos(
          game->infoConsole->GetMsgPos());
        return true;
    }
};
...
class ChatActionExecutor : public IUnsyncedActionExecutor
{
    ...
    ChatActionExecutor(const std::string& commandPostfix,
                       const std::string& userInputPrefix,
                       bool setUserInputPrefix)
      : IUnsyncedActionExecutor("Chat" + commandPostfix,
                                "Starts waiting for intput to be sent to " +
                                  commandPostfix)
  ...
  public:
    ...
    static void RegisterCommandVariants()
    {
        ...
        unsyncedGameCommands->AddActionExecutor(
          new ChatActionExecutor("", "", false));
        unsyncedGameCommands->AddActionExecutor(
          new ChatActionExecutor("All", "", true));
        unsyncedGameCommands->AddActionExecutor(
          new ChatActionExecutor("Ally", "a:", true));
        unsyncedGameCommands->AddActionExecutor(
          new ChatActionExecutor("Spec", "s:", true));
    }
...
// TODO merge together with "TrackOff" to "Track 0|1", and deprecate the two old
// ones
...
class PauseActionExecutor : public IUnsyncedActionExecutor
{
  public:
    PauseActionExecutor()
      : IUnsyncedActionExecutor("Pause", "Pause/Unpause the game")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        clientNet->Send(
          CBaseNetProtocol::Get().SendPause(gu->myPlayerNum, newPause));
        return true;
    }
};
...
class DebugActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        LogSystemStatus("debug-info rendering mode",
                        globalRendering->drawdebug);
        return true;
    }
};
...
class MuteActionExecutor : public IUnsyncedActionExecutor
{
  public:
    MuteActionExecutor()
      : IUnsyncedActionExecutor("MuteSound",
...
{
  public:
    DrawTreesActionExecutor()
      : IUnsyncedActionExecutor("DrawTrees",
...
{
  public:
    DynamicSkyActionExecutor()
      : IUnsyncedActionExecutor("DynamicSky",
...
{
  public:
    DynamicSunActionExecutor()
      : IUnsyncedActionExecutor("DynamicSun",
  ...
  public:
    SpeedControlActionExecutor()
      : IUnsyncedActionExecutor("SpeedControl",
                                "Sets how server adjusts speed according to "
                                "player's load (CPU), 1: use average, 2: use "
                                "highest,")
    {}
...
class GameInfoActionExecutor : public IUnsyncedActionExecutor
{
  public:
    GameInfoActionExecutor()
      : IUnsyncedActionExecutor("GameInfo",
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (globalRendering->fullScreen) {
            ...
            SDL_SetWindowBordered(globalRendering->window,
                                  borderless ? SDL_FALSE : SDL_TRUE);
            SDL_SetWindowSize(globalRendering->window, res.x, res.y);
            SDL_SetWindowPosition(globalRendering->window,
                                  configHandler->GetInt("WindowPosX"),
                                  configHandler->GetInt("WindowPosY"));
        }
        return true;
    }
};
...
class GroundDetailActionExecutor : public IUnsyncedActionExecutor
{
  public:
    GroundDetailActionExecutor()
      : IUnsyncedActionExecutor("GroundDetail",
...
class SpeedUpActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        clientNet->Send(
          CBaseNetProtocol::Get().SendUserSpeed(gu->myPlayerNum, speed));
        return true;
    }
};
...
class ControlUnitActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ControlUnitActionExecutor()
      : IUnsyncedActionExecutor("ControlUnit",
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        clientNet->Send(
          CBaseNetProtocol::Get().SendDirectControl(gu->myPlayerNum));
        return true;
    }
};
...
class ShowPathTravActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        CPathTexture* pathTexInfo = dynamic_cast<CPathTexture*>(
          infoTextureHandler->GetInfoTexture("path"));
        if (pathTexInfo)
...
class ShowPathCostActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ShowPathCostActionExecutor()
      : IUnsyncedActionExecutor("ShowPathCost",
...
class ToggleInfoActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ToggleInfoActionExecutor()
      : IUnsyncedActionExecutor("ToggleInfo",
  ...
  public:
    ShowPathTypeActionExecutor()
      : IUnsyncedActionExecutor("ShowPathType",
                                "Shows path traversability for a given "
                                "MoveDefName, MoveDefID or UnitDefName")
    {}
            ...
            if (!action.GetArgs().empty()) {
                if (!set) {
                    ...
                    if (failed)
                        i = -1;
                    const MoveDef* md =
                      moveDefHandler->GetMoveDefByName(action.GetArgs());
                    if (!md && i < moveDefHandler->GetNumMoveDefs())
                        md = moveDefHandler->GetMoveDefByPathType(i);
                    if (md) {
                        set = true;
                        pathTexInfo->ShowMoveDef(md->pathType);
                        LOG("Showing PathView for MoveDef: %s",
                            md->name.c_str());
                    }
                }
                ...
                if (!set) {
                    const UnitDef* ud =
                      unitDefHandler->GetUnitDefByName(action.GetArgs());
                    if (ud) {
                        set = true;
                        pathTexInfo->ShowUnitDef(ud->id);
                        LOG("Showing BuildView for UnitDef: %s",
                            ud->name.c_str());
                    }
                }
            }
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (inputReceivers.empty() ||
            (dynamic_cast<CShareBox*>(inputReceivers.front()) != NULL))
            return false;
  ...
  public:
    QuitMessageActionExecutor()
      : IUnsyncedActionExecutor("QuitMessage",
                                "Deprecated, see /Quit instead (was used to "
                                "quite the game immediately)")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (inputReceivers.empty() ||
            dynamic_cast<CQuitBox*>(inputReceivers.front()))
            return false;

        const CKeyBindings::HotkeyList& quitList =
          keyBindings->GetHotkeys("quitmenu");
        const std::string quitKey =
          quitList.empty() ? "<none>" : *quitList.begin();
...
class IncreaseGUIOpacityActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        CInputReceiver::guiAlpha =
          std::min(CInputReceiver::guiAlpha + 0.1f, 1.0f);
...
class DecreaseGUIOpacityActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        CInputReceiver::guiAlpha =
          std::max(CInputReceiver::guiAlpha - 0.1f, 0.0f);
        configHandler->Set("GuiOpacity", CInputReceiver::guiAlpha);
        return true;
    }
};
...
class ScreenShotActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ScreenShotActionExecutor()
      : IUnsyncedActionExecutor("ScreenShot",
  ...
  public:
    GrabInputActionExecutor()
      : IUnsyncedActionExecutor("GrabInput",
                                "Prevents/Enables the mouse from leaving the "
                                "game window (windowed mode only)")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        SDL_bool newMode;
        if (action.GetArgs().empty()) {
            newMode = (SDL_GetWindowGrab(globalRendering->window)) ? SDL_FALSE
                                                                   : SDL_TRUE;
        } else {
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        if (action.GetArgs().empty()) {
            ...
            const int argcount =
              sscanf(args, "%f %f %f", &size, &alpha, &scale);
            if (argcount > 1) {
...
class TeamHighlightActionExecutor : public IUnsyncedActionExecutor
{
  public:
    TeamHighlightActionExecutor()
      : IUnsyncedActionExecutor("TeamHighlight",
    ...
    {
        if (action.GetArgs().empty()) {
            globalConfig->teamHighlight = abs(globalConfig->teamHighlight + 1) %
                                          CTeamHighlight::HIGHLIGHT_SIZE;
        } else {
            globalConfig->teamHighlight = abs(atoi(action.GetArgs().c_str())) %
                                          CTeamHighlight::HIGHLIGHT_SIZE;
        }
        LOG("Team highlighting: %s",
            ((globalConfig->teamHighlight == CTeamHighlight::HIGHLIGHT_PLAYERS)
               ? "Players only"
               : ((globalConfig->teamHighlight == CTeamHighlight::HIGHLIGHT_ALL)
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        const std::string fileName =
          action.GetArgs().empty() ? "cmdcolors.txt" : action.GetArgs();
...
class FontActionExecutor : public IUnsyncedActionExecutor
{
  public:
    FontActionExecutor()
      : IUnsyncedActionExecutor("Font", "Reloads the fonts")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        try {
            ...
            const float outlineWeight =
              configHandler->GetFloat("FontOutlineWeight");
            const int smallOutlineWidth =
              configHandler->GetInt("SmallFontOutlineWidth");
            const float smallOutlineWeight =
              configHandler->GetFloat("SmallFontOutlineWeight");

            newFont = CglFont::LoadFont(
              action.GetArgs(), fontSize, outlineWidth, outlineWeight);
            newSmallFont = CglFont::LoadFont(action.GetArgs(),
                                             smallFontSize,
                                             smallOutlineWidth,
                                             smallOutlineWeight);
...
class ConsoleActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ConsoleActionExecutor()
      : IUnsyncedActionExecutor("Console",
...
class FPSHudActionExecutor : public IUnsyncedActionExecutor
{
  public:
    FPSHudActionExecutor()
      : IUnsyncedActionExecutor("FPSHud",
                                "Enables/Disables HUD (GUI interface) shown in "
...
class DebugDrawAIActionExecutor : public IUnsyncedActionExecutor
{
  public:
    DebugDrawAIActionExecutor()
      : IUnsyncedActionExecutor("DebugDrawAI",
...
class MapMarksActionExecutor : public IUnsyncedActionExecutor
{
  public:
    MapMarksActionExecutor()
      : IUnsyncedActionExecutor("MapMarks",
...
class AllMapMarksActionExecutor : public IUnsyncedActionExecutor
{
  public:
    AllMapMarksActionExecutor()
      : IUnsyncedActionExecutor("AllMapMarks",
...
class ClearMapMarksActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ClearMapMarksActionExecutor()
      : IUnsyncedActionExecutor("ClearMapMarks",
...
class NoLuaDrawActionExecutor : public IUnsyncedActionExecutor
{
  public:
    NoLuaDrawActionExecutor()
      : IUnsyncedActionExecutor("NoLuaDraw",
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        LogSystemStatus("Ground-decals rendering",
                        IGroundDecalDrawer::GetDrawDecals());
        return true;
    }
};
...
class GatherModeActionExecutor : public IUnsyncedActionExecutor
{
  public:
    GatherModeActionExecutor()
      : IUnsyncedActionExecutor("GatherMode",
  ...
  public:
    PasteTextActionExecutor()
      : IUnsyncedActionExecutor("PasteText",
                                "Paste either the argument string(s) or if "
                                "none given, the content of the clip-board to "
                                "chat input")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (action.GetInnerAction().rawline.length() > pastecommand.length()) {
            game->userInput.insert(game->writingPos,
                                   action.GetInnerAction().rawline.substr(
                                     pastecommand.length(),
                                     action.GetInnerAction().rawline.length() -
                                       pastecommand.length()));
            game->writingPos +=
              action.GetInnerAction().rawline.length() - pastecommand.length();
        } else {
            game->PasteClipboard();
        }
        return true;
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        if (action.GetInnerAction().rawline.length() > buffercommand.length()) {
            game->consoleHistory->AddLine(
              action.GetInnerAction().rawline.substr(
                buffercommand.length(),
                action.GetInnerAction().rawline.length() -
                  buffercommand.length()));
        } else {
            LOG_L(L_WARNING, "/%s: wrong syntax", GetCommand().c_str());
...
{
  public:
    DistDrawActionExecutor()
      : IUnsyncedActionExecutor(
          "DistDraw",
          "Set the distance between units and camera, at which they turn"
          " into far-textures (flat/texture-only representation) (Graphic "
          "setting)")
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        if (!action.GetArgs().empty()) {
            const vector<string>& args =
              CSimpleParser::Tokenize(action.GetArgs(), 0);

            if (args.size() == 2) {
                const int objType = Clamp(
                  atoi(args[0].c_str()), int(LUAOBJ_UNIT), int(LUAOBJ_FEATURE));
                const float lodScale = atof(args[1].c_str());

                LuaObjectDrawer::SetLODScale(objType, lodScale);
            } else if (args.size() == 3) {
                const int objType = Clamp(
                  atoi(args[1].c_str()), int(LUAOBJ_UNIT), int(LUAOBJ_FEATURE));
...
class WireMapActionExecutor : public IUnsyncedActionExecutor
{
  public:
    WireMapActionExecutor()
      : IUnsyncedActionExecutor("WireMap",
                                "Enable/Disable drawing of the map as "
                                "wire-frame (no textures) (Graphic setting)")
    {}
  ...
  public:
    DebugColVolDrawerActionExecutor()
      : IUnsyncedActionExecutor("DebugColVol",
                                "Enable/Disable drawing of collision volumes")
    {}
...
class DebugPathDrawerActionExecutor : public IUnsyncedActionExecutor
{
  public:
    DebugPathDrawerActionExecutor()
      : IUnsyncedActionExecutor(
          "DebugPath",
          "Enable/Disable drawing of pathfinder debug-data")
    {}

    bool Execute(const UnsyncedAction& action) const
    {
        LogSystemStatus("path-debug rendering mode",
                        pathDrawer->ToggleEnabled());
        return true;
    }
};
...
class DebugTraceRayDrawerActionExecutor : public IUnsyncedActionExecutor
{
  public:
    DebugTraceRayDrawerActionExecutor()
      : IUnsyncedActionExecutor("DebugTraceRay",
                                "Enable/Disable drawing of traceray debug-data")
    {}

    bool Execute(const UnsyncedAction& action) const
    {
        globalRendering->drawdebugtraceray =
          !globalRendering->drawdebugtraceray;
        LogSystemStatus("traceray debug rendering mode",
                        globalRendering->drawdebugtraceray);
        return true;
    }
};
  ...
  public:
    CrashActionExecutor()
      : IUnsyncedActionExecutor("Crash",
                                "Invoke an artificial crash through a "
  ...
  public:
    GiveActionExecutor()
      : IUnsyncedActionExecutor(
          ...
          " map, instantly; by default to your own team",
          true)
    {}
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        if (action.GetArgs().find('@') == string::npos) {
            ...
            if (ir == minimap) {
                ...
                const float dist =
                  CGround::LineGroundCol(pos, pos + (dir * 30000.0f));
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        for (CUnitSet::iterator it = selectedUnitsHandler.selectedUnits.begin();
             it != selectedUnitsHandler.selectedUnits.end();
             ++it) {
            ss << " " << (*it)->id;
        }
...
class SendActionExecutor : public IUnsyncedActionExecutor
{
  public:
    SendActionExecutor()
      : IUnsyncedActionExecutor("Send",
                                "Send a string as raw network message to the "
                                "game host (for debugging only)")
    {}
...
{
  public:
    SaveGameActionExecutor()
      : IUnsyncedActionExecutor("SaveGame",
...
class DumpStateActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        switch (args.size()) {
            case 2: {
                DumpState(atoi(args[0].c_str()), atoi(args[1].c_str()), 1);
            } break;
            case 3: {
                DumpState(atoi(args[0].c_str()),
                          atoi(args[1].c_str()),
                          atoi(args[2].c_str()));
            } break;
            default: {
  ...
  public:
    SaveActionExecutor(bool _usecreg)
      : IUnsyncedActionExecutor((_usecreg ? "Save" : "LuaSave"),
                                "Save the game state to a specific file, add "
                                "-y to overwrite when file is already present")
      , usecreg(_usecreg)
    {}
  ...
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        if (action.GetArgs() == "sound") {
            ...
            LOG_L(L_WARNING,
                  "Give either of these as argument: sound, profiling");
        }
        return true;
    }
};
...
class RedirectToSyncedActionExecutor : public IUnsyncedActionExecutor
{
  public:
    RedirectToSyncedActionExecutor(const std::string& command)
      : IUnsyncedActionExecutor(command,
                                "Redirects command /" + command +
                                  " to its synced processor")
    {}
...
class CommandListActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    static void PrintExecutorToConsole(
      const IActionExecutor<action_t, synced_v>* executor)
    {
  ...
  private:
    void PrintToConsole(
      const std::map<std::string, ISyncedActionExecutor*>& executors) const
    {
    ...
    void PrintToConsole(
      const std::map<std::string, IUnsyncedActionExecutor*>& executors) const
    {
...
class CommandHelpActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    bool Execute(const UnsyncedAction& action) const
    {
        ...
        const std::vector<std::string> args =
          CSimpleParser::Tokenize(action.GetArgs(), 1);
        if (!args.empty()) {
            const std::string commandLower = StringToLower(args[0]);

            // try if an unsynced chat command with this name is available
            const IUnsyncedActionExecutor* unsyncedExecutor =
              unsyncedGameCommands->GetActionExecutor(commandLower);
            ...
            const ISyncedActionExecutor* syncedExecutor =
              syncedGameCommands->GetActionExecutor(commandLower);
            ...
            LOG_L(
              L_WARNING,
              "No chat command registered with name \"%s\" (case-insensitive)",
              args[0].c_str());
        } else {
  ...
  private:
    template<class action_t, bool synced_v>
    static void PrintExecutorHelpToConsole(
      const IActionExecutor<action_t, synced_v>* executor)
    {

        // XXX extend this in case more info about commands are available (for
        // example "Usage: name {args}")
...
bool
CGame::ActionReleased(const Action& action)
{
    ...
    // HACK   somehow weird things happen when MouseRelease is called for button
    // 4 and 5. Note that SYS_WMEVENT on windows also only sends MousePress
...
UnsyncedGameCommands::CreateInstance()
{
    if (singleton == NULL) {
        ...
        throw std::logic_error(
          "UnsyncedGameCommands singleton is already initialized");
    }
}
...
UnsyncedGameCommands::DestroyInstance()
{
    if (singleton != NULL) {
        ...
        LOG_L(L_WARNING,
              "UnsyncedGameCommands singleton was not initialized or is "
              "already destroyed");
    }
}
