...
CPlayer::SetControlledTeams()
{
    ...
    // AI teams
    const CSkirmishAIHandler::id_ai_t aiIds =
      skirmishAIHandler.GetAllSkirmishAIs();
    for (CSkirmishAIHandler::id_ai_t::const_iterator ai = aiIds.begin();
         ai != aiIds.end();
         ++ai) {
...
CPlayer::JoinTeam(int newTeam)
{
    ...
    if (gu->myPlayerNum == this->playerNum) {
        ...
        gu->myPlayingAllyTeam = gu->myAllyTeam =
          teamHandler->AllyTeam(gu->myTeam);
...
CPlayer::StartControllingUnit()
{
    ...
    if (curControlleeUnit != NULL) {
        ...
        // player took control
        const std::vector<int>& ourSelectedUnits =
          selectedUnitsHandler.netSelected[this->playerNum];
        ...
        if (eventHandler.AllowDirectUnitControl(this->playerNum,
                                                newControlleeUnit)) {
