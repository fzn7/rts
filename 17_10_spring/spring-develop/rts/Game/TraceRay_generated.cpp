               ...
               const CSolidObject* obj)
{
    ...
    if (obj->GetBlockingMapID() < unitHandler->MaxUnits()) {
        // obj is a unit
        if (!ret) {
            ret = ((cv->GetPointSurfaceDistance(
                      static_cast<const CUnit*>(obj), NULL, pos3D) -
                    coneSize) <= 0.0f);
        }
        if (!ret) {
            ret = ((cv->GetPointSurfaceDistance(
                      static_cast<const CUnit*>(obj), NULL, expPos3D) -
                    coneSize) <= 0.0f);
        }
    } else {
        // obj is a feature
        if (!ret) {
            ret = ((cv->GetPointSurfaceDistance(
                      static_cast<const CFeature*>(obj), NULL, pos3D) -
                    coneSize) <= 0.0f);
        }
        if (!ret) {
            ret = ((cv->GetPointSurfaceDistance(
                      static_cast<const CFeature*>(obj), NULL, expPos3D) -
                    coneSize) <= 0.0f);
        }
    }
...
#define go geometricObjects
        ...
        if (ret) {
            go->SetColor(
              go->AddLine(expPos3D - (UpVector * expPos3D.dot(UpVector)),
                          expPos3D,
                          3,
                          1,
                          GAME_SPEED),
                         ...
                         const CSolidObject* obj)
{
    ...
    const float3 expPos3D =
      expPos2D +
      (UpVector * (quadratic * objDst1D * objDst1D + linear * objDst1D));
...
#define go geometricObjects
        ...
        if (ret) {
            go->SetColor(go->AddLine(expPos2D, expPos3D, 3, 1, GAME_SPEED),
                         1.0f,
                         0.0f,
                         0.0f,
                         1.0f);
         ...
         CollisionQuery* hitColQuery)
{
        ...
        if (!ignoreFeatures) {
            for (const int quadIdx : quads) {
                ...
                for (CFeature* f : quad.features) {
                    ...
                    //     for collisions with projectiles so we can skip it
                    //     here
                    if (!f->HasCollidableStateBit(
                          CSolidObject::CSTATE_BIT_QUADMAPRAYS))
                        continue;

                    if (CCollisionHandler::DetectHit(
                          f,
                          f->GetTransformMatrix(true),
                          start,
                          start + dir * length,
                          &cq,
                          true)) {
                        ...
                        // we want the closest feature (intersection point) on
                        // the ray
                        if (len < length) {
                            length = len;
                            hitFeature = f;
            ...
            for (const int quadIdx : quads) {
                ...
                for (CUnit* u : quad.units) {
                    ...
                    if (CCollisionHandler::DetectHit(
                        ...
                        // we want the closest unit (intersection point) on the
                        // ray
                        if (len < length) {
                            length = len;
                            hitUnit = u;
    ...
    if (!ignoreGround) {
        // ground intersection
        const float groundLength =
          CGround::LineGroundCol(start, start + dir * length);
                ...
                std::vector<SShieldDist>& hitShields)
{
    ...
    for (const int quadIdx : quads) {
        ...
        for (CPlasmaRepulser* r : quad.repulsers) {
            if (!r->CanIntercept(emitter->weaponDef->interceptedByShieldType,
                                 emitter->owner->allyteam))
                continue;

            if (CCollisionHandler::DetectHit(r->owner,
                                             &r->collisionVolume,
                                             r->owner->GetTransformMatrix(true),
                                             start,
                                             start + dir * length,
                                             &cq,
                                             true)) {
                ...
                const auto hitCmp = [](const float a, const SShieldDist& b) {
                    return (a < b.dist);
                };
                const auto insPos = std::upper_bound(
                  hitShields.begin(), hitShields.end(), len, hitCmp);
            ...
            bool ignoreWater)
{
    ...
    const float groundRayLength =
      CGround::LineGroundCol(start, dir, guiRayLength, false);
    const float waterRayLength =
      CGround::LinePlaneCol(start, dir, guiRayLength, 0.0f);
    ...
    for (const int quadIdx : quads) {
        ...
        for (const CUnit* u : quad.units) {
            const bool unitIsEnemy =
              !teamHandler->Ally(u->allyteam, gu->myAllyTeam);
            const bool unitOnRadar =
              (useRadar && losHandler->InRadar(u, gu->myAllyTeam));
            const bool unitInSight =
              (u->losStatus[gu->myAllyTeam] & (LOS_INLOS | LOS_CONTRADAR));
            const bool unitVisible = !unitIsEnemy || unitOnRadar ||
                                     unitInSight || gu->spectatingFullView;

            if (u == exclude)
                continue;
            // test this bit only in synced traces, rely on noSelect here
            if (false &&
                !u->HasCollidableStateBit(CSolidObject::CSTATE_BIT_QUADMAPRAYS))
                continue;
            if (u->noSelect)
                continue;
            if (!unitVisible)
            ...
            if (CCollisionHandler::MouseHit(u,
                                            u->GetTransformMatrix(false),
                                            start,
                                            start + dir * guiRayLength,
                                            &cv,
                                            &cq)) {
                ...
                const bool factoryHitBeforeUnit =
                  ((hitFactory && ingressDist < minIngressDist) ||
                   (!hitFactory && egressDist < minIngressDist));
                const bool unitHitInsideFactory =
                  ((hitFactory && ingressDist < minEgressDist) ||
                   (!hitFactory && ingressDist < minIngressDist));

                // give units in a factory higher priority than the factory
                // itself
                if (hitUnit == nullptr ||
                    (factoryUnderCursor && factoryHitBeforeUnit) ||
        ...
        for (const CFeature* f : quad.features) {
            if (!gu->spectatingFullView &&
                !f->IsInLosForAllyTeam(gu->myAllyTeam))
                continue;
            // test this bit only in synced traces, rely on noSelect here
            if (false &&
                !f->HasCollidableStateBit(CSolidObject::CSTATE_BIT_QUADMAPRAYS))
                continue;
            if (f->noSelect)
                continue;
            ...
            if (CCollisionHandler::MouseHit(f,
                                            f->GetTransformMatrix(false),
                                            start,
                                            start + dir * guiRayLength,
                                            &cv,
                                            &cq)) {
                ...
                const bool factoryHitBeforeUnit =
                  (hitFactory && hitDist < minEgressDist);
                const bool unitHitInsideFactory =
                  (!hitFactory && hitDist < minIngressDist);

                // we want the closest feature (intersection point) on the ray
                // give features in a factory (?) higher priority than the
                // factory itself
                if (hitUnit == nullptr || factoryHitBeforeUnit ||
                    unitHitInsideFactory) {
                    hitFactory = false;
    ...
    for (const int quadIdx : quads) {
        ...
        if (!ignoreAllies) {
            for (const CUnit* u : quad.teamUnits[allyteam]) {
                ...
                if (TestTrajectoryConeHelper(
                      from, dir, length, linear, quadratic, spread, 0.0f, u)) {
                    return true;
                }
            }
        }
        ...
        if (!ignoreFeatures) {
            for (const CFeature* f : quad.features) {
                ...
                if (TestTrajectoryConeHelper(
                      from, dir, length, linear, quadratic, spread, 0.0f, f))
                    return true;
            }
        }
    }
