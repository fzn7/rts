...
CSpringController::KeyMove(float3 move)
{
    ...
    pos += (camera->GetRight() * move.x + flatForward * move.y) * pixelSize *
           2.0f * scrollSpeed;
    Update();
}
...
CSpringController::MouseWheelMove(float move)
{
    const float shiftSpeed =
      (KeyInput::GetKeyModState(KMOD_SHIFT) ? 2.0f : 1.0f);
    ...
    if (KeyInput::GetKeyModState(KMOD_CTRL)) {
        rot.x =
          Clamp(rot.x - (move * shiftSpeed * 0.005f), PI * 0.51f, PI * 0.99f);
...
CSpringController::ZoomIn(const float3& curCamPos, const float2& zoomParams)
{
    ...
    const float zoomInDist = CGround::LineGroundCol(
      curCamPos, curCamPos + mouse->dir * 150000.f, false);
    ...
    const float3 wantedCamPos =
      zoomedCamPos - mouse->dir * zoomInDist * zoomParams.y;

    // figure out how far we will end up from the ground at new wanted point
    const float newDist = CGround::LineGroundCol(
      wantedCamPos, wantedCamPos + dir * 150000.f, false);
...
CSpringController::ZoomOut(const float3& curCamPos, const float2& zoomParams)
{
    if (KeyInput::GetKeyModState(KMOD_ALT)) {
        ...
        pos.z =
          mapDims.mapy * SQUARE_SIZE * 0.55f; // somewhat longer toward bottom
...
{
    const float minRot =
      std::floor(rot.y / fastmath::HALFPI) * fastmath::HALFPI;
...
CSpringController::GetPos() const
{
    ...
    cpos.y = std::max(
      cpos.y, CGround::GetHeightAboveWater(cpos.x, cpos.z, false) + 5.0f);
    return cpos;
}
