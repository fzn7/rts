...
#pragma GCC diagnostic ignored "-Wformat"
    ...
    fprintf(pFile,
            "# GAME_FRAME effFPS drawFPS simFPS num_units num_features "
            "game_speed lua_usage\n");
    while (dit != drawFPS.cend() && sit != simFPS.cend()) {
        if (dit->first < sit->first) {
            fprintf(pFile,
                    "%f %f %f %f " _STPF_ " " _STPF_ " %f %f\n",
                    dit->first,
                    rit->second,
                    dit->second,
                    sit->second,
...
CBenchmark::GameFrame(int gameFrame)
{
    ...
    if (gameFrame >= startFrame) {
        simFPS[gameFrame] =
          (gu->avgSimFrameTime == 0.0f) ? 0.0f : 1000.0f / gu->avgSimFrameTime;
...
{
    if (!simFPS.empty()) {
        realFPS[game->lastSimFrame + globalRendering->timeOffset] =
          globalRendering->FPS;
        drawFPS[game->lastSimFrame + globalRendering->timeOffset] =
          (gu->avgDrawFrameTime == 0.0f) ? 0.0f
                                         : 1000.0f / gu->avgDrawFrameTime;
    }
}
