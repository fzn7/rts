...
CBitmap::CBitmap(const unsigned char* data,
                 int _xsize,
                 int _ysize,
                 int _channels)
  : xsize(_xsize)
...
CBitmap::Load(std::string const& filename, unsigned char defaultAlpha)
{
    ...
    if (filename.find(".dds") != std::string::npos) {
        ...
        AllocDummy(); // allocate a dummy texture, as dds aren't supported in
                      // headless
        return true;
#endif // !BITMAP_NO_OPENGL
    }
...

    {
        ...
        const bool success =
          !!ilLoadL(IL_TYPE_UNKNOWN, buffer, file.FileSize());
...
CBitmap::Save(std::string const& filename, bool opaque) const
{
    ...
    const std::string fullpath =
      dataDirsAccess.LocateFile(filename, FileQueryFlags::WRITE);
...
CBitmap::SaveFloat(std::string const& filename) const
{
    ...
    // seems IL_ORIGIN_SET only works in ilLoad and not in ilTexImage nor in
    // ilSaveImage so we need to flip the image ourself
    ...
    // note: DevIL only generates a 16bit grayscale PNG when format is
    // IL_UNSIGNED_SHORT!
...
CBitmap::CreateTexture(float aniso, bool mipmaps) const
{
    ...
    // GL_ARB_texture_non_power_of_two indicates that the hardware will actually
    // support it.
    if (!globalRendering->supportNPOTs &&
        (xsize != next_power_of_2(xsize) || ysize != next_power_of_2(ysize))) {
        CBitmap bm =
          CreateRescaled(next_power_of_2(xsize), next_power_of_2(ysize));
    ...
    if (mipmaps) {
        ...
        // glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8 ,xsize, ysize, 0,GL_RGBA,
        // GL_UNSIGNED_BYTE, &mem[0]);  gluBuild2DMipmaps(GL_TEXTURE_2D,GL_RGBA8
        // ,xsize, ysize, GL_RGBA, GL_UNSIGNED_BYTE, &mem[0]);
...
CBitmap::CreateDDSTexture(unsigned int texID, float aniso, bool mipmaps) const
{
    ...
    switch (ddsimage->get_type()) {
        ...
        case nv_dds::TextureFlat: // 1D, 2D, and rectangle textures
            ...
            if (aniso > 0.0f)
                glTexParameterf(
                  GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, aniso);
            HandleDDSMipmap(
              GL_TEXTURE_2D, mipmaps, ddsimage->get_num_mipmaps());
            break;
        case nv_dds::Texture3D:
            ...
            HandleDDSMipmap(
              GL_TEXTURE_3D, mipmaps, ddsimage->get_num_mipmaps());
            break;
        case nv_dds::TextureCubemap:
            ...
            if (aniso > 0.0f)
                glTexParameterf(
                  GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAX_ANISOTROPY_EXT, aniso);
            HandleDDSMipmap(
              GL_TEXTURE_CUBE_MAP, mipmaps, ddsimage->get_num_mipmaps());
            break;
        default:
            assert(false);
            break;
    ...
    for (int a = 0; a < 3; ++a) {
        ...
        for (int y = 0; y < ysize; ++y) {
            for (int x = 0; x < xsize; ++x) {
                ...
                if ((mem[index + 3] != 0) &&
                    !((mem[index + 0] == red) && (mem[index + 1] == green) &&
                      (mem[index + 2] == blue))) {
                    cCol += mem[index + a];
                    ++numCounted;
                }
    ...
    for (int a = 0; a < 3; ++a) {
        for (int y = 0; y < ysize; ++y) {
            for (int x = 0; x < xsize; ++x) {
                ...
                mem[index + a] =
                  (unsigned char)(std::min(255.f, std::max(0.0f, nc * 255)));
            }
        }
    }
}
...
CBitmap::CopySubImage(const CBitmap& src, int xpos, int ypos)
{
    if (xpos + src.xsize > xsize || ypos + src.ysize > ysize) {
        LOG_L(L_WARNING,
              "CBitmap::CopySubImage src image does not fit into dst!");
        return;
    }

    if (compressed || src.compressed) {
        LOG_L(L_WARNING,
              "CBitmap::CopySubImage can't copy compressed textures!");
        return;
    }
    ...
    for (int y = 0; y < src.ysize; ++y) {
        ...
        // copy the whole line
        std::copy(&src.mem[pixelSrc],
                  &src.mem[pixelSrc] + channels * src.xsize,
                  &mem[pixelDst]);
    }
}
...
CBitmap::CanvasResize(const int newx, const int newy, const bool center) const
{
    ...
    if (xsize > newx || ysize > newy) {
        LOG_L(L_WARNING,
              "CBitmap::CanvasResize can only upscale (tried to resize %ix%i "
              "to %ix%i)!",
              xsize,
              ysize,
              newx,
...
CBitmap::CreateSDLSurface() const
{
    ...
    if (channels < 3) {
        LOG_L(L_WARNING,
              "CBitmap::CreateSDLSurface works only with 24bit RGB and 32bit "
              "RGBA pictures!");
        return surface;
    }
    ...
    surface = SDL_CreateRGBSurfaceFrom(surfData,
                                       xsize,
                                       ysize,
                                       8 * channels,
                                       xsize * channels,
...
CBitmap::CreateRescaled(int newx, int newy) const
{
    ...
    if (compressed) {
        LOG_L(L_WARNING,
              "CBitmap::CreateRescaled doesn't work with compressed textures!");
        bm.AllocDummy();
        return bm;
    }
...
CBitmap::GrayScale()
{
    ...
    for (int y = 0; y < ysize; ++y) {
        for (int x = 0; x < xsize; ++x) {
            const int base = ((y * xsize) + x) * 4;
            const float illum = (mem[base + 0] * 0.299f) +
                                (mem[base + 1] * 0.587f) +
                                (mem[base + 2] * 0.114f);
...
CBitmap::ReverseYAxis()
{
    ...
    for (int y = 0; y < (ysize / 2); ++y) {
        ...
        // copy the whole line
        std::copy(mem.begin() + pixelHigh,
                  mem.begin() + pixelHigh + channels * xsize,
                  tmpLine);
        std::copy(mem.begin() + pixelLow,
                  mem.begin() + pixelLow + channels * xsize,
                  mem.begin() + pixelHigh);
