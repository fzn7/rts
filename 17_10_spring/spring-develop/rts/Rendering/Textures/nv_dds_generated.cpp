...
//     bool upload_texture2D(unsigned int imageIndex = 0, GLenum target =
//     GL_TEXTURE_2D); bool upload_textureRectangle(); bool upload_texture3D();
...
void
CDDSImage::create_textureFlat(unsigned int format,
...
void
CDDSImage::create_texture3D(unsigned int format,
                            unsigned int components,
                            const CTexture& baseImage)
{
    assert(format != 0);
...
inline bool
same_size(const CTexture& a, const CTexture& b)
{
...
void
CDDSImage::create_textureCubemap(unsigned int format,
                                 unsigned int components,
...
bool
CDDSImage::load(string filename, bool flipImage)
{
    ...
    } else if (ddsh.ddspf.dwFlags == DDSF_RGBA &&
               ddsh.ddspf.dwRGBBitCount == 32) {
        m_format = GL_BGRA_EXT;
        m_components = 4;
    } else if (ddsh.ddspf.dwFlags == DDSF_RGB &&
               ddsh.ddspf.dwRGBBitCount == 32) {
        m_format = GL_BGRA_EXT;
        m_components = 4;
    } else if (ddsh.ddspf.dwFlags == DDSF_RGB &&
    ...
    for (unsigned int n = 0;
         n < (unsigned int)(m_type == TextureCubemap ? 6 : 1);
         n++) {
...
bool
CDDSImage::write_texture(const CTexture& texture, FILE* fp)
{
...
bool
CDDSImage::save(std::string filename, bool flipImage)
{
    assert(m_valid);
    ...
    if (is_compressed()) {
        ...
        ddsh.dwPitchOrLinearSize =
          get_dword_aligned_linesize(get_width(), m_components * 8);
    }
...
CDDSImage::upload_texture1D()
{
    ...
    if (is_compressed()) {
        glCompressedTexImage1DARB(GL_TEXTURE_1D,
                                  0,
                                  m_format,
                                  baseImage.get_width(),
                                  0,
...
bool
CDDSImage::upload_texture2D(unsigned int imageIndex, int target)
{
    assert(m_valid);
    assert(!m_images.empty());
    assert(imageIndex >= 0);
...
bool
CDDSImage::upload_textureRectangle()
{
...
bool
CDDSImage::upload_textureCubemap()
{
    assert(m_valid);
    assert(!m_images.empty());
...
inline unsigned int
CDDSImage::clamp_size(unsigned int size)
{
    if (size <= 0)
        size = 1;
...
inline unsigned int
CDDSImage::size_dxtc(unsigned int width, unsigned int height)
{
...
inline unsigned int
CDDSImage::size_rgb(unsigned int width, unsigned int height)
{
...
void
CDDSImage::flip(CSurface& surface)
{
    ...
    if (!is_compressed()) {
        ...
        for (unsigned int j = 0; j < (yblocks >> 1); j++) {
            ...
            bottom = (DXTColBlock*)((unsigned char*)surface +
...
void
CDDSImage::flip_texture(CTexture& texture)
{
    flip(texture);
...
void
CDDSImage::swap(void* byte1, void* byte2, unsigned int size)
{
...
void
CDDSImage::flip_blocks_dxtc1(DXTColBlock* line, unsigned int numBlocks)
{
...
void
CDDSImage::flip_blocks_dxtc3(DXTColBlock* line, unsigned int numBlocks)
{
    DXTColBlock* curblock = line;
...
void
CDDSImage::flip_dxt5_alpha(DXT5AlphaBlock* block)
{
...
void
CDDSImage::flip_blocks_dxtc5(DXTColBlock* line, unsigned int numBlocks)
{
    DXTColBlock* curblock = line;
...
CTexture::CTexture(unsigned int w,
                   unsigned int h,
                   unsigned int d,
...
void
CTexture::create(unsigned int w,
                 unsigned int h,
                 unsigned int d,
                 unsigned int imgsize,
                 const unsigned char* pixels)
...
CSurface::CSurface(unsigned int w,
                   unsigned int h,
                   unsigned int d,
                   unsigned int imgsize,
                   const unsigned char* pixels)
...
// creates an empty image
void
CSurface::create(unsigned int w,
                 unsigned int h,
                 unsigned int d,
                 unsigned int imgsize,
                 const unsigned char* pixels)
