...
Load(const std::string& texName, unsigned int texID)
{
    ...
    if (bitmap.compressed) {
        ...
        if (resize)
            bitmap =
              bitmap.CreateRescaled(resizeDimensions.x, resizeDimensions.y);
        if (invert)
            bitmap.InvertColors();
        if (greyed)
            bitmap.GrayScale();
        ...
        if (nearest || linear) {
            ...
            if (!(count_bits_set(bitmap.xsize) == 1 &&
                  count_bits_set(bitmap.ysize) == 1) &&
                (!GLEW_ARB_texture_non_power_of_two ||
                 (globalRendering->atiHacks && nearest))) {
                bitmap = bitmap.CreateRescaled(next_power_of_2(bitmap.xsize),
                                               next_power_of_2(bitmap.ysize));
            }
        ...
        if (aniso && GLEW_EXT_texture_filter_anisotropic) {
            ...
            glTexParameterf(
              GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, maxAniso);
        }
    }
