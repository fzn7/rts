...
inline int
CLegacyAtlasAlloc::CompareTex(SAtlasEntry* tex1, SAtlasEntry* tex2)
{
...
bool
CLegacyAtlasAlloc::IncreaseSize()
{
...
CLegacyAtlasAlloc::Allocate()
{
    ...
    for (int a = 0; a < static_cast<int>(memtextures.size()); ++a) {
        ...
        while (!done) {
            ...
            if ((thisSub.front().x + curtex->size.x + TEXMARGIN) >
                atlasSize.x) {
                thisSub.clear();
                continue;
            }
            ...
            if ((thisSub.front().y + curtex->size.y + TEXMARGIN) < max.y) {
                nextSub.push_back(
                  int2(thisSub.front().x + TEXMARGIN,
                       thisSub.front().y + curtex->size.y + TEXMARGIN));
            }

            thisSub.front().x += (curtex->size.x + TEXMARGIN);

            while (thisSub.size() > 1 &&
                   thisSub.front().x >= (++thisSub.begin())->x) {
        ...
        if (recalc) {
            ...
            for (std::vector<SAtlasEntry*>::iterator it = memtextures.begin();
                 it != memtextures.end();
                 ++it) {
                (*it)->texCoords = float4();
            }
