...
{
    drawArray = new float[VA_INIT_VERTEXES]; // please don't change this, some
                                             // files rely on specific initial
                                             // sizes
...
{
    if ((char*)stripArrayPos >
        ((char*)stripArraySize - 4 * sizeof(unsigned int))) {
        EnlargeStripArray();
    }
...
CVertexArray::DrawArrays(const GLenum mode, const unsigned int stride)
{
    ...
    if (!IsPrimitiveSplitable(mode)) {
        ...
        const int optVertCount =
          maxVertices - (maxVertices % GetPrimitiveRestartEach(mode));
...
#define SET_DISABLE_ACTIVE_TEX(texUnit)                                        \
    ...
    glTexCoordPointer(
      2, GL_FLOAT, stride, drawArray + 3); // FIXME? (format-specific)
