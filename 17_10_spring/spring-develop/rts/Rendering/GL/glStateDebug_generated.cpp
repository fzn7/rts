...
VERIFYGL(F func,
         ...
         std::string area)
{
    if (CGLStateChecker::errorsSet.find(area + pstr) ==
        CGLStateChecker::errorsSet.end()) {
        T _temp;
        func(pname, &_temp);
        if (_temp != defaultValue) {
            LOG_L(L_ERROR,
                  "%s was not set correctly when %s: %f",
                  pstr.c_str(),
                  area.c_str(),
                  (float)_temp);
            LOG_L(L_ERROR,
                  "last time set here: %s",
                  CGLStateChecker::lastSet[pstr].c_str());
          ...
          std::string area)
{
    if (CGLStateChecker::errorsSet.find(area + pstr) ==
        ...
        if (_temp[0] != default0 || _temp[1] != default1) {
            LOG_L(L_ERROR,
                  "%s was not set correctly when %s: %f %f",
                  pstr.c_str(),
                  area.c_str(),
                  (float)_temp[0],
                  (float)_temp[1]);
          ...
          std::string area)
{
    if (CGLStateChecker::errorsSet.find(area + pstr) ==
        ...
        func(pname, _temp);
        if (_temp[0] != default0 || _temp[1] != default1 ||
            _temp[2] != default2 || _temp[3] != default3) {
            LOG_L(L_ERROR,
                  "%s was not set correctly when %s: %f %f %f %f",
                  pstr.c_str(),
                  area.c_str(),
                  (float)_temp[0],
                  (float)_temp[1],
...
CGLStateChecker::VerifyState(std::string area)
{
    if (Threading::IsMainThread()) {
        ...
        VERIFYGLBOOL4(
          GL_COLOR_WRITEMASK, GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE, _area);
