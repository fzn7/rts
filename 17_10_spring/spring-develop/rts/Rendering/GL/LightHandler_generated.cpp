...
GL::LightHandler::AddLight(const GL::Light& light)
{
    ...
    const auto it =
      std::find_if(lights.begin(), lights.end(), [&](const GL::Light& lgt) {
          return (lgt.GetTTL() == 0);
      });
...
GL::LightHandler::Update(Shader::IProgramObject* shader)
{
    ...
    for (GL::Light& light : lights) {
        ...
        const float4& lightPos = (lightTrackPos != nullptr)
                                   ? float4(*lightTrackPos, 1.0f)
                                   : light.GetPosition();
        const float3& lightDir = (lightTrackDir != nullptr)
                                   ? float3(*lightTrackDir)
                                   : light.GetDirection();
        const bool lightVisible =
          (gu->spectatingFullView || light.GetIgnoreLOS() ||
           losHandler->InLos(lightPos, gu->myAllyTeam));
        ...
        if (light.GetRelativeTime() > light.GetTTL()) {
            ...
            glLightf(
              lightID, GL_CONSTANT_ATTENUATION, light.GetAttenuation().x);
            glLightf(lightID, GL_LINEAR_ATTENUATION, light.GetAttenuation().y);
            glLightf(
              lightID, GL_QUADRATIC_ATTENUATION, light.GetAttenuation().z);
#endif
            glDisable(lightID);
        }
    }
