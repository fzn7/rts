...
static void
ParseUniformsTable(Shader::IProgramObject* program,
                   const LuaTable* root,
            ...
            case UNIFORM_TYPE_FLOAT_MATRIX: {
                if (subTable.GetType(key) == LuaTable::TABLE) {
                    ...
                    switch (values.GetLength()) {
                        ...
                        default:
                            LOG_L(L_ERROR,
                                  "%s-shader uniformMatrix error: only square "
                                  "matrices supported right now: %s",
...
ParseUniformSetupTables(Shader::IProgramObject* program, const LuaTable* root)
{
    ...
    ParseUniformsTable(
      program, root, "uniformMatrix", UNIFORM_TYPE_FLOAT_MATRIX);
}
                   ...
                   const GLenum type)
{
    ...
    program->AttachShaderObject(
      shaderHandler->CreateShaderObject(sources, definitions, type));
}
    ...
    switch (keyType) {
        ...
        default:
            if (root->KeyExists(key)) {
                // FIXME shaders.errorLog = "\"" + string(key) + "\" must be a
                // string or a table value!";
            }
    };
}
...
LoadFromLua(Shader::IProgramObject* program, const std::string& filename)
{
    ...
    if (!p.Execute()) {
        LOG_L(L_ERROR,
              "Failed to parse lua shader \"%s\": %s",
              filename.c_str(),
              p.GetErrorLog().c_str());
        return false;
    }
    ...
    if (vertSrcs.str().empty() && fragSrcs.str().empty() &&
        geomSrcs.str().empty())
        return false;

    CreateShaderObject(
      program, shdrDefs.str(), vertSrcs.str(), GL_VERTEX_SHADER);
    CreateShaderObject(
      program, shdrDefs.str(), geomSrcs.str(), GL_GEOMETRY_SHADER_EXT);
    CreateShaderObject(
      program, shdrDefs.str(), fragSrcs.str(), GL_FRAGMENT_SHADER);
    ...
    if (!program->IsValid()) {
        ...
        LOG_L(
          L_ERROR, fmt, program->GetName().c_str(), program->GetLog().c_str());
        return false;
    } else {
        program->Enable();
