...
CFarTextureHandler::CFarTextureHandler()
{
    ...
    texSize.y = std::max(iconSize.y,
                         4 * NUM_ICON_ORIENTATIONS * iconSize.x * iconSize.y /
                           texSize.x); // minimum space for 4 icons
...
CFarTextureHandler::HaveFarIcon(const CSolidObject* obj) const
{
    ...
    return ((iconCache.size() > teamID) &&
            (iconCache[teamID].size() > modelID) &&
            (iconCache[teamID][modelID].farTexNum != 0));
}
...
CFarTextureHandler::CreateFarTexture(const CSolidObject* obj)
{
    ...
    fbo.CreateRenderBuffer(
      GL_DEPTH_ATTACHMENT_EXT, GL_DEPTH_COMPONENT16, texSize.x, texSize.y);
    ...
    viewMat.Translate(
      float3(0.0f, 0.0f, -obj->GetDrawRadius() * (2.0f + 1.0f)));
    ...
    for (int orient = 0; orient < NUM_ICON_ORIENTATIONS; ++orient) {
        ...
        glViewport(
          pos.x * iconSize.x, pos.y * iconSize.y, iconSize.x, iconSize.y);
    ...
    iconCache[obj->team][model->id].texScales = { obj->GetDrawRadius(),
                                                  obj->GetDrawRadius() };
    iconCache[obj->team][model->id].texOffset =
      UpVector * obj->GetDrawRadius() * 0.5f;
}
...
CFarTextureHandler::DrawFarTexture(const CSolidObject* obj, CVertexArray* va)
{
    ...
    int orient =
      GetHeadingFromVector(-camera->GetDir().x, -camera->GetDir().z) -
      obj->heading;
    orient += USHRT_MAX_; // make it positive only
    orient +=
      (orientStep >>
       1); // we want that frontdir is from -orientStep/2 upto orientStep/2
    orient %= USHRT_MAX_; // we have an angle so it's periodical
    orient /= orientStep; // get the final direction index

    const float2 objIconSize = { float(iconSize.x) / texSize.x,
                                 float(iconSize.y) / texSize.y };
    ...
    va->AddVertexQT(
      pos + upv + rgv, objTexCoors.x, objTexCoors.y + objIconSize.y);
    va->AddVertexQT(pos + upv - rgv,
                    objTexCoors.x + objIconSize.x,
                    objTexCoors.y + objIconSize.y);
    va->AddVertexQT(
      pos - upv - rgv, objTexCoors.x + objIconSize.x, objTexCoors.y);
}
...
CFarTextureHandler::Draw()
{
    ...
    {
        const float3 camNorm =
          ((camera->GetDir() * XZVector) - (UpVector * 0.1f)).ANormalize();
...
CFarTextureHandler::CheckResizeAtlas()
{
    const unsigned int maxSprites =
      ((texSize.x / iconSize.x) * (texSize.y / iconSize.y) /
       NUM_ICON_ORIENTATIONS) -
      1;
    ...
    if (globalRendering->supportNPOTs) {
        ...
        texSize.y += std::max(iconSize.y,
                              4 * NUM_ICON_ORIENTATIONS * iconSize.x *
                                iconSize.y / texSize.x);
    } else {
        texSize.y <<= 1;
    }
    ...
    glGetTexImage(GL_TEXTURE_2D,
                  ...
                  GL_RGBA,
                  GL_UNSIGNED_BYTE,
                  &oldPixels[0]); // TODO use the FBO?
    memset(&oldPixels[0] + texSize.x * oldTexSizeY * 4,
           0,
           texSize.x * (texSize.y - oldTexSizeY) * 4);
