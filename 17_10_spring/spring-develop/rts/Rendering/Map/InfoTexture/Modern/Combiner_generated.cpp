...
CInfoTextureCombiner::CInfoTextureCombiner()
  ...
  , disabled(true)
{
    texSize = (configHandler->GetBool("HighResInfoTexture"))
                ? int2(mapDims.pwr2mapx, mapDims.pwr2mapy)
                : int2(mapDims.pwr2mapx >> 1, mapDims.pwr2mapy >> 1);
    // texSize = (configHandler->GetBool("HighResInfoTexture")) ? int2(512, 512)
    // : int2(256, 256);
    texChannels = 4;
    ...
    shader = shaderHandler->CreateProgramObject(
      "[CInfoTextureCombiner]", "CInfoTextureCombiner", false);

    if (!fbo.IsValid() /*|| !shader->IsValid()*/) { // don't check shader (it
                                                    // gets created/switched at
                                                    // runtime)
        throw opengl_error("");
    }
}
...
CInfoTextureCombiner::SwitchMode(const std::string& name)
{
    ...
    if (name == "los") {
        disabled = !CreateShader(
          "shaders/GLSL/infoLOS.lua", true, float4(0.5f, 0.5f, 0.5f, 1.0f));
    } else if (name == "metal") {
        disabled = !CreateShader(
          "shaders/GLSL/infoMetal.lua", true, float4(0.f, 0.f, 0.f, 1.0f));
    } else if (name == "height") {
