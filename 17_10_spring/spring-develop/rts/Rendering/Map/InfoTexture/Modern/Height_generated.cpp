  ...
  , needUpdate(true)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D,
                    ...
                    GL_RGBA,
                    GL_UNSIGNED_BYTE,
                    &CHeightLinePalette::paletteColored[0].r);
    glTexSubImage2D(GL_TEXTURE_2D,
                    0,
                    0,
                    1,
                    ...
                    GL_RGBA,
                    GL_UNSIGNED_BYTE,
                    &CHeightLinePalette::paletteBlackAndWhite[0].r);
    ...
    shader = shaderHandler->CreateProgramObject(
      "[CHeightTexture]", "CHeightTexture", false);
    ...
    if (!shader->IsValid()) {
        ...
        shader->SetUniform("paletteOffset",
                           configHandler->GetBool("ColorElev") ? 0.0f : 1.0f);
        shader->Disable();
        shader->Validate();
        if (!shader->IsValid()) {
...
CHeightTexture::Update()
{
    ...
    if (!fbo.IsValid() || !shader->IsValid() ||
        (heightMapTexture->GetTextureID() == 0))
        return UpdateCPU();
