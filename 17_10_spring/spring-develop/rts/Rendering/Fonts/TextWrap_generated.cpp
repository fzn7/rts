 ...
 */
static inline bool
IsUpperCase(const char32_t& c)
{
    // overkill to add unicode
    return (c >= 0x41 && c <= 0x5A) || (c >= 0xC0 && c <= 0xD6) ||
           (c >= 0xD8 && c <= 0xDE) || (c == 0x8A) || (c == 0x8C) ||
           (c == 0x8E) || (c == 0x9F);
}
...
{
    // returns two pieces 'L'eft and 'R'ight of the split word (returns L, *wi
    // becomes R)
    ...
    if (w.isLineBreak) {
        ...
        do {
            ...
            if (smart) {
                const float penalty =
                  GetPenalty(co, lastCharPos, w.text.length());
                       ...
                       float maxWidth)
{
    ...
    if (wi_end->isLineBreak) {
        if (l->start == l->end || l->end == words.begin()) {
            // there is just the linebreak in that line, so replace linebreak
            // with just a null space
            word w;
            w.pos = wi_end->pos;
            w.isSpace = true;
            w.numSpaces = 0;
    ...
    while (l->end != l->start) {
        ...
        // we can cut the last word to get enough freespace (so show as many as
        // possible characters of that word)
    ...
    // but if we put an ellipsis in there, it is better to show as many as
    // possible characters of those words
    std::list<word>::iterator nextwi(l->end);
    ++nextwi;
    if ((!l->forceLineBreak) && (nextwi != words.end()) &&
                           ...
                           float maxHeight)
{
    ...
    const unsigned int maxLines =
      (unsigned int)std::floor(std::max(0.0f, maxHeight / GetLineHeight()));
    ...
    bool currLineValid =
      false; // true if there was added any data to the current line
    ...
    for (;;) {
        ...
        if (wi->isLineBreak) {
            ...
            if (currLine->width > maxWidth) {
                ...
                // line grew too long by adding the last word, insert a
                // LineBreak
                ...
                if (splitAllWords || splitLastWord) {
                    // last word W is larger than 0.5 * maxLineWidth, split it
                    // into get 'L'eft and 'R'ight parts of the split (wL
                    // becomes Left, *wi becomes R)
                    ...
                    if (splitLastWord && wL.width == 0.0f) {
                        // With smart splitting it can happen that the word
                        // isn't split at all, this can cause a race condition
                        // when the word is longer than maxWidth. In this case
                        // we have to force an unaesthetic split.
    ...
    // empty row
    if (!currLineValid ||
        (currLine->start == words.end() && !currLine->forceLineBreak)) {
        lines.pop_back();
        currLine = &(lines.back());
    }
                             ...
                             std::list<colorcode>& colorcodes) const
{
    ...
    for (auto& c : colorcodes) {
        ...
        if (wi2->isSpace || wi2->isLineBreak) {
            ...
            if (wi2 == words->end() ||
                (wi == words->end() && (wi2->pos + wi2->numSpaces) < c.pos))
                return;

            wi2 = words->insert(wi2, wc);
        } else {
            if (wi == words->end() &&
                (wi2->pos + wi2->text.size()) < (c.pos + 1)) {
                return;
            }
...
CTextWrap::WrapInPlace(std::u8string& text,
                       ...
                       float maxHeight)
{
    // TODO make an option to insert '-' for word wrappings (and perhaps try to
    // syllabificate)
