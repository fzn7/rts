  ...
  : CEventClient("[CProjectileDrawer]", 123456, false)
{
    ...
    textureAtlas->SetName("ProjectileTextureAtlas");
    groundFXAtlas = new CTextureAtlas();
    groundFXAtlas->SetName("ProjectileEffectsAtlas");

    LuaParser resourcesParser(
      "gamedata/resources.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    LuaParser mapResParser(
      "gamedata/resources_map.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    ...
    const LuaTable& resProjTexturesTable =
      resGraphicsTable.SubTable("projectileTextures");
    const LuaTable& resSmokeTexturesTable = resGraphicsTable.SubTable("smoke");
    const LuaTable& resGroundFXTexturesTable =
      resGraphicsTable.SubTable("groundfx");
    ...
    ParseAtlasTextures(
      true, resProjTexturesTable, blockedTexNames, textureAtlas);
    ParseAtlasTextures(
      true, resGroundFXTexturesTable, blockedTexNames, groundFXAtlas);
        ...
        if (resSmokeTexturesTable.IsValid()) {
            for (smokeTexCount = 0; true; smokeTexCount++) {
                const std::string& tex =
                  resSmokeTexturesTable.GetString(smokeTexCount + 1, "");
                if (tex.empty()) {
                    break;
                }
                const std::string texName = "bitmaps/" + tex;
                const std::string smokeName =
                  "ismoke" + IntToString(smokeTexCount, "%02i");
            ...
            for (smokeTexCount = 0; smokeTexCount < 12; smokeTexCount++) {
                ...
                const std::string texName =
                  "bitmaps/smoke/smoke" + smokeNum + ".tga";
...

    {
        ...
        textureAtlas->AddTexFromMem("perlintex",
                                    perlinTexSize,
                                    perlinTexSize,
                                    CTextureAtlas::RGBA32,
                                    &perlinTexMem[0]);
    ...
    if (mapResParser.Execute()) {
        // allow map-specified atlas textures (for gaia-projectiles and
        // ground-flashes)
        const LuaTable& mapResTable = mapResParser.GetRoot();
        const LuaTable& mapResGraphicsTable = mapResTable.SubTable("graphics");
        const LuaTable& mapResProjTexturesTable =
          mapResGraphicsTable.SubTable("projectileTextures");
        const LuaTable& mapResGroundFXTexturesTable =
          mapResGraphicsTable.SubTable("groundfx");

        ParseAtlasTextures(
          false, mapResProjTexturesTable, blockedTexNames, textureAtlas);
        ParseAtlasTextures(
          false, mapResGroundFXTexturesTable, blockedTexNames, groundFXAtlas);
    }

    if (!textureAtlas->Finalize()) {
        LOG_L(L_ERROR,
              "Could not finalize projectile-texture atlas. Use less/smaller "
              "textures.");
    }
...
#undef GETTEX
    ...
    if (!groundFXAtlas->Finalize()) {
        LOG_L(L_ERROR,
              "Could not finalize groundFX texture atlas. Use less/smaller "
              "textures.");
    }
    ...
    if (perlinFB.IsValid()) {
        // we never refresh the full texture (just the perlin part). So we need
        // to reload it then.
                                      ...
                                      CTextureAtlas* textureAtlas)
{
    ...
    for (texturesMapIt = texturesMap.begin();
         texturesMapIt != texturesMap.end();
         ++texturesMapIt) {
        ...
        if (blockTextures ||
            (blockedTextures.find(textureName) == blockedTextures.end())) {
            textureAtlas->AddTexFromFile(texturesMapIt->first,
                                         "bitmaps/" + texturesMapIt->second);
        }
    }
        ...
        if (textureSubTable.IsValid()) {
            ...
            for (texturesMapIt = texturesMap.begin();
                ...
                const std::string textureName =
                  StringToLower(texturesMapIt->first);

                if (blockTextures) {
                    blockedTextures.insert(textureName);
                }
                if (blockTextures || (blockedTextures.find(textureName) ==
                                      blockedTextures.end())) {
                    textureAtlas->AddTexFromFile(
                      texturesMapIt->first, "bitmaps/" + texturesMapIt->second);
                }
            }
    ...
    for (WeaponDef& wd : weaponDefHandler->weaponDefs) {
        ...
        if (wd.type == "Cannon") {
            ...
            if (wd.visuals.colorMap == nullptr) {
                wd.visuals.colorMap =
                  CColorMap::LoadFromDefString("1.0 1.0 1.0 0.1 "
        ...
        if (wd.visuals.texNames[0] != "") {
            wd.visuals.texture1 =
              &textureAtlas->GetTexture(wd.visuals.texNames[0]);
        }
        if (wd.visuals.texNames[1] != "") {
            wd.visuals.texture2 =
              &textureAtlas->GetTexture(wd.visuals.texNames[1]);
        }
        if (wd.visuals.texNames[2] != "") {
            wd.visuals.texture3 =
              &textureAtlas->GetTexture(wd.visuals.texNames[2]);
        }
        if (wd.visuals.texNames[3] != "") {
            wd.visuals.texture4 =
              &textureAtlas->GetTexture(wd.visuals.texNames[3]);
        }

        if (!wd.visuals.ptrailExpGenTag.empty()) {
            // these can only be custom EG's so prefix is not required game-side
            wd.ptrailExplosionGeneratorID = explGenHandler->LoadGeneratorID(
              CEG_PREFIX_STRING + wd.visuals.ptrailExpGenTag);
        }

        if (!wd.visuals.impactExpGenTag.empty()) {
            wd.impactExplosionGeneratorID =
              explGenHandler->LoadGeneratorID(wd.visuals.impactExpGenTag);
        }

        if (!wd.visuals.bounceExpGenTag.empty()) {
            wd.bounceExplosionGeneratorID =
              explGenHandler->LoadGeneratorID(wd.visuals.bounceExpGenTag);
        }
    }
}
                                   ...
                                   bool drawRefraction)
{
    ...
    for (auto binIt = projectileBin.cbegin(); binIt != projectileBin.cend();
         ++binIt) {
                                     ...
                                     const CSolidObject* owner)
{
    ...
    return (gu->spectatingFullView ||
            (owner != nullptr && th->Ally(owner->allyteam, gu->myAllyTeam)) ||
                                     ...
                                     bool drawRefraction)
{
    ...
    if (drawRefraction &&
        (pro->drawPos.y > pro->GetDrawRadius()) /*!pro->IsInWater()*/)
        return;
    if (drawReflection &&
        !CUnitDrawer::ObjectVisibleReflection(
          pro->drawPos, camera->GetPos(), pro->GetDrawRadius()))
        return;
...
CProjectileDrawer::DrawProjectilesMiniMap()
{
    ...
    for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
         modelType++) {
        const auto& projectileBin =
          modelRenderers[modelType]->GetProjectileBin();
...
CProjectileDrawer::DrawFlyingPieces(int modelType)
{
    ...
    const FlyingPieceContainer& container =
      projectileHandler->flyingPieces[modelType];
    ...
    for (const FlyingPiece& fp : container) {
        const bool noLosTst =
          ...
          teamHandler->AlliedTeams(gu->myTeam, fp.GetTeam());
        const bool inAirLos =
          noLosTst || losHandler->InAirLos(fp.GetPos(), gu->myAllyTeam);
...
CProjectileDrawer::Draw(bool drawReflection, bool drawRefraction)
{
...

    {
        ...
        for (CProjectile* p : unsortedProjectiles) {
            p->Draw(); // FIXME rename to explosionSpawners ? and why to call
                       // Draw() for them??
        }
    }
...
{
    Shader::IProgramObject* po = shadowHandler->GetShadowGenProg(
      CShadowHandler::SHADOWGEN_PROGRAM_PROJECTILE);
...
CProjectileDrawer::DrawProjectileModel(const CProjectile* p)
{
    ...
    if (p->weapon) {
        ...
        glPushMatrix();
        glMultMatrixf(wp->GetTransformMatrix(wp->GetProjectileType() ==
        ...
        glRotatef(
          pp->GetDrawAngle(), pp->spinVec.x, pp->spinVec.y, pp->spinVec.z);
...
CProjectileDrawer::DrawGroundFlashes()
{
    ...
    for (CGroundFlash* gf : gfc) {
        const bool inLos = gf->alwaysVisible || gu->spectatingFullView ||
                           losHandler->InAirLos(gf, gu->myAllyTeam);
        if (!inLos)
            continue;
...
{
    perlinFB.Bind();
    glViewport(perlintex->xstart * (textureAtlas->GetSize()).x,
               perlintex->ystart * (textureAtlas->GetSize()).y,
               perlinTexSize,
               perlinTexSize);
...
CProjectileDrawer::GenerateNoiseTex(unsigned int tex)
{
    ...
    for (int a = 0; a < perlinBlendTexSize * perlinBlendTexSize; ++a) {
        const unsigned char rnd =
          int(std::max(0.0f, gu->RandFloat() * 555.0f - 300.0f));
...
CProjectileDrawer::RenderProjectileDestroyed(const CProjectile* const p)
{
    if (p->model) {
        ...
        auto it = std::find(renderProjectiles.begin(),
                            renderProjectiles.end(),
                            const_cast<CProjectile*>(p));
