...
CDirtProjectile::Draw()
{
    ...
    col[3] =
      (unsigned char)(alpha) /*- (globalRendering->timeOffset * alphaFalloff)*/;

    const float interSize = size + globalRendering->timeOffset * sizeExpansion;
    const float texx = texture->xstart + (texture->xend - texture->xstart) *
                                           ((1.0f - partAbove) * 0.5f);

    va->AddVertexTC(drawPos - camera->GetRight() * interSize -
                      camera->GetUp() * interSize * partAbove,
                    texx,
                    texture->ystart,
                    col);
    va->AddVertexTC(drawPos + camera->GetRight() * interSize -
...
CDirtProjectile::GetMemberInfo(SExpGenSpawnableMemberInfo& memberInfo)
{
    ...
    CHECK_MEMBER_INFO_PTR(
      CDirtProjectile, texture, projectileDrawer->textureAtlas->GetTexturePtr)
