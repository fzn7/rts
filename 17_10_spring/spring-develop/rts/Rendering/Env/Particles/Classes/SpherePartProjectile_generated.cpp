  ...
  , ttl(ttl)
{
    ...
    for (int y = 0; y < 5; ++y) {
        ...
        for (int x = 0; x < 5; ++x) {
            ...
            vectors[y * 5 + x] = float3(std::sin(xp) * std::cos(yp),
                                        std::sin(yp),
                                        std::cos(xp) * std::cos(yp));
        }
    }
    ...
    texx = projectileDrawer->sphereparttex->xstart +
           (projectileDrawer->sphereparttex->xend -
            projectileDrawer->sphereparttex->xstart) *
             0.5f;
    texy = projectileDrawer->sphereparttex->ystart +
           (projectileDrawer->sphereparttex->yend -
            projectileDrawer->sphereparttex->ystart) *
             0.5f;
}
...
CSpherePartProjectile::Draw()
{
    ...
    const float interSize =
      sphereSize + expansionSpeed * globalRendering->timeOffset;
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            float alpha =
              ...
              (1.0f -
               std::min(
                 1.0f, float(age + globalRendering->timeOffset) / (float)ttl)) *
            ...
            va->AddVertexQTC(
              centerPos + vectors[y * 5 + x] * interSize, texx, texy, col);
            va->AddVertexQTC(
              centerPos + vectors[y * 5 + x + 1] * interSize, texx, texy, col);
            alpha =
              baseAlpha *
              (1.0f - std::min(1.0f,
                               (float)(age + globalRendering->timeOffset) /
                                 (float)ttl)) *
            ...
            col[2] = (unsigned char)(color.z * 255.0f * alpha);
            col[3] = ((unsigned char)(40 * alpha)) + 1;
            va->AddVertexQTC(centerPos +
                               vectors[(y + 1) * 5 + x + 1] * interSize,
                             texx,
                             texy,
                             col);
            va->AddVertexQTC(centerPos + vectors[(y + 1) * 5 + x] * interSize,
                             texx,
                             texy,
                             col);
        }
                                    ...
                                    float3 color)
{
    for (int y = 0; y < 16; y += 4) {
        for (int x = 0; x < 32; x += 4) {
            new CSpherePartProjectile(
              owner, pos, x, y, expansionSpeed, alpha, ttl, color);
        }
    }
}
...
void
CSpherePartSpawner::Init(const CUnit* owner, const float3& offset)
{
    CProjectile::Init(owner, offset);
    deleteMe = true;
    CSpherePartProjectile::CreateSphere(
      owner, ttl, alpha, expansionSpeed, pos, color);
}
