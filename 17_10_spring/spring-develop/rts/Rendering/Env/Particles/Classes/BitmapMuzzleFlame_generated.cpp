...
CBitmapMuzzleFlame::Draw()
{
    ...
    const float3 udir =
      (std::fabs(dir.dot(UpVector)) >= 0.99f) ? FwdVector : UpVector;
    const float3 xdir = (dir.cross(udir)).SafeANormalize();
    const float3 ydir = (dir.cross(xdir)).SafeANormalize();

    va->AddVertexTC(
      pos + ydir * isize, sideTexture->xstart, sideTexture->ystart, col);
    va->AddVertexTC(pos + ydir * isize + dir * ilength,
                    sideTexture->xend,
                    sideTexture->ystart,
                    col);
    va->AddVertexTC(pos - ydir * isize + dir * ilength,
                    ...
                    col);
    va->AddVertexTC(
      pos - ydir * isize, sideTexture->xstart, sideTexture->yend, col);

    va->AddVertexTC(
      pos + xdir * isize, sideTexture->xstart, sideTexture->ystart, col);
    va->AddVertexTC(pos + xdir * isize + dir * ilength,
                    sideTexture->xend,
                    sideTexture->ystart,
                    col);
    va->AddVertexTC(pos - xdir * isize + dir * ilength,
                    ...
                    col);
    va->AddVertexTC(
      pos - xdir * isize, sideTexture->xstart, sideTexture->yend, col);

    float3 frontpos = pos + dir * frontOffset * ilength;

    va->AddVertexTC(frontpos - xdir * isize + ydir * isize,
                    frontTexture->xstart,
                    frontTexture->ystart,
                    col);
    va->AddVertexTC(frontpos + xdir * isize + ydir * isize,
...
CBitmapMuzzleFlame::GetMemberInfo(SExpGenSpawnableMemberInfo& memberInfo)
{
    ...
    CHECK_MEMBER_INFO_PTR(CBitmapMuzzleFlame,
                          sideTexture,
    ...
    CHECK_MEMBER_INFO_PTR(
      CBitmapMuzzleFlame, colorMap, CColorMap::LoadFromDefString)
