...
CRepulseGfx::Draw()
{
    ...
    pos = (repulsed->pos - zdir * 10.0f) +
          (repulsed->speed * globalRendering->timeOffset);
    inArray = true;
    ...
    for (int y = 0; y < loopCountY;
         ++y) { //! CAUTION: loop count must match EnlargeArrays above
        ...
        for (int x = 0; x < loopCountX; ++x) {
            ...
            va->AddVertexQTC(pos + xdirDS * (dx + 0) + ydirDS * (dy + 0) +
                               zdir * difs[(y)*5 + x],
                             txo + (ry)*txs,
                             tyo + (rx)*tys,
                             col);
            va->AddVertexQTC(pos + xdirDS * (dx + 0) + ydirDS * (dy + 1) +
                               zdir * difs[(y + 1) * 5 + x],
                             txo + (ry + 0.25f) * txs,
                             tyo + (rx)*tys,
                             col);
            va->AddVertexQTC(pos + xdirDS * (dx + 1) + ydirDS * (dy + 1) +
                               zdir * difs[(y + 1) * 5 + x + 1],
                             txo + (ry + 0.25f) * txs,
                             tyo + (rx + 0.25f) * tys,
                             col);
            va->AddVertexQTC(pos + xdirDS * (dx + 1) + ydirDS * (dy + 0) +
                               zdir * difs[(y)*5 + x + 1],
                             txo + (ry)*txs,
                             tyo + (rx + 0.25f) * tys,
                             col);
...
CRepulseGfx::Update()
{
    ...
    if (repulsed && owner() &&
        (repulsed->pos - owner()->pos).SqLength() > sqMaxDist) {
        deleteMe = true;
    }
}
