    ...
    if (directional) {
        for (int i = 0; i < numParticles; i++) {
            ...
            if (p->life < 1.0f) {
                ...
                if (p->speed.SqLength() > 0.001f) {
                    va->AddVertexQTC(interPos - ydir * size - xdir * size,
                                     texture->xstart,
                                     texture->ystart,
                                     color);
                    va->AddVertexQTC(interPos - ydir * size + xdir * size,
                    ...
                    va->AddVertexQTC(interPos - camera->GetUp() * size -
                                       camera->GetRight() * size,
                                     texture->xstart,
                                     texture->ystart,
                                     color);
                    va->AddVertexQTC(interPos - camera->GetUp() * size +
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            if (p->life < 1.0f) {
                ...
                va->AddVertexQTC(interPos - cameraRight - cameraUp,
                                 texture->xstart,
                                 texture->ystart,
                                 color);
                va->AddVertexQTC(interPos + cameraRight - cameraUp,
...
CSimpleParticleSystem::Init(const CUnit* owner, const float3& offset)
{
    ...
    if (colorMap == NULL) {
        ...
        LOG_L(L_WARNING,
              "[CSimpleParticleSystem::%s] no color-map specified",
              __FUNCTION__);
    }
    if (texture == NULL) {
        texture = &projectileDrawer->textureAtlas->GetTexture("simpleparticle");
        LOG_L(L_WARNING,
              "[CSimpleParticleSystem::%s] no texture specified",
              __FUNCTION__);
    }
    ...
    for (auto& p : particles) {
        ...
        p.speed = ((up * emitMul.y) * fastmath::cos(ay) -
                   ((right * emitMul.x) * fastmath::cos(az) -
                    (forward * emitMul.z) * fastmath::sin(az)) *
                     fastmath::sin(ay)) *
                  (particleSpeed + (gu->RandFloat() * particleSpeedSpread));
        p.life = 0;
        p.decayrate =
          1.0f / (particleLife + (gu->RandFloat() * particleLifeSpread));
        p.size = particleSize + gu->RandFloat() * particleSizeSpread;
    }

    drawRadius = (particleSpeed + particleSpeedSpread) *
                 (particleLife * particleLifeSpread);
}
...
CSimpleParticleSystem::GetMemberInfo(SExpGenSpawnableMemberInfo& memberInfo)
{
    ...
    CHECK_MEMBER_INFO_PTR(CSimpleParticleSystem,
                          texture,
                          projectileDrawer->textureAtlas->GetTexturePtr)
    CHECK_MEMBER_INFO_PTR(
      CSimpleParticleSystem, colorMap, CColorMap::LoadFromDefString)
...
CSphereParticleSpawner::Init(const CUnit* owner, const float3& offset)
{
    ...
    if (colorMap == NULL) {
        colorMap = CColorMap::LoadFromFloatVector(std::vector<float>(8, 1.0f));
        LOG_L(L_WARNING,
              "[CSphereParticleSpawner::%s] no color-map specified",
              __FUNCTION__);
    }
    if (texture == NULL) {
        texture = &projectileDrawer->textureAtlas->GetTexture("sphereparticle");
        LOG_L(L_WARNING,
              "[CSphereParticleSpawner::%s] no texture specified",
              __FUNCTION__);
    }
    ...
    for (int i = 0; i < numParticles; i++) {
        ...
        const float ay =
          (emitRot + emitRotSpread * gu->RandFloat()) * (PI / 180.0);

        const float3 pspeed =
          ((up * emitMul.y) * std::cos(ay) -
           ((right * emitMul.x) * std::cos(az) -
            (forward * emitMul.z) * std::sin(az)) *
             std::sin(ay)) *
          (particleSpeed + (gu->RandFloat() * particleSpeedSpread));

        CGenericParticleProjectile* particle =
          new CGenericParticleProjectile(owner, pos + offset, pspeed);
        ...
        particle->SetRadiusAndHeight(particle->size + sizeGrowth * particleLife,
                                     0.0f);
    }
