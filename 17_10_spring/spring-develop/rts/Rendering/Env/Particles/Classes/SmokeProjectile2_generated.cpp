...
{
    if (offset.y - CGround::GetApproximateHeight(offset.x, offset.z, false) >
        10)
        useAirLos = true;
...
{
    inArray = true;
    const float interAge =
      std::min(1.0f, age + ageSpeed * globalRendering->timeOffset);
    unsigned char col[4];
    unsigned char alpha;
    if (interAge < 0.05f) {
    ...
    const float gglow =
      std::max(0.f, (1 - (interAge * glowFalloff * 2.5f)) * 127);
    ...
    const float3 interPos =
      pos + (wantedPos + speed * globalRendering->timeOffset - pos) * 0.1f *
