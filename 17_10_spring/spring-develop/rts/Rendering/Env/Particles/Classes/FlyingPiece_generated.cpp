  ...
  , piece(_piece)
{
    ...
    const S3DModelPiecePart& shatterPiecePart =
      piece->shatterParts[gu->RandInt() % piece->shatterParts.size()];
    ...
    for (const auto& cp : shatterPieceData) {
        ...
        splitterParts.back().speed =
          speed + flyDir * mix<float>(1.f, EXPLOSION_SPEED, gu->RandFloat());
        splitterParts.back().rotationAxisAndSpeed =
          float4(gu->RandVector().ANormalize(), gu->RandFloat() * 0.1f);
...
FlyingPiece::Update()
{
    ...
    pos = pos0 + (speed * dragFactors.x) +
          UpVector * (mapInfo->map.gravity * dragFactors.y);
    ...
    for (auto& cp : splitterParts) {
        ...
        if ((p.y + pieceRadius * 2.f) >=
            CGround::GetApproximateHeight(p.x, p.z, false)) {
            return true;
        }
    }
...
FlyingPiece::GetDragFactors() const
{
    ...
    // need to be updated each frame (-> cpu usage). Doing so for each polygon
    // is massively slow. So I wanted to replace it with a stateless system,
    // computing the current position just from the params t & individual speed
    // (start pos is 0!).
    //
    ...
    //
    // 2.
    // The positional offset caused by gravity is similar but a bit more
    // complicated:
    //  xg(t=1) := g
    ...
    //           = g * gravityDrag
    // The first sum is again a geometric series as above, the 2nd one is very
    // similar. The solution can be found here:
    // https://de.wikipedia.org/w/index.php?title=Geometrische_Reihe&oldid=149159222#Verwandte_Summenformel_1
    //
    // The advantage of those 2 drag factors are that they only depend on the
    // time (which even can be interpolated), so we only have to compute them
    // once per frame. And can then get the position of each particle with a
    // simple multiplication, saving memory & cpu time.
    ...
    const float airDragPowTwo =
      airDragPowOne * airDrag; // := std::pow(airDrag, interAge + 2.f);
    ...
    dragFactors.y -=
      (interAge * (airDragPowTwo - airDragPowOne) - airDragPowOne + airDrag) *
      invAirDrag * invAirDrag; // 2nd sum
...
{
    const float3 interPos = cp.speed * dragFactors.x +
                            UpVector * mapInfo->map.gravity * dragFactors.y;
    const float4& rot = cp.rotationAxisAndSpeed;

    CMatrix44f m = pieceMatrix;
    m.GetPos() += interPos; // note: not the same as .Translate(pos) which does
                            // `m = m * translate(pos)`, but we want `m =
                            // translate(pos) * m`
...
FlyingPiece::Draw(const FlyingPiece* prev) const
{
    ...
    const float3 dragFactors =
      GetDragFactors(); // speedDrag, gravityDrag, interAge
    ...
    for (auto& cp : splitterParts) {
        ...
        glDrawRangeElements(GL_TRIANGLES,
                            0,
                            piece->GetVertexCount() - 1,
                            cp.indexCount,
                            GL_UNSIGNED_INT,
                            piece->vboShatterIndices.GetPtr(cp.vboOffset));
        glPopMatrix();
    }
}
