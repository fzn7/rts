...
void
CGeoThermSmokeProjectile::Update()
{
    UpdateDir();
    ...
    CWorldObject::SetVelocity(speed +
                              XZVector * (wind.GetCurrentWind() / GAME_SPEED));
...
{
    for (ProjectileContainer::iterator it =
           projectileHandler->unsyncedProjectiles.begin();
         it != projectileHandler->unsyncedProjectiles.end();
         ++it) {
