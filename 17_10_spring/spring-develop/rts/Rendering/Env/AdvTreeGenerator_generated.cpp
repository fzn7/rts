...
CAdvTreeGenerator::CAdvTreeGenerator()
{
    ...
    const LuaTable treesTable =
      resourcesParser.GetRoot().SubTable("graphics").SubTable("trees");
    ...
    FBO fbo;
    fbo.Bind();
    fbo.CreateRenderBuffer(
      GL_DEPTH_ATTACHMENT_EXT, GL_DEPTH_COMPONENT16, 256, 256);
    ...
    for (int y = 0; y < 256; y++) {
        for (int x = 0; x < 256; x++) {
            ...
            tree[y][x][3] = 255;
            tree[y][x + 1024][0] =
              (unsigned char)(bm.mem[(y * 256 + x) * 4] * 0.6f);
            tree[y][x + 1024][1] =
              (unsigned char)(bm.mem[(y * 256 + x) * 4 + 1] * 0.6f);
            tree[y][x + 1024][2] =
              (unsigned char)(bm.mem[(y * 256 + x) * 4 + 2] * 0.6f);
            tree[y][x + 1024][3] = 255;
        }
    }
    ...
    glBuildMipmaps(
      GL_TEXTURE_2D, GL_RGBA8, 2048, 256, GL_RGBA, GL_UNSIGNED_BYTE, data);
...
void
CAdvTreeGenerator::DrawTrunk(const float3& start,
                             const float3& end,
                             const float3& orto1,
                             const float3& orto2,
                             float size)
{
    ...
    for (int a = 0; a <= numIter; a++) {
        ...
        float col =
          0.4f +
          (((orto1 * std::sin(angle) + orto2 * std::cos(angle)).dot(flatSun))) *
            0.3f;
        barkva->AddVertexTN(start + orto1 * std::sin(angle) * size +
                              orto2 * std::cos(angle) * size,
                            angle / PI * 0.125f * 0.5f,
                            0,
                            float3(0, 0, col));
        barkva->AddVertexTN(end + orto1 * std::sin(angle) * size * 0.2f +
                              orto2 * std::cos(angle) * size * 0.2f,
                            angle / PI * 0.125f * 0.5f,
                            3,
                            float3(0, 0, col));
    }
...
CAdvTreeGenerator::MainTrunk(int numBranch, float height, float width)
{
    ...
    for (int a = 0; a < numBranch; ++a) {
        ...
        const float length = (height * (0.4f + 0.1f * gu->RandFloat())) *
                             std::sqrt(float(numBranch - a) / numBranch);
                                 ...
                                 int depth)
{
    ...
    for (int a = 0; a < numTrunks; a++) {
        ...
        const float newLength =
          length * (float(numTrunks - a) / (numTrunks + 1));

        float3 newbase =
          start + dir * length * (float(a + 1) / (numTrunks + 1));
        float3 newDir = dir + orto1 * std::cos(angle) * dirDif +
                        orto2 * std::sin(angle) * dirDif;
        newDir.ANormalize();
                                ...
                                float3& orto2)
{
    ...
    auto ADD_LEAF = [&](const float3& pos) {
        ...
        const float col =
          0.5f + (npos.dot(flatSun) * 0.3f) + 0.1f * gu->RandFloat();

        va->AddVertexTN(
          pos,
          0.126f + tex + flipTex,
          0.98f,
          float3(0.09f * MAX_TREE_HEIGHT, -0.09f * MAX_TREE_HEIGHT, col));
        va->AddVertexTN(
          pos,
          0.249f + tex - flipTex,
          0.98f,
    ...
    for (int a = 0; a < numLeaves + 1; a++) {
        ...
        pos += (orto1 * std::sin(angle) + orto2 * std::cos(angle)) *
               mix(gu->RandFloat(), std::sqrt((float)a + 1), 0.6f) * 0.1f *
               MAX_TREE_HEIGHT;
...
CAdvTreeGenerator::CreateFarTex(Shader::IProgramObject* treeShader)
{
    ...
    fbo.Bind();
    fbo.CreateRenderBuffer(
      GL_DEPTH_ATTACHMENT_EXT, GL_DEPTH_COMPONENT16, 64, 64);
...
#define L (*sunLighting)
        if (globalRendering->haveGLSL) {
            ...
            treeShader->SetUniform3f(
              10, 0.0f, 0.0f, 0.0f); // tree position-offset
            treeShader->SetUniform4f(11,
                                     L.groundDiffuseColor.x,
                                     L.groundDiffuseColor.y,
                                     L.groundDiffuseColor.z,
                                     0.85f);
            treeShader->SetUniform4f(14,
                                     L.groundAmbientColor.x,
                                     L.groundAmbientColor.y,
                                     L.groundAmbientColor.z,
                                     0.85f);
            treeShader->SetUniform4f(
              12, 0.0f, 0.0f, 0.0f, 0.02f); // w = alpha / height modifier
        }
#undef L
        ...
        for (int a = 0; a < 8; ++a) {
            treeShader->SetUniform3f(
              ((globalRendering->haveGLSL) ? 0 : 13), 1.0f, 0.0f, 0.0f);
            treeShader->SetUniform3f(
              ((globalRendering->haveGLSL) ? 1 : 9), 0.0f, 1.0f, 0.0f);
            ...
            glOrtho(-MAX_TREE_HEIGHT * 0.5f,
                    MAX_TREE_HEIGHT * 0.5f,
                    0,
                    MAX_TREE_HEIGHT,
                    -MAX_TREE_HEIGHT * 0.5f,
                    MAX_TREE_HEIGHT * 0.5f);
            ...
            treeShader->SetUniform3f(
              ((globalRendering->haveGLSL) ? 0 : 13), 0.0f, 0.0f, 1.0f);
            ...
            treeShader->SetUniform3f(
              ((globalRendering->haveGLSL) ? 0 : 13), 0.0f, 0.0f, 1.0f);
            treeShader->SetUniform3f(
              ((globalRendering->haveGLSL) ? 1 : 9), 1.0f, 0.0f, 0.0f);
    ...
    memcpy(&data[512 * (192) * 4],
           &data[512 * (64) * 4],
           512 * 64 * 4); // far-away trees
    memcpy(&data[512 * (192 + 256) * 4],
           &data[512 * (64 + 256) * 4],
           512 * 64 * 4); // far-away pine trees

    memcpy(&data2[512 * (64) * 4], &data[512 * (64) * 4], 512 * 192 * 4);
    memcpy(
      &data2[512 * (64 + 256) * 4], &data[512 * (64 + 256) * 4], 512 * 192 * 4);
    ...
    glTexParameteri(
      GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
    glBuildMipmaps(
      GL_TEXTURE_2D, GL_RGBA8, 512, 512, GL_RGBA, GL_UNSIGNED_BYTE, &data[0]);
    ...
    glTexParameteri(
      GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
    glBuildMipmaps(
      GL_TEXTURE_2D, GL_RGBA8, 512, 512, GL_RGBA, GL_UNSIGNED_BYTE, &data2[0]);
                                 ...
                                 unsigned int displist)
{
    ...
    for (int y = 0; y < 64; ++y) {
        for (int x = 0; x < 64; ++x) {
            if (buf[(y * 64 + x) * 4] == 0 && buf[(y * 64 + x) * 4 + 1] == 0 &&
                buf[(y * 64 + x) * 4 + 2] == 0) {
                ...
                mem[((y + dy) * 512 + x + dx) * 4 + 1] =
                  buf[(y * 64 + x) * 4 + 1];
                mem[((y + dy) * 512 + x + dx) * 4 + 2] =
                  buf[(y * 64 + x) * 4 + 2];
    ...
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            if (buf[(y * 256 + x) * 4] == 0 &&
                ...
                data[((y + ypos) * xsize + x + xpos) * 4 + 0] =
                  buf[(y * 256 + x) * 4];
                data[((y + ypos) * xsize + x + xpos) * 4 + 1] =
                  buf[(y * 256 + x) * 4 + 1];
                data[((y + ypos) * xsize + x + xpos) * 4 + 2] =
                  buf[(y * 256 + x) * 4 + 2];
...
CAdvTreeGenerator::CreateGranTexBranch(const float3& start, const float3& end)
{
    ...
    while (tipDist < length * (0.15f * gu->RandFloat() + 0.83f)) {
        ...
        float3 bend =
          bstart +
          bdir * tipDist * ((6 - tipDist) * (0.10f + 0.05f * gu->RandFloat()));
...
CAdvTreeGenerator::PineTree(int numBranch, float height)
{
    ...
    for (int a = 0; a < numBranch; ++a) {
        ...
        float h =
          height * std::pow(sh + float(a) / numBranch * (1 - sh), (float)0.7f);
        float angle =
          baseAngle + (a * 0.618f + 0.1f * gu->RandFloat()) * 2 * PI;
        float3 dir(orto1 * std::sin(angle) + orto2 * std::cos(angle));
        dir.y = (a - numBranch) * 0.01f - (0.2f + 0.2f * gu->RandFloat());
        dir.ANormalize();
        float size =
          std::sqrt((float)numBranch - a + 5) * 0.08f * MAX_TREE_HEIGHT;
        DrawPineBranch(float3(0, h, 0), dir, size);
    }
    // create the top
    float col = 0.55f + 0.2f * gu->RandFloat();
    va->AddVertexTN(float3(0, height - 0.09f * MAX_TREE_HEIGHT, 0),
                    0.126f + 0.5f,
                    0.02f,
                    float3(0, 0, col));
    va->AddVertexTN(float3(0, height - 0.03f * MAX_TREE_HEIGHT, 0),
                    0.249f + 0.5f,
                    0.02f,
                    float3(0.05f * MAX_TREE_HEIGHT, 0, col));
    va->AddVertexTN(float3(0, height - 0.03f * MAX_TREE_HEIGHT, 0),
                    0.126f + 0.5f,
                    0.98f,
                    float3(-0.05f * MAX_TREE_HEIGHT, 0, col));
    ...
    for (int a = 0; a < numIter; a++) {
        ...
        float col =
          ...
          (((orto1 * std::sin(angle) + orto2 * std::cos(angle)).dot(flatSun))) *
            0.3f;
        float col2 =
          0.45f + (((orto1 * std::sin(angle2) + orto2 * std::cos(angle2))
                      .dot(flatSun))) *
                    0.3f;

        va->AddVertexTN(start + orto1 * std::sin(angle) * size +
                          orto2 * std::cos(angle) * size,
                        angle / PI * 0.125f * 0.5f + 0.5f,
                        0,
                        float3(0, 0, col));
        va->AddVertexTN(end + orto1 * std::sin(angle) * size * 0.1f +
                          orto2 * std::cos(angle) * size * 0.1f,
                        angle / PI * 0.125f * 0.5f + 0.5f,
                        3,
                        float3(0, 0, col));
        va->AddVertexTN(start + orto1 * std::sin(angle2) * size +
                          orto2 * std::cos(angle2) * size,
                        angle2 / PI * 0.125f * 0.5f + 0.5f,
                        0,
                        float3(0, 0, col2));
        ...
        va->AddVertexTN(end + orto1 * std::sin(angle) * size * 0.1f +
                        ...
                        3,
                        float3(0, 0, col));
        va->AddVertexTN(end + orto1 * std::sin(angle2) * size * 0.1f +
                          orto2 * std::cos(angle2) * size * 0.1f,
                        angle2 / PI * 0.125f * 0.5f + 0.5f,
                        3,
                        float3(0, 0, col2));
    }
                                  ...
                                  float size)
{
    ...
    va->AddVertexTN(start + dir * size * 0.5f + orto1 * size * 0.5f +
                      orto2 * size * 0.2f,
                    0.5f + 0.249f + tex,
                    0.02f,
                    float3(0, 0, col2));
    va->AddVertexTN(start + dir * size * 0.5f,
                    0.5f + 0.1875f + tex,
                    0.50f,
                    float3(0, 0, col4));
    ...
    va->AddVertexTN(start + dir * size * 0.5f + orto1 * size * 0.5f +
                    ...
                    0.02f,
                    float3(0, 0, col2));
    va->AddVertexTN(start + dir * size + orto2 * size * 0.1f,
                    0.5f + 0.249f + tex,
                    0.98f,
                    float3(0, 0, col5));
    ...
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            if (buf2[(y * 256 + x) * 4 + 1] != 0) {
                ...
                buf[y + ypos][x + xpos][0] =
                  (unsigned char)(0.24f * 1.2f * 255);
                buf[y + ypos][x + xpos][1] =
                  (unsigned char)(0.40f * 1.2f * 255);
                buf[y + ypos][x + xpos][2] =
                  (unsigned char)(0.23f * 1.2f * 255);
