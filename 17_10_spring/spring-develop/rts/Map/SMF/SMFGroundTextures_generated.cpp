...
CSMFGroundTextures::LoadTiles(CSMFMapFile& file)
{
    ...
    if ((mapDims.mapx != header.mapx) || (mapDims.mapy != header.mapy)) {
        throw content_error(
          "Error loading map: size from header doesn't match map size.");
    }
    ...
    if (!smf.smtFileNames.empty()) {
        if (smf.smtFileNames.size() != tileHeader.numTileFiles) {
            LOG_L(L_WARNING,
                  ...
                  smf.smtFileNames.size(),
                  tileHeader.numTileFiles);
        } else {
    ...
    for (int a = 0; a < tileHeader.numTileFiles; ++a) {
        ...
        ifs->ReadString(&fileNameBuffer[0],
                        sizeof(char) * (sizeof(fileNameBuffer) - 1));
        ...
        if (!tileFile.FileExists()) {
            // try absolute path
            smtFilePath =
              (!smtHeaderOverride) ? smtFileName : smf.smtFileNames[a];
        ...
        if (!tileFile.FileExists()) {
            ...
            memset(&tiles[curTile * SMALL_TILE_SIZE],
                   0xaa,
                   numSmallTiles * SMALL_TILE_SIZE);
        ...
        if (strcmp(tfh.magic, "spring tilefile") != 0 || tfh.version != 1 ||
            ...
            sprintf(
              t,
              "[CSMFGroundTextures] file \"%s\" does not match .smt format",
              smtFilePath.c_str());
  ...
  defined(GLEW_ARB_ES3_compatibility)
    if (RecompressTilesIfNeeded()) {
        ...
        // ETC2 is backward compatible with ETC1! GLEW doesn't have the ETC1
        // extension :<
...
CSMFGroundTextures::ConvolveHeightMap(const int mapWidth, const int mipLevel)
{
    ...
    for (int y = 0; y < nby; ++y) {
        for (int x = 0; x < nbx; ++x) {
            ...
            // NOTE: we leave out the borders on sampling because it is easier
            // to do the Sobel kernel convolution
            for (int x2 = x * mipSquareSize + 1;
                 x2 < (x + 1) * mipSquareSize - 1;
                 x2++) {
                for (int y2 = y * mipSquareSize + 1;
                     y2 < (y + 1) * mipSquareSize - 1;
                     y2++) {
                    heightMaxima[y * nbx + x] =
                      std::max(hdata[y2 * mx + x2], heightMaxima[y * nbx + x]);
                    heightMinima[y * nbx + x] =
                      std::min(hdata[y2 * mx + x2], heightMinima[y * nbx + x]);

                    // Gx sobel kernel
                    const float gx = -1.0f * hdata[(y2 - 1) * mx + x2 - 1] +
                                     ...
                                     1.0f * hdata[(y2 + 1) * mx + x2 + 1];
                    // Gy sobel kernel
                    const float gy = -1.0f * hdata[(y2 + 1) * mx + x2 - 1] +
                    ...
                    const float g =
                      (math::fabs(gx) + math::fabs(gy)) / mipSquareSize;
...
CSMFGroundTextures::RecompressTilesIfNeeded()
{
    ...
    // note 2: Nvidia supports ETC but preprocesses the texture (on the CPU)
    // each upload = slow -> makes no sense to add it as another map compression
    // format note 3: for both DXT1 & ETC1/2 blocksize is 8 bytes per 4x4 pixel
    // block -> perfect for us :)

    loadscreen->SetLoadMessage("Recompressing Map Tiles with ETC1");

    rg_etc1::pack_etc1_block_init();
    rg_etc1::etc1_pack_params pack_params;
    pack_params.m_quality =
      rg_etc1::cLowQuality; // must be low, all others take _ages_ to process
    ...
    for_mt(0, tiles.size() / 8, [&](const int i) {
        ...
        rg_etc1::pack_etc1_block(
          &tiles[i * 8], (const unsigned int*)rgba, pack_params);
    });
...
CSMFGroundTextures::TexSquareInView(int btx, int bty) const
{
    ...
    static const float bigTexSquareRadius =
      fastmath::apxsqrt(smfMap->bigTexSize * smfMap->bigTexSize +
    ...
    for (int y = 0; y < smfMap->numBigTexY; ++y) {
        ...
        for (int x = 0; x < smfMap->numBigTexX; ++x) {
            ...
            if (!TexSquareInView(x, y)) {
                if ((square->GetMipLevel() < 3) &&
                    ((globalRendering->drawFrame - square->GetDrawFrame()) >
                     120)) {
            ...
            float dx =
              cam->GetPos().x - (x * smfMap->bigSquareSize * SQUARE_SIZE);
            dx -= (SQUARE_SIZE << 6);
            dx = std::max(0.0f, float(math::fabs(dx) - (SQUARE_SIZE << 6)));

            const float hAvg = (heightMaxima[y * smfMap->numBigTexX + x] +
            ...
            //    the minimum mip level is the closest ceiling mip level that we
            //    can use based on distance, FOV and tile size; we can increase
            //    this mip level IF the stretch factor requires us to do so.
            //
            //    we will approximate tile size with a sphere 512 elmos in
            //    radius, which translates to a diameter of =~ sqrt2 *
            //    bigTexSize =~ 1400 pixels
            //
            //    half (vertical) FOV is 45 degs, for default and most other
            //    camera modes
            int wantedLevel = 0;
            float heightDiff = heightMaxima[y * smfMap->numBigTexX + x] -
                               heightMinima[y * smfMap->numBigTexX + x];
            ...
            if (stretchFactors[y * smfMap->numBigTexX + x] > 16000 &&
                wantedLevel > 0)
                wantedLevel--;
                                        ...
                                        int texID)
{
    ...
    GroundSquare* square =
      &squares[texSquareY * smfMap->numBigTexX + texSquareX];
                                        ...
                                        int texMipLevel)
{
    ...
    pbo.New(numSqBytes);
    ExtractSquareTiles(
      texSquareX, texSquareY, texMipLevel, (GLint*)pbo.MapBuffer());
    pbo.UnmapBuffer();

    glBindTexture(ttarget, texID);
    glCompressedTexImage2D(ttarget,
                           0,
                           tileTexFormat,
                           texSizeX,
                           texSizeY,
                                       ...
                                       GLint* tileBuf) const
{
    ...
    for (int y1 = 0; y1 < BLOCK_SIZE; y1++) {
        for (int x1 = 0; x1 < BLOCK_SIZE; x1++) {
            ...
            const GLint* tile =
              (GLint*)&tiles[tileIdx * SMALL_TILE_SIZE + mipOffset];

            const int doff =
              (x1 * numBlocks) + (y1 * numBlocks * numBlocks) * BLOCK_SIZE;

            for (int b = 0; b < numBlocks; b++) {
                const GLint* sbuf = &tile[b * numBlocks * 2];
                GLint* dbuf = &tileBuf[(doff + b * numBlocks * BLOCK_SIZE) * 2];

                // at MIP level n: ((8 >> n) * 2 * 4) = (64 >> n) bytes for each
                // <b>
...
CSMFGroundTextures::LoadSquareTexture(int x, int y, int level)
{
    ...
    if (smfMap->GetTexAnisotropyLevel(false) != 0.0f)
        glTexParameterf(ttarget,
                        GL_TEXTURE_MAX_ANISOTROPY_EXT,
                        smfMap->GetTexAnisotropyLevel(false));
