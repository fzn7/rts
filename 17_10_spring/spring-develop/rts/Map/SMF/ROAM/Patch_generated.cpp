...
CTriNodePool::InitPools(bool shadowPass, size_t newPoolSize)
{
    ...
    const size_t allocPerThread =
      std::max(newPoolSize / numThreads, newPoolSize / 3) & (~(size_t)0x1);
    ...
    try {
        ...
        LOG_L(L_FATAL,
              "Failed to allocate memory for ROAM (reducing pool size): %s",
              e.what());
...
{
    assert((poolSize & 0x1) ==
           0); // we always allocate left & right, so we need an even pool
    m_NextTriNode = 0;
...
Patch::Split(TriTreeNode* tri)
{
    ...
    // If this triangle is not in a proper diamond, force split our base
    // neighbor
    ...
    if (!tri->IsBranch()) {
        ...
        // special handling the cause that only one of them is NULL wouldn't
        // make sense (only less performance)
    ...
    if (tri->BaseNeighbor != nullptr) {
        if (tri->BaseNeighbor->IsBranch()) {
            ...
            // Base Neighbor (in a diamond with us) was not split yet, so do
            // that now.
            Split(tri->BaseNeighbor);
        }
    } else {
                        ...
                        const int node)
{
    ...
    if (node < (1 << VARIANCE_DEPTH)) {
        ...
        const float myVariance =
          std::min(currentVariance[node], varianceMaxLimit);
    ...
    if (tri->IsBranch()) {
        ...
        const int2 center = { (left.x + right.x) >> 1,
                              (left.y + right.y) >> 1 };
...
Patch::GenerateIndices()
{
    indices.clear();
    RecursRender(
      &baseLeft, int2(0, PATCH_SIZE), int2(PATCH_SIZE, 0), int2(0, 0));
    RecursRender(&baseRight,
                 int2(PATCH_SIZE, 0),
                 int2(0, PATCH_SIZE),
                 int2(PATCH_SIZE, PATCH_SIZE));
}
...
Patch::GetHeight(int2 pos)
{
    ...
    assert(
      readMap->GetCornerHeightMapUnsynced()[(coors.y + pos.y) * mapDims.mapxp1 +
                                            (coors.x + pos.x)] ==
      vertices[vindex]);
                             ...
                             const int node)
{
    ...
    //
    if ((hgts.x * hgts.y) < 0.0f || (hgts.x * mhgt) < 0.0f ||
        (hgts.y * mhgt) < 0.0f)
        myVariance = std::max(myVariance * 1.5f, 20.0f);

    // myVariance = MAX(abs(left.x - rght.x), abs(left.y - rght.y)) *
    // myVariance;
    ...
    if ((abs(left.x - rght.x) >= 4) || (abs(left.y - rght.y) >= 4)) {
        ...
        const float child1Variance =
          RecursComputeVariance(apex, left, mpos, hgts1, (node << 1));
        const float child2Variance =
          RecursComputeVariance(rght, apex, mpos, hgts2, (node << 1) + 1);

        // final variance for this node is the max of its own variance and that
        // of its children
...
Patch::Tessellate(const float3& campos, int groundDetail, bool shadowPass)
{
    ...
    camDistLODFactor *= 300.0f / groundDetail; // MAGIC NUMBER 1: increase the
                                               // dividend to reduce LOD in
                                               // camera distance
    camDistLODFactor = std::max(1.0f, camDistLODFactor);
    camDistLODFactor = 1.0f / camDistLODFactor;

    // MAGIC NUMBER 2:
    //   variances are clamped by it, so it regulates how strong areas are
    //   tessellated. Note, the maximum tessellation is untouched by it. Instead
    //   it reduces the maximum LOD in distance, while the param above defines
    //   the overall FallOff rate.
                          ...
                          bool leftChild)
{
    if (tri->IsLeaf()) {
        const float3& v1 =
          *(float3*)&vertices[(apex.x + apex.y * (PATCH_SIZE + 1)) * 3];
        const float3& v2 =
          *(float3*)&vertices[(left.x + left.y * (PATCH_SIZE + 1)) * 3];
        const float3& v3 =
          *(float3*)&vertices[(rght.x + rght.y * (PATCH_SIZE + 1)) * 3];
    ...
    if ((depth & 1) == 0) {
        RecursBorderRender(
          va, tri->LeftChild, apex, left, center, depth + 1, !leftChild);
        return RecursBorderRender(va,
                                  tri->RightChild,
                                  rght,
                                  apex,
                                  center,
                                  depth + 1,
                                  leftChild); // return is needed for tail call
                                              // optimization (it's still
                                              // unlikely gcc does so...)
    }

    if (leftChild) {
        return RecursBorderRender(
          va, tri->LeftChild, apex, left, center, depth + 1, leftChild);
    } else {
        return RecursBorderRender(
          va, tri->RightChild, rght, apex, center, depth + 1, !leftChild);
    }
}
...
Patch::Upload()
{
    switch (renderMode) {
        ...
        case VBO: {
            ...
            GL_BUFFER_SIZE_ARB, &bufferSize); if(rend != bufferSize) {
                    glDeleteBuffersARB(1, &vertexIndexBuffer);
                    LOG( "[createVBO()] Data size is mismatch with input
            array\n" );
            }
            */
...
class CPatchInViewChecker : public CReadMap::IQuadDrawer
{
  public:
    ...
    {
        patchArray[y * numPatchesX + x]
          .lastDrawFrames[testCamera->GetCamType()] =
...
void
Patch::UpdateVisibility(CCamera* cam,
