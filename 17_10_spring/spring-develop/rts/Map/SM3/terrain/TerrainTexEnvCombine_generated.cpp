...
NodeTexEnvSetup::GetDebugDesc()
{
    ...
    const char* opstr[] = {
        "Replace",     "Mul", "Add", "AddSigned", "Sub", "Previous",
    ...
    const char* srcstr[] = { "None", "TextureSrc", "ColorSrc" };

    for (size_t a = 0; a < stages.size(); a++) {
        str += "{" + std::string(opstr[(int)stages[a].operation]) + ", " +
               srcstr[(int)stages[a].source];
        if (stages[a].source == TexEnvStage::TextureSrc)
            str += " = " + std::string(stages[a].srcTexture->name);
        str += "} ";
    };
    return str;
}
  ...
  , curSetup(NULL)
{
    ...
    glTexImage2D(
      GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &pixel);
}
                                   ...
                                   RenderSetup* renderSetup)
{
    ...
    const size_t maxTextureUnits =
      (maxTextureUnitsSigned < 0) ? 0 : maxTextureUnitsSigned;
    unsigned int c = 0;
    ...
    while ((c < maxTextureUnits) && (c < shaderDef->stages.size())) {
        ...
        if (st.operation == ShaderDef::Alpha &&
            c == maxTextureUnits - 1) // blending needs 2 stages
            break;
    ...
    // Switch to multipass when the stages couldn't all be handled with
    // multitexturing
    while (c < shaderDef->stages.size()) {
        ...
        } else if ((st.operation == ShaderDef::Add) ||
    ...
    if (!shaderDef->hasLighting) {
        // multiply with color: this is either done through an extra texture
        // stage, or with another pass
...
bool
TexEnvSetupHandler::SetupShader(IShaderSetup* shadercfg, NodeSetupParams& parms)
{
    ...
    for (size_t cur = 0; cur < ns->stages.size(); cur++) {
        ...
        switch (st.operation) {
            ...
            case TexEnvStage::InsertAlpha:
                ...
                break;
            case TexEnvStage::Interp: // color = tex * previous.alpha + tex *
                                      // previous.alpha
                // Use previous alpha
                glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_RGB_ARB, GL_PREVIOUS_ARB);
                glTexEnvi(
                  GL_TEXTURE_ENV, GL_COMBINE_RGB_ARB, GL_INTERPOLATE_ARB);
                break;
