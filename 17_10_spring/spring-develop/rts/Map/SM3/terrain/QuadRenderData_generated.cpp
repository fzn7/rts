...
RenderDataManager::InitializeNode(TQuad* q)
{
    ...
    const Heightmap* hm =
      roothm->GetLevel(q->depth); // get the right heightmap level
        ...
        for (int x = q->hmPos.x; x <= (q->hmPos.x + QUAD_W); x++) {
            *(v++) = Vector3(
            ...
            if (vda & VRT_TangentSpaceMatrix) {
                ...
                // Take the inverse of the tangent space -> world space
                // transformation
                Vector3* tgs2ws = v;
...
RenderDataManager::InitializeNodeNormalMap(TQuad* q, int cfgNormalMapLevel)
{
    ...
    for (; hm->highDetail; hm = hm->highDetail, level++)
        if (level == q->depth + cfgNormalMapLevel)
            break;
    ...
    if (rd->normalMap && (rd->normalMapW == w) &&
    ...
    // fill texture
    glTexImage2D(
      GL_TEXTURE_2D, 0, 3, texw, texw, 0, GL_RGB, GL_UNSIGNED_BYTE, normals);
