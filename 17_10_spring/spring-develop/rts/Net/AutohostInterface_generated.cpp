...
AutohostInterface::AutohostInterface(const std::string& remoteIP,
  ...
  , initialized(false)
{
    std::string errorMsg = AutohostInterface::TryBindSocket(
      autohost, remoteIP, remotePort, localIP, localPort);
                                 ...
                                 int localPort)
{
    ...
    try {
        ...
        if (err) {
            throw std::runtime_error("Failed to parse address " + remoteIP +
                                     ": " + err.message());
        }
        ...
        if (localIP.empty()) {
            ...
            if (err) {
                throw std::runtime_error("Failed to parse local IP " + localIP +
                                         ": " + err.message());
            }
            if (localAddr.is_v6() != remoteAddr.is_v6()) {
                throw std::runtime_error(
                  "Local IP " + localAddr.to_string() + " and remote IP " +
        ...
        //		(remoteAddr.is_v6() ? 6 : 4), remoteAddr.c_str(),
        //remotePort);
        socket.connect(ip::udp::endpoint(remoteAddr, remotePort));
    } catch (const std::runtime_error& ex) { // includes also
                                             // boost::system::system_error, as
                                             // it inherits from runtime_error
        socket.close();
        errorMsg = ex.what();
        if (errorMsg.empty()) {
                                ...
                                const std::vector<uchar>& winningAllyTeams)
{
    const unsigned char msgsize =
      1 + 1 + 1 + (winningAllyTeams.size() * sizeof(uchar));
    std::vector<boost::uint8_t> buffer(msgsize);
...
AutohostInterface::GetChatMessage()
{
    if (autohost.is_open()) {
        ...
        if ((bytes_avail = autohost.available()) > 0) {
            ...
            /*const size_t bytesReceived = */ autohost.receive(
