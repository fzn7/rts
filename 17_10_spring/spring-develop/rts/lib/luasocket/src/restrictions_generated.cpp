...
CLuaSocketRestrictions::addRule(RestrictType type, const std::string& rule)
{
    if (rule == WILDCARD_HOST) { // allow everything
        restrictions[type].push_back(
          TSocketRule(WILDCARD_HOST, WILDCARD_PORT, true));
        return;
    }
    ...
    if (delimpos == std::string::npos) {
        LOG_L(L_ERROR,
              "Invalid %d rule: %s, rule has to be hostname:port",
              type,
              rule.c_str());
        return;
    }
    const std::string strport =
      rule.substr(delimpos + 1, delimpos - rule.length());
...
{
    {
        ...
        for (int i = start; i < end; i++) {
            ...
            for (it = restrictions[i].begin(); it != restrictions[i].end();
                 ++it) {
                const TSocketRule& rule = *it;
                if ((hostname == rule.hostname) ||
                    (rule.hostname == WILDCARD_HOST)) {
    ...
    for (int i = 0; i < ALL_RULES; i++) {
        ...
        for (it = restrictions[i].begin(); it != restrictions[i].end(); ++it) {
            ...
            if ((rule.hostname == hostname) &&
                (getRule((RestrictType)i, ip, rule.port) ==
                 NULL)) { // add rule with ip, when not exisitng
