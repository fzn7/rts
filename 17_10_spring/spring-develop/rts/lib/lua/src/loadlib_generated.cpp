...
ll_require(lua_State* L)
{
    ...
    if (lua_toboolean(L, -1)) {                /* is it there? */
        if (lua_touserdata(L, -1) == sentinel) /* check loops */
            luaL_error(
              L, "loop or previous error loading module " LUA_QS, name);
    ...
    for (i = 1;; i++) {
        ...
        if (lua_isnil(L, -1))
            luaL_error(
              L, "module " LUA_QS " not found:%s", name, lua_tostring(L, -2));
        ...
        const char* def)
{
    ...
    if (path == NULL)           /* no environment variable? */
        ...
        path = luaL_gsub(
          L, path, LUA_PATHSEP LUA_PATHSEP, LUA_PATHSEP AUXMARK LUA_PATHSEP);
