...
trydecpoint(LexState* ls, SemInfo* seminfo)
{
    ...
    if (!luaO_str2d(luaZ_buffer(ls->buff), &seminfo->r)) {
        ...
        buffreplace(
          ls, ls->decpoint, '.'); /* undo change (for error message) */
                ...
                switch (ls->current) {
                    ...
                    default: {
                        if (!isdigit(ls->current))
                            ...
                            if (c > UCHAR_MAX)
                                luaX_lexerror(
                                  ls, "escape sequence too large", TK_STRING);
                            save(ls, c);
                        }
                        continue;
                    }
        ...
        switch (ls->current) {
            ...
            default: {
                if (isspace(ls->current)) {
                    ...
                    ts = luaX_newstring(
                      ls, luaZ_buffer(ls->buff), luaZ_bufflen(ls->buff));
