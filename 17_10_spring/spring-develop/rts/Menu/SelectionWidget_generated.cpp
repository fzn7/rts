  ...
  : agui::GuiElement(parent)
{
    ...
    script->Clicked.connect(
      boost::bind(&SelectionWidget::ShowScriptList, this));
...
SelectionWidget::ShowModList()
{
    ...
    curSelect->Selected.connect(
      boost::bind(&SelectionWidget::SelectMod, this, _1));
    curSelect->WantClose.connect(
      boost::bind(&SelectionWidget::CleanWindow, this));

    const std::vector<CArchiveScanner::ArchiveData>& found =
      archiveScanner->GetPrimaryMods();

    std::map<std::string, std::string, doj::alphanum_less<std::string>>
      modMap; // name, desc  (using a map to sort)
    for (std::vector<CArchiveScanner::ArchiveData>::const_iterator it =
           found.begin();
         it != found.end();
         ++it) {
...
SelectionWidget::ShowMapList()
{
    ...
    curSelect->Selected.connect(
      boost::bind(&SelectionWidget::SelectMap, this, _1));
    curSelect->WantClose.connect(
      boost::bind(&SelectionWidget::CleanWindow, this));

    const std::vector<std::string>& arFound = archiveScanner->GetMaps();

    std::set<std::string, doj::alphanum_less<std::string>>
      mapSet; // use a set to sort them
    for (std::vector<std::string>::const_iterator it = arFound.begin();
         it != arFound.end();
         ++it) {
        mapSet.insert((*it).c_str());
    }

    for (std::set<std::string>::iterator sit = mapSet.begin();
         sit != mapSet.end();
         ++sit) {
...
{
    // FIXME: lua ai's should be handled in AIScriptHandler.cpp, too respecting
    // the selected game and map
    ...
    CAIScriptHandler::ScriptList scriptList =
      CAIScriptHandler::Instance().GetScriptList();
    for (CAIScriptHandler::ScriptList::iterator it = scriptList.begin();
         it != scriptList.end();
         ++it) {
...
SelectionWidget::ShowScriptList()
{
    ...
    curSelect->Selected.connect(
      boost::bind(&SelectionWidget::SelectScript, this, _1));
    curSelect->WantClose.connect(
      boost::bind(&SelectionWidget::CleanWindow, this));
...
SelectionWidget::SelectMod(const std::string& mod)
{
    ...
    // SelectScript(SelectionWidget::NoScriptSelect); //reset AI as LuaAI maybe
