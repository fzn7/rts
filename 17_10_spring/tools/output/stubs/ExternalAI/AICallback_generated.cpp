...
void
CAICallback::SendStartPos(bool ready, float3 startPos)
{
    if (ready) {
        clientNet->Send(
          CBaseNetProtocol::Get().SendStartPos(gu->myPlayerNum,
                                               team,
...
{
    const CSkirmishAIHandler::ids_t& teamAIs =
      skirmishAIHandler.GetSkirmishAIsInTeam(this->team);
    const SkirmishAIData* aiData = skirmishAIHandler.GetSkirmishAI(
      *(teamAIs.begin())); // FIXME is there a better way?
...
CAICallback::SendResources(float mAmount, float eAmount, int receivingTeamId)
{
    ...
    if ((team != receivingTeamId) &&
        ...
        teamHandler->Team(receivingTeamId) && teamHandler->Team(team) &&
        ...
        clientNet->Send(CBaseNetProtocol::Get().SendAIShare(
          ubyte(gu->myPlayerNum),
          skirmishAIHandler.GetCurrentAIID(),
          ubyte(team),
          ubyte(receivingTeamId),
          mAmount,
          eAmount,
          empty));
    }
...
{
    handleerror(0,
                "AI wants to use deprecated function \"CreateSharedMemArea\"",
                "Spring is closing:",
                MBF_OK | MBF_EXCL);
    return NULL;
}
...
{
    handleerror(0,
                "AI wants to use deprecated function \"ReleasedSharedMemArea\"",
                "Spring is closing:",
                MBF_OK | MBF_EXCL);
}
...
CAICallback::GiveOrder(int unitId, Command* c)
{
    ...
    clientNet->Send(
      CBaseNetProtocol::Get().SendAICommand(gu->myPlayerNum,
                                            skirmishAIHandler.GetCurrentAIID(),
                                            unitId,
                                            c->GetID(),
                                            c->aiCommandId,
...
{
    // NOTE: this function is never called, implemented in
    // SSkirmishAICallbackImpl
    return NULL;
}
...
CAICallback::GetEnemyUnits(int* unitIds, int unitIds_max)
{
    ...
    return FilterUnitsVector(
      unitHandler->activeUnits, unitIds, unitIds_max, &unit_IsEnemyAndInLos);
}
...
CAICallback::GetEnemyUnitsInRadarAndLos(int* unitIds, int unitIds_max)
{
    verify();
    myAllyTeamId = teamHandler->AllyTeam(team);
    return FilterUnitsVector(unitHandler->activeUnits,
                             unitIds,
                             unitIds_max,
                             &unit_IsEnemyAndInLosOrRadar);
}
                           ...
                           int unitIds_max)
{
    ...
    myAllyTeamId = teamHandler->AllyTeam(team);
    return FilterUnitsVector(
      units, unitIds, unitIds_max, &unit_IsEnemyAndInLos);
}
...
CAICallback::GetFriendlyUnits(int* unitIds, int unitIds_max)
{
    ...
    myAllyTeamId = teamHandler->AllyTeam(team);
    return FilterUnitsVector(
      unitHandler->activeUnits, unitIds, unitIds_max, &unit_IsFriendly);
}
...
CAICallback::GetNeutralUnits(int* unitIds, int unitIds_max)
{
    ...
    myAllyTeamId = teamHandler->AllyTeam(team);
    return FilterUnitsVector(
      unitHandler->activeUnits, unitIds, unitIds_max, &unit_IsNeutralAndInLos);
}
                             ...
                             int unitIds_max)
{
    ...
    myAllyTeamId = teamHandler->AllyTeam(team);
    return FilterUnitsVector(
      units, unitIds, unitIds_max, &unit_IsNeutralAndInLos);
}
...
{
    return readMap->GetInitMinHeight();
}
float
CAICallback::GetMaxHeight()
{
    return readMap->GetInitMaxHeight();
}
...
{
    const int jammerAllyTeam =
      modInfo.separateJammers ? teamHandler->AllyTeam(team) : 0;
...
CAICallback::CreateSplineFigure(const float3& pos1,
                                ...
                                int group)
{
    return geometricObjects->AddSpline(
      pos1, pos2, pos3, pos4, width, arrow, lifetime, group);
}
...
CAICallback::CanBuildAt(const UnitDef* unitDef, const float3& pos, int facing)
{
    ...
    return !!CGameHelper::TestUnitBuildSquare(
      bi, blockingF, teamHandler->AllyTeam(team), false);
}
...
CAICallback::ClosestBuildSite(const UnitDef* unitDef,
                              ...
                              int facing)
{
    return CGameHelper::ClosestBuildSite(
      team, unitDef, pos, searchRadius, minDist, facing);
}
...
CAICallback::GetFeatures(int* featureIds, int featureIds_sizeMax)
{
    ...
    for (CFeatureSet::const_iterator it = fset.begin();
         (it != fset.end()) && (featureIds_size < featureIds_sizeMax);
         ++it) {
        const CFeature* f = *it;
        assert(f);
                         ...
                         float radius)
{
    ...
    for (it = ft.begin();
         (it != ft.end()) && (featureIds_size < featureIds_sizeMax);
         ++it) {
        const CFeature* f = *it;
        assert(f);
...
CAICallback::GetValue(int id, void* data)
{
    ...
    switch (id) {
        ...
        case AIVAL_GUI_CAMERA_DIR: {
            *(static_cast<float3*>(data)) =
              camHandler->GetCurrentController().GetDir();
            return true;
        }
        case AIVAL_GUI_CAMERA_POS: {
            *(static_cast<float3*>(data)) =
              camHandler->GetCurrentController().GetPos();
            return true;
        }
        ...
        case AIVAL_LOCATE_FILE_W: {
            std::string f((char*)data);
            std::string f_abs = dataDirsAccess.LocateFile(
              f, FileQueryFlags::WRITE | FileQueryFlags::CREATE_DIRS);
...
CAICallback::HandleCommand(int commandId, void* data)
{
    switch (commandId) {
        ...
        case AIHCAddMapPointId: {
            const AIHCAddMapPoint* cmdData =
              static_cast<AIHCAddMapPoint*>(data);
            /*
               TODO: gu->myPlayerNum makes the command to look like as it comes
               from the local player, "team" should be used (but needs some
               major changes in other engine parts)
            */
            clientNet->Send(CBaseNetProtocol::Get().SendMapDrawPoint(
              gu->myPlayerNum,
              (short)cmdData->pos.x,
              (short)cmdData->pos.z,
              std::string(cmdData->label),
              false));
            return 1;
        } break;
        case AIHCAddMapLineId: {
            ...
            // see TODO above
            clientNet->Send(
              CBaseNetProtocol::Get().SendMapDrawLine(gu->myPlayerNum,
                                                      (short)cmdData->posfrom.x,
                                                      (short)cmdData->posfrom.z,
                                                      (short)cmdData->posto.x,
                                                      (short)cmdData->posto.z,
                                                      false));
            return 1;
        } break;
        case AIHCRemoveMapPointId: {
            const AIHCRemoveMapPoint* cmdData =
              static_cast<AIHCRemoveMapPoint*>(data);
            // see TODO above
            clientNet->Send(CBaseNetProtocol::Get().SendMapErase(
              gu->myPlayerNum, (short)cmdData->pos.x, (short)cmdData->pos.z));
            return 1;
        } break;
        ...
        case AIHCGetDataDirId: {
            // NOTE: these commands should never arrive, handled in
            // SSkirmishAICallbackImpl
            assert(false);
            return 0;
        } break;
        ...
        case AIHCTraceRayId: {
            ...
            if (CHECK_UNITID(cmdData->srcUID)) {
                ...
                if (srcUnit != NULL) {
                    ...
                    const float realLen = TraceRay::TraceRay(cmdData->rayPos,
                                                             cmdData->rayDir,
                                                             cmdData->rayLen,
                                                             cmdData->flags,
                                                             srcUnit,
        ...
        case AIHCFeatureTraceRayId: {
            ...
            if (CHECK_UNITID(cmdData->srcUID)) {
                ...
                if (srcUnit != NULL) {
                    CUnit* hitUnit = NULL;
                    CFeature* hitFeature = NULL;
                    // FIXME add COLLISION_NOENEMIES || COLLISION_NOFRIENDLIES
                    // || COLLISION_NONEUTRALS?
                    const float realLen = TraceRay::TraceRay(cmdData->rayPos,
                                                             cmdData->rayDir,
                                                             cmdData->rayLen,
                                                             cmdData->flags,
                    ...
                    if (hitFeature != NULL) {
                        const bool isFeatureVisible =
                          hitFeature->IsInLosForAllyTeam(
        ...
        case AIHCPauseId: {
            ...
            clientNet->Send(CBaseNetProtocol::Get().SendPause(gu->myPlayerNum,
                                                              cmdData->enable));
        ...
        case AIHCDebugDrawId: {
            ...
            switch (cmdData->cmdMode) {
                case AIHCDebugDraw::AIHC_DEBUGDRAWER_MODE_ADD_GRAPH_POINT: {
                    debugDrawerAI->AddGraphPoint(
                      this->team, cmdData->lineId, cmdData->x, cmdData->y);
                } break;
                case AIHCDebugDraw::AIHC_DEBUGDRAWER_MODE_DEL_GRAPH_POINTS: {
                    debugDrawerAI->DelGraphPoints(
                      this->team, cmdData->lineId, cmdData->numPoints);
                } break;
                case AIHCDebugDraw::AIHC_DEBUGDRAWER_MODE_SET_GRAPH_POS: {
                    debugDrawerAI->SetGraphPos(
                      this->team, cmdData->x, cmdData->y);
                } break;
                case AIHCDebugDraw::AIHC_DEBUGDRAWER_MODE_SET_GRAPH_SIZE: {
                    debugDrawerAI->SetGraphSize(
                      this->team, cmdData->w, cmdData->h);
                } break;
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_SET_GRAPH_LINE_COLOR: {
                    debugDrawerAI->SetGraphLineColor(
                      this->team, cmdData->lineId, cmdData->color);
                } break;
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_SET_GRAPH_LINE_LABEL: {
                    debugDrawerAI->SetGraphLineLabel(
                      this->team, cmdData->lineId, cmdData->label);
                } break;
                ...
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_UPDATE_OVERLAY_TEXTURE: {
                ...
                case AIHCDebugDraw::AIHC_DEBUGDRAWER_MODE_DEL_OVERLAY_TEXTURE: {
                    debugDrawerAI->DelOverlayTexture(this->team,
                                                     cmdData->texHandle);
                } break;
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_SET_OVERLAY_TEXTURE_POS: {
                    debugDrawerAI->SetOverlayTexturePos(
                      this->team, cmdData->texHandle, cmdData->x, cmdData->y);
                } break;
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_SET_OVERLAY_TEXTURE_SIZE: {
                    debugDrawerAI->SetOverlayTextureSize(
                      this->team, cmdData->texHandle, cmdData->w, cmdData->h);
                } break;
                case AIHCDebugDraw::
                  AIHC_DEBUGDRAWER_MODE_SET_OVERLAY_TEXTURE_LABEL: {
                    debugDrawerAI->SetOverlayTextureLabel(
                      this->team, cmdData->texHandle, cmdData->label);
                } break;
...
CAICallback::IsDebugDrawerEnabled() const
{
    ...
    // see eg. Debug_Drawer_isEnabled in
        ...
        switch (property) {
            ...
            case AIVAL_STOCKPILED: {
                if (!unit->stockpileWeapon ||
                    !teamHandler->Ally(unit->allyteam, allyTeam)) {
                    return false;
                }
