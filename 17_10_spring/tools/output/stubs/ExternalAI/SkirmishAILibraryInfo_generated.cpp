...
CSkirmishAILibraryInfo::CSkirmishAILibraryInfo(const std::string& aiInfoFile,
                                               const std::string& aiOptionFile)
{
    ...
    for (auto ii = tmpInfo.begin(); ii != tmpInfo.end(); ++ii) {
        // TODO remove this, once we support non-string value types for Skirmish
        // AI info
...
CSkirmishAILibraryInfo::GetInfo(const std::string& key) const
{
    ...
    if (!found) {
        LOG_L(L_WARNING,
              "Skirmish AI property '%s' could not be found.",
              key.c_str());
        return DEFAULT_VALUE;
    }
...
{
    static const std::string snKey =
      StringToLower(SKIRMISH_AI_PROPERTY_SHORT_NAME);
    ...
    if (keyLower == snKey || keyLower == vKey) {
        if (value.find_first_of(BAD_CHARS) != std::string::npos) {
            ...
            LOG_L(L_WARNING,
                  "Skirmish AI property (%s or %s) contains illegal characters "
                  "(%s).",
