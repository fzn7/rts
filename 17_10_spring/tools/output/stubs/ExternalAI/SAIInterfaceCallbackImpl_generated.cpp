...
CHECK_INTERFACE_ID(const int interfaceId)
{
    if (interfaceId < 0 || (size_t)interfaceId >= infos.size()) {
        ...
        buf << "Is " << interfaceId << ", but should be between min 0 and max "
            << infos.size() << ".";
        aiInterfaceCallback_Log_exception(
          interfaceId, buf.str().c_str(), 1, true);
    }
}
                                                   ...
                                                   const char* const key)
{
    ...
    const IAILibraryManager::T_skirmishAIInfos& skirmishInfos =
      IAILibraryManager::GetInstance()->GetSkirmishAIInfos();
    const IAILibraryManager::T_skirmishAIInfos::const_iterator inf =
      skirmishInfos.find(aiKey);
                                  ...
                                  bool die)
{
    ...
    LOG_L(L_ERROR,
          "AI Interface <%s-%s>: severety %i: [%s] %s",
          info->GetName().c_str(),
          info->GetVersion().c_str(),
          severety,
          (die ? "AI Interface shutting down" : "AI Interface still running"),
          msg);

    if (die) {
        // TODO: FIXME: unload all skirmish AIs of this interface plus the
        // interface itsself
    }
}
                                                ...
                                                bool dir)
{
    ...
    if (!aiInterfaceCallback_DataDirs_Roots_locatePath(
                                        ...
                                        bool common)
{
    ...
    std::string interfaceShortName =
      aiInterfaceCallback_AIInterface_Info_getValueByKey(
    ...
    if (common) {
        ...
        interfaceVersion = aiInterfaceCallback_AIInterface_Info_getValueByKey(
                                          ...
                                          bool common)
{
    ...
    if (!aiInterfaceCallback_DataDirs_locatePath(interfaceId,
                                                 path,
                                                 pathMaxSize,
                                                 relPath,
                                                 writeable,
...
aiInterfaceCallback_DataDirs_getWriteableDir(int interfaceId)
{
    ...
    // if it is not yet
    for (size_t wdd = writeableDataDirs.size(); wdd <= (size_t)interfaceId;
         ++wdd)
    ...
    if (writeableDataDirs[interfaceId].empty()) {
        ...
        const bool exists = aiInterfaceCallback_DataDirs_locatePath(
          interfaceId, tmpRes, sizeof(tmpRes), "", true, true, true, false);
...
aiInterfaceCallback_init(struct SAIInterfaceCallback* callback)
{
    ...
    callback->Engine_Version_getMajor =
      &aiInterfaceCallback_Engine_Version_getMajor;
    callback->Engine_Version_getMinor =
      &aiInterfaceCallback_Engine_Version_getMinor;
    callback->Engine_Version_getPatchset =
      &aiInterfaceCallback_Engine_Version_getPatchset;
    callback->Engine_Version_getCommits =
      &aiInterfaceCallback_Engine_Version_getCommits;
    callback->Engine_Version_getHash =
      &aiInterfaceCallback_Engine_Version_getHash;
    callback->Engine_Version_getBranch =
      &aiInterfaceCallback_Engine_Version_getBranch;
    callback->Engine_Version_getAdditional =
      &aiInterfaceCallback_Engine_Version_getAdditional;
    callback->Engine_Version_getBuildTime =
      &aiInterfaceCallback_Engine_Version_getBuildTime;
    callback->Engine_Version_isRelease =
      &aiInterfaceCallback_Engine_Version_isRelease;
    callback->Engine_Version_getNormal =
      &aiInterfaceCallback_Engine_Version_getNormal;
    callback->Engine_Version_getSync =
      &aiInterfaceCallback_Engine_Version_getSync;
    callback->Engine_Version_getFull =
      &aiInterfaceCallback_Engine_Version_getFull;
    callback->AIInterface_Info_getSize =
      &aiInterfaceCallback_AIInterface_Info_getSize;
    callback->AIInterface_Info_getKey =
      &aiInterfaceCallback_AIInterface_Info_getKey;
    callback->AIInterface_Info_getValue =
      &aiInterfaceCallback_AIInterface_Info_getValue;
    callback->AIInterface_Info_getDescription =
      &aiInterfaceCallback_AIInterface_Info_getDescription;
    callback->AIInterface_Info_getValueByKey =
      &aiInterfaceCallback_AIInterface_Info_getValueByKey;
    ...
    callback->SkirmishAIs_Info_getValueByKey =
      &aiInterfaceCallback_SkirmishAIs_Info_getValueByKey;
    ...
    callback->DataDirs_getConfigDir =
      &aiInterfaceCallback_DataDirs_getConfigDir;
    callback->DataDirs_getWriteableDir =
      &aiInterfaceCallback_DataDirs_getWriteableDir;
    callback->DataDirs_locatePath = &aiInterfaceCallback_DataDirs_locatePath;
    callback->DataDirs_allocatePath =
      &aiInterfaceCallback_DataDirs_allocatePath;
    callback->DataDirs_Roots_getSize =
      &aiInterfaceCallback_DataDirs_Roots_getSize;
    callback->DataDirs_Roots_getDir =
      &aiInterfaceCallback_DataDirs_Roots_getDir;
    callback->DataDirs_Roots_locatePath =
      &aiInterfaceCallback_DataDirs_Roots_locatePath;
    callback->DataDirs_Roots_allocatePath =
      &aiInterfaceCallback_DataDirs_Roots_allocatePath;
}
