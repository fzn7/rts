...
IsUnitInLosOrRadarOfAllyTeam(const CUnit& unit, const int allyTeamId)
{
    ...
    return (losHandler->globalLOS[allyTeamId] ||
            (unit.losStatus[allyTeamId] & (LOS_INLOS | LOS_INRADAR)));
}
...
CEngineOutHandler::UnitEnteredLos(const CUnit& unit, int allyTeamId)
{
    ...
    DO_FOR_ALLIED_SKIRMISH_AIS(
      EnemyEnterLOS(unitId), allyTeamId, unitAllyTeamId)
}
...
CEngineOutHandler::UnitLeftLos(const CUnit& unit, int allyTeamId)
{
    ...
    DO_FOR_ALLIED_SKIRMISH_AIS(
      EnemyLeaveLOS(unitId), allyTeamId, unitAllyTeamId)
}
...
CEngineOutHandler::UnitEnteredRadar(const CUnit& unit, int allyTeamId)
{
    ...
    DO_FOR_ALLIED_SKIRMISH_AIS(
      EnemyEnterRadar(unitId), allyTeamId, unitAllyTeamId)
}
...
CEngineOutHandler::UnitLeftRadar(const CUnit& unit, int allyTeamId)
{
    ...
    DO_FOR_ALLIED_SKIRMISH_AIS(
      EnemyLeaveRadar(unitId), allyTeamId, unitAllyTeamId)
}
...
CEngineOutHandler::UnitDestroyed(const CUnit& destroyed, const CUnit* attacker)
{
    ...
    if (team_skirmishAIs.find(dt) != team_skirmishAIs.end()) {
        const bool attackerInLosOrRadar =
          attacker &&
          IsUnitInLosOrRadarOfAllyTeam(*attacker, destroyed.allyteam);
    ...
    for (auto& ai : id_skirmishAI) {
        ...
        if (!saw->IsCheatEventsEnabled() &&
            !IsUnitInLosOrRadarOfAllyTeam(destroyed, aiAllyTeam))
            continue;

        int myAttackerId = -1;

        if ((attacker != NULL) &&
            teamHandler->Ally(aiAllyTeam, attacker->allyteam)) {
                               ...
                               bool paralyzer)
{
    ...
    if (team_skirmishAIs.find(dt) != team_skirmishAIs.end()) {
        ...
        if (attacker) {
            attackDir_damagedsView =
              attacker->GetErrorPos(damaged.allyteam) - damaged.pos;
            attackDir_damagedsView.ANormalize();
        }

        const bool attackerInLosOrRadar =
          attacker && IsUnitInLosOrRadarOfAllyTeam(*attacker, damaged.allyteam);
    ...
    if (attacker) {
        ...
        const float3 attackDir =
          (attacker->pos - damaged.GetErrorPos(attacker->allyteam))
            .ANormalize();
        const bool damagedInLosOrRadar =
          IsUnitInLosOrRadarOfAllyTeam(damaged, attacker->allyteam);

        for (auto ai : team_skirmishAIs[at]) {
            CSkirmishAIWrapper* saw = id_skirmishAI[ai].get();

            if (damagedInLosOrRadar || saw->IsCheatEventsEnabled()) {
                saw->EnemyDamaged(damagedUnitId,
                                  attackerUnitId,
                                  damage,
                                  attackDir,
                                  weaponDefID,
                               ...
                               float strength)
{
    ...
    DO_FOR_ALLIED_SKIRMISH_AIS(SeismicPing(allyTeamId, unitId, pos, strength),
                               allyTeamId,
                               unitAllyTeamId)
}
                                      ...
                                      int playerId)
{
    ...
    DO_FOR_TEAM_SKIRMISH_AIS(PlayerCommandGiven(selectedUnitIds, c, playerId),
                             teamId);
}
...
CEngineOutHandler::CommandFinished(const CUnit& unit, const Command& command)
{
    ...
    const int teamId = unit.team;
    const int unitId = unit.id;
    const int aiCommandTopicId =
      extractAICommandTopic(&command, unitHandler->MaxUnits());

    DO_FOR_TEAM_SKIRMISH_AIS(
      CommandFinished(unitId, command.aiCommandId, aiCommandTopicId), teamId);
}
                                   ...
                                   std::vector<const char*>& outData)
{
    ...
    if (aiTeam != -1) {
        ...
        for (auto aiIDsIter = aiIDs.begin(); aiIDsIter != aiIDs.end();
             ++aiIDsIter) {
...
CEngineOutHandler::CreateSkirmishAI(const size_t skirmishAIId)
{
    ...
    // const bool unpauseAfterAIInit =
    // configHandler->GetBool("AI_UnpauseAfterInit");
    ...
    playerHandler->Player(gu->myPlayerNum)->name; LOG( "Player %s (auto)-paused
    the game for letting Skirmish AI" " %s initialize for controlling team
    %i.%s", myPlayerName.c_str(), key.GetShortName().c_str(), teamId,
                            (unpauseAfterAIInit ?
                             " The game is auto-unpaused as soon as the AI is
    ready." :
                             ""));
    ...
    if (aiData->status != SKIRMAISTATE_RELOADING) {
        clientNet->Send(CBaseNetProtocol::Get().SendAIStateChanged(
          gu->myPlayerNum, skirmishAIId, SKIRMAISTATE_INITIALIZING));
    }

    if (aiData->isLuaAI) {
        // currently, we need to do nothing for Lua AIs
        clientNet->Send(CBaseNetProtocol::Get().SendAIStateChanged(
          gu->myPlayerNum, skirmishAIId, SKIRMAISTATE_ALIVE));
    } else {
...
CEngineOutHandler::DestroySkirmishAI(const size_t skirmishAIId)
{
    ...
    clientNet->Send(CBaseNetProtocol::Get().SendAIStateChanged(
      gu->myPlayerNum, skirmishAIId, SKIRMAISTATE_DEAD));
}
