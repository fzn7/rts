...
IrrXMLReader*
createIrrXMLReader(const char* filename)
{
...
//! Creates an instance of an UFT-8 or ASCII character xml parser.
IrrXMLReader*
createIrrXMLReader(IFileReadCallBack* callback)
{
...
{
    return new CXMLReaderImpl<char16, IXMLBase>(
      new CFileReadCallBack(filename));
}
...
{
    return new CXMLReaderImpl<char32, IXMLBase>(
      new CFileReadCallBack(filename));
}
