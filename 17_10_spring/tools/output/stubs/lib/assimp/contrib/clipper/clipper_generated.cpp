...
/*******************************************************************************
 *                                                                              *
 * Author    :  Angus Johnson * Version   :  4.6.3
 ...
 *                                                                              *
 * See also: * "Polygon Offsetting by Computing Winding Numbers"
  ...
  public:
    ...
    Int128 operator/(const Int128& rhs) const
    {
        ...
        if (denom.hi < 0)
            Negate(denom);
        if (denom > result)
            return Int128(0); // result is only a fraction of 1
        Negate(denom);
...
bool
Orientation(const Polygon& poly)
{
    ...
    if (UseFullInt64Range) {
        Int128 cross =
          Int128(vec1.X) * Int128(vec2.Y) - Int128(vec2.X) * Int128(vec1.Y);
        return cross > 0;
    } else {
...
bool
Orientation(OutRec* outRec, bool UseFullInt64Range)
{
...
inline bool
PointsEqual(const IntPoint& pt1, const IntPoint& pt2)
{
...
Area(const Polygon& poly)
{
    ...
    if (UseFullInt64Range) {
        ...
        for (int i = 0; i < highI; ++i)
            a += (float)poly[i].X * poly[i + 1].Y -
                 (float)poly[i + 1].X * poly[i].Y;
        return a / 2;
    }
}
//------------------------------------------------------------------------------
...
bool
PointInPolygon(const IntPoint& pt, OutPt* pp, bool UseFullInt64Range)
{
    OutPt* pp2 = pp;
    bool result = false;
    if (UseFullInt64Range) {
...
bool
SlopesEqual(TEdge& e1, TEdge& e2, bool UseFullInt64Range)
{
    if (e1.ybot == e1.ytop)
        return (e2.ybot == e2.ytop);
    else if (e1.xbot == e1.xtop)
        return (e2.xbot == e2.xtop);
...
bool
SlopesEqual(const IntPoint pt1,
            const IntPoint pt2,
            const IntPoint pt3,
            bool UseFullInt64Range)
{
...
SetDx(TEdge& e)
{
    if (e.ybot == e.ytop)
        ...
        e.dx = (float)(e.xtop - e.xbot) / (float)(e.ytop - e.ybot);
}
//---------------------------------------------------------------------------
...
{
    if ((val < 0))
        return static_cast<long64>(val - 0.5);
    else
        return static_cast<long64>(val + 0.5);
}
//------------------------------------------------------------------------------
...
long64
TopX(TEdge& edge, const long64 currentY)
{
...
long64
TopX(const IntPoint pt1, const IntPoint pt2, const long64 currentY)
{
...
bool
IntersectPoint(TEdge& edge1, TEdge& edge2, IntPoint& ip, bool UseFullInt64Range)
{
    float b1, b2;
    ...
    return
      // can be *so close* to the top of one edge that the rounded Y equals one
      // ytop ...
...
bool
GetOverlapSegment(IntPoint pt1a,
                  IntPoint pt1b,
                  IntPoint pt2a,
                  IntPoint pt2b,
                  IntPoint& pt1,
...
bool
FindSegment(OutPt*& pp, IntPoint& pt1, IntPoint& pt2)
{
...
bool
Pt3IsBetweenPt1AndPt2(const IntPoint pt1,
                      const IntPoint pt2,
                      const IntPoint pt3)
{
...
OutPt*
InsertPolyPtBetween(OutPt* p1, OutPt* p2, const IntPoint pt)
{
    if (p1 == p2)
        throw "JoinError";
...
bool
ClipperBase::AddPolygon(const Polygon& pg, PolyType polyType)
{
    int len = (int)pg.size();
    if (len < 3)
        return false;
...
void
ClipperBase::InsertLocalMinima(LocalMinima* newLm)
{
    if (!m_MinimaList) {
        m_MinimaList = newLm;
...
TEdge*
ClipperBase::AddBoundsToLML(TEdge* e)
{
    ...
    if (NEAR_EQUAL(e->dx,
                   HORIZONTAL)) // horizontal edges never start a left bound
    {
...
bool
ClipperBase::AddPolygons(const Polygons& ppg, PolyType polyType)
{
    bool result = true;
...
bool
Clipper::Execute(ClipType clipType,
                 Polygons& solution,
...
bool
PolySort(OutRec* or1, OutRec* or2)
{
    if (or1 == or2)
        return false;
    if (!or1->pts || !or2->pts) {
...
OutRec*
FindAppendLinkEnd(OutRec* outRec)
{
    while (outRec->AppendLink)
...
Clipper::FixHoleLinkage(OutRec* outRec)
{
    ...
    if (outRec == tmp)
        throw clipperException("HoleLinkage error");
...
void
Clipper::InsertScanbeam(const long64 Y)
{
    if (!m_Scanbeam) {
...
void
Clipper::DisposeOutRec(PolyOutList::size_type index, bool ignorePts)
{
    OutRec* outRec = m_PolyOuts[index];
    if (!ignorePts && outRec->pts)
...
Clipper::SetWindingCount(TEdge& edge)
{
    ...
    // AEL
    while (e && e->polyType != edge.polyType)
        e = e->prevInAEL;
    if (!e) {
...
void
Clipper::AddJoin(TEdge* e1, TEdge* e2, int e1OutIdx, int e2OutIdx)
{
...
void
Clipper::AddHorzJoin(TEdge* e, int idx)
{
...
void
Clipper::InsertLocalMinimaIntoAEL(const long64 botY)
{
    while (m_CurrentLM && (m_CurrentLM->Y == botY)) {
        ...
        if (lb->outIdx >= 0 && lb->prevInAEL && lb->prevInAEL->outIdx >= 0 &&
            lb->prevInAEL->xcurr == lb->xbot &&
        ...
        if (lb->nextInAEL != rb) {
            ...
            while (e != rb) {
                ...
                // assumes  that param1 will be to the right of param2 ABOVE the
                // intersection ...
...
void
Clipper::IntersectEdges(TEdge* e1,
                        TEdge* e2,
                        const IntPoint& pt,
                        IntersectProtects protects)
{
    ...
    // point ...
    bool e1stops = !(ipLeft & protects) && !e1->nextInLML && e1->xtop == pt.X &&
                   e1->ytop == pt.Y;
    ...
    if (e1Contributing && e2contributing) {
        if (e1stops || e2stops || (e1Wc != 0 && e1Wc != 1) ||
            (e2Wc != 0 && e2Wc != 1) ||
        ...
        if ((e2Wc == 0 || e2Wc == 1) &&
            (m_ClipType != ctIntersection || e2->polyType == ptSubject ||
             (e2->windCnt2 != 0)))
            DoEdge1(e1, e2, pt);
    } else if (e2contributing) {
        if ((e1Wc == 0 || e1Wc == 1) &&
            (m_ClipType != ctIntersection || e1->polyType == ptSubject ||
             (e1->windCnt2 != 0)))
            DoEdge2(e1, e2, pt);
            ...
            switch (m_ClipType) {
                ...
                case ctDifference:
                    if ((e1->polyType == ptClip && e2->polyType == ptClip &&
                         e1Wc2 > 0 && e2Wc2 > 0) ||
                        (e1->polyType == ptSubject &&
                         e2->polyType == ptSubject && e1Wc2 <= 0 && e2Wc2 <= 0))
                        AddLocalMinPoly(e1, e2, pt);
                    break;
                case ctXor:
                    AddLocalMinPoly(e1, e2, pt);
...
bool
GetNextNonDupOutPt(OutPt* pp, OutPt*& next)
{
    next = pp->next;
...
bool
GetPrevNonDupOutPt(OutPt* pp, OutPt*& prev)
{
    prev = pp->prev;
...
GetLowermostRec(OutRec* outRec1, OutRec* outRec2)
{
    ...
    if (outPt1->pt.Y > outPt2->pt.Y)
        ...
        return outRec2;
    else if (outRec1->bottomE2 == 0)
        return outRec2;
    else if (outRec2->bottomE2 == 0)
        return outRec1;
    else {
...
void
Clipper::AddOutPt(TEdge* e, TEdge* altE, const IntPoint& pt)
{
...
bool
Clipper::IsTopHorz(const long64 XPos)
{
...
bool
IsMaxima(TEdge* e, const long64 Y)
{
...
bool
IsIntermediate(TEdge* e, const long64 Y)
{
...
void
Clipper::SwapPositionsInAEL(TEdge* edge1, TEdge* edge2)
{
...
Clipper::ProcessHorizontal(TEdge* horzEdge)
{
        ...
        if (eMaxPair || ((dir == dLeftToRight) && (e->xcurr <= horzRight)) ||
            ...
            if (e->xcurr == horzEdge->xtop && !eMaxPair) {
                if (SlopesEqual(*e, *horzEdge->nextInLML, m_UseFullRange)) {
                    ...
                    // we really have got to the end of the intermediate horz
                    // edge so quit.  nb: More -ve slopes follow more +ve slopes
                    // ABOVE the horizontal.
                    break;
            }

            if (e == eMaxPair) {
                // horzEdge is evidently a maxima horizontal and we've arrived
                // at its end.
                if (dir == dLeftToRight)
                    IntersectEdges(
                      horzEdge, e, IntPoint(e->xcurr, horzEdge->ycurr), ipNone);
                else
                    IntersectEdges(
                      e, horzEdge, IntPoint(e->xcurr, horzEdge->ycurr), ipNone);
                ...
                // are  processed as if layered with the current horizontal edge
                // (horizEdge)  being infinitesimally lower that the next (e).
                // Therfore, we  intersect with e only if e.xcurr is within the
                // bounds of horzEdge ...
                if (dir == dLeftToRight)
                    IntersectEdges(horzEdge,
                                   e,
                                   IntPoint(e->xcurr, horzEdge->ycurr),
    ...
    if (horzEdge->nextInLML) {
        ...
        if (horzEdge->outIdx >= 0)
            IntersectEdges(horzEdge,
                           eMaxPair,
                           IntPoint(horzEdge->xtop, horzEdge->ycurr),
                           ipBoth);
        if (eMaxPair->outIdx >= 0)
            throw clipperException("ProcessHorizontal error");
...
void
Clipper::UpdateEdgeIntoAEL(TEdge*& e)
{
    if (!e->nextInLML)
...
bool
Clipper::ProcessIntersections(const long64 botY, const long64 topY)
{
    if (!m_ActiveEdges)
        return true;
    try {
...
void
Clipper::BuildIntersectList(const long64 botY, const long64 topY)
{
    if (!m_ActiveEdges)
        return;
...
bool
Process1Before2(IntersectNode& node1, IntersectNode& node2)
{
    bool result;
...
void
Clipper::AddIntersectNode(TEdge* e1, TEdge* e2, const IntPoint& pt)
{
...
void
Clipper::DoMaxima(TEdge* e, long64 topY)
{
...
void
Clipper::ProcessEdgesAtTopOfScanbeam(const long64 topY)
{
    TEdge* e = m_ActiveEdges;
    while (e) {
        ...
        //   horizontal.
        if (IsMaxima(e, topY) &&
            !NEAR_EQUAL(GetMaximaPair(e)->dx, HORIZONTAL)) {
            ...
            if (IsIntermediate(e, topY) &&
                NEAR_EQUAL(e->nextInLML->dx, HORIZONTAL)) {
                if (e->outIdx >= 0) {
                    ...
                    for (HorzJoinList::size_type i = 0; i < m_HorizJoins.size();
                        ...
                        if (GetOverlapSegment(
                              ...
                              IntPoint(e->nextInLML->xtop, e->nextInLML->ytop),
                              pt,
                              pt2))
                            AddJoin(
    ...
    while (e) {
        if (IsIntermediate(e, topY)) {
            ...
            if (e->outIdx >= 0 && e->prevInAEL && e->prevInAEL->outIdx >= 0 &&
                e->prevInAEL->xcurr == e->xbot &&
                e->prevInAEL->ycurr == e->ybot &&
            ...
            } else if (e->outIdx >= 0 && e->nextInAEL &&
                       e->nextInAEL->outIdx >= 0 &&
                       e->nextInAEL->ycurr > e->nextInAEL->ytop &&
                       e->nextInAEL->ycurr < e->nextInAEL->ybot &&
                       e->nextInAEL->xcurr == e->xbot &&
                       e->nextInAEL->ycurr == e->ybot &&
                       SlopesEqual(
                         IntPoint(e->xbot, e->ybot),
                         IntPoint(e->xtop, e->ytop),
                         IntPoint(e->xbot, e->ybot),
...
void
Clipper::BuildResult(Polygons& polys)
{
    int k = 0;
...
void
Clipper::BuildResultEx(ExPolygons& polys)
{
...
bool
Clipper::FixupIntersections()
{
    if (!m_IntersectNodes->next)
        return true;
...
void
Clipper::InsertEdgeIntoAEL(TEdge* edge)
{
    edge->prevInAEL = 0;
    edge->nextInAEL = 0;
    if (!m_ActiveEdges) {
...
void
Clipper::DoEdge1(TEdge* edge1, TEdge* edge2, const IntPoint& pt)
{
...
void
Clipper::DoEdge2(TEdge* edge1, TEdge* edge2, const IntPoint& pt)
{
...
void
Clipper::DoBothEdges(TEdge* edge1, TEdge* edge2, const IntPoint& pt)
{
    AddOutPt(edge1, edge2, pt);
    AddOutPt(edge2, edge1, pt);
    SwapSides(*edge1, *edge2);
...
void
Clipper::CheckHoleLinkages1(OutRec* outRec1, OutRec* outRec2)
{
...
void
Clipper::CheckHoleLinkages2(OutRec* outRec1, OutRec* outRec2)
{
...
void
Clipper::JoinCommonEdges(bool fixHoleLinkages)
{
    for (JoinList::size_type i = 0; i < m_Joins.size(); i++) {
        JoinRec* j = m_Joins[i];
        ...
        if (!FindSegment(pp1a, pt1, pt2))
            continue;
        if (j->poly1Idx == j->poly2Idx) {
            // we're searching the same polygon for overlapping segments so
            // segment 2 mustn't be the same as segment 1 ...
            pp2a = pp1a->next;
            if (!FindSegment(pp2a, pt3, pt4) || (pp2a == pp1a))
                continue;
        } else if (!FindSegment(pp2a, pt3, pt4))
            continue;

        if (!GetOverlapSegment(pt1, pt2, pt3, pt4, pt1, pt2))
            continue;
...
struct DoublePoint
{
    float X;
    float Y;
    DoublePoint(float x = 0, float y = 0)
      : X(x)
      , Y(y)
    {}
};
...
Polygon
BuildArc(const IntPoint& pt, const float a1, const float a2, const float r)
{
    int steps =
      std::max(6, int(math::sqrt(math::fabs(r)) * math::fabs(a2 - a1)));
...
class PolyOffsetBuilder
{
  ...
  public:
    PolyOffsetBuilder(const Polygons& in_polys,
                      Polygons& out_polys,
                      float delta,
                      JoinType jointype,
                      float MiterLimit)
    {
        ...
        for (m_i = 0; m_i < in_polys.size(); m_i++) {
            ...
            if (delta < 0) {
                if (a1 > 0 && a1 < deltaSq * pi)
                    len = 0;
            } else if (a1 < 0 && -a1 < deltaSq * pi)
            ...
            normals.resize(len);
            normals[len - 1] =
              GetUnitNormal(in_polys[m_i][len - 1], in_polys[m_i][0]);
            for (m_j = 0; m_j < len - 1; ++m_j)
                normals[m_j] =
                  GetUnitNormal(in_polys[m_i][m_j], in_polys[m_i][m_j + 1]);
  ...
  private:
    ...
    {
        IntPoint pt1 =
          IntPoint((long64)Round(m_p[m_i][m_j].X + normals[m_k].X * m_delta),
                   (long64)Round(m_p[m_i][m_j].Y + normals[m_k].Y * m_delta));
        IntPoint pt2 =
          IntPoint((long64)Round(m_p[m_i][m_j].X + normals[m_j].X * m_delta),
                   (long64)Round(m_p[m_i][m_j].Y + normals[m_j].Y * m_delta));
        if ((normals[m_k].X * normals[m_j].Y -
             normals[m_j].X * normals[m_k].Y) *
              m_delta >=
            0) {
    ...
    void DoMiter()
    {
        if ((normals[m_k].X * normals[m_j].Y -
            ...
            float q = m_delta / m_R;
            AddPoint(
              IntPoint((long64)Round(m_p[m_i][m_j].X +
                                     (normals[m_k].X + normals[m_j].X) * q),
                       (long64)Round(m_p[m_i][m_j].Y +
                                     (normals[m_k].Y + normals[m_j].Y) * q)));
        } else {
            IntPoint pt1 = IntPoint(
              (long64)Round(m_p[m_i][m_j].X + normals[m_k].X * m_delta),
              (long64)Round(m_p[m_i][m_j].Y + normals[m_k].Y * m_delta));
            IntPoint pt2 = IntPoint(
              (long64)Round(m_p[m_i][m_j].X + normals[m_j].X * m_delta),
              (long64)Round(m_p[m_i][m_j].Y + normals[m_j].Y * m_delta));
            AddPoint(pt1);
            AddPoint(m_p[m_i][m_j]);
            AddPoint(pt2);
    ...
    void DoRound()
    {
        ...
        IntPoint pt2 =
          IntPoint((long64)Round(m_p[m_i][m_j].X + normals[m_j].X * m_delta),
                   (long64)Round(m_p[m_i][m_j].Y + normals[m_j].Y * m_delta));
        AddPoint(pt1);
        // round off reflex angles (ie > 180 deg) unless almost flat (ie <
        // ~10deg).
        if ((normals[m_k].X * normals[m_j].Y -
             normals[m_j].X * normals[m_k].Y) *
              m_delta >=
            0) {
            if (normals[m_j].X * normals[m_k].X +
                  normals[m_j].Y * normals[m_k].Y <
                0.985) {
                float a1 = math::atan2(normals[m_k].Y, normals[m_k].X);
                float a2 = math::atan2(normals[m_j].Y, normals[m_j].X);
...
void
OffsetPolygons(const Polygons& in_polys,
               Polygons& out_polys,
               float delta,
               JoinType jointype,
               float MiterLimit)
{
    if (&out_polys == &in_polys) {
        ...
        PolyOffsetBuilder(in_polys, out_polys, delta, jointype, MiterLimit);
}
//------------------------------------------------------------------------------
