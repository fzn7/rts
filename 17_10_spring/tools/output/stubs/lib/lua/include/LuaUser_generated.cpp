...
spring_lua_alloc(void* ud, void* ptr, size_t osize, size_t nsize)
{
    ...
    if ((nsize > osize) && (totalBytesAlloced > maxAllocedBytes)) {
        ...
        LOG_L(L_FATAL,
              maxAllocFmtStr,
              (lcd->owner->GetName()).c_str(),
              (unsigned int)totalBytesAlloced,
              maxAllocedBytes);
        return NULL;
    }
...
{
    return sprintf(dst, "%ld", x);
}
static inline int
sprintf64(char* dst, long long int x)
{
    return sprintf(dst, "%lld", x);
}
#endif
...
PrintIntPart(char* buf, float f, const bool carrierBit = false)
{
    ...
    if (f < (std::numeric_limits<int>::max() - carrierBit)) {
      ...
      if (f < (SPRING_INT64_MAX - carrierBit)) {
        return sprintf64(buf,
                         boost::int64_t(f) +
                           carrierBit); // much faster than printing a float!
    } else {
...
{
    // XXX: Hacks, with streflop enabled we limit the FPU normally to 32bit
    // float
    //     and doing any double math will use floats math then!
    //     But here we need the precision of doubles, so switch the FPU to it
    //     just for this casting.
...
spring_lua_ftoa(float f, char* buf, int precision)
{
    ...
    const bool carrierBit = HandleRounding(
      &fractF, log10, charsInStdNotation, nDigits, scienceNotation, precision);
