...
os_rename(lua_State* L)
{
    ...
    return os_pushresult(
      L, G(L)->rename_func(L, fromname, toname) == 0, fromname);
}
...
// SPRING
#ifndef BUILDING_AI
    // Lua's used clock() ran too fast on some linux systems, so rely on
    // spring_time instead
...
{
    lua_pushnumber(L,
                   difftime((time_t)(luaL_checknumber(L, 1)),
