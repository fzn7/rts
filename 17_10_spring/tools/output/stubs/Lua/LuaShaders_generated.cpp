...
static void
ParseUniformType(lua_State* L, int loc, int type)
{
    switch (type) {
        case UNIFORM_TYPE_FLOAT: {
            if (lua_israwnumber(L, -1)) {
                ...
                float array[32] = { 0.0f };
                const int count = LuaUtils::ParseFloatArray(
                  L, -1, array, sizeof(array) / sizeof(float));
                ...
                switch (count) {
                    ...
                    case 4: {
                        glUniform4f(
                          loc, array[0], array[1], array[2], array[3]);
                        break;
                    }
                    default: {
        ...
        case UNIFORM_TYPE_INT: {
            if (lua_israwnumber(L, -1)) {
                ...
                int array[32] = { 0 };
                const int count = LuaUtils::ParseIntArray(
                  L, -1, array, sizeof(array) / sizeof(int));
...
ParseUniformSetupTables(lua_State* L, int index, GLuint progName)
{
    ...
    const bool success =
      ...
      ParseUniformsTable(
        L, "uniformFloat", index, UNIFORM_TYPE_FLOAT, progName) &&
      ParseUniformsTable(L, "uniformInt", index, UNIFORM_TYPE_INT, progName) &&
      ParseUniformsTable(
        L, "uniformMatrix", index, UNIFORM_TYPE_FLOAT_MATRIX, progName);
              ...
              bool& success)
{
    ...
    if (result != GL_TRUE) {
        if (shaders.errorLog.empty()) {
            shaders.errorLog =
              "Empty error message:  code = " + IntToString(result) + " (0x" +
                 ...
                 std::vector<std::string>& data)
{
    ...
    LuaShaders& shaders = CLuaHandle::GetActiveShaders(L);
    shaders.errorLog =
      "\"" + string(key) + "\" must be a string or a table value!";
...
ApplyGeometryParameters(lua_State* L, int table, GLuint prog)
{
    ...
    struct
    {
        const char* name;
        GLenum param;
    } parameters[] = { { "geoInputType", GL_GEOMETRY_INPUT_TYPE_EXT },
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (lua_israwnumber(L, -1)) {
            glProgramParameteriEXT(
              prog, parameters[i].param, /*type*/ lua_toint(L, -1));
        }
        lua_pop(L, 1);
    }
}
...
LuaShaders::CreateShader(lua_State* L)
{
    ...
    bool success;
    const GLuint vertObj =
      CompileObject(L, shdrDefs, vertSrcs, GL_VERTEX_SHADER, success);

    if (!success)
        return 0;

    const GLuint geomObj =
      CompileObject(L, shdrDefs, geomSrcs, GL_GEOMETRY_SHADER_EXT, success);
    ...
    const GLuint fragObj =
      CompileObject(L, shdrDefs, fragSrcs, GL_FRAGMENT_SHADER, success);
...
LuaShaders::GetActiveUniforms(lua_State* L)
{
    ...
    for (GLint i = 0; i < uniformCount; i++) {
        ...
        GLchar name[1024];
        glGetActiveUniform(
          progName, i, sizeof(name), &length, &size, &type, name);
...
LuaShaders::UniformArray(lua_State* L)
{
    ...
    switch (luaL_checkint(L, 2)) {
        case UNIFORM_TYPE_INT: {
            int array[32] = { 0 };
            const int count =
              LuaUtils::ParseIntArray(L, 3, array, sizeof(array) / sizeof(int));
        ...
        case UNIFORM_TYPE_FLOAT: {
            float array[32] = { 0.0f };
            const int count = LuaUtils::ParseFloatArray(
              L, 3, array, sizeof(array) / sizeof(float));
