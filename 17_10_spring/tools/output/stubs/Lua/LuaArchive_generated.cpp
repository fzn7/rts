...
LuaArchive::HasArchive(lua_State* L)
{
    ...
    CArchiveScanner::ArchiveData archiveData =
      archiveScanner->GetArchiveData(archiveName);
...
{
    const std::string archiveName = luaL_checksstring(L, 1);
    const unsigned int checksumSingl =
      archiveScanner->GetSingleArchiveChecksum(archiveName);
    const unsigned int checksumAccum =
      archiveScanner->GetArchiveCompleteChecksum(archiveName);
...
LuaArchive::GetAvailableAIs(lua_State* L)
{
        ...
        lua_newtable(L);
        {
            for (int j = 0; j < luaAIInfos[i].size(); j++) {
                if (luaAIInfos[i][j].key == SKIRMISH_AI_PROPERTY_SHORT_NAME) {
                    HSTR_PUSH_STRING(
                      L, "shortName", luaAIInfos[i][j].GetValueAsString());
                } else if (luaAIInfos[i][j].key ==
                           SKIRMISH_AI_PROPERTY_VERSION) {
                    HSTR_PUSH_STRING(
                      L, "version", luaAIInfos[i][j].GetValueAsString());
                }
            }
        }
        lua_rawseti(L, -2, ++count);
    ...
    IAILibraryManager::T_skirmishAIKeys::const_iterator i =
      skirmishAIKeys.begin();
    IAILibraryManager::T_skirmishAIKeys::const_iterator e =
      skirmishAIKeys.end();
