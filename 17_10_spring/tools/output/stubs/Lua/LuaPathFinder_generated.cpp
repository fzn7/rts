...
bool
LuaPathFinder::PushEntries(lua_State* L)
{
...
path_next(lua_State* L)
{
    ...
    const float3 point =
      pathManager->NextWayPoint(NULL, pathID, 0, callerPos, minDist, synced);
...
LuaPathFinder::RequestPath(lua_State* L)
{
    ...
    const bool synced = CLuaHandle::GetHandleSynced(L);
    const int pathID =
      pathManager->RequestPath(NULL, moveDef, start, end, radius, synced);
...
LuaPathFinder::InitPathNodeCostsArray(lua_State* L)
{
    ...
    std::map<unsigned int, NodeCostOverlay>& map =
      synced ? costArrayMapSynced : costArrayMapUnsynced;
...
LuaPathFinder::SetPathNodeCosts(lua_State* L)
{
    ...
    lua_pushboolean(L,
                    pathManager->SetNodeExtraCosts(
                      &overlay.costs[0], overlay.sizex, overlay.sizez, synced));
    return 1;
}
...
LuaPathFinder::SetPathNodeCost(lua_State* L)
{
    ...
    if (overlay.Empty()) {
        ...
        // lua_pushboolean(L, pathManager->SetNodeExtraCost(hmx, hmz, cost,
        // synced));
        lua_pushboolean(L, false);
    } else {
