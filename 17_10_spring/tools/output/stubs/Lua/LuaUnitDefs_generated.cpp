...
LuaUnitDefs::PushEntries(lua_State* L)
{
    ...
    const std::array<const IndxFuncType, 3> indxFuncs = { UnitDefIndex,
                                                          UnitDefNewIndex,
                                                          UnitDefMetatable };
    ...
    for (auto it = defsMap.cbegin(); it != defsMap.cend(); ++it) {
        ...
        PushObjectDefProxyTable(
          L, indxOpers, iterOpers, indxFuncs, iterFuncs, def);
    }
...
UnitDefIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in UnitDefs __index",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
UnitDefNewIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in UnitDefs __newindex",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
WeaponsTable(lua_State* L, const void* data)
{
    ...
    for (size_t i = 0; i < weapons.size(); i++) {
        ...
        lua_newtable(L);
        {
            ...
            HSTR_PUSH_NUMBER(
              L, "slavedTo", udw.slavedTo - 1 + LUA_WEAPON_BASE_INDEX);
...
MoveDefTable(lua_State* L, const void* data)
{
    ...
    switch (md->speedModClass) {
        case MoveDef::Tank: {
            HSTR_PUSH_STRING(L, "family", "tank");
            HSTR_PUSH_STRING(L, "type", "ground");
            break;
        }
        case MoveDef::KBot: {
            HSTR_PUSH_STRING(L, "family", "kbot");
            HSTR_PUSH_STRING(L, "type", "ground");
            break;
        }
        case MoveDef::Hover: {
            HSTR_PUSH_STRING(L, "family", "hover");
            HSTR_PUSH_STRING(L, "type", "hover");
            break;
        }
        case MoveDef::Ship: {
            HSTR_PUSH_STRING(L, "family", "ship");
            HSTR_PUSH_STRING(L, "type", "ship");
            break;
        }
    }
    ...
    HSTR_PUSH_NUMBER(
      L, "depthMod", md->depthModParams[MoveDef::DEPTHMOD_LIN_COEFF]);
...
{
    return (
      LuaUtils::PushModelTable(L, static_cast<const SolidObjectDef*>(data)));
}
...
{
    return (
      LuaUtils::PushModelName(L, static_cast<const SolidObjectDef*>(data)));
}

static int
ModelHeight(lua_State* L, const void* data)
{
    return (LuaUtils::PushModelHeight(
      L, static_cast<const SolidObjectDef*>(data), true));
}

static int
ModelRadius(lua_State* L, const void* data)
{
    return (LuaUtils::PushModelRadius(
      L, static_cast<const SolidObjectDef*>(data), true));
}
...
{
    return (
      LuaUtils::PushColVolTable(L, static_cast<const CollisionVolume*>(data)));
}
...
InitParamMap()
{
    ...
    */
    ...
    //	unsigned char* yardmaps[4];			//Iterations of the Ymap for building
    //rotation
