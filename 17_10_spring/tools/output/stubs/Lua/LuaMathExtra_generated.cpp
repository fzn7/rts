...
{
    lua_pushnumber(L,
                   math::hypot(luaL_checknumber_noassert(L, 1),
...
LuaMathExtra::round(lua_State* L)
{
    ...
    if (lua_gettop(L) > 1) {
        ...
        const int i =
          std::min(7, int(sizeof(POWERS_OF_TEN) / sizeof(float)) - 1);
        ...
        lua_pushnumber(L,
                       xinteg +
                         math::floor((xfract * POWERS_OF_TEN[n]) + 0.5f) /
                           POWERS_OF_TEN[n]);
    } else {
...
{
    lua_pushnumber(L,
                   ::smoothstep(luaL_checkfloat(L, 1),
                                luaL_checkfloat(L, 2),
                                luaL_checkfloat(L, 3)));
    return 1;
}
