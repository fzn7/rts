...
Heightmap::UpdateLowerUnsynced(int sx, int sy, int w, int h)
{
    ...
    for (int x = sx / 2; x < (sx + w) / 2; x++)
        for (int y = sy / 2; y < (sy + h) / 2; y++) {
            lowDetail->dataSynced[y * lowDetail->w + x] =
              atSynced(x * 2, y * 2);
            lowDetail->dataUnsynced[y * lowDetail->w + x] =
              atUnsynced(x * 2, y * 2);
        }
    ...
    for (int y = ystart; y < QUAD_W; y++) {
        if (sides & left_bit) {
            if (y & 1) {
                ...
                if (y < yend)
                    TRI(VRT(1, y), VRT(1, y + 1), VRT(0, y + 1));
            } else {
        ...
        if ((y >= ystart) && (y < yend)) {
            for (int x = xstart; x < xend; x++) {
                ...
                TRI(
                  y * VERTC + x, (y + 1) * VERTC + x + 1, (y + 1) * VERTC + x);
            }
        }
        ...
        if (sides & right_bit) {
            ...
            if (y & 1) {
                ...
                if (y < yend)
                    TRI(VRT(x, y), VRT(x + 1, y + 1), VRT(x, y + 1));
            } else {
    ...
    if (sides & down_bit) {
        ...
        for (int x = xstart; x < QUAD_W; x++) {
            if (x & 1) {
                ...
                if (x < xend)
                    TRI(VRT(x, y), VRT(x + 1, y), VRT(x + 1, y + 1));
            } else {
                TRI(VRT(x, y), VRT(x + 1, y), VRT(x, y + 1));
            }
        }
...
TextureUsage::AddTextureCoordRead(int maxCoords, BaseTexture* texture)
{
    if (texture->ShareTexCoordUnit()) {
        for (size_t a = 0; a < coordUnits.size(); a++) {
            ...
            if (t == texture)
                return (int)a;
            if (t->ShareTexCoordUnit() &&
                (t->tilesize.x == texture->tilesize.x) &&
                (t->tilesize.y == texture->tilesize.y))
                return (int)a;
        }
    } else {
        for (size_t a = 0; a < coordUnits.size(); a++)
            if (coordUnits[a] == texture)
                return (int)a;
    }
