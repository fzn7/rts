...
CReadMap::LoadMap(const std::string& mapname)
{
    ...
    if (FileSystem::GetExtension(mapname) == "sm3") {
        throw content_error("[CReadMap::LoadMap] SM3 maps are no longer "
                            "supported as of Spring 95.0");
    ...
    rm->metalMap =
      new CMetalMap(metalmapPtr, mbi.width, mbi.height, mapInfo->map.maxMetal);

    if (metalmapPtr != nullptr)
        rm->FreeInfoMap("metal", metalmapPtr);

    if (typemapPtr && tbi.width == (mapDims.mapx >> 1) &&
        tbi.height == (mapDims.mapy >> 1)) {
...
CReadMap::Serialize(creg::ISerializer* s)
{
    ...
    const int* ioshm =
      reinterpret_cast<const int*>(GetOriginalHeightMapSynced());
...
CReadMap::Initialize()
{
    ...
    boundingRadius = math::sqrt(Square(mapDims.mapx * SQUARE_SIZE) +
                                Square(mapDims.mapy * SQUARE_SIZE)) *
                     0.5f;

    {
        ...
        unsigned int reqMemFootPrintKB =
          (((mapDims.mapxp1) * mapDims.mapyp1 * 2 * sizeof(float)) /
           1024) + // cornerHeightMap{Synced, Unsynced}
          (((mapDims.mapxp1) * mapDims.mapyp1 * sizeof(float)) /
           1024) + // originalHeightMap
          ((mapDims.mapx * mapDims.mapy * 2 * 2 * sizeof(float3)) /
           1024) + // faceNormals{Synced, Unsynced}
          ((mapDims.mapx * mapDims.mapy * 2 * sizeof(float3)) /
           1024) + // centerNormals{Synced, Unsynced}
          (((mapDims.mapxp1) * mapDims.mapyp1 * sizeof(float3)) /
           1024) + // VisVertexNormals
          ((mapDims.mapx * mapDims.mapy * sizeof(float)) /
           1024) + // centerHeightMap
          ((mapDims.hmapx * mapDims.hmapy * sizeof(float)) / 1024) + // slopeMap
          ((mapDims.hmapx * mapDims.hmapy * sizeof(float)) /
           1024) + // MetalMap::extractionMap
          ((mapDims.hmapx * mapDims.hmapy * sizeof(unsigned char)) /
           1024); // MetalMap::metalMap

        // mipCenterHeightMaps[i]
        for (int i = 1; i < numHeightMipMaps; i++) {
            reqMemFootPrintKB +=
              ((((mapDims.mapx >> i) * (mapDims.mapy >> i)) * sizeof(float)) /
               1024);
        }
    ...
    for (int i = 1; i < numHeightMipMaps; i++) {
        mipCenterHeightMaps[i - 1].resize((mapDims.mapx >> i) *
                                          (mapDims.mapy >> i));
        mipPointerHeightMaps[i] = &mipCenterHeightMaps[i - 1][0];
    }
    ...
    // FIXME can't call that yet cause sky & skyLight aren't created yet
    // (crashes in SMFReadMap.cpp) UpdateDraw(true);
}
...
CReadMap::CalcHeightmapChecksum()
{
    ...
    checksum =
      HsiehHash(mapInfo->map.name.c_str(), mapInfo->map.name.size(), checksum);
...
CReadMap::CalcTypemapChecksum()
{
    unsigned int checksum = 0;
    checksum =
      HsiehHash(&typeMap[0], typeMap.size() * sizeof(typeMap[0]), checksum);
    ...
    for (unsigned int i = 0; i < CMapInfo::NUM_TERRAIN_TYPES; i++) {
        ...
        checksum = HsiehHash(&tt.hardness,
                             offsetof(CMapInfo::TerrainType, receiveTracks) -
                               offsetof(CMapInfo::TerrainType, hardness),
                             checksum);
    }
...
CReadMap::UpdateDraw(bool firstCall)
{
    ...
    {
        if (!unsyncedHeightMapUpdates.empty())
            unsyncedHeightMapUpdates.swap(
              unsyncedHeightMapUpdatesTemp); // swap to avoid Optimize() inside
                                             // a mutex
    }
    {
        if (!firstCall) {
            if (!unsyncedHeightMapUpdatesTemp.empty()) {
                ...
                int updateArea =
                  unsyncedHeightMapUpdatesTemp.GetTotalArea() * 0.0625f +
                  (50 * 50);

                while (updateArea > 0 &&
                       !unsyncedHeightMapUpdatesTemp.empty()) {
                    const SRectangle& rect =
                      unsyncedHeightMapUpdatesTemp.front();
    ...
    if (!unsyncedHeightMapUpdatesTemp.empty())
        unsyncedHeightMapUpdates.splice(unsyncedHeightMapUpdates.end(),
                                        unsyncedHeightMapUpdatesTemp);
...
CReadMap::UpdateHeightMapSynced(SRectangle rect, bool initialize)
{
    ...
    if (initialize) {
        ...
        const int losSquaresX =
          losHandler->los.size.x; // size of LOS square in heightmap coords
        const SRectangle& lm =
          rect * (SQUARE_SIZE * losHandler->los.invDiv); // LOS space

        // we updated the heightmap so change their digest (byte-overflow is
        // intentional!)
...
CReadMap::UpdateCenterHeightmap(const SRectangle& rect, bool initialize)
{
    ...
    for (int y = rect.z1; y <= rect.z2; y++) {
        for (int x = rect.x1; x <= rect.x2; x++) {
            ...
            const float height =
              heightmapSynced[idxTL] + heightmapSynced[idxTR] +
              heightmapSynced[idxBL] + heightmapSynced[idxBR];
    ...
    for (int i = 0; i < numHeightMipMaps - 1; i++) {
        ...
        for (int y = sy; y < ey; y += 2) {
            for (int x = sx; x < ex; x += 2) {
                const float height = topMipMap[(x) + (y)*hmapx] +
...
CReadMap::UpdateFaceNormals(const SRectangle& rect, bool initialize)
{
    ...
    for_mt(z1, z2 + 1, [&](const int y) {
        ...
        for (int x = x1; x <= x2; x++) {
            ...
            // square-normal
            centerNormalsSynced[y * mapDims.mapx + x] =
              (fnTL + fnBR).Normalize();
            centerNormals2DSynced[y * mapDims.mapx + x] =
              (fnTL + fnBR).Normalize2D();

#ifdef USE_UNSYNCED_HEIGHTMAP
            if (initialize) {
                faceNormalsUnsynced[(y * mapDims.mapx + x) * 2] =
                  faceNormalsSynced[(y * mapDims.mapx + x) * 2];
                faceNormalsUnsynced[(y * mapDims.mapx + x) * 2 + 1] =
                  faceNormalsSynced[(y * mapDims.mapx + x) * 2 + 1];
                centerNormalsUnsynced[y * mapDims.mapx + x] =
                  centerNormalsSynced[y * mapDims.mapx + x];
                centerNormals2DUnsynced[y * mapDims.mapx + x] =
                  centerNormals2DSynced[y * mapDims.mapx + x];
            }
#endif
        }
...
CReadMap::UpdateSlopemap(const SRectangle& rect, bool initialize)
{
    ...
    for (int y = sy; y <= ey; y++) {
        for (int x = sx; x <= ex; x++) {
            ...
            maxslope =
              std::min(maxslope, faceNormalsSynced[(idx0 + 1) * 2 + 1].y);
            ...
            maxslope =
              std::min(maxslope, faceNormalsSynced[(idx1 + 1) * 2 + 1].y);
...
{
    InitHeightMapDigestsVectors();
    const int losSqSize = losHandler->los.divisor /
                          SQUARE_SIZE; // size of LOS square in heightmap coords
    const SRectangle& lm =
      rect * (SQUARE_SIZE * losHandler->los.invDiv); // LOS space
...
#define PUSH_RECT                                                              \
    if (subrect.GetArea() > 0) {                                               \
        ...
        subrect =                                                              \
          SRectangle(hmx + losSqSize, hmz, hmx + losSqSize, hmz + losSqSize);  \
    } else {                                                                   \
        ...
        for (int lmx = lm.x1; lmx <= lm.x2; ++lmx) {
            ...
            if (!(gu->spectatingFullView ||
                  losHandler->InLos(SquareToFloat3(hmx, hmz),
                                    gu->myAllyTeam))) {
                PUSH_RECT
                continue;
            }
...
CReadMap::InitHeightMapDigestsVectors()
{
    ...
    if (syncedHeightMapDigests.empty()) {
        const int size =
          (losHandler->los.size.x + 1) * (losHandler->los.size.y + 1);
...
CReadMap::UpdateLOS(const SRectangle& rect)
{
    ...
    // currently we use the LOS for view updates (alternatives are AirLOS and/or
    // radar) cause the others use different resolutions we must check it here
    // for safety (if you want to use another source you need to change the res.
    // of syncedHeightMapDigests etc.)
    assert(rect.GetWidth() <= losHandler->los.divisor / SQUARE_SIZE);

    // HACK: UpdateLOS() is called for single LOS squares, but we use <= in
    // HeightMapUpdateLOSCheck().
    // This would make our update area 4x as large, so we need to make the
    // rectangle a point. Better would be to use < instead of <= everywhere.
    SRectangle r = rect;
    r.x2 = r.x1;
    r.z2 = r.z1;
...
{
    return (!mapInfo->map.voidWater && !IsAboveWater());
}
bool
CReadMap::HasOnlyVoidWater() const
{
    return (mapInfo->map.voidWater && IsUnderWater());
}
