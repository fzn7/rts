  ...
  , featureFileOffset(0)
{
    ...
    if (strcmp(header.magic, "spring map file") != 0 || header.version != 1 ||
        header.tilesize != 32 || header.texelPerSquare != 8 ||
        header.squareSize != 8)
        throw content_error("Incorrect map file " + mapFileName);
}

void
CSMFMapFile::ReadMinimap(void* data)
{
...
int
CSMFMapFile::ReadMinimap(std::vector<boost::uint8_t>& data, unsigned miplevel)
{
    int offset = 0;
    int mipsize = 1024;
    for (unsigned i = 0; i < std::min((unsigned)MINIMAP_NUM_MIPMAP, miplevel);
         i++) {
...
void
CSMFMapFile::ReadHeightmap(unsigned short* heightmap)
{
...
void
CSMFMapFile::ReadHeightmap(float* sHeightMap,
                           float* uHeightMap,
                           float base,
                           float mod)
{
...
void
CSMFMapFile::ReadFeatureInfo(MapFeatureInfo* f)
{
...
const char*
CSMFMapFile::GetFeatureTypeName(int typeID) const
{
...
void
CSMFMapFile::GetInfoMapSize(const string& name, MapBitmapInfo* info) const
{
    if (name == "height") {
...
bool
CSMFMapFile::ReadInfoMap(const string& name, void* data)
{
    if (name == "height") {
...
bool
CSMFMapFile::ReadGrassMap(void* data)
{
...
void
CSMFMapFile::ReadMapHeader(SMFHeader& head, CFileHandler& file)
{
...
void
CSMFMapFile::ReadMapFeatureHeader(MapFeatureHeader& head, CFileHandler& file)
{
...
void
CSMFMapFile::ReadMapFeatureStruct(MapFeatureStruct& head, CFileHandler& file)
{
...
void
CSMFMapFile::ReadMapTileHeader(MapTileHeader& head, CFileHandler& file)
{
...
void
CSMFMapFile::ReadMapTileFileHeader(TileFileHeader& head, CFileHandler& file)
{
    file.Read(&head.magic, sizeof(head.magic));
    head.version = ReadInt(file);
    head.numTiles = ReadInt(file);
