...
static inline float
InterpolateHeight(float x, float z, const float* heightmap)
{
    // NOTE:
...
static inline float
LineGroundSquareCol(const float* heightmap,
                    ...
                    const int ys)
{
    const bool inMap = (xs >= 0) && (ys >= 0) && (xs <= mapDims.mapxm1) &&
                       (ys <= mapDims.mapym1);
    //	assert(inMap);
    if (!inMap)
        return -1.0f;
    ...
    // top-left corner vertex
    {
        ...
        if (toFacePlaneDist <= 0.0f) {
            ...
            if (fromFacePlaneDist != toFacePlaneDist) {
                const float alpha =
                  fromFacePlaneDist / (fromFacePlaneDist - toFacePlaneDist);
                const float3 col = mix(from, to, alpha);

                if ((col.x >= cornerVertex.x) && (col.z >= cornerVertex.z) &&
                    (col.x + col.z <=
                     cornerVertex.x + cornerVertex.z + SQUARE_SIZE)) {
...
inline static bool
ClampInMapHeight(float3& from, float3& to)
{
...
float
CGround::LineGroundCol(float3 from, float3 to, bool synced)
{
    ...
    if (synced) { // TODO do this in unsynced too once the map border rendering
                  // is finished?
        // check if our start position is underground (assume ground is
        // unpassable for cannons etc.)
    ...
    // Clamping is done cause LineGroundSquareCol() operates on the 2 triangles
    // faces each heightmap square is formed of.
    ...
    if ((fsx == tsx) && (fsz == tsz)) {
        ...
        // you can set epsilon=0 and then handle the `beyond end`-case (xn
        // >= 1.0f && zn >= 1.0f) separate (we already need to do so cause of
        // floating point precision limits, so skipping epsilon doesn't add any
        // additional performance cost nor precision issue)
        //
        // case : dir>0
        ...
        while (keepgoing) {
            ...
            const bool beyondEnd =
              ((curx - tsx) * dirx > 0) || ((curz - tsz) * dirz > 0);
            ...
            // calculate the `normalized position` of the next edge in x & z
            // direction
            //  `normalized position`:=n :   x = from.x + n * (to.x - from.x)
            //  (with 0<= n <=1)
            int nextx = curx + dirx;
            int nextz = curz + dirz;
            ...
            // advance to the next nearest edge in either x or z dir, or in the
            // case we reached the end make sure we set it to the exact square
            // positions (floating point precision sometimes hinders us to hit
            // it)
            if (xn >= 1.0f && zn >= 1.0f) {
...
float
CGround::LineGroundCol(const float3 pos,
                       const float3 dir,
                       float len,
                       bool synced)
{
...
float
CGround::LinePlaneCol(const float3 pos, const float3 dir, float len, float hgt)
{
...
float
CGround::GetApproximateHeight(float x, float z, bool synced)
{
...
float
CGround::GetApproximateHeightUnsafe(int x, int z, bool synced)
{
    const float* heightMap = readMap->GetSharedCenterHeightMap(synced);
...
float
CGround::GetHeightAboveWater(float x, float z, bool synced)
{
    return std::max(0.0f, GetHeightReal(x, z, synced));
}

float
CGround::GetHeightReal(float x, float z, bool synced)
{
    return InterpolateHeight(x, z, readMap->GetSharedCornerHeightMap(synced));
}

float
CGround::GetOrigHeight(float x, float z)
{
    return InterpolateHeight(x, z, readMap->GetOriginalHeightMapSynced());
}

const float3&
CGround::GetNormal(float x, float z, bool synced)
{
    const int xsquare = Clamp(int(x) / SQUARE_SIZE, 0, mapDims.mapxm1);
    const int zsquare = Clamp(int(z) / SQUARE_SIZE, 0, mapDims.mapym1);
...
const float3&
CGround::GetNormalAboveWater(float x, float z, bool synced)
{
...
float
CGround::GetSlope(float x, float z, bool synced)
{
    const int xhsquare =
      Clamp(int(x) / (2 * SQUARE_SIZE), 0, mapDims.hmapx - 1);
    const int zhsquare =
      Clamp(int(z) / (2 * SQUARE_SIZE), 0, mapDims.hmapy - 1);
...
float3
CGround::GetSmoothNormal(float x, float z, bool synced)
{
    const int sx =
      Clamp(int(math::floor(x / SQUARE_SIZE)), 1, mapDims.mapx - 2);
    const int sz =
      Clamp(int(math::floor(z / SQUARE_SIZE)), 1, mapDims.mapy - 2);
...
float
CGround::TrajectoryGroundCol(float3 from,
                             ...
                             float quadratic)
{
    ...
    std::pair<float, float> near_far =
      GetMapBoundaryIntersectionPoints(from, dir * length);
...
int
CGround::GetSquare(const float3& pos)
{
