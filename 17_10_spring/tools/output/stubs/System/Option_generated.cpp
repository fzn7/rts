                   ...
                   std::set<string>& optionsSet)
{
    ...
    if (!optTbl.IsValid()) {
        throw content_error("parseOption: subtable " + IntToString(index) +
                            " is invalid");
    }
    ...
    if (opt.key.empty() ||
        ...
        throw content_error("parseOption: (key=\"" + opt.key +
                            "\") empty key or key contains bad characters (\"" +
                            Option_badKeyChars + "\")");
    }
    ...
    if (opt.type == "bool") {
        ...
        for (int i = 1; listTbl.KeyExists(i); i++) {
            ...
            if (!itemTbl.IsValid()) {
                throw content_error("parseOption: (key=" + opt.key +
                                    ") subtables: subtable " + IntToString(i) +
            ...
            if (item.key.empty() ||
                (item.key.find_first_of(Option_badKeyChars) != string::npos)) {
                throw content_error(
                  "parseOption: (key=" + opt.key + ") subtables: (key=\"" +
                  item.key +
                  "\") empty key or key contains bad characters (\"" +
                  Option_badKeyChars + "\")");
            }
                             ...
                             std::set<std::string>* optionsSet)
{
    ...
    option_parseOptionsInternal(
      options, luaParser, "<Lua-Text-Chunk>", optionsSet);
}
                       ...
                       std::set<std::string>* optionsSet)
{
    ...
    if (configName.empty())
        throw "Could not determine config-file name from the map name '" +
          mapName + "'!";
