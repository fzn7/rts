...
bool
CAIScriptHandler::IsSkirmishAITestScript(const std::string& scriptName) const
{
...
CAIScriptHandler::GetSkirmishAIData(const std::string& scriptName) const
{
    const ScriptMap::const_iterator scriptsIt = scripts.find(scriptName);

    if (scriptsIt == scripts.end()) {
        throw std::runtime_error("start-script \"" + scriptName +
                                 "\" does not exist");
    }

    const CSkirmishAIScript* aiScript =
      dynamic_cast<const CSkirmishAIScript*>(scriptsIt->second);

    if (aiScript == NULL) {
        throw std::runtime_error("start-script \"" + scriptName +
                                 "\" is not a CSkirmishAIScript");
    }
...
void
CAIScriptHandler::Add(CScript* script)
{
