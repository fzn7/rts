...
void
CEventHandler::SetupEvent(const string& eName, EventClientList* list, int props)
{
...
void
CEventHandler::AddClient(CEventClient* ec)
{
...
void
CEventHandler::RemoveClient(CEventClient* ec)
{
    if (mouseOwner == ec) {
        mouseOwner = NULL;
    }
...
void
CEventHandler::GetEventList(vector<string>& list) const
{
    list.clear();
...
bool
CEventHandler::IsKnown(const string& eName) const
{
    return (eventMap.find(eName) != eventMap.end());
}

bool
CEventHandler::IsManaged(const string& eName) const
{
...
bool
CEventHandler::IsUnsynced(const string& eName) const
{
    const EventMap::const_iterator it = eventMap.find(eName);
...
bool
CEventHandler::IsController(const string& eName) const
{
    const EventMap::const_iterator it = eventMap.find(eName);
...
bool
CEventHandler::InsertEvent(CEventClient* ec, const string& ciName)
{
...
bool
CEventHandler::RemoveEvent(CEventClient* ec, const string& ciName)
{
    EventMap::iterator it = eventMap.find(ciName);
    if ((it == eventMap.end()) || (it->second.GetList() == NULL)) {
        return false;
...
void
CEventHandler::ListInsert(EventClientList& ecList, CEventClient* ec)
{
...
void
CEventHandler::ListRemove(EventClientList& ecList, CEventClient* ec)
{
...
bool
CEventHandler::CommandFallback(const CUnit* unit, const Command& cmd)
{
    CONTROL_ITERATE_DEF_TRUE(CommandFallback, unit, cmd)
}

bool
CEventHandler::AllowCommand(const CUnit* unit,
                            const Command& cmd,
                            bool fromSynced)
{
...
bool
CEventHandler::AllowUnitCreation(const UnitDef* unitDef,
                                 const CUnit* builder,
...
bool
CEventHandler::AllowUnitTransfer(const CUnit* unit, int newTeam, bool capture)
{
    CONTROL_ITERATE_DEF_TRUE(AllowUnitTransfer, unit, newTeam, capture)
}

bool
CEventHandler::AllowUnitBuildStep(const CUnit* builder,
                                  const CUnit* unit,
                                  float part)
{
...
bool
CEventHandler::AllowFeatureCreation(const FeatureDef* featureDef,
                                    int allyTeamID,
                                    const float3& pos)
{
...
bool
CEventHandler::AllowFeatureBuildStep(const CUnit* builder,
...
bool
CEventHandler::AllowResourceLevel(int teamID, const string& type, float level)
{
    CONTROL_ITERATE_DEF_TRUE(AllowResourceLevel, teamID, type, level)
}

bool
CEventHandler::AllowResourceTransfer(int oldTeam,
                                     int newTeam,
                                     const string& type,
                                     float amount)
{
    CONTROL_ITERATE_DEF_TRUE(
      AllowResourceTransfer, oldTeam, newTeam, type, amount)
}

bool
CEventHandler::AllowDirectUnitControl(int playerID, const CUnit* unit)
{
    CONTROL_ITERATE_DEF_TRUE(AllowDirectUnitControl, playerID, unit)
}

bool
CEventHandler::AllowBuilderHoldFire(const CUnit* unit, int action)
{
    CONTROL_ITERATE_DEF_TRUE(AllowBuilderHoldFire, unit, action)
}

bool
CEventHandler::AllowStartPosition(int playerID,
                                  unsigned char readyState,
                                  const float3& clampedPos,
                                  const float3& rawPickPos)
{
    CONTROL_ITERATE_DEF_TRUE(
      AllowStartPosition, playerID, readyState, clampedPos, rawPickPos)
}
...
bool
CEventHandler::MoveCtrlNotify(const CUnit* unit, int data)
{
    CONTROL_ITERATE_DEF_FALSE(MoveCtrlNotify, unit, data)
}

int
CEventHandler::AllowWeaponTargetCheck(unsigned int attackerID,
                                      ...
                                      unsigned int attackerWeaponDefID)
{
    ...
    for (int i = 0; i < listAllowWeaponTargetCheck.size();) {
        ...
        int result2 = ec->AllowWeaponTargetCheck(
          attackerID, attackerWeaponNum, attackerWeaponDefID);
        if (result2 > result)
            result = result2;
        if (i < listAllowWeaponTargetCheck.size() &&
            ec == listAllowWeaponTargetCheck[i])
            ++i; /* the call-in may remove itself from the list */
    }
    return result;
}
...
bool
CEventHandler::AllowWeaponTarget(unsigned int attackerID,
                                 unsigned int targetID,
                                 unsigned int attackerWeaponNum,
                                 unsigned int attackerWeaponDefID,
                                 float* targetPriority)
{
    CONTROL_ITERATE_DEF_TRUE(AllowWeaponTarget,
                             attackerID,
                             targetID,
                             attackerWeaponNum,
                             attackerWeaponDefID,
...
bool
CEventHandler::AllowWeaponInterceptTarget(const CUnit* interceptorUnit,
                                          const CWeapon* interceptorWeapon,
                                          const CProjectile* interceptorTarget)
{
    CONTROL_ITERATE_DEF_TRUE(AllowWeaponInterceptTarget,
                             interceptorUnit,
                             interceptorWeapon,
                             interceptorTarget)
}
...
bool
CEventHandler::UnitPreDamaged(const CUnit* unit,
                              ...
                              float* impulseMult)
{
    CONTROL_ITERATE_DEF_FALSE(UnitPreDamaged,
                              unit,
                              attacker,
                              damage,
                              weaponDefID,
...
bool
CEventHandler::FeaturePreDamaged(const CFeature* feature,
                                 const CUnit* attacker,
                                 float damage,
                                 int weaponDefID,
                                 int projectileID,
                                 ...
                                 float* impulseMult)
{
    CONTROL_ITERATE_DEF_FALSE(FeaturePreDamaged,
                              feature,
                              attacker,
                              damage,
                              weaponDefID,
...
bool
CEventHandler::ShieldPreDamaged(const CProjectile* projectile,
...
{
    CONTROL_ITERATE_DEF_FALSE(ShieldPreDamaged,
                              projectile,
                              shieldEmitter,
                              shieldCarrier,
                              bounceProjectile,
...
bool
CEventHandler::SyncedActionFallback(const string& line, int playerID)
{
    for (int i = 0; i < listSyncedActionFallback.size();) {
        ...
        if (ec->SyncedActionFallback(line, playerID))
            return true;
        if (i < listSyncedActionFallback.size() &&
            ec == listSyncedActionFallback[i])
            ++i; /* the call-in may remove itself from the list */
    }
    return false;
}
...
void
CEventHandler::Save(zipFile archive)
{
    ITERATE_EVENTCLIENTLIST(Save, archive);
}

void
CEventHandler::GamePreload()
{
    ITERATE_EVENTCLIENTLIST(GamePreload);
}

void
CEventHandler::GameStart()
{
    ITERATE_EVENTCLIENTLIST(GameStart);
}

void
CEventHandler::GameOver(const std::vector<unsigned char>& winningAllyTeams)
{
    ITERATE_EVENTCLIENTLIST(GameOver, winningAllyTeams);
}

void
CEventHandler::GamePaused(int playerID, bool paused)
{
    ITERATE_EVENTCLIENTLIST(GamePaused, playerID, paused);
}

void
CEventHandler::GameFrame(int gameFrame)
{
    ITERATE_EVENTCLIENTLIST(GameFrame, gameFrame);
}

void
CEventHandler::GameID(const unsigned char* gameID, unsigned int numBytes)
{
    ITERATE_EVENTCLIENTLIST(GameID, gameID, numBytes);
}

void
CEventHandler::TeamDied(int teamID)
{
    ITERATE_EVENTCLIENTLIST(TeamDied, teamID);
}

void
CEventHandler::TeamChanged(int teamID)
{
    ITERATE_EVENTCLIENTLIST(TeamChanged, teamID);
}

void
CEventHandler::PlayerChanged(int playerID)
{
    ITERATE_EVENTCLIENTLIST(PlayerChanged, playerID);
}

void
CEventHandler::PlayerAdded(int playerID)
{
    ITERATE_EVENTCLIENTLIST(PlayerAdded, playerID);
}

void
CEventHandler::PlayerRemoved(int playerID, int reason)
{
...
void
CEventHandler::UnitHarvestStorageFull(const CUnit* unit)
{
...
void
CEventHandler::CollectGarbage()
{
    ITERATE_EVENTCLIENTLIST(CollectGarbage);
}

void
CEventHandler::DbgTimingInfo(DbgTimingInfoType type,
...
void
CEventHandler::Load(IArchive* archive)
{
...
void
CEventHandler::SunChanged()
{
    ITERATE_EVENTCLIENTLIST(SunChanged);
}

void
CEventHandler::ViewResize()
{
    ITERATE_EVENTCLIENTLIST(ViewResize);
}

void
CEventHandler::GameProgress(int gameFrame)
{
...
bool
CEventHandler::CommandNotify(const Command& cmd)
{
...
bool
CEventHandler::KeyPress(int key, bool isRepeat)
{
...
bool
CEventHandler::KeyRelease(int key)
{
...
bool
CEventHandler::TextInput(const std::string& utf8)
{
...
bool
CEventHandler::MousePress(int x, int y, int button)
{
...
void
CEventHandler::MouseRelease(int x, int y, int button)
{
    if (mouseOwner) {
...
bool
CEventHandler::MouseMove(int x, int y, int dx, int dy, int button)
{
    if (mouseOwner == NULL) {
        return false;
    }
...
bool
CEventHandler::MouseWheel(bool up, float value)
{
...
bool
CEventHandler::JoystickEvent(const std::string& event, int val1, int val2)
{
...
bool
CEventHandler::IsAbove(int x, int y)
{
...
string
CEventHandler::GetTooltip(int x, int y)
{
...
bool
CEventHandler::AddConsoleLine(const std::string& msg,
...
void
CEventHandler::LastMessagePosition(const float3& pos)
{
    ITERATE_EVENTCLIENTLIST(LastMessagePosition, pos);
}

bool
CEventHandler::GroupChanged(int groupID)
{
...
bool
CEventHandler::GameSetup(const string& state,
                         bool& ready,
...
void
CEventHandler::DownloadQueued(int ID,
...
void
CEventHandler::DownloadStarted(int ID)
{
    ITERATE_EVENTCLIENTLIST(DownloadStarted, ID);
}

void
CEventHandler::DownloadFinished(int ID)
{
    ITERATE_EVENTCLIENTLIST(DownloadFinished, ID);
}

void
CEventHandler::DownloadFailed(int ID, int errorID)
{
    ITERATE_EVENTCLIENTLIST(DownloadFailed, ID, errorID);
}

void
CEventHandler::DownloadProgress(int ID, long downloaded, long total)
{
    ITERATE_EVENTCLIENTLIST(DownloadProgress, ID, downloaded, total);
}

string
CEventHandler::WorldTooltip(const CUnit* unit,
                            const CFeature* feature,
...
bool
CEventHandler::MapDrawCmd(int playerID,
                          int type,
                          const float3* pos0,
                          const float3* pos1,
                          const string* label)
{
    CONTROL_REVERSE_ITERATE_DEF_TRUE(
      MapDrawCmd, playerID, type, pos0, pos1, label)
    return false;
}
...
void
CEventHandler::MetalMapChanged(const int x, const int z)
{
