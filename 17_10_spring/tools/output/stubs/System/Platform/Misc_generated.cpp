...
GetUserDirFromSystemApi()
{
    ...
    SHGetFolderPath(
      NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, strPath);
    return strPath;
#else
...
GetProcessExecutableFile()
{
    ...
    const int ret =
      ::GetModuleFileName(hModule, procExeFile, sizeof(procExeFile));
...
#error implement this
    ...
    if (procExeFilePath.empty()) {
        LOG_L(L_WARNING,
              "Failed to get file path of the process executable, reason: %s",
              error);
    }
...
#define SHARED_LIBRARY_EXTENSION "so"
    ...
    if (moduleName.empty()) {
        ...
        if (moduleName.find("." SHARED_LIBRARY_EXTENSION) ==
            std::string::npos) {
    ...
    if (hModule != NULL) {
        ...
        const int ret =
          ::GetModuleFileName(hModule, moduleFile, sizeof(moduleFile));
...
#warning implement this (or use linux version)
    ...
    if (moduleFilePath.empty()) {
        ...
        LOG_L(L_WARNING,
              "Failed to get file path of the module \"%s\", reason: %s",
              moduleName.c_str(),
              error);
    }
...
GetShortFileName(const std::string& file)
{
    ...
    // FIXME:
    // stackoverflow.com/questions/843843/getshortpathname-unpredictable-results
    const int length =
      GetShortPathName(file.c_str(), &shortPathC[0], file.size() + 1);
               ...
               bool asSubprocess)
{
    ...
    if (asSubprocess) {
        ...
        if (!CreateProcess(
              NULL, argsCStr, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
            delete[] argsCStr;
            LOG("[%s] Error creating subprocess (%lu)",
                __FUNCTION__,
                GetLastError());
            return execError;
        }
...
#define EXECVP execvp
    ...
    if (EXECVP(args[0].c_str(), &processArgs[0]) == -1) {
        LOG("[%s] error: \"%s\" %s (%d)",
            __FUNCTION__,
            args[0].c_str(),
            (execError = strerror(errno)).c_str(),
            errno);
    }
#undef EXECVP
