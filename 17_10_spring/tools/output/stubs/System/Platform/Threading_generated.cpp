...
GetAffinity()
{
    ...
    int numCpus =
      std::min(CPU_COUNT(&curAffinity),
               32); // w/o the min(.., 32) `(1 << n)` could overflow!
...
SetAffinityHelper(const char* threadName, boost::uint32_t affinity)
{
    ...
    if (cpuMask == ~0) {
        ...
        LOG("[Threading] %s thread CPU affinity mask set: %d (config is %d)",
            threadName,
            cpuMask,
            affinity);
    } else if (cpuMask == 0) {
        LOG_L(L_ERROR,
              "[Threading] %s thread CPU affinity mask failed: %d",
              threadName,
              affinity);
    } else {
        LOG("[Threading] %s thread CPU affinity mask set: %d",
            threadName,
            cpuMask);
    }
}
                          ...
                          boost::uint32_t avoidCores)
{
    ...
    // find an unused core
    // count down cause hyperthread cores are appended to the end and we prefer
    // those for our worker threads (the physical cores are prefered for task
    // specific threads)
    {
...
InitThreadPool()
{
...

    {
        ...
        ThreadPool::SetThreadSpinTime(
          configHandler->GetInt("WorkerThreadSpinTime"));
#else
        int workerCount = -1;
#endif
        const int numCores = ThreadPool::GetMaxThreads();

        // For latency reasons our worker threads yield rarely and so eat a lot
        // cputime with idleing. So it's better we always leave 1 core free for
        // our other threads, drivers & OS
        if (workerCount < 0) {
            if (numCores == 2) {
        ...
        if (workerCount > numCores) {
            LOG_L(L_WARNING,
                  "Set ThreadPool workers to %i, but there are just %i cores!",
                  workerCount,
                  numCores);
            workerCount = numCores;
        }
    ...
    ompCores = parallel_reduce(
      [&]() -> boost::uint32_t {
          ...
          boost::uint32_t ompCore =
            GetCpuCoreForWorkerThread(i - 1, ompAvailCores, mainAffinity);
          // boost::uint32_t ompCore = ompAvailCores;
          Threading::SetAffinity(ompCore);
          return ompCore;
      },
      [](boost::uint32_t a, boost::unique_future<boost::uint32_t>& b)
        -> boost::uint32_t { return a | b.get(); });
...
// Note: only available with mingw64!!!
    ...
    if (GetLogicalCpuCores() > 1) {
        ...
        pthread_setschedparam(
          Threading::GetCurrentThread(), SCHED_BATCH, &param);
    }
#endif
}
...
{
#ifdef WIN32
    // we need to use this cause GetCurrentThread() just returns a pseudo
    // handle, which returns in all threads the current active one, so we need
    // to translate it with DuplicateHandle to an absolute handle valid in our
    // watchdog thread
                ...
                std::shared_ptr<Threading::ThreadControls>* ppCtlsReturn)
{
    ...
    boost::thread localthread(
      boost::bind(Threading::ThreadStart, taskFunc, ppCtlsReturn, &tempCtls));

    // Wait so that we know the thread is running and fully initialized before
    // returning.
...
IsMainThread()
{
    return NativeThreadIdsEqual(Threading::GetCurrentThreadId(),
                                nativeMainThreadID);
}
bool
...
{
    threadError.reset(new Error(err));
}
Error*
GetThreadError()
{
