...
struct StackFrame
{
    int level; // level in the original unwinding (inlined functions share the
               // same level as their "caller")
    void* ip;  // instruction pointer from libunwind or backtrace()
    std::string mangled; // mangled name retrieved from libunwind (not printed,
                         // memoized for debugging)
    ...
    std::list<StackFunction> entries; // function names and lines (possibly
                                      // several inlined) retrieved from
                                      // addr2line
    StackFrame()
      : level(0)
      , ip(0)
...
CreateAbsolutePath(const std::string& relativePath)
{
    ...
    if (absolutePath.length() > 0 && (absolutePath[0] == '/')) {
        ...
        absolutePath =
          FileSystemAbstraction::EnsurePathSepAtEnd(GetBinaryLocation()) +
          absolutePath;
    }
...
FindBaseMemoryAddresses(std::map<std::string, uintptr_t>& binPath_baseMemAddr)
{
    ...
    for (bpbmai = binPath_baseMemAddr.begin();
         bpbmai != binPath_baseMemAddr.end();
         ++bpbmai) {
    ...
    // libaries of the current process + other stuff (which we are not
    // interested in)
    ...
    if (mapsFile != NULL) {
        ...
        // (example 32bit) 08048000-08056000 r-xp 00000000 03:0c 64593
        // /usr/sbin/gpm (example 64bit) ffffffffff600000-ffffffffff601000 r-xp
        ...
        // read all lines
        while (!paths_notFound.empty() &&
               (fgets(line, 511, mapsFile) != NULL)) {
            // parse the line
            const int red = sscanf(line,
                                   "%lx-%*x %*s %lx %*s %*u %s",
                                   &mem_start,
                                   &binAddr_offset,
                                   binPathName);
            ...
            if (red == 3) {
                if (binAddr_offset == 0) {
                    ...
                    // go through all paths of the binaries involved in the
                    // stack trace
                    for (pi = paths_notFound.begin();
                         pi != paths_notFound.end();
                         ++pi) {
...
ExtractPath(const std::string& line)
{
    ...
    // /usr/lib/libstdc++.so.6(_ZN9__gnu_cxx27__verbose_terminate_handlerEv+0x16d)
    // [0x7fc023553fcd]
...
TranslateStackTrace(bool* aiCrash, StackTrace& stacktrace, const int logLevel)
{
    ...
    for (auto it = stacktrace.begin(); it != stacktrace.end(); ++it) {
        ...
        LOG_L(L_DEBUG,
              "symbol = \"%s\", path = \"%s\", absPath = \"%s\", addr = 0x%lx",
              it->symbol.c_str(),
              path.c_str(),
              absPath.c_str(),
              it->addr);
        ...
        containsDriverSo = (containsDriverSo ||
                            (path.find("libGLcore.so") != std::string::npos));
        containsDriverSo =
          (containsDriverSo || (path.find("psb_dri.so") != std::string::npos));
        containsDriverSo =
          (containsDriverSo || (path.find("i965_dri.so") != std::string::npos));
        containsDriverSo = (containsDriverSo ||
                            (path.find("fglrx_dri.so") != std::string::npos));
        if (!containsAIInterfaceSo &&
            (absPath.find("Interfaces") != std::string::npos)) {
            containsAIInterfaceSo = true;
        }
        if (!containsSkirmishAISo &&
            (absPath.find("Skirmish") != std::string::npos)) {
    ...
    if (containsDriverSo) {
        LOG_I(
          logLevel,
          "This stack trace indicates a problem with your graphic card driver. "
          "Please try upgrading or downgrading it. "
          "Specifically recommended is the latest driver, and one that is as "
          "old as your graphic card. "
          "Also try lower graphic details and disabling Lua widgets in "
          "spring-settings.\n");
    }
    ...
    if (containsAIInterfaceSo) {
        LOG_I(
          logLevel,
          "This stack trace indicates a problem with an AI Interface library.");
        if (aiCrash)
            *aiCrash = true;
    }
    if (containsSkirmishAISo) {
    ...
    //   This is nested so that the outer loop covers all the entries for one
    //   library -- this means fewer addr2line calls.
    for (auto it = binPath_baseMemAddr.cbegin();
         it != binPath_baseMemAddr.cend();
         it++) {
        ...
        if (cmdOut != NULL) {
            ...
            while (res != NULL) {
                i = indices.front();
                indices.pop();
                // First scan the address and ensure that it matches the one we
                // were expecting
                uintptr_t parsedAddr = 0;
                int matched = sscanf(line, "0x%lx", &parsedAddr);
                if (matched != 1 || parsedAddr != stacktrace[i].addr) {
                    LOG_L(L_WARNING,
                          "Mismatched address received from addr2line: 0x%lx "
                          "!= 0x%lx",
                          parsedAddr,
                          stacktrace[i].addr);
                    break;
                ...
                while (res != NULL) {
                    ...
                    entry.funcname =
                      funcname.substr(0, funcname.rfind(" (discriminator"));

                    if (fgets_addr2line(line, maxLength, cmdOut) == nullptr) {
                        break;
                    }
                ...
                if (!stacktrace[i].entries.empty()) { // Ensure that the last
                                                      // entry in a sequence of
                                                      // results is marked as
                                                      // "not inlined"
...
LogStacktrace(const int logLevel, StackTrace& stacktrace)
{
    ...
    for (auto fit = stacktrace.begin(); fit != stacktrace.end(); fit++) {
        for (auto eit = fit->entries.begin(); eit != fit->entries.end();
             eit++) {
            ...
            int abbrev_start = 0;
            if (fileline[0] == '/') { // See if we can shorten the file/line bit
                                      // by removing the common path
                if (CommonStringLength(fileline, exe_path, &abbrev_start) >
                    1) { // i.e. one char for first '/'
                    eit->abbrev_fileline =
                      std::string(".../") +
                      fileline.substr(abbrev_start, std::string::npos);
                } else if (CommonStringLength(
                             fileline, cwd_path, &abbrev_start) > 1) {
                    eit->abbrev_fileline =
                      std::string("./") +
                      fileline.substr(abbrev_start, std::string::npos);
                }
            ...
            colFileline =
              std::max(colFileline, (int)eit->abbrev_fileline.length());
        }
    }
        ...
        for (auto eit = fit->entries.begin(); eit != fit->entries.end();
            ...
            if (eit->inLine) {
                LOG_I(logLevel,
                      ...
                      fit->level,
                      colFileline,
                      eit->abbrev_fileline.c_str(),
                      eit->abbrev_funcname.c_str());
            } else {
                LOG_I(logLevel,
                      "[%02u]   %*s  %s",
                      fit->level,
...
GetSigAction(void (*sigact_handler)(int, siginfo_t*, void*))
{
    ...
    if (sigact_handler == nullptr) {
        // the default signal handler uses the old sa_handler interface, so just
        // identify this case with sigact_handler == null
...
thread_unwind(ucontext_t* uc, void** iparray, StackTrace& stacktrace)
{
    ...
    unw_cursor_t cursor;
    // Effective ucontext_t. If uc not supplied, use unw_getcontext locally.
    // This is appropriate inside signal handlers.
#if defined(__APPLE__)
    ...
    /*
     * Note: documentation seems to indicate that uc_link contains a pointer to
    a "successor" context
     * that is to be resumed after the current one (as you might expect in a
    signal handler).
     * In practice however, the Linux OS seems to re-use the existing context of
    a thread, and so this approach
     * does not work. The uc_link is sometimes non-NULL however, but I do not
    know what the relation is to the
     * target context (uc).
     *
    while (uc->uc_link) {
            uc = uc->uc_link;
           ...
           const int logLevel = LOG_LEVEL_ERROR)
{
    ...
    if (threadName != NULL) {
        LOG_I(logLevel,
              "Stacktrace (%s) for Spring %s:",
              threadName,
              (SpringVersion::GetFull()).c_str());
    } else {
        LOG_I(logLevel,
              "Stacktrace for Spring %s:",
              (SpringVersion::GetFull()).c_str());
    }
    ...
    // Get untranslated stacktrace symbols
    {
        ...
        if (numLines > MAX_STACKTRACE_DEPTH) {
            LOG_L(
              L_ERROR,
              "thread_unwind returned more lines than we allotted space for!");
        }
        char** lines =
          backtrace_symbols(iparray, numLines); // give them meaningfull names
           ...
           const int logLevel)
{
    ...
    if (!Threading::IsMainThread(thread) &&
        Threading::GetCurrentThread() != thread) {
                    ...
                    const std::string& threadName)
{
    ...
    LOG_L(L_WARNING,
          "Suspended-thread Stacktrace (%s) for Spring %s:",
          threadName.c_str(),
          (SpringVersion::GetFull()).c_str());
    ...
    // Get untranslated stacktrace symbols
    {
        ...
        const int numLines =
          thread_unwind(&ctls->ucontext, iparray, stacktrace);
...
{
    LOG_L(L_ERROR,
          "Halted Stacktrace for Spring %s using libunwind:",
          (SpringVersion::GetFull()).c_str());
    ...
    // Print out the translated StackTrace. Ignore the frames that occur inside
    // the signal handler (before its line in the trace) -- they are likely some
    // kind of padding or just garbage.
...
HandleSignal(int signal, siginfo_t* siginfo, void* pctx)
{
    ...
    // Turn off signal handling for this signal temporarily in order to disable
    // recursive events (e.g. SIGSEGV)
    reentrances++;
    ...
    // append the signal name (it seems there is no OS function to map signum to
    // signame :<)
    if (signal == SIGSEGV) {
        error += " (SIGSEGV)";
    ...
    const bool fatalSignal =
      (signal == SIGSEGV) || (signal == SIGILL) || (signal == SIGPIPE) ||
      (signal == SIGFPE) || // causes a endless loop, and process never gets far
                            // the causing cmd :<
      (signal == SIGABRT) || // same
      (signal == SIGBUS);
    ...
    if (!keepRunning) {
        ...
        ErrorMessageBox(buf.str(),
                        "Spring crashed",
                        MBF_OK | MBF_CRASH); // this also calls exit()
    }
...
NewHandler()
{
    LOG_L(
      L_ERROR,
      "Failed to allocate memory"); // make sure this ends up in the log also

    OutputStacktrace();

    ErrorMessageBox(
      "Failed to allocate memory", "Spring: Fatal Error", MBF_OK | MBF_CRASH);
}
...
Install()
{
    ...
    sigaction(SIGBUS, &sa, NULL); // on macosx EXC_BAD_ACCESS (mach exception)
                                  // is translated to SIGBUS
