...
CpuId::getIdsIntel()
{
    ...
    LOG_L(L_WARNING,
          "[CpuId] Max cpuid leaf is less than 4! Using OS processor number.");
    setDefault();
}
...
CpuId::getIdsIntelEnumerate()
{
    ...
    for (size_t processor = 0; processor < processorNumber; processor++) {
        ...
        affinityMaskOfPackages[packages.size() - 1] |= ((uint64_t)1)
                                                       << processor;
    }
...
{
    LOG_L(L_DEBUG,
          "[CpuId] ht/smt/cmt detection for AMD is not implemented! Using "
          "processor number reported by OS.");
}
