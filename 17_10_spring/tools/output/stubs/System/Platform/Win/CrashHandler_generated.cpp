...
SigAbrtHandler(int signal)
{
    ...
    ErrorMessageBox("Abort / abnormal termination",
                    "Spring: Fatal Error",
                    MBF_OK | MBF_CRASH);
}
...
InitImageHlpDll()
{
    ...
    // Note: For some strange reason it doesn't work ~4 times after it was
    // loaded&unloaded the first time.
    int i = 0;
    do {
                 ...
                 const int logLevel = LOG_LEVEL_ERROR)
{
    ...
    if (e) {
        ...
        for (int allocIter = 0;; ++allocIter) {
            HANDLE allocThread =
              CreateThread(NULL, 0, &AllocTest, NULL, CREATE_SUSPENDED, NULL);
...
#else
        DWORD eip, esp, ebp;
        __asm__ __volatile__("call func; func: pop %%eax; mov %%eax, %0;"
                             : "=m"(eip)
                             :
                             : "%eax");
...
#error "CrashHandler: Unsupported platform"
    ...
    while (true) {
        ...
        char* printstringsnew =
          (char*)GlobalAlloc(GMEM_FIXED, (count + 1) * BUFFER_SIZE);
        ...
        // Check if we have symbols, only works on VC (mingw doesn't have a
        ...
        if (SymFromAddr(process, sf.AddrPC.Offset, nullptr, pSym)) {
            ...
            SymGetLineFromAddr64(
              GetCurrentProcess(), sf.AddrPC.Offset, &displacement, &line);

            SNPRINTF(printstrings + count * BUFFER_SIZE,
                     BUFFER_SIZE,
                     "(%d) %s:%u %s [0x%08llX]",
                     count,
...
#endif
        {
            // This is the code path taken on MinGW, and VC if no debugging syms
            // are found.
        ...
        // OpenGL lib names (Nvidia): "nvoglnt.dll" "nvoglv32.dll"
        // "nvoglv64.dll" (last one is a guess)
    ...
    if (containsOglDll) {
        LOG_I(
          ...
          "Make sure to use a driver removal utility, before installing other "
          "drivers.");
    }
...
PrepareStacktrace(const int logLevel)
{
    ...
    SymEnumerateModules(
      GetCurrentProcess(), (PSYM_ENUMMODULES_CALLBACK)EnumModules, NULL);
}
...
{
    LOG_L(L_ERROR,
          "Error handler invoked for Spring %s.",
          (SpringVersion::GetFull()).c_str());
...
ExceptionHandler(LPEXCEPTION_POINTERS e)
{
    ...
    LOG_L(
      L_ERROR, "Spring %s has crashed.", (SpringVersion::GetFull()).c_str());
    PrepareStacktrace();
    ...
    LOG_L(L_ERROR,
          ...
          (PVOID)e->ExceptionRecord->ExceptionAddress);

    // Print stacktrace.
    StacktraceInline(NULL, e); // inline: avoid modifying the stack, it might
                               // confuse StackWalk when using the context
                               // record passed to ExceptionHandler
    ...
    ErrorMessageBox(buf.str(),
                    "Spring: Unhandled exception",
                    MBF_OK | MBF_CRASH); // calls exit()!
