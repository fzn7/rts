  ...
  , curHeightRolloffModifier(1)
{
    ...
    if (!CheckError("CSoundSource::CSoundSource")) {
        ...
        alSourcef(
          id, AL_REFERENCE_DISTANCE, referenceDistance * ELMOS_TO_METERS);
...
CSoundSource::~CSoundSource()
{
    if (efxEnabled) {
        alSource3i(
          id, AL_AUXILIARY_SEND_FILTER, AL_EFFECTSLOT_NULL, 0, AL_FILTER_NULL);
...
CSoundSource::Update()
{
    ...
    if (curPlaying) {
        if (in3D && (efxEnabled != efx->enabled)) {
            alSourcef(id,
                      ...
                      (efx->enabled) ? efx->GetAirAbsorptionFactor() : 0);
            alSource3i(id,
                       AL_AUXILIARY_SEND_FILTER,
                       (efx->enabled) ? efx->sfxSlot : AL_EFFECTSLOT_NULL,
                       0,
                       AL_FILTER_NULL);
            alSourcei(id,
                      AL_DIRECT_FILTER,
                      (efx->enabled) ? efx->sfxFilter : AL_FILTER_NULL);
        ...
        if (heightRolloffModifier != curHeightRolloffModifier) {
            ...
            alSourcef(id,
                      AL_ROLLOFF_FACTOR,
                      ROLLOFF_FACTOR * curPlaying->rolloff *
                        heightRolloffModifier);
        }

        if (!IsPlaying(true) ||
            ((curPlaying->loopTime > 0) && (spring_gettime() > loopStop)))
            Stop();
    }
    ...
    if (efxEnabled && (efxUpdates != efx->updates)) {
        //! airAbsorption & LowPass aren't auto updated by OpenAL on change, so
        //! we need to do it per source
                   ...
                   bool relative)
{
    ...
    if (relative || !item->in3D) {
        ...
        if (efx->enabled) {
            efxEnabled = true;
            alSourcef(
              id, AL_AIR_ABSORPTION_FACTOR, efx->GetAirAbsorptionFactor());
            alSource3i(
              id, AL_AUXILIARY_SEND_FILTER, efx->sfxSlot, 0, AL_FILTER_NULL);
            alSourcei(id, AL_DIRECT_FILTER, efx->sfxFilter);
            efxUpdates = efx->updates;
        }
        ...
        if (gain > 1.0f) {
            //! OpenAL on Mac cannot handle AL_GAIN > 1 well, so we will adjust
            //! settings to get the same output with AL_GAIN = 1.
            ALint model = alGetInteger(AL_DISTANCE_MODEL);
            ALfloat rolloff =
              ROLLOFF_FACTOR * item->rolloff * heightRolloffModifier;
            ALfloat ref = referenceDistance * ELMOS_TO_METERS;
            if ((model == AL_INVERSE_DISTANCE_CLAMPED) ||
                (model == AL_INVERSE_DISTANCE)) {
                alSourcef(id,
                          AL_REFERENCE_DISTANCE,
                          ((gain - 1.0f) * ref / rolloff) + ref);
                alSourcef(
                  id, AL_ROLLOFF_FACTOR, (gain + rolloff - 1.0f) / gain);
                         ...
                         float volume)
{
    ...
    //! COggStreams only appends buffers, giving errors when a buffer of another
    //! format is still assigned
...
CSoundSource::UpdateVolume()
{
    ...
    if (curStream) {
        ...
        alSourcef(
          id, AL_GAIN, curVolume * curPlaying->GetGain() * curChannel->volume);
    }
}
