  ...
  : acceptNewConnections(false)
{
    ...
    if (IsAcceptingConnections()) {
        LOG("[UDPListener] successfully bound socket on port %i",
...
UDPListener::TryBindSocket(int port, SocketPtr* socket, const std::string& ip)
{
    ...
    try {
        ...
        if ((port < 0) || (port > 65535)) {
            throw std::range_error("Port is out of range [0, 65535]: " +
                                   IntToString(port));
        }
        ...
        if (ip.empty()) {
            ...
            throw std::runtime_error("Failed to parse hostname \"" + ip +
                                     "\": " + err.message());
        }

        if (!supportsIPv6 && addr.address().is_v6()) {
            throw std::runtime_error(
              "IP v6 not supported, can not use address " +
              addr.address().to_string());
        }

        if (addr.address().is_loopback()) {
            LOG_L(L_WARNING,
                  "Opening socket on loopback address. Other users will not be "
                  "able to connect!");
        }
        ...
        if (addr.address().is_v4()) {
            ...
            if (err) {
                throw std::runtime_error("Failed to open IP V4 socket: " +
                                         err.message());
            }
        }
        ...
        LOG("Binding UDP socket to IP %s %s (%s) port %i",
            (addr.address().is_v6() ? "(v6)" : "(v4)"),
            addr.address().to_string().c_str(),
            ip.c_str(),
            addr.port());
    } catch (
      const std::runtime_error&
        ex) { // includes boost::system::system_error and std::range_error
        socket->reset();
        errorMsg = ex.what();
        if (errorMsg.empty()) {
        ...
        LOG_L(L_ERROR,
              "Binding UDP socket to IP %s failed: %s",
              ip.c_str(),
              errorMsg.c_str());
    }
...
UDPListener::Update()
{
    ...
    while ((bytes_avail = mySocket->available()) > 0) {
        ...
        boost::system::error_code err;
        size_t bytesReceived = mySocket->receive_from(
        ...
        } else { // still have the packet (means no connection with the sender's
                 // address found)
            if (acceptNewConnections && data.lastContinuous == -1 &&
                data.nakType == 0) {
                if (!data.chunks.empty() &&
                    (*data.chunks.begin())->chunkNumber == 0) {
                    // new client wants to connect
                    boost::shared_ptr<UDPConnection> incoming(
                      new UDPConnection(mySocket, sender_endpoint));
                ...
                LOG_L(L_WARNING,
                      "Dropping packet from unknown IP: [%s]:%i",
                      sender_endpoint.address().to_string().c_str(),
                      sender_endpoint.port());
#ifdef DEBUG
                std::string conns;
                for (ConnMap::iterator it = conn.begin(); it != conn.end();
                     ++it) {
                    conns += str(boost::format(" [%s]:%i;") %
    ...
    for (ConnMap::iterator i = conn.begin(); i != conn.end();) {
        if (i->second.expired()) {
            LOG_L(L_DEBUG,
                  "Connection closed: [%s]:%i",
                  i->first.address().to_string().c_str(),
                  i->first.port());
            i = conn.erase(i);
            continue;
        }
...
{
    boost::shared_ptr<UDPConnection> newConn(
      new UDPConnection(mySocket, ip::udp::endpoint(WrapIP(ip), port)));
...
UDPListener::UpdateConnections()
{
    for (ConnMap::iterator i = conn.begin(); i != conn.end();) {
        ...
        if (uc && i->first != uc->GetEndpoint()) {
            conn[uc->GetEndpoint()] =
              uc; // inserting does not invalidate iterators
            i = conn.erase(i);
        } else
            ++i;
    }
