...
DataDirsAccess::FindFilesSingleDir(std::vector<std::string>& matches,
                                   ...
                                   int flags) const
{
    assert(datadir.empty() || datadir[datadir.length() - 1] ==
    ...
    if (flags & FileQueryFlags::WRITE) {
        ...
        if (flags & FileQueryFlags::CREATE_DIRS) {
            FileSystem::CreateDirectory(
              FileSystem::GetDirectory(writeableFile));
        }
...
DataDirsAccess::LocateDir(std::string dir, int flags) const
{
    ...
    if (flags & FileQueryFlags::WRITE) {
        ...
        const std::vector<std::string>& datadirs =
          dataDirLocater.GetDataDirPaths();
...
DataDirsAccess::FindDirsInDirectSubDirs(const std::string& relPath) const
{
    ...
    // list of subdirs in all occurences of the relative path in the data
    // directories
    std::vector<std::string> mainDirs;

    // find all subdirectories in the rootDirs
    for (const std::string& dir : rootDirs) {
        const std::vector<std::string>& localMainDirs =
          CFileHandler::SubDirs(dir, pattern, SPRING_VFS_RAW);
        mainDirs.insert(
          mainDirs.end(), localMainDirs.begin(), localMainDirs.end());
    }
