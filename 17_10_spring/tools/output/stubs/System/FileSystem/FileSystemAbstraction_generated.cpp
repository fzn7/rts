...
FileSystemAbstraction::GetFileModificationDate(const std::string& file)
{
    try {
        ...
        LOG_L(L_WARNING,
              "Failed fetching last modification time from file: %s",
              file.c_str());
...
{
// TODO uncomment this and test if there are conflicts in the code when this
// returns true but other custom code doesn't (e.g. with IsFSRoot)  const
// boost::filesystem::path f(file);  return f.is_absolute();
...
FileSystemAbstraction::MkDir(const std::string& dir)
{
    ...
    if (DirExists(dir)) {
        ...
        // If it doesn't exist we try to mkdir it and return success if that
        // succeeds.
#ifndef _WIN32
    bool dirCreated =
...
FileSystemAbstraction::DeleteFile(const std::string& file)
{
    try {
        const boost::filesystem::path f(file);
        // same as posix remove(), but on windows that func doesn't allow
        // deletion of dirs so it's easier to use boost from the start
        return (boost::filesystem::remove_all(f) >= 1);
    } catch (const boost::filesystem::filesystem_error& ex) {
        LOG_L(
          L_WARNING, "Could not delete file %s: %s", file.c_str(), ex.what());
        return false;
    }
}
          ...
          int flags)
{
    ...
    while ((ep = readdir(dp))) {
        ...
        if (ep->d_name[0] != '.') {
            // is it a file? (we just treat sockets / pipes / fifos /
            // character&block devices as files...) (need to stat because d_type
            // is DT_UNKNOWN on linux :-/)
            struct stat info;
