...
Utf16_To_Utf8(char* dest, size_t* destLen, const UInt16* src, size_t srcLen)
{
    ...
    for (;;) {
        ...
        do {
            ...
            if (dest)
                dest[destPos] =
                  (char)(0x80 + ((value >> (6 * numAdds)) & 0x3F));
            destPos++;
        } while (numAdds != 0);
    }
    *destLen = destPos;
...
int
CSevenZipArchive::GetFileName(const CSzArEx* db, int i)
{
  ...
  , isOpen(false)
{
    ...
    if (res == SZ_OK) {
        ...
        isOpen = false;
        LOG_L(
          L_ERROR, "Error opening \"%s\": %s", name.c_str(), GetErrorStr(res));
        return;
    }
    ...
    for (unsigned int i = 0; i < db.db.NumFiles; ++i) {
        ...
        if (!f->IsDir) {
            ...
            if (written <= 0) {
                LOG_L(L_ERROR,
                      "Error getting filename in Archive: %s %d, file skipped "
                      "in %s",
                      GetErrorStr(res),
                      res,
                      name.c_str());
