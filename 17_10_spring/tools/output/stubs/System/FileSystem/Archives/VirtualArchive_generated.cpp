...
CVirtualArchiveFactory::DoCreateArchive(const std::string& fileName) const
{
    ...
    for (std::vector<CVirtualArchive*>::const_iterator it = archives.begin();
         it != archives.end();
         ++it) {
...
CVirtualArchiveOpen::CVirtualArchiveOpen(CVirtualArchive* archive,
  ...
  , archive(archive)
{
    // Set subclass name index to archive's index (doesn't update while archive
    // is open)
...
{
    for (std::vector<CVirtualFile*>::iterator it = files.begin();
         it != files.end();
         ++it) {
        delete *it;
    }
...
CVirtualArchive::WriteToFile()
{
    std::string zipFilePath =
      dataDirsAccess.LocateFile(fileName, FileQueryFlags::WRITE) + ".sdz";
    LOG("Writing zip file for virtual archive %s to %s",
        fileName.c_str(),
        zipFilePath.c_str());
    ...
    for (std::vector<CVirtualFile*>::iterator it = files.begin();
        ...
        zipOpenNewFileInZip(zip,
                            ...
                            Z_DEFLATED,
                            Z_BEST_COMPRESSION);
        zipWriteInFileInZip(zip,
                            file->buffer.empty() ? NULL : &file->buffer[0],
                            file->buffer.size());
