...
CacheDir::IsCacheDir(const std::string& dir)
{
    ...
    bool isTagged = CacheDir::FileContentStartsWith(
      cacheFile, CacheDir::tagFile_content, CacheDir::tagFile_content_size);
                      ...
                      bool forceRewrite)
{
    ...
    if (currentCacheState == wantedCacheState) {
        ...
        if (wantedCacheState && forceRewrite) {
            requestedStatePresent =
              CacheDir::WriteCacheTagFile(cacheFile, additionalText);
        } else {
                                ...
                                size_t content_size)
{
    ...
    if (fileH != NULL) {
        content_size =
          ((content_size > content.size()) ? content.size() : content_size);
        char currFileChar;
        size_t i = 0;
        startsWith = true;
