...
const KnownInfoTag knownTags[] = {
    ...
    { "description",
      "example: Little units blowing up other little units",
      false },
    { "mapfile",
      "in case its a map, store location of smf/sm3 file",
      false }, // FIXME is this ever used in the engine?! or does it auto calc
               // the location?
    { "modtype", "1=primary, 0=hidden, 3=map", true },
    { "depend",
      "a table with all archives that needs to be loaded for this one",
      false },
    { "replace",
      "a table with archives that got replaced with this one",
      false }
};
                                          ...
                                          bool fromCache)
{
        ...
        if (!ArchiveData::IsReservedKey(keyLower)) {
            ...
            switch (luaType) {
                case LuaTable::STRING: {
                    SetInfoItemValueString(key,
                                           archiveTable.GetString(key, ""));
                } break;
                case LuaTable::NUMBER: {
                    SetInfoItemValueFloat(key,
                                          archiveTable.GetFloat(key, 0.0f));
                } break;
                case LuaTable::BOOLEAN: {
                    SetInfoItemValueBool(key, archiveTable.GetBool(key, false));
                } break;
                default: {
                    // just ignore unsupported types (most likely to be
                    // lua-tables)
                    // throw content_error("Lua-type " + IntToString(luaType) +
                    // " not supported in archive-info, but it is used on key \""
                    // + *key + "\"");
                } break;
            }
        }
    ...
    if (!version.empty()) {
        if (name.find(version) == std::string::npos) {
            ...
            LOG_L(L_WARNING,
                  "Invalid Name detected, please contact the author of the "
                  "archive to remove the Version from the Name: %s, Version: "
                  "%s",
                  name.c_str(),
                  version.c_str());
        }
...
CArchiveScanner::ArchiveData::GetInfoItem(const std::string& key)
{
    ...
    const std::map<std::string, InfoItem>::iterator ii =
...
CArchiveScanner::ArchiveData::GetInfoItem(const std::string& key) const
{
    ...
    const std::map<std::string, InfoItem>::const_iterator ii =
      info.find(StringToLower(key));
    if (ii != info.end()) {
        infoItem = &(ii->second);
    }
...
CArchiveScanner::ArchiveData::EnsureInfoItem(const std::string& key)
{
    ...
    if (IsReservedKey(keyLower)) {
        throw content_error("You may not use key " + key +
                            " in archive info, as it is reserved.");
    }
...
CArchiveScanner::ArchiveData::GetInfoItems() const
{
    ...
    for (std::map<std::string, InfoItem>::const_iterator i = info.begin();
         i != info.end();
         ++i) {
  ...
  : isDirty(false)
{
    ...
    const std::string cacheFolder =
      dataDirLocater.GetWriteDirPath() +
      FileSystem::EnsurePathSepAtEnd(FileSystem::GetCacheBaseDir());
    cachefile = cacheFolder + IntToString(INTERNAL_VER, "ArchiveCache%i.lua");
    ReadCacheData(GetFilepath());
    if (archiveInfos.empty()) {
        // when versioned ArchiveCache%i.lua is missing or empty, try old
...
CArchiveScanner::ScanAllDirs()
{
    ...
    for (auto d = datadirs.rbegin(); d != datadirs.rend(); ++d) {
        ...
        // ArchiveCache has been parsed at this point --> archiveInfos is
        // populated
#if !defined(DEDICATED) && !defined(UNITSYNC)
                          ...
                          bool doChecksum)
{
    ...
    /*for (auto it = foundArchives.begin(); it != foundArchives.end(); ++it) {
            ...
            while (jt != foundArchives.end()) {
                    std::string f1 =
    StringToLower(FileSystem::GetFilename(*it)); std::string f2 =
    StringToLower(FileSystem::GetFilename(*jt)); if ((f1 == f2) ||
    FileSystem::ComparePaths(*it, *jt)) { jt = foundArchives.erase(jt); } else {
                            ++jt;
                    }
            }
    }*/
                         ...
                         std::list<std::string>* foundArchives)
{
    ...
    while (!subDirs.empty()) {
        ...
        const std::vector<std::string>& found = dataDirsAccess.FindFiles(
          subDirs.front(), "*", FileQueryFlags::INCLUDE_DIRS);
        subDirs.pop_front();

        for (std::string fullName : found) {
            FileSystem::EnsureNoPathSepAtEnd(fullName);
            const std::string lcfpath =
              StringToLower(FileSystem::GetDirectory(fullName));
            ...
            } else if (FileSystem::DirExists(fullName)) {
    ...
    for (unsigned fid = 0; fid != ar->NumFiles(); ++fid) {
        ...
        if (metaFileClass == 1) {
            ...
            error = "Unpacking/reading cost for meta file " + name +
                    " is too high, please repack the archive (make sure to use "
                    "a non-solid algorithm, if applicable)";
            return false;
        } else if (metaFileClass == 2) {
            // 2nd class
            LOG_SL(LOG_SECTION_ARCHIVESCANNER,
                   L_WARNING,
                   "Archive %s: The cost for reading a 2nd-class meta-file is "
                   "too high: %s",
                   fullName.c_str(),
                   name.c_str());
        }
...
CArchiveScanner::SearchMapFile(const IArchive* ar, std::string& error)
{
    ...
    // check for smf/sm3 and if the uncompression of important files is too
    // costy
    for (unsigned fid = 0; fid != ar->NumFiles(); ++fid) {
        std::string name;
        int size;
...
{
    return ((fileName == "bitmaps.sdz") || (fileName == "springcontent.sdz") ||
            (fileName == "maphelper.sdz") || (fileName == "cursors.sdz"));
}
...
CArchiveScanner::ScanArchive(const std::string& fullName, bool doChecksum)
{
    ...
    if (hasMapinfo) {
        ...
        if (ad.GetMapFile().empty()) {
            LOG_L(L_WARNING,
                  "%s: mapfile isn't set in mapinfo.lua, please set it for "
                  "faster loading!",
                  fullName.c_str());
    ...
    if (!error.empty()) {
        ...
        LOG_L(
          L_WARNING, "Failed to scan %s (%s)", fullName.c_str(), error.c_str());
    ...
    if (hasMapinfo || !mapfile.empty()) {
        // it is a map
        if (ad.GetName().empty()) {
            // FIXME The name will never be empty, if version is set (see HACK
            // in ArchiveData)
            ad.SetInfoItemValueString("name_pure",
                                      FileSystem::GetBasename(mapfile));
                                 ...
                                 bool doChecksum)
{
    ...
    *modified = info.st_mtime;
    ...
    std::map<std::string, BrokenArchive>::iterator bai =
      brokenArchives.find(lcfn);
    if (bai != brokenArchives.end()) {
        if ((unsigned)info.st_mtime == bai->second.modified &&
            fpath == bai->second.path) {
    ...
    if (aii != archiveInfos.end()) {
        ...
        if ((unsigned)info.st_mtime == aii->second.modified &&
            fpath == aii->second.path) {
        ...
        if (aii->second.updated) {
            ...
            LOG_L(L_ERROR,
                  "Found a \"%s\" already in \"%s\", ignoring.",
                  fullName.c_str(),
                  (aii->second.path + aii->second.origName).c_str());
            if (IsBaseContent(filename)) {
                throw user_error(
                  std::string("duplicate base content detected:\n\t") +
                  aii->second.path + std::string("\n\t") + fpath +
                  std::string("\nPlease fix your configuration/installation as "
                                ...
                                std::string& err)
{
    ...
    if (!ar->GetFile(fileName, buf) || buf.empty()) {
        ...
        if (ar->GetArchiveName().find(".sdp") != std::string::npos) {
            err += " (archive's rapid tag: " +
                   GetRapidTagFromPackage(
                     FileSystem::GetBasename(ar->GetArchiveName())) +
                   ")";
        }
        return false;
    }
 ...
 */
unsigned int
CArchiveScanner::GetCRC(const std::string& arcName)
{
    CRC crc;
    ...
    // Push the filenames into a std::vector, cause OMP can better iterate over
    // those
    ...
    // Hint: Multithreading only speedups `.sdd` loading. For those the CRC
    // generation is extremely slow -
    //       it has to load the full file to calc it! For the other formats
    //       (sd7, sdz, sdp) the CRC is saved in the metainformation of the
    //       container and so the loading is much faster. Neither does any of
    //       our current (2011) packing libraries support multithreading :/
    for_mt(0, crcs.size(), [&](const int i) {
        CRCPair& crcp = crcs[i];
        const unsigned int nameCRC =
          CRC::GetCRC(crcp.filename->data(), crcp.filename->size());
...
CArchiveScanner::ReadCacheData(const std::string& filename)
{
    ...
    if (!p.Execute()) {
        LOG_L(L_ERROR,
              "Failed to parse archive cache: %s",
              p.GetErrorLog().c_str());
        return;
    }
    ...
    for (int i = 1; archives.KeyExists(i); ++i) {
        ...
        ai.modified =
          strtoul(curArchive.GetString("modified", "0").c_str(), 0, 10);
        ai.checksum =
          strtoul(curArchive.GetString("checksum", "0").c_str(), 0, 10);
        ai.updated = false;
        ...
        if (ai.archiveData.GetModType() == modtype::map) {
            ...
            AddDependency(ai.archiveData.GetDependencies(),
                          "Spring content v1");
        }
    }
...
{
    auto it =
      std::remove_if(deps.begin(), deps.end(), [&](const std::string& dep) {
          return (dep == exclude);
      });
...
CArchiveScanner::WriteCacheData(const std::string& filename)
{
    ...
    spring::map_erase_if(archiveInfos,
                         [](const decltype(archiveInfos)::value_type& p) {
                             return !p.second.updated;
                         });
    spring::map_erase_if(brokenArchives,
                         [](const decltype(brokenArchives)::value_type& p) {
                             return !p.second.updated;
                         });
        ...
        if (!archData.GetName().empty()) {
            ...
            for (const auto& ii : archData.GetInfo()) {
                if (ii.second.valueType == INFO_VALUE_TYPE_STRING) {
                    SafeStr(out,
                            std::string("\t\t\t\t" + ii.first + " = ").c_str(),
                            ii.second.valueTypeString);
                } else {
                    fprintf(out,
                            "\t\t\t\t%s = %s,\n",
                            ii.first.c_str(),
                            ii.second.GetValueAsString(false).c_str());
                }
            }
...
sortByName(std::vector<CArchiveScanner::ArchiveData>& data)
{
    std::sort(data.begin(),
              data.end(),
              [](const CArchiveScanner::ArchiveData& a,
                 const CArchiveScanner::ArchiveData& b) {
...
CArchiveScanner::GetPrimaryMods() const
{
    ...
    for (std::map<std::string, ArchiveInfo>::const_iterator i =
           archiveInfos.begin();
         i != archiveInfos.end();
         ++i) {
        const ArchiveData& aid = i->second.archiveData;
        if ((!aid.GetName().empty()) &&
            (aid.GetModType() == modtype::primary)) {
            // Add the archive the mod is in as the first dependency
            ArchiveData md = aid;
            md.GetDependencies().insert(md.GetDependencies().begin(),
                                        i->second.origName);
            ret.push_back(md);
        }
    }
...
CArchiveScanner::GetAllMods() const
{
    ...
    for (std::map<std::string, ArchiveInfo>::const_iterator i =
        ...
        const ArchiveData& aid = i->second.archiveData;
        if ((!aid.GetName().empty()) &&
            ((aid.GetModType() & (modtype::primary | modtype::hidden)) != 0)) {
            // Add the archive the mod is in as the first dependency
            ArchiveData md = aid;
            md.GetDependencies().insert(md.GetDependencies().begin(),
                                        i->second.origName);
...
CArchiveScanner::GetAllArchivesUsedBy(const std::string& root, int depth) const
{
    ...
    std::map<std::string, ArchiveInfo>::const_iterator aii =
      archiveInfos.find(lcname);
    if (aii == archiveInfos.end()) {
#ifdef UNITSYNC
    ...
    while (aii->second.replaced.length() > 0) {
        // FIXME instead of this, call this function recursively, to get the
    ...
    for (const std::string& dep : aii->second.archiveData.GetDependencies()) {
        const std::vector<std::string>& deps =
          GetAllArchivesUsedBy(dep, depth + 1);
...
CArchiveScanner::GetMaps() const
{
    ...
    for (std::map<std::string, ArchiveInfo>::const_iterator aii =
           archiveInfos.begin();
         aii != archiveInfos.end();
         ++aii) {
        if (!(aii->second.archiveData.GetName().empty()) &&
            aii->second.archiveData.GetModType() == modtype::map) {
...
CArchiveScanner::CheckArchive(const std::string& name, unsigned checksum)
{
    ...
    if (localChecksum != checksum) {
        char msg[1024];
        sprintf(msg,
                "Checksum of %s (checksum 0x%x) differs from the host's copy "
                "(checksum 0x%x). "
                "This may be caused by a corrupted download or there may even "
                "be 2 different versions in circulation. Make sure you and the "
                "host have installed "
                "the chosen archive and its dependencies and consider "
                "redownloading it.",
                name.c_str(),
                localChecksum,
                checksum);
...
{
    for (std::map<std::string, ArchiveInfo>::const_iterator it =
           archiveInfos.begin();
         it != archiveInfos.end();
         ++it) {
...
CArchiveScanner::NameFromArchive(const std::string& archiveName) const
{
    ...
    std::map<std::string, ArchiveInfo>::const_iterator aii =
      archiveInfos.find(lcArchiveName);
    if (aii != archiveInfos.end()) {
...
CArchiveScanner::GetMetaFileClass(const std::string& filePath)
{
    ...
    if (lowerFilePath == "mapinfo.lua") { // basic archive info
        ...
        metaFileClass = 1;
        //	} else if ((ext == "smf") || (ext == "sm3")) {             // to
        //generate minimap 		metaFileClass = 1;
        ...
        metaFileClass = 2;
    } else if (StringStartsWith(lowerFilePath,
                                "sidepics/")) { // used by lobbies
        metaFileClass = 2;
    } else if (StringStartsWith(lowerFilePath,
                                "gamedata/")) { // used by lobbies
        metaFileClass = 2;
    } else if (lowerFilePath ==
               "armor.txt") { // used by lobbies (disabled units list)
        metaFileClass = 2;
    } else if (lowerFilePath ==
               "springignore.txt") { // used by lobbies (disabled units list)
        metaFileClass = 2;
    } else if (StringStartsWith(
                 lowerFilePath,
                 "units/")) { // used by lobbies (disabled units list)
        metaFileClass = 2;
    } else if (StringStartsWith(
                 lowerFilePath,
                 "features/")) { // used by lobbies (disabled units list)
        metaFileClass = 2;
    } else if (StringStartsWith(
                 lowerFilePath,
                 "weapons/")) { // used by lobbies (disabled units list)
        metaFileClass = 2;
    }
