...
{
    const CArchiveScanner::ArchiveData& aData =
      archiveScanner->GetArchiveData(lazyName);
    ...
    for (std::vector<CArchiveScanner::ArchiveData>::const_iterator it =
        ...
        if (lowerLazyName == StringToLower(it->GetShortName())) {
            ...
            if (versionInt == matchingVersionInt) {
                ...
                const int compareInt =
                  matchingVersion.compare(it->GetVersion());
                if (compareInt <= 0) {
                    matchingName = it->GetNameVersioned();
                    matchingVersion = it->GetVersion();
                    // matchingVersionInt = versionInt;
...
GetRandomGame(const std::string& lazyName, std::string* applicableName)
{
    ...
    if (lazyName == "random") {
        const std::vector<CArchiveScanner::ArchiveData>& games =
          archiveScanner->GetPrimaryMods();
        if (!games.empty()) {
            *applicableName =
              games[gu->RandInt() % games.size()].GetNameVersioned();
            return true;
        }
    }
#endif // UNITSYNC
...
GetMapBySubString(const std::string& lazyName, std::string* applicableName)
{
    ...
    for (std::vector<std::string>::const_iterator it = found.begin();
         it != found.end();
         ++it) {
        ...
        bool fits = true;
        for (std::vector<std::string>::const_iterator jt = substrings.begin();
             jt != substrings.end();
             ++jt) {
