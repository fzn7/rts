...
SpringApp::Initialize()
{
    ...
    const std::map<std::string, std::string> settings =
      configHandler->GetDataWithoutDefaults();
    ...
    globalRendering->SetFullScreen(configHandler->GetBool("Fullscreen"),
                                   cmdline->IsSet("window"),
                                   cmdline->IsSet("fullscreen"));
    ...
    // load QTCreator's gdb helper dll; a variant of this should also work on
    // other OSes
    {
...
{
    // SDL will cause a creation of gpu-driver thread that will clone its name
    // from the starting threads (= this one = mainthread)
...
SpringApp::CreateSDLWindow(const char* title)
{
    ...
    // Create GL debug context when wanted (allows further GL verbose
    // informations, but runs slower)
    ...
    if (globalRendering->FSAA > 0) {
        if (getenv("LIBGL_ALWAYS_SOFTWARE") != NULL) {
            LOG_L(L_WARNING,
                  "FSAALevel > 0 and LIBGL_ALWAYS_SOFTWARE set, this will very "
                  "likely crash!");
        }
    ...
    switch (globalRendering->winState) {
        case CGlobalRendering::WINSTATE_MAXIMIZED:
            sdlflags |= SDL_WINDOW_MAXIMIZED;
            break;
        case CGlobalRendering::WINSTATE_MINIMIZED:
    ...
    if (!window) {
        char buf[1024];
        SNPRINTF(
          buf, sizeof(buf), "Could not set video mode:\n%s", SDL_GetError());
    ...
    SDL_SetWindowMinimumSize(
      window, globalRendering->minWinSizeX, globalRendering->minWinSizeY);
    sdlGlCtx = SDL_GL_CreateContext(window);
    globalRendering->window = window;

#ifdef STREFLOP_H
    // Something in SDL_SetVideoMode (OpenGL drivers?) messes with the FPU
    // control word. Set single precision floating point math.
    ...
    // (happens at 300fps, neither fullscreen nor vsync fixes it, so disable
    // compositing) On Windows Aero often uses vsync, and so when Spring runs
    // windowed it will run with vsync too, resulting in bad performance.
...
SpringApp::GetDisplayGeometry()
{
    ...
    SDL_GetWindowSize(
      window, &globalRendering->winSizeX, &globalRendering->winSizeY);
    SDL_GetWindowPosition(
      window, &globalRendering->winPosX, &globalRendering->winPosY);

    globalRendering->UpdateViewPortGeometry();

    // XXX SDL2 is crap ...
    // Reading window state fails if it is changed via the window manager, like
    // clicking on the titlebar (2013)
    // https://bugzilla.libsdl.org/show_bug.cgi?id=1508 &
    // https://bugzilla.libsdl.org/show_bug.cgi?id=2282 happens on linux too!
    ...
    } else if (state & SDL_WINDOW_MINIMIZED) {
...
SpringApp::SaveWindowPosition()
{
    ...
    } else if (globalRendering->winState ==
               CGlobalRendering::WINSTATE_MINIMIZED) {
...
SpringApp::SetupViewportGeometry()
{
    ...
    const int vpy = globalRendering->winSizeY - globalRendering->viewSizeY -
                    globalRendering->viewPosY;

    if (agui::gui != nullptr) {
        agui::gui->UpdateScreenGeometry(
          globalRendering->viewSizeX, globalRendering->viewSizeY, vpx, vpy);
    }
}
...
SpringApp::InitOpenGL()
{
    ...
    // Print Final Mode (call after SetupViewportGeometry, which updates
    // viewSizeX/Y)
    SDL_DisplayMode dmode;
    SDL_GetWindowDisplayMode(window, &dmode);
    bool isBorderless =
      (SDL_GetWindowFlags(window) & SDL_WINDOW_BORDERLESS) != 0;
    LOG("[%s] video mode set to %ix%i:%ibit @%iHz %s",
        __FUNCTION__,
        globalRendering->viewSizeX,
        globalRendering->viewSizeY,
        SDL_BITSPERPIXEL(dmode.format),
        dmode.refresh_rate,
        globalRendering->fullScreen ? (isBorderless ? "(borderless)" : "")
                                    : "(windowed)");
}
...
SpringApp::LoadFonts()
{
    ...
    smallFont = CglFont::LoadFont(
      smallFontFile, smallFontSize, smallOutlineWidth, smallOutlineWeight);

    const static std::string installBroken =
      ", installation of spring broken? did you run make install?";
    if (!font) {
        throw content_error(std::string("Failed to load FontFile: ") +
                            fontFile + installBroken);
    }
    if (!smallFont) {
        throw content_error(std::string("Failed to load SmallFontFile: ") +
                            smallFontFile + installBroken);
    }
}
...
{
    cmdline->SetUsageDescription(
      "Usage: " + binaryName + " [options] [path_to_script.txt or demo.sdfz]");
    cmdline->AddSwitch(
      0, "sync-version", "Display program sync version (for online gaming)");
    ...
    cmdline->AddSwitch('t',
                       "textureatlas",
                       "Dump each finalized textureatlas in textureatlasN.tga");
    cmdline->AddInt(0,
                    "benchmark",
                    "Enable benchmark mode (writes a benchmark.data file). The "
                    "given number specifies the timespan to test.");
    ...
    cmdline->AddSwitch(0,
                       "list-config-vars",
                       "Dump a list of config vars and meta data to stdout");
    cmdline->AddSwitch(0,
                       "list-def-tags",
                       "Dump a list of all unitdef-, weapondef-, ... tags and "
                       "meta data to stdout");
    cmdline->AddSwitch(0,
                       "list-ceg-classes",
                       "Dump a list of available projectile classes to stdout");
    cmdline->AddSwitch(
      0, "test-creg", "Test if all CREG classes are completed");
    ...
    cmdline->AddString(0,
                       "isolation-dir",
                       "Specify the isolation-mode data-dir (see --isolation)");
    cmdline->AddString('d', "write-dir", "Specify where Spring writes to.");
    cmdline->AddString(
      'g', "game", "Specify the game that will be instantly loaded");
    cmdline->AddString(
      'm', "map", "Specify the map that will be instantly loaded");
    ...
    if (cmdline->IsSet("sync-version")) {
        // Note, the missing "Spring " is intentionally to make it compatible
        // with `spring-dedicated --sync-version`
    ...
    if (cmdline->IsSet("list-config-vars")) {
        ...
        const int res = CCustomExplosionGenerator::OutputProjectileClassInfo()
                          ? EXIT_SUCCESS
                          : EXIT_FAILURE;
        exit(res);
    }
    ...
    const string configSource =
      (cmdline->IsSet("config") ? cmdline->GetString("config") : "");
    ...
    LOG("[%s] command-line args: \"%s\"",
        __FUNCTION__,
        cmdline->GetCmdLine().c_str());
    FileSystemInitializer::PreInitializeConfigHandler(configSource, safemode);
    ...
    if (cmdline->IsSet("benchmark")) {
        ...
        if (cmdline->IsSet("benchmarkstart")) {
            CBenchmark::startFrame =
              cmdline->GetInt("benchmarkstart") * 60 * GAME_SPEED;
        }
        CBenchmark::endFrame = CBenchmark::startFrame +
                               cmdline->GetInt("benchmark") * 60 * GAME_SPEED;
    }
}
...
SpringApp::StartScript(const std::string& inputFile)
{
    ...
    if (!fh.FileExists())
        throw content_error("Setup-script does not exist in given location: " +
                            inputFile);
...
SpringApp::LoadSpringMenu()
{
    ...
    if (defaultscript.empty() &&
        CFileHandler("defaultstartscript.txt", SPRING_VFS_PWD_ALL)
          .FileExists()) {
...
#ifdef HEADLESS
        handleerror(NULL,
                    "The headless version of the engine can not be run in "
                    "interactive mode.\n"
...
SpringApp::Startup()
{
    ...
    if (inputFile.empty()) {
        ...
        if (cmdline->IsSet("game") && cmdline->IsSet("map")) {
            ...
            activeController = RunScript(StartScriptGen::CreateMinimalSetup(
              cmdline->GetString("game"), cmdline->GetString("map")));
            return;
        }
        LoadSpringMenu();
        return;
    ...
    if (inputFile.find("spring://") == 0) {
        ...
        if (!ParseSpringUri(inputFile,
                            clientSetup->myPlayerName,
                            clientSetup->myPasswd,
                            clientSetup->hostIP,
...
SpringApp::Reload(const std::string& script)
{
    ...
    // Lua shutdown functions need to access 'game' but SafeDelete sets it to
    // NULL.
    game->KillLua();
...
SpringApp::ShutDown()
{
    ...
    if (game != nullptr) {
        game->KillLua(); // must be called before `game` var gets nulled, else
                         // stuff in LuaSyncedRead.cpp will fail
    }
    SafeDelete(game);
    SafeDelete(pregame);
    ...
    switch (event.type) {
        case SDL_WINDOWEVENT: {
            switch (event.window.event) {
                ...
                // case SDL_WINDOWEVENT_RESIZED: //this is event is always
                // preceded by:
                ...
                case SDL_WINDOWEVENT_FOCUS_GAINED: {
                    ...
                    KeyInput::Update(0,
                                     ((keyBindings != NULL)
                                        ? keyBindings->GetFakeMetaKey()
                                        : -1));
                } break;
                case SDL_WINDOWEVENT_FOCUS_LOST: {
                    Watchdog::ClearTimer(WDT_MAIN, true);

                    // SDL has some bug and does not update modstate on
                    // alt+tab/minimize etc.
                    // FIXME check if still happens with SDL2 (2013)
                    SDL_SetModState((SDL_Keymod)(
                      SDL_GetModState() & (KMOD_NUM | KMOD_CAPS | KMOD_MODE)));
        ...
        case SDL_TEXTINPUT: {
            ...
            if (activeController->userWriting && !catched) {
                ...
                ac->writingPos =
                  Clamp<int>(ac->writingPos, 0, ac->userInput.length());
        ...
        case SDL_KEYDOWN: {
            KeyInput::Update(
              ...
              ((keyBindings != NULL) ? keyBindings->GetFakeMetaKey() : -1));

            if (activeController) {
                activeController->KeyPressed(
                  KeyInput::GetNormalizedKeySymbol(event.key.keysym.sym),
                  event.key.repeat);
            }
        } break;
        case SDL_KEYUP: {
