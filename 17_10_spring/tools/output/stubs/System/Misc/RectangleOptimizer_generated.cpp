...
{
    unsigned ret = 0;
    for (CRectangleOptimizer::const_iterator it = rectangles.begin();
         it != rectangles.end();
         ++it) {
                                 ...
                                 const SRectangle& rect2)
{
    ...
    return (rect1.x1 == rect2.x1 && rect1.x2 == rect2.x2) ||
           (rect1.z1 == rect2.z1 && rect1.z2 == rect2.z2);
}
...
CRectangleOptimizer::HandleMerge(SRectangle& rect1, SRectangle& rect2)
{
    ...
    if (edgesInRect12.count() == 3) {
        if (sharedEdges.count() == 3 ||
            ...
            // FIXME check if no overlapping (only touching), if so check
            // maxAreaPerRect!!!
...
CRectangleOptimizer::HandleOverlapping(SRectangle* rect1, SRectangle* rect2)
{
    ...
    if (edgesInRect12.count() == 4) {
    ...
    } else if ((edgesInRect12[0] || edgesInRect12[1]) &&
               (edgesInRect12[2] || edgesInRect12[3])) {
    ...
    } else if ((edgesInRect12.count() == 2) &&
               (edgesInRect12[0] != edgesInRect21[0])) {
        // assert(edgesInRect12.count() == 2);
        ...
        if (edgesInRect12[0]) {
            assert(edgesInRect12[0] && edgesInRect12[1] && !edgesInRect12[2] &&
                   !edgesInRect12[3]);
            rect2->z2 = rect1->z1;
            rect3.z1 = rect1->z2;
        } else {
