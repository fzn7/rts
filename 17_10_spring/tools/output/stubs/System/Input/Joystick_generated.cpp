...
InitJoystick()
{
    ...
    if (useJoystick) {
        ...
        if (err == -1) {
            LOG_L(L_ERROR,
                  "Could not initialise joystick subsystem: %s",
                  SDL_GetError());
            return;
        } else {
            stick = new Joystick();
  ...
  : myStick(nullptr)
{
    ...
    if (myStick) {
        ...
        inputCon =
          input.AddHandler(boost::bind(&Joystick::HandleEvent, this, _1));
    } else {
...
Joystick::HandleEvent(const SDL_Event& event)
{
    switch (event.type) {
        ...
        case SDL_JOYBUTTONDOWN: {
            eventHandler.JoystickEvent(
              "JoyButtonDown", event.jbutton.button, event.jbutton.state);
            break;
        }
        case SDL_JOYBUTTONUP: {
            eventHandler.JoystickEvent(
              "JoyButtonUp", event.jbutton.button, event.jbutton.state);
            break;
        }
