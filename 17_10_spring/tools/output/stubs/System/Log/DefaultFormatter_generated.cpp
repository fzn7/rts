             ...
             va_list arguments)
{
    ...
    do {
        ...
        // NOTE: earlier glibc versions and MSVC will return -1 when buffer is
        // too small const bool bufferTooSmall = writtenChars >= freeBufferSize
        // || writtenChars < 0;
                           ...
                           int level)
{
    ...
    if ((strlen(*buffer) + 1) >= *bufferSize) {
        ...
        log_formatter_createPrefix(
          buffer, bufferSize, section, level); // recursive
    }
}
