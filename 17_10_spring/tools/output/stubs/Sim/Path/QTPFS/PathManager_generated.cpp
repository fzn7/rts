...
QTPFS::PathManager::~PathManager()
{
    ...
    std::map<unsigned int, PathSearchTrace::Execution*>::const_iterator
      tracesIt;
    ...
    for (unsigned int layerNum = 0; layerNum < nodeLayers.size(); layerNum++) {
        ...
        for (searchesIt = pathSearches[layerNum].begin();
             searchesIt != pathSearches[layerNum].end();
             ++searchesIt) {
            delete (*searchesIt);
        }
    ...
    for (tracesIt = pathTraces.begin(); tracesIt != pathTraces.end();
         ++tracesIt) {
...
QTPFS::PathManager::Finalize()
{
...

    {
        ...
        updateThread =
          new boost::thread(boost::bind(&PathManager::ThreadUpdate, this));
#endif
    }
...
{
    LAYERS_PER_UPDATE =
      std::max(1u, mapInfo->pfs.qtpfs_constants.layersPerUpdate);
    MAX_TEAM_SEARCHES =
      std::max(1u, mapInfo->pfs.qtpfs_constants.maxTeamSearches);
}
...
QTPFS::PathManager::Load()
{
    ...
    {
        const boost::uint32_t mapCheckSum =
          archiveScanner->GetArchiveCompleteChecksum(gameSetup->mapName);
        const boost::uint32_t modCheckSum =
          archiveScanner->GetArchiveCompleteChecksum(gameSetup->modName);
        const std::string& cacheDirName =
          GetCacheDirName(mapCheckSum, modCheckSum);
        ...
        for (unsigned int layerNum = 0; layerNum < nodeLayers.size();
             layerNum++) {
            ...
            if (haveCacheDir) {
                // if cache-dir exists, must set node relations after
                // de-serializing its trees
                nodeLayers[layerNum].ExecNodeNeighborCacheUpdates(
                  MAP_RECTANGLE, numTerrainChanges);
            }
#endif

            pfsCheckSum ^= nodeTrees[layerNum]->GetCheckSum();
            maxNumLeafNodes =
              std::max(nodeLayers[layerNum].GetNumLeafNodes(), maxNumLeafNodes);
        }
    ...
    {
        const std::string sumStr =
          "pfs-checksum: " + IntToString(pfsCheckSum, "%08x") + ", ";
        const std::string memStr =
          "mem-footprint: " + IntToString(GetMemFootPrint()) + "MB";
        pmLoadScreen.AddLoadMessage("[" + std::string(__FUNCTION__) + "] " +
                                    sumStr + memStr);
...
QTPFS::PathManager::SpawnBoostThreads(MemberFunc f, const SRectangle& r)
{
    static std::vector<boost::thread*> threads(
      std::min(GetNumThreads(), nodeLayers.size()), NULL);

    for (unsigned int threadNum = 0; threadNum < threads.size(); threadNum++) {
        threads[threadNum] =
          new boost::thread(boost::bind(f, this, threadNum, threads.size(), r));
    }
...
QTPFS::PathManager::InitNodeLayersThreaded(const SRectangle& rect)
{
    ...
    const char* fmtString =
      "[PathManager::%s] using %u threads for %u node-layers (%s)";
...
#ifdef QTPFS_OPENMP_ENABLED
    {
        sprintf(loadMsg,
                ...
                nodeLayers.size(),
                (haveCacheDir ? "cached" : "uncached"));
        pmLoadScreen.AddLoadMessage(loadMsg);

#ifndef NDEBUG
        const char* preFmtStr = "  initializing node-layer %u (thread %u)";
        const char* pstFmtStr =
          "  initialized node-layer %u (%u MB, %u leafs, ratio %f)";
#endif
        ...
        for_mt(0, nodeLayers.size(), [&, loadMsg](const int layerNum) {
            ...
            const unsigned int mem =
              (tree->GetMemFootPrint() + layer.GetMemFootPrint()) /
              (1024 * 1024);
                                         ...
                                         const SRectangle& rect)
{
    ...
    const unsigned int numExcessLayers =
      (threadNum == (numThreads - 1)) ? (nodeLayers.size() % numThreads) : 0;
    ...
    for (unsigned int layerNum = minLayer; layerNum < maxLayer; layerNum++) {
        ...
        const NodeLayer& layer = nodeLayers[layerNum];
        const unsigned int mem =
          (tree->GetMemFootPrint() + layer.GetMemFootPrint()) / (1024 * 1024);
...
QTPFS::PathManager::UpdateNodeLayer(unsigned int layerNum, const SRectangle& r)
{
    ...
    mr.x2 = std::min((r.x2 + md->xsizeh) + int(QTNode::MinSizeX() >> 1),
                     mapDims.mapx);
    mr.z2 = std::min((r.z2 + md->zsizeh) + int(QTNode::MinSizeZ() >> 1),
                     mapDims.mapy);
    ur.x1 = mr.x1;
    ur.z1 = mr.z1;
    ur.x2 = mr.x2;
...
#ifndef QTPFS_CONSERVATIVE_NEIGHBOR_CACHE_UPDATES
        nodeLayers[layerNum].ExecNodeNeighborCacheUpdates(ur,
                                                          numTerrainChanges);
#endif
    }
}
                                               ...
                                               bool flushQueue)
{
    ...
    //
    unsigned int maxExecutedUpdates =
      nodeLayers[layerNum].NumQueuedUpdates() * 0.05f;
    ...
    while (nodeLayers[layerNum].HaveQueuedUpdate()) {
...
#ifndef QTPFS_CONSERVATIVE_NEIGHBOR_CACHE_UPDATES
            // NOTE:
            //   since any terrain changes have already happened when we start
            //   eating through the queue for this layer, <numTerrainChanges>
            //   would have the same value for each queued update we consume and
            //   is not useful here: in case queue-item j referenced some or all
            //   of the same nodes as item i (j > i), it could cause nodes
            //   updated during processing of i to not be updated again when j
            //   gets processed --> dangling neighbor pointers
            //
        ...
        if ((!flushQueue) &&
            ((numExecutedUpdates += 1) >= maxExecutedUpdates)) {
                                    ...
                                    boost::uint32_t modCheckSum) const
{
    ...
    char loadMsg[512] = { '\0' };
    const char* fmtString = "[PathManager::%s] using cache-dir %s "
                            "(map-checksum %08x, mod-checksum %08x)";

    sprintf(
      loadMsg, fmtString, __FUNCTION__, dir.c_str(), mapCheckSum, modCheckSum);
    pmLoadScreen.AddLoadMessage(loadMsg);
...
QTPFS::PathManager::Serialize(const std::string& cacheFileDir)
{
    ...
    for (unsigned int i = 0; i < nodeTrees.size(); i++) {
        ...
        fileNames[i] =
          cacheFileDir + "tree" + IntToString(i, "%02x") + "-" + md->name;
        ...
        if (haveCacheDir) {
#ifdef QTPFS_CACHE_XACCESS
            {
                ...
                while (FileSystem::GetFileSize(fileNames[i] + "-tmp") !=
                       sizeof(unsigned int)) {
                    boost::this_thread::sleep(boost::posix_time::millisec(100));
                }

                fileStreams[i]->open((fileNames[i] + "-tmp").c_str(),
                                     std::ios::in | std::ios::binary);
                fileStreams[i]->read(reinterpret_cast<char*>(&fileSizes[i]),
                                     sizeof(unsigned int));
                fileStreams[i]->close();
            ...
            fileStreams[i]->open(fileNames[i].c_str(),
                                 std::ios::in | std::ios::binary);
        ...
        nodeTrees[i]->Serialize(
          *fileStreams[i], nodeLayers[i], &fileSizes[i], haveCacheDir);
...
#ifdef QTPFS_CACHE_XACCESS
        if (!haveCacheDir) {
            // signal any other (concurrently loading) Spring processes; needed
            // for validation-tests
            fileStreams[i]->open((fileNames[i] + "-tmp").c_str(),
                                 std::ios::out | std::ios::binary);
            fileStreams[i]->write(reinterpret_cast<const char*>(&fileSizes[i]),
                                  sizeof(unsigned int));
            fileStreams[i]->flush();
            fileStreams[i]->close();
        }
                                  ...
                                  unsigned int type)
{
    ...
    // if type is TERRAINCHANGE_OBJECT_INSERTED or
    // TERRAINCHANGE_OBJECT_INSERTED_YM, this rectangle covers the yardmap of a
    // CSolidObject* and will be tesselated to maximum depth automatically
    numTerrainChanges += 1;
...
QTPFS::PathManager::ThreadUpdate()
{
    ...
    while (!nodeLayers.empty()) {
        ...
        const unsigned int numPathTypeUpdates = std::min(
          static_cast<unsigned int>(nodeLayers.size()), layersPerUpdateTmp);
        ...
        for (unsigned int pathTypeUpdate = minPathTypeUpdate;
             pathTypeUpdate < maxPathTypeUpdate;
             pathTypeUpdate++) {
...
#ifdef QTPFS_STAGGERED_LAYER_UPDATES
            // NOTE: *must* be called between QueueDeadPathSearches and
            // ExecuteQueuedSearches
            ExecQueuedNodeLayerUpdates(pathTypeUpdate,
                                       !pathSearches[pathTypeUpdate].empty());
#endif

            ExecuteQueuedSearches(pathTypeUpdate);
        }

        std::copy(numCurrExecutedSearches.begin(),
                  numCurrExecutedSearches.end(),
                  numPrevExecutedSearches.begin());
...
QTPFS::PathManager::ExecuteQueuedSearches(unsigned int pathType)
{
    ...
    if (!searches.empty()) {
        ...
        while (searchesIt != searches.end()) {
            if (ExecuteSearch(
                  searches, searchesIt, nodeLayer, pathCache, pathType)) {
                searchStateOffset += NODE_STATE_OFFSET;
            }
        }
    }
...
#define DeleteSearch(s, it)                                                    \
...

    {
        ...
        SharedPathMap::const_iterator sharedPathsIt =
          sharedPaths.find(path->GetHash());
        ...
        const unsigned int numCurrSearches =
          numCurrExecutedSearches[search->GetTeam()];
        const unsigned int numPrevSearches =
          numPrevExecutedSearches[search->GetTeam()];
...
QTPFS::PathManager::QueueDeadPathSearches(unsigned int pathType)
{
    ...
    if (!deadPaths.empty()) {
        ...
        for (deadPathsIt = deadPaths.begin(); deadPathsIt != deadPaths.end();
             ++deadPathsIt) {
            QueueSearch(deadPathsIt->second,
                        NULL,
                        moveDef,
                        ZeroVector,
                        ZeroVector,
                                ...
                                const bool synced)
{
    ...
    if (oldPath != NULL) {
        ...
        const float3& pos =
          (obj != NULL) ? obj->pos : oldPath->GetSourcePoint();
        ...
        newSearch->SetTeam((object != NULL) ? object->team
                                            : teamHandler->ActiveTeams());
    }

    assert(
      (pathCaches[moveDef->pathType].GetTempPath(newPath->GetID()))->GetID() ==
      0);
                                ...
                                bool synced)
{
    ...
    return (QueueSearch(
      NULL, object, moveDef, sourcePoint, targetPoint, radius, synced));
}
                                 ...
                                 bool synced)
{
    ...
    if (tempPath->GetID() != 0) {
        ...
        const float3 targetDirec =
          (targetPoint - sourcePoint).SafeNormalize() * SQUARE_SIZE;
        return float3(
          sourcePoint.x + targetDirec.x, -1.0f, sourcePoint.z + targetDirec.z);
    }
    if (livePath->GetID() == 0) {
    ...
    for (unsigned int i = (livePath->GetNextPointIndex());
         i < (livePath->NumPoints() - 1);
         i++) {
        ...
        // always exists)
        const float3 &p0 = livePath->GetPoint(i),
                     v0 = float3(p0.x - point.x, 0.0f, p0.z - point.z);
        const float3 &p1 = livePath->GetPoint(i + 1),
                     v1 = float3(p1.x - point.x, 0.0f, p1.z - point.z);
    ...
    if ((livePath->GetNextPointIndex() == 0) &&
        (nxtPointIdx == (livePath->NumPoints() - 1)))
        nxtPointIdx = 1;
