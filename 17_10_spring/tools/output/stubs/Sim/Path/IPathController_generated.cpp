                                        ...
                                        bool isReversing) const
{
    ...
    const float speeds[] = { targetSpeed * targetSpeedSign,
                             currentSpeed * currentSpeedSign };

    // all possible cases
    //   tar=[6,+1] && cur=[4,+1] --> dif=+2, delta= accRate*+1 (fwd-acc from +4
    //   to +6) tar=[4,+1] && cur=[6,+1] --> dif=-2, delta=-decRate*+1 (fwd-dec
    //   from +6 to +4) tar=[6,-1] && cur=[4,-1] --> dif=+2, delta= accRate*-1
    //   (rev-acc from -4 to -6) tar=[4,-1] && cur=[6,-1] --> dif=-2,
    //   delta=-decRate*-1 (rev-dec from -6 to -4) tar=[4,-1] && cur=[4,+1] -->
    //   dif=-8, delta=-decRate*+1 (fwd-dec from +5 to 0, rev-acc from 0 to -5)
    //   tar=[4,+1] && cur=[4,-1] --> dif=-8, delta=-decRate*-1 (rev-dec from -5
    //   to 0, fwd-acc from 0 to +5)
    //
    ...
    const float deltaSpeed =
      mix(modAccRate, -modDecRate, (rawSpeedDiff < 0.0f));
                                          ...
                                          float* curTurnSpeed) const
{
    ...
    const short curDeltaHeading =
      newHeading -
      short(oldHeading + (*curTurnSpeed) * (maxTurnAccel / maxTurnSpeed));

    const float minTurnAccel =
      std::min(float(std::abs(curDeltaHeading)), maxTurnAccel);
    const float rawTurnAccel =
      Clamp(Sign(curDeltaHeading) * maxTurnAccel, -minTurnAccel, minTurnAccel);
    const float newTurnSpeed =
      Clamp((*curTurnSpeed) + rawTurnAccel * (1 - owner->IsInAir()),
            -maxTurnSpeed,
            maxTurnSpeed);
    ...
    const float modTurnAccel =
      rawTurnAccel *
      TurnAccelerationSign(turnBrakeDist, curDeltaHeading, newDeltaHeading);

    (*curTurnSpeed) += (modTurnAccel * (1 - owner->IsInAir()));
    (*curTurnSpeed) =
      Clamp((*curTurnSpeed) * 0.99f, -maxTurnSpeed, maxTurnSpeed);
