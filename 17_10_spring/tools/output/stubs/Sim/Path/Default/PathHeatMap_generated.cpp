...
PathHeatMap::GetInstance()
{
    if (gPathHeatMap == NULL)
        gPathHeatMap =
          new PathHeatMap(PATH_HEATMAP_XSCALE, PATH_HEATMAP_ZSCALE);
                     ...
                     unsigned int pathID)
{
    ...
    if (!points.empty()) {
        ...
        for (std::vector<int2>::const_iterator it = points.begin();
             it != points.end();
             ++it) {
                         ...
                         unsigned int ownerID) const
{
    ...
    const unsigned int val = (heatMapOffset >= heatMap[idx].value)
                               ? 0
                               : (heatMap[idx].value - heatMapOffset);
