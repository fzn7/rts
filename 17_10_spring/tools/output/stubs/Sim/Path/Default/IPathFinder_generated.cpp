                     ...
                     const unsigned int maxNodes)
{
    ...
    assert((unsigned)mStartBlock.x < nbrOfBlocks.x &&
           (unsigned)mStartBlock.y < nbrOfBlocks.y);

    // Check cache (when there is one)
    int2 goalBlock;
    goalBlock.x = pfDef.goalSquareX / BLOCK_SIZE;
    goalBlock.y = pfDef.goalSquareZ / BLOCK_SIZE;
    const CPathCache::CacheItem* ci = GetCache(mStartBlock,
                                               goalBlock,
                                               pfDef.sqGoalRadius,
                                               moveDef.pathType,
                                               pfDef.synced);
                        ...
                        const CSolidObject* owner)
{
    ...
    // although our starting square may be inside the goal radius, the starting
    // coordinate may be outside. in this case we do not want to return
    // CantGetCloser, but instead a path to our starting square.
    ...
    blockStates.nodeMask[mStartBlockIdx] &=
      PATHOPT_OBSOLETE; // clear all except PATHOPT_OBSOLETE
