...
PathFlowMap::GetInstance()
{
    if (gPathFlowMap == NULL)
        gPathFlowMap =
          new PathFlowMap(PATH_FLOWMAP_XSCALE, PATH_FLOWMAP_ZSCALE);
...
PathFlowMap::PathFlowMap(unsigned int scalex, unsigned int scalez)
{
    ...
    pathOptDirs[PATHOPT_LEFT | PATHOPT_UP] =
      (pathOptDirs[PATHOPT_LEFT] + pathOptDirs[PATHOPT_UP]) * s;
    pathOptDirs[PATHOPT_RIGHT | PATHOPT_UP] =
      (pathOptDirs[PATHOPT_RIGHT] + pathOptDirs[PATHOPT_UP]) * s;
    pathOptDirs[PATHOPT_RIGHT | PATHOPT_DOWN] =
      (pathOptDirs[PATHOPT_RIGHT] + pathOptDirs[PATHOPT_DOWN]) * s;
    pathOptDirs[PATHOPT_LEFT | PATHOPT_DOWN] =
      (pathOptDirs[PATHOPT_LEFT] + pathOptDirs[PATHOPT_DOWN]) * s;

    for (unsigned int n = 0; n < xsize * zsize; n++) {
        const unsigned int x = n % xsize;
        const unsigned int z = n / xsize;
        const float3 p =
          float3((x * xfact) + (xfact >> 1), 0.0f, (z * zfact) + (zfact >> 1));
            ...
            std::set<unsigned int>::iterator nit;
                    ...
                    for (it = fIndices.begin(); it != fIndices.end(); ) {
                            ...
                            if (bIndices.find(*it) == bIndices.end()) {
                                    // if the cell at index <*it> was NOT
       written to
                                    // during any AddFlow call last frame
       (meaning no
                                    // units were projected into it), start
       decaying
            ...
            for (it = bIndices.begin(); it != bIndices.end(); ++it) {
                    ...
                    // note: if FLOW_DECAY_ENABLED == 1, all cells whose
       normalized
                    // flow-strength is less than FLOW_EPSILON will also be
       decayed
                    // (this can be problematic if the range of unit mass values
       is
...
PathFlowMap::AddFlow(const CSolidObject* o)
{
    ...
    /*
            if
       (!o->HasCollidableStateBit(CSolidObject::CSTATE_BIT_SOLIDOBJECTS)) {
                    return;
            }
            if (!o->pos.IsInBounds()) {
                    return;
            ...
            const float3& flowVec = (Square(o->speed.w) >= 1.0f)?
       float3(o->speed): GetVectorFromHeading(o->heading); const unsigned int
       cellIdx = GetCellIdx(o);
            ...
            #if (FLOW_NGB_PROJECTION == 1)
            {
                    ...
                    if (halfSpaces[0]) {  i = ((z    ) * xsize + (x - 1));
       bIndices.insert(i);  ngbs[0] = &bCells[i];  } if (halfSpaces[1]) {  i =
       ((z    ) * xsize + (x + 1));  bIndices.insert(i);  ngbs[0] = &bCells[i];
       } if (halfSpaces[2]) {  i = ((z - 1) * xsize + (x    ));
       bIndices.insert(i);  ngbs[1] = &bCells[i];  } if (halfSpaces[3]) {  i =
       ((z + 1) * xsize + (x    ));  bIndices.insert(i);  ngbs[1] = &bCells[i];
       }

                         if (halfSpaces[0] && halfSpaces[2]) {  i = ((z - 1) *
       xsize + (x - 1));  bIndices.insert(i);  ngbs[2] = &bCells[i];  } else if
       (halfSpaces[0] && halfSpaces[3]) {  i = ((z + 1) * xsize + (x - 1));
       bIndices.insert(i);  ngbs[2] = &bCells[i];  } else if (halfSpaces[1] &&
       halfSpaces[2]) {  i = ((z - 1) * xsize + (x + 1));  bIndices.insert(i);
       ngbs[2] = &bCells[i];  } else if (halfSpaces[1] && halfSpaces[3]) {  i =
       ((z + 1) * xsize + (x + 1));  bIndices.insert(i);  ngbs[2] = &bCells[i];
       }

                    if (ngbs[0] != NULL) {  ngbs[0]->flowVector +=
       float3(flowVec.x, o->mass * o->moveDef->flowMod * 0.666f, flowVec.z);
       ngbs[0]->numObjects += 1;  } if (ngbs[1] != NULL) {  ngbs[1]->flowVector
       += float3(flowVec.x, o->mass * o->moveDef->flowMod * 0.666f, flowVec.z);
       ngbs[1]->numObjects += 1;  } if (ngbs[2] != NULL) {  ngbs[2]->flowVector
       += float3(flowVec.x, o->mass * o->moveDef->flowMod * 0.333f, flowVec.z);
       ngbs[2]->numObjects += 1;  }
            }
            #endif

            maxFlow[bBufferIdx] = std::max(maxFlow[bBufferIdx],
       bCell.flowVector.y);
    */
}
...
PathFlowMap::GetFlowVec(unsigned int hmx, unsigned int hmz) const
{
    ...
    /*
            ...
            const unsigned int fCellIdx = (hmz / zscale) * xsize + (hmx /
       xscale);
                         ...
                         unsigned int pathOpt) const
{
    ...
    /*
            ...
            const float flowScale = ((flowVec.dot(pathDir) * -1.0f) + 1.0f) *
       0.5f; const float flowCost = (flowVec.y * FLOW_COST_MULT) * flowScale;
