                             ...
                             int group)
{
    ...
    for (int a = 0; a < 20; ++a) {
        ...
        CGeoSquareProjectile* gsp =
          new CGeoSquareProjectile(old1, old2, dir1, dir2, w1, w2);
                              ...
                              const float3& p4)
{
    ...
    float3 res = p1 * ni * ni * ni + p2 * 3 * i * ni * ni +
                 p3 * 3 * i * i * ni + p4 * i * i * i;
    return res;
}
                           ...
                           int group)
{
    ...
    if (arrow) {
        CGeoSquareProjectile* gsp =
          new CGeoSquareProjectile(start,
                                   start * 0.2f + end * 0.8f,
                                   dir,
                                   dir,
                                   width * 0.5f,
        ...
        gsp = new CGeoSquareProjectile(
          start * 0.2f + end * 0.8f, end, dir, dir, width, 0);
        geoGroups[group].squares.push_back(gsp);

    } else {
        CGeoSquareProjectile* gsp = new CGeoSquareProjectile(
          start, end, dir, dir, width * 0.5f, width * 0.5f);
        geoGroups[group].squares.push_back(gsp);
    }
...
CGeometricObjects::Update()
{
    while (!toBeDeleted.empty() &&
           (toBeDeleted.begin()->first <= gs->frameNum)) {
