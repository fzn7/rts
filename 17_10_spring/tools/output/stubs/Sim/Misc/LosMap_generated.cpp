...
class CLosTables
{
  public:
    ...
    static const int2 GetLosTableRaySquare(size_t losSize,
                                           size_t rayIndex,
                                           size_t squareIdx)
    {
...
void
CLosTables::GenerateForLosSize(size_t losSize)
{
...
CLosTables::GetCircleSurface(const int radius)
{
    ...
    MidpointCircleAlgo(radius, [&](int x, int y) {
        ...
        // second makes sure that only (0,radius) or (radius, 0) is generated
        // (the other one is generated by mirroring later)
        if (y != x && y != 0)
                       ...
                       const int radius)
{
    ...
    for (auto it = circlePoints.rbegin(); it != circlePoints.rend();
         ++it) { // note, we reverse iterate the list!
        const int2& p = *it;
        for (int a = p.x; a >= 1 && a >= p.y; --a) {
            ...
            if (!getpixel(t2) &&
                t2 != int2(0, radius)) { // (0,radius) is a mirror of
                                         // (radius,0), so don't add it
                  ...
                  int radius)
{
    ...
    for (const auto& ray1 : losRaysCopy) {
        ...
        for (auto& ray2 : losRaysCopy) {
            ...
            if (std::includes(
                  ray1.begin(), ray1.end(), ray2.begin(), ray2.end())) {
                // prepare for deletion
                ray2.clear();
            }
        }
    }
    auto jt = std::remove_if(losRaysCopy.begin(),
                             losRaysCopy.end(),
                             [](LosLine& ray) { return ray.empty(); });
...
// only AddRaycast supports UnsyncedHeightMap updates
    ...
    MidpointCircleAlgoPerLine(instance->radius, [&](int width, int y) {
        ...
        if (y_ < size.y) {
            ...
            const unsigned ex =
              Clamp(instance->basePos.x + width + 1, 0, size.x);
...
CLosMap::AddRaycast(SLosInstance* instance, int amount)
{
    if (instance->squares.empty() ||
        instance->squares.front().length == SLosInstance::EMPTY_RLE.length) {
        return;
    }

#ifdef USE_UNSYNCED_HEIGHTMAP
    // Inform ReadMap when squares enter LoS
    const bool updateUnsyncedHeightMap =
      (sendReadmapEvents && instance->allyteam >= 0 &&
       (instance->allyteam == gu->myAllyTeam || gu->spectatingFullView));
    if ((amount > 0) && updateUnsyncedHeightMap) {
        for (const SLosInstance::RLE rle : instance->squares) {
            ...
            for (int l = rle.length; l > 0; --l, ++idx) {
                ...
                const int2 p2 = std::min(p1 + int2(1, 1),
                                         int2(mapDims.mapxm1, mapDims.mapym1));
...
CLosMap::LosAdd(SLosInstance* li) const
{
    auto MAP_SQUARE_FULLRES = [&](int2 pos) {
        ...
        // assert(ipos.y * mapDims.mapx + ipos.x < (mapDims.mapx *
        // mapDims.mapy));
    ...
    if (SRectangle(0, 0, size.x, size.y).Inside(li->basePos) &&
        li->baseHeight <= heightmapFull[MAP_SQUARE_FULLRES(li->basePos)])
        return;

    // add all squares that are in the los radius
    SRectangle safeRect(
      li->radius, li->radius, size.x - li->radius, size.y - li->radius);
    if (safeRect.Inside(li->basePos)) {
        // we aren't touching the map borders -> we don't need to check for the
        // map boundaries
        UnsafeLosAdd(li);
    } else {
        ...
        int threadNum)
{
    ...
    if (anglesMap[oidx] < *prevAng) {
        const float invR =
          isqrt_lookup(off.x * off.x + off.y * off.y, threadNum);
...
{
    // How does it work?
    // We spawn rays (those created by CLosTables::GenerateForLosSize), and cast
    // them on the heightmap. Meaning we compute the angle to the given squares
    // and compare them with the highest cached one on that ray. When the new
    // angle is higher the square is visible and gets added to the squares
    // array.
    //
    // How does prevAng optimisation work?
    // We don't really need to save every angle as the maximum, if we're going
    // up a mountain we can just mark them true and continue until we reach the
    // top. So now, only hilltops are cached in maxAng, and they're only cached
    // when checking the square after the hilltop, since otherwise we can't know
    // that the ascent ended.
    ...
    std::vector<bool> squaresMap(area,
                                 false); // saves the list of visible squares
    std::vector<float> anglesMap(area, -1e8);
    int threadNum = ThreadPool::GetThreadNum();

    // Optimization: precalc all angles, cause:
    // 1. Many squares are accessed by multiple rays. Imagine you got a 128
    // radius circle
    //    then the center squares are accessed much more often than the circle
    //    border ones.
    // 2. The heightmap is much bigger than the circle, and won't fit into the
    // L2/L3. So
    //    when we buffer the precalc in a vector just large enough for the
    //    processed data, we reduce latter the amount of cache misses.
...
CLosMap::SafeLosAdd(SLosInstance* li) const
{
    ...
    if (safeRect.Inside(pos)) {
        ...
        for (size_t i = 0; i < numRays; ++i) {
            ...
            for (size_t n = 0; n < numSquares; n++) {
                const int2 square =
                  CLosTables::GetLosTableRaySquare(radius, i, n);
