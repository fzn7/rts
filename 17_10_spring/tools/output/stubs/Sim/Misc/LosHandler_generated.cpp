...
float
ILosType::GetRadius(const CUnit* unit) const
{
    switch (type) {
        case LOS_TYPE_LOS:
            return (unit->losRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_AIRLOS:
            return (unit->airLosRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_RADAR:
            return (unit->radarRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_SONAR:
            return (unit->sonarRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_JAMMER:
            return (unit->jammerRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_SEISMIC:
            return (unit->seismicRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_SONAR_JAMMER:
            return (unit->sonarJamRadius / SQUARE_SIZE) >> mipLevel;
        case LOS_TYPE_COUNT:
...
ILosType::GetHeight(const CUnit* unit) const
{
    ...
    const float emitHeight = (type == LOS_TYPE_LOS || type == LOS_TYPE_AIRLOS)
                               ? unit->unitDef->losHeight
                               : unit->unitDef->radarHeight;
    const float losHeight = unit->midPos.y + emitHeight;
    const int bucketSize = 1 << (mipLevel + 2);
    const float iLosHeight = (int(losHeight) / bucketSize + 0.5f) *
                             bucketSize; // save losHeight in buckets
    return iLosHeight;
}
...
ILosType::UpdateUnit(CUnit* unit)
{
    ...
    //   zone behind
    const bool isPureSight =
      (type == LOS_TYPE_LOS) || (type == LOS_TYPE_AIRLOS);
    ...
    auto IS_FITTING_INSTANCE = [&](SLosInstance* li) -> bool {
        return (li && (li->basePos == baseLos) && (li->baseHeight == height) &&
                (li->radius == radius) && (li->allyteam == allyteam));
    };
...
ILosType::DeleteInstance(SLosInstance* li)
{
    ...
    auto it =
      std::find_if(pair.first, pair.second, [&](decltype(*pair.first)& p) {
          return (p.second == li);
      });
    ...
    if (li->isQueuedForTerraform) {
        auto it = std::find_if(
          delayedTerraQue.begin(),
          delayedTerraQue.end(),
          [&](const DelayedInstance& inst) { return inst.instance == li; });
                               ...
                               SLosInstance::TLosStatus status)
{
    ...
    assert((li->status & status) == 0 ||
           status == SLosInstance::TLosStatus::RECALC);
    li->status |= status;

    // sanity checks (debug only)
    constexpr auto b =
      SLosInstance::TLosStatus::REACTIVATE | SLosInstance::TLosStatus::RECALC;
    assert((li->status & b) != b ||
           (li->status & SLosInstance::TLosStatus::REMOVE));

    constexpr auto c =
      SLosInstance::TLosStatus::NEW | SLosInstance::TLosStatus::RECALC;
    assert((li->status & c) != c);

    constexpr auto d =
      SLosInstance::TLosStatus::NEW | SLosInstance::TLosStatus::REACTIVATE;
    assert((li->status & d) != d);

    constexpr auto e =
      SLosInstance::TLosStatus::NEW | SLosInstance::TLosStatus::REMOVE;
...
{
    constexpr auto a =
      SLosInstance::TLosStatus::REACTIVATE | SLosInstance::TLosStatus::REMOVE;
    if ((li->status & a) == a) {
        assert(li->refCount > 0);
        li->status &= ~a;
    }
    ...
    } else if (li->status & SLosInstance::TLosStatus::REMOVE) {
        assert(li->refCount == 0);
        return SLosInstance::TLosStatus::REMOVE;
    } else if (li->status & SLosInstance::TLosStatus::REACTIVATE) {
        assert(li->refCount > 0);
        return SLosInstance::TLosStatus::REACTIVATE;
    } else if (li->status & SLosInstance::TLosStatus::RECALC) {
        assert(li->refCount > 0);
...
{
    // delayed delete
    while (!delayedDeleteQue.empty() &&
           delayedDeleteQue.front().timeoutTime < gs->frameNum) {
        UnrefInstance(delayedDeleteQue.front().instance);
        delayedDeleteQue.pop_front();
    }

    // relos after terraform is delayed
    while (!delayedTerraQue.empty() &&
           delayedTerraQue.front().timeoutTime < gs->frameNum) {
    ...
    if (algoType == LOS_ALGO_RAYCAST) {
        while (!losCache.empty() &&
               ((losCache.size() + losDeleted.size()) > CACHE_SIZE)) {
...
ILosType::UpdateHeightMapSynced(SRectangle rect)
{
    ...
    auto CheckOverlap = [&](SLosInstance* li, SRectangle rect) -> bool {
        ...
        return (Square(circleDistance.x) + Square(circleDistance.y)) <=
               Square(radius);
    };
...
CLosHandler::~CLosHandler()
{
    ...
    for (ILosType* lt: losTypes) {
            ...
            for (SLosInstance& li: lt->instances) {
                    memUsage += li.squares.capacity() *
    sizeof(SLosInstance::RLE);
            }
    ...
    LOG_L(L_WARNING, "LosHandler MemUsage: ~%.1fMB", memUsage / (1024.f *
    1024.f));*/

    LOG(
      "LosHandler stats: total instances=%u; shared=%.0f%%; from cache=%.0f%%",
      unsigned(ILosType::cacheHits + ILosType::cacheFails),
      100.f * float(ILosType::cacheHits - ILosType::cacheReactivated) /
        (ILosType::cacheHits + ILosType::cacheFails),
      100.f * float(ILosType::cacheReactivated) /
        (ILosType::cacheHits + ILosType::cacheFails));
}
...
CLosHandler::InLos(const CUnit* unit, int allyTeam) const
{
    ...
    if (unit->useAirLos)
        return (InAirLos(unit->pos, allyTeam) ||
                InAirLos(unit->pos + unit->speed, allyTeam));
    ...
    return (InLos(unit->pos, allyTeam) ||
            InLos(unit->pos + unit->speed, allyTeam));
}
...
CLosHandler::InRadar(const float3 pos, int allyTeam) const
{
    if (pos.y < 0.0f) {
        ...
        // note: only check jammers when we have a common jammer map, else
        // jammers only apply to objects!
        return (sonar.InSight(pos, allyTeam) &&
                !(!modInfo.separateJammers && sonarJammer.InSight(pos, 0)));
    }

    return (radar.InSight(pos, allyTeam) &&
            !(!modInfo.separateJammers && jammer.InSight(pos, 0)));
}
...
CLosHandler::InRadar(const CUnit* unit, int allyTeam) const
{
    if (unit->IsUnderWater()) {
        ...
        return (sonar.InSight(unit->pos, allyTeam) &&
