...
void CQuadField::Resize(int quad_size)
{
        ...
        CQuadField* newQuadField = new CQuadField(int2(mapDims.mapx,
mapDims.mapy), quad_size);

        quadField = NULL;

        for (int zq = 0; zq < oldQuadField->GetNumQuadsZ(); zq++) {
                for (int xq = 0; xq < oldQuadField->GetNumQuadsX(); xq++) {
                        const CQuadField::Quad& quad =
oldQuadField->GetQuadAt(xq, zq);

                        // COPY the object lists because the Remove* functions
modify them
                        // NOTE:
                        //   teamUnits is updated internally by RemoveUnit and
MovedUnit
                        //
                        //   if a unit exists in multiple quads in the old
field, it will
                        //   be removed from all of them and there is no danger
of double
                        //   re-insertion (important if new grid has higher
resolution) const std::list<CUnit*      > units       = quad.units; const
std::list<CFeature*   > features    = quad.features; const
std::list<CProjectile*> projectiles = quad.projectiles;

                        for (std::list<CUnit*>::const_iterator it =
units.begin(); it != units.end(); ++it) { oldQuadField->RemoveUnit(*it);
                                newQuadField->MovedUnit(*it); // handles
addition
                        }

                        for (std::list<CFeature*>::const_iterator it =
features.begin(); it != features.end(); ++it) {
                                oldQuadField->RemoveFeature(*it);
                                newQuadField->AddFeature(*it);
                        }

                        for (std::list<CProjectile*>::const_iterator it =
projectiles.begin(); it != projectiles.end(); ++it) {
...
{
    return int2(Clamp(int(p.x / quadSizeX), 0, numQuadsX - 1),
...
{
    return Clamp(int(p.z / quadSizeZ), 0, numQuadsZ - 1) * numQuadsX +
...
CQuadField::GetQuads(float3 pos, float radius)
{
    ...
    const float maxSqLength =
      (radius + quadSizeX * 0.72f) * (radius + quadSizeZ * 0.72f);

    for (int z = min.y; z <= max.y; ++z) {
        for (int x = min.x; x <= max.x; ++x) {
            assert(x < numQuadsX);
            assert(z < numQuadsZ);
            const float3 quadPos = float3(x * quadSizeX + quadSizeX * 0.5f,
                                          0,
                                          z * quadSizeZ + quadSizeZ * 0.5f);
...
CQuadField::GetQuadsOnRay(const float3& start, const float3& dir, float length)
{
    ...
    const bool noXdir = (math::floor(start.x * invQuadSize.x) ==
                         math::floor(to.x * invQuadSize.x));
    const bool noZdir = (math::floor(start.z * invQuadSize.z) ==
                         math::floor(to.z * invQuadSize.z));
    ...
    if (noZdir) {
        ...
        const int row =
          Clamp<int>(start.z * invQuadSize.z, 0, numQuadsZ - 1) * numQuadsX;
    ...
    // all other
    // hint at code: we iterate the affected z-range and then for each z we
    // compute what xs are touched
    ...
    for (int z = startZ; z <= finalZ; z++) {
        ...
        if ((startZuc < 0 && z == 0) ||
            (startZuc >= numQuadsZ && z == finalZ)) {
            t0 = ((startZuc)*quadSizeZ - start.z) * invDirZ;
        }
        if ((finalZuc < 0 && z == 0) ||
            (finalZuc >= numQuadsZ && z == finalZ)) {
        ...
        unsigned startX =
          Clamp<int>((dir.x * t0 + start.x) * invQuadSize.x, 0, numQuadsX - 1);
        unsigned finalX =
          Clamp<int>((dir.x * t1 + start.x) * invQuadSize.x, 0, numQuadsX - 1);
        if (finalX < startX)
            std::swap(startX, finalX);
        assert(finalX < numQuadsX);
...
CQuadField::MovedUnit(CUnit* unit)
{
    ...
    for (const int qi : newQuads) {
        ...
        VectorInsertUnique(
          baseQuads[qi].teamUnits[unit->allyteam], unit, false);
    }
...
{
    auto newQuads =
      std::move(GetQuads(repulser->weaponMuzzlePos, repulser->GetRadius()));

    // compare if the quads have changed, if not stop here
    if (newQuads.size() == repulser->quads.size()) {
        if (std::equal(
              newQuads.begin(), newQuads.end(), repulser->quads.begin())) {
            return;
        }
    }
...
CQuadField::GetUnitsExact(const float3& pos, float radius, bool spherical)
{
    ...
    for (const int qi : quads) {
        for (CUnit* u : baseQuads[qi].units) {
            ...
            const float posUnitDstSq =
              spherical ? pos.SqDistance(u->pos) : pos.SqDistance2D(u->pos);
...
CQuadField::GetFeaturesExact(const float3& pos, float radius, bool spherical)
{
    ...
    for (const int qi : quads) {
        for (CFeature* f : baseQuads[qi].features) {
            ...
            const float totRadSq = totRad * totRad;
            const float posDstSq =
              spherical ? pos.SqDistance(f->pos) : pos.SqDistance2D(f->pos);
                                      ...
                                      std::vector<CPlasmaRepulser*>* repulsers)
{
    ...
    for (const int qi : quads) {
        ...
        for (CUnit* u : quad.units) {
            ...
            if (pos.SqDistance(colvol->GetWorldSpacePos(u)) >=
                (totRad * totRad))
                continue;
        ...
        for (CFeature* f : quad.features) {
            ...
            if (pos.SqDistance(colvol->GetWorldSpacePos(f)) >=
                (totRad * totRad))
                continue;
