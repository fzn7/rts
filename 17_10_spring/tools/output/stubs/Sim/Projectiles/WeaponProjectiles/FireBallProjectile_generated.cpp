...
#define ept projectileDrawer->explotex
        va->AddVertexQTC(sparks[i].pos - camera->GetRight() * sparks[i].size -
                           camera->GetUp() * sparks[i].size,
                         ept->xstart,
                         ept->ystart,
                         col);
        va->AddVertexQTC(sparks[i].pos + camera->GetRight() * sparks[i].size -
...
#undef ept
    ...
    for (int i = 0; i < numFire;
         i++) //! CAUTION: loop count must match EnlargeArrays above
    {
        col[0] = (maxCol - i) * 25;
        col[1] = (maxCol - i) * 15;
        col[2] = (maxCol - i) * 10;
#define dgt projectileDrawer->dguntex
        va->AddVertexQTC(interPos - camera->GetRight() * size -
                           camera->GetUp() * size,
                         dgt->xstart,
                         dgt->ystart,
                         col);
...
CFireBallProjectile::Update()
{
    ...
    explGenHandler->GenExplosion(cegID,
                                 ...
                                 speed,
                                 ttl,
                                 !sparks.empty() ? sparks[0].size : 0.0f,
                                 0.0f,
                                 NULL,
                                 NULL);
    UpdateGroundBounce();
