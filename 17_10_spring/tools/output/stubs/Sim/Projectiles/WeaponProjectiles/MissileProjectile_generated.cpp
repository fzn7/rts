...
CMissileProjectile::Update()
{
        ...
        if (!luaMoveCtrl) {
            ...
            if (weaponDef->tracks && target != NULL) {
                const CSolidObject* so =
                  dynamic_cast<const CSolidObject*>(target);
                ...
                if (so != nullptr) {
                    ...
                    const CWeaponProjectile* po =
                      dynamic_cast<const CWeaponProjectile*>(target);
                    if (po != nullptr)
                        targetVel = po->speed;
                }
            }
            ...
            const float3 targetLeadDir =
              (targetPos + targetLeadVec - pos).Normalize();
            ...
            if (targetDirDif.SqLength() < Square(weaponDef->turnrate)) {
                ...
                targetDirDif = (targetDirDif - (dir * (targetDirDif.dot(dir))))
                                 .SafeNormalize();
                dir =
                  (dir + (targetDirDif * weaponDef->turnrate)).SafeNormalize();
            }
    ...
    if (weaponDef->visuals.smokeTrail) {
        ...
        if ((age % 8) == 0) {
            smokeTrail = new CSmokeTrailProjectile(own,
                                                   pos,
                                                   oldSmoke,
                                                   dir,
                                                   oldDir,
...
CMissileProjectile::UpdateWobble()
{
    ...
    if (owner() != NULL)
        wobbleFact *= CUnit::ExperienceScale(owner()->limExperience,
...
CMissileProjectile::Draw()
{
    ...
    va->AddVertexQTC(drawPos - camera->GetRight() * fsize -
                       camera->GetUp() * fsize,
                                  ...
                                  float shieldMaxSpeed)
{
    ...
    if (ttl > 0) {
        ...
        const float tracking =
          std::max(shieldForce * 0.05f, weaponDef->turnrate * 2);
