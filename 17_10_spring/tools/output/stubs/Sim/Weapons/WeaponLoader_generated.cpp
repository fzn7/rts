...
CWeaponLoader::LoadWeapon(CUnit* owner, const UnitDefWeapon* defWeapon)
{
    ...
    if (StringToLower(weaponDef->name) == "noweapon") {
        ...
        weapon = new CNoWeapon(owner, weaponDef);
        LOG_L(L_ERROR,
              "weapon-type %s unknown, using noweapon",
              weaponType.c_str());
    }
                          ...
                          const UnitDefWeapon* defWeapon)
{
    ...
    if (defWeapon->slavedTo > 0 &&
        defWeapon->slavedTo <= owner->weapons.size()) {
    ...
    if (!weaponDef->avoidNeutral)
        weapon->avoidFlags |= Collision::NONEUTRALS;
    if (!weaponDef->avoidFriendly)
        weapon->avoidFlags |= Collision::NOFRIENDLIES;
    if (!weaponDef->avoidFeature)
        weapon->avoidFlags |= Collision::NOFEATURES;
    if (!weaponDef->avoidGround)
        weapon->avoidFlags |= Collision::NOGROUND;
