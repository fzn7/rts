...
CLaserCannon::UpdateRange(const float val)
{
    ...
    range = std::max(1.0f, std::floor(val / weaponDef->projectilespeed)) *
...
CLaserCannon::FireImpl(const bool scriptCall)
{
    ...
    if (onlyForward && owner->unitDef->IsStrafingAirUnit()) {
        // [?] StrafeAirMovetype cannot align itself properly, change back when
        // that is fixed
        dir = owner->frontdir;
    }
    ...
    params.ttl = mix(std::max(ttlreq, ttlmax),
                     std::min(ttlreq, ttlmax),
                     weaponDef->selfExplode);
