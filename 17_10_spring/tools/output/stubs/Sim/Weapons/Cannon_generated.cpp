...
CCannon::Init()
{
    gravity = (weaponDef->myGravity == 0) ? mapInfo->map.gravity
                                          : -(weaponDef->myGravity);
...
void
CCannon::UpdateRange(const float val)
{
...
bool
CCannon::HaveFreeLineOfFire(const float3 pos,
                            ...
                            bool useMuzzle) const
{
    ...
    const float quadratic =
      gravity / (projectileSpeed * projectileSpeed) * 0.5f;
    const float groundDist =
      ((avoidFlags & Collision::NOGROUND) == 0)
        ? CGround::TrajectoryGroundCol(
            weaponMuzzlePos, flatDir, flatLength - 10, linear, quadratic)
        : -1.0f;
    const float spread =
      (AccuracyExperience() + SprayAngleExperience()) * 0.6f * 0.9f;

    if (groundDist > 0.0f) {
        return false;
    }

    // FIXME add a forcedUserTarget (a forced fire mode enabled with meta key or
    // something) and skip the test below then
    if (TraceRay::TestTrajectoryCone(weaponMuzzlePos,
                                     flatDir,
                                     flatLength,
                                     dir.y,
                                     quadratic,
...
void
CCannon::FireImpl(const bool scriptCall)
{
    float3 diff = currentTargetPos - weaponMuzzlePos;
    float3 dir =
      (diff.SqLength() > 4.0f)
        ? GetWantedDir(diff)
        : diff; // prevent vertical aim when emit-sfx firing the weapon

    dir += (gs->randVector() * SprayAngleExperience() + SalvoErrorExperience());
    dir.SafeNormalize();

    int ttl = 0;
    const float sqSpeed2D =
      dir.SqLength2D() * projectileSpeed * projectileSpeed;
    const int predict = math::ceil(
      (sqSpeed2D == 0.0f) ? (-2.0f * projectileSpeed * dir.y / gravity)
...
CCannon::SlowUpdate()
{
    if (weaponDef->highTrajectory == 2 &&
        owner->useHighTrajectory != highTrajectory) {
...
float3
CCannon::GetWantedDir(const float3& diff)
{
    // try to cache results, sacrifice some (not much too much even for a pewee)
    // accuracy it saves a dozen or two expensive calculations per second when 5
    // guardians are shooting at several slow- and fast-moving targets
...
float3
CCannon::GetWantedDir2(const float3& diff) const
{
    ...
    if (Dsq == 0.0f) {
        ...
        if (Dsq < 1e12f && math::fabs(dy) < 1e6f) {
            const float root1 =
              v * v * v * v + 2.0f * v * v * g * dy - g * g * DFsq;

            if (root1 >= 0.0f) {
                const float root2 =
                  2.0f * DFsq * Dsq *
                  (v * v + g * dy +
                   (highTrajectory ? -1.0f : 1.0f) * math::sqrt(root1));

                if (root2 >= 0.0f) {
                    Vxz = math::sqrt(root2) / (2.0f * Dsq);
                    Vy = (dxz == 0.0f || Vxz == 0.0f)
                           ? v
                           : (Vxz * dy / dxz - dxz * g / (2.0f * Vxz));
                }
            }
        }
    }
...
float
CCannon::GetRange2D(float yDiff, float rFact) const
{
    const float speedFactor = 0.7071067f; // sin pi/4 == cos pi/4
    const float smoothHeight = 100.0f;    // completely arbitrary
    const float speed2d =
      projectileSpeed * speedFactor; // speed in one direction in max-range case
...
float
CCannon::GetRange2D(const float yDiff) const
{
