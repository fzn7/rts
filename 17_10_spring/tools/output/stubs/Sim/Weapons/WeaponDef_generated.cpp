...
#define WEAPONTAG(T, name, ...)                                                \
    DEFTAG(WeaponDefs, WeaponDef, T, name, ##__VA_ARGS__)
...
WEAPONTAG(float, weaponacceleration)
  ...
  .defaultValue(0.0f)
  .scaleValue(1.0f / (GAME_SPEED * GAME_SPEED));
...
WEAPONTAG(bool, turret)
  .defaultValue(false)
  .description("Does the unit aim within an arc (up-to and including full 360° "
               "turret traverse) or always aim along the owner's heading?");
...
WEAPONDUMMYTAG(float, maxFireAngle)
  ...
  .scaleValue(TAANG2RAD); // default value is 20degree
...
WEAPONTAG(float, wobble)
  .defaultValue(0.0f)
  .scaleValue(float(TAANG2RAD) / GAME_SPEED);
...
WEAPONDUMMYTAG(float, flighttime)
  ...
  .description("Flighttime of missiles in seconds."); // needs to be written as
                                                      // int and read as float
...
WEAPONTAG(bool, allowNonBlockingAim)
  .defaultValue(false)
  .description(
    "When false, the weapon is blocked from fireing till AimWeapon() returns.");

// Target Error
TAGFUNCTION(AccuracyToSin,
            float,
            math::sin(x* PI / 0xafff)) // should really be tan but TA seem to
                                       // cap it somehow, should also be 7fff or
                                       // ffff theoretically but neither seems
                                       // good
...
WEAPONTAG(float, minIntensity)
  .defaultValue(0.0f)
  .description("The minimum percentage the weapon's damage can fall-off to "
               "over its range. Setting to 1.0 will disable fall off entirely. "
               "Unrelated to the visual-only intensity tag.");
...
WEAPONTAG(bool, sweepFire)
  .defaultValue(false)
  .description("Makes BeamLasers continue firing while aiming for a new "
               "target, 'sweeping' across the terrain.");
...
WEAPONTAG(float, fireStarter)
  ...
  .maximumValue(100.0f)
  .scaleValue(0.01f)
  .description("The percentage chance of the weapon setting fire to static map "
               "features on impact.");
WEAPONTAG(bool, paralyzer)
  .defaultValue(false)
  .description("Is the weapon a paralyzer? If true the weapon only stuns enemy "
               "units and does not cause damage in the form of lost "
               "hit-points.");
WEAPONTAG(int, paralyzeTime, damages.paralyzeDamageTime)
  .defaultValue(10)
  .minimumValue(0)
  .description("Determines the maximum length of time in seconds that the "
               "target will be paralyzed. The timer is restarted every time "
               "the target is hit by the weapon. Cannot be less than 0.");
WEAPONTAG(bool, stockpile)
  .defaultValue(false)
  .description("Does each round of the weapon have to be built and stockpiled "
               "by the player? Will only correctly function for the first of "
               "each stockpiled weapons a unit has.");
...
WEAPONTAG(int, targetable)
  .defaultValue(0)
  .description("Bitmask representing the types of weapon that can intercept "
               "this weapon. Each digit of binary that is set to one means "
               "that a weapon with the corresponding digit in its interceptor "
               "tag will intercept this weapon. Instant-hitting weapons such "
               "as [#BeamLaser], [#LightningCannon] and [#Rifle] cannot be "
               "targeted.");
WEAPONTAG(int, interceptor)
  .defaultValue(0)
  .description("Bitmask representing the types of weapons that this weapon can "
               "intercept. Each digit of binary that is set to one means that "
               "a weapon with the corresponding digit in its targetable tag "
               "will be intercepted by this weapon.");
WEAPONDUMMYTAG(unsigned, interceptedByShieldType).description("");
WEAPONTAG(float, coverageRange)
  .externalName("coverage")
  .defaultValue(0.0f)
  .description("The radius in elmos within which an interceptor weapon will "
               "fire on targetable weapons.");
...
WEAPONTAG(bool, dynDamageInverted, damages.dynDamageInverted)
  .defaultValue(false)
  .description("If true the damage curve is inverted i.e. the weapon does more "
               "damage at greater ranges as opposed to less.");
WEAPONTAG(float, dynDamageExp, damages.dynDamageExp)
  .defaultValue(0.0f)
  .description("Exponent of the range-dependent damage formula, the default of "
               "0.0 disables dynamic damage, 1.0 means linear scaling, 2.0 "
               "quadratic and so on.");
...
WEAPONTAG(float, dynDamageRange, damages.dynDamageRange)
  .defaultValue(0.0f)
  .description("If set to non-zero values the weapon will use this value in "
               "the range-dependant damage formula instead of the actual "
               "range.");
...
WEAPONTAG(bool, smartShield)
  ...
  .defaultValue(false)
  .description("Determines whether or not projectiles fired by allied units "
               "can pass through the shield (true) or are intercepted as enemy "
               "weapons are (false).");
WEAPONTAG(bool, exteriorShield)
  .externalName("shield.exterior")
  .fallbackName("exteriorShield")
  .defaultValue(false)
  .description("Determines whether or not projectiles fired within the "
               "shield's radius can pass through the shield (true) or are "
               "intercepted (false).");
...
WEAPONTAG(float, shieldForce)
  ...
  .defaultValue(0.0f)
  .description("The force applied by the repulsor to the weapon - higher "
               "values will deflect weapons away at higher velocities.");
...
WEAPONTAG(float, shieldPower)
  ...
  .defaultValue(0.0f)
  .description("Essentially the maximum allowed hit-points of the shield - "
               "reduced by the damage of a weapon upon impact.");
WEAPONTAG(float, shieldStartingPower)
  .externalName("shield.startingPower")
  .fallbackName("shieldStartingPower")
  .defaultValue(0.0f)
  .description("How many hit-points the shield starts with - otherwise the "
               "shield must regenerate from 0 until it reaches maximum power.");
...
WEAPONTAG(float, shieldEnergyUse)
  ...
  .defaultValue(0.0f)
  .description("The amount of the energy resource consumed by the shield to "
               "absorb or repulse weapons, continually drained by a repulsor "
               "as long as the projectile is in range.");
WEAPONDUMMYTAG(float, shieldRechargeDelay)
  .externalName("rechargeDelay")
  .fallbackName("shieldRechargeDelay")
  .defaultValue(0)
  .scaleValue(GAME_SPEED)
  .description("The delay in seconds before a shield begins to regenerate "
               "after it is hit."); // must be read as float
WEAPONTAG(unsigned int, shieldInterceptType)
  .externalName("shield.interceptType")
  .fallbackName("shieldInterceptType")
  .defaultValue(0)
  .description("Bitmask representing the types of weapons that this shield can "
               "intercept. Each digit of binary that is set to one means that "
               "a weapon with the corresponding digit in its "
               "interceptedByShieldType will be intercepted by this shield "
...
WEAPONTAG(float3, shieldBadColor)
  .externalName("shield.badColor")
  .fallbackName("shieldBadColor")
  .defaultValue(float3(1.0f, 0.5f, 0.5f))
  .description("The RGB colour the shield transitions to as its hit-points are "
               "reduced towards 0.");
WEAPONTAG(float3, shieldGoodColor)
  .externalName("shield.goodColor")
  .fallbackName("shieldGoodColor")
  .defaultValue(float3(0.5f, 0.5f, 1.0f))
  .description("The RGB colour the shield transitions to as its hit-points are "
               "regenerated towards its maximum power.");
...
WEAPONTAG(std::string, shieldArmorTypeName)
  ...
  .description("Specifies the armorclass of the shield; you can input either "
               "an armorclass name OR a unitdef name to share that unit's "
               "armorclass");
...
WEAPONTAG(float, intensity)
  .defaultValue(0.9f)
  .description("Alpha transparency for non-model projectiles. Lower values are "
               "more opaque, but 0.0 will cause the projectile to disappear "
               "entirely.");
...
WEAPONTAG(std::string, cegTag, visuals.ptrailExpGenTag)
  .defaultValue("")
  .description("The name, without prefixes, of a CEG to be emitted by the "
               "projectile each frame.");
  ...
  , collisionFlags(0)
{
    ...
    if (wdTable.KeyExists("cylinderTargetting"))
        LOG_L(L_WARNING,
              "WeaponDef (%s) cylinderTargetting is deprecated and will be "
              "removed in the next release (use cylinderTargeting).",
              name.c_str());

    if (wdTable.KeyExists("color1") || wdTable.KeyExists("color2"))
        LOG_L(L_WARNING,
              "WeaponDef (%s) color1 & color2 (= hue & sat) are removed. Use "
              "rgbColor instead!",
              name.c_str());

    if (wdTable.KeyExists("isShield"))
        LOG_L(L_WARNING,
              "WeaponDef (%s) The \"isShield\" tag has been removed. Use the "
              "weaponType=\"Shield\" tag instead!",
              name.c_str());

    shieldRechargeDelay =
      int(wdTable.GetFloat("rechargeDelay", 0) * GAME_SPEED);
    ...
    maxFireAngle =
      math::cos(wdTable.GetFloat("firetolerance", 3640.0f) * TAANG2RAD);

    // FIXME may be smarter to merge the collideXYZ tags with avoidXYZ and
    // removing the collisionFlags tag (and move the code into CWeapon)?
    collisionFlags = 0;
    if (!wdTable.GetBool("collideEnemy", true)) {
        collisionFlags |= Collision::NOENEMIES;
    }
    if (!wdTable.GetBool("collideFriendly", true)) {
        collisionFlags |= Collision::NOFRIENDLIES;
    }
    if (!wdTable.GetBool("collideFeature", true)) {
        collisionFlags |= Collision::NOFEATURES;
    }
    if (!wdTable.GetBool("collideNeutral", true)) {
        collisionFlags |= Collision::NONEUTRALS;
    }
    if (!wdTable.GetBool("collideGround", true)) {
        collisionFlags |= Collision::NOGROUND;
    }
    ...
    // FIXME defaults depend on other tags
    {
        ...
        if (type == "Melee") {
            targetBorder =
              Clamp(wdTable.GetFloat("targetBorder", 1.0f), -1.0f, 1.0f);
            cylinderTargeting = Clamp(
              wdTable.GetFloat("cylinderTargeting",
                               wdTable.GetFloat("cylinderTargetting", 1.0f)),
              0.0f,
              128.0f);
        }

        if (type == "Flame") {
            // FIXME move to lua (for all other weapons this tag is named
            // `duration` and has a different default)
    ...
    // setup the default damages
    {
        ...
        const float tempsize =
          2.0f +
          std::min(defDamage * 0.0025f, damages.damageAreaOfEffect * 0.1f);
        const float gd = std::max(30.0f, defDamage / 20.0f);
        const float defExpSpeed =
          (8.0f + (gd * 2.5f)) / (9.0f + (math::sqrt(gd) * 0.7f)) * 0.5f;

        size = wdTable.GetFloat("size", tempsize);
        damages.explosionSpeed =
          wdTable.GetFloat("explosionSpeed", defExpSpeed);
...

    {
        ...
        soundTrigger =
          wdTable.GetBool("soundTrigger", singleSampleShot || singleShotWeapon);
    }
    ...
    if (type == "Cannon") {
        ...
        projectileType = largeBeamLaser ? WEAPON_LARGEBEAMLASER_PROJECTILE
                                        : WEAPON_BEAMLASER_PROJECTILE;
        defInterceptType = 2;
    ...
    if (ownerExpAccWeight < 0.0f)
        LOG_L(L_ERROR,
              "ownerExpAccWeight is negative in weaponDef %s",
              name.c_str());

    interceptedByShieldType =
      wdTable.GetInt("interceptedByShieldType", defInterceptType);
...
void
WeaponDef::ParseWeaponSounds(const LuaTable& wdTable)
{
    ...
    const bool forceSetVolume = (fireSound.getVolume(0) == -1.0f) ||
    ...
    if (damages.GetDefault() <= 50.0f) {
        ...
        float fireSoundVolume = math::sqrt(damages.GetDefault() * 0.5f) *
                                ((type == "LaserCannon") ? 0.5f : 1.0f);
...
void
WeaponDef::LoadSound(const LuaTable& wdTable,
                     ...
                     std::vector<GuiSoundSet::Data>& soundData)
{
    ...
    if (soundKey == "soundStart") {
        ...
        volume = wdTable.GetFloat(soundKey + "Volume",
                                  wdTable.GetFloat("soundHitVolume", -1.0f));
...
WeaponDef::LoadModel()
{
    if (visuals.model == NULL) {
        if (!visuals.modelName.empty()) {
            ...
            // LOG_L(L_WARNING, "[WeaponDef::%s] weapon \"%s\" has no model
            // defined", __FUNCTION__, name.c_str());
        }
    }
...
S3DModel*
WeaponDef::LoadModel() const
{
    // not very sweet, but still better than replacing "const WeaponDef"
    // _everywhere_
