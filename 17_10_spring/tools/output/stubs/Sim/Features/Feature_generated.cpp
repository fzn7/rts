...
CFeature::IsInLosForAllyTeam(int argAllyTeam) const
{
    ...
    switch (modInfo.featureVisibility) {
        ...
        case CModInfo::FEATURELOS_GAIAALLIED:
            return (isGaia || allyteam == argAllyTeam ||
                    losHandler->InLos(pos, argAllyTeam));
...
CFeature::Initialize(const FeatureLoadParams& params)
{
    ...
    moveCtrl.SetVelocityMask(mix(
      OnesVector, UpVector, udef == nullptr && def->drawType < DRAWTYPE_TREE));
    ...
    switch (def->drawType) {
        ...
        default: {
            ...
            SetMidAndAimPos(
              UpVector * TREE_RADIUS, UpVector * TREE_RADIUS, true);
    ...
    if (collisionVolume.DefaultToFootPrint())
        collisionVolume.InitBox(
          float3(xsize * SQUARE_SIZE, height, zsize * SQUARE_SIZE));
...
CFeature::AddBuildPower(CUnit* builder, float amount)
{
    ...
    if (amount > 0.0f) {
        ...
        isRepairingBeforeResurrect =
          true; // Stop them exploiting chunk reclaiming
        ...
        // Work out how much to try to put back, based on the speed this unit
        // would reclaim at.
        ...
        const bool canExecRepair = (builderTeam->res.metal >= metalUse &&
                                    builderTeam->res.energy >= energyUse);
        const bool repairAllowed =
          !canExecRepair
            ? false
            : eventHandler.AllowFeatureBuildStep(builder, this, step);
        ...
        if (repairAllowed) {
            ...
            if (reclaimLeft >= 1.0f) {
                isRepairingBeforeResurrect =
                  false; // They can start reclaiming it again if they so wish
                reclaimLeft = 1;
            } else if (reclaimLeft <= 0.0f) {
                // this can happen when a mod tampers the feature in
    ...
    const float metalFraction =
      std::min(def->metal * fractionReclaimed, resources.metal);
    const float energyFraction =
      std::min(def->energy * fractionReclaimed, resources.energy);
    const float energyUseScaled =
      metalFraction * modInfo.reclaimFeatureEnergyCostFactor;
    ...
    if (reclaimLeftTemp == 0.0f) {
        ...
        if (oldChunk != newChunk) {
            ...
            order.add.metal =
              std::min(numChunks * def->metal * chunkSize, resources.metal);
            order.add.energy =
              std::min(numChunks * def->energy * chunkSize, resources.energy);
        }
    }
                   ...
                   int projectileID)
{
    ...
    float impulseMult = float((def->drawType >= DRAWTYPE_TREE) ||
                              (udef != NULL && !udef->IsImmobileUnit()));

    if (eventHandler.FeaturePreDamaged(this,
                                       attacker,
                                       baseDamage,
                                       weaponDefID,
                                       projectileID,
    ...
    eventHandler.FeatureDamaged(
      this, attacker, baseDamage, weaponDefID, projectileID);

    if (health <= 0.0f && def->destructable) {
        FeatureLoadParams params = { featureDefHandler->GetFeatureDefByID(
                                       def->deathFeatureDefID),
                                     NULL,
                                     pos,
                                     speed,
                                     -1,
                                     ...
                                     buildFacing,
                                     0 };
        CFeature* deathFeature =
          featureHandler->CreateWreckage(params, 0, false);
        ...
        if (deathFeature != NULL) {
            ...
            deathFeature->resources.metal *=
              (def->metal != 0.0f) ? resources.metal / def->metal : 1.0f;
            deathFeature->resources.energy *=
              (def->energy != 0.0f) ? resources.energy / def->energy : 1.0f;
        }
...
CFeature::UpdateTransformAndPhysState()
{
    ...
    UpdatePhysicalStateBit(CSolidObject::PSTATE_BIT_MOVING,
                           (SetSpeed(speed) != 0.0f));
                         ...
                         const float3& velMask)
{
    ...
    if (!IsInWater()) {
        ...
        CWorldObject::SetVelocity(
          ((speed * XZVector) + gravAccel * (1 - def->floating)) * velMask);
    }
    ...
    if ((pos.y + speed.y) <= newGroundHeight) {
        speed.y = std::min(newGroundHeight - pos.y,
...
CFeature::UpdatePosition()
{
    ...
    if (moveCtrl.enabled) {
        ...
        UpdateQuadFieldPosition(speed =
                                  (moveCtrl.velVector += moveCtrl.accVector));
    } else {
        const float3 dragAccel =
          GetDragAccelerationVec(float4(mapInfo->atmosphere.fluidDensity,
        ...
        // horizontal movement
        if (UpdateVelocity(dragAccel,
                           gravAccel,
                           moveCtrl.movementMask,
                           moveCtrl.velocityMask))
        ...
        Move(UpVector *
               (std::max(CGround::GetHeightReal(pos.x, pos.z), pos.y) - pos.y),
             true);
    ...
    if (!pos.equals(oldPos,
                    float3(float3::cmp_eps(), 0.0f, float3::cmp_eps()))) {
        eventHandler.FeatureMoved(this, oldPos);
        return true;
    }
...
CFeature::Update()
{
    ...
    if (smokeTime != 0) {
        if (!((gs->frameNum + id) & 3) &&
            projectileHandler->GetParticleSaturation() < 0.7f) {
            new CSmokeProjectile(NULL,
                                 midPos + gu->RandVector() * radius * 0.3f,
                                 gu->RandVector() * 0.3f + UpVector,
                                 smokeTime / 6 + 20,
                                 6,
                                 0.4f,
                                 0.5f);
...
CFeature::StartFire()
{
    ...
    myFire =
      new CFireProjectile(midPos, UpVector, 0, 300, 70, radius * 0.8f, 20.0f);
}
...
CFeature::EmitGeoSmoke()
{
    if ((gs->frameNum + id % 5) % 5 == 0) {
        ...
        const vector<CSolidObject*>& objs = quadField->GetSolidsExact(
          pos, 0.0f, 0xFFFFFFFF, CSolidObject::CSTATE_BIT_SOLIDOBJECTS);
    ...
    if (u == NULL || !u->unitDef->needGeo) {
        ...
        if (projectileHandler->GetParticleSaturation() < partSat) {
            const float3 pPos =
              gu->RandVector() * 10.0f + float3(pos.x, pos.y - 10.0f, pos.z);
            const float3 pSpeed = (gu->RandVector() * 0.5f) + (UpVector * 2.0f);

            new CGeoThermSmokeProjectile(
              pPos, pSpeed, int(50 + gu->RandFloat() * 7), this);
        }
    }
}
...
{
    return (id + unitHandler->MaxUnits());
}
