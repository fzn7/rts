...
CUnit::~CUnit()
{
    ...
    if (delayedWreckLevel >= 0) {
        ...
        FeatureLoadParams params = { featureDefHandler->GetFeatureDefByID(
                                       featureDefID),
                                     unitDef,
                                     pos,
                                     deathSpeed,
...
CUnit::InitStatic()
{
    ...
    //! SlowUpdate runs every 16th simframe (a second has GAME_SPEED=30
    //! gameframes!)
    empDeclineRate =
      2.0f * (float)UNIT_SLOWUPDATE_RATE / (float)GAME_SPEED / 40.0f;
    expGrade = 0.0f;
...
CUnit::PreInit(const UnitLoadParams& params)
{
    ...
    {
        const FeatureDef* wreckFeatureDef =
          featureDefHandler->GetFeatureDef(unitDef->wreckName);
        if (wreckFeatureDef != nullptr) {
            featureDefID = wreckFeatureDef->id;

            while (wreckFeatureDef != nullptr) {
                wreckFeatureDef->PreloadModel();
                wreckFeatureDef = featureDefHandler->GetFeatureDefByID(
                  wreckFeatureDef->deathFeatureDefID);
            }
        }
    }
    ...
    if (collisionVolume.DefaultToFootPrint())
        collisionVolume.InitBox(
          float3(xsize * SQUARE_SIZE, model->height, zsize * SQUARE_SIZE));
    ...
    losStatus[allyteam] =
      LOS_ALL_MASK_BITS | LOS_INLOS | LOS_INRADAR | LOS_PREVLOS | LOS_CONTRADAR;

#ifdef TRACE_SYNC
    tracefile << "[" << __FUNCTION__ << "] id: " << id
              << ", name: " << unitDef->name << " ";
    ...
    blockMap =
      (unitDef->GetYardMap().empty()) ? NULL : &unitDef->GetYardMap()[0];
    ...
    armoredMultiple = std::max(
      0.0001f,
      unitDef->armoredMultiple); // armored multiple of 0 will crash spring
    ...
    flankingBonusAvgDamage =
      (unitDef->flankingBonusMax + unitDef->flankingBonusMin) * 0.5f;
    flankingBonusDifDamage =
      (unitDef->flankingBonusMax - unitDef->flankingBonusMin) * 0.5f;

    useHighTrajectory = (unitDef->highTrajectoryType == 1);

    energyTickMake = unitDef->energyMake +
    ...
    if (unitDef->selfdExpWeaponDef != nullptr)
        selfdExpDamages =
          DynDamageArray::IncRef(&unitDef->selfdExpWeaponDef->damages);
    if (unitDef->deathExpWeaponDef != nullptr)
        deathExpDamages =
          DynDamageArray::IncRef(&unitDef->deathExpWeaponDef->damages);
}
...
CUnit::PostInit(const CUnit* builder)
{
    ...
    UpdateCollidableStateBit(CSolidObject::CSTATE_BIT_SOLIDOBJECTS,
                             unitDef->collidable &&
                               (!immobile || !unitDef->canKamikaze));
    Block();
    ...
    if (unitDef->floatOnWater && IsInWater()) {
        Move(UpVector * (std::max(CGround::GetHeightReal(pos.x, pos.z),
                                  -unitDef->waterline) -
                         pos.y),
             true);
    }
    ...
    if (commandAI->CanChangeFireState()) {
        if (unitDef->fireState <= FIRESTATE_NONE) {
            if (builder != NULL &&
                dynamic_cast<CFactoryCAI*>(builder->commandAI) != NULL) {
...
CUnit::FinishedBuilding(bool postInit)
{
    ...
    if (unitDef->isFeature && CUnit::spawnFeature) {
        FeatureLoadParams p = { featureDefHandler->GetFeatureDefByID(
                                  featureDefID),
                                NULL,
                                pos,
                                ZeroVector,
    ...
    for (TransportedUnit& tu : transportedUnits) {
        ...
        if (!unitDef->releaseHeld) {
            if (!selfDestruct) {
                ...
                transportee->DoDamage(DamageArray(1e6f),
                                      ZeroVector,
                                      NULL,
                                      -DAMAGE_EXTSOURCE_KILLED,
                                      -1);
            }
            ...
            if (transportee->pos.y <
                CGround::GetHeightReal(transportee->pos.x,
                                       transportee->pos.z)) {
                ...
                if (transportee->unitDef->IsGroundUnit()) {
                    transportee->SetPhysicalStateBit(
                      CSolidObject::PSTATE_BIT_FLYING);
                    transportee->SetPhysicalStateBit(
                      CSolidObject::PSTATE_BIT_SKIDDING);
                }
            }
            ...
            if (unitDef->canfly && transportee->unitDef->canmove) {
                transportee->commandAI->GiveCommand(
                  Command(CMD_MOVE, transportee->pos));
            }

            transportee->SetStunned(transportee->paralyzeDamage >
                                    (modInfo.paralyzeOnMaxHealth
                                       ? transportee->maxHealth
                                       : transportee->health));
            transportee->SetVelocityAndSpeed(speed *
                                             (0.5f + 0.5f * gs->randFloat()));
...
CUnit::GetErrorVector(int argAllyTeam) const
{
    ...
    if (teamHandler->Ally(argAllyTeam, allyteam) ||
        (losStatus[argAllyTeam] & LOS_INLOS))
        return ZeroVector;

    // this is a ghosted building, so don't add an error
    if (gameSetup->ghostedBuildings && (losStatus[argAllyTeam] & LOS_PREVLOS) &&
        unitDef->IsImmobileUnit())
        return ZeroVector;

    if ((losStatus[argAllyTeam] & LOS_INRADAR) != 0)
        return (posErrorVector *
                losHandler->GetAllyTeamRadarErrorSize(argAllyTeam));
...
CUnit::Drop(const float3& parentPos, const float3& parentDir, CUnit* parent)
{
    ...
    fallSpeed = unitDef->unitFallSpeed > 0 ? unitDef->unitFallSpeed
                                           : parent->unitDef->fallSpeed;
...
CUnit::UpdateResources()
{
    ...
    resourcesMakeI.metal = resourcesUseI.metal = resourcesMakeI.energy =
      resourcesUseI.energy = 0.0f;
}
...
CUnit::CalcLosStatus(int at)
{
    ...
    if (losHandler->InLos(this, at)) {
        newStatus |=
          (mask & (LOS_INLOS | LOS_INRADAR | LOS_PREVLOS | LOS_CONTRADAR));
...
CUnit::SlowUpdate()
{
    ...
    if (IsStunned()) {
        // call this because we can be pushed into a different quad while
        // stunned which would make us invulnerable to most non-/small-AOE
        // weapon impacts
        static_cast<AMoveType*>(moveType)->SlowUpdate();

        const bool b0 = (paralyzeDamage <=
                         (modInfo.paralyzeOnMaxHealth ? maxHealth : health));
        const bool b1 =
          (transporter == NULL || !transporter->unitDef->IsTransportUnit() ||
    ...
    if (beingBuilt) {
        if (modInfo.constructionDecay &&
            (lastNanoAdd < (gs->frameNum - modInfo.constructionDecayTime))) {
    ...
    if (unitDef->canKamikaze) {
        if (fireState >= FIRESTATE_FIREATWILL) {
            ...
            if (unitDef->kamikazeUseLOS) {
                CGameHelper::GetEnemyUnits(
                  pos, unitDef->kamikazeDist, allyteam, nearbyUnits);
            } else {
                CGameHelper::GetEnemyUnitsNoLosTest(
                  pos, unitDef->kamikazeDist, allyteam, nearbyUnits);
            }

            for (auto it = nearbyUnits.cbegin(); it != nearbyUnits.cend();
                 ++it) {
                ...
                if (dif.SqLength() < Square(unitDef->kamikazeDist)) {
                    if (victim->speed.dot(dif) <= 0.0f) {
                        // self destruct when target starts moving away from us,
                        // should maximize damage
                        KillUnit(nullptr, true, false);
                        return;
                    }
                }
        ...
        if ((curTarget.type == Target_Unit &&
             (curTarget.unit->pos.SqDistance(pos) <
              Square(unitDef->kamikazeDist))) ||
            (curTarget.type == Target_Pos &&
             (curTarget.groundPos.SqDistance(pos) <
              Square(unitDef->kamikazeDist)))) {
            KillUnit(nullptr, true, false);
            return;
        }
...
{
    return (!beingBuilt && !IsStunned() && !dontUseWeapons && !dontFire &&
            !isDead);
}
...
CUnit::GetFlankingDamageBonus(const float3& attackDir)
{
    ...
    if (flankingBonusMode == 1) {
        ...
        flankingBonus =
          (flankingBonusAvgDamage -
           attackDir.dot(flankingBonusDir) * flankingBonusDifDamage);
    } else {
        float3 adirRelative;
        ...
        flankingBonus =
          (flankingBonusAvgDamage -
           adirRelative.dot(flankingBonusDir) * flankingBonusDifDamage);
    }
...
CUnit::DoWaterDamage()
{
    ...
    DoDamage(DamageArray(mapInfo->water.damage),
             ZeroVector,
             NULL,
             -DAMAGE_EXTSOURCE_WATER,
             -1);
}
                ...
                int projectileID)
{
    ...
    if (baseDamage > 0.0f) {
        if (attacker != NULL) {
            ...
            baseDamage *=
              GetFlankingDamageBonus((attacker->pos - pos).SafeNormalize());
        }
    ...
    if (eventHandler.UnitPreDamaged(this,
                                    attacker,
                                    baseDamage,
                                    weaponDefID,
                                    projectileID,
    ...
    script->WorldHitByWeapon(-(float3(impulse * impulseMult)).SafeNormalize2D(),
                             weaponDefID,
                             /*inout*/ baseDamage);
    ...
    if (!isParalyzer) {
        ...
        if (baseDamage > 0.0f) {
            ...
            AddUnitDamageStats(
              attacker, Clamp(maxHealth - health, 0.0f, baseDamage), true);
            AddUnitDamageStats(
              this, Clamp(maxHealth - health, 0.0f, baseDamage), false);
        ...
        // paralyzeDamage may not get higher than baseHealth * (paralyzeTime +
        // 1), which means the unit will be destunned after <paralyzeTime>
        // seconds. (maximum paralyzeTime of all paralyzer weapons which
        // recently hit it ofc)
        //
        ...
        const float sumParalysisDamage =
          paralysisDecayRate * damages.paralyzeDamageTime;
        const float maxParalysisDamage =
          std::max(baseHealth + sumParalysisDamage - paralyzeDamage, 0.0f);
    ...
    eventHandler.UnitDamaged(
      this, attacker, baseDamage, weaponDefID, projectileID, isParalyzer);
    eoh->UnitDamaged(
      *this, attacker, baseDamage, weaponDefID, projectileID, isParalyzer);
    ...
    if (baseDamage > 0.0f) {
        if ((attacker != NULL) &&
            !teamHandler->Ally(allyteam, attacker->allyteam)) {
            const float scaledExpMod =
              0.1f * experienceMod * (power / attacker->power);
            const float scaledDamage =
              std::max(0.0f, (baseDamage + std::min(0.0f, health))) / maxHealth;
            // alternative
            // scaledDamage = (max(healthPreDamage, 0) - max(health, 0)) /
            // maxHealth
    ...
    if (health <= 0.0f) {
        ...
        if (!teamHandler->Ally(allyteam, attacker->allyteam)) {
            attacker->AddExperience(expMultiplier * 0.1f *
...
CUnit::ApplyImpulse(const float3& impulse)
{
    if (GetTransporter() != NULL) {
        ...
        GetTransporter()->ApplyImpulse((impulse * mass) /
                                       (GetTransporter()->mass));
        return;
    }
    ...
    const float3 modImpulse =
      impulse - (groundNormal * groundImpulseScale * IsOnGround());
...
CUnit::AddExperience(float exp)
{
    ...
    if (expHealthScale > 0.0f) {
        ...
        maxHealth = std::max(
          0.1f, unitDef->health * (1.0f + (limExperience * expHealthScale)));
...
CUnit::DoSeismicPing(float pingSize)
{
    ...
    if (!(losStatus[gu->myAllyTeam] & LOS_INLOS) &&
        losHandler->InSeismicDistance(this, gu->myAllyTeam)) {
        const float3 err =
          float3(0.5f - rx, 0.0f, 0.5f - rz) *
          losHandler->GetAllyTeamRadarErrorSize(gu->myAllyTeam);

        new CSeismicGroundFlash(
          pos + err, 30, 15, 0, pingSize, 1, float3(0.8f, 0.0f, 0.0f));
    }
    for (int a = 0; a < teamHandler->ActiveAllyTeams(); ++a) {
        if (losHandler->InSeismicDistance(this, a)) {
            const float3 err = float3(0.5f - rx, 0.0f, 0.5f - rz) *
                               losHandler->GetAllyTeamRadarErrorSize(a);
...
CUnit::ChangeTeam(int newteam, ChangeType type)
{
    ...
    if (unitHandler->unitsByDefs[newteam][unitDef->id].size() >=
        unitDef->maxThisUnit)
        return false;
    ...
    VectorInsertUnique(
      unitHandler->unitsByDefs[newteam][unitDef->id], this, false);

    for (int at = 0; at < teamHandler->ActiveAllyTeams(); ++at) {
        if (teamHandler->Ally(at, allyteam)) {
            SetLosStatus(at,
                         LOS_ALL_MASK_BITS | LOS_INLOS | LOS_INRADAR |
...
CUnit::ChangeTeamReset()
{
    ...
    for (std::vector<CUnit*>::const_iterator ui = alliedunits.begin();
         ui != alliedunits.end();
         ++ui) {
...
CUnit::CalculateTerrainType()
{
    ...
    // optimization: there's only about one unit that actually needs this
    // information
    // ==> why are we even bothering with it? the callin parameter barely makes
    // sense
...
CUnit::SetGroup(CGroup* newGroup, bool fromFactory)
{
    ...
    if (!newGroup->AddUnit(this)) {
        ...
        // add unit to the set of selected units iff its new group is already
        // selected and (user wants the unit to be auto-selected or the unit is
        // not newly built)
        if (selectedUnitsHandler.IsGroupSelected(newGroup->id) &&
            (selectedUnitsHandler.AutoAddBuiltUnitsToSelectedGroup() ||
             !fromFactory)) {
...
CUnit::AddBuildPower(CUnit* builder, float amount)
{
    ...
    if (amount >= 0.0f) {
        ...
        if (beingBuilt) {
            // build
            const float step =
              std::min(amount / buildTime, 1.0f - buildProgress);
            const float metalCostStep = cost.metal * step;
            const float energyCostStep = cost.energy * step;

            if (builderTeam->res.metal < metalCostStep ||
                builderTeam->res.energy < energyCostStep) {
            ...
            if (builder->UseMetal(metalCostStep)) {
                // FIXME eventHandler.AllowUnitBuildStep() may have changed the
                // storages!!! so the checks can be invalid!
                // TODO add a builder->UseResources(SResources(cost.metalStep,
                // cost.energyStep))
            ...
            // repair
            const float step =
              std::min(amount / buildTime, 1.0f - (health / maxHealth));
            const float energyUse = (cost.energy * step);
            const float energyUseScaled =
              energyUse * modInfo.repairEnergyCostFactor;
        ...
        const float metalRefundStepScaled =
          metalRefundStep * modInfo.reclaimUnitEfficiency;
        const float energyRefundStepScaled =
          energyRefundStep * modInfo.reclaimUnitEnergyCostFactor;
        const float healthStep = maxHealth * step;
        const float buildProgressStep =
          int(modInfo.reclaimUnitMethod == 0) * step;
        ...
        if (modInfo.reclaimUnitMethod == 0) {
            ...
            if (postHealth <= 0.0f || postBuildProgress <= 0.0f) {
                order.add.metal =
                  (cost.metal * buildProgress) * modInfo.reclaimUnitEfficiency;
                killMe = true; // to make 100% sure the unit gets killed, and so
                               // no resources are reclaimed twice!
            }
        }
...
{
    // Don't allow the reclaim if the unit is finished and we arent allowed to
    // reclaim it
    if (!beingBuilt) {
...
CanDispatch(const CUnit* u, const CTeam* team, const SResourceOrder& order)
{
    ...
    if (u->harvestStorage.empty()) {
        const bool haveEnoughStorageFree =
          ((order.add + team->res) <= team->resStorage);
        canDispatch = canDispatch && haveEnoughStorageFree;
    } else {
        const bool haveEnoughHarvestStorageFree =
          ((order.add + u->harvested) <= u->harvestStorage);
...
LimitToFullStorage(const CUnit* u, const CTeam* team, SResourceOrder* order)
{
    ...
    for (int i = 0; i < SResourcePack::MAX_RESOURCES; ++i) {
        ...
        if (u->harvestStorage.empty()) {
            GetScale(
              order->add[i], team->resStorage.res[i] - team->res[i], &scale);
        } else {
            GetScale(
              order->add[i], u->harvestStorage[i] - u->harvested[i], &scale);
        }
    }
    ...
    if (order->separate) {
        bool nonempty = false;
        for (int i = 0; i < SResourcePack::MAX_RESOURCES; ++i) {
            if ((order->use[i] != 0.0f || order->add[i] != 0.0f) &&
                scales[i] != 0.0f)
                nonempty = true;
...
{
    const float windHeading =
      ClampRad(GetHeadingFromVectorF(-x, -z) - heading * TAANG2RAD);
...
CUnit::IncomingMissile(CMissileProjectile* missile)
{
    ...
    new CFlareProjectile(
      pos,
      speed,
      this,
      (int)(gs->frameNum + unitDef->flareDelay * (1 + gs->randFloat()) * 15));
...
CUnit::StopAttackingAllyTeam(int ally)
{
    ...
    for (std::vector<CWeapon*>::iterator it = weapons.begin();
         it != weapons.end();
         ++it) {
...
CUnit::GetNewCloakState(bool stunCheck)
{
    ...
    if (wantCloak || (scriptCloak >= 1)) {
        const CUnit* closestEnemy =
          CGameHelper::GetClosestEnemyUnitNoLosTest(NULL,
                                                    midPos,
                                                    decloakDistance,
                                                    allyteam,
                                                    unitDef->decloakSpherical,
                                                    false);
        const float cloakCost = (Square(speed.w) > 0.2f)
                                  ? unitDef->cloakCostMoving
                                  : unitDef->cloakCost;
...
CUnit::CanTransport(const CUnit* unit) const
{
    ...
    if (unit->GetTransporter() != NULL)
        return false;
    if (!unit->unitDef->transportByEnemy &&
        !teamHandler->AlliedTeams(unit->team, team))
        return false;
...
CUnit::DetachUnitCore(CUnit* unit)
{
    ...
    for (TransportedUnit& tu : transportedUnits) {
        ...
        unit->SetStunned(
          unit->paralyzeDamage >
          (modInfo.paralyzeOnMaxHealth ? unit->maxHealth : unit->health));
        ...
        transportCapacityUsed -= unit->xsize / SPRING_FOOTPRINT_SCALE;
        transportMassUsed -= unit->mass;
        mass = Clamp(mass - unit->mass,
                     CSolidObject::MINIMUM_MASS,
                     CSolidObject::MAXIMUM_MASS);
...
CUnit::DetachUnit(CUnit* unit)
{
    if (DetachUnitCore(unit)) {
        ...
        if (unitDef->IsTransportUnit() &&
            (queue.empty() || (queue.front().GetID() != CMD_WAIT))) {
                          ...
                          float* wantedHeightPtr) const
{
    ...
    float wantedHeight =
      GetTransporteeWantedHeight(wantedPos, unit, &canLoadUnload);
                                  ...
                                  bool* allowedPos) const
{
    ...
    if (unit->GetTransporter() != NULL) {
        ...
        isAllowedHeight = transporteeUnitDef->CheckTerrainConstraints(
          transporteeMoveDef, wantedHeight, &clampedHeight);
        ...
        if (isAllowedHeight) {
            if (transporteeMoveDef != NULL) {
                ...
                switch (transporteeMoveDef->speedModClass) {
                    case MoveDef::Ship: {
                        wantedHeight = std::max(-transporteeUnitDef->waterline,
                                                wantedHeight);
        ...
        if (dynamic_cast<const CBuilding*>(unit) != NULL) {
            ...
            if (isAllowedHeight &&
                (!CGameHelper::TestUnitBuildSquare(bi, f, -1, true) ||
                 f != NULL))
    ...
    if (this->moveDef == NULL) {
        isAllowedHeight &= unitDef->CheckTerrainConstraints(
          NULL, rawContactHeight, &modContactHeight);
    }
