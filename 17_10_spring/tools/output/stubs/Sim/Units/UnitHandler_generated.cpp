  ...
  , maxUnitRadius(0.0f)
{
    ...
    unitsByDefs.resize(
      ...
      std::vector<std::vector<CUnit*>>(unitDefHandler->unitDefs.size()));
...
void
CUnitHandler::InsertActiveUnit(CUnit* unit)
{
...
bool
CUnitHandler::AddUnit(CUnit* unit)
{
...
void
CUnitHandler::DeleteUnit(CUnit* unit)
{
    unitsToBeRemoved.push_back(unit);
}

void
CUnitHandler::DeleteUnitsNow()
{
...
void
CUnitHandler::DeleteUnitNow(CUnit* delUnit)
{
...
CUnitHandler::Update()
{
...

    {
        ...
        for (activeUpdateUnit = 0; activeUpdateUnit < activeUnits.size();
             ++activeUpdateUnit) {
...

    {
        ...
        for (activeUpdateUnit = 0; activeUpdateUnit < activeUnits.size();
            ...
            // there are many ways to fiddle with "deathScriptFinished", so a
            // unit may arrive here not having been properly killed (with isDead
            // still false) make sure we always call Killed; no-op if isDead is
            // already true
...

    {
        ...
        for (; activeSlowUpdateUnit < activeUnits.size() && n != 0;
             ++activeSlowUpdateUnit) {
...
void
CUnitHandler::AddBuilderCAI(CBuilderCAI* b)
{
...
void
CUnitHandler::RemoveBuilderCAI(CBuilderCAI* b)
{
...
bool
CUnitHandler::CanBuildUnit(const UnitDef* unitdef, int team) const
{
