...
{
    const_cast<UnitLoadParams&>(params).unitDef =
      unitDefHandler->GetUnitDefByName(name);
...

    {
        ...
        if (params.teamID < 0) {
            ...
            if ((params.teamID = teamHandler->GaiaTeamID()) < 0)
                throw content_error(
                  "Invalid team and no gaia team to put unit in");
        }
        ...
        if (ud->IsFactoryUnit()) {
            ...
            // non-hovering fighter or bomber aircraft; coupled to
            // StrafeAirMoveType
            new CAirCAI(unit);
  ...
  int team)
{
    if (args.size() < 2) {
        LOG_L(L_WARNING,
              "[%s] not enough arguments (\"/give [amount] <objectName | "
              "'all'> [team] [@x, y, z]\")",
              __FUNCTION__);
        return;
    }
    ...
    if (sscanf(args[args.size() - 1].c_str(),
               ...
               &pos.z) != 3) {
        LOG_L(L_WARNING,
              "[%s] invalid position argument (\"/give [amount] <objectName | "
              "'all'> [team] [@x, y, z]\")",
              __FUNCTION__);
        return;
    }
    ...
    if (amountArgIdx >= 0) {
        ...
        if ((amount < 0) || (args[amountArgIdx].find_first_not_of(
                               "0123456789") != std::string::npos)) {
            LOG_L(L_WARNING,
                  "[%s] invalid amount argument: %s",
                  __FUNCTION__,
                  args[amountArgIdx].c_str());
            return;
        }
    }
    ...
    if (teamArgIdx >= 0) {
        ...
        if ((!teamHandler->IsValidTeam(team)) ||
            (args[teamArgIdx].find_first_not_of("0123456789") !=
             std::string::npos)) {
            LOG_L(L_WARNING,
                  "[%s] invalid team argument: %s",
                  __FUNCTION__,
                  args[teamArgIdx].c_str());
            return;
        }
                       ...
                       int featureAllyTeam)
{
    ...
    if (objectName == "all") {
        unsigned int numRequestedUnits =
          unitDefHandler->unitDefs.size() - 1; /// defid=0 is not valid
        unsigned int currentNumUnits = receivingTeam->units.size();

        // make sure team unit-limit is not exceeded
        if ((currentNumUnits + numRequestedUnits) >
            receivingTeam->GetMaxUnits()) {
        ...
        pos.x =
          Clamp(pos.x, sqHalfMapSize, float3::maxxpos - sqHalfMapSize - 1);
        pos.z =
          Clamp(pos.z, sqHalfMapSize, float3::maxzpos - sqHalfMapSize - 1);

        for (int a = 1; a <= numRequestedUnits; ++a) {
            Watchdog::ClearPrimaryTimers(); // the other thread may be waiting
                                            // for a mutex held by this one,
                                            // triggering hang detection
            const float px =
              pos.x + (a % sqSize - sqSize / 2) * 10 * SQUARE_SIZE;
            const float pz =
              pos.z + (a / sqSize - sqSize / 2) * 10 * SQUARE_SIZE;
        ...
        if (receivingTeam->AtUnitLimit()) {
            LOG_L(L_WARNING,
                  "[%s] unable to give more units to team %d (current: %u, "
                  "team limit: %u, global limit: %u)",
                  __FUNCTION__,
                  team,
                  currentNumUnits,
                  receivingTeam->GetMaxUnits(),
        ...
        const FeatureDef* featureDef =
          featureDefHandler->GetFeatureDef(objectName, false);

        if (unitDef == NULL && featureDef == NULL) {
            LOG_L(L_WARNING,
                  "[%s] %s is not a valid object-name",
                  __FUNCTION__,
                  objectName.c_str());
            return;
        }
        ...
        if (unitDef != NULL) {
            ...
            const int squareSize =
              math::ceil(math::sqrt((float)numRequestedUnits));
            const float3 squarePos =
              float3(pos.x - (((squareSize - 1) * xsize * SQUARE_SIZE) / 2),
                     pos.y,
...
CUnitLoader::FlattenGround(const CUnit* unit)
{
    ...
    const int tx1 =
      (int)std::max(0.0f, (bi.pos.x - (bi.GetXSize() * hss)) / SQUARE_SIZE);
    const int tz1 =
      (int)std::max(0.0f, (bi.pos.z - (bi.GetZSize() * hss)) / SQUARE_SIZE);
...
CUnitLoader::RestoreGround(const CUnit* unit)
{
    ...
    for (int z = tz1; z <= tz2; z++) {
        for (int x = tx1; x <= tx2; x++) {
            ...
            heightdiff +=
              heightmap[index] - readMap->GetOriginalHeightMapSynced()[index];
            ++num;
        }
    }
    ...
    for (int z = tz1; z <= tz2; z++) {
        for (int x = tx1; x <= tx2; x++) {
            int index = z * mapDims.mapxp1 + x;
            readMap->SetHeight(
              index, heightdiff + readMap->GetOriginalHeightMapSynced()[index]);
        }
    }
