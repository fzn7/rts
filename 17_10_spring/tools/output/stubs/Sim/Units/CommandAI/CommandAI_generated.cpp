  ...
  , targetLostTimer(TARGET_LOST_TIMER)
{
...

    {
        ...
        c.name = "Wait";
        c.tooltip =
          c.name + ": Tells the unit to wait processing its command-queue";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
...

    {
        ...
        c.name = "SquadWait";
        c.tooltip =
          c.name + ": Wait for a number of units to arrive before continuing";
        c.mouseicon = c.name;
    ...
    if (owner->unitDef->canRepeat) {
        ...
        c.tooltip = c.name + ": If on, the unit will continuously\n push "
                             "finished orders to the end of its\n order queue";
        c.mouseicon = c.name;
    ...
    if (owner->unitDef->highTrajectoryType > 1) {
        ...
        c.tooltip = c.name + ": If set to high, weapons that\n support it will "
                             "try to fire in a higher\n trajectory than usual "
                             "(experimental)";
        c.mouseicon = c.name;
    ...
    if (owner->unitDef->onoffable) {
        ...
        c.params.push_back(
          IntToString(owner->unitDef->activateWhenBuilt, "%d"));
...
{
    const SCommandDescription* cmdDescPtr =
      commandDescriptionCache->GetPtr(cmdDesc);
    if (cmdDescIdx >= possibleCommands.size()) {
        possibleCommands.push_back(cmdDescPtr);
    } else {
        // preserve order
        possibleCommands.insert(possibleCommands.begin() + cmdDescIdx,
                                cmdDescPtr);
    }
...
CCommandAI::ClearCommandDependencies()
{
    while (!commandDeathDependences.empty()) {
        DeleteDeathDependence(*commandDeathDependences.begin(),
...
CCommandAI::AddCommandDependency(const Command& c)
{
    ...
    if (c.IsObjectCommand(cpos)) {
        ...
        CObject* ref = (refId < unitHandler->MaxUnits())
                         ...
                         : static_cast<CObject*>(featureHandler->GetFeature(
                             refId - unitHandler->MaxUnits()));
        if (ref) {
...
CCommandAI::IsAttackCapable() const
{
    ...
    const bool b =
      (!ud->weapons.empty() || ud->canKamikaze || (ud->IsFactoryUnit()));
...
IsCommandInMap(const Command& c)
{
    ...
    LOG_L(L_DEBUG,
          "Dropped command %d: outside of map (x:%f y:%f z:%f)",
          c.GetID(),
          pos.x,
          pos.y,
          pos.z);
...
AdjustGroundAttackCommand(const Command& c, bool fromSynced, bool aiOrder)
{
    ...
    cc.params[1] = std::min(
      cPos.y, CGround::GetHeightAboveWater(cPos.x, cPos.z, true || fromSynced));
    // cc.params[1] = CGround::GetHeightReal(cPos.x, cPos.z, true ||
    // fromSynced);
...
CCommandAI::AllowedCommand(const Command& c, bool fromSynced)
{
    ...
    switch (cmdID) {
        ...
        default: {
            ...
            if (cmdID < 0) {
                ...
                const CBuilderCAI* bcai =
                  dynamic_cast<const CBuilderCAI*>(this);
                const CFactoryCAI* fcai =
                  dynamic_cast<const CFactoryCAI*>(this);
    ...
    const CSkirmishAIHandler::ids_t& saids =
      skirmishAIHandler.GetSkirmishAIsInTeam(owner->team);
    ...
    switch (cmdID) {
        ...
        case CMD_CAPTURE: {
            ...
            if (capturee && !capturee->pos.IsInBounds()) {
                return false;
            }
        } break;
        ...
        case CMD_RECLAIM: {
            ...
            if (reclaimeeUnit && !reclaimeeUnit->unitDef->reclaimable) {
                return false;
            }
            if (reclaimeeUnit && !reclaimeeUnit->AllowedReclaim(owner)) {
                return false;
            }
            if (reclaimeeUnit && !reclaimeeUnit->pos.IsInBounds()) {
                return false;
            }
            ...
            if (reclaimeeUnit == NULL && !c.params.empty()) {
                ...
                if (reclaimeeFeatureID >= unitHandler->MaxUnits()) {
                    reclaimeeFeature = featureHandler->GetFeature(
                      reclaimeeFeatureID - unitHandler->MaxUnits());

                    if (reclaimeeFeature &&
                        !reclaimeeFeature->def->reclaimable) {
                        return false;
                    }
                }
            }
        ...
        case CMD_REPAIR: {
            ...
            if (repairee && !repairee->pos.IsInBounds()) {
                return false;
            }
            if (repairee && ((repairee->beingBuilt && !ud->canAssist) ||
                             (!repairee->beingBuilt && !ud->canRepair))) {
                return false;
            }
        } break;
    }
    ...
    if (cmdID == CMD_REPEAT && (c.params.empty() || !ud->canRepeat ||
                                ((int)c.params[0] % 2) !=
                                  (int)c.params[0] /* only 0 or 1 allowed */)) {
        return false;
    }
    ...
    if (cmdID == CMD_CLOAK && (c.params.empty() || !ud->canCloak ||
                               ((int)c.params[0] % 2) !=
                                 (int)c.params[0] /* only 0 or 1 allowed */)) {
        return false;
    }
...
CCommandAI::GiveAllowedCommand(const Command& c, bool fromSynced)
{
            ...
            if (owner->unitDef->canSelfD) {
                if (!(c.options & SHIFT_KEY) || commandQue.empty()) {
                    if (owner->selfDCountdown != 0) {
                        ...
                        owner->selfDCountdown =
                          owner->unitDef->selfDCountdown * 2 + 1;
                    }
    ...
    if (!(c.options & SHIFT_KEY)) {
        ...
        ClearTargetLock((commandQue.empty()) ? Command(CMD_STOP)
                                             : commandQue.front());
        ClearCommandDependencies();
        SetOrderTarget(NULL);
    ...
    if (facCAI) {
        if (c.options & CONTROL_KEY) {
            ...
            if ((newCmd.GetID() != CMD_STOP) && (newCmd.GetID() != CMD_WAIT) &&
                ((newCmd.GetID() >= 0) ||
                 (bOpts.find(newCmd.GetID()) == bOpts.end()))) {
                return;
            }
            facBuildQueue = true;
        } else {
    ...
    for (unsigned int p = 0; p < c.params.size(); p++) {
        ...
        do {
            for (ci = queue->begin(); ci != queue->end(); ++ci) {
                ...
                if (facBuildQueue) {
                    // if ci == queue->begin() and !queue->empty(), this
                    // pop_front()'s via CFAI::ExecuteStop; otherwise only
                    // modifies *ci (not <queue>)
...
CCommandAI::GetCancelQueued(const Command& c, CCommandQueue& q)
{
    ...
    while (ci != q.begin()) {
        ...
        const bool attackAndFight =
          (cmdID == CMD_ATTACK && cmd2ID == CMD_FIGHT && c2.params.size() == 1);
            ...
            if (c.params.size() == 1) {
                ...
                if (cmdID < 0) {
                    ...
                    if (math::fabs(bc1.pos.x - bc2.pos.x) * 2 <=
                          std::max(bc1.GetXSize(), bc2.GetXSize()) *
                            SQUARE_SIZE &&
                        math::fabs(bc1.pos.z - bc2.pos.z) * 2 <=
                          std::max(bc1.GetZSize(), bc2.GetZSize()) *
                            SQUARE_SIZE) {
                        return ci;
                    }
                } else {
                    ...
                    if ((c1p - c2p).SqLength2D() >=
                        (COMMAND_CANCEL_DIST * COMMAND_CANCEL_DIST))
                        continue;
        ...
        do {
            ...
            if (t.GetID() == c.GetID() || (c.GetID() < 0 && t.GetID() < 0)) {
                if (c.params.size() == 1) {
                    ...
                    // NOTE: uses a BuildInfo structure, but <t> can be ANY
                    // command
                    BuildInfo tbi;
                    if (tbi.Parse(t)) {
                        const float dist2X =
                          2.0f * math::fabs(cbi.pos.x - tbi.pos.x);
                        const float dist2Z =
                          2.0f * math::fabs(cbi.pos.z - tbi.pos.z);
                        const float addSizeX =
                          SQUARE_SIZE * (cbi.GetXSize() + tbi.GetXSize());
                        const float addSizeZ =
                          SQUARE_SIZE * (cbi.GetZSize() + tbi.GetZSize());
                        const float maxSizeX =
                          SQUARE_SIZE *
                          std::max(cbi.GetXSize(), tbi.GetXSize());
                        const float maxSizeZ =
                          SQUARE_SIZE *
                          std::max(cbi.GetZSize(), tbi.GetZSize());
...
CCommandAI::UpdateTargetLostTimer(int targetUnitID)
{
    ...
    const UnitDef* targetUnitDef =
      (targetUnit != NULL) ? targetUnit->unitDef : NULL;
...
CCommandAI::AddDeathDependence(CObject* o, DependenceType dep)
{
    if (dep == DEPENDENCE_COMMANDQUE) {
        if (commandDeathDependences.insert(o)
              .second) // prevent multiple dependencies for the same object
...
CCommandAI::DependentDied(CObject* o)
{
    ...
    if (commandDeathDependences.erase(o) && o != owner) {
        ...
        do {
            lastTag = curTag;
            for (CCommandQueue::iterator qit = dq.begin(); qit != dq.end();
                 ++qit) {
                Command& c = *qit;
                int cpos;
                if (c.IsObjectCommand(cpos) &&
                    (c.params[cpos] == CSolidObject::GetDeletingRefID())) {
                    curTag = c.tag;
...
CCommandAI::FinishCommand()
{
    ...
    const Command cmd =
      commandQue.front(); // cppcheck false positive, copy is needed here
    const bool dontRepeat = (cmd.options & INTERNAL_ORDER);

    if (repeatOrders && !dontRepeat && (cmd.GetID() != CMD_STOP) &&
    ...
    for (unsigned int n = 0; n < possibleCommands.size(); n++) {
        ...
        c.name = IntToString(stockpileWeapon->numStockpiled) + "/" +
                 IntToString(stockpileWeapon->numStockpiled +
                             stockpileWeapon->numStockpileQued);
...
CCommandAI::WeaponFired(CWeapon* weapon, const bool searchForNewTarget)
{
    ...
    const bool haveGroundAttackCmd =
      (c.GetID() == CMD_ATTACK && c.GetParamsCount() >= 3);
    const bool haveAreaAttackCmd = (c.GetID() == CMD_AREA_ATTACK);

    bool orderFinished = false;

    if (searchForNewTarget) {
        // manual fire or attack commands with meta will only fire a single
        // salvo noAutoTarget weapons finish an attack commands after a salvo if
        // they have more orders queued
        if (weapon->weaponDef->manualfire && !(c.options & META_KEY))
            orderFinished = true;

        if (weapon->noAutoTarget && !(c.options & META_KEY) &&
            haveGroundAttackCmd && HasMoreMoveCommands())
            orderFinished = true;
        ...
        // return argument says if a new area attack target was chosen, else
...
CCommandAI::SkipParalyzeTarget(const CUnit* target)
{
    ...
    // visible and stunned?
    if ((target->losStatus[owner->allyteam] & LOS_INLOS) &&
        target->IsStunned() && HasMoreMoveCommands())
        return true;
...
CCommandAI::StopAttackingAllyTeam(int ally)
{
    ...
    for (CCommandQueue::iterator it = commandQue.begin();
         it != commandQue.end();
         ++it) {
        const Command& c = *it;
    ...
    for (std::vector<int>::reverse_iterator it = todel.rbegin();
         it != todel.rend();
         ++it) {
