  ...
  , moveDir(gs->randFloat() > 0.5f)
{
    ...
    if (owner->unitDef->canPatrol) {
        ...
        c.name = "Patrol";
        c.tooltip =
          c.name + ": Order the unit to patrol to one or more waypoints";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canFight) {
        ...
        c.name = "Fight";
        c.tooltip =
          c.name + ": Order the unit to take action while moving to a position";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canGuard) {
        ...
        c.tooltip =
          c.name +
          ": Order a unit to guard another unit and attack units attacking it";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canfly) {
        {
            ...
            c.name = "Repair level";
            c.tooltip = c.name + ": Sets at which health level an aircraft "
                                 "will try to find a repair pad";
            c.mouseicon = c.name;
    ...
    if (owner->unitDef->IsTransportUnit()) {
        ...
        }
        {
            ...
            c.name = "Unload units";
            c.tooltip =
              c.name + ": Sets the transport to unload units in an area";
            c.mouseicon = c.name;
            possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
        }
    }
...
CMobileCAI::ExecuteLoadOnto(Command& c)
{
    ...
    if (!inCommand) {
        inCommand = true;
        Command newCommand(
          CMD_LOAD_UNITS, INTERNAL_ORDER | SHIFT_KEY, owner->id);
...
CMobileCAI::ExecutePatrol(Command& c)
{
    ...
    if (c.params.size() < 3) {
        LOG_L(L_ERROR,
              "[MCAI::%s][f=%d][id=%d] CMD_FIGHT #params < 3",
              __FUNCTION__,
              gs->frameNum,
              owner->id);
        return;
...
CMobileCAI::ExecuteFight(Command& c)
{
    ...
    if (c.params.size() == 1 && !owner->weapons.empty()) {
        ...
        if ((orderTarget != NULL) &&
            !w->Attack(SWeaponTarget(orderTarget, false))) {
            CUnit* newTarget = CGameHelper::GetClosestValidTarget(
              owner->pos, owner->maxRange, owner->allyteam, this);

            if ((newTarget != NULL) &&
                w->Attack(SWeaponTarget(newTarget, false))) {
    ...
    if (c.params.size() >= 6) {
        ...
        // fight command finished by the
        // 'if((curPos-pos).SqLength2D()<(64*64)){' condition, but is actually
        // updated correctly if you click somewhere outside the area close to
        // the line (for a new command).
    ...
    if (owner->unitDef->canAttack && owner->fireState >= FIRESTATE_FIREATWILL &&
        !owner->weapons.empty()) {
        const float3 curPosOnLine =
          ClosestPointOnLine(commandPos1, commandPos2, owner->pos);
        const float searchRadius =
          owner->maxRange + 100 * owner->moveState * owner->moveState;
        CUnit* enemy = CGameHelper::GetClosestValidTarget(
          curPosOnLine, searchRadius, owner->allyteam, this);
    ...
    for (CWeapon* w : owner->weapons) {
        if (w->TestTarget(enemy->pos, SWeaponTarget(enemy)) &&
            (owner->moveState != MOVESTATE_HOLDPOS ||
             w->TryTargetRotate(enemy, false, false))) {
            return true;
        }
    }
...
CMobileCAI::ExecuteAttack(Command& c)
{
    ...
    if (tempOrder && orderTarget) {
        const float3& closestPos =
          ClosestPointOnLine(commandPos1, commandPos2, owner->pos);
        const float curTargetDist =
          LinePointDist(closestPos, commandPos2, orderTarget->pos);
        const float maxTargetDist =
          (owner->moveType->GetManeuverLeash() * owner->moveState +
           owner->maxRange);

        if (owner->moveState < MOVESTATE_ROAM &&
            curTargetDist > maxTargetDist) {
            StopMoveAndFinishCommand();
            return;
        }
    }
    if (!inCommand) {
        if (c.params.size() == 1) {
            ...
            // check if we have valid target parameter and that we aren't
            // attacking ourselves
            if (targetUnit == NULL) {
                StopMoveAndFinishCommand();
                return;
            ...
            const float3 tgtErrPos =
              targetUnit->GetErrorPos(owner->allyteam, false);
            ...
            if (targetUnit->unitDef->IsImmobileUnit()) {
                ...
                const float fpSqRadius =
                  (targetUnit->unitDef->xsize * targetUnit->unitDef->xsize +
                   targetUnit->unitDef->zsize * targetUnit->unitDef->zsize);
                const float fpRadius =
                  (math::sqrt(fpSqRadius) * 0.5f) * SQUARE_SIZE;
                // 2 * SQUARE_SIZE is a buffer that work to make sure it's
                // pathable ground
                radius = std::max(radius, fpRadius + 2 * SQUARE_SIZE);
            }
            // FIXME: don't call SetGoal() if target is already in range of some
            // weapon?
            SetGoal(tgtErrPos - tgtPosDir * radius, owner->pos);
            SetOrderTarget(targetUnit);
            owner->AttackUnit(targetUnit,
                              (c.options & INTERNAL_ORDER) == 0,
                              c.GetID() == CMD_MANUALFIRE);
    ...
    if (targetDied || (c.params.size() == 1 &&
                       UpdateTargetLostTimer(int(c.params[0])) == 0)) {
        // cancel keeppointingto
        StopMoveAndFinishCommand();
        return;
    }

    // user clicked on enemy unit (note that we handle aircrafts slightly
    // differently)
    if (orderTarget != NULL) {
        ...
        const float targetGoalDist =
          errPos.SqDistance2D(owner->moveType->goalPos);
        const float targetPosDist =
          Square(10.0f + orderTarget->pos.distance2D(owner->pos) * 0.2f);
        const float minPointingDist =
          std::min(1.0f * owner->losRadius, owner->maxRange * 0.9f);

        // FIXME? targetMidPosMaxDist is 3D, but compared with a 2D value
        const float targetMidPosDist2D = targetMidPosVec.Length2D();
        // const float targetMidPosMaxDist = owner->maxRange -
        // (Square(orderTarget->speed.w) / owner->unitDef->maxAcc);
        ...
        const short targetHead =
          GetHeadingFromVector(-targetMidPosVec.x, -targetMidPosVec.z);
        ...
        for (CWeapon* w : owner->weapons) {
            ...
            tryTargetRotate =
              w->TryTargetRotate(trg.unit, trg.isUserTarget, trg.isManualFire);
        ...
        if (tryTargetRotate) {
            const bool canChaseTarget =
              (!tempOrder || owner->moveState != MOVESTATE_HOLDPOS);
            const bool targetBehind =
              (targetMidPosVec.dot(orderTarget->speed) < 0.0f);
            if (canChaseTarget && tryTargetHeading && targetBehind &&
                !owner->unitDef->IsHoveringAirUnit()) {
                SetGoal(owner->pos + (orderTarget->speed * 80),
                        owner->pos,
                        SQUARE_SIZE,
                        orderTarget->speed.w * 1.1f);
            } else {
                StopMove();

                if (gs->frameNum > lastCloseInTry + MAX_CLOSE_IN_RETRY_TICKS) {
                    owner->moveType->KeepPointingTo(
                      orderTarget->midPos, minPointingDist, true);
                }
            }
        ...
        // if ((our movetype has type HoverAirMoveType and length of 2D vector
        // from us to target less than 90% of our maximum range) OR squared
        // length of 2D vector from us to target less than 1024) then we are
        // close enough
        else if (targetMidPosDist2D < (owner->maxRange * 0.9f)) {
            if (owner->unitDef->IsHoveringAirUnit() ||
                (targetMidPosVec.SqLength2D() < 1024)) {
                StopMove();
                owner->moveType->KeepPointingTo(
                  orderTarget->midPos, minPointingDist, true);
            }
            // if (((first weapon range minus first weapon length greater than
            // distance to target) and length of 2D vector from us to target
            // less than 90% of our maximum range) then we are close enough, but
            // need to move sideways to get a shot.
            // assumption is flawed: The unit may be aiming or otherwise unable
            // to shoot
                ...
                goalDiff *= (targetMidPosDist2D < (owner->maxRange * 0.3f))
                              ? 1 / cos
                              : cos;
        ...
        // if 2D distance of (target position plus attacker error vector times
        // 128) to goal position greater than (10 plus 20% of 2D distance
        // between attacker and target) then we need to close in on target more
            ...
            if (orderTarget->unitDef->IsImmobileUnit()) {
                // FIXME change into a function
                const float fpSqRadius =
                  (orderTarget->unitDef->xsize * orderTarget->unitDef->xsize +
                   orderTarget->unitDef->zsize * orderTarget->unitDef->zsize);
                const float fpRadius =
                  (math::sqrt(fpSqRadius) * 0.5f) * SQUARE_SIZE;
                // 2 * SQUARE_SIZE is a buffer that work to make sure it's
                // pathable ground
    ...
    // user wants to attack the ground
        ...
        if (c.GetID() == CMD_MANUALFIRE) {
            ...
            if (owner->AttackGround(trg.groundPos, trg.isUserTarget, true)) {
                ...
                owner->moveType->KeepPointingTo(
                  attackPos, owner->maxRange * 0.9f, true);
                StopMove();
                return;
            }
        } else {
            for (CWeapon* w : owner->weapons) {
                // NOTE:
                //   we call TryTargetHeading which is less restrictive than
                //   TryTarget (eg. the former succeeds even if the unit has not
                //   already aligned itself with <attackVec>)
                if (!w->TryTargetHeading(attackHead, trg))
                    continue;

                if (owner->AttackGround(
                      trg.groundPos, trg.isUserTarget, false)) {
                    StopMoveAndKeepPointing(
                      trg.groundPos, owner->maxRange * 0.9f, true);
                    return;
                }
...
CMobileCAI::NonMoving()
{
    ...
    if (lastBuggerOffTime > gs->frameNum - BUGGER_OFF_TTL) {
        ...
        if (length < buggerOffRadius) {
            float3 goalPos =
              buggerOffPos + dif * ((buggerOffRadius + 128) / length);
            bool randomize = (goalPos.x == lastBuggerGoalPos.x) &&
                             (goalPos.z == lastBuggerGoalPos.z);
            lastBuggerGoalPos.x = goalPos.x;
            lastBuggerGoalPos.z = goalPos.z;
            if (randomize) {
                lastBuggerGoalPos.y += 32.0f; // gradually increase the
                                              // amplitude of the random factor
                goalPos.x += (2.0f * lastBuggerGoalPos.y) * gs->randFloat() -
                             lastBuggerGoalPos.y;
                goalPos.z += (2.0f * lastBuggerGoalPos.y) * gs->randFloat() -
                             lastBuggerGoalPos.y;
            } else
...
CMobileCAI::FinishCommand()
{
    ...
    if (owner->unitDef->IsTransportUnit()) {
        CHoverAirMoveType* am =
          dynamic_cast<CHoverAirMoveType*>(owner->moveType);
...
CMobileCAI::MobileAutoGenerateTarget()
{
    ...
    if ((owner->pos - lastUserGoal).SqLength2D() <=
        (MAX_USERGOAL_TOLERANCE_DIST * MAX_USERGOAL_TOLERANCE_DIST)) {
        NonMoving();
        return false;
    }
    if (owner->unitDef->IsHoveringAirUnit()) {
    ...
    if (owner->curTarget.type == Target_Unit) {
        ...
        if (owner->lastAttacker != nullptr) {
            const bool freshAttack =
              (gs->frameNum < (owner->lastAttackFrame + GAME_SPEED * 7));
            const bool canChaseAttacker = !(owner->unitDef->noChaseCategory &
                                            owner->lastAttacker->category);
            if (freshAttack && canChaseAttacker) {
                const float r =
                  owner->pos.SqDistance2D(owner->lastAttacker->pos);
        ...
        if (newAttackTargetId < 0 && owner->fireState >= FIRESTATE_FIREATWILL &&
            (gs->frameNum >= lastIdleCheck + 10)) {
            ...
            if (newAttackTargetId < 0) {
                const float searchRadius =
                  owner->maxRange +
                  150.0f * owner->moveState * owner->moveState;
                const CUnit* enemy = CGameHelper::GetClosestValidTarget(
                  owner->pos, searchRadius, owner->allyteam, this);
...
{
    return (!tempOrder) || // Check if the weapon actually targets the unit's
                           // order target
...
CMobileCAI::StartSlowGuard(float speed)
{
    ...
    const Command& c =
      (commandQue.size() > 1) ? commandQue[1] : Command(CMD_STOP);
...
{
    const float tmp =
      owner->moveType->CalcStaticTurnRadius() + (SQUARE_SIZE << 1);
...
CMobileCAI::SetTransportee(CUnit* unit)
{
    ...
    if (orderTarget != nullptr &&
        orderTarget->loadingTransportId == owner->id) {
    ...
    if (unit != nullptr) {
        CUnit* transport =
          (unit->loadingTransportId == -1)
            ? NULL
            : unitHandler->GetUnitUnsafe(unit->loadingTransportId);
        // let the closest transport be loadingTransportId, in case of multiple
        // fighting transports
        if ((transport == NULL) ||
            ((transport != owner) && (transport->pos.SqDistance(unit->pos) >
                                      owner->pos.SqDistance(unit->pos)))) {
    ...
    if (c.params.size() == 1) {
        ...
        if (c.options & INTERNAL_ORDER) {
            if (unit->commandAI->commandQue.empty()) {
                ...
                if ((currentUnitCommand.GetID() == CMD_LOAD_ONTO) &&
        ...
        if (owner->CanTransport(unit) &&
            ...
            const bool inLoadingRadius =
              (sqDist <= Square(owner->unitDef->loadingRadius));
            ...
            const bool outOfRange =
              (owner->moveType->goalPos.SqDistance2D(unit->pos) >
               Square(owner->unitDef->loadingRadius - SQUARE_SIZE));
            const bool moveCloser =
              (!inLoadingRadius &&
               (!owner->IsMoving() ||
                (am != NULL &&
                 am->aircraftState != AAirMoveType::AIRCRAFT_FLYING)));

            if (outOfRange || moveCloser) {
                SetGoal(unit->pos,
                        owner->pos,
                        std::min(64.0f, owner->unitDef->loadingRadius));
            }
            ...
            if (inLoadingRadius) {
                if (am != NULL) {
                    ...
                    wantedPos.y =
                      owner->GetTransporteeWantedHeight(wantedPos, unit);

                    // calls am->StartMoving() which sets forceHeading to false
                    // (and also changes aircraftState, possibly in mid-pickup)
                    // --> must check that wantedPos == goalPos using some
                    // epsilon tolerance we do not want the forceHeading change
                    // at point of pickup because am->UpdateHeading() will
                    // suddenly notice a large deltaHeading and break the
                    // DOCKING_ANGLE constraint so call am->ForceHeading() next
                    SetGoal(wantedPos, owner->pos, 1.0f);

                    am->ForceHeading(owner->GetTransporteeWantedHeading(unit));
                    am->SetWantedAltitude(
                      wantedPos.y -
                      CGround::GetHeightAboveWater(wantedPos.x, wantedPos.z));
                    am->maxDrift = 1.0f;

                    // FIXME: kill the hardcoded constants, use the command's
                    // radius
                    const bool b1 = (owner->pos.SqDistance(wantedPos) <
                                     Square(AIRTRANSPORT_DOCKING_RADIUS));
                    const bool b2 = (std::abs(owner->heading - unit->heading) <
                                     AIRTRANSPORT_DOCKING_ANGLE);
                    ...
                    if (b1 && b2 && b3) {
                        ...
                        SetTransportee(NULL);
                        owner->AttachUnit(unit,
                                          owner->script->QueryTransport(unit));
            ...
            } else if (owner->moveType->progressState == AMoveType::Failed &&
                       sqDist < (200 * 200)) {
                // if we're pretty close already but CGroundMoveType fails
                // because it considers the goal clogged (with the future
                // passenger...), just try to move to the point halfway between
                // the transport and the passenger.
...
CMobileCAI::AllowedCommand(const Command& c, bool fromSynced)
{
    ...
    switch (c.GetID()) {
        ...
        case CMD_UNLOAD_UNITS: {
            ...
            // allow unloading empty transports for easier setup of transport
            // bridges
            ...
            if (c.GetParamsCount() == 5) {
                if (fromSynced) {
                    // point transported buildings (...) in their wanted
                    // direction after unloading
                    for (auto& tu : transportees) {
                        tu.unit->buildFacing =
                          std::abs(int(c.GetParam(4))) % NUM_FACINGS;
                    }
                }
            }

            if (c.GetParamsCount() >= 4) {
                // find unload positions for transportees (WHY can this run in
                // unsynced context?)
                for (const CUnit::TransportedUnit& tu : transportees) {
                    const CUnit* u = tu.unit;

                    const float radius =
                      (c.GetID() == CMD_UNLOAD_UNITS) ? c.GetParam(3) : 0.0f;
                    const float spread =
                      u->radius * owner->unitDef->unloadSpread;

                    float3 foundPos;

                    if (FindEmptySpot(c.GetPos(0),
                                      radius,
                                      spread,
                                      foundPos,
                                      u,
                    ...
                    // FIXME: support arbitrary unloading order for other unload
                    // types also
                          ...
                          bool fromSynced)
{
    ...
    const bool isAirTrans =
      (dynamic_cast<AAirMoveType*>(owner->moveType) != NULL);
    const float amax =
      std::max(100.0f, std::min(1000.0f, (radius * radius / 100.0f)));
    ...
    for (int a = 0; a < amax; ++a) {
        ...
        for (int b = 0; b < bmax; ++b) {
            // FIXME: using a deterministic technique might be better, since it
            // would allow an unload command to be tested for validity from
            // unsynced (with predictable results)
            const float ang =
              2.0f * PI * (fromSynced ? gs->randFloat() : gu->RandFloat());
            const float len =
              math::sqrt(fromSynced ? gs->randFloat() : gu->RandFloat());
        ...
        if (moveDef != NULL &&
            CGround::GetSlope(pos.x, pos.z) > moveDef->maxSlope)
            continue;

        const std::vector<CSolidObject*>& units = quadField->GetSolidsExact(
          pos, spread, 0xFFFFFFFF, CSolidObject::CSTATE_BIT_SOLIDOBJECTS);

        if (isAirTrans &&
            (units.size() > 1 || (units.size() == 1 && units[0] != owner)))
            continue;
...
CMobileCAI::FindUnitToTransport(float3 center, float radius)
{
    ...
    for (std::vector<CUnit*>::const_iterator ui = units.begin();
         ui != units.end();
         ++ui) {
        CUnit* unit = (*ui);
        float dist = unit->pos.SqDistance2D(owner->pos);

        if (unit->loadingTransportId != -1 &&
            unit->loadingTransportId != owner->id) {
            CUnit* trans = unitHandler->GetUnitUnsafe(unit->loadingTransportId);
            if ((trans != NULL) &&
                teamHandler->AlliedTeams(owner->team, trans->team)) {
                continue; // don't refuse to load comm only because the enemy is
                          // trying to nap it at the same time
            }
        }
        if (dist >= bestDist)
            continue;
...
CMobileCAI::SpotIsClear(float3 pos, CUnit* unitToUnload)
{
    if (!owner->CanLoadUnloadAtPos(pos, unitToUnload))
        return false;
    if (unitToUnload->moveDef != NULL &&
        CGround::GetSlope(pos.x, pos.z) > unitToUnload->moveDef->maxSlope)
        return false;

    const float radius =
      std::max(1.0f, math::ceil(unitToUnload->radius / SQUARE_SIZE)) *
      SQUARE_SIZE;

    if (!quadField->NoSolidsExact(
          pos, radius, 0xFFFFFFFF, CSolidObject::CSTATE_BIT_SOLIDOBJECTS))
        return false;
...
CMobileCAI::SpotIsClearIgnoreSelf(float3 pos, CUnit* unitToUnload)
{
    ...
    for (auto objectsIt = units.begin(); objectsIt != units.end();
         ++objectsIt) {
                               ...
                               std::vector<float3>& dropSpots)
{
    ...
    const float3 spreadVector =
      (endpos - startpos).Normalize() * owner->unitDef->unloadSpread;
...
CMobileCAI::UnloadUnits_Drop(Command& c)
{
    ...
    const bool canUnload = FindEmptyDropSpots(
      startingDropPos,
      startingDropPos + approachVector * std::max(16.0f, c.params[3]),
      dropSpots);
...
CMobileCAI::UnloadUnits_LandFlood(Command& c)
{
    ...
    const bool canUnload =
      FindEmptySpot(pos, radius, spread, found, transportee);
...
CMobileCAI::UnloadLand(Command& c)
{
    ...
    if (wantedPos.SqDistance2D(owner->pos) <
        Square(owner->unitDef->loadingRadius * 0.9f)) {
        CHoverAirMoveType* am =
          dynamic_cast<CHoverAirMoveType*>(owner->moveType);
        ...
        if (am != NULL) {
            ...
            am->SetWantedAltitude(wantedPos.y - CGround::GetHeightAboveWater(
                                                  wantedPos.x, wantedPos.z));
            ...
            const bool b1 = (owner->pos.SqDistance(wantedPos) <
                             Square(AIRTRANSPORT_DOCKING_RADIUS));
            const bool b2 = (std::abs(owner->heading - am->GetForcedHeading()) <
                             AIRTRANSPORT_DOCKING_ANGLE);
            ...
            if (b1 && b2 && b3) {
                ...
                if (!SpotIsClearIgnoreSelf(wantedPos, transportee)) {
                    ...
                    if (FindEmptySpot(wantedPos,
                                      std::max(16.0f * SQUARE_SIZE,
                                               transportee->radius * 4.0f),
                                      transportee->radius,
                                      newWantedPos,
                                      transportee)) {
                        c.SetPos(0, newWantedPos);
                        SetGoal(newWantedPos +
                                  UpVector * transportee->model->height,
                                owner->pos);
                        return;
                    }
                } else {
...
CMobileCAI::UnloadDrop(Command& c)
{
    ...
    if (am != NULL) {
        ...
        if (owner->pos.SqDistance2D(pos) < Square(40.0f) ||
            (((pos - owner->pos).Normalize()).SqDistance(owner->frontdir) >
               0.25 &&
    ...
    if (wantedPos.SqDistance2D(owner->pos) <
        ...
        if (am != NULL) {
            ...
            if (owner->pos.y -
                  CGround::GetHeightAboveWater(wantedPos.x, wantedPos.z) <
                SQUARE_SIZE) {
                // nail it to the ground before it tries jumping up, only to
                // land again...
                am->SetState(am->AIRCRAFT_LANDED);
                // call this so that other animations such as opening doors may
                // be started
