...
void
SolidObjectDecalDef::Parse(const LuaTable& table)
{
    groundDecalTypeName = table.GetString(
      "groundDecalType", table.GetString("buildingGroundDecalType", ""));
    trackDecalTypeName = table.GetString("trackType", "StdTank");

    useGroundDecal = table.GetBool(
      "useGroundDecal", table.GetBool("useBuildingGroundDecal", false));
    groundDecalType = -1;
    groundDecalSizeX = table.GetInt(
      "groundDecalSizeX", table.GetInt("buildingGroundDecalSizeX", 4));
    groundDecalSizeY = table.GetInt(
      "groundDecalSizeY", table.GetInt("buildingGroundDecalSizeY", 4));
    groundDecalDecaySpeed =
      table.GetFloat("groundDecalDecaySpeed",
                     table.GetFloat("buildingGroundDecalDecaySpeed", 0.1f));
...
void
SolidObjectDef::PreloadModel() const
{
...
SolidObjectDef::LoadModel() const
{
    if (model == nullptr) {
        if (!modelName.empty()) {
            ...
            // LOG_L(L_WARNING, "[SolidObjectDef::%s] object \"%s\" has no model
...
{
    collisionVolume =
      CollisionVolume(table.GetString("collisionVolumeType", ""),
    ...
    // it ignore hits
    collisionVolume.SetDefaultToPieceTree(
      table.GetBool("usePieceCollisionVolumes", false));
    collisionVolume.SetDefaultToFootPrint(
      table.GetBool("useFootPrintCollisionVolume", false));
