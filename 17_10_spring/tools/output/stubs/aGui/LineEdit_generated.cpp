...
LineEdit::DrawSelf()
{
    ...
    if (hasFocus) {
        ...
        float caretWidth =
          font->GetSize() * font->GetTextWidth(caretStr) / float(screensize[0]);
        ...
        float cursorHeight =
          font->GetSize() * font->GetLineHeight() / float(screensize[1]);
        float cw =
          font->GetSize() * font->GetCharacterWidth(c) / float(screensize[0]);
