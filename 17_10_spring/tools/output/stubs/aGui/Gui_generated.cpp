...
Gui::Draw()
{
    ...
    glLoadIdentity();
    for (ElList::reverse_iterator it = elements.rbegin(); it != elements.rend();
         ++it) {
        (*it).element->Draw();
    }
}
...
Gui::Clean()
{
    for (ElList::iterator it = toBeAdded.begin(); it != toBeAdded.end(); ++it) {
        ...
        for (ElList::iterator elIt = elements.begin(); elIt != elements.end();
             ++elIt) {
    ...
    for (ElList::iterator it = toBeRemoved.begin(); it != toBeRemoved.end();
         ++it) {
        for (ElList::iterator elIt = elements.begin(); elIt != elements.end();
             ++elIt) {
            if (it->element == elIt->element) {
...
{
    // has to be delayed, otherwise deleting a button during a callback would
    // segfault
...
Gui::UpdateScreenGeometry(int screenx,
                          ...
                          int screenOffsetY)
{
    GuiElement::UpdateDisplayGeo(
      screenx, screeny, screenOffsetX, screenOffsetY);
}

bool
Gui::MouseOverElement(const GuiElement* elem, int x, int y) const
{
    for (ElList::const_iterator it = elements.begin(); it != elements.end();
         ++it) {
