...
CONFIG(bool, AutoAddBuiltUnitsToFactoryGroup)
  .defaultValue(false)
  .description("Controls whether or not units built by factories will inherit "
               "that factory's unit group.");
...
void
CSelectedUnitsHandler::Init(unsigned numPlayers)
{
    ...
    autoAddBuiltUnitsToFactoryGroup =
      configHandler->GetBool("AutoAddBuiltUnitsToFactoryGroup");
    autoAddBuiltUnitsToSelectedGroup =
      configHandler->GetBool("AutoAddBuiltUnitsToSelectedGroup");
    netSelected.resize(numPlayers);
}

bool
CSelectedUnitsHandler::IsUnitSelected(const CUnit* unit) const
{
    return (selectedUnits.find(const_cast<CUnit*>(unit)) !=
            selectedUnits.end());
}

bool
CSelectedUnitsHandler::IsUnitSelected(const int unitID) const
{
...
void
CSelectedUnitsHandler::ToggleBuildIconsFirst()
{
    buildIconsFirst = !buildIconsFirst;
    possibleCommandsChanged = true;
}

CSelectedUnitsHandler::AvailableCommandsStruct
CSelectedUnitsHandler::GetAvailableCommands()
{
    ...
    for (const CUnit* u : selectedUnits) {
        for (const SCommandDescription* cmdDesc :
             u->commandAI->GetPossibleCommands()) {
...
void
CSelectedUnitsHandler::GiveCommand(Command c, bool fromUser)
{
    ...
    if (fromUser) { // add some statistics
        ...
        if (selectedGroup != -1) {
            playerHandler->Player(gu->myPlayerNum)->currentStats.unitCommands +=
              grouphandlers[gu->myTeam]->groups[selectedGroup]->units.size();
        } else {
            playerHandler->Player(gu->myPlayerNum)->currentStats.unitCommands +=
              selectedUnits.size();
        }
    ...
    if (cmd_id == CMD_GROUPCLEAR) {
        for (CUnitSet::iterator ui = selectedUnits.begin();
             ui != selectedUnits.end();
             ++ui) {
            if ((*ui)->group) {
                (*ui)->SetGroup(0);
                possibleCommandsChanged = true;
...
void
CSelectedUnitsHandler::HandleUnitBoxSelection(const float4& planeRight,
                                              ...
                                              const float4& planeBottom)
{
    ...
    for (; team <= lastTeam; team++) {
        for (CUnit* u : teamHandler->Team(team)->units) {
            ...
            if (vec.dot4(planeRight) < 0.0f && vec.dot4(planeLeft) < 0.0f &&
                vec.dot4(planeTop) < 0.0f && vec.dot4(planeBottom) < 0.0f) {
                if (KeyInput::GetKeyModState(KMOD_CTRL) &&
                    (selectedUnits.find(u) != selectedUnits.end())) {
                    RemoveUnit(u);
                } else {
                    AddUnit(u);
                    unit = u;
    ...
    if (addedunits >= 2) {
        ...
        Channels::UnitReply->PlayRandomSample(unit->unitDef->sounds.select,
                                              unit);
    }
}

void
CSelectedUnitsHandler::HandleSingleUnitClickSelection(CUnit* unit,
                                                      bool doInViewTest,
                                                      bool selectType)
{
    // FIXME make modular?
    ...
    if (!selectType) {
        if (KeyInput::GetKeyModState(KMOD_CTRL) &&
            (selectedUnits.find(unit) != selectedUnits.end())) {
            RemoveUnit(unit);
        } else {
            AddUnit(unit);
        }
    } else {
        // double click, select all units of same type (on screen, unless CTRL
        // is pressed)
        int team, lastTeam;
        ...
        for (; team <= lastTeam; team++) {
            for (CUnit* u : teamHandler->Team(team)->units) {
                if (u->unitDef->id == unit->unitDef->id) {
                    if (!doInViewTest || KeyInput::GetKeyModState(KMOD_CTRL) ||
                        camera->InView((u)->midPos)) {
                        AddUnit(u);
                    }
                }
            }
...
void
CSelectedUnitsHandler::AddUnit(CUnit* unit)
{
    ...
    if (trans != NULL && trans->unitDef->IsTransportUnit() &&
        !trans->unitDef->isFirePlatform) {
        return;
    }
...
void
CSelectedUnitsHandler::RemoveUnit(CUnit* unit)
{
...
void
CSelectedUnitsHandler::ClearSelected()
{
...
void
CSelectedUnitsHandler::SelectGroup(int num)
{
    ClearSelected();
    selectedGroup = num;
...
void
CSelectedUnitsHandler::SelectUnits(const std::string& line)
{
    ...
    for (int i = 0; i < (int)args.size(); i++) {
        ...
        if (arg == "clear") {
            ...
            if ((unitIndex < 0) || (static_cast<unsigned int>(unitIndex) >=
...
void
CSelectedUnitsHandler::SelectCycle(const std::string& command)
{
...
CSelectedUnitsHandler::Draw()
{
    ...
    if (color1.a > 0) {
        ...
        if (selectedGroup != -1) {
            // note: units in this set are not necessarily all selected
            // themselves, eg. if autoAddBuiltUnitsToSelectedGroup is true, so
            // we check IsUnitSelected for each
        ...
        for (CUnitSet::const_iterator ui = unitSet->begin();
             ui != unitSet->end();
             ++ui) {
            const CUnit* unit = *ui;
            ...
            const int uhxsize = (unit->xsize * SQUARE_SIZE) >> 1,
                      ...
                      mhxsize = (moveDef == NULL)
                                  ? uhxsize
                                  : ((moveDef->xsize * SQUARE_SIZE) >> 1),
                      mhzsize = (moveDef == NULL)
                                  ? uhzsize
                                  : ((moveDef->zsize * SQUARE_SIZE) >> 1);
            const float3 verts[8] = {
                // UnitDef footprint corners
                float3(unit->drawPos.x + uhxsize,
                       unit->drawPos.y,
                       unit->drawPos.z + uhzsize),
                float3(unit->drawPos.x - uhxsize,
                       unit->drawPos.y,
                       unit->drawPos.z + uhzsize),
                float3(unit->drawPos.x - uhxsize,
                ...
                float3(unit->drawPos.x + mhxsize,
                       unit->drawPos.y,
                       unit->drawPos.z + mhzsize),
                float3(unit->drawPos.x - mhxsize,
                       unit->drawPos.y,
                       unit->drawPos.z + mhzsize),
                float3(unit->drawPos.x - mhxsize,
            ...
            if (globalRendering->drawdebug &&
                (mhxsize != uhxsize || mhzsize != uhzsize)) {
    ...
    if (cmdColors.buildBox[3] > 0.0f) {
        if (!selectedUnits.empty() &&
            ...
            for (const auto bi : unitHandler->GetBuilderCAIs()) {
                ...
                } else if (teamHandler->AlliedTeams(builder->team,
                                                    gu->myTeam)) {
                    if (myColor) {
...
void
CSelectedUnitsHandler::DependentDied(CObject* o)
{
...
void
CSelectedUnitsHandler::NetSelect(std::vector<int>& s, int playerId)
{
...
void
CSelectedUnitsHandler::NetOrder(Command& c, int playerId)
{
    assert(unsigned(playerId) < netSelected.size());
...
void
CSelectedUnitsHandler::ClearNetSelect(int playerId)
{
    netSelected[playerId].clear();
}

void
CSelectedUnitsHandler::AiOrder(int unitid, const Command& c, int playerId)
{
    CUnit* unit = unitHandler->units[unitid];
    if (unit == NULL) {
        return;
    ...
    if (!player->CanControlTeam(unit->team)) {
        ...
        // LOG_L(L_WARNING, "Invalid order from player %i for (unit %i %s, team
        // %i)", 		playerId, unitid, unit->unitDefName.c_str(),
        //unit->team);
        return;
    }
...
bool
CSelectedUnitsHandler::CommandsChanged()
{
...
static inline bool
CanDamage(const UnitDef* ud)
{
    return ((ud->canAttack && !ud->weapons.empty()) || ud->canKamikaze);
}

static inline bool
IsBetterLeader(const UnitDef* newDef, const UnitDef* oldDef)
{
...
// CMouseHandler::DrawCursor --> DrawCentroidCursor -->
// CGuiHandler::GetDefaultCommand --> GetDefaultCmd
// LuaUnsyncedRead::GetDefaultCommand --> CGuiHandler::GetDefaultCommand -->
// GetDefaultCmd
int
CSelectedUnitsHandler::GetDefaultCmd(const CUnit* unit, const CFeature* feature)
{
    int luaCmd;
    ...
    if (targetUnit) {
        targetIsEnemy =
          !teamHandler->Ally(gu->myAllyTeam, targetUnit->allyteam);
    }
...
void
CSelectedUnitsHandler::PossibleCommandChange(CUnit* sender)
{
...
void
CSelectedUnitsHandler::DrawCommands()
{
    glDisable(GL_TEXTURE_2D);
    glDisable(GL_DEPTH_TEST);
    ...
    if (selectedGroup != -1) {
        CUnitSet& groupUnits =
          grouphandlers[gu->myTeam]->groups[selectedGroup]->units;
...
std::string
CSelectedUnitsHandler::GetTooltip()
{
    std::string s = "";
    {
        if (!selectedUnits.empty()) {
            ...
            // show the player name instead of unit name if it has FBI tag
            // showPlayerName
...
void
CSelectedUnitsHandler::SetCommandPage(int page)
{
    for (const CUnit* u : selectedUnits) {
...
void
CSelectedUnitsHandler::SendCommand(const Command& c)
{
    if (selectionChanged) {
        // send new selection
    ...
    clientNet->Send(CBaseNetProtocol::Get().SendCommand(
      gu->myPlayerNum, c.GetID(), c.options, c.params));
}

void
CSelectedUnitsHandler::SendCommandsToUnits(const std::vector<int>& unitIDs,
                                           ...
                                           bool pairwise)
{
    ...
    for (unsigned c = 0; c < commandCount; c++) {
        ...
        if (sameCmdParamSize != 0xFFFF &&
            sameCmdParamSize != commands[c].params.size())
            sameCmdParamSize = 0xFFFF;
    }

    unsigned msgLen = 0;
    msgLen += (1 + 2 + 1 + 1 + 1 + 4 + 1 + 2); // msg type, msg size, player ID,
                                               // AI ID, pairwise, sameCmdID,
                                               // sameCmdOpt, sameCmdParamSize
    msgLen += 2;                               // unitID count
    msgLen += unitIDCount * 2;
    msgLen += 2; // command count
    int psize = ((sameCmdID == 0) ? 4 : 0) + ((sameCmdOpt == 0xFF) ? 1 : 0) +
                ((sameCmdParamSize == 0xFFFF) ? 2 : 0);
    msgLen += commandCount * psize; // id, options, params size
    msgLen += totalParams * 4;
    if (msgLen > 8192) {
        LOG_L(L_WARNING,
              "Discarded oversized NETMSG_AICOMMANDS packet: %i",
              msgLen);
    ...
    *packet << static_cast<unsigned short>(unitIDCount);
    for (std::vector<int>::const_iterator it = unitIDs.begin();
         it != unitIDs.end();
         ++it) {
