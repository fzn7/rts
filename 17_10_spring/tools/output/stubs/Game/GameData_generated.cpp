...
GameData::GameData(boost::shared_ptr<const RawPacket> pckt)
{
    ...
    while ((ret = uncompress(
              &buffer[0], &rawSize, &compressed[0], compressed.size())) ==
           Z_BUF_ERROR) {
        bufSize *= 2;
        rawSize = bufSize;
    ...
    if (ret != Z_OK)
        throw netcode::UnpackPacketException(
          "Error while decompressing GameData");
...
GameData::Pack() const
{
    if (compressed.empty()) {
        ...
        const int error =
          compress(&compressed[0],
                   &bufsize,
                   reinterpret_cast<const boost::uint8_t*>(setupText.c_str()),
                   setupText.length());
        compressed.resize(bufsize);
        assert(error == Z_OK);
    }
