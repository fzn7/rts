...
CONFIG(bool, HardwareCursor)
  .defaultValue(false)
  .description("Sets hardware mouse cursor rendering. If you have a low "
               "framerate, your mouse cursor will seem \"laggy\". Setting "
               "hardware cursor will render the mouse cursor separately from "
               "spring and the mouse will behave normally. Note, not all GPU "
               "drivers support it in fullscreen mode!");
CONFIG(bool, InvertMouse).defaultValue(false);
CONFIG(float, DoubleClickTime)
  .defaultValue(200.0f)
...
CMouseHandler::LoadCursors()
{
    ...
    AssignMouseCursor("ManualFire",
                      "cursordgun",
                      mCenter,
                      false); // backup (backward compability)
...
void
CMouseHandler::MouseMove(int x, int y, int dx, int dy)
{
    ...
    const int screenCenterX =
      globalRendering->viewSizeX / 2 + globalRendering->viewPosX;
    const int screenCenterY =
      globalRendering->viewSizeY / 2 + globalRendering->viewPosY;
    ...
    if (locked) {
        camHandler->GetCurrentController().MouseMove(
          float3(dx, dy, invertMouse ? -1.0f : 1.0f));
        return;
    }
    ...
    if (!game->IsGameOver()) {
        playerHandler->Player(gu->myPlayerNum)->currentStats.mousePixels +=
          movedPixels;
    }
...
void
CMouseHandler::MousePress(int x, int y, int button)
{
    ...
    bp.chorded =
      (buttons[SDL_BUTTON_LEFT].pressed || buttons[SDL_BUTTON_RIGHT].pressed);
    bp.pressed = true;
    bp.time = gu->gameTime;
    bp.x = x;
    bp.y = y;
 ...
 *  returns the topright & bottomleft corner positions of the SelectionBox in
 * (cam->right, cam->up)-space
 */
void
CMouseHandler::GetSelectionBoxCoeff(const float3& pos1,
                                    const float3& dir1,
                                    const float3& pos2,
                                    const float3& dir2,
                                    float2& topright,
                                    float2& btmleft)
{
    float dist = CGround::LineGroundCol(
      pos1, pos1 + dir1 * globalRendering->viewRange * 1.4f, false);
    if (dist < 0)
        dist = globalRendering->viewRange * 1.4f;
    float3 gpos1 = pos1 + dir1 * dist;

    dist = CGround::LineGroundCol(
      pos2, pos2 + dir2 * globalRendering->viewRange * 1.4f, false);
    if (dist < 0)
        dist = globalRendering->viewRange * 1.4f;
    ...
    if (topright.x < btmleft.x)
        std::swap(topright.x, btmleft.x);
    if (topright.y < btmleft.y)
        std::swap(topright.y, btmleft.y);
}

void
CMouseHandler::MouseRelease(int x, int y, int button)
{
    ...
    if (activeReceiver) {
        ...
        if (!buttons[SDL_BUTTON_LEFT].pressed &&
            !buttons[SDL_BUTTON_MIDDLE].pressed &&
            !buttons[SDL_BUTTON_RIGHT].pressed)
            activeReceiver = NULL;
        return;
    }

    // Switch camera mode on a middle click that wasn't a middle mouse drag
    // scroll. the latter is determined by the time the mouse was held down:
    // switch (dragScrollThreshold)
    //   <= means a camera mode switch
    //    > means a drag scroll
    if (button == SDL_BUTTON_MIDDLE) {
        if (buttons[SDL_BUTTON_MIDDLE].time >
            (gu->gameTime - dragScrollThreshold))
    ...
    if ((button == SDL_BUTTON_LEFT) && !buttons[button].chorded) {
        ...
        if (bp.movement > 4) {
            ...
            GetSelectionBoxCoeff(
              bp.camPos, bp.dir, camera->GetPos(), dir, topright, btmleft);

            // GetSelectionBoxCoeff returns us the corner pos, but we want to do
            // a inview frustum check. To do so we need the frustum planes (=
...
#define signf(x) ((x > 0.0f) ? 1.0f : -1)
            if (topright.y != 0)
                norm1 = (camera->GetDir() * signf(-topright.y)) +
                        (camera->GetUp() / math::fabs(topright.y));
            if (btmleft.y != 0)
                norm2 = (camera->GetDir() * signf(btmleft.y)) -
                        (camera->GetUp() / math::fabs(btmleft.y));
            if (topright.x != 0)
                norm3 = (camera->GetDir() * signf(-topright.x)) +
                        (camera->GetRight() / math::fabs(topright.x));
            if (btmleft.x != 0)
                norm4 = (camera->GetDir() * signf(btmleft.x)) -
                        (camera->GetRight() / math::fabs(btmleft.x));
            ...
            TraceRay::GuiTraceRay(camera->GetPos(),
                                  dir,
            ...
            const bool selectType =
              (bp.lastRelease >= (gu->gameTime - doubleClickTime) &&
               unit == _lastClicked);

            selectedUnitsHandler.HandleSingleUnitClickSelection(
              const_cast<CUnit*>(unit), true, selectType);
        }
...
void
CMouseHandler::MouseWheel(float delta)
{
...
void
CMouseHandler::DrawSelectionBox()
{
...
std::string
CMouseHandler::GetCurrentTooltip()
{
    std::string s;
    std::list<CInputReceiver*>& inputReceivers = GetInputReceivers();
    std::list<CInputReceiver*>::iterator ri;
    ...
    {
        dist = TraceRay::GuiTraceRay(
          camera->GetPos(), dir, range, NULL, unit, feature, true, false, true);
...
void
CMouseHandler::WarpMouse(int x, int y)
{
    if (!locked) {
...
void
CMouseHandler::ShowMouse()
{
    if (hide) {
        hide = false;
        cursorText = "none"; // force hardware cursor rebinding (else we have
                             // standard b&w cursor)

        // I don't use SDL_ShowCursor here 'cos it would cause a flicker with
        // hwCursor (flicker caused by switching between default cursor and
        // later the really one e.g. `attack`) instead update state and cursor
        // at the same time
        if (hardwareCursor) {
            hwHide = true;
        } else {
...
void
CMouseHandler::ToggleMiddleClickScroll()
{
    if (locked) {
        locked = false;
        ShowMouse();
...
void
CMouseHandler::ToggleHwCursor(const bool& enable)
{
...
void
CMouseHandler::ChangeCursor(const std::string& cmdName, const float& scale)
{
    newCursor = cmdName;
    cursorScale = scale;
}
...
void
CMouseHandler::SetCursor(const std::string& cmdName, const bool& forceRebind)
{
...
void
CMouseHandler::DrawScrollCursor()
{
    const float scaleL =
      math::fabs(std::min(0.0f, scrollx)) * crossMoveScale + 1.0f;
    const float scaleT =
      math::fabs(std::min(0.0f, scrolly)) * crossMoveScale + 1.0f;
    const float scaleR =
      math::fabs(std::max(0.0f, scrollx)) * crossMoveScale + 1.0f;
    const float scaleB =
      math::fabs(std::max(0.0f, scrolly)) * crossMoveScale + 1.0f;
...
CMouseHandler::DrawFPSCursor()
{
    ...
    for (float angle = 0.0f; angle < fastmath::PI2;
         angle += fastmath::PI2 / 3.f) {
        for (int i = -stepNumHalf; i < stepNumHalf; i++) {
            glVertex2f(0.1f * fastmath::sin(angle),
                       0.1f * fastmath::cos(angle));
            glVertex2f(0.8f * fastmath::sin(angle + i * step),
                       0.8f * fastmath::cos(angle + i * step));
            glVertex2f(0.8f * fastmath::sin(angle + (i + 1) * step),
                       0.8f * fastmath::cos(angle + (i + 1) * step));
        }
    }
    glEnd();
...
CMouseHandler::DrawCursor()
{
    ...
    if (locked) {
        if (crossSize > 0.0f) {
            ...
            glPushMatrix();
            glTranslatef(0.5f - globalRendering->pixelX * 0.5f,
                         0.5f - globalRendering->pixelY * 0.5f,
                         0.f);
    ...
    if (cursorScale >= 0.0f) {
        ...
        // hovered minimap, show default cursor and draw `special` cursor
        // scaled-down bottom right of the default one
...
bool
CMouseHandler::AssignMouseCursor(const std::string& cmdName,
...
bool
CMouseHandler::ReplaceMouseCursor(const string& oldName,
...
void
CMouseHandler::SafeDeleteCursor(CMouseCursor* cursor)
{
...
void
CMouseHandler::ConfigNotify(const std::string& key, const std::string& value)
{
