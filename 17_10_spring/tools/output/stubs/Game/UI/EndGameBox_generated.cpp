...
CEndGameBox::MousePress(int x, int y, int button)
{
    ...
    if (InBox(mx, my, box)) {
        ...
        if (dispMode > 0 && mx > box.x1 + 0.01f && mx < box.x1 + 0.12f &&
            my < box.y1 + 0.57f &&
            my > box.y1 + 0.571f - stats.size() * 0.02f) {
            moveBox = false;
        }
        return true;
    }
...
CEndGameBox::Draw()
{
    ...
    font->glPrint(box.x1 + exitBox.x1 + 0.025f,
                  box.y1 + exitBox.y1 + 0.005f,
                  1.0f,
                  FONT_SCALE | FONT_NORM,
                  "Exit");
    font->glPrint(box.x1 + playerBox.x1 + 0.015f,
                  box.y1 + playerBox.y1 + 0.005f,
                  0.7f,
                  FONT_SCALE | FONT_NORM,
                  "Player stats");
    font->glPrint(box.x1 + sumBox.x1 + 0.015f,
                  box.y1 + sumBox.y1 + 0.005f,
                  0.7f,
                  FONT_SCALE | FONT_NORM,
                  "Team stats");
    font->glPrint(box.x1 + difBox.x1 + 0.015f,
                  box.y1 + difBox.y1 + 0.005f,
                  0.7f,
                  FONT_SCALE | FONT_NORM,
                  "Team delta stats");

    if (winners.empty()) {
        font->glPrint(box.x1 + 0.25f,
                      box.y1 + 0.65f,
                      1.0f,
                      FONT_SCALE | FONT_NORM,
                      "Game result was undecided");
    } else {
    ...
    if (dispMode == 0) {
        ...
        std::string headers[] = { "Name", "MC/m",   "MP/m",
                                  "KP/m", "Cmds/m", "ACS" };

        for (int a = 0; a < 6; ++a) {
            font->glPrint(box.x1 + xpos,
                          box.y1 + 0.55f,
                          0.8f,
                          FONT_SCALE | FONT_NORM,
                          headers[a].c_str());
        ...
        for (int a = 0; a < playerHandler->ActivePlayers(); ++a) {
            ...
            if (game->totalGameTime > 0) { // prevent div zero
                SNPRINTF(values[1],
                         100,
                         "%i",
                         int(pStats.mouseClicks * 60 / game->totalGameTime));
                SNPRINTF(values[2],
                         100,
                         "%i",
                         int(pStats.mousePixels * 60 / game->totalGameTime));
                SNPRINTF(values[3],
                         100,
                         "%i",
                         int(pStats.keyPresses * 60 / game->totalGameTime));
                SNPRINTF(values[4],
                         100,
                         "%i",
                         int(pStats.numCommands * 60 / game->totalGameTime));
            } else {
                for (int i = 1; i < 5; i++)
        ...
        if ((mx > box.x1 + 0.01f) && (mx < box.x1 + 0.12f) &&
            ...
            va->AddVertex0(
              float3(box.x1 + 0.01f, box.y1 + 0.55f - (sel * 0.02f), 0));
            va->AddVertex0(float3(
              box.x1 + 0.01f, box.y1 + 0.55f - (sel * 0.02f) + 0.02f, 0));
            va->AddVertex0(float3(
              box.x1 + 0.12f, box.y1 + 0.55f - (sel * 0.02f) + 0.02f, 0));
            va->AddVertex0(
              float3(box.x1 + 0.12f, box.y1 + 0.55f - (sel * 0.02f), 0));
        ...
        if (dispMode == 1) {
            maxy =
              std::max(stats[stat1].max, (stat2 != -1) ? stats[stat2].max : 0);
        } else {
            maxy = std::max(stats[stat1].maxdif,
                            (stat2 != -1) ? stats[stat2].maxdif : 0) /
                   TeamStatistics::statsPeriod;
        }

        int numPoints = stats[0].values[0].size();

        for (int a = 0; a < 5; ++a) {
            font->glPrint(box.x1 + 0.12f,
                          box.y1 + 0.07f + (a * 0.135f),
                          0.8f,
                          FONT_SCALE | FONT_NORM,
                          FloatToSmallString(maxy * 0.25f * a));
            font->glFormat(
              box.x1 + 0.135f + (a * 0.135f),
              box.y1 + 0.057f,
              0.8f,
              FONT_SCALE | FONT_NORM,
              "%02i:%02i",
              (int)(a * 0.25f * numPoints * TeamStatistics::statsPeriod / 60),
              (int)(a * 0.25f * (numPoints - 1) * TeamStatistics::statsPeriod) %
                60);
        }

        font->glPrint(box.x1 + 0.55f,
                      box.y1 + 0.65f,
                      0.8f,
                      FONT_SCALE | FONT_NORM,
                      stats[stat1].name);
        ...
        for (int team = 0; team < teamHandler->ActiveTeams(); team++) {
            ...
            for (int a = 0; a < numPoints; ++a) {
                ...
                if (dispMode == 1) {
                    ...
                    value = (stats[stat1].values[team][a] -
                             stats[stat1].values[team][a - 1]) /
                            TeamStatistics::statsPeriod;
                }

                glVertex3f(box.x1 + 0.15f + a * scalex,
                           box.y1 + 0.08f + value * scaley,
                           0.0f);
            }
            glEnd();
...
CEndGameBox::FillTeamStats()
{
    ...
    for (int team = 0; team < teamHandler->ActiveTeams(); team++) {
        ...
        for (std::list<TeamStatistics>::const_iterator si =
               pteam->statHistory.begin();
             si != pteam->statHistory.end();
             ++si) {
            ...
            stats[11].AddStat(
              team,
              si->metalProduced + si->metalReceived -
                (si->metalUsed + si->metalSent + si->metalExcess));
            stats[12].AddStat(
              team,
              si->energyProduced + si->energyReceived -
                (si->energyUsed + si->energySent + si->energyExcess));

            stats[13].AddStat(
              team,
              si->unitsProduced + si->unitsReceived + si->unitsCaptured -
