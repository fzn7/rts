...
static const std::vector<DefaultBinding> defaultBindings = {
    ...
    { "Ctrl+v",
      "select "
      "AllMap+_Not_Builder_Not_Commander_InPrevSel_Not_InHotkeyGroup+_"
      "SelectAll+" },
    { "Ctrl+w",
      "select AllMap+_Not_Aircraft_Weapons+_ClearSelection_SelectAll+" },
...
CKeyBindings::GetActionList(const CKeySet& ks) const
{
    ...
    if (ks.AnyMod()) {
        ...
        if (haveNormal && !haveAnyMod) {
            ...
            static ActionList merged; // FIXME switch to thread_local when all
                                      // buildbots are using >=gcc4.7
            merged = nit->second;
    ...
    if (debugEnabled) {
        LOG("GetActions: hex=0x%02X acii=\"%s\":",
            ks.Key(),
            ks.GetString(false).c_str());
        if (alPtr != &empty) {
            int i = 1;
            for (const auto& a : *alPtr) {
                LOG("   %i. action=\"%s\"  rawline=\"%s\"  shortcut=\"%s\"",
                    i++,
                    a.command.c_str(),
                    a.rawline.c_str(),
                    a.boundWith.c_str());
...
{
    static ActionList
      out; // FIXME switch to thread_local when all buildbots are using >=gcc4.7
    out.clear();
              ...
              const size_t pos = std::string::npos)
{
    ...
    // -> when parsing fails, this functions replaces one by one all "," by
    // their hexcode
...
CKeyBindings::UnBindKeyset(const std::string& keystr)
{
    ...
    if (!ks.Parse(keystr)) {
        LOG_L(
          L_WARNING, "UnBindKeyset: could not parse key: %s", keystr.c_str());
        return false;
    }
    bool success = false;
...
CKeyBindings::SetFakeMetaKey(const std::string& keystr)
{
    ...
    if (!ks.Parse(keystr)) {
        LOG_L(
          L_WARNING, "SetFakeMetaKey: could not parse key: %s", keystr.c_str());
        return false;
    }
    fakeMetaKey = ks.Key();
    return true;
        ...
        for (const Action& action : al) {
            ...
            if (unitDefHandler && (action.command.find("buildunit_") == 0)) {
                ...
                const UnitDef* unitDef =
                  unitDefHandler->GetUnitDefByName(unitName);
                if (unitDef) {
                    comment =
