...
CGameInfo::CGameInfo()
{
    ...
    sprintf(buf,
            "%.2f (%.2f e/f^2)",
            -(mapInfo->map.gravity * GAME_SPEED * GAME_SPEED),
            -mapInfo->map.gravity);
    ...
    sprintf(buf,
            ...
            wind.GetMinWind(),
            wind.GetMaxWind(),
            (wind.GetMinWind() + wind.GetMaxWind()) * 0.5f);
    values.push_back(buf);
...
CGameInfo::Draw()
{
    ...
    const float rowHeight =
      std::max(labelsHeight, valuesHeight) + 2.0f * yBorder;
    ...
    for (i = 0; i < (int)labels.size(); i++) {
        ...
        font->glPrint(
          box.x1 + xBorder, y, 1.0f, FONT_SCALE | FONT_NORM, lfs.msg);
        font->glPrint(box.x2 - xBorder,
                      y,
                      1.0f,
