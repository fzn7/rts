...
CONFIG(int, InfoMessageTime)
  .defaultValue(10)
  .description("Timeout till old messages disappear from the ingame console.");
  ...
  , maxLines(1)
{
    ...
    const int vars =
      sscanf(geo.c_str(), "%f %f %f %f", &xpos, &ypos, &width, &height);
    if (vars != 4) {
        xpos = 0.26f;
        ypos = 0.96f;
        width = 0.41f;
...
CInfoConsole::Draw()
{
    ...
    const float fontHeight =
      fontSize * smallFont->GetLineHeight() * globalRendering->pixelY;
...
CInfoConsole::Update()
{
    ...
    const float maxHeight =
      (height * globalRendering->viewSizeY) - (border * 2);
    maxLines =
      (smallFont->GetLineHeight() > 0)
...
void
CInfoConsole::PushNewLinesToEventHandler()
{
    if (newLines == 0)
        return;
    ...
    for (std::deque<RawLine>::iterator it = newRawLines.begin();
         it != newRawLines.end();
         ++it) {
        const RawLine& rawLine = (*it);
        eventHandler.AddConsoleLine(
          rawLine.text, rawLine.section, rawLine.level);
    }
}

int
CInfoConsole::GetRawLines(std::deque<RawLine>& lines)
{
    boost::recursive_mutex::scoped_lock scoped_lock(infoConsoleMutex);
    lines = rawData;
    int tmp = newLines;
...
void
CInfoConsole::RecordLogMessage(const std::string& section,
                               int level,
                               const std::string& text)
{
    ...
    std::list<std::string> lines =
      smallFont->SplitIntoLines(toustring(wrappedText));
...
void
CInfoConsole::LastMessagePosition(const float3& pos)
{
...
const float3&
CInfoConsole::GetMsgPos(const float3& defaultPos)
{
