  ...
  , enabled(true)
{
    ...
    metalBox.y2 =
      metalBox.y1 + (box.y2 - box.y1); // extend to the very top of the screen

    energyBox.x1 = 0.48f;
    energyBox.y1 = 0.01f;
    energyBox.x2 = box.x2 - 0.03f - box.x1;
    energyBox.y2 =
      energyBox.y1 + (box.y2 - box.y1); // extend to the very top of the screen
}
...
CResourceBar::Draw()
{
    ...
    const float headerFontSize =
      (box.y2 - box.y1) * 0.7 * globalRendering->viewSizeY;
    const float labelsFontSize =
      (box.y2 - box.y1) * 0.5 * globalRendering->viewSizeY;
    const int fontOptions = (guihandler && guihandler->GetOutlineFonts())
                              ? FONT_OUTLINE | FONT_NORM
                              : FONT_NORM;

    smallFont->SetTextColor(0.8f, 0.8f, 1, 0.8f);
    smallFont->glPrint(metalx - 0.004f,
                       (box.y1 + box.y2) * 0.5,
                       headerFontSize,
                       FONT_VCENTER | fontOptions,
                       "Metal");

    smallFont->SetTextColor(1, 1, 0.4f, 0.8f);
    smallFont->glPrint(energyx - 0.018f,
                       (box.y1 + box.y2) * 0.5,
                       headerFontSize,
                       FONT_VCENTER | fontOptions,
                       "Energy");
    ...
    smallFont->glFormat(
      metalx + 0.044f,
      box.y2 - 2 * globalRendering->pixelY,
      labelsFontSize,
      FONT_ASCENDER | fontOptions,
      "+%s",
    ...
    smallFont->glPrint(energybarx2 - 0.01f,
                       energyy - 0.005f,
                       labelsFontSize,
                       fontOptions,
                       FloatToSmallString(myTeam->resStorage.energy));
    smallFont->glPrint(energybarx1 + energybarlen / 2.0f,
                       energyy - 0.005f,
                       labelsFontSize,
                       fontOptions,
                       FloatToSmallString(myTeam->res.energy));

    smallFont->glPrint(metalbarx2 - 0.01f,
                       metaly - 0.005f,
                       labelsFontSize,
                       fontOptions,
                       FloatToSmallString(myTeam->resStorage.metal));
    smallFont->glPrint(metalbarx1 + metalbarlen / 2.0f,
                       metaly - 0.005f,
                       labelsFontSize,
                       fontOptions,
...
CResourceBar::GetTooltip(int x, int y)
{
    ...
    return "Shows your stored " + resourceName +
           " as well as\n"
           "income(\xff\x40\xff\x40green\xff\xff\xff\xff) and expenditures "
           "(\xff\xff\x20\x20red\xff\xff\xff\xff).\n"
    ...
    if (InBox(mx, my, box)) {
        ...
        if (!gu->spectating) {
            if (InBox(mx, my, box + metalBox)) {
                moveBox = false;
                const float metalShare = Clamp((mx - (box.x1 + metalBox.x1)) /
            ...
            if (InBox(mx, my, box + energyBox)) {
                moveBox = false;
                const float energyShare = Clamp((mx - (box.x1 + energyBox.x1)) /
