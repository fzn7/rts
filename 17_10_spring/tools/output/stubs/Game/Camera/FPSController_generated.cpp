...
CFPSController::KeyMove(float3 move)
{
    move *= move.z * 400;
    pos +=
      (camera->GetDir() * move.y + camera->GetRight() * move.x) * scrollSpeed;
    Update();
}
...
CFPSController::MouseMove(float3 move)
{
    ...
    camera->SetRotX(Clamp(
      camera->GetRot().x + mouseScale * move.y * move.z, 0.01f, PI * 0.99f));
    dir = camera->GetDir();
    Update();
}
...
CFPSController::Update()
{
    if (!gu->fpsMode) {
        ...
        const float gndHeight =
          CGround::GetHeightAboveWater(pos.x, pos.z, false);
