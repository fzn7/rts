  ...
  const CInMapDrawModel* inMapDrawModel) const
{
    ...
    const bool allied = (teamHandler->Ally(gu->myAllyTeam, allyTeam) &&
                         teamHandler->Ally(allyTeam, gu->myAllyTeam));
    const bool maySee = (gu->spectating || (!spectator && allied) ||
                         inMapDrawModel->drawAllMarks);
                          ...
                          int playerID)
{
    ...
    if (allowed &&
        eventHandler.MapDrawCmd(playerID, MAPDRAW_POINT, &pos, NULL, &label)) {
        return false;
    }
    ...
    const int quad =
      int(pos.z * QUAD_SCALE) * drawQuadsX + int(pos.x * QUAD_SCALE);
                         ...
                         int playerID)
{
    ...
    if (AllowedMsg(sender) &&
        eventHandler.MapDrawCmd(playerID, MAPDRAW_LINE, &pos1, &pos2, NULL)) {
        return false;
    }
    ...
    const int quad =
      int(pos1.z * QUAD_SCALE) * drawQuadsX + int(pos1.x * QUAD_SCALE);
...
CInMapDrawModel::EraseNear(const float3& constPos, int playerID)
{
    ...
    if (AllowedMsg(sender) &&
        eventHandler.MapDrawCmd(playerID, MAPDRAW_ERASE, &pos, NULL, NULL)) {
        return;
    }
        ...
        for (int x = xStart; x <= xEnd; ++x) {
            ...
            for (pi = dq->points.begin(); pi != dq->points.end(); /* none */) {
                if (pi->GetPos().SqDistance2D(pos) < (radius * radius) &&
                    (pi->IsBySpectator() == sender->spectator)) {
            ...
            for (li = dq->lines.begin(); li != dq->lines.end(); /* none */) {
                ...
                if (li->GetPos1().SqDistance2D(pos) < (radius * radius) &&
                    (li->IsBySpectator() == sender->spectator)) {
