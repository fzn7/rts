...
class AirMeshActionExecutor : public IUnsyncedActionExecutor
{
  public:
    ...
    AirMeshActionExecutor()
      : IUnsyncedActionExecutor(COMMAND_NAME,
                                "Show/Hide the smooth air-mesh map overlay")
    {}

    bool Execute(const UnsyncedAction& action) const
    {
        InverseOrSetBool(smoothHeightMeshDrawer->DrawEnabled(),
                         action.GetArgs());
        LogSystemStatus("smooth air-mesh map overlay",
                        smoothHeightMeshDrawer->DrawEnabled());
        return true;
    }
};
...
SmoothHeightMeshDrawer::~SmoothHeightMeshDrawer()
{
    ...
    if (unsyncedGameCommands != NULL) {
        unsyncedGameCommands->RemoveActionExecutor(
          AirMeshActionExecutor::COMMAND_NAME);
    }
}
...
SmoothHeightMeshDrawer::Draw(float yoffset)
{
    ...
    for (unsigned int zq = 0; zq <= numQuadsZ; zq++) {
        for (unsigned int xq = 0; xq <= numQuadsX; xq++) {
            ...
            const float h2 =
              smoothGround->GetHeightAboveWater(x + quadSize, z) + yoffset;
            const float h3 =
              smoothGround->GetHeightAboveWater(x + quadSize, z + quadSize) +
              yoffset;
            const float h4 =
              smoothGround->GetHeightAboveWater(x, z + quadSize) + yoffset;
