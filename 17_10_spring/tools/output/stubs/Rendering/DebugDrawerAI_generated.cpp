...
DebugDrawerAI::DebugDrawerAI()
  : draw(false)
{
    graphs.resize(teamHandler->ActiveTeams(),
                  Graph(GRAPH_MIN_SCALE, GRAPH_MAX_SCALE));
    texsets.resize(teamHandler->ActiveTeams(), TexSet());
}
DebugDrawerAI::~DebugDrawerAI()
{
    for (std::vector<Graph>::iterator it = graphs.begin(); it != graphs.end();
         ++it) {
        (*it).Clear();
    }
    for (std::vector<TexSet>::iterator it = texsets.begin();
         it != texsets.end();
         ++it) {
        (*it).Clear();
    }
...
void
DebugDrawerAI::Graph::AddPoint(int lineNum, float x, float y)
{
...
DebugDrawerAI::Graph::DelPoints(int lineNum, int numPoints)
{
    ...
    for (ListIt dit = line->lineData.begin(); dit != line->lineData.end();
         ++dit) {
...
DebugDrawerAI::Graph::Draw()
{
...

    {
        ...
        va->AddVertexC(pos + float3(-(((maxLabelWidth * 1.33f) /
        ...
        if (scale.y > 0.0f && scale.x > 0.0f) {
            ...
            for (float s = 0.0f; s <= (scale.y + 0.01f);
                 s += (scale.y * 0.1f)) {
                va->Initialize();
                va->AddVertexC(pos + float3(0.0f, (s / scale.y) * size.y, 0.0f),
                               color);
                va->AddVertexC(
                  pos + float3(size.x, (s / scale.y) * size.y, 0.0f), color);
            ...
            // vertical grid lines
            for (float s = 0.0f; s <= (scale.x + 0.01f);
                 s += (scale.x * 0.1f)) {
                va->Initialize();
                va->AddVertexC(pos + float3((s / scale.x) * size.x, 0.0f, 0.0f),
                               color);
                va->AddVertexC(
                  pos + float3((s / scale.x) * size.x, size.y, 0.0f), color);
                va->DrawArrayC(GL_LINES);

                const float tx =
                  (pos.x + (s / scale.x) * size.x) - (size.x * 0.05f);
...

    {
        ...
        if (!lines.empty()) {
            ...
            for (LineIt lit = lines.begin(); lit != lines.end(); ++lit) {
                ...
                const float sx =
                  (maxLabelWidth / globalRendering->viewSizeX) * size.x;
                const float tx =
                  pos.x - ((line.lineLabelWidth / maxLabelWidth) * 1.33f) * sx;
                const float ty =
                  pos.y + ((lineNum * linePad * 2.0f) + linePad) * size.y;

                font->SetTextColor(
                  line.lineColor.x, line.lineColor.y, line.lineColor.z, 1.0f);
                font->glPrint(
                  tx, ty, 1.0f, FONT_SCALE | FONT_NORM, line.lineLabel);
                ...
                for (ListIt pit = data.begin(); pit != data.end(); ++pit) {
                    ...
                    const float px1 =
                      ((pit->x - minScale.x) / scale.x) * size.x;
                    const float py1 =
                      ((pit->y - minScale.y) / scale.y) * size.y;
                    const float px2 =
                      (npit == data.end())
                        ? px1
                        : ((npit->x - minScale.x) / scale.x) * size.x;
                    const float py2 =
                      (npit == data.end())
                        ? py1
                        : ((npit->y - minScale.y) / scale.y) * size.y;
...
{
    for (std::map<int, TexSet::Texture*>::iterator it = textures.begin();
         it != textures.end();
         ++it) {
        delete (it->second);
    }
...
DebugDrawerAI::TexSet::AddTexture(const float* data, int w, int h)
{
    if (!globalRendering->supportNPOTs &&
        (w != next_power_of_2(w) || h != next_power_of_2(h))) {
        return 0;
    }
...
DebugDrawerAI::TexSet::Draw()
{
    if (!textures.empty()) {
        ...
        for (std::map<int, TexSet::Texture*>::iterator it = textures.begin();
            ...
            const float tx =
              pos.x + size.x * 0.5f -
              ((tex->GetLabelWidth() * 0.5f) / globalRendering->viewSizeX) *
                size.x;
            const float ty =
              pos.y + size.y +
              ((tex->GetLabelHeight() * 0.5f) / globalRendering->viewSizeY) *
                size.y;
  ...
  , labelHeight(0.0f)
{
    ...
    glTexImage2D(
      GL_TEXTURE_2D, 0, intFormat, w, h, 0, extFormat, dataType, data);
    glBindTexture(GL_TEXTURE_2D, 0);
}
