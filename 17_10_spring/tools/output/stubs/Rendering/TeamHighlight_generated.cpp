...
CTeamHighlight::Disable()
{
    ...
    if (!oldColors.empty()) {
        for (std::map<int, int>::iterator i = oldColors.begin();
             i != oldColors.end();
             ++i) {
...
CTeamHighlight::Update(int frameNum)
{
    ...
    bool hl = false;
    if ((globalConfig->teamHighlight == HIGHLIGHT_PLAYERS &&
        ...
        for (int ti = 0; ti < teamHandler->ActiveTeams(); ++ti) {
            ...
            if (!gu->spectatingFullView &&
                !teamHandler->AlliedTeams(gu->myTeam, ti)) {
                continue;
            }
            ...
            if (!hasPlayers || !t->HasLeader()) {
                ...
                teamhighlight =
                  std::max(0, std::min(minPing, maxhl)) / float(maxhl);
            }
