...
CGlobalRendering::PostInit()
{
...

    {
        ...
        const std::string vendor =
          (glVendor != NULL) ? StringToLower(std::string(glVendor)) : "";
        const std::string renderer =
          (glRenderer != NULL) ? StringToLower(std::string(glRenderer)) : "";

        haveATI = (vendor.find("ati ") != std::string::npos) ||
                  (vendor.find("amd ") != std::string::npos);
        haveMesa = (renderer.find("mesa ") != std::string::npos) ||
                   (renderer.find("gallium ") != std::string::npos);
        ...
        if (useGlslShaders < 0) {
            ...
            // rely on extension detection (don't force enable shaders, when the
            // extensions aren't exposed!)
        }

        if (haveATI) {
            // x-series doesn't support NPOTs (but hd-series does)
            supportNPOTs = (renderer.find(" x") == std::string::npos &&
                            renderer.find(" 9") == std::string::npos);
        }
    }
    ...
    if (GLEW_ARB_texture_compression) {
        // we don't even need to check it, 'cos groundtextures must have that
        // extension default to off because it reduces quality (smallest mipmap
        // level is bigger)
    ...
    maxSmoothPointSize =
      std::min(aliasedPointSizeRange.y, smoothPointSizeRange.y);
    ...
    // some GLSL relevant information
    {
        ...
        glslMaxVaryings /= 4; // GL_MAX_VARYING_FLOATS returns max individual
                              // floats, we want float4
    }

    // detect if GL_DEPTH_COMPONENT24 is supported (many ATIs don't do so)
    {
        // ATI seems to support GL_DEPTH_COMPONENT24 for static textures, but
        // you can't render to them
        /*
        GLint state = 0;
        glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, 16, 16, 0,
        GL_LUMINANCE, GL_FLOAT, NULL);
        glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_WIDTH,
        &state); support24bitDepthBuffers = (state > 0);
        */
        ...
        if (FBO::IsSupported() && !atiHacks) {
            ...
            fbo.CreateRenderBuffer(
              GL_COLOR_ATTACHMENT0_EXT, GL_RGBA8, fboSizeX, fboSizeY);
    ...
    LOG(
      ...
      "\tFBO support: %i, NPOT-texture support: %i, 24bit Z-buffer support: "
      "%i\n"
      "\tmaximum texture size: %i, compress MIP-map textures: %i\n"
      "\tmaximum SmoothPointSize: %0.0f, maximum vec4 varying/attributes: "
      "%i/%i\n"
      ...
      glslMaxAttributes,
      glslMaxDrawBuffers,
      glslMaxRecommendedIndices,
      glslMaxRecommendedVertices,
      glslMaxUniformBufferBindings,
      glslMaxUniformBufferSize / 1024);
...
{
    int2 res = int2(configHandler->GetInt("XResolution"),
                    configHandler->GetInt("YResolution"));

    if (!fullscreen) {
        res = int2(configHandler->GetInt("XResolutionWindowed"),
                   configHandler->GetInt("YResolutionWindowed"));
    }
    ...
    //  -> i.e. DVI monitor runs then at 640x400 and HDMI at full-HD (yes with
    //  display _cloning_!)
    {
