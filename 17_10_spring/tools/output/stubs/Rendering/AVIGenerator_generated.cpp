...
CAVIGenerator::initVFW()
{
    ...
    VideoForWindowsVersion_ptr = (VideoForWindowsVersion_type)GetProcAddress(
      msvfw32, "VideoForWindowsVersion");
    ICCompressorChoose_ptr =
      (ICCompressorChoose_type)GetProcAddress(msvfw32, "ICCompressorChoose");
    ICCompressorFree_ptr =
      (ICCompressorFree_type)GetProcAddress(msvfw32, "ICCompressorFree");
    ICOpen_ptr = (ICOpen_type)GetProcAddress(msvfw32, "ICOpen");

    AVIFileInit_ptr = (AVIFileInit_type)GetProcAddress(avifil32, "AVIFileInit");
    AVIFileOpenA_ptr =
      (AVIFileOpenA_type)GetProcAddress(avifil32, "AVIFileOpenA");
    AVIFileCreateStreamA_ptr = (AVIFileCreateStreamA_type)GetProcAddress(
      avifil32, "AVIFileCreateStreamA");
    AVIMakeCompressedStream_ptr = (AVIMakeCompressedStream_type)GetProcAddress(
      avifil32, "AVIMakeCompressedStream");
    AVIStreamSetFormat_ptr =
      (AVIStreamSetFormat_type)GetProcAddress(avifil32, "AVIStreamSetFormat");
    AVIStreamRelease_ptr =
      (AVIStreamRelease_type)GetProcAddress(avifil32, "AVIStreamRelease");
    AVIFileRelease_ptr =
      (AVIFileRelease_type)GetProcAddress(avifil32, "AVIFileRelease");
    AVIFileExit_ptr = (AVIFileExit_type)GetProcAddress(avifil32, "AVIFileExit");
    AVIStreamWrite_ptr =
      (AVIStreamWrite_type)GetProcAddress(avifil32, "AVIStreamWrite");
    ...
    if (!VideoForWindowsVersion_ptr || !ICCompressorChoose_ptr ||
        !ICCompressorFree_ptr || !ICOpen_ptr) {
        errorMsg = "initVFW Error.";
        return false;
    }

    if (!AVIFileInit_ptr || !AVIFileOpenA_ptr || !AVIFileCreateStreamA_ptr ||
        !AVIMakeCompressedStream_ptr || !AVIStreamSetFormat_ptr ||
        !AVIStreamRelease_ptr || !AVIFileRelease_ptr || !AVIFileExit_ptr ||
        !AVIStreamWrite_ptr) {
        errorMsg = "initVFW Error.";
        return false;
    }
...
CAVIGenerator::InitAVICompressionEngine()
{
    ...
    if (wVer < 0x010a) {
        ...
        errorMsg = "Version of Video for Windows too old. Come on, join the "
                   "21th century!";
        return S_FALSE;
    }
    ...
    if (!ICCompressorChoose_ptr(NULL,
    ...
    strHdr.dwSuggestedBufferSize =
      bitmapInfo
        .biSizeImage; // Recommended buffer size, in bytes, for the stream.
    ...
    opts.dwFlags = ((cv.lDataRate > 0) ? AVICOMPRESSF_DATARATE : 0) |
    ...
    hr = AVIFileOpenA_ptr(
      ...
      fileName.c_str(), // Null-terminated string containing the name of the
                        // file to open
      OF_WRITE | OF_CREATE |
        OF_SHARE_EXCLUSIVE, // Access mode to use when opening the file.
    ...
    if (hr != AVIERR_OK) {
        ...
        switch (hr) {
            case AVIERR_BADFORMAT: {
                errorMsg += "The file couldn't be read, indicating a corrupt "
                            "file or an unrecognized format.";
            } break;
            case AVIERR_MEMORY: {
                errorMsg += "The file could not be opened because of "
                            "insufficient memory.";
            } break;
            ...
            case REGDB_E_CLASSNOTREG: {
                errorMsg += "According to the registry, the type of file "
                            "specified in AVIFileOpen does not have a handler "
                            "to process it";
            } break;
            default: {
    ...
    hr =
      AVIMakeCompressedStream_ptr(&m_pStreamCompressed, m_pStream, &opts, NULL);
    if (hr != AVIERR_OK) {
        ...
        switch (hr) {
            ...
            case AVIERR_UNSUPPORTED: {
                errorMsg += "Compression is not supported for this type of "
                            "data. This error might be returned if you try to "
                            "compress data that is not audio or video.";
            } break;
            default: {
                errorMsg += "Unknown error.";
            }
...
CAVIGenerator::InitEngine()
{
    ...
    HWND mainWindow =
      FindWindow(NULL, ("Spring " + SpringVersion::GetFull()).c_str());
    if (globalRendering->fullScreen) {
        ShowWindow(mainWindow, SW_SHOWMINNOACTIVE);
    }
    boost::mutex::scoped_lock lock(AVIMutex);
    AVIThread = new boost::thread(
      boost::bind(&CAVIGenerator::AVIGeneratorThreadProc, this));
...
CAVIGenerator::AddFrame(unsigned char* pixelData)
{
    ...
    // compress bitmap
    HRESULT hr =
      AVIStreamWrite_ptr(m_pStreamCompressed,    // stream pointer
...
CAVIGenerator::AVIGeneratorThreadProc()
{
    ...
    // Run init from the encoding thread because custom controls displayed by
    // codecs  sends window messages to the thread it started from, thus
    // deadlocking if  sending to the main thread whilst it is blocking on
    // readOpenglPixelDataThreaded().
