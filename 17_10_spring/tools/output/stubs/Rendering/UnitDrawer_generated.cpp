...
static const void
BindShadowTexDummy(const CS3OTextureHandler::S3OTexMat*)
{}
static const void
KillShadowTexDummy(const CS3OTextureHandler::S3OTexMat*)
{}

static const void
BindShadowTex(const CS3OTextureHandler::S3OTexMat* textureMat)
{
    glActiveTexture(GL_TEXTURE0);
...
{}
static const void
SetTeamColorValid(const IUnitDrawerState* state, int team, const float2 alpha)
{
    state->SetTeamColor(team, alpha);
}
...
{
    const auto texFun =
      bindModelTexFuncs[shadowHandler->InShadowPass()][mdlType];
...
{
    renderStatePushFuncs[mdlType]();
}
void
CUnitDrawer::PopModelRenderState(int mdlType)
{
    renderStatePopFuncs[mdlType]();
}
...
{
    PushModelRenderState(o->model);
}
void
CUnitDrawer::PopModelRenderState(const CSolidObject* o)
{
    PopModelRenderState(o->model);
}
  ...
  : CEventClient("[CUnitDrawer]", 271828, false)
{
    ...
    alphaValues.x = std::max(
      0.11f,
      std::min(1.0f, 1.0f - configHandler->GetFloat("UnitTransparency")));
    ...
    for (size_t unitDefID = 1; unitDefID < unitDefHandler->unitDefs.size();
         unitDefID++) {
        UnitDef& ud = unitDefHandler->unitDefs[unitDefID];

        for (unsigned int n = 0; n < ud.modelCEGTags.size(); n++) {
            ud.SetModelExplosionGeneratorID(
              n, explGenHandler->LoadGeneratorID(ud.modelCEGTags[n]));
        }
        for (unsigned int n = 0; n < ud.pieceCEGTags.size(); n++) {
            // these can only be custom EG's so prefix is not required game-side
            ud.SetPieceExplosionGeneratorID(
              n,
              explGenHandler->LoadGeneratorID(CEG_PREFIX_STRING +
                                              ud.pieceCEGTags[n]));
        }
    }
    ...
    for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
         modelType++) {
        opaqueModelRenderers[modelType] =
    ...
    unitDrawerStates[DRAWER_STATE_SSP] = IUnitDrawerState::GetInstance(
      globalRendering->haveARB, globalRendering->haveGLSL);
    unitDrawerStates[DRAWER_STATE_FFP] =
      IUnitDrawerState::GetInstance(false, false);
    ...
    advShading = (unitDrawerStates[DRAWER_STATE_SSP]->Init(this) &&
                  cubeMapHandler->Init());

    // note: state must be pre-selected before the first drawn frame
    // Sun*Changed can be called first, e.g. if DynamicSun is enabled
    unitDrawerStates[DRAWER_STATE_SEL] =
      const_cast<IUnitDrawerState*>(GetWantedDrawerState(false));
}
...
CUnitDrawer::~CUnitDrawer()
{
    ...
    unitDrawerStates[DRAWER_STATE_SSP]->Kill();
    IUnitDrawerState::FreeInstance(unitDrawerStates[DRAWER_STATE_SSP]);
    unitDrawerStates[DRAWER_STATE_FFP]->Kill();
    IUnitDrawerState::FreeInstance(unitDrawerStates[DRAWER_STATE_FFP]);
    ...
    for (int allyTeam = 0; allyTeam < deadGhostBuildings.size(); ++allyTeam) {
        for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
            ...
            for (GhostSolidObject* ghost :
                ...
                // <ghost> might be the gbOwner of a decal; groundDecals is
                // deleted after us
...
CUnitDrawer::Update()
{
    ...
    if ((useDistToGroundForIcons =
           (camHandler->GetCurrentController()).GetUseDistToGroundForIcons())) {
        const float3& camPos = camera->GetPos();
        // use the height at the current camera position
        // const float groundHeight = CGround::GetHeightAboveWater(camPos.x,
        // camPos.z, false);
        // use the middle between the highest and lowest position on the map as
        // average
        const float groundHeight =
          (readMap->GetCurrMinHeight() + readMap->GetCurrMaxHeight()) * 0.5f;
...
CUnitDrawer::Draw(bool drawReflection, bool drawRefraction)
{
    ...
    assert((CCamera::GetActiveCamera())->GetCamType() !=
           CCamera::CAMTYPE_SHADOW);
                             ...
                             bool drawRefraction)
{
    ...
    for (auto unitBinIt = unitBin.cbegin(); unitBinIt != unitBin.cend();
         ++unitBinIt) {
                            ...
                            bool drawRefraction)
{
    ...
    if ((unit->pos).SqDistance(camera->GetPos()) >
        (unit->sqRadius * unitDrawDistSqr)) {
...
{
    const std::vector<TempDrawUnit>& tmpOpaqueUnits =
      tempOpaqueUnits[modelType];
    ...
    for (CUnit* u : iconUnits) {
        DrawIcon(
          ...
          !gu->spectatingFullView &&
            ...
            (u->losStatus[gu->myAllyTeam] & (LOS_PREVLOS | LOS_CONTRADAR)) !=
...
{
    if (unit == (drawReflection
                   ? nullptr
                   : (gu->GetMyPlayer())->fpsController.GetControllee()))
        return false;
    if (unit->noDraw)
        return false;
    if (unit->IsInVoid())
    ...
    if (drawReflection && !ObjectVisibleReflection(unit->drawMidPos,
                                                   cam->GetPos(),
                                                   unit->GetDrawRadius()))
        return false;
...
CUnitDrawer::CanDrawOpaqueUnitShadow(const CUnit* unit) const
{
    ...
    const bool unitInLOS =
      ((unit->losStatus[gu->myAllyTeam] & LOS_INLOS) || gu->spectatingFullView);
    const bool unitInView =
      cam->InView(unit->drawMidPos, unit->GetDrawRadius());
...
CUnitDrawer::DrawShadowPass()
{
...

    {
        ...
        for (int modelType = MODELTYPE_S3O; modelType < MODELTYPE_OTHER;
             modelType++) {
...
CUnitDrawer::DrawIcon(CUnit* unit, bool useDefaultIcon)
{
    ...
    // If the icon is to be drawn as a radar blip, we want to get the default
    // icon.
    ...
    //  * The unit radius, depending on whether the mod defined 'radiusadjust'
    //  is true or false.
    const float dist = std::min(
      8000.0f, fastmath::sqrt_builtin(camera->GetPos().SqDistance(pos)));
    const float iconScale =
      0.4f * fastmath::sqrt_builtin(dist); // makes far icons bigger
...
CUnitDrawer::SetupAlphaDrawing(bool deferredPass)
{
    ...
    unitDrawerStates[DRAWER_STATE_SEL] =
      const_cast<IUnitDrawerState*>(GetWantedDrawerState(true));
    unitDrawerStates[DRAWER_STATE_SEL]->Enable(
      this, deferredPass && false, true);
                           ...
                           bool drawGhostBuildingsPass)
{
    ...
    if (drawGhostBuildingsPass) {
        ...
        SetTeamColour(
          unit->team,
          float2((losStatus & LOS_CONTRADAR) ? alphaValues.z : alphaValues.y,
                 1.0f));
        model->DrawStatic();
        glPopMatrix();
...
CUnitDrawer::DrawGhostedBuildings(int modelType)
{
    ...
    for (auto it = deadGhostedBuildings.begin();
         it != deadGhostedBuildings.end();
         ++it) {
...
CUnitDrawer::SetupOpaqueDrawing(bool deferredPass)
{
    ...
    assert(!deferredPass ||
           unitDrawerStates[DRAWER_STATE_SEL]->CanDrawDeferred());
}
...
CUnitDrawer::GetWantedDrawerState(bool alphaPass) const
{
    ...
    const bool enableShaders =
      unitDrawerStates[DRAWER_STATE_SSP]->CanEnable(this);
    const bool permitShaders =
      !alphaPass || unitDrawerStates[DRAWER_STATE_SSP]->CanDrawAlpha();
...
CUnitDrawer::DrawIndividualNoTrans(const CUnit* unit, bool noLuaCall)
{
    ...
    if (!LuaObjectDrawer::DrawSingleObjectNoTrans(
          unit, LUAOBJ_UNIT /*, noLuaCall*/)) {
        PushIndividualOpaqueState(unit, false);
...
static const void
DrawModelWireBuildStage(const CUnit* unit,
...
CUnitDrawer::DrawUnitModelBeingBuiltOpaque(const CUnit* unit, bool noLuaCall)
{
    ...
    const float wireColorMult =
      std::fabs(128.0f - ((gs->frameNum * 4) & 255)) / 255.0f + 0.5f;
    ...
    const float3 stageColors[2] = {
        frameColors[globalRendering->teamNanospray],
        frameColors[globalRendering->teamNanospray]
    };
    const float3 stageBounds = { std::max(model->mins.y, -model->height),
                                 model->height,
                                 unit->buildProgress };
    ...
    const double upperPlanes[] = {
        0.0, -1.0,
        0.0, stageBounds.x + stageBounds.y * (stageBounds.z * 3.0),
        0.0, -1.0,
        0.0, stageBounds.x + stageBounds.y * (stageBounds.z * 3.0 - 1.0),
        0.0, -1.0,
        0.0, stageBounds.x + stageBounds.y * (stageBounds.z * 3.0 - 2.0),
    };
    const double lowerPlanes[] = {
        0.0, 1.0,
        0.0, -stageBounds.x - stageBounds.y * (stageBounds.z * 10.0 - 9.0),
        0.0, 1.0,
        0.0, -stageBounds.x - stageBounds.y * (stageBounds.z * 3.0 - 2.0),
        0.0, 1.0,
        0.0, (0.0),
    ...
    stageFunc(unit,
              &upperPlanes[STAGE_WIRE * 4],
              &lowerPlanes[STAGE_WIRE * 4],
              noLuaCall);

    // flat-colored, conditional
    selState->SetNanoColor(float4(stageColors[1] * flatColorMult, 1.0f));
    stageFunc =
      drawModelBuildStageFuncs[(STAGE_FLAT + 1) * (stageBounds.z > 0.333f)];
    stageFunc(unit,
              &upperPlanes[STAGE_FLAT * 4],
              &lowerPlanes[STAGE_FLAT * 4],
              noLuaCall);
    ...
    stageFunc =
      drawModelBuildStageFuncs[(STAGE_FILL + 1) * (stageBounds.z > 0.666f)];
    stageFunc(unit,
              &upperPlanes[STAGE_FILL * 4],
              &lowerPlanes[STAGE_FILL * 4],
              noLuaCall);
                             ...
                             bool noLuaCall)
{
    const unsigned int b0 =
      lodCall || !unit->beingBuilt || !unit->unitDef->showNanoFrame;
...
CUnitDrawer::UpdateUnitIconState(CUnit* unit)
{
    ...
    if ((losStatus & LOS_INLOS) || gu->spectatingFullView)
        unit->isIcon =
          DrawAsIcon(unit, (unit->pos - camera->GetPos()).SqLength());
                                 ...
                                 const std::vector<Command>& commands)
{
    ...
    const bool canBuild = !!CGameHelper::TestUnitBuildSquare(buildInfo,
                                                             feature,
                                                             -1,
                                                             false,
                                                             &buildableSquares,
    ...
    for (unsigned int i = 0; i < buildableSquares.size(); i++) {
        ...
        va->AddVertexQ0(buildableSquares[i] +
                        float3(SQUARE_SIZE, 0, SQUARE_SIZE));
    ...
    if (h < 0.0f) {
        ...
        va->AddVertexQC(float3(x1, h, z1), s);
        va->AddVertexQC(float3(x1, 0.f, z1), e);
        va->AddVertexQC(float3(x1, h, z2), s);
        va->AddVertexQC(float3(x1, 0.f, z2), e);
        va->AddVertexQC(float3(x2, h, z2), s);
        va->AddVertexQC(float3(x2, 0.f, z2), e);
        va->AddVertexQC(float3(x2, h, z1), s);
        va->AddVertexQC(float3(x2, 0.f, z1), e);
...
GetUnitIcon(const CUnit* unit)
{
    ...
    const bool unitVisible = ((losStatus & (LOS_INLOS | LOS_INRADAR)) &&
                              ((losStatus & prevMask) == prevMask));
    const bool customIcon =
      (minimap->UseUnitIcons() && (unitVisible || gu->spectatingFullView));
...
CUnitDrawer::DrawUnitMiniMapIcons() const
{
    ...
    for (auto iconIt = unitsByIcon.cbegin(); iconIt != unitsByIcon.cend();
         ++iconIt) {
...
CUnitDrawer::RenderUnitDestroyed(const CUnit* unit)
{
    ...
    // TODO - make ghosted buildings per allyTeam - so they are correctly dealt
    // with when spectating
    for (int allyTeam = 0; allyTeam < deadGhostBuildings.size(); ++allyTeam) {
        if (unitDef->IsBuildingUnit() && gameSetup->ghostedBuildings &&
            ...
            S3DModel* gbModel =
              (decoyDef == nullptr) ? u->model : decoyDef->LoadModel();
...
CUnitDrawer::UnitLeftLos(const CUnit* unit, int allyTeam)
{
    ...
    if (gameSetup->ghostedBuildings && unit->unitDef->IsImmobileUnit())
        VectorInsertUnique(
          liveGhostBuildings[allyTeam][MDL_TYPE(unit)], u, true);
...
CUnitDrawer::PlayerChanged(int playerNum)
{
    ...
    for (auto iconIt = unitsByIcon.begin(); iconIt != unitsByIcon.end();
         ++iconIt) {
        (iconIt->second).clear();
    }
...
{
    unitDrawerStates[DRAWER_STATE_SEL]->UpdateCurrentShaderSky(this,
                                                               sky->GetLight());
}
...
{
    if (objPos.y < 0.0f)
        return (CGround::GetApproximateHeight(objPos.x, objPos.z, false) <=
                maxRadius);
    ...
    return (CGround::GetApproximateHeight(zeroPos.x, zeroPos.z, false) <=
            maxRadius);
}
