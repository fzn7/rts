...
{
    str +=
      boost::str(boost::format(string("#define ") + name +
                               " vec4(%1$.12f,%2$.12f,%3$.12f,%4$.12f)\n") %
                 (x) % (y) % (z) % (w));
}
...
{
    str += boost::str(boost::format(string("#define ") + name +
                                    " vec3(%1$.12f,%2$.12f,%3$.12f)\n") %
                      (v.x) % (v.y) % (v.z));
}
...
{
    str += boost::str(
      boost::format(string("#define ") + name + " vec2(%1$.12f,%2$.12f)\n") %
      x % y);
}
...
{
    str +=
      boost::str(boost::format(string("#define ") + name + " %1$.12f\n") % x);
}
            ...
            int* sizeY = NULL)
{
    ...
    if (!bm.Load(filename))
        throw content_error("[" LOG_SECTION_BUMP_WATER
...
DrawRadialDisc(CVertexArray* va)
{
    ...
    for (int n = 0; n < 4; ++n) {
        ...
        for (float alpha = 0.0f; (alpha - fastmath::PI2) < alphainc;
             alpha += alphainc) {
  ...
  , wasVisibleLastFrame(false)
{
    ...
    // LOAD USER CONFIGS
    reflTexSize =
      next_power_of_2(configHandler->GetInt("BumpWaterTexSizeReflection"));
    ...
    shoreWaves = (configHandler->GetBool("BumpWaterShoreWaves")) &&
                 mapInfo->water.shoreWaves;
    endlessOcean =
      (configHandler->GetBool("BumpWaterEndlessOcean")) &&
      mapInfo->water.hasWaterPlane &&
      ((readMap->HasVisibleWater()) || (mapInfo->water.forceRendering));
    dynWaves = (configHandler->GetBool("BumpWaterDynamicWaves")) &&
               (mapInfo->water.numTiles > 1);
    useUniforms = (configHandler->GetBool("BumpWaterUseUniforms"));

    // CHECK HARDWARE
    if (!globalRendering->haveGLSL) {
        throw content_error(
          "[" LOG_SECTION_BUMP_WATER
          "] your hardware/driver setup does not support GLSL");
    }
    ...
    if (shoreWaves) {
        ...
        glGetTexLevelParameteriv(
          GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &maxw);
        glGetTexLevelParameteriv(
          GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_HEIGHT, &maxh);
    ...
    if (shoreWaves) {
        ...
        // GL_LINEAR_MIPMAP_LINEAR);  glTexImage2D(GL_TEXTURE_2D, 0,
        // GL_RGBA16F_ARB, mapDims.mapx, mapDims.mapy, 0, GL_RGBA, GL_FLOAT,
        // NULL);
        ...
        {
            blurShader = shaderHandler->CreateProgramObject(
              "[BumpWater]", "CoastBlurShader", false);
            blurShader->AttachShaderObject(shaderHandler->CreateShaderObject(
              "GLSL/bumpWaterCoastBlurFS.glsl", "", GL_FRAGMENT_SHADER));
            blurShader->Link();
            ...
            if (!blurShader->IsValid()) {
                ...
                throw content_error(
                  string("[" LOG_SECTION_BUMP_WATER
                         "] shorewaves-shader compilation error!"));
            }
            ...
            if (!blurShader->IsValid()) {
                ...
                throw content_error(
                  string("[" LOG_SECTION_BUMP_WATER
                         "] shorewaves-shader validation error!"));
            }
        }

        coastFBO.reloadOnAltTab = true;
        coastFBO.Bind();
        coastFBO.AttachTexture(
          coastTexture, GL_TEXTURE_2D, GL_COLOR_ATTACHMENT0_EXT);
        ...
        if (coastFBO.CheckStatus("BUMPWATER(Coastmap)")) {
            ...
            UnsyncedHeightMapUpdate(
              SRectangle(0, 0, mapDims.mapx, mapDims.mapy));
    ...
    if (dynWaves) {
        //! SETUP DYNAMIC WAVES
        tileOffsets =
          new unsigned char[mapInfo->water.numTiles * mapInfo->water.numTiles];
        ...
        if (anisotropy > 0.0f) {
            glTexParameterf(
              GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, anisotropy);
        }
        glTexImage2D(GL_TEXTURE_2D,
                     0,
                     GL_RGBA8,
    ...
    if (GLEW_EXT_framebuffer_object) {
        ...
        switch (depthBits) {
            case 16:
                depthRBOFormat = GL_DEPTH_COMPONENT16;
                break;
            case 24:
                depthRBOFormat = GL_DEPTH_COMPONENT24;
                break;
            case 32:
                depthRBOFormat = GL_DEPTH_COMPONENT32;
                break;
        }

        if (reflection > 0) {
            reflectFBO.Bind();
            reflectFBO.CreateRenderBuffer(GL_DEPTH_ATTACHMENT_EXT,
                                          depthRBOFormat,
                                          reflTexSize,
                                          reflTexSize);
        ...
        if (refraction > 0) {
            refractFBO.Bind();
            refractFBO.CreateRenderBuffer(GL_DEPTH_ATTACHMENT_EXT,
                                          depthRBOFormat,
                                          screenTextureX,
                                          screenTextureY);
    ...
    GLSLDefineConstf3(definitions,
                      "MapMid",
                      float3(mapDims.mapx * SQUARE_SIZE * 0.5f,
                             0.0f,
                             mapDims.mapy * SQUARE_SIZE * 0.5f));
    GLSLDefineConstf2(definitions,
                      "ScreenInverse",
                      1.0f / globalRendering->viewSizeX,
                      1.0f / globalRendering->viewSizeY);
    GLSLDefineConstf2(definitions,
                      "ScreenTextureSizeInverse",
                      1.0f / screenTextureX,
                      1.0f / screenTextureY);
    GLSLDefineConstf2(definitions,
                      "ViewPos",
    ...
    if (useUniforms) {
        ...
        GLSLDefineConstf4(definitions,
                          "SurfaceColor",
                          mapInfo->water.surfaceColor * 0.4,
                          mapInfo->water.surfaceAlpha);
        GLSLDefineConstf4(definitions,
                          "PlaneColor",
                          mapInfo->water.planeColor * 0.4,
                          mapInfo->water.surfaceAlpha);
        GLSLDefineConstf3(
          definitions, "DiffuseColor", mapInfo->water.diffuseColor);
        GLSLDefineConstf3(
          definitions, "SpecularColor", mapInfo->water.specularColor);
        GLSLDefineConstf1(
          definitions, "SpecularPower", mapInfo->water.specularPower);
        GLSLDefineConstf1(
          definitions, "SpecularFactor", mapInfo->water.specularFactor);
        GLSLDefineConstf1(
          definitions, "AmbientFactor", mapInfo->water.ambientFactor);
        GLSLDefineConstf1(
          definitions, "DiffuseFactor", mapInfo->water.diffuseFactor * 15.0f);
        GLSLDefineConstf3(
          definitions,
          "SunDir",
          sky->GetLight()->GetLightDir()); // FIXME: not a constant
        ...
        GLSLDefineConstf1(
          definitions, "FresnelPower", mapInfo->water.fresnelPower);
        GLSLDefineConstf1(
          definitions, "ReflDistortion", mapInfo->water.reflDistortion);
        GLSLDefineConstf2(definitions,
                          "BlurBase",
                          0.0f,
                          mapInfo->water.blurBase / globalRendering->viewSizeY);
        GLSLDefineConstf1(
          definitions, "BlurExponent", mapInfo->water.blurExponent);
        GLSLDefineConstf1(
          definitions, "PerlinStartFreq", mapInfo->water.perlinStartFreq);
        GLSLDefineConstf1(
          definitions, "PerlinLacunarity", mapInfo->water.perlinLacunarity);
        GLSLDefineConstf1(
          definitions, "PerlinAmp", mapInfo->water.perlinAmplitude);
        GLSLDefineConstf1(definitions, "WindSpeed", mapInfo->water.windSpeed);
        GLSLDefineConstf1(definitions,
                          "shadowDensity",
                          sky->GetLight()->GetGroundShadowDensity());
    }

    {
        ...
        const float scaleX =
          (mapX > mapZ) ? (mapDims.mapy / 64) / 16.0f * (float)mapX / mapZ
                        : (mapDims.mapx / 64) / 16.0f;
        const float scaleZ =
          (mapX > mapZ) ? (mapDims.mapy / 64) / 16.0f
                        : (mapDims.mapx / 64) / 16.0f * (float)mapZ / mapX;
        GLSLDefineConst4f(definitions,
                          "TexGenPlane",
                          1.0f / mapX,
                          1.0f / mapZ,
                          scaleX / mapX,
    ...
    // LOAD SHADERS
    {
        waterShader = shaderHandler->CreateProgramObject(
          "[BumpWater]", "WaterShader", false);
        waterShader->AttachShaderObject(shaderHandler->CreateShaderObject(
          "GLSL/bumpWaterVS.glsl", definitions, GL_VERTEX_SHADER));
        waterShader->AttachShaderObject(shaderHandler->CreateShaderObject(
          "GLSL/bumpWaterFS.glsl", definitions, GL_FRAGMENT_SHADER));
        waterShader->Link();
        ...
        if (!waterShader->IsValid()) {
            ...
            LOG_L(L_ERROR, fmt, log);
            throw content_error(string("[" LOG_SECTION_BUMP_WATER
                                       "] water-shader compilation error!"));
        }
    ...
    if (endlessOcean) {
        ...
        for (int z = 0; z < 9; z++) {
            for (int x = 0; x < 10; x++) {
                va->AddVertex0(
                  float3(x * (mapX / 9.0f), 0.0f, (z + 0) * (mapZ / 9.0f)));
                va->AddVertex0(
                  float3(x * (mapX / 9.0f), 0.0f, (z + 1) * (mapZ / 9.0f)));
            }
            va->EndStrip();
        }
        va->DrawArray0(GL_TRIANGLE_STRIP);
    ...
    /*
            ...
            windStrength = (smoothstep(0.0f, 12.0f, wind.GetCurrentStrength()) *
       0.5f + 4.0f); windVec = windndir * windStrength;
    */
    ...
    if (GLEW_ARB_occlusion_query2 &&
        configHandler->GetBool("BumpWaterOcclusionQuery")) {
        GLint bitsSupported;
        glGetQueryiv(
          GL_ANY_SAMPLES_PASSED, GL_QUERY_COUNTER_BITS, &bitsSupported);
        if (bitsSupported > 0) {
...
CBumpWater::Update()
{
    ...
    /*
            ...
            windStrength += (smoothstep(0.0f, 12.0f, wind.GetCurrentStrength())
       * 0.5f + 4.0f) * 0.0001f; windVec   = windndir * windStrength;
    */
...
CBumpWater::UpdateWater(CGame* game)
{
    ...
    if (occlusionQuery && !wasVisibleLastFrame) {
        ...
        glGetQueryObjectuiv(
          occlusionQuery, GL_QUERY_RESULT_AVAILABLE, &occlusionQueryResult);
        if (occlusionQueryResult) {
            glGetQueryObjectuiv(
              occlusionQuery, GL_QUERY_RESULT, &occlusionQueryResult);
...
CBumpWater::UploadCoastline(const bool forceFull)
{
    ...
    const float* heightMap = (!gs->PreSimFrame())
    ...
    for (size_t i = 0; i < coastmapAtlasRects.size(); i++) {
        ...
        unsigned int a = 0;
        unsigned char* texpixels = (unsigned char*)atlas.AddTex(
          IntToString(i), caRect.xsize, caRect.ysize);

        for (int y = 0; y < caRect.ysize; ++y) {
            const int yindex =
              (y + caRect.iy1) * (mapDims.mapx + 1) + caRect.ix1;
            ...
            for (int x = 0; x < caRect.xsize; ++x) {
                ...
                texpixels[index2 + 2] = (height < 0.0f)
                                          ? CReadMap::EncodeHeight(height)
                                          : 255; //! waterdepth
...
CBumpWater::UpdateCoastmap()
{
    ...
    for (size_t i = 0; i < coastmapAtlasRects.size(); i++) {
        ...
        glTexCoord4f(r.tx1, r.ty1, 0.0f, 0.0f);
        glVertex2f(r.x1, r.y1);
        glTexCoord4f(r.tx1, r.ty2, 0.0f, 1.0f);
        glVertex2f(r.x1, r.y2);
        glTexCoord4f(r.tx2, r.ty2, 1.0f, 1.0f);
        glVertex2f(r.x2, r.y2);
        glTexCoord4f(r.tx2, r.ty1, 1.0f, 0.0f);
    ...
    if (atlasX > 0 && atlasY > 0) {
        ...
        for (int i = 0; i < 5; ++i) {
            coastFBO.AttachTexture(
              coastUpdateTexture, GL_TEXTURE_2D, GL_COLOR_ATTACHMENT0_EXT);
            ...
            for (size_t j = 0; j < coastmapAtlasRects.size(); j++) {
                ...
                if (!r.isCoastline)
                    continue;
                glTexCoord4f(r.x1, r.y1, 0.0f, 0.0f);
                glVertex2f(r.tx1, r.ty1);
                glTexCoord4f(r.x1, r.y2, 0.0f, 1.0f);
                glVertex2f(r.tx1, r.ty2);
                glTexCoord4f(r.x2, r.y2, 1.0f, 1.0f);
                glVertex2f(r.tx2, r.ty2);
                glTexCoord4f(r.x2, r.y1, 1.0f, 0.0f);
                glVertex2f(r.tx2, r.ty1);
            }
            glEnd();
...
CBumpWater::UpdateDynWaves(const bool initialize)
{
    ...
    glBlendColor(
      1.0f, 1.0f, 1.0f, (initialize) ? 1.0f : (modFrameNum + 1) / 600.0f);
    ...
    for (unsigned char y = 0; y < tiles; ++y) {
        for (unsigned char x = 0; x < tiles; ++x) {
            ...
            tx = offset % tiles;
            ty = (offset - tx) / tiles;
            glTexCoord2f(tx * tilesize, ty * tilesize);
            glVertex2f(x * tilesize, y * tilesize);
            glTexCoord2f(tx * tilesize, (ty + 1) * tilesize);
            glVertex2f(x * tilesize, (y + 1) * tilesize);
            glTexCoord2f((tx + 1) * tilesize, (ty + 1) * tilesize);
            glVertex2f((x + 1) * tilesize, (y + 1) * tilesize);
            glTexCoord2f((tx + 1) * tilesize, ty * tilesize);
...
{
    glUniform4f(uniforms[0],
                mapInfo->water.surfaceColor.x * 0.4,
                mapInfo->water.surfaceColor.y * 0.4,
                mapInfo->water.surfaceColor.z * 0.4,
                mapInfo->water.surfaceAlpha);
    glUniform4f(uniforms[1],
                mapInfo->water.planeColor.x * 0.4,
                mapInfo->water.planeColor.y * 0.4,
                mapInfo->water.planeColor.z * 0.4,
                mapInfo->water.surfaceAlpha);
    glUniform3f(uniforms[2],
                mapInfo->water.diffuseColor.x,
                mapInfo->water.diffuseColor.y,
                mapInfo->water.diffuseColor.z);
    glUniform3f(uniforms[3],
                mapInfo->water.specularColor.x,
                mapInfo->water.specularColor.y,
                mapInfo->water.specularColor.z);
    ...
    glUniform2f(
      uniforms[13], 0.0f, mapInfo->water.blurBase / globalRendering->viewSizeY);
...
{
    if (!occlusionQueryResult ||
        (!mapInfo->water.forceRendering && !readMap->HasVisibleWater()))
        return;
    ...
    waterShader->SetUniform1f(
      1, (gs->frameNum + globalRendering->timeOffset) / 15000.0f);

    if (shadowHandler->ShadowsLoaded()) {
        waterShader->SetUniformMatrix4fv(
          13, false, shadowHandler->GetShadowMatrixRaw());
...
CBumpWater::DrawRefraction(CGame* game)
{
    ...
    glDisable(GL_FOG); // fog has overground settings, if at all we should add
                       // special underwater settings
