...
IWater::ApplyPushedChanges(CGame* game)
{
    for (auto i = waterModes.cbegin(); i != waterModes.cend(); ++i) {
        ...
        LOG("Set water rendering mode to %i (%s)",
            water->GetID(),
            water->GetName());
    }
...
IWater::GetWater(IWater* curRenderer, int nxtRendererMode)
{
    ...
    if (nxtRendererMode < 0) {
        nxtRendererMode =
          (curRenderer == nullptr)
            ? Clamp(configHandler->GetInt("Water"), 0, NUM_WATER_RENDERERS - 1)
            : curRenderer->GetID() + 1;
    }
        ...
        switch (nxtRendererMode) {
            case WATER_RENDERER_DYNAMIC: {
                try {
                    ...
                    LOG_L(L_ERROR,
                          "Loading Dynamic Water failed, error: %s",
                          ex.what());
                }
            } break;
            ...
            case WATER_RENDERER_BUMPMAPPED: {
                try {
                    ...
                    SafeDelete(nxtRenderer);
                    LOG_L(L_ERROR,
                          "Loading Bumpmapped Water failed, error: %s",
                          ex.what());
                }
            } break;
            ...
            case WATER_RENDERER_REFL_REFR: {
                try {
                    ...
                    SafeDelete(nxtRenderer);
                    LOG_L(L_ERROR,
                          "Loading Refractive Water failed, error: %s",
                          ex.what());
                }
            } break;
            ...
            case WATER_RENDERER_REFLECTIVE: {
                try {
                    ...
                    SafeDelete(nxtRenderer);
                    LOG_L(L_ERROR,
                          "Loading Reflective Water failed, error: %s",
                          ex.what());
                }
            } break;
        }
