...
LegacyTrackHandler::~LegacyTrackHandler()
{
    ...
    for (auto ti = tracksToBeDeleted.cbegin(); ti != tracksToBeDeleted.cend();
         ++ti)
        delete *ti;
...
void
LegacyTrackHandler::LoadDecalShaders()
{
#define sh shaderHandler
    ...
    decalShaders[DECAL_SHADER_ARB] =
      sh->CreateProgramObject("[LegacyTrackHandler]", "DecalShaderARB", true);
    decalShaders[DECAL_SHADER_GLSL] =
      sh->CreateProgramObject("[LegacyTrackHandler]", "DecalShaderGLSL", false);
    ...
    if (globalRendering->haveGLSL) {
        ...
        decalShaders[DECAL_SHADER_GLSL]->AttachShaderObject(
          sh->CreateShaderObject(
            "GLSL/GroundDecalsFragProg.glsl", extraDef, GL_FRAGMENT_SHADER));
...
LegacyTrackHandler::AddTracks()
{
    ...
    for (const UnitTrackStruct* uts : tracksToBeAdded) {
        ...
        // if the unit is moving in a straight line only place marks at half the
        // rate by replacing old ones
        bool replace = false;
        ...
        if (unit->myTrack->parts.size() > 1) {
            ...
            replace =
              (((tp.pos1 + (*pi).pos1) * 0.5f).SqDistance((*pi2).pos1) < 1.0f);
        }
...
LegacyTrackHandler::DrawTracks()
{
    ...
    for (TrackType& tt : trackTypes) {
        ...
        for (UnitTrackStruct* track : tt.tracks) {
            ...
            if (gs->frameNum >
                ((track->parts.front()).creationTime + track->lifeTime)) {
                tracksToBeCleaned.push_back(TrackToClean(track, &(tt.tracks)));
            ...
            if (!camera->InView((frontPart.pos1 + backPart.pos1) * 0.5f,
                                frontPart.pos1.distance(backPart.pos1) +
                                  500.0f))
                continue;

            // walk across the track parts from front (oldest) to back (newest)
            // and draw a quad between "connected" parts (ie. parts differing 8
            // sim-frames in age)
            std::deque<TrackPart>::const_iterator curPart =
              (track->parts.begin());
            std::deque<TrackPart>::const_iterator nxtPart =
              ++(track->parts.begin());

            curPartColor[3] =
              std::max(0.0f,
                       255.0f - (gs->frameNum - (*curPart).creationTime) *
                                  track->alphaFalloff);
            ...
            for (; nxtPart != track->parts.end(); ++nxtPart) {
                nxtPartColor[3] =
                  std::max(0.0f,
                           255.0f - (gs->frameNum - (*nxtPart).creationTime) *
                                      track->alphaFalloff);

                if ((*nxtPart).connected) {
                    va->AddVertexQTC(
                      (*curPart).pos1, (*curPart).texPos, 0, curPartColor);
                    va->AddVertexQTC(
                      (*curPart).pos2, (*curPart).texPos, 1, curPartColor);
                    va->AddVertexQTC(
                      (*nxtPart).pos2, (*nxtPart).texPos, 1, nxtPartColor);
                    va->AddVertexQTC(
                      (*nxtPart).pos1, (*nxtPart).texPos, 0, nxtPartColor);
                }
...
void
LegacyTrackHandler::CleanTracks()
{
...
bool
LegacyTrackHandler::GetDrawTracks() const
{
    // FIXME move track updating to ::Update()
    if ((!tracksToBeAdded.empty()) || (!tracksToBeCleaned.empty()) ||
        (!tracksToBeDeleted.empty()))
        return true;
...
void
LegacyTrackHandler::BindTextures()
{
    {
...
void
LegacyTrackHandler::KillTextures()
{
    {
...
void
LegacyTrackHandler::BindShader(const float3& ambientColor)
{
...
void
LegacyTrackHandler::AddTrack(CUnit* unit, const float3& newPos)
{
    if (!unit->leaveTracks)
        return;
    ...
    if (decalDef.trackDecalType < 0) {
        const_cast<SolidObjectDecalDef&>(decalDef).trackDecalType =
          GetTrackType(decalDef.trackDecalTypeName);
        if (decalDef.trackDecalType < -1)
            return;
    }

    if (unit->myTrack != NULL &&
        unit->myTrack->lastUpdate >= (gs->frameNum - 7))
        return;

    if (!gu->spectatingFullView &&
        (unit->losStatus[gu->myAllyTeam] & LOS_INLOS) == 0)
        return;
    ...
    const int tmi =
      Clamp(tmz * mapDims.hmapx + tmx, 0, mapDims.hmapx * mapDims.hmapy - 1);
    ...
    const float trackLifeTime =
      GAME_SPEED * decalLevel * decalDef.trackDecalStrength;
    ...
    trackPart.pos1.y =
      CGround::GetHeightReal(trackPart.pos1.x, trackPart.pos1.z, false);
    trackPart.pos2.y =
      CGround::GetHeightReal(trackPart.pos2.x, trackPart.pos2.z, false);
    ...
    if ((*unitTrack) == nullptr) {
        ...
        const float texShift =
          (partDist / decalDef.trackDecalWidth) * decalDef.trackDecalStretch;
...
int
LegacyTrackHandler::GetTrackType(const std::string& name)
{
...
unsigned int
LegacyTrackHandler::LoadTexture(const std::string& name)
{
    ...
    if (!bm.Load(fullName)) {
        LOG_L(L_WARNING,
              "Could not load track decal from file %s",
              fullName.c_str());
        return 0;
    }
...
void
LegacyTrackHandler::RemoveTrack(CUnit* unit)
{
...
void
LegacyTrackHandler::UnitMoved(const CUnit* unit)
{
    AddTrack(const_cast<CUnit*>(unit), unit->pos);
}

void
LegacyTrackHandler::RenderUnitDestroyed(const CUnit* unit)
{
