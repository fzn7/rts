  ...
  , updateNeeded(true)
{
    ...
    groundShadowDensity = light.groundShadowDensity;
    unitShadowDensity = light.unitShadowDensity;
    orbitMinSunHeight = configHandler->GetFloat(
      "DynamicSunMinElevation"); // FIXME mapinfo option???
...
DynamicSkyLight::Update()
{
    if (!luaControl) {
        ...
        const float shadowDensity =
          std::min(1.0f, lightIntensity * shadowDensityFactor);
        const CMapInfo::light_t& light = mapInfo->light;
...
DynamicSkyLight::CalculateSunPos(const float startAngle) const
{
    ...
    const float sunAng =
      startAngle - initialSunAngle - angularVelocity * gameSeconds;
    const float4 sunPos =
      sunRotation.Mul(float3(sunOrbitRad * std::cos(sunAng),
                             sunOrbitHeight,
                             sunOrbitRad * std::sin(sunAng)));
                                ...
                                float orbitTime)
{
    ...
    if (newLightDir.w == FLT_MAX) {
        ...
        const float sunAzimuth =
          (sunLen <= 0.001f) ? PI / 2.0f : std::atan(newLightDir.y / sunLen);
        const float sunHeight = std::tan(sunAzimuth - 0.001f);

        float3 v1(
          std::cos(initialSunAngle), sunHeight, std::sin(initialSunAngle));
        v1.ANormalize();
        ...
        if (v1.y <= orbitMinSunHeight) {
            ...
            float3 v2(std::cos(initialSunAngle + PI),
                      orbitMinSunHeight,
                      std::sin(initialSunAngle + PI));
            v2.ANormalize();
            float3 v3 = v2 - v1;
        ...
        // new: newLightDir is center position of orbit, and newLightDir.w is
        // orbit height
