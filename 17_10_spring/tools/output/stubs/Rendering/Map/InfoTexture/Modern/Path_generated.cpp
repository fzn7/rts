...
GetBuildColor(const BuildSquareStatus& status)
{
...
CPathTexture::GetCurrentBuildCmdUnitDef()
{
    ...
    if (guihandler->commands[guihandler->inCommand].type !=
        CMDTYPE_ICON_BUILDING)
        return nullptr;

    return unitDefHandler->GetUnitDefByID(
...
CPathTexture::Update()
{
    ...
    const int updateLines =
      std::max((64 * 64) / texSize.x, ThreadPool::GetNumThreads());
    ...
    SColor* infoTexMem = reinterpret_cast<SColor*>(infoTexPBO.MapBuffer(
      offset * sizeof(SColor),
      (updateProcess - start) * texSize.x * sizeof(SColor)));

    // FIXME make global func
    const bool losFullView = ((gu->spectating && gu->spectatingFullView) ||
                              losHandler->globalLOS[gu->myAllyTeam]);
    ...
    if (ud != nullptr) {
        ...
        for_mt(start, updateProcess, [&](const int y) {
            for (int x = 0; x < texSize.x; ++x) {
                ...
                if (losFullView ||
                    losHandler->InLos(SquareToFloat3(sq), gu->myAllyTeam)) {
                    if (CMoveMath::IsBlocked(*md, sq.x, sq.y, NULL) &
                        CMoveMath::BLOCK_STRUCTURE) {
                        scale -= 0.25f;
                    }
                    if (CMoveMath::IsBlocked(*md, sq.x + 1, sq.y, NULL) &
                        CMoveMath::BLOCK_STRUCTURE) {
                        scale -= 0.25f;
                    }
                    if (CMoveMath::IsBlocked(*md, sq.x, sq.y + 1, NULL) &
                        CMoveMath::BLOCK_STRUCTURE) {
                        scale -= 0.25f;
                    }
                    if (CMoveMath::IsBlocked(*md, sq.x + 1, sq.y + 1, NULL) &
                        CMoveMath::BLOCK_STRUCTURE) {
                        scale -= 0.25f;
                    }
                }
