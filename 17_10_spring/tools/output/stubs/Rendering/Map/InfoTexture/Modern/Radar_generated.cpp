  ...
  , uploadTexJammer(0)
{
    ...
    infoTexPBO.Bind();
    infoTexPBO.New(texSize.x * texSize.y * texChannels * sizeof(unsigned short),
                   GL_STREAM_DRAW);
    infoTexPBO.Unbind();
    ...
    shader = shaderHandler->CreateProgramObject(
      "[CRadarTexture]", "CRadarTexture", false);
    shader->AttachShaderObject(
      shaderHandler->CreateShaderObject(vertexCode, "", GL_VERTEX_SHADER));
    shader->AttachShaderObject(
      shaderHandler->CreateShaderObject(fragmentCode, "", GL_FRAGMENT_SHADER));
    shader->Link();
    if (!shader->IsValid()) {
        const char* fmt = "%s-shader compilation error: %s";
        LOG_L(
          L_ERROR, fmt, shader->GetName().c_str(), shader->GetLog().c_str());
    } else {
        shader->Enable();
    ...
    if (!losHandler->globalLOS[gu->myAllyTeam]) {
        ...
        for (int y = 0; y < texSize.y; ++y) {
            for (int x = 0; x < texSize.x; ++x) {
                ...
                infoTexMem[idx * 2 + 1] =
                  (myJammer[idx] != 0 && myLos[idx] != 0) ? 255 : 0;
            }
        }
    } else {
        for (int y = 0; y < texSize.y; ++y) {
...
{
    if (!fbo.IsValid() || !shader->IsValid() || uploadTexRadar == 0 ||
        uploadTexJammer == 0)
        return UpdateCPU();
    ...
    // Trick: Upload the ushort as 2 ubytes, and then check both for `!=0` in
    // the shader.
    // Faster than doing it on the CPU! And uploading it as shorts would be
    // slow, cause the GPU has no native support for them and so the
    // transformation would happen on the CPU, too.
    ...
    glBindTexture(GL_TEXTURE_2D,
                  infoTextureHandler->GetInfoTexture("los")->GetTexture());
    glBegin(GL_QUADS);
    glVertex2f(-1.f, -1.f);
    glVertex2f(-1.f, +1.f);
    glVertex2f(+1.f, +1.f);
