  ...
  , uploadTex(0)
{
    ...
    shader =
      shaderHandler->CreateProgramObject("[CLosTexture]", "CLosTexture", false);
...
CLosTexture::Update()
{
    ...
    const unsigned short* myLos =
      &losHandler->los.losMaps[gu->myAllyTeam].front();
    memcpy(
      infoTexMem, myLos, texSize.x * texSize.y * texChannels * sizeof(short));
    infoTexPBO.UnmapBuffer();
