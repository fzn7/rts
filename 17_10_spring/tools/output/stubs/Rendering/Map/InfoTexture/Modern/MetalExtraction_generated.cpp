  ...
  , updateN(0)
{
    ...
    shader = shaderHandler->CreateProgramObject(
      "[CMetalExtractionTexture]", "CMetalExtractionTexture", false);
...
{
    // los-checking is done in FBO: when FBO isn't working don't expose hidden
    // data!
    if (!fbo.IsValid() || !shader->IsValid())
        return;
    ...
    //&losHandler->airLosMaps[gu->myAllyTeam].front();
    assert(metalMap->GetSizeX() == texSize.x &&
           metalMap->GetSizeZ() == texSize.y);
