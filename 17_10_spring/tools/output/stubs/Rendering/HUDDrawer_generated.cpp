...
HUDDrawer::DrawCameraDirectionArrow(const CUnit* unit)
{
    ...
    if (unit->moveType->useHeading) {
        ...
        glRotatef(GetHeadingFromVector(camera->GetDir().x, camera->GetDir().z) *
                      180.0f / 32768 +
                    180,
                  0.0f,
                  0.0f,
...
HUDDrawer::DrawWeaponStates(const CUnit* unit)
{
    ...
    font->glFormat(-0.9f,
                   ...
                   "Health: %.0f / %.0f",
                   (float)unit->health,
                   (float)unit->maxHealth);

    if (playerHandler->Player(gu->myPlayerNum)->fpsController.mouse2) {
        font->glPrint(
          -0.9f, 0.30f, 1.0f, FONT_SCALE | FONT_NORM, "Free-Fire Mode");
    }
    ...
    if (numWeaponsToPrint > 0) {
        ...
        const float lineHeight =
          std::min((yMax - yMin) / numWeaponsToPrint, maxLineHeight);
                ...
                if (wd->stockpile && !w->numStockpiled) {
                    if (w->numStockpileQued) {
                        ...
                        font->glFormat(-0.9f,
                                       ...
                                       int(100.0f * w->buildPercent + 0.5f));
                    } else {
                        glColor4f(0.8f, 0.2f, 0.2f, 0.8f);
                        font->glFormat(-0.9f,
                                       yPos,
                    ...
                    font->glFormat(
                      ...
                      "%s: Reloading (%i%%)",
                      wd->description.c_str(),
                      100 - int(100.0f * (w->reloadStatus - gs->frameNum) /
                                  int(w->reloadTime / unit->reloadSpeed) +
                                0.5f));
                } else if (!w->angleGood) {
...
HUDDrawer::DrawTargetReticle(const CUnit* unit)
{
    ...
    for (unsigned int a = 0; a < unit->weapons.size(); ++a) {
        ...
        if (w->HaveTarget()) {
            ...
            for (int b = 0; b <= 80; ++b) {
                glVertexf3(pos + (v2 * fastmath::sin(b * 2 * PI / 80) +
                                  v3 * fastmath::cos(b * 2 * PI / 80)) *
                                   radius);
            }
            glEnd();
            ...
            if (!w->onlyForward) {
                ...
                glVertexf3(pos + (v2 * fastmath::sin(PI * 0.25f) +
                                  v3 * fastmath::cos(PI * 0.25f)) *
                                   radius);
                glVertexf3(pos + (v2 * fastmath::sin(PI * 1.25f) +
                                  v3 * fastmath::cos(PI * 1.25f)) *
                                   radius);
            ...
            if ((w->GetCurrentTargetPos() - camera->GetPos())
                  .ANormalize()
                  .dot(camera->GetDir()) < 0.7f) {
                glVertexf3(w->GetCurrentTargetPos());
