...
IUnitDrawerState::EnableTexturesCommon() const
{
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP_ARB,
                  cubeMapHandler->GetEnvReflectionTextureID());
                           ...
                           bool alphaPass)
{
    ...
    const float4 color = {
        1.0f, 1.0f, 1.0, mix(1.0f, ud->alphaValues.x, (1.0f * alphaPass))
    };
...
UnitDrawerStateFFP::SetTeamColor(int team, const float2 alpha) const
{
    ...
    glTexEnvfv(GL_TEXTURE_ENV,
               ...
               std::move(GetTeamColor(team, alpha.x)));
...
UnitDrawerStateARB::Init(const CUnitDrawer* ud)
{
    ...
    // if GLEW_NV_vertex_program2 is supported, transparent objects are clipped
    // against GL_CLIP_PLANE3
    const char* vertProgNamesARB[2] = { "ARB/units3o.vp", "ARB/units3o2.vp" };
    const char* fragProgNamesARB[2] = { "ARB/units3o.fp",
                                        "ARB/units3o_shadow.fp" };

#define sh shaderHandler
    modelShaders[MODEL_SHADER_NOSHADOW_STANDARD] =
      sh->CreateProgramObject("[UnitDrawer]", "S3OShaderDefARB", true);
    modelShaders[MODEL_SHADER_NOSHADOW_STANDARD]->AttachShaderObject(
      sh->CreateShaderObject(
        vertProgNamesARB[GLEW_NV_vertex_program2], "", GL_VERTEX_PROGRAM_ARB));
    modelShaders[MODEL_SHADER_NOSHADOW_STANDARD]->AttachShaderObject(
      sh->CreateShaderObject(fragProgNamesARB[0], "", GL_FRAGMENT_PROGRAM_ARB));
    modelShaders[MODEL_SHADER_NOSHADOW_STANDARD]->Link();

    modelShaders[MODEL_SHADER_SHADOWED_STANDARD] =
      sh->CreateProgramObject("[UnitDrawer]", "S3OShaderAdvARB", true);
    modelShaders[MODEL_SHADER_SHADOWED_STANDARD]->AttachShaderObject(
      sh->CreateShaderObject(
        vertProgNamesARB[GLEW_NV_vertex_program2], "", GL_VERTEX_PROGRAM_ARB));
    modelShaders[MODEL_SHADER_SHADOWED_STANDARD]->AttachShaderObject(
      sh->CreateShaderObject(fragProgNamesARB[1], "", GL_FRAGMENT_PROGRAM_ARB));
...
UnitDrawerStateARB::CanEnable(const CUnitDrawer* ud) const
{
    ...
    return (
      ud->UseAdvShading() /*&& (!water->DrawReflectionPass() && !water->DrawRefractionPass())*/);
}
                           ...
                           bool alphaPass)
{
    ...
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4fv(
      10, &sky->GetLight()->GetLightDir().x);
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4f(
      11,
      sunLighting->unitDiffuseColor.x,
      sunLighting->unitDiffuseColor.y,
      sunLighting->unitDiffuseColor.z,
      0.0f);
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4f(
      12,
      sunLighting->unitAmbientColor.x,
      sunLighting->unitAmbientColor.y,
      sunLighting->unitAmbientColor.z,
      1.0f); //!
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4f(
      13, camera->GetPos().x, camera->GetPos().y, camera->GetPos().z, 0.0f);
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniformTarget(
      GL_FRAGMENT_PROGRAM_ARB);
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4f(
      10, 0.0f, 0.0f, 0.0f, sky->GetLight()->GetUnitShadowDensity());
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4f(
      11,
      sunLighting->unitAmbientColor.x,
      sunLighting->unitAmbientColor.y,
...
UnitDrawerStateARB::SetTeamColor(int team, const float2 alpha) const
{
    ...
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniformTarget(
      GL_FRAGMENT_PROGRAM_ARB);
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4fv(
      14, std::move(GetTeamColor(team, alpha.x)));
}
...
#define sh shaderHandler
    ...
    const std::string extraDefs =
      ("#define BASE_DYNAMIC_MODEL_LIGHT " +
       IntToString(lightHandler->GetBaseLight()) + "\n") +
      ("#define MAX_DYNAMIC_MODEL_LIGHTS " +
       IntToString(lightHandler->GetMaxLights()) + "\n");

    for (unsigned int n = MODEL_SHADER_NOSHADOW_STANDARD;
         n <= MODEL_SHADER_SHADOWED_DEFERRED;
         n++) {
        modelShaders[n] =
          sh->CreateProgramObject("[UnitDrawer]", shaderNames[n], false);
        modelShaders[n]->AttachShaderObject(sh->CreateShaderObject(
          "GLSL/ModelVertProg.glsl", extraDefs, GL_VERTEX_SHADER));
        modelShaders[n]->AttachShaderObject(sh->CreateShaderObject(
          "GLSL/ModelFragProg.glsl", extraDefs, GL_FRAGMENT_SHADER));

        modelShaders[n]->SetFlag("USE_SHADOWS", int((n & 1) == 1));
        modelShaders[n]->SetFlag("DEFERRED_MODE",
                                 int(n >= MODEL_SHADER_NOSHADOW_DEFERRED));
        modelShaders[n]->SetFlag("GBUFFER_NORMTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_NORMTEX);
        modelShaders[n]->SetFlag("GBUFFER_DIFFTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_DIFFTEX);
        modelShaders[n]->SetFlag("GBUFFER_SPECTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_SPECTEX);
        modelShaders[n]->SetFlag("GBUFFER_EMITTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_EMITTEX);
        modelShaders[n]->SetFlag("GBUFFER_MISCTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_MISCTEX);
        modelShaders[n]->SetFlag("GBUFFER_ZVALTEX_IDX",
                                 GL::GeometryBuffer::ATTACHMENT_ZVALTEX);

        modelShaders[n]->Link();
        modelShaders[n]->SetUniformLocation(
          "diffuseTex"); // idx  0 (t1: diffuse + team-color)
        modelShaders[n]->SetUniformLocation(
          "shadingTex"); // idx  1 (t2: spec/refl + self-illum)
        ...
        modelShaders[n]->SetUniform1f(13,
                                      sky->GetLight()->GetUnitShadowDensity());
                            ...
                            bool alphaPass)
{
    ...
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniformMatrix4fv(
      7, false, camera->GetViewMatrix());
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniformMatrix4fv(
      8, false, camera->GetViewMatrixInverse());
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniformMatrix4fv(
      14, false, shadowHandler->GetShadowMatrixRaw());
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4fv(
      15, &(shadowHandler->GetShadowParams().x));

    const_cast<GL::LightHandler*>(ud->GetLightHandler())
      ->Update(modelShaders[MODEL_SHADER_ACTIVE]);
}
...
UnitDrawerStateGLSL::SetTeamColor(int team, const float2 alpha) const
{
    ...
    modelShaders[MODEL_SHADER_ACTIVE]->SetUniform4fv(
      9, std::move(GetTeamColor(team, alpha.x)));
