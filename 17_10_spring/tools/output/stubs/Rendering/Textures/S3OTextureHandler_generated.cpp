                                   ...
                                   bool invertAlpha)
{
    ...
    LoadAndCacheTexture(
      model, 1, invertAxis, false, true); // never invert alpha for tex2
    cacheMutex.unlock();
}
...
CS3OTextureHandler::LoadTexture(S3DModel* model)
{
    ...
    const unsigned int tex1ID =
      LoadAndCacheTexture(model, 0, false, false, false);
    const unsigned int tex2ID =
      LoadAndCacheTexture(model, 1, false, false, false);
                                        ...
                                        bool preloadCall)
{
    ...
    if (bitmapIt != bitmapCache.end()) {
        ...
        if (!bitmap->Load(textureName)) {
            if (!bitmap->Load("unittextures/" + textureName)) {
                LOG_L(L_WARNING,
                      "[%s] could not load texture \"%s\" from model \"%s\"",
                      __FUNCTION__,
                      textureName.c_str(),
                      model->name.c_str());

                // file not found (or headless build), set a single pixel so
                // unit is visible
    ...
    textureCache[textureName] = { texID,
