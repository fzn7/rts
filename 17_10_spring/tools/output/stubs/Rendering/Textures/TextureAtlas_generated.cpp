  ...
  , freeTexture(true)
{
    ...
    // atlasAllocator->SetMaxSize(globalRendering->maxTextureSize,
...
CTextureAtlas::AddTexFromFile(std::string name, std::string file)
{
    ...
    const int ret =
      AddTexFromMem(name, bitmap.xsize, bitmap.ysize, RGBA32, &bitmap.mem[0]);
    if (ret == 1) {
...
CTextureAtlas::Finalize()
{
    ...
    for (std::vector<MemTex*>::iterator it = memtextures.begin();
         it != memtextures.end();
         ++it) {
        delete[](char*)(*it)->data;
        delete (*it);
...
CTextureAtlas::CreateTexture()
{
    ...
    {
        // make spacing between textures black transparent to avoid ugly lines
        ...
        for (std::vector<MemTex*>::iterator it = memtextures.begin();
            ...
            const float4 texCoords =
              atlasAllocator->GetTexCoords((*it)->names[0]);
        ...
        if (debug) {
            ...
            tex.Save(name + "-" + IntToString(atlasSize.x) + "x" +
                     IntToString(atlasSize.y) + ".png");
        }
    }
    ...
    glTexParameteri(GL_TEXTURE_2D,
                    ...
                    (maxMipMaps > 0) ? GL_LINEAR_MIPMAP_NEAREST : GL_NEAREST);
    ...
    if (maxMipMaps > 0) {
        glBuildMipmaps(
          ...
          GL_RGBA,
          GL_UNSIGNED_BYTE,
          pbo.GetPtr()); // FIXME disable texcompression //FIXME 2 PBO!!!!
    } else {
