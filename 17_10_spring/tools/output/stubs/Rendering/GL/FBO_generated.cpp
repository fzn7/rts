...
{
    static GLenum _targets[4] = {
        GL_TEXTURE_2D, GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_1D, GL_TEXTURE_3D
    };
    GLint format;
    glBindTexture(_targets[i], id);
    glGetTexLevelParameteriv(
      _targets[i], 0, GL_TEXTURE_INTERNAL_FORMAT, &format);
...
FBO::DownloadAttachment(const GLenum attachment)
{
    ...
    if (target == GL_RENDERBUFFER_EXT) {
        ...
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_WIDTH_EXT, &tex->xsize);
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_HEIGHT_EXT, &tex->ysize);
        ...
        GLint _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_RED_SIZE_EXT, &_cbits);
        bits += _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_GREEN_SIZE_EXT, &_cbits);
        bits += _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_BLUE_SIZE_EXT, &_cbits);
        bits += _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_ALPHA_SIZE_EXT, &_cbits);
        bits += _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_DEPTH_SIZE_EXT, &_cbits);
        bits += _cbits;
        glGetRenderbufferParameterivEXT(
          GL_RENDERBUFFER_EXT, GL_RENDERBUFFER_STENCIL_SIZE_EXT, &_cbits);
        bits += _cbits;
    } else {
        ...
        glGetTexLevelParameteriv(
          target, 0, GL_TEXTURE_INTERNAL_FORMAT, (GLint*)&tex->format);
    ...
    switch (target) {
        case GL_TEXTURE_3D:
            tex->pixels = new unsigned char[tex->xsize * tex->ysize *
                                            tex->zsize * (bits / 8)];
            glGetTexImage(tex->target,
                          0,
                          /*FIXME*/ GL_RGBA,
        ...
        case GL_RENDERBUFFER_EXT:
            tex->pixels =
              new unsigned char[tex->xsize * tex->ysize * (bits / 8)];
...
FBO::GLContextLost()
{
    ...
    for (std::vector<FBO*>::iterator fi = fboList.begin(); fi != fboList.end();
         ++fi) {
...
FBO::GLContextReinit()
{
    ...
    for (std::map<GLuint, FBO::TexData*>::iterator ti = texBuf.begin();
         ti != texBuf.end();
         ++ti) {
        ...
        if (glIsTexture(tex->id)) {
            ...
            switch (tex->target) {
                case GL_TEXTURE_3D:
                    // glTexSubImage3D(tex->target, 0, 0,0,0, tex->xsize,
                    // tex->ysize, tex->zsize, /*FIXME?*/GL_RGBA,
                    // /*FIXME?*/GL_UNSIGNED_BYTE, tex->pixels);
                    ...
                    break;
                case GL_TEXTURE_1D:
                    // glTexSubImage1D(tex->target, 0, 0, tex->xsize,
                    // /*FIXME?*/GL_RGBA, /*FIXME?*/GL_UNSIGNED_BYTE,
                    // tex->pixels);
                    glTexImage1D(tex->target,
                                 0,
                                 tex->format,
                                 tex->xsize,
                                 /*FIXME?*/ GL_RGBA,
                    ...
                    break;
                default: //GL_TEXTURE_2D & GL_TEXTURE_RECTANGLE
                    // glTexSubImage2D(tex->target, 0, 0,0, tex->xsize,
                    // tex->ysize, /*FIXME?*/GL_RGBA, /*FIXME?*/GL_UNSIGNED_BYTE,
                    // tex->pixels);
...
FBO::~FBO()
{
    ...
    for (std::vector<GLuint>::iterator ri = myRBOs.begin(); ri != myRBOs.end();
         ++ri) {
...
FBO::CheckStatus(std::string name)
{
    ...
    switch (status) {
        ...
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
            LOG_L(L_WARNING,
                  "FBO-%s: None/Unsupported textures/buffers attached!",
                  name.c_str());
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
            LOG_L(L_WARNING,
                  "FBO-%s: Missing a required texture/buffer attachment!",
                  name.c_str());
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
            LOG_L(L_WARNING,
                  "FBO-%s: Has mismatched texture/buffer dimensions!",
                  name.c_str());
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
            LOG_L(
              L_WARNING, "FBO-%s: Incomplete buffer formats!", name.c_str());
            break;
        ...
        case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
            LOG_L(L_WARNING,
                  "FBO-%s: GL_FRAMEBUFFER_UNSUPPORTED_EXT",
                  name.c_str());
            break;
        default:
                   ...
                   const int zSlice)
{
    ...
    if (texTarget == GL_TEXTURE_1D) {
        glFramebufferTexture1DEXT(
          GL_FRAMEBUFFER_EXT, attachment, GL_TEXTURE_1D, texId, mipLevel);
        ...
        glFramebufferTexture2DEXT(
          GL_FRAMEBUFFER_EXT, attachment, texTarget, texId, mipLevel);
    }
}
...
FBO::AttachRenderBuffer(const GLuint rboId, const GLenum attachment)
{
    ...
    glFramebufferRenderbufferEXT(
      GL_FRAMEBUFFER_EXT, attachment, GL_RENDERBUFFER_EXT, rboId);
}
...
FBO::Detach(const GLenum attachment)
{
    ...
    if (target == GL_RENDERBUFFER_EXT) {
        ...
        glFramebufferRenderbufferEXT(
          GL_FRAMEBUFFER_EXT, attachment, GL_RENDERBUFFER_EXT, 0);

        for (std::vector<GLuint>::iterator ri = myRBOs.begin();
             ri != myRBOs.end();
             ++ri) {
            if (*ri == id) {
                glDeleteRenderbuffersEXT(1, &(*ri));
        ...
        glFramebufferTexture2DEXT(
          GL_FRAMEBUFFER_EXT, attachment, GL_TEXTURE_2D, 0, 0);
    }
}
                        ...
                        const GLsizei height)
{
    ...
    glFramebufferRenderbufferEXT(
      GL_FRAMEBUFFER_EXT, attachment, GL_RENDERBUFFER_EXT, rbo);
