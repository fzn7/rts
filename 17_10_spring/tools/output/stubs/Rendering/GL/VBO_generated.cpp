...
VBO::IsVBOSupported()
{
    return (GLEW_ARB_vertex_buffer_object && GLEW_ARB_map_buffer_range &&
            configHandler->GetBool("UseVBO"));
}
...
VBO::IsPBOSupported()
{
    return (GLEW_EXT_pixel_buffer_object && GLEW_ARB_map_buffer_range &&
            configHandler->GetBool("UsePBO"));
}

bool
VBO::IsSupported() const
{
  ...
  , VBOused(true)
{
    ...
    if (immutableStorage && !GLEW_ARB_buffer_storage) {
        // note: We can't fallback to traditional BufferObjects, cause then we
        // would have to map/unmap on each change.
        //      Only sysram/cpu VAs give an equivalent behaviour.
        VBOused = false;
        immutableStorage = false;
        // LOG_L(L_ERROR, "VBO/PBO: cannot create immutable storage, gpu drivers
...
void
VBO::Resize(GLsizeiptr _size, GLenum usage)
{
    assert(bound);
    assert(!mapped);

    if (_size == size &&
        usage == this->usage) // no size change -> nothing to do
        return;

    if (size == 0) // first call: none *BO there yet to copy old data from, so
                   // use ::New() (faster)
    ...
    if (VBOused) {
        ...
        if (GLEW_ARB_copy_buffer) {
            ...
            if (osize > 0)
                glCopyBufferSubData(
                  curBoundTarget, GL_COPY_WRITE_BUFFER, 0, 0, osize);
...
void
VBO::New(GLsizeiptr _size, GLenum usage, const void* data_)
{
    assert(bound);
    assert(!mapped);
    ...
    if (immutableStorage && size != 0) {
        LOG_L(L_ERROR,
              "VBO/PBO: cannot recreate already existing persistent storage "
              "buffer");
        return;
    }
    ...
    if (VBOused) {
...
#ifdef GLEW_ARB_buffer_storage
        if (immutableStorage) {
            usage = GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT |
        ...
        delete[] data;
        data = nullptr; // to prevent a dead-pointer in case of an out-of-memory
...
GLubyte*
VBO::MapBuffer(GLbitfield access)
{
    assert(!mapped);
...
GLubyte*
VBO::MapBuffer(GLintptr offset, GLsizeiptr _size, GLbitfield access)
{
    assert(!mapped);
    assert(offset + _size <= size);
    mapped = true;

    // glMapBuffer & glMapBufferRange use different flags for their access
    // argument for easier handling convert the glMapBuffer ones here
    switch (access) {
        case GL_WRITE_ONLY:
...
#ifdef GLEW_ARB_buffer_storage
            if (immutableStorage) {
                access = GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT |
                         GL_MAP_COHERENT_BIT;
            }
#endif
            break;
    ...
    if (_size == 0) {
        // nvidia incorrectly returns GL_INVALID_VALUE when trying to call
        // glMapBufferRange with size zero so catch it ourselves
        nullSizeMapped = true;
        return NULL;
    }
    ...
    if (VBOused) {
        GLubyte* ptr =
          (GLubyte*)glMapBufferRange(curBoundTarget, offset, _size, access);
        assert(ptr);
        return ptr;
    } else {
        assert(data);
...
VBO::Invalidate()
{
    ...
    // note: allocating memory doesn't actually block the memory it just makes
    // room in _virtual_ memory space
    New(size, usage, nullptr);
}

const GLvoid*
VBO::GetPtr(GLintptr offset) const
{
    assert(bound);
