...
DefaultPathDrawer::DrawInMiniMap()
{
    ...
    for (const int2& sb : pe->updatedBlocks) {
        ...
        glRectf(blockIdxX,
                blockIdxY,
                blockIdxX + pe->GetBlockSize(),
                blockIdxY + pe->GetBlockSize());
    }
    ...
    switch (extraTex) {
        case CLegacyInfoTextureHandler::drawPathTrav: {
            ...
            if (useCurrentBuildOrder &&
                guihandler->commands[guihandler->inCommand].type !=
                  CMDTYPE_ICON_BUILDING) {
                useCurrentBuildOrder = false;
            }
            ...
            if (useCurrentBuildOrder) {
                for (int ty = starty; ty < endy; ++ty) {
                    for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                        const float3 pos(tx * (SQUARE_SIZE << 1) + SQUARE_SIZE,
                                         0.0f,
                                         ty * (SQUARE_SIZE << 1) + SQUARE_SIZE);
                        const int idx =
                          ((ty * (mapDims.pwr2mapx >> 1)) + tx) * 4 - offset;
                        ...
                        if (!losHandler->InLos(pos, gu->myAllyTeam)) {
                            ...
                            const UnitDef* ud = unitDefHandler->GetUnitDefByID(
                              -guihandler->commands[guihandler->inCommand].id);
                            const BuildInfo bi(
                              ud, pos, guihandler->buildFacing);

                            CFeature* f = nullptr;

                            if (CGameHelper::TestUnitBuildSquare(
                                  bi, f, gu->myAllyTeam, false)) {
                                if (f != nullptr) {
                        ...
                        texMem[idx + CLegacyInfoTextureHandler::COLOR_R] =
                          col.r;
                        texMem[idx + CLegacyInfoTextureHandler::COLOR_G] =
                          col.g;
                        texMem[idx + CLegacyInfoTextureHandler::COLOR_B] =
                          col.b;
                        texMem[idx + CLegacyInfoTextureHandler::COLOR_A] =
                          col.a;
                    }
                }
            } else {
                ...
                if (md != nullptr) {
                    ...
                    for (int ty = starty; ty < endy; ++ty) {
                        for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                            ...
                            const int sqy = (ty << 1);
                            const int texIdx =
                              ((ty * (mapDims.pwr2mapx >> 1)) + tx) * 4 -
                              offset;
                            const bool losSqr = losHandler->InLos(
                              SquareToFloat3(sqx, sqy), gu->myAllyTeam);

                            float scale = 1.0f;

                            if (los || losSqr) {
                                if (CMoveMath::IsBlocked(
                                      *md, sqx, sqy, nullptr) &
                                    CMoveMath::BLOCK_STRUCTURE) {
                                    scale -= 0.25f;
                                }
                                if (CMoveMath::IsBlocked(
                                      *md, sqx + 1, sqy, nullptr) &
                                    CMoveMath::BLOCK_STRUCTURE) {
                                    scale -= 0.25f;
                                }
                                if (CMoveMath::IsBlocked(
                                      *md, sqx, sqy + 1, nullptr) &
                                    CMoveMath::BLOCK_STRUCTURE) {
                                    scale -= 0.25f;
                                }
                                if (CMoveMath::IsBlocked(
                                      *md, sqx + 1, sqy + 1, nullptr) &
                                    CMoveMath::BLOCK_STRUCTURE) {
                                    scale -= 0.25f;
                                }
                            }

                            // NOTE: raw speedmods are not necessarily clamped
                            // to [0, 1]
                            const float sm =
                              CMoveMath::GetPosSpeedMod(*md, sqx, sqy);
                            const SColor& smc = GetSpeedModColor(sm * scale);

                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_R] = smc.r;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_G] = smc.g;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_B] = smc.b;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_A] = smc.a;
                        }
                    }
                } else {
                    ...
                    for (int ty = starty; ty < endy; ++ty) {
                        for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                            ...
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_R] = 100;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_G] = 0;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_B] = 0;
                            texMem[texIdx +
                                   CLegacyInfoTextureHandler::COLOR_A] = 255;
                        }
                    }
                }
            }
        ...
        case CLegacyInfoTextureHandler::drawPathHeat: {
            ...
            for (int ty = starty; ty < endy; ++ty) {
                for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                    const unsigned int texIdx =
                      ((ty * (mapDims.pwr2mapx >> 1)) + tx) * 4 - offset;

                    texMem[texIdx + CLegacyInfoTextureHandler::COLOR_R] =
                      Clamp(8 * phm->GetHeatValue(tx << 1, ty << 1), 32, 255);
            ...
            if (maxFlow > 0.0f) {
                for (int ty = starty; ty < endy; ++ty) {
                    for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                        ...
                        texMem[texIdx + CLegacyInfoTextureHandler::COLOR_R] =
                          (((flow.x + 1.0f) * 0.5f) * 255);
                        texMem[texIdx + CLegacyInfoTextureHandler::COLOR_B] =
                          (((flow.z + 1.0f) * 0.5f) * 255);
                        texMem[texIdx + CLegacyInfoTextureHandler::COLOR_G] =
                          ((flow.y) * 255);
                        texMem[texIdx + CLegacyInfoTextureHandler::COLOR_A] =
                          255;
                    }
                }
            }
        ...
        case CLegacyInfoTextureHandler::drawPathCost: {
            ...
            const unsigned int medResBlockSize = pm->medResPE->BLOCK_SIZE,
                               medResBlocksX = pm->medResPE->GetNumBlocks().x;
            const unsigned int lowResBlockSize = pm->lowResPE->BLOCK_SIZE,
                               lowResBlocksX = pm->lowResPE->GetNumBlocks().x;
            ...
            for (int ty = starty; ty < endy; ++ty) {
                for (int tx = 0; tx < mapDims.hmapx; ++tx) {
                    ...
                    //    ty is in [0, mapDims.hmapy> (highResInfoTexWanted ==
                    //    false)
                    ...
                    float gCost[3] = {
                        ...
                        medResStates
                          .gCost[(hy / medResBlockSize) * medResBlocksX +
                                 (hx / medResBlockSize)],
                        lowResStates
                          .gCost[(hy / lowResBlockSize) * lowResBlocksX +
                                 (hx / lowResBlockSize)],
                    };
                    ...
                    texMem[texIdx + CLegacyInfoTextureHandler::COLOR_R] =
                      (gCost[0] / gCostMax[0]) * 255;
                    texMem[texIdx + CLegacyInfoTextureHandler::COLOR_G] =
                      (gCost[1] / gCostMax[1]) * 255;
                    texMem[texIdx + CLegacyInfoTextureHandler::COLOR_B] =
                      (gCost[2] / gCostMax[2]) * 255;
                    texMem[texIdx + CLegacyInfoTextureHandler::COLOR_A] = 255;
                }
            }
        } break;
...
DefaultPathDrawer::Draw() const
{
    ...
    const std::map<unsigned int, CPathManager::MultiPath*>& pathMap =
      pm->pathMap;
    ...
    for (pi = pathMap.begin(); pi != pathMap.end(); ++pi) {
        ...
        for (PathIt pvi = path->lowResPath.path.begin();
             pvi != path->lowResPath.path.end();
             ++pvi) {
            float3 pos = *pvi;
            pos.y += 5;
            glVertexf3(pos);
        ...
        for (PathIt pvi = path->medResPath.path.begin();
             pvi != path->medResPath.path.end();
             ++pvi) {
            float3 pos = *pvi;
            pos.y += 5;
            glVertexf3(pos);
        ...
        for (PathIt pvi = path->maxResPath.path.begin();
             pvi != path->maxResPath.path.end();
             ++pvi) {
            float3 pos = *pvi;
            pos.y += 5;
            glVertexf3(pos);
...
DefaultPathDrawer::Draw(const CPathFinder* pf) const
{
    ...
    for (unsigned int idx = 0; idx < pf->openBlockBuffer.GetSize(); idx++) {
        ...
        const unsigned int dir =
          pf->blockStates.nodeMask[square] & PATHOPT_CARDINALS;
...
DefaultPathDrawer::Draw(const CPathEstimator* pe) const
{
    ...
    const int overlayNumber =
      (gs->frameNum % (overlayPeriod * 2)) / overlayPeriod;
    ...
    if (drawLowResPE || drawMedResPE) {
        ...
        const int vertexBaseNr = md->pathType * peNumBlocks.x * peNumBlocks.y *
        ...
        for (int z = 0; z < peNumBlocks.y; z++) {
            for (int x = 0; x < peNumBlocks.x; x++) {
                ...
                float3 p1;
                p1.x = (blockStates.peNodeOffsets[md->pathType][blockNr].x) *
                       SQUARE_SIZE;
                p1.z = (blockStates.peNodeOffsets[md->pathType][blockNr].y) *
                       SQUARE_SIZE;
                ...
                for (int dir = 0; dir < PATH_DIRECTION_VERTICES; dir++) {
                    const int obx =
                      x + (CPathEstimator::GetDirectionVectorsTable())[dir].x;
                    const int obz =
                      z + (CPathEstimator::GetDirectionVectorsTable())[dir].y;
                    ...
                    const int vertexNr =
                      vertexBaseNr + blockNr * PATH_DIRECTION_VERTICES +
                      GetBlockVertexOffset(dir, peNumBlocks.x);
                    const float cost =
                      (pe->vertexCosts[vertexNr] * PATH_NODE_SPACING) /
                      pe->BLOCK_SIZE;

                    float3 p2;
                    p2.x =
                      (blockStates.peNodeOffsets[md->pathType][obBlockNr].x) *
                      SQUARE_SIZE;
                    p2.z =
                      (blockStates.peNodeOffsets[md->pathType][obBlockNr].y) *
                      SQUARE_SIZE;
                    p2.y =
                      CGround::GetHeightAboveWater(p2.x, p2.z, false) + 10.0f;

                    glColor3f(1.0f / std::sqrt(cost),
                              1.0f / cost,
                              0.75f * drawLowResPE);
                    glVertexf3(p1);
                    glVertexf3(p2);
        ...
        for (int z = 0; z < peNumBlocks.y; z++) {
            for (int x = 0; x < peNumBlocks.x; x++) {
                ...
                for (int dir = 0; dir < PATH_DIRECTION_VERTICES; dir++) {
                    ...
                    font->SetTextColor(
                      1.0f, 1.0f / cost, 0.75f * drawLowResPE, 1.0f);
                    font->glWorldPrint(
                      p2, 5.0f, FloatToString(cost, "f(%.2f)"));
                }
            }
        }
    }
...

    {
        ...
        for (unsigned int idx = 0; idx < pe->openBlockBuffer.GetSize(); idx++) {
            ...
            const int2 obp =
              pe->BlockIdxToPos(blockNr) - pe->PE_DIRECTION_VECTORS[pathDir];
            ...
            p1.y = CGround::GetHeightAboveWater(p1.x, p1.z, false) + 15.0f;
            float3 p2;
            p2.x = (blockStates.peNodeOffsets[md->pathType][obBlockNr].x) *
                   SQUARE_SIZE;
            p2.z = (blockStates.peNodeOffsets[md->pathType][obBlockNr].y) *
                   SQUARE_SIZE;
            p2.y = CGround::GetHeightAboveWater(p2.x, p2.z, false) + 15.0f;
    ...
    if ((drawLowResPE || drawMedResPE) && false) {
        ...
        for (unsigned int blockIdx = 0; blockIdx < openBlockBuffer.GetSize();
             blockIdx++) {
            ...
            SNPRINTF(blockCostsStr,
                     sizeof(blockCostsStr),
                     "f(%.2f) g(%.2f)",
                     ob->fCost,
                     ob->gCost);
