    ...
    uniformStates)
{
    ...
    glGetProgramiv(
      newProgID, GL_ACTIVE_UNIFORM_MAX_LENGTH, &maxUniformNameLength);
    ...
    for (int i = 0; i < numUniforms; ++i) {
        ...
        GLint oldLoc =
          -1; // only use when we don't got data in our own state tracker
        const GLint newLoc = glGetUniformLocation(newProgID, &name[0]);

        if (newLoc < 0)
            continue;

        // Try to find old data for the uniform either in the old shader itself
        // or in our own state tracker
        ...
        if (it != uniformStates->end()) {
            ...
            // Uniform not found in state tracker, try to read it from old
            // shader object
            oldUniformState =
              &(uniformStates->emplace(hash, name).first->second);
        }
        ...
        if (oldUniformState->IsUninit()) {
            ...
            // FIXME read data from old shader & save data in _new_
            // uniformState?
        }
...
#define HANDLE_TYPE(type, size, ftype, internalTypeName)                       \
    case type##size: {                                                         \
        ...
        if (oldLoc >= 0) {                                                     \
            ...
            memcpy(_value,                                                     \
                   oldUniformState->GetIntValues(),                            \
                   size * sizeof(internalTypeName));                           \
        }                                                                      \
...
#define HANDLE_MATTYPE(type, size, ftype, internalTypeName)                    \
    case type##size: {                                                         \
        ...
        if (oldLoc >= 0) {                                                     \
            ...
            memcpy(_value,                                                     \
                   oldUniformState->GetIntValues(),                            \
                   size* size * sizeof(internalTypeName));                     \
        }                                                                      \
        glUniformMatrix##size##ftype(newLoc, 1, false, &_value[0]);            \
            ...
            case ATOMIC: {
                assert(false);
                /*GLint binding;
                glGetActiveAtomicCounterBufferiv(oldProgID, i,
                GL_ATOMIC_COUNTER_BUFFER_BINDING, &binding); glUniform1f(newLoc,
                1, binding);*/
            } break;

            default:
                LOG_L(L_WARNING,
                      "Unknown GLSL uniform \"%s\" has unknown vartype \"%X\"",
                      name.c_str(),
                      type);
        }
    }
...
CopyShaderState_UniformBlocks(GLuint newProgID, GLuint oldProgID)
{
    ...
    glGetProgramiv(
      oldProgID, GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, &maxNameLength);
    ...
    for (int i = 0; i < numUniformBlocks; ++i) {
        ...
        glGetActiveUniformBlockName(
          oldProgID, i, maxNameLength, &nameLength, &name[0]);
        ...
        glGetActiveUniformBlockiv(
          oldProgID, oldLoc, GL_UNIFORM_BLOCK_BINDING, &value);
...
CopyShaderState_ShaderStorage(GLuint newProgID, GLuint oldProgID)
{
    ...
    GLint numUniformBlocks, maxNameLength = 0;
    glGetProgramInterfaceiv(oldProgID,
                            GL_SHADER_STORAGE_BLOCK,
                            GL_ACTIVE_RESOURCES,
                            &numUniformBlocks);
    glGetProgramInterfaceiv(
      oldProgID, GL_SHADER_STORAGE_BLOCK, GL_MAX_NAME_LENGTH, &maxNameLength);
    ...
    for (int i = 0; i < numUniformBlocks; ++i) {
        GLsizei nameLength = 0;
        glGetProgramResourceName(oldProgID,
                                 GL_SHADER_STORAGE_BLOCK,
                                 i,
                                 maxNameLength,
                                 &nameLength,
        ...
        GLuint oldLoc = glGetProgramResourceIndex(
          oldProgID, GL_SHADER_STORAGE_BLOCK, &name[0]);
        GLuint newLoc = glGetProgramResourceIndex(
          newProgID, GL_SHADER_STORAGE_BLOCK, &name[0]);
...
CopyShaderState_Attributes(GLuint newProgID, GLuint oldProgID)
{
    ...
    for (int i = 0; i < numAttributes; ++i) {
        ...
        GLenum type = 0;
        glGetActiveAttrib(
          oldProgID, i, maxNameLength, &nameLength, &size, &type, &name[0]);
        name[maxNameLength - 1] = 0;
...
CopyShaderState_TransformFeedback(GLuint newProgID, GLuint oldProgID)
{
    ...
    glGetProgramiv(
      oldProgID, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, &maxNameLength);
    ...
    glTransformFeedbackVaryings(
      newProgID, numVaryings, (const GLchar**)&varyingsPtr[0], bufferMode);
#endif
}
...
CopyShaderState_ContainsGeometryShader(GLuint oldProgID)
{
    ...
    glGetAttachedShaders(
      oldProgID, 3, &numAttachedShaders, &attachedShaderIDs[0]);

    for (GLsizei n = 0; n < numAttachedShaders; n++) {
        glGetShaderiv(
          attachedShaderIDs[n], GL_SHADER_TYPE, &attachedShaderType);
