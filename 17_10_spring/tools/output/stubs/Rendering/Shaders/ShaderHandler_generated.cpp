                                    ...
                                    bool arbProgram)
{
    ...
    if (programObjects.find(poClass) != programObjects.end()) {
        if (programObjects[poClass].find(poName) !=
            programObjects[poClass].end()) {
            LOG_L(L_WARNING,
                  "[%s] There is already a shader program named \"%s\"!",
                  __FUNCTION__,
                  poName.c_str());
            return (programObjects[poClass][poName]);
        }
    ...
    if (po == Shader::nullProgramObject) {
        LOG_L(L_ERROR,
              "[%s] Tried to create \"%s\" a %s shader program on hardware w/o "
              "support for it!",
              __FUNCTION__,
              poName.c_str(),
              arbProgram ? "ARB" : "GLSL");
                                   ...
                                   int soType)
{
    ...
    if (so == Shader::nullShaderObject) {
        LOG_L(L_ERROR,
              "[%s] Tried to create a shader (\"%s\") on hardware that does "
              "not support them!",
              __FUNCTION__,
              soName.c_str());
        return so;
