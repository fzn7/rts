                         ...
                         const float2)
{
    ...
    assert((!s->IsCustomType() && !s->IsEngineType()) ||
           (o->team == LuaObjectDrawer::GetBinObjTeam()));
}
...
{
    return ((int(s->IsCustomType()) * 1 + int(s->IsEngineType()) * 2) *
            (o->team != LuaObjectDrawer::GetBinObjTeam())); // FIXME
}
...
LuaObjectDrawer::Init()
{
    ...
    bufferClearAllowed =
      configHandler->GetBool("AllowDeferredModelBufferClear");
...
LuaObjectDrawer::SetDrawPassGlobalLODFactor(LuaObjType objType)
{
    if (shadowHandler->InShadowPass()) {
        LuaObjectMaterialData::SetGlobalLODFactor(
          objType, GetLODScaleShadow(objType) * camera->GetLPPScale());
        return;
    }

    if (water->DrawReflectionPass()) {
        LuaObjectMaterialData::SetGlobalLODFactor(
          objType, GetLODScaleReflection(objType) * camera->GetLPPScale());
        return;
    }

    if (water->DrawRefractionPass()) {
        LuaObjectMaterialData::SetGlobalLODFactor(
          objType, GetLODScaleRefraction(objType) * camera->GetLPPScale());
        return;
    }

    LuaObjectMaterialData::SetGlobalLODFactor(
      objType, GetLODScale(objType) * camera->GetLPPScale());
}

LuaMatType
LuaObjectDrawer::GetDrawPassOpaqueMat()
{
    return opaqueMats[water->DrawReflectionPass()];
}
LuaMatType
LuaObjectDrawer::GetDrawPassAlphaMat()
{
    return alphaMats[water->DrawReflectionPass()];
}
                                  ...
                                  bool deferredPass)
{
    ...
    for (auto it = bins.cbegin(); it != bins.cend(); ++it) {
        DrawMaterialBin(
          *it, prevMat, objType, matType, deferredPass, inAlphaBin);
        prevMat = *it;
    }
    ...
    for (int objTeam = minObjTeam; objTeam <= maxObjTeam; objTeam++) {
        for (const CSolidObject* obj : objectBuckets[objTeam]) {
            ...
            const LuaObjectLODMaterial* lodMat =
              matData->GetLuaLODMaterial(matType);
                               ...
                               bool noLuaCall)
{
    ...
    const unsigned int tcFunIdx =
      GetTeamColorFuncIndex(obj, &luaMat->shaders[deferredPass]);
    ...
    switch (objType) {
        case LUAOBJ_UNIT: {
            ...
            tcolFunc(
              deferredPass, unit, luaMat, float2(1.0f, 1.0f * alphaMatBin));
            CALL_FUNC_VA(
              unitDrawer, drawFunc, unit, preList, postList, true, noLuaCall);
        } break;
        case LUAOBJ_FEATURE: {
            ...
            tcolFunc(deferredPass,
                     feat,
                     luaMat,
                     float2(feat->drawAlpha, 1.0f * alphaMatBin));
            CALL_FUNC_VA(featureDrawer,
                         drawFunc,
                         feat,
                         preList,
                         postList,
                                        ...
                                        bool applyTrans)
{
    ...
    if ((lodMat = matData->GetLuaLODMaterial(
           LuaObjectDrawer::GetDrawPassOpaqueMat())) == nullptr)
        return false;
    if (!lodMat->IsActive())
        return false;
    ...
    switch (objType) {
        case LUAOBJ_UNIT: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupOpaqueUnitDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetOpaqueUnitDrawState;
            }
        } break;
        case LUAOBJ_FEATURE: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupOpaqueFeatureDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetOpaqueFeatureDrawState;
            }
        } break;
        default: {
            assert(false);
    ...
    LuaObjectMaterialData::SetGlobalLODFactor(
      objType, LuaObjectDrawer::GetLODScale(objType) * camera->GetLPPScale());
    ...
    // NOTE: doesn't make sense to support deferred mode for this? (extra arg in
    // gl.Unit, etc)
    currMat->Execute(LuaMaterial::defMat, false);

    DrawBinObject(
      obj, objType, lodMat, currMat, false, false, applyTrans, true);
    ...
    switch (objType) {
        case LUAOBJ_UNIT: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupAlphaUnitDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetAlphaUnitDrawState;
            }
        } break;
        case LUAOBJ_FEATURE: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupAlphaFeatureDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetAlphaFeatureDrawState;
            }
        } break;
        default: {
            assert(false);
    ...
    switch (objType) {
        case LUAOBJ_UNIT: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupShadowUnitDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetShadowUnitDrawState;
            }
        } break;
        case LUAOBJ_FEATURE: {
            for (int modelType = MODELTYPE_3DO; modelType < MODELTYPE_OTHER;
                 modelType++) {
                luaMatHandler.setupDrawStateFuncs[modelType] =
                  SetupShadowFeatureDrawState;
                luaMatHandler.resetDrawStateFuncs[modelType] =
                  ResetShadowFeatureDrawState;
            }
        } break;
        default: {
            assert(false);
