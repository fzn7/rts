...
class ConnectWindow : public agui::Window
{
  public:
      ...
      : agui::Window("Connect to server")
    {
        ...
        /*agui::TextElement* label = */ new agui::TextElement(
          "Address:", input); // will be deleted in input
        address = new agui::LineEdit(input);
        address->DefaultAction.connect(
          boost::bind(&ConnectWindow::Finish, this, true));
        ...
        close->Clicked.connect(
          boost::bind(&ConnectWindow::Finish, this, false));
        GeometryChange();
    }
...
class SettingsWindow : public agui::Window
{
  public:
      ...
      : agui::Window(name)
    {
        ...
        agui::VerticalLayout* wndLayout = new agui::VerticalLayout(this);
        HorizontalLayout* input = new HorizontalLayout(wndLayout);
        /*agui::TextElement* value_label = */ new agui::TextElement(
          "Value:", input); // will be deleted in input
        value = new agui::LineEdit(input);
        value->DefaultAction.connect(
          boost::bind(&SettingsWindow::Finish, this, true));
        value->SetFocus(true);
        ...
        close->Clicked.connect(
          boost::bind(&SettingsWindow::Finish, this, false));
        GeometryChange();
    }
  ...
  , curSelect(NULL)
{
    ...
    { // GUI stuff
        ...
        ;
        {
            const std::string archive = archiveScanner->ArchiveFromName(
              configHandler->GetString("MenuArchive"));
            const std::string archivePath =
              archiveScanner->GetArchivePath(archive) + archive;
            vfsHandler->AddArchive(archivePath, false);
            const std::vector<std::string> files =
              CFileHandler::FindFiles("bitmaps/ui/background/", "*");
            if (!files.empty()) {
        ...
        /*agui::TextElement* title = */ new agui::TextElement(
          "Spring", menu); // will be deleted in menu
        ...
        editsettings->Clicked.connect(
          boost::bind(&SelectMenu::ShowSettingsList, this));

        Button* direct = new Button("Direct connect", menu);
        direct->Clicked.connect(
          boost::bind(&SelectMenu::ShowConnectWindow, this, true));
...
SelectMenu::Single()
{
    if (selw->userMod == SelectionWidget::NoModSelect) {
        ...
        pregame->LoadSetupscript(
          StartScriptGen::CreateDefaultSetup(selw->userMap,
                                             selw->userMod,
                                             selw->userScript,
...
SelectMenu::ShowConnectWindow(bool show)
{
    if (show && !conWindow) {
        ...
        conWindow->Connect.connect(
          boost::bind(&SelectMenu::DirectConnect, this, _1));
        conWindow->WantClose.connect(
          boost::bind(&SelectMenu::ShowConnectWindow, this, false));
...
SelectMenu::ShowSettingsWindow(bool show, std::string name)
{
    if (show) {
        ...
        settingsWindow->OK.connect(
          boost::bind(&SelectMenu::ShowSettingsWindow, this, false, _1));
        settingsWindow->WantClose.connect(
          boost::bind(&SelectMenu::ShowSettingsWindow, this, false, ""));
...
SelectMenu::ShowSettingsList()
{
    if (!curSelect) {
        ...
        curSelect->Selected.connect(
          boost::bind(&SelectMenu::SelectSetting, this, _1));
        curSelect->WantClose.connect(
          boost::bind(&SelectMenu::CleanWindow, this));
    }
    curSelect->list->RemoveAllItems();
    const std::map<std::string, std::string>& data = configHandler->GetData();
    typedef std::map<std::string, std::string, doj::alphanum_less<std::string>>
      DataSorted;
    const DataSorted dataSorted(data.begin(), data.end());
    for (DataSorted::const_iterator iter = dataSorted.begin();
         iter != dataSorted.end();
         ++iter)
        curSelect->list->AddItem(iter->first + " = " + iter->second, "");
    if (data.find(userSetting) != data.end())
        curSelect->list->SetCurrentItem(userSetting + " = " +
                                        configHandler->GetString(userSetting));
