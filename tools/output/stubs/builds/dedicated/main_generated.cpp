...
#define LOG_SECTION_CURRENT LOG_SECTION_DEFAULT
    ...
    cmdLine.AddInt(
      0, "sleeptime", "Number of seconds to sleep between game-over checks");
...
main(int argc, char* argv[])
{
    ...
    try {
        ...
        cmdLine.SetUsageDescription("Usage: " + binaryName +
                                    " [options] path_to_script.txt");
        ...
        if (!fh.FileExists())
            throw content_error("script does not exist in given location: " +
                                scriptName);
        ...
        const unsigned sleepTime =
          cmdLine.IsSet("sleeptime") ? cmdLine.GetInt("sleeptime") : 1;
        const unsigned randSeed =
          time(NULL) % ((spring_gettime().toNanoSecsi() + 1) * 9007);
        ...
        if (dsGameSetup->mapHash != 0) {
            ...
            dsGameData->SetMapChecksum(
              archiveScanner->GetArchiveCompleteChecksum(dsGameSetup->mapName));
        ...
        if (dsGameSetup->modHash != 0) {
            ...
            const std::string& modArchive =
              archiveScanner->ArchiveFromName(dsGameSetup->modName);
            const unsigned int modCheckSum =
        ...
        while (!server->HasFinished()) {
            ...
            if (printData) {
                ...
                const boost::scoped_ptr<CDemoRecorder>& demoRec =
                  server->GetDemoRecorder();
                const boost::uint8_t* gameID =
                  (demoRec->GetFileHeader()).gameID;
