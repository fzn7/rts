...
struct color_quad_u8
{
  ...
  public:
    ...
    explicit inline color_quad_u8(parameter_t y,
    ...
    explicit inline color_quad_u8(eNoClamp,
                                  parameter_t y,
                                  parameter_t alpha = component_traits::cMax)
    {
    ...
    inline color_quad_u8& set(parameter_t y,
                              parameter_t alpha = component_traits::cMax)
    {
        y = clamp(y);
        alpha = clamp(alpha);
    ...
    inline color_quad_u8& set_noclamp_y_alpha(
      parameter_t y,
      parameter_t alpha = component_traits::cMax)
    {
        RG_ETC1_ASSERT((y >= component_traits::cMin) &&
                       (y <= component_traits::cMax));
        RG_ETC1_ASSERT((alpha >= component_traits::cMin) &&
                       (alpha <= component_traits::cMax));
    ...
    inline color_quad_u8& set_noclamp_rgba(parameter_t red,
                                           parameter_t green,
                                           parameter_t blue,
                                           parameter_t alpha)
    {
        RG_ETC1_ASSERT((red >= component_traits::cMin) &&
                       (red <= component_traits::cMax));
        RG_ETC1_ASSERT((green >= component_traits::cMin) &&
                       (green <= component_traits::cMax));
        RG_ETC1_ASSERT((blue >= component_traits::cMin) &&
                       (blue <= component_traits::cMax));
        RG_ETC1_ASSERT((alpha >= component_traits::cMin) &&
                       (alpha <= component_traits::cMax));
    ...
    inline color_quad_u8& set_noclamp_rgb(parameter_t red,
                                          parameter_t green,
                                          parameter_t blue)
    {
        RG_ETC1_ASSERT((red >= component_traits::cMin) &&
    ...
    inline color_quad_u8& clamp(const color_quad_u8& l, const color_quad_u8& h)
    {
        for (uint i = 0; i < cNumComps; i++)
            c[i] = static_cast<component_t>(
              rg_etc1::clamp<parameter_t>(c[i], l[i], h[i]));
        return *this;
    }
    ...
    inline color_quad_u8& clamp(parameter_t l, parameter_t h)
    {
        for (uint i = 0; i < cNumComps; i++)
            c[i] =
              static_cast<component_t>(rg_etc1::clamp<parameter_t>(c[i], l, h));
        return *this;
    }
    ...
    {
        return static_cast<parameter_t>(
          (19595U * r + 38470U * g + 7471U * b + 32768U) >> 16U);
    }
    ...
    {
        return rg_etc1::square(r - c.r) + rg_etc1::square(g - c.g) +
               rg_etc1::square(b - c.b);
    }

    inline uint squared_distance_rgba(const color_quad_u8& c) const
    {
        return rg_etc1::square(r - c.r) + rg_etc1::square(g - c.g) +
               rg_etc1::square(b - c.b) + rg_etc1::square(a - c.a);
    }
    ...
    friend color_quad_u8 operator+(const color_quad_u8& lhs,
                                   const color_quad_u8& rhs)
    {
...
static const int
  g_etc1_inten_tables[cETC1IntenModifierValues][cETC1SelectorValues] = {
      { -8, -2, 2, 8 },       { -17, -5, 5, 17 },    { -29, -9, 9, 29 },
      { -42, -13, 13, 42 },   { -60, -18, 18, 60 },  { -80, -24, 24, 80 },
      { -106, -33, 33, 106 }, { -183, -47, 47, 183 }
  };

static const uint8 g_etc1_to_selector_index[cETC1SelectorValues] = { 2,
                                                                     3,
                                                                     1,
                                                                     0 };
static const uint8 g_selector_index_to_etc1[cETC1SelectorValues] = { 3,
                                                                     2,
                                                                     0,
                                                                     1 };

// Given an ETC1 diff/inten_table/selector, and an 8-bit desired color, this
// table encodes the best packed_color in the low byte, and the abs error in the
// high byte.
static uint16
  g_etc1_inverse_lookup[2 * 8 * 4]
                       [256]; // [diff/inten_table/selector][desired_color]

// g_color8_to_etc_block_config[color][table_index] = Supplies for each 8-bit
// color value a list of packed ETC1 diff/intensity
// table/selectors/packed_colors that map to that color. To pack: diff | (inten
// << 1) | (selector << 4) | (packed_c << 8)
...
struct etc1_block
{
    ...
    // Returned selector value ranges from 0-3 and is a direct index into
    // g_etc1_inten_tables.
    ...
    // Selector "val" ranges from 0-3 and is a direct index into
    // g_etc1_inten_tables.
    ...
    // Base color 5
    static uint16 pack_color5(const color_quad_u8& color,
                              bool scaled,
                              uint bias = 127U);
    static uint16 pack_color5(uint r,
                              uint g,
                              uint b,
                              bool scaled,
                              uint bias = 127U);
    ...
    static color_quad_u8 unpack_color5(uint16 packed_color5,
                                       bool scaled,
                                       uint alpha = 255U);
    static void unpack_color5(uint& r,
                              uint& g,
                              uint& b,
                              uint16 packed_color,
                              bool scaled);
    ...
    // Abs color 4
    static uint16 pack_color4(const color_quad_u8& color,
                              bool scaled,
                              uint bias = 127U);
    static uint16 pack_color4(uint r,
                              uint g,
                              uint b,
                              bool scaled,
                              uint bias = 127U);

    static color_quad_u8 unpack_color4(uint16 packed_color4,
                                       bool scaled,
                                       uint alpha = 255U);
    static void unpack_color4(uint& r,
                              uint& g,
                              uint& b,
    ...
    // subblock colors
    static void get_diff_subblock_colors(color_quad_u8* pDst,
                                         uint16 packed_color5,
                                         uint table_idx);
    static bool get_diff_subblock_colors(color_quad_u8* pDst,
                                         uint16 packed_color5,
                                         uint16 packed_delta3,
                                         uint table_idx);
    static void get_abs_subblock_colors(color_quad_u8* pDst,
                                        uint16 packed_color4,
                                        uint table_idx);

    static inline void unscaled_to_scaled_color(color_quad_u8& dst,
                                                const color_quad_u8& src,
                                                bool color4)
    {
        if (color4) {
            dst.r = src.r | (src.r << 4);
...
T*
indirect_radix_sort(uint num_indices,
                    T* pIndices0,
                    T* pIndices1,
                    const Q* pKeys,
                    uint key_ofs,
...
#define RG_ETC1_GET_KEY(p)                                                     \
    (*(const uint*)((const uint8*)(pKeys + *(p)) + key_ofs))
#define RG_ETC1_GET_KEY_FROM_INDEX(i)                                          \
    (*(const uint*)((const uint8*)(pKeys + (i)) + key_ofs))
                          ...
                          uint alpha)
{
    ...
    const bool success =
      unpack_color5(result, packed_color5, packed_delta3, scaled, alpha);
    r = result.r;
    g = result.g;
    b = result.b;
    return success;
                                     ...
                                     uint table_idx)
{
    ...
    const int ir = static_cast<int>(r), ig = static_cast<int>(g),
                  ...
                  bool preserve_alpha)
{
    ...
    if (diff_flag) {
        ...
        if (!etc1_block::get_diff_subblock_colors(
              subblock_colors1, base_color5, delta_color3, table_index1))
            success = false;
    } else {
        const uint16 base_color4_0 = block.get_base4_color(0);
        etc1_block::get_abs_subblock_colors(
          subblock_colors0, base_color4_0, table_index0);

        const uint16 base_color4_1 = block.get_base4_color(1);
        etc1_block::get_abs_subblock_colors(
          subblock_colors1, base_color4_1, table_index1);
    }
...
struct etc1_solution_coordinates
{
    ...
    inline etc1_solution_coordinates(uint r,
                                     uint g,
                                     uint b,
                                     uint inten_table,
                                     bool color4)
    ...
    inline etc1_solution_coordinates(const color_quad_u8& c,
                                     uint inten_table,
                                     bool color4)
      : m_unscaled_color(c)
      , m_inten_table(inten_table)
    ...
    inline etc1_solution_coordinates& operator=(
      const etc1_solution_coordinates& rhs)
    {
    ...
    inline void get_block_colors(color_quad_u8* pBlock_colors)
    {
        ...
        pBlock_colors[0].set(
          br + pInten_table[0], bg + pInten_table[0], bb + pInten_table[0]);
        pBlock_colors[1].set(
          br + pInten_table[1], bg + pInten_table[1], bb + pInten_table[1]);
        pBlock_colors[2].set(
          br + pInten_table[2], bg + pInten_table[2], bb + pInten_table[2]);
        pBlock_colors[3].set(
          br + pInten_table[3], bg + pInten_table[3], bb + pInten_table[3]);
    }
...
etc1_optimizer::compute()
{
    ...
    // Scan through a subset of the 3D lattice centered around the avg block
    // color trying each 3D (555 or 444) lattice point as a potential block
    // color. Each time a better solution is found try to refine the current
    // solution's block color based of the current selectors and intensity table
    // index.
        ...
        for (int ydi = 0; ydi < scan_delta_size; ydi++) {
            ...
            for (int xdi = 0; xdi < scan_delta_size; xdi++) {
                ...
                etc1_solution_coordinates coords(
                  mbr, mbg, mbb, 0, m_pParams->m_use_color4);
                if (m_pParams->m_quality == cHighQuality) {
                    if (!evaluate_solution(
                          coords, m_trial_solution, &m_best_solution))
                        continue;
                } else {
                    if (!evaluate_solution_fast(
                          coords, m_trial_solution, &m_best_solution))
                        continue;
                }

                // Now we have the input block, the avg. color of the input
                // pixels, a set of trial selector indices, and the block
                // color+intensity index. Now, for each component, attempt to
                // refine the current solution by solving a simple linear
                // equation. For example, for 4 colors: The goal is: pixel0 -
                // (block_color+inten_table[selector0]) + pixel1 -
                // (block_color+inten_table[selector1]) + pixel2 -
                // (block_color+inten_table[selector2]) + pixel3 -
                // (block_color+inten_table[selector3]) = 0 Rearranging this:
                // (pixel0 + pixel1 + pixel2 + pixel3) -
                // (block_color+inten_table[selector0]) -
                // (block_color+inten_table[selector1]) -
                // (block_color+inten_table[selector2]) -
                // (block_color+inten_table[selector3]) = 0 (pixel0 + pixel1 +
                // pixel2 + pixel3) - block_color - inten_table[selector0] -
                // block_color-inten_table[selector1] -
                // block_color-inten_table[selector2] -
                // block_color-inten_table[selector3] = 0 (pixel0 + pixel1 +
                // pixel2 + pixel3) - 4*block_color - inten_table[selector0] -
                // inten_table[selector1] - inten_table[selector2] -
                // inten_table[selector3] = 0 (pixel0 + pixel1 + pixel2 +
                // pixel3) - 4*block_color - (inten_table[selector0] +
                // inten_table[selector1] + inten_table[selector2] +
                // inten_table[selector3]) = 0 (pixel0 + pixel1 + pixel2 +
                // pixel3)/4 - block_color - (inten_table[selector0] +
                // inten_table[selector1] + inten_table[selector2] +
                // inten_table[selector3])/4 = 0 block_color = (pixel0 + pixel1
                // + pixel2 + pixel3)/4 - (inten_table[selector0] +
                // inten_table[selector1] + inten_table[selector2] +
                // inten_table[selector3])/4 So what this means:
                // optimal_block_color = avg_input - avg_inten_delta
                // So the optimal block color can be computed by taking the
                // average block color and subtracting the current average of
                // the intensity delta. Unfortunately, optimal_block_color must
                // then be quantized to 555 or 444 so it's not always possible
                // to improve matters using this formula. Also, the above
                // formula is for unclamped intensity deltas. The actual
                // implementation takes into account clamping.

                const uint max_refinement_trials =
                  (m_pParams->m_quality == cLowQuality)
                    ? 2
                    : (((xd | yd | zd) == 0) ? 4 : 2);
                for (uint refinement_trial = 0;
                     refinement_trial < max_refinement_trials;
                     refinement_trial++) {
                    const uint8* pSelectors = m_best_solution.m_selectors;
                    const int* pInten_table =
                      g_etc1_inten_tables[m_best_solution.m_coords
                                            .m_inten_table];

                    int delta_sum_r = 0, delta_sum_g = 0, delta_sum_b = 0;
                    const color_quad_u8 base_color(
                      m_best_solution.m_coords.get_scaled_color());
                    for (uint r = 0; r < n; r++) {
                        ...
                        delta_sum_r +=
                          rg_etc1::clamp<int>(base_color.r + yd, 0, 255) -
                          base_color.r;
                        delta_sum_g +=
                          rg_etc1::clamp<int>(base_color.g + yd, 0, 255) -
                          base_color.g;
                        delta_sum_b +=
                          rg_etc1::clamp<int>(base_color.b + yd, 0, 255) -
                          base_color.b;
                    }
                    if ((!delta_sum_r) && (!delta_sum_g) && (!delta_sum_b))
                        break;
                    const float avg_delta_r_f =
                      static_cast<float>(delta_sum_r) / n;
                    const float avg_delta_g_f =
                      static_cast<float>(delta_sum_g) / n;
                    const float avg_delta_b_f =
                      static_cast<float>(delta_sum_b) / n;
                    const int br1 = rg_etc1::clamp<int>(
                      static_cast<uint>((m_avg_color[0] - avg_delta_r_f) *
                                          m_limit / 255.0f +
                                        .5f),
                      0,
                      m_limit);
                    const int bg1 = rg_etc1::clamp<int>(
                      static_cast<uint>((m_avg_color[1] - avg_delta_g_f) *
                                          m_limit / 255.0f +
                                        .5f),
                      0,
                      m_limit);
                    const int bb1 = rg_etc1::clamp<int>(
                      static_cast<uint>((m_avg_color[2] - avg_delta_b_f) *
                                          m_limit / 255.0f +
                                        .5f),
                      0,
                      m_limit);
                    ...
                    if ((mbr == br1) && (mbg == bg1) && (mbb == bb1))
                        skip = true;
                              ...
                              m_best_solution.m_coords.m_unscaled_color.r) &&
                             (bg1 ==
                              m_best_solution.m_coords.m_unscaled_color.g) &&
                             (bb1 ==
                              m_best_solution.m_coords.m_unscaled_color.b))
                        skip = true;
                    ...
                    etc1_solution_coordinates coords1(
                      br1, bg1, bb1, 0, m_pParams->m_use_color4);
                    if (m_pParams->m_quality == cHighQuality) {
                        if (!evaluate_solution(
                              coords1, m_trial_solution, &m_best_solution))
                            break;
                    } else {
                        if (!evaluate_solution_fast(
                              coords1, m_trial_solution, &m_best_solution))
...
#ifdef RG_ETC1_BUILD_DEBUG
    {
        ...
        for (uint i = 0; i < n; i++)
            actual_error +=
              pSrc_pixels[i].squared_distance_rgb(block_colors[pSelectors[i]]);
    ...
    m_pResult->m_block_color_unscaled =
      m_best_solution.m_coords.m_unscaled_color;
...
etc1_optimizer::init(const params& p, results& r)
{
    ...
    m_br = rg_etc1::clamp<int>(
      static_cast<uint>(m_avg_color[0] * m_limit / 255.0f + .5f), 0, m_limit);
    m_bg = rg_etc1::clamp<int>(
      static_cast<uint>(m_avg_color[1] * m_limit / 255.0f + .5f), 0, m_limit);
    m_bb = rg_etc1::clamp<int>(
      static_cast<uint>(m_avg_color[2] * m_limit / 255.0f + .5f), 0, m_limit);

    if (m_pParams->m_quality <= cMediumQuality) {
        m_pSorted_luma_indices = indirect_radix_sort(n,
                                                     m_sorted_luma[0],
                                                     m_sorted_luma[1],
                                                     m_luma,
                                                     0,
                                  ...
                                  potential_solution* pBest_solution)
{
    ...
    for (uint inten_table = 0; inten_table < cETC1IntenModifierValues;
         inten_table++) {
        ...
        for (uint s = 0; s < 4; s++) {
            const int yd = pInten_table[s];
            block_colors[s].set(
              base_color.r + yd, base_color.g + yd, base_color.b + yd, 0);
        }
        ...
        for (uint c = 0; c < n; c++) {
            ...
            uint trial_error =
              rg_etc1::square(src_pixel.r - block_colors[1].r) +
                                       ...
                                       potential_solution* pBest_solution)
{
    ...
    for (int inten_table = cETC1IntenModifierValues - 1; inten_table >= 0;
         --inten_table) {
        const int* pInten_table = g_etc1_inten_tables[inten_table];
        ...
        // evaluate_solution_fast() enforces/assumesd a total ordering of the
        // input colors along the intensity (1,1,1) axis to more quickly
        // classify the inputs to selectors. The inputs colors have been
        // presorted along the projection onto this axis, and ETC1 block colors
        // are always ordered along the intensity axis, so this classification
        // is fast. 0   1   2   3
        //   01  12  23
        const uint block_inten_midpoints[3] = { block_inten[0] + block_inten[1],
                                                block_inten[1] + block_inten[2],
                                                block_inten[2] +
                                                  block_inten[3] };
        ...
        if ((m_pSorted_luma[n - 1] * 2) < block_inten_midpoints[0]) {
            if (block_inten[0] > m_pSorted_luma[n - 1]) {
                const uint min_error =
                  labs(block_inten[0] - m_pSorted_luma[n - 1]);
            ...
            for (uint c = 0; c < n; c++)
                total_error +=
                  block_colors[0].squared_distance_rgb(pSrc_pixels[c]);
            ...
            for (uint c = 0; c < n; c++)
                total_error +=
                  block_colors[3].squared_distance_rgb(pSrc_pixels[c]);
        } else {
            ...
            for (c = 0; c < n; c++) {
                ...
                m_temp_selectors[sorted_pixel_index] =
                  static_cast<uint8>(cur_selector);
                total_error += block_colors[cur_selector].squared_distance_rgb(
            ...
            while (c < n) {
                ...
                total_error += block_colors[3].squared_distance_rgb(
                  pSrc_pixels[sorted_pixel_index]);
                ++c;
            }
        }
...
etc1_decode_value(uint diff, uint inten, uint selector, uint packed_c)
{
    ...
    limit;
    RG_ETC1_ASSERT((diff < 2) && (inten < 8) && (selector < 4) &&
                   (packed_c < limit));
    int c;
    if (diff)
    ...
    for (uint diff = 0; diff < 2; diff++) {
        ...
        for (uint inten = 0; inten < 8; inten++) {
            for (uint selector = 0; selector < 4; selector++) {
                const uint inverse_table_index =
                  diff + (inten << 1) + (selector << 4);
                for (uint color = 0; color < 256; color++) {
                    uint best_error = cUINT32_MAX, best_packed_c = 0;
                    for (uint packed_c = 0; packed_c < limit; packed_c++) {
                        int v =
                          etc1_decode_value(diff, inten, selector, packed_c);
                            ...
                            etc1_pack_params& pack_params)
{
    ...
    // For each possible 8-bit value, there is a precomputed list of
    // diff/inten/selector configurations that allow that 8-bit value to be
    // encoded with no error.
    for (uint i = 0; i < 3; i++) {
        ...
        for (int delta = -delta_range; delta <= delta_range; delta++) {
            const int c_plus_delta =
              rg_etc1::clamp<int>(pColor[i] + delta, 0, 255);
            ...
            if (!c_plus_delta)
                ...
                pTable =
                  g_color8_to_etc_block_config_1_to_254[c_plus_delta - 1];
            ...
            do {
                ...
                RG_ETC1_ASSERT(etc1_decode_value(diff, inten, selector, p0) ==
                               (uint)c_plus_delta);
#endif
                ...
                const uint trial_error =
                  rg_etc1::square(c_plus_delta - pColor[i]) +
                  rg_etc1::square(p1 >> 8) + rg_etc1::square(p2 >> 8);
                if (trial_error < best_error) {
                    best_error = trial_error;
                    best_x = x;
...
found_perfect_match:
    ...
    block.m_bytes[3] =
      static_cast<uint8>(((inten | (inten << 3)) << 2) | (diff << 1));

    const uint etc1_selector = g_selector_index_to_etc1[(best_x >> 4) & 3];
    *reinterpret_cast<uint16*>(&block.m_bytes[4]) =
      (etc1_selector & 2) ? 0xFFFF : 0;
    *reinterpret_cast<uint16*>(&block.m_bytes[6]) =
      (etc1_selector & 1) ? 0xFFFF : 0;

    const uint best_packed_c0 = (best_x >> 8) & 255;
    if (diff) {
        block.m_bytes[best_i] = static_cast<uint8>(best_packed_c0 << 3);
        block.m_bytes[s_next_comp[best_i]] =
          static_cast<uint8>(best_packed_c1 << 3);
        block.m_bytes[s_next_comp[best_i + 1]] =
          static_cast<uint8>(best_packed_c2 << 3);
    } else {
        block.m_bytes[best_i] =
          static_cast<uint8>(best_packed_c0 | (best_packed_c0 << 4));
        block.m_bytes[s_next_comp[best_i]] =
          static_cast<uint8>(best_packed_c1 | (best_packed_c1 << 4));
        block.m_bytes[s_next_comp[best_i + 1]] =
          static_cast<uint8>(best_packed_c2 | (best_packed_c2 << 4));
    }
        ...
        for (int delta = -delta_range; delta <= delta_range; delta++) {
            ...
            do {
                ...
                if ((diff) && (pBase_color5_unscaled)) {
                    ...
                    int delta =
                      p0 - static_cast<int>(pBase_color5_unscaled->c[i]);
                    if ((delta < cETC1ColorDeltaMin) ||
                        (delta > cETC1ColorDeltaMax)) {
                        if (*pTable == 0xFFFF)
                            break;
                        continue;
                    }
...
#ifdef RG_ETC1_BUILD_DEBUG
                {
                    ...
                    const uint p0 = (x >> 8) & 255;
                    RG_ETC1_ASSERT(
                      etc1_decode_value(diff, inten, selector, p0) ==
                      (uint)c_plus_delta);
                }
#endif
                ...
                if ((diff) && (pBase_color5_unscaled)) {
                    int delta1 =
                      (p1 & 0xFF) - static_cast<int>(
                                      pBase_color5_unscaled->c[s_next_comp[i]]);
                    int delta2 =
                      (p2 & 0xFF) -
                      static_cast<int>(
                        pBase_color5_unscaled->c[s_next_comp[i + 1]]);
                    if ((delta1 < cETC1ColorDeltaMin) ||
                        (delta1 > cETC1ColorDeltaMax) ||
                        (delta2 < cETC1ColorDeltaMin) ||
                        (delta2 > cETC1ColorDeltaMax)) {
                        if (*pTable == 0xFFFF)
                            break;
                        continue;
                    }
...
found_perfect_match:
    ...
    results.m_block_color_unscaled[s_next_comp[best_i]] =
      static_cast<uint8>(best_packed_c1);
    results.m_block_color_unscaled[s_next_comp[best_i + 1]] =
      static_cast<uint8>(best_packed_c2);
...
dither_block_555(color_quad_u8* dest, const color_quad_u8* block)
{
    ...
    for (int ch = 0; ch < 3; ch++) {
        ...
        for (int y = 0; y < 4; y++) {
            ...
            dp[4] =
              quant[bp[4] +
                    ((7 * ep1[0] + 3 * ep2[2] + 5 * ep2[1] + ep2[0]) >> 4)];
            ep1[1] = bp[4] - dp[4];

            // pixel 2
            dp[8] =
              quant[bp[8] +
                    ((7 * ep1[1] + 3 * ep2[3] + 5 * ep2[2] + ep2[1]) >> 4)];
            ep1[2] = bp[8] - dp[8];
                ...
                etc1_pack_params& pack_params)
{
    const color_quad_u8* pSrc_pixels =
      reinterpret_cast<const color_quad_u8*>(pSrc_pixels_rgba);
    ...
    if (!r)
        return static_cast<unsigned int>(
          16 * pack_etc1_block_solid_color(
                 dst_block, &pSrc_pixels[0].r, pack_params));
    ...
    for (uint flip = 0; flip < 2; flip++) {
        for (uint use_color4 = 0; use_color4 < 2; use_color4++) {
            ...
            for (subblock = 0; subblock < 2; subblock++) {
                if (flip)
                    memcpy(subblock_pixels,
                           pSrc_pixels + subblock * 8,
                           sizeof(color_quad_u8) * 8);
                else {
                ...
                if (params.m_quality == cHighQuality) {
                    static const int s_scan_delta_0_to_4[] = { -4, -3, -2,
                                                               -1, 0,  1,
                                                               2,  3,  4 };
                    params.m_scan_delta_size =
                      RG_ETC1_ARRAY_SIZE(s_scan_delta_0_to_4);
                ...
                if (params.m_quality >= cMediumQuality) {
                    // TODO: Fix fairly arbitrary/unrefined thresholds that
                    // control how far away to scan for potentially better
                    // solutions.
                    ...
                    if (results[subblock].m_error > refinement_error_thresh0) {
                        if (params.m_quality == cMediumQuality) {
                            static const int s_scan_delta_2_to_3[] = {
                                -3, -2, 2, 3
                            };
                            params.m_scan_delta_size =
                              RG_ETC1_ARRAY_SIZE(s_scan_delta_2_to_3);
                            params.m_pScan_deltas = s_scan_delta_2_to_3;
                        } else {
                            static const int s_scan_delta_5_to_5[] = { -5, 5 };
                            static const int s_scan_delta_5_to_8[] = {
                                -8, -7, -6, -5, 5, 6, 7, 8
                            };
                            if (results[subblock].m_error >
                                refinement_error_thresh1) {
                                params.m_scan_delta_size =
                                  RG_ETC1_ARRAY_SIZE(s_scan_delta_5_to_8);
                                params.m_pScan_deltas = s_scan_delta_5_to_8;
                            } else {
                                params.m_scan_delta_size =
                                  RG_ETC1_ARRAY_SIZE(s_scan_delta_5_to_5);
    ...
    int dr = best_results[1].m_block_color_unscaled.r -
             best_results[0].m_block_color_unscaled.r;
    int dg = best_results[1].m_block_color_unscaled.g -
             best_results[0].m_block_color_unscaled.g;
    int db = best_results[1].m_block_color_unscaled.b -
             best_results[0].m_block_color_unscaled.b;
    RG_ETC1_ASSERT(best_use_color4 ||
                   (rg_etc1::minimum(dr, dg, db) >= cETC1ColorDeltaMin) &&
                     (rg_etc1::maximum(dr, dg, db) <= cETC1ColorDeltaMax));

    if (best_use_color4) {
        dst_block.m_bytes[0] =
          static_cast<uint8>(best_results[1].m_block_color_unscaled.r |
                             (best_results[0].m_block_color_unscaled.r << 4));
        dst_block.m_bytes[1] =
          static_cast<uint8>(best_results[1].m_block_color_unscaled.g |
                             (best_results[0].m_block_color_unscaled.g << 4));
        dst_block.m_bytes[2] =
          static_cast<uint8>(best_results[1].m_block_color_unscaled.b |
                             (best_results[0].m_block_color_unscaled.b << 4));
    } else {
        if (dr < 0)
            dr += 8;
        dst_block.m_bytes[0] = static_cast<uint8>(
          (best_results[0].m_block_color_unscaled.r << 3) | dr);
        if (dg < 0)
            dg += 8;
        dst_block.m_bytes[1] = static_cast<uint8>(
          (best_results[0].m_block_color_unscaled.g << 3) | dg);
        if (db < 0)
            db += 8;
        dst_block.m_bytes[2] = static_cast<uint8>(
          (best_results[0].m_block_color_unscaled.b << 3) | db);
    }

    dst_block.m_bytes[3] =
      static_cast<uint8>((best_results[1].m_block_inten_table << 2) |
