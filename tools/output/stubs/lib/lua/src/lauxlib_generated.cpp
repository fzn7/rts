...
luaL_argerror(lua_State* L, int narg, const char* extramsg)
{
    ...
    if (strcmp(ar.namewhat, "method") == 0) {
        ...
        if (narg == 0) /* error is in the self argument itself? */
            return luaL_error(
              L, "calling " LUA_QS " on bad self (%s)", ar.name, extramsg);
    }
    if (ar.name == NULL)
        ar.name = "?";
    return luaL_error(
      L, "bad argument #%d to " LUA_QS " (%s)", narg, ar.name, extramsg);
}
                 ...
                 const char* const lst[])
{
    ...
    return luaL_argerror(
      L, narg, lua_pushfstring(L, "invalid option " LUA_QS, name));
}
...
luaL_checknumber(lua_State* L, int narg)
{
    ...
    if (math::isinf(d) || math::isnan(d))
        luaL_argerror(
          L, narg, "number expected, got NAN (check your code for div0)");
        // assert(!math::isinf(d));
        // assert(!math::isnan(d));
#endif
