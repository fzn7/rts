...
{
    lua_pushnumber(L,
                   math::atan2(luaL_checknumber_noassert(L, 1),
                               luaL_checknumber_noassert(L, 2)));
    return 1;
}
...
{
    lua_pushnumber(L,
                   math::fmod(luaL_checknumber_noassert(L, 1),
                              luaL_checknumber_noassert(L, 2)));
    return 1;
}
...
{
    lua_pushnumber(L,
                   math::pow(luaL_checknumber_noassert(L, 1),
                             luaL_checknumber_noassert(L, 2)));
    return 1;
}
...
{
    lua_pushnumber(
      L, math::ldexp(luaL_checknumber_noassert(L, 1), luaL_checkint(L, 2)));
    return 1;
}
...
math_random(lua_State* L)
{
    ...
    switch (lua_gettop(L)) {      /* check number of arguments */
        ...
        case 1: { /* only upper limit */
            ...
            lua_pushnumber(L,
                           math::floor(r * u) + 1); /* int between 1 and `u' */
            break;
        }
