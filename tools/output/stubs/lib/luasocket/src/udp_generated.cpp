...
meth_sendto(lua_State* L)
{
    ...
    if (!luaSocketRestrictions->isAllowed(
          CLuaSocketRestrictions::UDP_CONNECT, ip, port)) {
        lua_pushnil(L);
    ...
    timeout_markstart(tm);
    err = socket_sendto(
      &udp->sock, data, count, &sent, (SA*)&addr, sizeof(addr), tm);
    if (err != IO_DONE) {
        lua_pushnil(L);
        lua_pushstring(L, udp_strerror(err));
        return 2;
...
meth_receivefrom(lua_State* L)
{
    ...
    count = MIN(count, sizeof(buffer));
    err = socket_recvfrom(
      &udp->sock, buffer, count, &got, (SA*)&addr, &addr_len, tm);
    if (err == IO_DONE) {
        lua_pushlstring(L, buffer, got);
...
meth_setpeername(lua_State* L)
{
    ...
    unsigned short port = connecting ? (unsigned short)luaL_checknumber(L, 3)
