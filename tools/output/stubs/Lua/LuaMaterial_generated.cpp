        ...
        switch (prev.type) {
            ...
            case LUASHADER_ASS: {
                if (luaMatHandler.resetDrawStateFuncs[prev.type]) {
                    luaMatHandler.resetDrawStateFuncs[prev.type](prev.type,
                                                                 deferredPass);
                }
            } break;
            case LUASHADER_NONE: {
...
LuaMatShader::Print(const string& indent, bool isDeferred) const
{
    ...
    LOG(
      "%s[shader][%s]", indent.c_str(), (isDeferred ? "deferred" : "standard"));
    ...
    for (lua_pushnil(L); lua_next(L, tableIdx) != 0; lua_pop(L, 1)) {
        ...
        if (key.find("uniforms") != std::string::npos) {
            ...
            if (key.find("standard") != std::string::npos) {
                uniforms[LuaMatShader::LUASHADER_PASS_FWD].Parse(L,
                                                                 lua_gettop(L));
                continue;
            }
            if (key.find("deferred") != std::string::npos) {
                uniforms[LuaMatShader::LUASHADER_PASS_DFR].Parse(L,
                                                                 lua_gettop(L));
                continue;
            }
        ...
        if (key.substr(0, 7) == "texunit") {
            ...
            if (key[7] == 's') {
                ...
                for (lua_pushnil(L); lua_next(L, texTable) != 0;
                     lua_pop(L, 1)) {
...
{
    for (int i : { LuaMatShader::LUASHADER_PASS_FWD,
...
{
    // NOTE: the order of the comparisons is important (it's sorted by the GL
    // perf cost of switching those states)
    int cmp = 0;
    ...
    if ((cmp = LuaMatShader::Compare(
           a.shaders[LuaMatShader::LUASHADER_PASS_FWD],
           b.shaders[LuaMatShader::LUASHADER_PASS_FWD])) != 0)
        return cmp;
    if ((cmp = LuaMatShader::Compare(
           a.shaders[LuaMatShader::LUASHADER_PASS_DFR],
           b.shaders[LuaMatShader::LUASHADER_PASS_DFR])) != 0)
        return cmp;
    ...
    // and there there < operator is also used to detect equality (a == b iff
    // !(a<b) && !(b<a)) that's why need to check _all_ properties
    if ((cmp = LuaMatUniforms::Compare(
           a.uniforms[LuaMatShader::LUASHADER_PASS_FWD],
           b.uniforms[LuaMatShader::LUASHADER_PASS_FWD])) != 0)
        return cmp;
    if ((cmp = LuaMatUniforms::Compare(
           a.uniforms[LuaMatShader::LUASHADER_PASS_DFR],
           b.uniforms[LuaMatShader::LUASHADER_PASS_DFR])) != 0)
        return cmp;
...
#define CULL_TO_STR(x)                                                         \
    (x == GL_FRONT) ? "front"                                                  \
                    : (x == GL_BACK) ? "back" : (x != 0) ? "false" : "unknown"
...
LuaMatUniforms::Validate(LuaMatShader* s)
{
    if (!s->IsCustomType()) {
        ...
        for (const auto& p : GetUniformsAndStandardName()) {
            ...
            LOG_L(L_WARNING,
                  "LuaMaterial: engine shaders prohibit the usage of uniform "
                  "\"%s\"",
                  p.second.c_str());
        }
        return;
    ...
    for (const auto& p : GetUniformsAndStandardName()) {
        ...
        glGetActiveUniform(
          s->openglID, p.first->loc, 0, nullptr, &size, &uniformType, nullptr);

        if (p.first->GetType() == uniformType)
            continue;

        LOG_L(L_WARNING,
              "LuaMaterial: incorrect uniform type for \"%s\", got: %s "
              "expected: %s",
              p.second.c_str(),
              GlUniformTypeToString(uniformType),
              GlUniformTypeToString(p.first->GetType()));
    }
}
...
{
    for (unsigned int i = LuaMatShader::LUASHADER_NONE;
         i < LuaMatShader::LUASHADER_LAST;
         i++) {
