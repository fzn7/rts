...
LuaParser::SetupEnv()
{
    ...
    // FIXME: replace "random" _as in_ LuaHandleSynced (can write your own for
    // now), we cannot use the LHS one cause gs-> is not valid in LuaParser
...
LuaParser::Execute()
{
    ...
    if (error != 0) {
        ...
        LOG_L(
          L_ERROR, "%i, %s, %s", error, codeLabel.c_str(), errorLog.c_str());
        LUA_CLOSE(&L);
        return false;
    }
...
LuaParser::TimeCheck(lua_State* L)
{
    ...
    LOG(
      "%s %ldms", name.c_str(), (long int)spring_tomsecs(endTime - startTime));
    return lua_gettop(L);
}
...
LuaParser::Include(lua_State* L)
{
    ...
    if (!fh.FileExists()) {
        char buf[1024];
        SNPRINTF(
          buf, sizeof(buf), "Include() file missing '%s'\n", filename.c_str());
...
LuaParser::FileExists(lua_State* L)
{
    ...
    lua_pushboolean(
      L, CFileHandler::FileExists(filename, currentParser->accessModes));
    return 1;
}
...
LuaTable::SubTable(const string& mixedKey) const
{
    ...
    const string key = !(parser ? parser->lowerCppKeys : true)
                         ? mixedKey
                         : StringToLower(mixedKey);
...
LuaTable::PushTable() const
{
    ...
    if ((refnum != LUA_NOREF) && (parser->currentRef == refnum)) {
        if (!lua_istable(L, -1)) {
            LOG_L(L_ERROR,
                  "Internal Error: LuaTable::PushTable() = %s",
                  path.c_str());
            parser->currentRef = LUA_NOREF;
            lua_settop(L, 0);
            return false;
...
ParseFloat3(lua_State* L, int index, float3& value)
{
    if (lua_istable(L, index)) {
        ...
        const int count = sscanf(
          lua_tostring(L, index), "%f %f %f", &value.x, &value.y, &value.z);
        if (count == 3) {
            return true;
        }
    }
...
ParseFloat4(lua_State* L, int index, float4& value)
{
    if (lua_istable(L, index)) {
        ...
        const int count = sscanf(lua_tostring(L, index),
                                 "%f %f %f %f",
                                 &value.x,
                                 &value.y,
                                 &value.z,
...
LuaTable::Get(const string& key, float def) const
{
    ...
    const float value = lua_tonumber(L, -1);
    if (unlikely(value == 0.f) && !lua_isnumber(L, -1) &&
        !lua_isstring(L, -1)) {
        lua_pop(L, 1);
        return def;
    }
