...
LuaFeatureDefs::PushEntries(lua_State* L)
{
    ...
    const std::array<const IndxFuncType, 3> indxFuncs = { FeatureDefIndex,
                                                          FeatureDefNewIndex,
                                                          FeatureDefMetatable };
    const std::array<const IterFuncType, 2> iterFuncs = { Pairs, Next };

    for (auto it = defsMap.cbegin(); it != defsMap.cend(); ++it) {
        const auto def = featureDefHandler->GetFeatureDefByID(
...
FeatureDefIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in FeatureDefs __index",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
FeatureDefNewIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in FeatureDefs __newindex",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
ModelHeight(lua_State* L, const void* data)
{
    return (LuaUtils::PushModelHeight(
      L, static_cast<const SolidObjectDef*>(data), false));
}
...
{
    return (LuaUtils::PushFeatureModelDrawType(
      L, static_cast<const FeatureDef*>(data)));
}
