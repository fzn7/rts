...
LuaZipFileWriter::PushUnsynced(lua_State* L)
{
    ...
    // FIXME when this is enabled LuaGaia/LuaRules unsynced has access to it
    // too!
...
LuaZipFileWriter::PushNew(lua_State* L, const string& filename, zipFile zip)
{
    ...
    ZipFileWriterUserdata* udata = static_cast<ZipFileWriterUserdata*>(
      lua_newuserdata(L, sizeof(ZipFileWriterUserdata)));
    ...
    if (zip) {
        ...
        string realname = dataDirsAccess.LocateFile(
          filename, FileQueryFlags::WRITE | FileQueryFlags::CREATE_DIRS);
...
{
    return static_cast<ZipFileWriterUserdata*>(
      luaL_checkudata(L, 1, "ZipFileWriter"));
}
...
LuaZipFileWriter::meta_open(lua_State* L)
{
    ...
    f->fileOpen = (Z_OK == zipOpenNewFileInZip(f->zip,
                                               name.c_str(),
                                               NULL,
                                               NULL,
                                               0,
 ...
 */
bool
LuaZipFileReader::PushNew(lua_State* L,
                          const string& filename,
                          IArchive* archive)
{
    luaL_checkstack(L, 2, __FUNCTION__);

    ZipFileReaderUserdata* udata = static_cast<ZipFileReaderUserdata*>(
      lua_newuserdata(L, sizeof(ZipFileReaderUserdata)));
...
{
    return static_cast<ZipFileReaderUserdata*>(
      luaL_checkudata(L, 1, "ZipFileReader"));
}
...
LuaZipFileReader::meta_open(lua_State* L)
{
    ...
    f->stream = new std::stringstream(
      std::string((char*)&*buf.begin(), buf.size()), std::ios_base::in);
        ...
        for (n = first; nargs-- && success; n++) {
            if (lua_type(L, n) == LUA_TNUMBER) {
                ...
                switch (p[1]) {
                    ...
                    case 'a': /* file */
                        read_chars(
                          L, f, ~((size_t)0)); /* read MAX_SIZE_T chars */
...
GenerateZipFileInfo(const string& path)
{
    ...
    zipfi->tmz_date.tm_year =
      atoi(fileModificationDate.substr(0, 4).c_str()) - 1900;
                 ...
                 const string& modes)
{
    ...
    for (vector<string>::iterator it = folderPaths.begin();
         it != folderPaths.end();
         ++it) {
        const string& childFolderPath = *it;
        const string childFolderName = FileSystem::GetFilename(
          childFolderPath.substr(0, childFolderPath.length() - 1));
    ...
    for (vector<string>::iterator it = filePaths.begin(); it != filePaths.end();
         ++it) {
                        ...
                        const string& modes)
{
    ...
    const string folderName =
      (includeFolder) ? FileSystem::GetFilename(normFolderPath) + "/" : "";
    char buf[1024] = { '\0' };

    if (zipFolderFile == NULL) {
        SNPRINTF(buf,
                 sizeof(buf),
                 "[%s] could not open zipfile \"%s\" for writing",
                 __FUNCTION__,
                 zipFilePath.c_str());
    ...
    if (!dataDirsAccess.InWriteDir(normFolderPath)) {
        SNPRINTF(buf,
                 sizeof(buf),
                 "[%s] cannot zip \"%s\": outside writable data-directory",
                 __FUNCTION__,
