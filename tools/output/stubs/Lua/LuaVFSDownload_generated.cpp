...
{
    void processEvent() { eventHandler.DownloadStarted(ID); }
};
struct dlFinished : public dlEvent
{
    void processEvent() { eventHandler.DownloadFinished(ID); }
};
struct dlFailed : public dlEvent
{
    int errorID;
    void processEvent() { eventHandler.DownloadFailed(ID, errorID); }
};
...
void
UpdateProgress(int done, int size)
{
...
Download(int ID, const std::string& filename, DownloadEnum::Category cat)
{
    ...
    // FIXME: Progress is incorrectly updated when rapid is queried to check for
    // existing packages.
    ...
    // TODO: count will be 1 when archives already exist. We should instead
    // return a different result with DownloadFailed/DownloadFinished?
    if (count == 0) { // there's nothing to download
        ...
        // TODO: This works but there are errors spammed as it's trying to clear
        // timers in the main thread, which this is not: Error:
        // [Watchdog::ClearTimer(id)] Invalid thread 4 (_threadId=(nil))
