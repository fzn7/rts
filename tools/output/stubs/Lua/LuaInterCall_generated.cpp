...
static CLuaHandle*
GetLuaHandle(lua_State* L, int index)
{
    ...
    switch (*addr) {
        case LUA_UI:
            if (!luaUI)
                return NULL; // handle is not currently active
            return luaUI;
        case LUA_RULES:
            if (!luaRules)
                return NULL; // handle is not currently active
            return (isSrcSynced)
                     ? static_cast<CLuaHandle*>(&luaRules->syncedLuaHandle)
                     : static_cast<CLuaHandle*>(&luaRules->unsyncedLuaHandle);
        case LUA_GAIA:
            if (!luaGaia)
                return NULL; // handle is not currently active
            return (isSrcSynced)
                     ? static_cast<CLuaHandle*>(&luaGaia->syncedLuaHandle)
...
IndexHook(lua_State* L)
{
    if (!lua_israwstring(L, -1)) {
        return luaL_error(
          L, "Script.XYZ: only strings allowed got ", lua_typename(L, -1));
    }
