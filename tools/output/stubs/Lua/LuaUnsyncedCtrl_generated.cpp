...
ParseRawProjectile(lua_State* L, const char* caller, int index, bool synced)
{
    if (!lua_isnumber(L, index)) {
        luaL_error(L,
                   "[%s] projectile ID parameter in %s() not a number\n",
                   __FUNCTION__,
                   caller);
        return nullptr;
    }
...
ParseRawUnit(lua_State* L, const char* caller, int index)
{
    if (!lua_isnumber(L, index)) {
        luaL_error(
          L, "[%s] ID parameter in %s() not a number\n", __FUNCTION__, caller);
        return nullptr;
    }
...
LuaUnsyncedCtrl::PlaySoundFile(lua_State* L)
{
    ...
    if (soundID > 0) {
        ...
        int index = 3;
        if (args >= 5 && lua_isnumber(L, 3) && lua_isnumber(L, 4) &&
            lua_isnumber(L, 5)) {
            pos =
              float3(lua_tofloat(L, 3), lua_tofloat(L, 4), lua_tofloat(L, 5));
            pos_given = true;
            index += 3;

            if (args >= 8 && lua_isnumber(L, 6) && lua_isnumber(L, 7) &&
                lua_isnumber(L, 8)) {
                speed = float3(
                  lua_tofloat(L, 6), lua_tofloat(L, 7), lua_tofloat(L, 8));
                speed_given = true;
                index += 3;
            }
        }

        //! last argument (with and without pos/speed arguments) is the optional
        //! `sfx channel`
        ...
        if (args >= index) {
            if (lua_isstring(L, index)) {
                ...
                } else if (channelStr == "userinterface" ||
                           channelStr == "ui") {
...
LuaUnsyncedCtrl::SetSoundEffectParams(lua_State* L)
{
    ...
    //! first parse the 'preset' key (so all following params use it as base and
    //! override it)
...
LuaUnsyncedCtrl::SetCameraState(lua_State* L)
{
    ...
    if (!lua_istable(L, 1))
        luaL_error(L,
                   "Incorrect arguments to SetCameraState(table[, camTime])");
...
LuaUnsyncedCtrl::AssignMouseCursor(lua_State* L)
{
    ...
    const bool worked =
      mouse->AssignMouseCursor(cmdName, fileName, hotSpot, overwrite);
...
LuaUnsyncedCtrl::SetDrawModelsDeferred(lua_State* L)
{
    ...
    unitDrawer->SetDrawForwardPass(
      luaL_optboolean(L, 3, unitDrawer->DrawForward()));

    featureDrawer->SetDrawDeferredPass(luaL_checkboolean(L, 2));
    featureDrawer->SetDrawForwardPass(
      luaL_optboolean(L, 4, featureDrawer->DrawForward()));
        ...
        if (lua_israwstring(L, -2)) {
            ...
            if (lua_isnumber(L, -1)) {
                if (key == "radius") {
                    ...
                    light.SetFOV(
                      std::max(0.0f, std::min(180.0f, lua_tofloat(L, -1))));
                } else if (key == "ttl") {
...
LuaUnsyncedCtrl::AddMapLight(lua_State* L)
{
    ...
    GL::LightHandler* lightHandler =
      readMap->GetGroundDrawer()->GetLightHandler();
    GL::Light light;
...
LuaUnsyncedCtrl::UpdateMapLight(lua_State* L)
{
    ...
    GL::LightHandler* lightHandler =
      readMap->GetGroundDrawer()->GetLightHandler();
    GL::Light* light =
      (lightHandler != NULL) ? lightHandler->GetLight(lightHandle) : NULL;
...
LuaUnsyncedCtrl::SetMapLightTrackingState(lua_State* L)
{
    ...
    if (!lua_isnumber(L, 2)) {
        luaL_error(L,
                   "[%s] 1st and 2nd arguments should be numbers, 3rd and 4th "
                   "should be booleans",
                   __FUNCTION__);
        return 0;
    }
    ...
    if (light != NULL) {
        ret = AddLightTrackingTarget(
          L, light, trackEnable, trackUnit, __FUNCTION__);
    }
...
LuaUnsyncedCtrl::SetMapShader(lua_State* L)
{
    ...
    for (unsigned int i = 0; i < 2; i++) {
        luaMapShaderData.shaderIDs[i] =
          shaders.GetProgramName(lua_tonumber(L, i + 1));
    }
...
LuaUnsyncedCtrl::SetMapSquareTexture(lua_State* L)
{
    ...
    if (texName.empty()) {
        ...
        lua_pushboolean(
          L, groundTextures->SetSquareLuaTexture(texSquareX, texSquareY, 0));
        return 1;
    }
    ...
    if ((luaTexture = luaTextures.GetInfo(texName)) != nullptr) {
        ...
        lua_pushboolean(L,
                        groundTextures->SetSquareLuaTexture(
                          texSquareX, texSquareY, luaTexture->id));
        return 1;
    }
    ...
    if ((namedTexture = CNamedTextures::GetInfo(texName)) != nullptr) {
        ...
        lua_pushboolean(L,
                        groundTextures->SetSquareLuaTexture(
                          texSquareX, texSquareY, namedTexture->id));
        return 1;
    }
...
ParseLuaTextureData(lua_State* L, bool mapTex)
{
    ...
    const std::string& texName =
      mapTex ? luaL_checkstring(L, 2) : luaL_checkstring(L, 1);

    if (mapTex) {
        // convert type=LUATEX_* to MAP_*
        luaTexData.type = LuaOpenGLUtils::GetLuaMatTextureType(texType) -
                          LuaMatTexture::LUATEX_SMF_GRASS;
    ...
    if (!texName.empty()) {
        ...
        if ((luaTexData.id == 0) &&
            ((luaTexture = luaTextures.GetInfo(texName)) != nullptr)) {
            luaTexData.id = luaTexture->id;
            luaTexData.size.x = luaTexture->xsize;
            luaTexData.size.y = luaTexture->ysize;
        }
        if ((luaTexData.id == 0) &&
            ((namedTexture = CNamedTextures::GetInfo(texName)) != nullptr)) {
...
LuaUnsyncedCtrl::SetMapShadingTexture(lua_State* L)
{
    ...
    if (readMap != nullptr) {
        lua_pushboolean(L,
                        readMap->SetLuaTexture(ParseLuaTextureData(L, true)));
    } else {
        lua_pushboolean(L, false);
    }
...
LuaUnsyncedCtrl::AddUnitIcon(lua_State* L)
{
    ...
    lua_pushboolean(
      L, icon::iconHandler->AddIcon(iconName, texName, size, dist, radAdjust));
    return 1;
}
...
LuaUnsyncedCtrl::ExtractModArchiveFile(lua_State* L)
{
    ...
    if (fhRAW.FileExists()) {
        luaL_error(
          L, "cannot extract file \"%s\": already exists", path.c_str());
        return 0;
    }
...
SetActiveCommandByIndex(lua_State* L)
{
    ...
    const bool success = guihandler->SetActiveCommand(
...
{
    if (!CLuaHandle::CheckModUICtrl(L)) {
        // FIXME: why not put this in UnsyncedRead without the ModUICtrl
        // restriction?
        lua_pushnumber(L, 0);
        return 1;
    }
    ...
    const int value =
      configHandler->IsSet(name) ? configHandler->GetInt(name) : def;
...
LuaUnsyncedCtrl::SetConfigInt(lua_State* L)
{
    ...
    if (configHandler->IsReadOnly(name)) {
        LOG_L(
          L_ERROR, "tried to set readonly (int) %s = %d", name.c_str(), value);
        return 0;
    }
    configHandler->EnableWriting(globalConfig->luaWritableConfigFile);
...
LuaUnsyncedCtrl::GetConfigString(lua_State* L)
{
    ...
    SET_IN_OVERLAY_WARNING;
    const string value =
      configHandler->IsSet(name) ? configHandler->GetString(name) : def;
...
LuaUnsyncedCtrl::SetConfigString(lua_State* L)
{
    ...
    if (configHandler->IsReadOnly(name)) {
        LOG_L(L_ERROR,
              "tried to set readonly (string) %s = %s",
              name.c_str(),
              value.c_str());
        return 0;
    }
                ...
                bool isStart = false)
{
    if (springArgs.empty() && !isStart) {
        ...
        LOG("[%s] Spring \"%s\" should be reloading",
            __FUNCTION__,
            (Platform::GetProcessExecutableFile()).c_str());
    } else {
        const std::string springFullName = Platform::GetProcessExecutableFile();
        const std::string scriptFullName =
          dataDirLocater.GetWriteDirPath() + "script.txt";
        ...
        LOG("[%s] Spring \"%s\" should be restarting",
            __FUNCTION__,
            springFullName.c_str());
...
{
    if (ReloadOrRestart(luaL_checkstring(L, 1), luaL_checkstring(L, 2), true) !=
        0) {
        lua_pushboolean(L, false);
        return 1;
    }
...
LuaUnsyncedCtrl::SetWMIcon(lua_State* L)
{
    ...
    if (iconTexture.Load(iconFileName)) {
        ...
        luaL_error(
          L, "Failed to load image from file \"%s\"", iconFileName.c_str());
    }
...
LuaUnsyncedCtrl::SendLuaUIMsg(lua_State* L)
{
    ...
    try {
        clientNet->Send(CBaseNetProtocol::Get().SendLuaMsg(
          gu->myPlayerNum, LUA_HANDLE_ORDER_UI, modeNum, data));
...
LuaUnsyncedCtrl::SendLuaGaiaMsg(lua_State* L)
{
    ...
    try {
        clientNet->Send(CBaseNetProtocol::Get().SendLuaMsg(
          gu->myPlayerNum, LUA_HANDLE_ORDER_GAIA, 0, data));
    } catch (const netcode::PackPacketException& ex) {
...
LuaUnsyncedCtrl::SendLuaRulesMsg(lua_State* L)
{
    ...
    try {
        clientNet->Send(CBaseNetProtocol::Get().SendLuaMsg(
          gu->myPlayerNum, LUA_HANDLE_ORDER_RULES, 0, data));
    } catch (const netcode::PackPacketException& ex) {
...
LuaUnsyncedCtrl::SetShareLevel(lua_State* L)
{
    ...
    if (shareType == "metal") {
        ...
        LOG_L(
          L_WARNING, "SetShareLevel() unknown resource: %s", shareType.c_str());
    }
    return 0;
}
...
LuaUnsyncedCtrl::ShareResources(lua_State* L)
{
    ...
    if (type == "units") {
        ...
        clientNet->Send(CBaseNetProtocol::Get().SendShare(
          gu->myPlayerNum, teamID, 1, 0.0f, 0.0f));
        selectedUnitsHandler.ClearSelected();
    } else if (args >= 3) {
        const float amount = lua_tofloat(L, 3);
        if (type == "metal") {
            clientNet->Send(CBaseNetProtocol::Get().SendShare(
              gu->myPlayerNum, teamID, 0, amount, 0.0f));
        } else if (type == "energy") {
            clientNet->Send(CBaseNetProtocol::Get().SendShare(
              gu->myPlayerNum, teamID, 0, 0.0f, amount));
        }
    }
    return 0;
}
...
LuaUnsyncedCtrl::MarkerAddPoint(lua_State* L)
{
    ...
    const bool onlyLocal = (lua_isnumber(L, 5)) ? bool(luaL_optnumber(L, 5, 1))
...
LuaUnsyncedCtrl::MarkerAddLine(lua_State* L)
{
    ...
    const bool onlyLocal = (lua_isnumber(L, 7)) ? bool(luaL_optnumber(L, 7, 0))
                                                : luaL_optboolean(L, 7, false);
...
LuaUnsyncedCtrl::SetSunLighting(lua_State* L)
{
    ...
    for (lua_pushnil(L); lua_next(L, 1) != 0; lua_pop(L, 1)) {
        ...
        if (lua_isnumber(L, -1)) {
            if (sl.SetValue(HsiehHash(key.c_str(), key.size(), 0),
                            lua_tofloat(L, -1)))
                continue;
...
{
    DynamicSkyLight* dynSkyLight =
      dynamic_cast<DynamicSkyLight*>(sky->GetLight());

    if (dynSkyLight == NULL)
        return 0;

    const float4 sunDir(luaL_checkfloat(L, 1),
                        luaL_checkfloat(L, 2),
                        luaL_checkfloat(L, 3),
                        luaL_checkfloat(L, 4));
...
LuaUnsyncedCtrl::SetSunDirection(lua_State* L)
{
    ...
    dynSkyLight->SetLightDir(float3(
      luaL_checkfloat(L, 1), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3)));
    return 0;
}
    ...
    if (lua_isstring(L, 2)) {
        ...
        if (loglvlstr == "debug") {
            ...
            return luaL_error(L,
                              ...
                              "Spring.SetLogSectionFilterLevel(logsection, "
                              "loglevel)");
        }
    }
...
{
    const FeatureDef* fd =
      featureDefHandler->GetFeatureDefByID(luaL_checkint(L, 1));
