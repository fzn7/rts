...
ParseTexture(const S3DModel* model, LuaMatTexture& texUnit, char texNum)
{
    ...
    const CS3OTextureHandler::S3OTexMat* stex =
      texturehandlerS3O->GetTexture(texType);
...
ParseNamedSubTexture(LuaMatTexture& texUnit, const std::string& texName)
{
    ...
    if (prefixHash == prefixHashes[0]) {
        ...
        if ((sepIdx + 2) == texName.size() && std::isdigit(texName.back())) {
            texUnit.data =
              reinterpret_cast<const void*>(int(texName.back()) - int('0'));
        }
    ...
    if (prefixHash == prefixHashes[1] || prefixHash == prefixHashes[2]) {
        ...
        CInfoTexture* itex =
          infoTextureHandler->GetInfoTexture(texName.substr(sepIdx + 1));
                                  ...
                                  const std::string& image)
{
    ...
    if (image[0] == LuaTextures::prefix) {
            ...
            case LuaMatTexture::LUATEX_3DOTEXTURE: {
                if (image.size() == 5) {
                    ...
                    switch (image[6]) {
                        case '1': {
                            texUnit.data =
                              reinterpret_cast<const void*>(int(1));
                        } break;
                        case '2': {
                            texUnit.data =
                              reinterpret_cast<const void*>(int(2));
                        } break;
                        default: {
                            return false;
                        } break;
        ...
        const CNamedTextures::TexInfo* texInfo =
          CNamedTextures::GetInfo(image, true);
        ...
        if (texInfo != nullptr) {
            ...
            LOG_L(L_WARNING,
                  "Lua: Couldn't load texture named \"%s\"!",
                  image.c_str());
            return false;
        }
    }
...
#define udGeomBuff (unitDrawer->GetGeometryBuffer())
    ...
    switch (type) {
        ...
        case LUATEX_UNITTEXTURE1: {
            texID = texturehandlerS3O
                      ->GetTexture(*reinterpret_cast<const int*>(&data))
                      ->tex1;
        } break;
        ...
        case LUATEX_UNITBUILDPIC:
            if (unitDefHandler != nullptr) {
                texID = unitDefHandler->GetUnitDefImage(
                  reinterpret_cast<const UnitDef*>(data));
            }
            break;
        case LUATEX_UNITRADARICON: {
            texID = (reinterpret_cast<const UnitDef*>(data))
                      ->iconType->GetTextureID();
        } break;
        ...
        // map shading textures (TODO: diffuse? needs coors and
        // {G,S}etMapSquareTexture rework)
        ...
        case LUATEX_SSMF_SNORMALS: {
            if (readMap != nullptr) {
                texID =
                  readMap->GetTexture((LUATEX_SSMF_SNORMALS - LUATEX_SMF_GRASS),
        ...
        case LUATEX_INFOTEX_SUFFIX:
            if (infoTextureHandler != nullptr) {
                const CInfoTexture* cInfoTex =
                  static_cast<const CInfoTexture*>(data);
        ...
        case LUATEX_MAP_GBUFFER_NORM: {
            texID = gdGeomBuff->GetBufferTexture(
        ...
        case LUATEX_MODEL_GBUFFER_NORM: {
            texID = udGeomBuff->GetBufferTexture(
              GL::GeometryBuffer::ATTACHMENT_NORMTEX);
        } break;
...
LuaMatTexture::GetTextureTarget() const
{
    ...
    switch (type) {
        ...
        case LUATEX_LUATEXTURE: {
            texType =
              reinterpret_cast<const LuaTextures::Texture*>(data)->target;
        } break;
    ...
    if (texID != 0) {
        ...
        if (enable) {
            switch (texType) {
                ...
                case GL_TEXTURE_CUBE_MAP_ARB: { /*glEnable(texType);*/
                } break;
                default: {
                } break;
            }
        ...
        switch (texType) {
            ...
            case GL_TEXTURE_CUBE_MAP_ARB: { /*glDisable(texType);*/
            } break;
            default: {
            } break;
        }
...
LuaMatTexture::Unbind() const
{
    ...
    switch (GetTextureTarget()) {
        ...
        case GL_TEXTURE_CUBE_MAP_ARB: { /*glDisable(GL_TEXTURE_CUBE_MAP_ARB);*/
        } break;
        default: {
        } break;
    }
...
#define sqint2(x) int2(x, x)
    ...
    switch (type) {
        case LUATEX_NAMED: {
            const auto namedTex =
              reinterpret_cast<const CNamedTextures::TexInfo*>(data);
            return int2(namedTex->xsize, namedTex->ysize);
        } break;

        case LUATEX_LUATEXTURE: {
            const auto luaTex =
              reinterpret_cast<const LuaTextures::Texture*>(data);
            return int2(luaTex->xsize, luaTex->ysize);
        } break;

        case LUATEX_UNITTEXTURE1: {
            const auto stex = texturehandlerS3O->GetTexture(
              *reinterpret_cast<const int*>(&data));
        ...
        case LUATEX_3DOTEXTURE: {
            if (texturehandler3DO != nullptr) {
                return int2(texturehandler3DO->GetAtlasTexSizeX(),
                            texturehandler3DO->GetAtlasTexSizeY());
            }
        } break;
        ...
        case LUATEX_HEIGHTMAP: {
            if (heightMapTexture != nullptr) {
                return int2(heightMapTexture->GetSizeX(),
        ...
        case LUATEX_SSMF_SNORMALS: {
            if (readMap != nullptr) {
                return (readMap->GetTextureSize(
                  (LUATEX_SSMF_SNORMALS - LUATEX_SMF_GRASS),
                  *reinterpret_cast<const int*>(&data)));
            }
        } break;
        ...
        case LUATEX_MAP_GBUFFER_ZVAL: {
            if (readMap != nullptr) {
                return (gdGeomBuff->GetWantedSize(
                  readMap->GetGroundDrawer()->DrawDeferred()));
            }
        } break;
        ...
        case LUATEX_FONTSMALL:
            return int2(smallFont->GetTextureWidth(),
                        smallFont->GetTextureHeight());
