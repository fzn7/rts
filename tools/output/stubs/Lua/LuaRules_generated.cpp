...
bool
CLuaRules::AddSyncedCode(lua_State* L)
{
...
bool
CLuaRules::AddUnsyncedCode(lua_State* L)
{
...
int
CLuaRules::UnpackCobArg(lua_State* L)
{
...
void
CLuaRules::Cob2Lua(const LuaHashString& name,
                   const CUnit* unit,
                   int& argsCount,
                   int args[MAX_LUA_COB_ARGS])
{
    static int callDepth = 0;
    if (callDepth >= 16) {
        LOG_L(L_WARNING,
              "CLuaRules::Cob2Lua() call overflow: %s",
    ...
    if (!lua_checkstack(L, 1 + 3 + argsCount)) {
        LOG_L(L_WARNING,
              "CLuaRules::Cob2Lua() lua_checkstack() error: %s",
              name.GetString().c_str());
        args[0] = 0; // failure
        lua_settop(L, top);
        return;
    ...
    if (!name.GetGlobalFunc(L)) {
        LOG_L(L_WARNING,
              "CLuaRules::Cob2Lua() missing function: %s",
              name.GetString().c_str());
        args[0] = 0; // failure
        lua_settop(L, top);
        return;
    ...
    const bool error =
      !syncedLuaHandle.RunCallIn(L, name, 3 + argsCount, LUA_MULTRET);
...
int
CLuaRules::PermitHelperAIs(lua_State* L)
{
