...
CUnsyncedLuaHandle::CUnsyncedLuaHandle(CLuaHandleSynced* _base,
                                       const string& _name,
                                       int _order)
...
bool
CUnsyncedLuaHandle::Init(const string& code, const string& file)
{
    if (!IsValid())
        return false;
...
void
CUnsyncedLuaHandle::RecvFromSynced(lua_State* srcState, int args)
{
    if (!IsValid())
        return;
...
bool
CUnsyncedLuaHandle::DrawUnit(const CUnit* unit)
{
...
bool
CUnsyncedLuaHandle::DrawShield(const CUnit* unit, const CWeapon* weapon)
{
    LUA_CALL_IN_CHECK(L, false);
...
bool
CUnsyncedLuaHandle::DrawProjectile(const CProjectile* projectile)
{
...
CSyncedLuaHandle::CSyncedLuaHandle(CLuaHandleSynced* _base,
                                   const string& _name,
                                   int _order)
...
bool
CSyncedLuaHandle::Init(const string& code, const string& file)
{
    if (!IsValid())
        return false;
    ...
    // load our libraries  (LuaSyncedCtrl overrides some LuaUnsyncedCtrl
    // entries)
...
bool
CSyncedLuaHandle::SyncedActionFallback(const string& msg, int playerID)
{
    string cmd = msg;
...
bool
CSyncedLuaHandle::CommandFallback(const CUnit* unit, const Command& cmd)
{
...
bool
CSyncedLuaHandle::AllowCommand(const CUnit* unit,
                               const Command& cmd,
                               bool fromSynced)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowUnitCreation(const UnitDef* unitDef,
                                    const CUnit* builder,
...
bool
CSyncedLuaHandle::AllowUnitTransfer(const CUnit* unit,
                                    int newTeam,
                                    bool capture)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowUnitBuildStep(const CUnit* builder,
                                     const CUnit* unit,
                                     float part)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowFeatureCreation(const FeatureDef* featureDef,
                                       int teamID,
                                       const float3& pos)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowFeatureBuildStep(const CUnit* builder,
...
bool
CSyncedLuaHandle::AllowResourceLevel(int teamID,
                                     const string& type,
                                     float level)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowResourceTransfer(int oldTeam,
                                        int newTeam,
                                        const string& type,
                                        float amount)
{
...
bool
CSyncedLuaHandle::AllowDirectUnitControl(int playerID, const CUnit* unit)
{
    LUA_CALL_IN_CHECK(L, true);
    luaL_checkstack(L, 6, __func__);
...
bool
CSyncedLuaHandle::AllowBuilderHoldFire(const CUnit* unit, int action)
{
    LUA_CALL_IN_CHECK(L, true);
...
bool
CSyncedLuaHandle::AllowStartPosition(int playerID,
...
bool
CSyncedLuaHandle::MoveCtrlNotify(const CUnit* unit, int data)
{
    LUA_CALL_IN_CHECK(L, false);
    luaL_checkstack(L, 6, __func__);
...
bool
CSyncedLuaHandle::TerraformComplete(const CUnit* unit, const CUnit* build)
{
    LUA_CALL_IN_CHECK(L, false);
 ...
 */
bool
CSyncedLuaHandle::UnitPreDamaged(const CUnit* unit,
                                 ...
                                 float* impulseMult)
{
    ...
    if (!RunCallInTraceback(
          L, cmdStr, inArgCount, outArgCount, traceBack.GetErrFuncIdx(), false))
        return false;
    ...
    if (lua_isnumber(L, -2)) {
        *newDamage = lua_tonumber(L, -2);
        ...
        LOG_L(L_WARNING,
              "%s(): 1st return-value should be a number (newDamage)",
    ...
    if (lua_isnumber(L, -1)) {
        *impulseMult = lua_tonumber(L, -1);
        ...
        LOG_L(L_WARNING,
              "%s(): 2nd return-value should be a number (impulseMult)",
              (cmdStr.GetString()).c_str());
    }

    lua_pop(L, outArgCount);
    return (*newDamage == 0.f &&
            *impulseMult == 0.f); // returns true to disable engine dmg handling
}

bool
CSyncedLuaHandle::FeaturePreDamaged(const CFeature* feature,
                                    const CUnit* attacker,
                                    float damage,
                                    int weaponDefID,
                                    int projectileID,
...
bool
CSyncedLuaHandle::ShieldPreDamaged(const CProjectile* projectile,
                                   ...
                                   const CUnit* beamCarrier)
{
    assert((projectile != nullptr) ||
           ((beamEmitter != nullptr) && (beamCarrier != nullptr)));
    LUA_CALL_IN_CHECK(L, false);
    ...
    // call the routine
    if (!RunCallInTraceback(
          L, cmdStr, numArgs, 1, traceBack.GetErrFuncIdx(), false))
        return false;
...
int
CSyncedLuaHandle::AllowWeaponTargetCheck(unsigned int attackerID,
...
bool
CSyncedLuaHandle::AllowWeaponTarget(unsigned int attackerID,
...
int
CSyncedLuaHandle::SyncedRandom(lua_State* L)
{
    ...
    if (args == 0) {
        ...
        if (maxn < 1) {
            luaL_error(L,
                       "error: too small upper limit (%d) given to "
                       "math.random(), should be >= 1 {synced}",
                       maxn);
        }
...
int
CSyncedLuaHandle::SyncedRandomSeed(lua_State* L)
{
...
int
CSyncedLuaHandle::SyncedNext(lua_State* L)
{
    constexpr int whiteList[] = {
        ...
        LUA_TBOOLEAN,
        LUA_TNIL,
        LUA_TTHREAD // FIXME LUA_TTHREAD is normally _not_ synced safe but LUS
                    // handler needs it atm (and uses it in a safe way)
    };
    ...
    if (retCount >= 2) {
        ...
        const auto it =
          std::find(std::begin(whiteList), std::end(whiteList), keyType);

        if (it == std::end(whiteList)) {
            if (LuaUtils::PushDebugTraceback(L) > 0) {
                lua_pushfstring(L,
                                "Iterating a table with keys of type \"%s\" in "
                                "synced context!",
...
int
CSyncedLuaHandle::SyncedPairs(lua_State* L)
{
...
int
CSyncedLuaHandle::SendToUnsynced(lua_State* L)
{
    const int args = lua_gettop(L);
    if (args <= 0) {
    ...
    static const int supportedTypes = (1 << LUA_TNIL) | (1 << LUA_TBOOLEAN) |
    ...
    for (int i = 1; i <= args; i++) {
        ...
        if (!(t & supportedTypes)) {
            luaL_error(
              L, "Incorrect data type for SendToUnsynced(), arg %d", i);
        }
    }
...
int
CSyncedLuaHandle::AddSyncedActionFallback(lua_State* L)
{
...
int
CSyncedLuaHandle::RemoveSyncedActionFallback(lua_State* L)
{
...
GetWatchDef(Unit) GetWatchDef(Feature) GetWatchDef(Weapon)

  SetWatchDef(Unit) SetWatchDef(Feature) SetWatchDef(Weapon)
...
void
CLuaHandleSynced::Init(const string& syncedFile,
                       ...
                       const string& modes)
{
    ...
    const bool haveUnsynced =
      unsyncedLuaHandle.Init(unsyncedCode, unsyncedFile);
...
string
CLuaHandleSynced::LoadFile(const string& filename, const string& modes) const
{
...
int
CLuaHandleSynced::LoadStringData(lua_State* L)
{
    size_t len;
...
int
CLuaHandleSynced::CallAsTeam(lua_State* L)
{
    const int args = lua_gettop(L);
    ...
    if (lua_isnumber(L, 1)) {
        ...
        if ((teamID < CEventClient::MinSpecialTeam) ||
            (teamID >= teamHandler->ActiveTeams())) {
            luaL_error(L, "Bad teamID in SetCtrlTeam");
        }
        // ctrl
        CLuaHandle::SetHandleCtrlTeam(L, teamID);
        CLuaHandle::SetHandleFullCtrl(
          L, prevCtrlTeam == CEventClient::AllAccessTeam);
        // read
        CLuaHandle::SetHandleReadTeam(L, teamID);
        CLuaHandle::SetHandleReadAllyTeam(
          L, (teamID < 0) ? teamID : teamHandler->AllyTeam(teamID));
        CLuaHandle::SetHandleFullRead(
          L, prevReadAllyTeam == CEventClient::AllAccessTeam);
        // select
