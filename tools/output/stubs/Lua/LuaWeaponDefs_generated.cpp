...
LuaWeaponDefs::PushEntries(lua_State* L)
{
    ...
    const std::array<const IndxFuncType, 3> indxFuncs = { WeaponDefIndex,
                                                          WeaponDefNewIndex,
                                                          WeaponDefMetatable };
...
WeaponDefIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in WeaponDefs __index",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
WeaponDefNewIndex(lua_State* L)
{
    ...
    switch (elem.type) {
        ...
        case ERROR_TYPE: {
            LOG_L(L_ERROR,
                  "[%s] ERROR_TYPE for key \"%s\" in WeaponDefs __newindex",
                  __FUNCTION__,
                  name);
            lua_pushnil(L);
            return 1;
...
InitParamMap()
{
    ...
    ADD_FLOAT("cylinderTargetting",
              wd.cylinderTargeting); // FIXME deprecated misspelling
