...
LuaUtils::CopyData(lua_State* dst, lua_State* src, int count)
{
    ...
    if (srcTop < count) {
        LOG_L(L_ERROR,
              "LuaUtils::CopyData: tried to copy more data than there is");
        return 0;
    }
    lua_checkstack(dst, count + 3); // +3 needed for table copying
    lua_lock(
      src); // we need to be sure tables aren't changed while we iterate them
...
BackupTable(LuaUtils::DataDump& d, lua_State* src, int index, int depth)
{
    ...
    for (lua_pushnil(src); lua_next(src, table) != 0; lua_pop(src, 1)) {
        ...
        d.table.push_back(
          std::pair<LuaUtils::DataDump, LuaUtils::DataDump>(dk, dv));
    }
...
RestoreTable(const LuaUtils::DataDump& d, lua_State* dst, int depth)
{
    ...
    lua_newtable(dst);
    for (std::vector<std::pair<LuaUtils::DataDump, LuaUtils::DataDump>>::
           const_iterator i = d.table.begin();
         i != d.table.end();
         ++i) {
...
LuaUtils::Restore(const std::vector<LuaUtils::DataDump>& backup, lua_State* dst)
{
    ...
    for (std::vector<DataDump>::const_iterator i = backup.begin();
         i != backup.end();
         ++i) {
        RestoreData(*i, dst, 0);
    }
...
CheckForNaNsReal(lua_State* L, const std::string& path)
{
    ...
    for (lua_pushnil(L); lua_next(L, table) != 0; lua_pop(L, 1)) {
        if (lua_istable(L, -1)) {
            // We can't work on -2 directly cause lua_tostring would replace the
            // value in -2, so we need to make a copy and convert that to a
            // string.
            lua_pushvalue(L, -2);
            ...
            if (math::isinf(value) || math::isnan(value)) {
                // We can't work on -2 directly cause lua_tostring would replace
                // the value in -2, so we need to make a copy and convert that
                // to a string.
                lua_pushvalue(L, -2);
                const char* key = lua_tostring(L, -1);
                LOG_L(
                  L_WARNING, "%s%s: Got Invalid NaN/Inf!", path.c_str(), key);
                lua_pop(L, 1);
                foundNaNs = true;
            }
        }
...
LuaUtils::PrintStack(lua_State* L)
{
    ...
    for (int i = 1; i <= top; i++) {
        ...
        switch (type) {
            ...
            case LUA_TBOOLEAN:
                LOG_L(
                  L_ERROR, "\t\t%s", lua_toboolean(L, i) ? "true" : "false");
                break;
            default: {
            }
        }
...
LuaUtils::PushModelTable(lua_State* L, const SolidObjectDef* def)
{
    ...
    const std::string& modelType =
      StringToLower(FileSystem::GetExtension(modelPath));
...
LuaUtils::ParseCommand(lua_State* L, const char* caller, int idIndex)
{
    ...
    for (lua_pushnil(L); lua_next(L, paramTableIdx) != 0; lua_pop(L, 1)) {
        if (lua_israwnumber(L, -2)) { // avoid 'n'
            if (!lua_isnumber(L, -1)) {
                luaL_error(L,
                           "%s(): expected <number idx=%d, number value> in "
                           "params-table",
                           caller,
                           lua_tonumber(L, -2));
            }
...
LuaUtils::Next(const ParamMap& paramMap, lua_State* L)
{
    ...
    if (lua_isstring(L, 2)) {
        ...
        if ((it != paramMap.end()) && (it->second.type != READONLY_TYPE)) {
            ...
            while (
              (it != paramMap.end()) &&
              (it->second.type == READONLY_TYPE || it->second.deprecated)) {
...
LuaUtils::Log(lua_State* L)
{
    ...
    if (args < 3)
        return luaL_error(
          L, "Incorrect arguments to Spring.Log(logsection, loglevel, ...)");
    ...
    if (lua_israwnumber(L, 2)) {
        ...
        if (loglvlstr == "debug") {
            ...
            return luaL_error(
              L,
              "Incorrect arguments to Spring.Log(logsection, loglevel, ...)");
        }
    } else {
        return luaL_error(
          L, "Incorrect arguments to Spring.Log(logsection, loglevel, ...)");
...
{
    const int curTop =
      lua_gettop(luaState); // use var so you can print it in gdb
