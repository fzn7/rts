...
bool
LuaSyncedRead::PushEntries(lua_State* L)
{
...
IsUnitVisible(lua_State* L, const CUnit* unit)
{
    ...
    return (unit->losStatus[CLuaHandle::GetHandleReadAllyTeam(L)] &
            (LOS_INLOS | LOS_INRADAR));
}
...
IsUnitTyped(lua_State* L, const CUnit* unit)
{
    ...
    const unsigned short losStatus =
      unit->losStatus[CLuaHandle::GetHandleReadAllyTeam(L)];
...
GetSolidObjectPosition(lua_State* L, const CSolidObject* o, bool isFeature)
{
    ...
    if (!isFeature && !IsAllyUnit(L, static_cast<const CUnit*>(o)))
        errorVec = static_cast<const CUnit*>(o)->GetLuaErrorVector(
          CLuaHandle::GetHandleReadAllyTeam(L),
          CLuaHandle::GetHandleFullRead(L));
...
GetSolidObjectRotation(lua_State* L, const CSolidObject* o)
{
    ...
    const CMatrix44f& matrix =
      o->GetTransformMatrix(CLuaHandle::GetHandleSynced(L));
...
GetSolidObjectBlocking(lua_State* L, const CSolidObject* o)
{
    ...
    lua_pushboolean(L,
                    o->HasPhysicalStateBit(CSolidObject::PSTATE_BIT_BLOCKING));
    lua_pushboolean(
      L, o->HasCollidableStateBit(CSolidObject::CSTATE_BIT_SOLIDOBJECTS));
    lua_pushboolean(
      L, o->HasCollidableStateBit(CSolidObject::CSTATE_BIT_PROJECTILES));
    lua_pushboolean(
      L, o->HasCollidableStateBit(CSolidObject::CSTATE_BIT_QUADMAPRAYS));
...
LuaSyncedRead::GetAllyTeamStartBox(lua_State* L)
{
    ...
    const float xmin =
      (mapDims.mapx * SQUARE_SIZE) * allyData[allyTeam].startRectLeft;
    const float zmin =
      (mapDims.mapy * SQUARE_SIZE) * allyData[allyTeam].startRectTop;
    const float xmax =
      (mapDims.mapx * SQUARE_SIZE) * allyData[allyTeam].startRectRight;
    const float zmax =
      (mapDims.mapy * SQUARE_SIZE) * allyData[allyTeam].startRectBottom;
...
LuaSyncedRead::GetTeamInfo(lua_State* L)
{
    ...
    lua_pushboolean(
      L, !skirmishAIHandler.GetSkirmishAIsInTeam(teamID).empty()); // hasAIs
    ...
    for (TeamBase::customOpts::const_iterator it = popts.begin();
         it != popts.end();
         ++it) {
        lua_pushsstring(L, it->first);
        lua_pushsstring(L, it->second);
...
LuaSyncedRead::GetTeamRulesParams(lua_State* L)
{
    ...
    } else if (teamHandler->AlliedTeams(team->teamNum,
                                        CLuaHandle::GetHandleReadTeam(L)) ||
        ...
        for (int i = start; i <= end; ++i, ++it) {
            ...
            lua_newtable(L);
            {
                if (i + 1 == teamStats.size()) {
                    // the `stats.frame` var indicates the frame when a new
                    // entry needs to get added, for the most recent stats entry
                    // this lies obviously in the future, so we just output the
                    // current frame here
                    HSTR_PUSH_NUMBER(
                      L, "time", gs->GetLuaSimFrame() / GAME_SPEED);
...
LuaSyncedRead::GetTeamLuaAI(lua_State* L)
{
    ...
    CSkirmishAIHandler::ids_t saids =
      skirmishAIHandler.GetSkirmishAIsInTeam(team->teamNum);
    for (CSkirmishAIHandler::ids_t::const_iterator ai = saids.begin();
         ai != saids.end();
         ++ai) {
...
LuaSyncedRead::GetPlayerInfo(lua_State* L)
{
    ...
    for (PlayerBase::customOpts::const_iterator it = popts.begin();
         it != popts.end();
         ++it) {
        lua_pushsstring(L, it->first);
        lua_pushsstring(L, it->second);
...
LuaSyncedRead::GetPlayerControlledUnit(lua_State* L)
{
    ...
    if ((CLuaHandle::GetHandleReadAllyTeam(L) == CEventClient::NoAccessTeam) ||
        ((CLuaHandle::GetHandleReadAllyTeam(L) >= 0) &&
         !teamHandler->Ally(unit->allyteam,
                            CLuaHandle::GetHandleReadAllyTeam(L)))) {
        return 0;
    }
...
LuaSyncedRead::GetAIInfo(lua_State* L)
{
    ...
    CSkirmishAIHandler::ids_t saids =
      skirmishAIHandler.GetSkirmishAIsInTeam(teamId);
    if (saids.empty()) {
        return numVals;
    }
...
LuaSyncedRead::GetAllyTeamInfo(lua_State* L)
{
    ...
    for (AllyTeam::customOpts::const_iterator it = popts.begin();
         it != popts.end();
         ++it) {
        lua_pushsstring(L, it->first);
        lua_pushsstring(L, it->second);
...
static inline bool
UnitInPlanes(const CUnit* unit, const vector<Plane>& planes)
{
...
#define PLANES_TEST                                                            \
    ...
    for (int team = startTeam; team <= endTeam; team++) {
        ...
        if (allegiance >= 0) {
            ...
            if (CLuaHandle::GetHandleReadAllyTeam(L) ==
                teamHandler->AllyTeam(team)) {
                LOOP_UNIT_CONTAINER(NULL_TEST, PLANES_TEST, false);
            }
        } else if (allegiance == EnemyUnits) {
            if (CLuaHandle::GetHandleReadAllyTeam(L) !=
...
LuaSyncedRead::GetUnitNearestAlly(lua_State* L)
{
    ...
    const CUnit* target = CGameHelper::GetClosestFriendlyUnit(
      unit, unit->pos, range, unit->allyteam);
...
LuaSyncedRead::GetUnitNearestEnemy(lua_State* L)
{
    ...
    const float range = luaL_optnumber(L, 2, 1.0e9f);
    const bool useLos = !CLuaHandle::GetHandleFullRead(L) ||
                        !lua_isboolean(L, 3) || lua_toboolean(L, 3);
    const CUnit* target = NULL;

    if (useLos) {
        target = CGameHelper::GetClosestEnemyUnit(
          unit, unit->pos, range, unit->allyteam);
    } else {
        target = CGameHelper::GetClosestEnemyUnitNoLosTest(
          unit, unit->pos, range, unit->allyteam, false, true);
    }
...
LuaSyncedRead::GetFeaturesInRectangle(lua_State* L)
{
    ...
    const vector<CFeature*>& rectFeatures =
      quadField->GetFeaturesExact(mins, maxs);
...
LuaSyncedRead::GetFeaturesInSphere(lua_State* L)
{
    ...
    const vector<CFeature*>& sphFeatures =
      quadField->GetFeaturesExact(pos, rad, true);
...
LuaSyncedRead::GetFeaturesInCylinder(lua_State* L)
{
    ...
    const vector<CFeature*>& cylFeatures =
      quadField->GetFeaturesExact(pos, rad, false);
...
LuaSyncedRead::GetProjectilesInRectangle(lua_State* L)
{
    ...
    const vector<CProjectile*>& rectProjectiles =
      quadField->GetProjectilesExact(mins, maxs);
...
{
    lua_pushboolean(
      L, lua_isnumber(L, 1) && ParseUnit(L, __FUNCTION__, 1) != nullptr);
    return 1;
}
...
LuaSyncedRead::GetUnitStates(lua_State* L)
{
    ...
    if (mt != nullptr) {
        const CHoverAirMoveType* hAMT =
          dynamic_cast<const CHoverAirMoveType*>(mt);
        if (hAMT != nullptr) {
            HSTR_PUSH_BOOL(L, "autoland", hAMT->autoLand);
        } else {
            const CStrafeAirMoveType* sAMT =
              dynamic_cast<const CStrafeAirMoveType*>(mt);
            if (sAMT != nullptr) {
...
LuaSyncedRead::GetUnitTooltip(lua_State* L)
{
    ...
    if (effectiveDef->showPlayerName) {
        ...
        if (unitTeam != NULL && unitTeam->HasLeader()) {
            ...
            CSkirmishAIHandler::ids_t teamAIs =
              skirmishAIHandler.GetSkirmishAIsInTeam(unit->team);
...
LuaSyncedRead::GetUnitFlanking(lua_State* L)
{
    ...
    if (lua_israwstring(L, 2)) {
        ...
        if (key == "mode") {
            ...
            lua_pushnumber(
              L, unit->flankingBonusAvgDamage - unit->flankingBonusDifDamage);
            return 1;
        ...
        lua_pushnumber(L, unit->flankingBonusMobilityAdd);
        lua_pushnumber(L,
                       unit->flankingBonusAvgDamage - // min
                         unit->flankingBonusDifDamage);
        lua_pushnumber(L,
                       unit->flankingBonusAvgDamage + // max
                         unit->flankingBonusDifDamage);
        lua_pushnumber(L, unit->flankingBonusDir.x);
        lua_pushnumber(L, unit->flankingBonusDir.y);
        lua_pushnumber(L, unit->flankingBonusDir.z);
...
LuaSyncedRead::GetUnitWeaponState(lua_State* L)
{
    ...
    if (key.empty()) { // backwards compatible
        ...
        lua_pushnumber(L,
                       (weapon->reloadTime / unit->reloadSpeed) / GAME_SPEED);
...
LuaSyncedRead::GetUnitWeaponVectors(lua_State* L)
{
    ...
    if ((type == "TorpedoLauncher") || (type == "MissileLauncher") ||
...
LuaSyncedRead::GetUnitWeaponTryTarget(lua_State* L)
{
    ...
    // by passing a position thatis not approximately the wanted checked
    // position, because
    // the checks for target using passed position for checking both free line
    // of fire and range  which would result in wrong test unless target was by
    // chance near coords <0,0,0>  while position alone works because NULL target
    // omits target class validity checks
...
LuaSyncedRead::GetUnitWeaponHaveFreeLineOfFire(lua_State* L)
{
    ...
    lua_pushboolean(
      L, weapon->HaveFreeLineOfFire(pos, SWeaponTarget(enemy, pos, true)));
    return 1;
}
...
LuaSyncedRead::GetUnitWeaponCanFire(lua_State* L)
{
    ...
    lua_pushboolean(L,
                    unit->weapons[weaponNum]->CanFire(
                      ignoreAngleGood, ignoreTargetType, ignoreRequestedDir));
    return 1;
}
...
int
LuaSyncedRead::GetUnitEstimatedPath(lua_State* L)
{
    CUnit* unit = ParseAllyUnit(L, __FUNCTION__, 1);
    if (unit == NULL) {
        return 0;
...
LuaSyncedRead::GetUnitLastAttacker(lua_State* L)
{
    ...
    if ((unit->lastAttacker == NULL) || !IsUnitVisible(L, unit->lastAttacker)) {
        return 0;
    }
...
{
    return (
      PushPieceCollisionVolumeData(L, ParseInLosUnit(L, __FUNCTION__, 1)));
}
...
LuaSyncedRead::GetUnitSeparation(lua_State* L)
{
    ...
    if (!IsAllyUnit(L, unit1))
        pos1 = unit1->GetLuaErrorPos(CLuaHandle::GetHandleReadAllyTeam(L),
                                     CLuaHandle::GetHandleFullRead(L));
    if (!IsAllyUnit(L, unit2))
        pos2 = unit2->GetLuaErrorPos(CLuaHandle::GetHandleReadAllyTeam(L),
                                     CLuaHandle::GetHandleFullRead(L));
...
#else
    const float dist = (luaL_optboolean(L, 3, false)) ? pos1.distance2D(pos2)
                                                      : pos1.distance(pos2);
#endif
...
LuaSyncedRead::GetUnitFeatureSeparation(lua_State* L)
{
    ...
    if (!IsAllyUnit(L, unit))
        pos1 = unit->GetLuaErrorPos(CLuaHandle::GetHandleReadAllyTeam(L),
                                    CLuaHandle::GetHandleFullRead(L));
...
#else
    lua_pushnumber(L,
                   (luaL_optboolean(L, 3, false)) ? pos1.distance2D(pos2)
                                                  : pos1.distance(pos2));
#endif
    return 1;
}
...
LuaSyncedRead::GetUnitMoveTypeData(lua_State* L)
{
    ...
    HSTR_PUSH_NUMBER(
      L, "maxWantedSpeed", amt->GetMaxWantedSpeed() * GAME_SPEED);
    ...
    const CGroundMoveType* groundmt =
      dynamic_cast<CGroundMoveType*>(unit->moveType);
    ...
    if (groundmt != NULL) {
        ...
        HSTR_PUSH_NUMBER(
          L, "maxReverseSpeed", groundmt->GetMaxReverseSpeed() * GAME_SPEED);
        HSTR_PUSH_NUMBER(
          L, "wantedSpeed", groundmt->GetWantedSpeed() * GAME_SPEED);
        HSTR_PUSH_NUMBER(
          L, "currentSpeed", groundmt->GetCurrentSpeed() * GAME_SPEED);
    ...
    const CHoverAirMoveType* hAMT =
      dynamic_cast<CHoverAirMoveType*>(unit->moveType);
    ...
    const CStrafeAirMoveType* sAMT =
      dynamic_cast<CStrafeAirMoveType*>(unit->moveType);
    ...
    const CStaticMoveType* staticmt =
      dynamic_cast<CStaticMoveType*>(unit->moveType);
    ...
    const CScriptMoveType* scriptmt =
      dynamic_cast<CScriptMoveType*>(unit->moveType);
...
LuaSyncedRead::GetUnitCommands(lua_State* L)
{
    ...
    const CCommandQueue* queue = (factoryCAI == NULL)
                                   ? &commandAI->commandQue
                                   : &factoryCAI->newUnitCommands;
    ...
    if (lua_isnoneornil(L, 2)) {
        ...
        if (spring_gettime() >= nextWarning) {
            ...
            if (calls > 1000) {
                luaL_error(L,
                           "[%s] called too often without a 2nd argument to "
                           "define maxNumCmds returned in the table, please "
                           "check your code!\n"
                           "Especially when you only read the first cmd or "
                           "want to check if the queue is non-empty, this can "
                           "be a huge performance leak!\n",
                           __FUNCTION__);
            }
            calls = 0;
        }
    ...
    const bool cmdsTable =
      luaL_optboolean(L, 3, true); // deprecated, prefer to set 2nd arg to 0
    ...
    for (it = commandQue.begin(); it != commandQue.end(); ++it) {
        if (it->GetID() < 0) { // a build command
            ...
            if (canBuild) {
                ...
                const UnitDef* order_ud =
                  unitDefHandler->GetUnitDefByID(unitDefID);
...
LuaSyncedRead::GetUnitRulesParams(lua_State* L)
{
    ...
    if (IsAllyUnit(L, unit) || game->IsGameOver()) {
        losMask |= LuaRulesParams::RULESPARAMLOS_PRIVATE_MASK;
    } else if (teamHandler->AlliedTeams(unit->team,
                                        CLuaHandle::GetHandleReadTeam(L)) ||
               ((CLuaHandle::GetHandleReadAllyTeam(L) < 0) &&
                CLuaHandle::GetHandleFullRead(L))) {
        losMask |= LuaRulesParams::RULESPARAMLOS_ALLIED_MASK;
        ...
        //! NoAccessTeam
    } else if (unit->losStatus[CLuaHandle::GetHandleReadAllyTeam(L)] &
               LOS_INLOS) {
...
LuaSyncedRead::GetUnitCmdDescs(lua_State* L)
{
    ...
    const std::vector<const SCommandDescription*>& cmdDescs =
      unit->commandAI->GetPossibleCommands();
...
LuaSyncedRead::GetFeatureRulesParams(lua_State* L)
{
    ...
    if (IsAlliedAllyTeam(L, feature->allyteam) || game->IsGameOver()) {
        losMask |= LuaRulesParams::RULESPARAMLOS_PRIVATE_MASK;
    } else if (teamHandler->AlliedTeams(feature->team,
                                        CLuaHandle::GetHandleReadTeam(L)) ||
               ((CLuaHandle::GetHandleReadAllyTeam(L) < 0) &&
                CLuaHandle::GetHandleFullRead(L))) {
        losMask |= LuaRulesParams::RULESPARAMLOS_ALLIED_MASK;
...
int
LuaSyncedRead::GetProjectileSpinSpeed(lua_State* L)
{
    lua_pushnumber(L, 0.0f);
    return 1;
} // DEPRECATED
int
LuaSyncedRead::GetProjectileSpinVec(lua_State* L)
{
    lua_pushnumber(L, 0.0f);
    lua_pushnumber(L, 0.0f);
    lua_pushnumber(L, 0.0f);
...
int
LuaSyncedRead::GetPieceProjectileParams(lua_State* L)
{
    const CProjectile* pro = ParseProjectile(L, __FUNCTION__, 1);
...
LuaSyncedRead::GetProjectileName(lua_State* L)
{
    ...
    if (pro->weapon) {
        const CWeaponProjectile* wpro =
          static_cast<const CWeaponProjectile*>(pro);
    ...
    if (pro->piece) {
        const CPieceProjectile* ppro =
          static_cast<const CPieceProjectile*>(pro);
    ...
    // neither weapon nor piece likely means the projectile is CExpGenSpawner,
    // should we return any name in this case?
    return 0;
}
...
LuaSyncedRead::GetGroundHeight(lua_State* L)
{
    ...
    const float z = luaL_checkfloat(L, 2);
    lua_pushnumber(
      L, CGround::GetHeightReal(x, z, CLuaHandle::GetHandleSynced(L)));
    return 1;
}
...
LuaSyncedRead::GetGroundBlocked(lua_State* L)
{
    ...
    for (int z = tz1; z <= tz2; z++) {
        for (int x = tx1; x <= tx2; x++) {
            const CSolidObject* s =
              groundBlockingObjectMap->GroundBlocked(x, z);
...
LuaSyncedRead::TestMoveOrder(lua_State* L)
{
    ...
    const MoveDef* moveDef =
      moveDefHandler->GetMoveDefByPathType(unitDef->pathType);
    ...
    if (los) {
        ret = moveDef->TestMoveSquare(
          NULL, pos, dir, testTerrain, testObjects, centerOnly);
    }
...
LuaSyncedRead::TestBuildOrder(lua_State* L)
{
    ...
    // 3 = BUILDSQUARE_OPEN
    int retval =
      CGameHelper::TestUnitBuildSquare(bi,
                                       feature,
                                       CLuaHandle::GetHandleReadAllyTeam(L),
                                       CLuaHandle::GetHandleSynced(L));

    // output of TestUnitBuildSquare was changed after this lua function was
    // writen keep backward-compability by mapping: BUILDSQUARE_OPEN =
...
LuaSyncedRead::Pos2BuildPos(lua_State* L)
{
    ...
    const float3 buildPos =
      CGameHelper::Pos2BuildPos(buildInfo, CLuaHandle::GetHandleSynced(L));
...
{
    const float3 pos(
      luaL_checkfloat(L, 1), luaL_checkfloat(L, 2), luaL_checkfloat(L, 3));
...
LuaSyncedRead::GetUnitLosState(lua_State* L)
{
    ...
    if (allyTeamID < 0) {
        losStatus =
          (allyTeamID == CEventClient::AllAccessTeam) ? LOS_ALL_MASK_BITS : 0;
    } else {
    ...
    if ((losStatus & LOS_INLOS) || ((losStatus & prevMask) == prevMask)) {
...
GetSolidObjectPieceInfoHelper(lua_State* L, const S3DModelPiece& op)
{
    ...
    HSTR_PUSH_STRING(
      L,
      "parent",
      ((op.parent != nullptr) ? op.parent->name.c_str() : "[null]"));
...
{
    return (
      GetSolidObjectPieceDirection(L, ParseTypedUnit(L, __FUNCTION__, 1)));
}
int
