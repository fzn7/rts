...
CDemoRecorder::SetFileHeader()
{
    ...
    STRNCPY(fileHeader.versionString,
            (SpringVersion::GetSync()).c_str(),
            sizeof(fileHeader.versionString) - 1);
...
{
    // using operator<<(basic_stringbuf*) requires the stream to be opened with
    // std::ios::in stringbuf::{eback(), egptr(), gptr()} are protected so we
    // cannot access them directly (plus data is not guaranteed to be stored
    // contiguously) ==> the only clean OO solution that avoids str()'s copy
    // would be to supply our own stringbuffer backend to demoStream which is
...
CDemoRecorder::SetName(const std::string& mapName,
                       ...
                       bool serverDemo)
{
    // Returns the current local time as "JJJJMMDD_HHmmSS", eg:
    // "20091231_115959"
...
CDemoRecorder::SetTeamStats(int teamNum, const std::list<TeamStatistics>& stats)
{
    ...
    for (std::list<TeamStatistics>::const_iterator it = stats.begin();
         it != stats.end();
         ++it)
...
{
    fileHeader.winningAllyTeamsSize =
      (winningAllyTeamIDs.size() * sizeof(unsigned char));
...
{
#ifdef _MSC_VER // MSVC8 behaves strange if tell/seek is called before anything
                // has been written
...
CDemoRecorder::WritePlayerStats()
{
    ...
    for (PlayerStatistics& stats : playerStats) {
        stats.swab();
        demoStream.write(reinterpret_cast<char*>(&stats),
                         sizeof(PlayerStatistics));
    }
...
CDemoRecorder::WriteWinnerList()
{
    ...
    for (std::vector<unsigned char>::const_iterator it =
           winningAllyTeams.begin();
         it != winningAllyTeams.end();
         ++it) {
