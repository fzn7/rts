  ...
  : playbackDemo(NULL)
{
    ...
    // each build.)
#ifndef _DEBUG
        || (SpringVersion::IsRelease() &&
            strcmp(fileHeader.versionString, SpringVersion::GetSync().c_str()))
#endif
    ) {
        const std::string demoMsg = std::string("Demofile ") + filename +
                                    " corrupt or created by a different "
                                    "version of Spring, expects version " +
...
CDemoReader::GetData(const float readTime)
{
    ...
    if (readTime >= nextDemoReadTime) {
        ...
        if (playbackDemo->Read((char*)(buf->data), chunkHeader.length) <
            chunkHeader.length) {
            delete buf;
            bytesRemaining = 0;
            return NULL;
        ...
        if (!ReachedEnd()) {
            ...
            if (playbackDemo->Read((char*)&chunkHeader, sizeof(chunkHeader)) <
                sizeof(chunkHeader)) {
                delete buf;
                bytesRemaining = 0;
                return NULL;
...
CDemoReader::LoadStats()
{
    ...
    playbackDemo->Seek(fileHeader.headerSize + fileHeader.scriptSize +
                       fileHeader.demoStreamSize);

    winningAllyTeams.clear();
    playerStats.clear();
    teamStats.clear();

    for (int allyTeamNum = 0; allyTeamNum < fileHeader.winningAllyTeamsSize;
         ++allyTeamNum) {
    ...
    for (int playerNum = 0; playerNum < fileHeader.numPlayers; ++playerNum) {
        ...
        playbackDemo->Read(reinterpret_cast<char*>(&buf),
    ...
    { // Team statistics follow player statistics.
        ...
        playbackDemo->Read((char*)(&numStatsPerTeam[0]),
                           numStatsPerTeam.size());
