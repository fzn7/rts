...
void
CGameStateCollector::Serialize(creg::ISerializer* s)
{
    ...
    s->SerializeObjectInstance(&CNullUnitScript::value,
                               CNullUnitScript::value.GetClass());
    ...
    s->SerializeObjectInstance(CCategoryHandler::Instance(),
                               CCategoryHandler::Instance()->GetClass());
    s->SerializeObjectInstance(projectileHandler,
                               projectileHandler->GetClass());
    ...
    for (int a = 0; a < teamHandler->ActiveTeams(); a++) {
        s->SerializeObjectInstance(grouphandlers[a],
                                   grouphandlers[a]->GetClass());
    }
    s->SerializeObjectInstance(commandDescriptionCache,
                               commandDescriptionCache->GetClass());
...
CCregLoadSaveHandler::SaveGame(const std::string& path)
{
    ...
    try {
        ...
        gzFile file =
          gzopen(dataDirsAccess.LocateFile(path, FileQueryFlags::WRITE).c_str(),
                 "wb9");
        if (file == nullptr) {
...
{
    CGZFileHandler saveFile(dataDirsAccess.LocateFile(FindSaveFile(path)),
                            SPRING_VFS_RAW_FIRST);
    ...
    if (saveVersion != SpringVersion::GetSync()) {
        if (SpringVersion::IsRelease()) {
            throw content_error(
              "File was saved by an incompatible engine version: " +
              saveVersion);
        } else {
            LOG_L(L_WARNING,
                  "File was saved by a different engine version: %s",
                  saveVersion.c_str());
        }
    }
