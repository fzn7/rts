  ...
  , canLoadDefs(false)
{
    ...
    Channels::General->SetVolume(configHandler->GetInt("snd_volgeneral") *
                                 0.01f);
    Channels::UnitReply->SetVolume(configHandler->GetInt("snd_volunitreply") *
                                   0.01f);
    ...
    Channels::UserInterface->SetVolume(configHandler->GetInt("snd_volui") *
                                       0.01f);
    ...
    *soundThread = Threading::CreateNewThread(
      boost::bind(&CSound::StartThread, this, maxSounds));
...
CSound::GetSoundId(const std::string& name)
{
    ...
    soundItemDefMap::const_iterator itemDefIt =
      soundItemDefs.find(StringToLower(name));
    ...
    LOG_L(
      L_ERROR, "CSound::GetSoundId: could not find sound: %s", name.c_str());
    return 0;
}
...
{
    {
        ...
        // alc... will create its own thread it will copy the name from the
        // current thread. Later we finally rename `our` audio thread.
        ...
        if ((device == NULL) && (deviceName != NULL)) {
            LOG_L(L_WARNING,
                  "Could not open the sound device \"%s\", trying the default "
                  "device ...",
                  deviceName);
            configDeviceName = "";
            deviceName = NULL;
        ...
        LOG("OpenAL info:");
        const bool hasAllEnum =
          alcIsExtensionPresent(NULL, "ALC_ENUMERATE_ALL_EXT");
        if (hasAllEnum || alcIsExtensionPresent(NULL, "ALC_ENUMERATION_EXT")) {
            LOG("  Available Devices:");
            const char* deviceSpecifier = alcGetString(
              NULL,
              hasAllEnum ? ALC_ALL_DEVICES_SPECIFIER : ALC_DEVICE_SPECIFIER);
        ...
        LOG("  ALC Extensions: %s",
            (const char*)alcGetString(device, ALC_EXTENSIONS));
        ...
        for (int i = 0; i < maxSounds; i++) {
            ...
            if (thenewone->IsValid()) {
                ...
                LOG_L(L_WARNING,
                      "Your hardware/driver can not handle more than %i "
                      "soundsources",
                      maxSounds);
                delete thenewone;
                break;
    ...
    while (!soundThreadQuit) {
        ...
        boost::this_thread::sleep(
          boost::posix_time::millisec(1000 / FREQ_IN_HZ));
...
{
    //! MakeItemFromDef is private. Only caller is LoadSoundDefs and it sets the
    //! mutex itself.
    // boost::recursive_mutex::scoped_lock lck(soundMutex);
    ...
    boost::shared_ptr<SoundBuffer> buffer =
      SoundBuffer::GetById(LoadSoundBuffer(it->second));
...
{
    // call from sound thread, cause OpenAL calls tend to cause L2 misses and so
    // are slow (no reason to call them from mainthread)
    if (!listenerNeedsUpdate)
        return;

    // not 100% threadsafe, but worst case we would skip a single listener
    // update (and it runs at multiple Hz!)
    listenerNeedsUpdate = false;

    const float3 myPosInMeters = myPos * ELMOS_TO_METERS;
    alListener3f(
      AL_POSITION, myPosInMeters.x, myPosInMeters.y, myPosInMeters.z);

    // reduce the rolloff when the camera is high above the ground (so we still
    // hear something in tab mode or far zoom) for altitudes up to and including
    // 600 elmos, the rolloff is always clamped to 1
    const float camHeight =
      std::max(1.0f, myPos.y - CGround::GetHeightAboveWater(myPos.x, myPos.z));
    ...
    // The user experiences the camera/listener not as a world-interacting
    // object. So changing sounds on camera movements were irritating, esp.
    // because zooming with the mouse wheel often is faster than the speed of
    // sound, causing very high frequencies. Note: soundsource related doppler
    // effects are not deactivated by this! Flying cannon shoots still change
    // their frequencies. Note2: by not updating the listener velocity
    // soundsource related velocities are calculated wrong, so even if the
    // camera is moving with a cannon shoot the frequency gets changed.
    /*
    ...
    velocityAvg.y *= 0.001f; //! scale vertical axis separatly (zoom with
    mousewheel is faster than speed of sound!) velocityAvg *= 0.15f;
    alListener3f(AL_VELOCITY, velocityAvg.x, velocityAvg.y, velocityAvg.z);
    */

    ALfloat ListenerOri[] = { camDir.x, camDir.y, camDir.z,
...
CSound::PrintDebugInfo()
{
    ...
    LOG_L(L_DEBUG,
          "# reserved for buffers: %i kB",
          (int)(SoundBuffer::AllocedSize() / 1024));
...
CSound::LoadSoundDefsImpl(const std::string& fileName, const std::string& modes)
{
    ...
    if (!parser.IsValid()) {
        ...
        if (!soundItemTable.IsValid()) {
            LOG_L(L_WARNING,
                  "CSound(): could not parse SoundItems table in %s",
                  fileName.c_str());
            return false;
        } else {
            ...
            for (std::vector<std::string>::const_iterator it = keys.begin();
                ...
                if (!buf.KeyExists("file")) {
                    // no file, drop
                    LOG_L(L_WARNING,
                          "Sound %s is missing file tag (ignoring)",
                          name.c_str());
                    continue;
                } else {
            ...
            LOG(
              " parsed %i sounds from %s", (int)keys.size(), fileName.c_str());
        }
    }

    // FIXME why do sounds w/o an own soundItemDef create (!=pointer) a new one
    // from the defaultItem?
    for (soundItemDefMap::iterator it = soundItemDefs.begin();
         it != soundItemDefs.end();
         ++it) {
...
CSound::LoadSoundBuffer(const std::string& path)
{
    ...
    if (id > 0) {
        ...
        if (ending == "wav") {
            ...
            LOG_L(L_WARNING,
                  "CSound::LoadALBuffer: unknown audio format: %s",
                  ending.c_str());
        }
    ...
    for (int i = 0; i < attrs.size(); ++i) {
        if (attrs[i] == ALC_MONO_SOURCES) {
            ...
            if (maxMonoSources < maxSounds) {
                LOG_L(L_WARNING,
                      "Hardware supports only %d Sound sources, MaxSounds=%d, "
                      "using Hardware Limit",
                      maxMonoSources,
                      maxSounds);
            }
