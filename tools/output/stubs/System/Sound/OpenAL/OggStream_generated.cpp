...
COggStream::Play(const std::string& path, float volume)
{
    ...
    vorbisInfo = ov_info(&oggStream, -1);
    {
        ...
        for (unsigned i = 0; i < vorbisComment->comments; ++i) {
            vorbisTags[i] = std::string(vorbisComment->user_comments[i],
...
COggStream::DisplayInfo()
{
    ...
    for (TagVector::const_iterator it = vorbisTags.begin();
         it != vorbisTags.end();
         ++it) {
        LOG("%s", it->c_str());
    }
...
COggStream::StartPlaying()
{
    ...
    CheckError("COggStream::StartPlaying");
    alSourcePlay(source);
    CheckError("COggStream::StartPlaying");
