...
size_t
VorbisRead(void* ptr, size_t size, size_t nmemb, void* datasource)
{
...
bool
SoundBuffer::LoadWAV(const std::string& file,
                     std::vector<boost::uint8_t> buffer)
{
    WAVHeader* header = (WAVHeader*)(&buffer[0]);

    if ((buffer.empty()) || memcmp(header->riff, "RIFF", 4) ||
        memcmp(header->wavefmt, "WAVEfmt", 7)) {
...
#undef hswabdword
    ...
    if (header->channels == 1) {
        if (header->BitsPerSample == 8)
            ...
            LOG_L(L_ERROR,
                  "ReadWAV (%s): invalid number of bits per sample (mono)",
                  file.c_str());
            return false;
        }
        ...
        if (header->BitsPerSample == 8)
            ...
            LOG_L(L_ERROR,
                  "ReadWAV (%s): invalid number of bits per sample (stereo)",
                  file.c_str());
            return false;
        }
    } else {
        LOG_L(
          L_ERROR, "ReadWAV (%s): invalid number of channels.", file.c_str());
        return false;
    }

    if (static_cast<unsigned>(header->datalen) >
        buffer.size() - sizeof(WAVHeader)) {
        LOG_L(
          L_ERROR,
        ...
        //header->BitsPerSample); 		LOG_L(L_WARNING, "OpenAL:
        //totalLength %d\n", header->totalLength); 		LOG_L(L_WARNING,
        //"OpenAL: length %d\n", header->length); 		LOG_L(L_WARNING,
        //"OpenAL: SamplesPerSec %d\n", header->SamplesPerSec);
        //		LOG_L(L_WARNING, "OpenAL: AvgBytesPerSec %d\n",
        //header->AvgBytesPerSec);

        header->datalen =
          boost::uint32_t(buffer.size() - sizeof(WAVHeader)) &
          (~boost::uint32_t((header->BitsPerSample * header->channels) / 8 -
                            1));
    }

    if (!AlGenBuffer(file,
                     format,
                     &buffer[sizeof(WAVHeader)],
                     header->datalen,
                     header->SamplesPerSec)) {
    ...
    length = float(header->datalen) /
             (header->channels * header->SamplesPerSec * header->BitsPerSample);
                        ...
                        std::vector<boost::uint8_t> buffer)
{
    ...
    const int result =
      ov_open_callbacks(&buf, &oggStream, NULL, 0, vorbisCallbacks);
    if (result < 0) {
        LOG_L(L_WARNING,
              "Could not open Ogg stream (reason: %s).",
    ...
    if (vorbisInfo->channels == 1) {
        ...
        LOG_L(L_ERROR,
              "File %s: invalid number of channels: %i",
              file.c_str(),
              vorbisInfo->channels);
        return false;
    }
    ...
    do {
        if (4 * pos >
            3 *
              decodeBuffer.size()) // enlarge buffer so ov_read has enough space
            decodeBuffer.resize(decodeBuffer.size() * 2);
        read = ov_read(&oggStream,
                       (char*)&decodeBuffer[pos],
                       decodeBuffer.size() - pos,
                       0,
                       2,
                       1,
                       &section);
...
{
    int numBytes = 0;
    for (bufferVecT::const_iterator it = ++buffers.begin(); it != buffers.end();
         ++it)
