    ...
    *sfxProperties = eaxPresets[default_preset];
    ...
    if (!supported) {
        if (!hasExtension) {
            ...
            LOG(
              "  EFX is supported but software does not seem to work properly");
        }
        return;
    }
    ...
    {
        static const ALuint effects[] = { AL_EFFECT_REVERB,
                                          AL_EFFECT_EAXREVERB,
                                          AL_EFFECT_CHORUS,
    ...
    {
        static const ALuint filters[] = { AL_FILTER_LOWPASS,
                                          AL_FILTER_HIGHPASS,
                                          AL_FILTER_BANDPASS };
    ...
    alcGetIntegerv(
      device, ALC_MAX_AUXILIARY_SENDS, 1, (ALCint*)&maxSlotsPerSource);

    //! Check requirements
    if (!effectsSupported[AL_EFFECT_EAXREVERB] ||
        !filtersSupported[AL_FILTER_LOWPASS] || (maxSlots < 1) ||
        (maxSlotsPerSource < 1)) {
        if (enabled) {
    ...
    if (!alIsAuxiliaryEffectSlot(sfxSlot) || !alIsEffect(sfxReverb) ||
        !alIsFilter(sfxFilter)) {
...
CEFX::SetPreset(std::string name, bool verbose, bool commit)
{
    ...
    std::map<std::string, EAXSfxProps>::const_iterator it =
      eaxPresets.find(name);
...
CEFX::SetHeightRolloffModifer(const float& mod)
{
    ...
    alEffectf(sfxReverb,
              ...
              sfxProperties->properties_f[AL_EAXREVERB_ROOM_ROLLOFF_FACTOR] *
...
CEFX::CommitEffects()
{
    ...
    for (std::map<ALuint, ALfloat>::iterator it =
           sfxProperties->properties_f.begin();
         it != sfxProperties->properties_f.end();
         ++it)
        alEffectf(sfxReverb, it->first, it->second);
    for (std::map<ALuint, ALint>::iterator it =
           sfxProperties->properties_i.begin();
         it != sfxProperties->properties_i.end();
         ++it)
        alEffecti(sfxReverb, it->first, it->second);
    for (std::map<ALuint, float3>::iterator it =
           sfxProperties->properties_v.begin();
         it != sfxProperties->properties_v.end();
         ++it)
    ...
    for (std::map<ALuint, ALfloat>::iterator it =
           sfxProperties->filter_properties_f.begin();
         it != sfxProperties->filter_properties_f.end();
         ++it)
...
{
    airAbsorptionFactor =
      Clamp(value, AL_MIN_AIR_ABSORPTION_FACTOR, AL_MAX_AIR_ABSORPTION_FACTOR);
}
