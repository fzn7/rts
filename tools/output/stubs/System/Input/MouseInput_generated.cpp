...
/*
        ...
        Instead it installs a window message proc and reads input from
   WM_MOUSEMOVE. On non-windows, the normal SDL events are used for mouse input
...
{
    inputCon = input.AddHandler(
      boost::bind(&IMouseInput::HandleSDLMouseEvent, this, _1));
}

bool
IMouseInput::HandleSDLMouseEvent(const SDL_Event& event)
{
    switch (event.type) {
        case SDL_MOUSEMOTION: {
            ...
            if (mouse) {
                mouse->MouseMove(
                  mousepos.x, mousepos.y, event.motion.xrel, event.motion.yrel);
            }
        } break;
        ...
        case SDL_MOUSEBUTTONUP: {
            ...
            if (mouse) {
                mouse->MouseRelease(
                  mousepos.x, mousepos.y, event.button.button);
            }
        } break;
        case SDL_MOUSEWHEEL: {
            if (mouse) {
        ...
        case SDL_WINDOWEVENT: {
            if (event.window.event == SDL_WINDOWEVENT_LEAVE) {
                // mouse left window (set mouse pos internally to window center
                // to prevent endless scrolling)
...
class CWin32MouseInput : public IMouseInput
{
  public:
    ...
    static LRESULT CALLBACK SpringWndProc(HWND wnd,
                                          UINT msg,
                                          WPARAM wParam,
                                          LPARAM lParam)
    {
...
IMouseInput::SetPos(int2 pos)
{
    ...
    if (pos.x == mousepos.x && pos.y == mousepos.y) {
        // calling SDL_WarpMouse at 300fps eats ~5% cpu usage, so only update
        // when needed
        return;
    }
    ...
    SDL_PeepEvents(
      &events[0], 100, SDL_GETEVENT, SDL_MOUSEMOTION, SDL_MOUSEMOTION);
}

IMouseInput*
IMouseInput::GetInstance()
{
    if (mouseInput == NULL) {
#if defined(WIN32) && !defined(HEADLESS)
...
void
IMouseInput::FreeInstance(IMouseInput* mouseInp)
{
    delete mouseInp;
    mouseInput = NULL;
}
