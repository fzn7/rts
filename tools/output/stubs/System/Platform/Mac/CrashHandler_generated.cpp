...
TranslateStackTrace(bool* aiCrash, StackTrace& stacktrace, const int logLevel)
{
    ...
    for (auto it = stacktrace.begin(); it != stacktrace.end(); ++it) {
        Dl_info info;
        it->addr = (dladdr(it->ip, &info) != 0) ? (uintptr_t)info.dli_fbase
                                                : INVALID_ADDR_INDICATOR;

        // Find library path
        // http://stackoverflow.com/questions/8240436/filename-of-memory-mapped-libraries-osx
        // Parsing output of vmmap will be a pain as it doesn't flush buffers
        // (fgets hangs). Painful workaround
        // http://www.programmingforums.org/thread36915-2.html
    ...
    //   library -- this means fewer atos calls.
    for (auto it = addr_path.cbegin(); it != addr_path.cend(); ++it) {
        ...
        buf << ADDR2LINE << " -o " << modulePath << " -arch x86_64 -l "
            << std::hex << it->first;
