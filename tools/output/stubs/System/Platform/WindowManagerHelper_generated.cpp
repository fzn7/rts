...
SetIcon(const CBitmap* icon)
{
    ...
    if (icon != NULL) {
        ...
        if (((icon->channels != 3) && (icon->channels != 4))
            ...
            || (icon->xsize != 32) || (icon->ysize != 32)
            //#endif
        ) {
            LOG_L(L_WARNING,
                  "window-manager icon: Trying to set a window-manager icon "
                  "with the wrong format.");
            LOG_L(L_WARNING,
                  "window-manager icon: It has to be 24bit or 32bit, and on "
                  "windows it additionally has to be 32x32 pixels.");
        } else {
            // supplied bitmap is usable as icon
            SDL_Surface* newIcon = icon->CreateSDLSurface();
            if (newIcon == NULL) {
                LOG_L(L_WARNING,
                      "window-manager icon: Failed to create SDL surface, "
                      "reason: %s",
                      SDL_GetError());
            } else {
                ...
                if (currentIcon != NULL) {
                    ...
                    unsigned char* pixelData =
                      (unsigned char*)currentIcon->pixels;
