...
ThreadSIGUSR1Handler(int signum, siginfo_t* info, void* pCtx)
{
    ...
    if (err != 0) {
        LOG_L(L_ERROR,
              "Couldn't get thread context within suspend signal handler: %s",
              strerror(err));
        return;
    }

    // Change the "running" flag to false. Note that we don't own a lock on the
    // suspend mutex, but in order to get here,
...
SetThreadSignalHandler()
{
    ...
    if (err != 0) {
        LOG_L(L_FATAL,
              "Error while setting new pthread's signal mask: %s",
              strerror(err));
        return false;
    }
...
SetCurrentThreadControls(bool isLoadThread)
{
    ...
    if (threadCtls.get() != nullptr) {
        ...
        LOG_L(L_WARNING,
              "Setting a ThreadControls object on a thread that already has "
              "such an object registered.");
    } else {
        // Installing new ThreadControls object, so install signal handler also
...

    {
        ...
        threadCtls.reset(new std::shared_ptr<Threading::ThreadControls>(
          new Threading::ThreadControls()));

        auto ppThreadCtls = threadCtls.get();
            ...
            ThreadControls* tempCtls)
{
    ...
    auto ppThreadCtls =
      threadCtls.get(); // std::shared_ptr<Threading::ThreadControls>*
...

    {
        ...
        LOG_L(L_DEBUG,
              "ThreadStart(): New thread's handle is %.4lx",
              pThreadCtls->handle);
        ...
        // Ok, the thread should be ready to run its task now, so unlock the
        // suspend mutex!
...
ThreadControls::Suspend()
{
    ...
    if (!running) {
        LOG_L(L_ERROR,
              "Cannot suspend if a thread's running flag is set to false. "
              "Refusing to suspend using pthread_kill.");
    ...
    if (err != 0) {
        LOG_L(L_ERROR,
              "Error while trying to send signal to suspend thread: %s",
              strerror(err));
        return Threading::THREADERR_MISC;
    }

    // Before leaving this function, we need some kind of guarantee that the
    // stalled thread is suspended, so spinwait until it is guaranteed.
    // FIXME: this sort of spin-waiting inside the watchdog loop could be
    // avoided by creating another worker thread
    //        inside SuspendedStacktrace itself to do the work of checking that
    //        the stalled thread has been suspended and performing the trace
    //        there.
