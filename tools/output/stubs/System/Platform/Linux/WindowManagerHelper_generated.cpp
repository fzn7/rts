...
BlockCompositing(SDL_Window* window)
{
    ...
    bool b = true;
    Atom blockCompositAtomKDE =
      XInternAtom(x11display, "_KDE_NET_WM_BLOCK_COMPOSITING", false);
    XChangeProperty(x11display,
                    x11window,
                    blockCompositAtomKDE,
                    XA_INTEGER,
                    8,
                    PropModeReplace,
    ...
    Atom blockCompositAtom =
      XInternAtom(x11display, "_NET_WM_BYPASS_COMPOSITOR", false);
    XChangeProperty(x11display,
                    x11window,
                    blockCompositAtom,
                    XA_INTEGER,
...
GetWindowState(SDL_Window* window)
{
    ...
    Atom maxVAtom =
      XInternAtom(x11display, "_NET_WM_STATE_MAXIMIZED_VERT", false);
    Atom maxHAtom =
      XInternAtom(x11display, "_NET_WM_STATE_MAXIMIZED_HORZ", false);
    Atom minAtom = XInternAtom(x11display, "_NET_WM_STATE_HIDDEN", false);

    status = XGetWindowProperty(x11display,
                                x11window,
                                atom,
                                0,
                                20,
