...
class unordered_bimap
{
  public:
    ...
    unordered_bimap(
      const std::initializer_list<std::pair<const First, Second>> list)
      : first_(list)
    {
...
int
SDL21_keysyms(const int SDL2_keycode)
{
    auto it = SDL_keysym_bimap.first().find(SDL2_keycode);
    if (it != SDL_keysym_bimap.first().end())
        return it->second;
    LOG_L(L_DEBUG,
          "Cannot translate SDL2 keycode to SDL1 keycode: %d",
          SDL2_keycode);
    return 0;
}

int
SDL12_keysyms(const int SDL1_keycode)
{
    auto it = SDL_keysym_bimap.second().find(SDL1_keycode);
    if (it != SDL_keysym_bimap.second().end())
        return it->second;
    LOG_L(L_DEBUG,
          "Cannot translate SDL1 keycode to SDL2 keycode: %d",
          SDL1_keycode);
    return 0;
}
