...
COffscreenGLContext::COffscreenGLContext()
{
    ...
    //! so don't render to the the default framebuffer (always bind
    //! FBOs,DLs,...) !!!
    ...
    if (!hdc || !mainRC) {
        throw opengl_error(
          "Couldn't create an offscreen GL context: wglGetCurrentDC failed!");
    }
    ...
    if (wglCreateContextAttribsARB) {
        ...
        if (!offscreenRC)
            LOG_L(L_WARNING,
                  "Couldn't create an offscreen GL context: "
    ...
    if (!offscreenRC) {
        ...
        if (!offscreenRC) {
            throw opengl_error("Couldn't create an offscreen GL context: "
    ...
    if (!status) {
        ...
        SNPRINTF(msg,
                 255,
                 "Couldn't create an offscreen GL context: wglShareLists "
                 "failed (error: %i)!",
                 (int)err);
...
COffscreenGLContext::WorkerThreadPost()
{
    ...
    const PFNGLACTIVETEXTUREPROC workerGlActiveTexture =
      (PFNGLACTIVETEXTUREPROC)wglGetProcAddress("glActiveTexture");
    if (workerGlActiveTexture != mainGlActiveTexture) {
        WorkerThreadFree();
        throw opengl_error("Could not activate worker rendering context (uses "
...
COffscreenGLContext::COffscreenGLContext()
{
    ...
    if (!cglPxlfmt)
        throw opengl_error(
          "Couldn't create an offscreen GL context: CGLChoosePixelFmt failed!");
    ...
    if (!cglWorkerCtx)
        throw opengl_error(
          "Couldn't create an offscreen GL context: CGLCreateContext failed!");
}
...
{
    if (!mainGlActiveTexture)
        mainGlActiveTexture = (PFNGLACTIVETEXTUREPROC)glXGetProcAddress(
    ...
    if (!mainCtx)
        throw opengl_error("Couldn't create an offscreen GL context: "
                           "glXGetCurrentContext failed!");

    if (!display)
        throw opengl_error("Couldn't create an offscreen GL context: Couldn't "
                           "determine display!");
    ...
    const int fbattribs[] = { GLX_RENDER_TYPE,
                              ...
                              None };
    GLXFBConfig* fbcfg =
      glXChooseFBConfig(display, scrnum, (const int*)fbattribs, &nelements);
    if (!fbcfg || (nelements == 0))
        throw opengl_error(
          "Couldn't create an offscreen GL context: glXChooseFBConfig failed!");

    //! Create a pbuffer (each render context needs a drawable)
    const int pbuf_attribs[] = { GLX_PBUFFER_WIDTH,
                                 1,
                                 GLX_PBUFFER_HEIGHT,
                                 1,
    ...
    if (!pbuf)
        throw opengl_error(
          "Couldn't create an offscreen GL context: glXCreatePbuffer failed!");
    ...
    /*
            ...
            workerCtx = glXCreateContextAttribsARB(display, fbcfg[0], mainCtx,
       true, ctx_attribs);
    */

    //! Create render context
    workerCtx =
      glXCreateNewContext(display, fbcfg[0], GLX_RGBA_TYPE, mainCtx, true);
    if (!workerCtx)
        throw opengl_error("Couldn't create an offscreen GL context: "
...
COffscreenGLContext::WorkerThreadPost()
{
    ...
    const PFNGLACTIVETEXTUREPROC workerGlActiveTexture =
      (PFNGLACTIVETEXTUREPROC)glXGetProcAddress(
        (const GLubyte*)"glActiveTexture");
    if (workerGlActiveTexture != mainGlActiveTexture) {
        WorkerThreadFree();
        throw opengl_error("Could not activate worker rendering context (uses "
...
{
    thread =
      new boost::thread(boost::bind(&COffscreenGLThread::WrapFunc, this, f));
}
