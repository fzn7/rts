...
FileSystem::ConvertGlobToRegex(const std::string& glob)
{
    ...
    for (std::string::const_iterator i = glob.begin(); i != glob.end(); ++i) {
        ...
        switch (c) {
            ...
            case '}':
                ...
                if (braces == 0) {
                    LOG_L(L_WARNING,
                          "%s: closing brace without an equivalent opening "
                          "brace\n%s",
                          __FUNCTION__,
                          glob.c_str());
                }
    ...
    if (braces > 0) {
        LOG_L(L_WARNING,
              "%s: unterminated brace expression\n%s",
              __FUNCTION__,
              glob.c_str());
    } else if (braces < 0) {
        LOG_L(L_WARNING,
              "%s: too many closing braces\n%s",
              __FUNCTION__,
              glob.c_str());
    }
#endif
...
{
    path1 =
      FileSystem::EnsureNoPathSepAtEnd(FileSystem::GetNormalizedPath(path1));
    path2 =
      FileSystem::EnsureNoPathSepAtEnd(FileSystem::GetNormalizedPath(path2));
...
{
    return FileSystemAbstraction::FileExists(
      FileSystem::GetNormalizedPath(file));
}
...
FileSystem::GetFileSize(std::string file)
{
    ...
    return FileSystemAbstraction::GetFileSize(
      FileSystem::GetNormalizedPath(file));
}
...
FileSystem::CreateDirectory(std::string dir)
{
    ...
    while ((slash = dir.find('/', prev_slash + 1)) != std::string::npos) {
        ...
        if (!FileSystemAbstraction::IsFSRoot(pathPart) &&
            !FileSystemAbstraction::MkDir(pathPart)) {
            return false;
        }
        prev_slash = slash;
    }
...
FileSystem::GetDirectory(const std::string& path)
{
    ...
    return ""; // XXX return "./"? (a short test caused a crash because
               // CFileHandler used in Lua couldn't find a file in the base-dir)
}
...
FileSystem::GetNormalizedPath(const std::string& path)
{
    ...
    std::string normalizedPath =
      StringReplace(path, "\\", "/"); // convert to POSIX path separators

    normalizedPath = StringReplace(normalizedPath, "/./", "/");
    normalizedPath =
      boost::regex_replace(normalizedPath, boost::regex("[/]{2,}"), "/");
    normalizedPath =
      boost::regex_replace(normalizedPath, boost::regex("[^/]+[/][.]{2}"), "");
    normalizedPath =
      boost::regex_replace(normalizedPath, boost::regex("[/]{2,}"), "/");
...
FileSystem::GetCacheDir()
{
    ...
    static const std::string cacheVersion =
      SpringVersion::GetMajor() + cacheType[SpringVersion::IsRelease()] +
      SpringVersion::GetBranch();
    static const std::string cacheDir =
      EnsurePathSepAtEnd(GetCacheBaseDir()) + cacheVersion;
    return cacheDir;
}
