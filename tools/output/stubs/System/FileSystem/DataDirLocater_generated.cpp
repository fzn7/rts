...
DataDirLocater::SubstEnvVars(const std::string& in) const
{
    ...
    for (int i = 0; i < 10;
         ++i) { // repeat substitution till we got a pure absolute path
        wordexp_t pwordexp;
        int r =
          wordexp(previous.c_str(),
                  &pwordexp,
                  WRDE_NOCMD); // expands $FOO, ${FOO}, ${FOO-DEF} ~/, etc.
        if (r == EXIT_SUCCESS) {
...
DataDirLocater::AddDirs(const std::string& dirs)
{
    ...
    size_t prev_colon = 0;
    size_t colon;
    while ((colon = dirs.find(cPD, prev_colon)) !=
           std::string::npos) { // cPD (depending on OS): ';' or ':'
...
{
#ifndef _WIN32
    if ((dataDir->path.c_str()[0] != '/') ||
        (dataDir->path.find("..") != std::string::npos))
#else
...
DataDirLocater::FilterUsableDataDirs()
{
    ...
    for (std::vector<DataDir>::iterator d = dataDirs.begin();
         d != dataDirs.end();
         ++d) {
        if (d->path != previous) {
            if (DeterminePermissions(&*d)) {
                ...
                LOG_L(
                  L_DEBUG, "Potentional data directory: %s", d->path.c_str());
            }
        }
    }
...
DataDirLocater::AddPortableDir()
{
    ...
    if ((curWorkDirParent != "") &&
        LooksLikeMultiVersionDataDir(curWorkDirParent)) {
...
DataDirLocater::AddHomeDirs()
{
    ...
    SHGetFolderPath(
      NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, pathMyDocsC);
    SHGetFolderPath(
      NULL, CSIDL_COMMON_APPDATA, NULL, SHGFP_TYPE_CURRENT, pathAppDataC);
    ...
    // My Documents\My Games seems to be the MS standard even if no official
    // guidelines exist most if not all new Games For Windows(TM) games use this
    // dir
...
DataDirLocater::AddEtcDirs()
{
    ...
    std::string dd_etc = "";
    {
        ...
        if (fileH) {
            ...
            while (fgets(lineBuf, sizeof(lineBuf), fileH)) {
                ...
                if ((strlen(lineBuf) > 0) &&
                    strspn(lineBuf, whiteSpaces) != strlen(lineBuf)) {
    ...
    AddDirs(
      dd_etc); // from /etc/spring/datadir FIXME add in IsolatedMode too? FIXME
#endif
}
...
DataDirLocater::AddShareDirs()
{
    ...
    const std::string dd_curWorkDirData =
      bundleResourceDir + "/share/games/spring";

    AddDirs(
      dd_curWorkDirData); // "Spring.app/Contents/Resources/share/games/spring"
#endif

#ifdef SPRING_DATADIR
    // CompilerInfo: using the defineflag SPRING_DATADIR & "SPRING_DATADIR" as
    // string works fine, the preprocessor won't touch the 2nd
    AddDirs(SPRING_DATADIR); // from -DSPRING_DATADIR, example
                             // /usr/games/share/spring/
#endif
}
...
DataDirLocater::LocateDataDirs()
{
    ...
    // Note: first pushed dir is writeDir & dir priority decreases with pos in
    // queue
    ...
    // LEVEL 3: additional custom data sources
    {
        ...
        if (configHandler) {
            // user defined in spring config (Linux: ~/.springrc, Windows:
            // .\springsettings.cfg)
...
#ifdef WIN32
          "- a SpringData=C:/path/to/data declaration in spring's config file "
          "./springsettings.cfg\n"
...
DataDirLocater::ChangeCwdToWriteDir()
{
    ...
    // Not only safety anymore, it's just easier if other code can safely assume
    // that writeDir == current working directory
...
DataDirLocater::IsPortableMode()
{
    ...
    // PortableMode (don't use HomeDirs as writedirs, instead save files next to
    // binary)
    return true;
}
...
DataDirLocater::LooksLikeMultiVersionDataDir(const std::string& dirPath)
{
    ...
    if (
      FileSystem::DirExists(dirPath + "/maps") &&
...
DataDirLocater::GetWriteDirPath() const
{
    ...
    if (!writeDir)
        LOG_L(L_ERROR,
              "DataDirLocater::GetWriteDirPath() called before "
              "DataDirLocater::LocateDataDirs()");
