...
CFileFilter::AddRule(const string& rule)
{
    ...
    r.regex =
      boost::regex(glob_to_regex(r.glob),
                   boost::regex::icase | boost::regex::no_escape_in_lists);
    rules.push_back(r);
    // printf("added %s%s: %s\n", r.negate ? "!" : "", r.glob.c_str(),
    // r.regex.expression());
}
...
{
    bool match = false;
    for (vector<Rule>::const_iterator it = rules.begin(); it != rules.end();
         ++it) {
...
#define PATH_SEPARATORS "/\\:"
    ...
    for (; i != glob.end(); ++i) {
        ...
        switch (c) {
            case '*':
                // In (shell) globbing the wildcards match anything except path
                // separators.
                regex << "[^" PATH_SEPARATORS "]*";
                break;
            case '?':
