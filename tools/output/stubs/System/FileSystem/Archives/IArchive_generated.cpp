...
const std::string&
IArchive::GetArchiveName() const
{
    return archiveFile;
}

bool
IArchive::FileExists(const std::string& normalizedFilePath) const
{
    return (lcNameIndex.find(normalizedFilePath) != lcNameIndex.end());
}

unsigned int
IArchive::FindFile(const std::string& filePath) const
{
    const std::string normalizedFilePath = StringToLower(filePath);
    const std::map<std::string, unsigned int>::const_iterator it =
      lcNameIndex.find(normalizedFilePath);
...
bool
IArchive::HasLowReadingCost(unsigned int fid) const
{
    return true;
}

unsigned int
IArchive::GetCrc32(unsigned int fid)
{
    CRC crc;
...
bool
IArchive::GetFile(const std::string& name, std::vector<boost::uint8_t>& buffer)
{
