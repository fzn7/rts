...
StringToBool(std::string str)
{
    ...
    if ((str.empty()) || (str == "0") || (str == "n") || (str == "no") ||
        (str == "f") || (str == "false") || (str == "off")) {
        value = false;
    }
...
StringEndsWith(const std::string& str, const char* postfix)
{
    if ((postfix == NULL) || (str.size() < strlen(postfix))) {
        ...
        return (
          str.compare(str.size() - strlen(postfix), str.size(), postfix) == 0);
    }
}
...
Utf8GetNextChar(const std::string& text, int& pos)
{
    ...
    // Originaly there were 5&6 byte versions too, but they were dropped in RFC
    // 3629. So UTF8 maps to UTF16 range only.
    ...
    if (remainingChars >= 4) {
        // we need to use memcpy cause text[pos] isn't memory aligned as ints
        // need to be
        ...
        switch (remainingChars) {
            case 3:
                utf8.c[2] = boost::uint8_t(text[pos + 2]);
            case 2:
                utf8.c[1] = boost::uint8_t(text[pos + 1]);
            case 1:
                utf8.c[0] = boost::uint8_t(text[pos]);
            default: {
            }
        };
    }
