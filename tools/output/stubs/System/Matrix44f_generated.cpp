...
CMatrix44f::IsOrthoNormal() const
{
    ...
    const float3 norma =
      float3(xdir.SqLength(), ydir.SqLength(), zdir.SqLength());
...
{
    return (col[0] == float4(1.0f, 0.0f, 0.0f, 0.0f)) &&
...
float3 CMatrix44f::operator*(const float3 v) const
{
    ...
    out =
      _mm_add_ps(out, _mm_mul_ps(_mm_loadu_ps(&md[1][0]), _mm_load1_ps(&v.y)));
    out =
      _mm_add_ps(out, _mm_mul_ps(_mm_loadu_ps(&md[2][0]), _mm_load1_ps(&v.z)));
    out =
      _mm_add_ps(out, _mm_mul_ps(_mm_loadu_ps(&md[3][0]), _mm_set1_ps(1.0f)));
...
float4 CMatrix44f::operator*(const float4 v) const
{
    ...
    out =
      _mm_add_ps(out, _mm_mul_ps(_mm_loadu_ps(&md[2][0]), _mm_load1_ps(&v.z)));
    out =
      _mm_add_ps(out, _mm_mul_ps(_mm_loadu_ps(&md[3][0]), _mm_load1_ps(&v.w)));
...
CMatrix44f::InvertInPlace()
{
    ...
    const float det = (md[0][0] * cofac[0][0]) + (md[0][1] * cofac[0][1]) +
                      (md[0][2] * cofac[0][2]) + (md[0][3] * cofac[0][3]);
...
CMatrix44f::Invert(bool* status) const
{
    ...
    const float det = (md[0][0] * cofac.md[0][0]) +
                      (md[0][1] * cofac.md[0][1]) +
                      (md[0][2] * cofac.md[0][2]) + (md[0][3] * cofac.md[0][3]);
...
CMatrix44f::GetEulerAnglesRgtHand(float eps) const
{
    ...
    if (Square(eps) > math::fabs(m[0 * 4 + 2] + 1.0f)) {
        ...
        angles[0][ANGLE_P] =
          (angles[0][ANGLE_R] + math::atan2(m[1 * 4 + 0], m[2 * 4 + 0]));
    ...
    // psi angles (pitch)
    angles[0][ANGLE_P] =
      math::atan2((m[1 * 4 + 2] / cosYaw[0]), (m[2 * 4 + 2] / cosYaw[0]));
    angles[1][ANGLE_P] =
      math::atan2((m[1 * 4 + 2] / cosYaw[1]), (m[2 * 4 + 2] / cosYaw[1]));
    // phi angles (roll)
    angles[0][ANGLE_R] =
      math::atan2((m[0 * 4 + 1] / cosYaw[0]), (m[0 * 4 + 0] / cosYaw[0]));
    angles[1][ANGLE_R] =
      math::atan2((m[0 * 4 + 1] / cosYaw[1]), (m[0 * 4 + 0] / cosYaw[1]));
