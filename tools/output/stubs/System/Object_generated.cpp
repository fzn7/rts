...
CObject::CObject()
  ...
  , listening()
{
    // Note1: this static var is shared between all different types of classes
    // synced & unsynced (CUnit, CFeature, CProjectile, ...)
    //  Still it doesn't break syncness even when synced objects have different
    //  sync_ids between clients as long as the sync_id is creation time
    //  dependent and monotonously increasing, so the _order_ remains between
    //  clients.

    // Use atomic fetch-and-add, so threads don't read half written data nor
    // write old (= smaller) numbers
    sync_id = ++cur_sync_id;
    ...
    if (ser->IsWriting()) {
        ...
        for (int dt = 0; dt < DEPENDENCE_COUNT; ++dt) {
            if (listening[dt]) {
                ...
                for (oi = dl.begin(); oi != dl.end(); ++oi) {
                    if ((*oi)->GetClass() != CObject::StaticClass()) {
                        ser->SerializeObjectPtr((void**)&*oi,
                                                (*oi)->GetClass());
                    } else {
        ...
        cur_sync_id = Threading::AtomicCounterInt64(
          std::max(sync_id, (boost::int64_t)cur_sync_id));
    }
}
...
// places, however objects are responsible for not adding the same dependence
// more than once, and preferably try to delete the dependence asap in order not
// to waste memory
void
CObject::AddDeathDependence(CObject* obj, DependenceType dep)
{
