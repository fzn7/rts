...
#if NETWORK_TEST && PACKET_MAX_LATENCY > 0 &&                                  \
  PACKET_MAX_LATENCY >= PACKET_MIN_LATENCY
#define EMULATE_LATENCY(cond)                                                  \
    ...
    if (cond)                                                                  \
        delayed[spring_gettime() +                                             \
                spring_msecs(PACKET_MIN_LATENCY +                              \
                             (PACKET_MAX_LATENCY - PACKET_MIN_LATENCY) *       \
                               RANDOM_NUMBER())] = data;                       \
    if (false)
#else
...
void
Chunk::UpdateChecksum(CRC& crc) const
{
...
UDPConnection::Update()
{
...
#ifdef ENABLE_DEBUG_STATS
    {
        const float debugMssgDeltaTime =
          (curTime - lastDebugMessageTime).toMilliSecsf();
        const float avgFramePacketRate =
          numReceivedFramePackets / debugMssgDeltaTime;

        if (debugMssgDeltaTime >= 1000.0f) {
            if (logMessages) {
                LOG_L(L_INFO,
                      "[UDPConnection::%s] %u NETMSG_*FRAME packets received "
                      "(%fms : %fp/ms) during (empty=%u total=%u) GetData "
                      "calls",
                      __FUNCTION__,
                      numReceivedFramePackets,
                      debugMssgDeltaTime,
                      avgFramePacketRate,
                      numEmptyGetDataCalls,
                      numTotalGetDataCalls);
    ...
    if (!sharedSocket && !closed) {
        ...
        while ((bytesAvail = mySocket->available()) > 0) {
            ...
            const size_t bytesReceived = mySocket->receive_from(
              boost::asio::buffer(buffer), sender_endpoint, flags, err);
...
UDPConnection::ProcessRawPacket(Packet& incoming)
{
    ...
    if (logMessages) {
        LOG_L(L_INFO,
              "\t[%s] checksum=(%u : %u) mtu=%u",
              __FUNCTION__,
              incoming.GetChecksum(),
              incoming.checksum,
              mtu);
    }
#endif
            ...
            if (incoming.nakType < 0) {
                for (int i = 0; i != -incoming.nakType; ++i) {
                    ...
                    if (unAckPos >= 0 && unAckPos < unackedChunks.size()) {
                        assert(unackedChunks[unAckPos]->chunkNumber ==
                               nextCont + i);
                ...
                for (int i = 0; i != incoming.naks.size(); ++i) {
                    ...
                    while (unAckPos < unAckDiff + incoming.naks[i]) {
                        // if there are gaps in the array, assume that further
                        // resends are not needed
                        if (unAckPos < unackedChunks.size())
                            resendRequested.erase(
                              unackedChunks[unAckPos]->chunkNumber);
    ...
    for (auto ci = incoming.chunks.begin(); ci != incoming.chunks.end(); ++ci) {
        ...
        if ((lastInOrder >= c->chunkNumber) ||
            (waitingPackets.find(c->chunkNumber) != waitingPackets.end())) {
            ++droppedChunks;
            continue;
        }

        waitingPackets.insert(c->chunkNumber,
                              new RawPacket(&c->data[0], c->data.size()));
    }

    packetMap::iterator wpi;

    // process all in order packets that we have waiting
    while ((wpi = waitingPackets.find(lastInOrder + 1)) !=
           waitingPackets.end()) {
        ...
        if (fragmentBuffer != NULL) {
            ...
            std::copy(fragmentBuffer->data,
                      fragmentBuffer->data + fragmentBuffer->length,
                      buf.begin());
            delete fragmentBuffer;
            fragmentBuffer = NULL;
        }
        ...
        lastInOrder++;
        std::copy(wpi->second->data,
                  wpi->second->data + wpi->second->length,
        ...
        for (unsigned pos = 0; pos < buf.size();) {
            ...
            const int pktlength =
              ProtocolDef::GetInstance()->PacketLength(bufp, msglength);

            // this returns false for zero/invalid pktlength
            if (ProtocolDef::GetInstance()->IsValidLength(pktlength,
                                                          msglength)) {
                msgQueue.push_back(boost::shared_ptr<const RawPacket>(
                  new RawPacket(bufp, pktlength)));

#ifdef ENABLE_DEBUG_STATS
                // server sends both of these, clients send only keyframe
                // messages
                // TODO: would be easy to feed this data into a Q3A-style
                // lagometer
                //
                if ((msgQueue.back())->data[0] == NETMSG_NEWFRAME ||
                    (msgQueue.back())->data[0] == NETMSG_KEYFRAME) {
                    const spring_time dt =
                      spring_gettime() - lastFramePacketRecvTime;

                    sumDeltaFramePacketRecvTime += dt.toMilliSecsf();
                    minDeltaFramePacketRecvTime =
                      std::min(dt.toMilliSecsf(), minDeltaFramePacketRecvTime);
                    maxDeltaFramePacketRecvTime =
                      std::max(dt.toMilliSecsf(), maxDeltaFramePacketRecvTime);
                    ...
                    if (logMessages) {
                        LOG_L(L_INFO,
                              "\t[%s] (received=%u enqueued=%u) packets "
                              "(dt=%fms mindt=%fms maxdt=%fms sumdt=%fms)",
                              __FUNCTION__,
                              numReceivedFramePackets,
                              numEnqueuedFramePackets,
                              dt.toMilliSecsf(),
                              minDeltaFramePacketRecvTime,
                              maxDeltaFramePacketRecvTime,
                              sumDeltaFramePacketRecvTime);
                    }
                }
#endif
...
UDPConnection::Flush(const bool forced)
{
    ...
    const bool waitMore =
      (lastChunkCreatedTime >= (curTime - spring_msecs(1000 / chunksPerSec)));
    // if the packet is tiny, reduce the send frequency further
    const int requiredLength =
      ((200 >> netLossFactor) -
       spring_tomsecs(curTime - lastChunkCreatedTime)) /
      10;

    int outgoingLength = 0;

    if (!waitMore) {
        for (auto pi = outgoingData.begin();
             (pi != outgoingData.end()) && (outgoingLength <= requiredLength);
             ++pi) {
    ...
    if (forced || (!waitMore && outgoingLength > requiredLength)) {
        ...
        // This is an attempt to fix the bug where players drop out of the game
        // if someone in the game gives a large order.
        bool partialPacket = false;
        bool sendMore = true;

        do {
            sendMore = (outgoing.GetAverage(true) <=
                        globalConfig->linkOutgoingBandwidth);
            sendMore |= ((globalConfig->linkOutgoingBandwidth <= 0) ||
                         partialPacket || forced);
            ...
            if (!outgoingData.empty() && sendMore) {
                ...
                if (!partialPacket &&
                    !ProtocolDef::GetInstance()->IsValidPacket(
                    ...
                    const unsigned numBytes =
                      std::min((unsigned)maxChunkSize - pos, packet->length);
                    ...
                    if (partialPacket) {
                        ...
                        packet.reset(new RawPacket(packet->data + numBytes,
            ...
            if ((pos > 0) &&
                (outgoingData.empty() || (pos == maxChunkSize) || !sendMore)) {
...
UDPConnection::CheckTimeout(int seconds, bool initial) const
{
    ...
    if (seconds == 0) {
        timeout = (dataRecv && !initial) ? globalConfig->networkTimeout
    ...
    return (timeout > 0 &&
            (spring_gettime() - lastPacketRecvTime) > spring_secs(timeout));
}
...
UDPConnection::Statistics() const
{
    ...
    msg += str(boost::format(
                 "Received: %1% bytes in %2% packets (%3% bytes/package)\n") %
               dataRecv % recvPackets % (SafeDivide(dataRecv, recvPackets)));
    msg += str(
      boost::format("Sent: %1% bytes in %2% packets (%3% bytes/package)\n") %
      dataSent % sentPackets % (SafeDivide(dataSent, sentPackets)));
    msg += str(boost::format("Relative protocol overhead: %1% up, %2% down\n") %
               SafeDivide(sentOverhead, dataSent) %
               SafeDivide(recvOverhead, dataRecv));
    msg += str(boost::format(
                 "%1% incoming chunks dropped, %2% outgoing chunks resent\n") %
...
{
    return str(boost::format("[%s]:%u") % addr.address().to_string() %
               addr.port());
}
...
UDPConnection::SendIfNecessary(bool flushed)
{
...

    {
        ...
        for (packetMap::iterator pi = waitingPackets.begin();
             pi != waitingPackets.end();
             ++pi) {
        ...
        if ((numContinuous < 8) &&
            (curTime - lastNakTime) > spring_msecs(200 >> netLossFactor)) {
    ...
    if (flushed || !newChunks.empty() ||
        (netLossFactor == MIN_LOSS_FACTOR && !resendRequested.empty()) ||
        (nak > 0) ||
        (curTime - lastPacketSendTime) > spring_msecs(200 >> netLossFactor)) {
        bool todo = true;

        int maxResend = resendRequested.size();
        int unackPrevSize = unackedChunks.size();

        std::map<boost::int32_t, ChunkPtr>::iterator resIter =
          resendRequested.begin();
        std::map<boost::int32_t, ChunkPtr>::iterator resMidIter,
          resMidIterStart, resMidIterEnd;
        std::map<boost::int32_t, ChunkPtr>::reverse_iterator resRevIter;

        if (netLossFactor != MIN_LOSS_FACTOR) {
            maxResend =
              std::min(maxResend, 20 * netLossFactor); // keep it reasonable, or
                                                       // it could cause a
                                                       // tremendous flood of
                                                       // packets
            ...
            for (int i = 0; i < resMidStart; ++i)
                ++resMidIterStart;
            if (resMidIterStart != resendRequested.end() &&
                lastMidChunk < resMidIterStart->first)
                lastMidChunk = resMidIterStart->first - 1;

            for (int i = 0; i < resMidEnd; ++i)
                --resMidIterEnd;

            while (resMidIter != resendRequested.end() &&
                   resMidIter->first <= lastMidChunk)
                ++resMidIter;

            if (resMidIter == resendRequested.end() ||
                resMidIterEnd == resendRequested.end() ||
        ...
        while (todo && ((outgoing.GetAverage() <=
                         globalConfig->linkOutgoingBandwidth) ||
            ...
            if (nak > 0) {
                ...
                for (unsigned i = 0; i != buf.naks.size(); ++i) {
                    buf.naks[i] =
                      dropped[i] -
                      (lastInOrder +
                       1); // zero means request resend of lastInOrder + 1
                }
                if (netLossFactor == MIN_LOSS_FACTOR)
            ...
            while (true) {
                bool canResend =
                  maxResend > 0 &&
                  ((buf.GetSize() +
                    (((netLossFactor == MIN_LOSS_FACTOR) || (rev == 0))
                       ? resIter->second->GetSize()
                       : ((rev == 1) ? resRevIter->second->GetSize()
                                     : resMidIter->second
                                         ->GetSize())) // resend chunk size
                    ) <= mtu);
                bool canSendNew =
                  !newChunks.empty() &&
                  ((buf.GetSize() + newChunks[0]->GetSize()) <= mtu);

                if (!canResend && !canSendNew)
                    break;

                // alternate between send and resend to make sure none is
                // starved
                resend = !resend;
                ...
                if (resend && canResend) {
                    if (netLossFactor == MIN_LOSS_FACTOR) {
                        ...
                        // on a lossy connection, just keep resending until it
                        // is acked
                        switch (rev) {
                            case 0:
                                buf.chunks.push_back(resIter->second);
                                ++resIter;
                                break;
                                // alternate between sending from front, middle
                                // and back of list of requested chunks,
                            case 1:
                                buf.chunks.push_back(resRevIter->second);
                                ++resRevIter;
                                break;
                                // since this improves performance on high
                                // latency connections
                            case 2:
                            case 3:
...
{
    while (!unackedChunks.empty() &&
           (lastAck >= (*unackedChunks.begin())->chunkNumber))
        unackedChunks.pop_front();

    // resend requested and later acked, happens every now and then
    while (!resendRequested.empty() &&
           lastAck >= resendRequested.begin()->first)
...
void
UDPConnection::BandwidthUsage::UpdateTime(unsigned newTime)
{
    if (newTime > (lastTime + 100)) {
        average = (average * 9 + float(trafficSinceLastTime) /
                                   float(newTime - lastTime) * 1000.0f) /
                  10.0f;
...
UDPConnection::BandwidthUsage::GetAverage(bool prel) const
{
    ...
    return average +
           (prel ? std::max(trafficSinceLastTime, prelTrafficSinceLastTime)
                 : trafficSinceLastTime);
}
...
{
    netLossFactor =
      std::max((int)MIN_LOSS_FACTOR, std::min(factor, (int)MAX_LOSS_FACTOR));
}
