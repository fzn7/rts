...
PostCregTest(int fineClasses, int brokenClasses, int ignore = 0)
{
    if (brokenClasses > 0) {
        LOG_L(L_WARNING,
              "CREG Results: %i of %i classes are broken",
              brokenClasses,
              brokenClasses + fineClasses);
    ...
    for (const creg::Class* c : creg::System::GetClasses()) {
        ...
        for (const creg::Class* c_base = c; c_base; c_base = c_base->base()) {
            for (const creg::Class::Member& m : c_base->members) {
                if (std::string(m.name) != "Reserved" &&
                    !memberNames.insert(m.name).second) {
                    LOG_L(L_WARNING,
                          "  Duplicated member registration %s::%s %s",
                          className.c_str(),
                          m.name,
                          c_base->name.c_str());
                    membersBroken = true;
        ...
        if (membersBroken) {
            ...
            // LOG( "CREG: Class %s fine, size %u", className.c_str(),
            // classSize);
            fineClasses++;
        }
    }
...
TestCregClasses2()
{
    ...
    for (const creg::Class* c : creg::System::GetClasses()) {
        ...
        size_t cregSize =
          1; // c++ class is min. 1byte large (part of sizeof definition)
        ...
        cregSize = std::ceil(cregSize / alignment) *
                   alignment; // FIXME too simple, gcc's appending rules are
                              // ways more complicated

        if (cregSize != classSize) {
            brokenClasses++;
            LOG_L(L_WARNING,
                  "  Missing member(s) in class %s, real size %i, creg size %i",
                  className.c_str(),
                  int(classSize),
                  int(cregSize));
            /*for (auto jt = classMembers.cbegin(); jt != classMembers.cend();
            ++jt) { const std::string memberName   = (*jt)->name; const size_t
            memberOffset = (*jt)->offset; const std::string typeName =
            (*jt)->type->GetName(); const size_t      typeSize =
            (*jt)->type->GetSize(); LOG_L(L_WARNING, "  member %20s, type %12s,
            offset %3u, size %u", memberName.c_str(), typeName.c_str(),
        ...
        for (int i = 0; i < classSize; ++i) {
            if (!memberMap[i]) {
                ...
                if (prevMember && nextMember) {
                    LOG_L(L_WARNING,
                          "  Missing member(s) in class %s, between %s & %s, "
                          "~%i byte(s)",
                          className.c_str(),
                          prevMember->name,
                          nextMember->name,
                    ...
                    LOG_L(
                      L_WARNING,
                      "  Missing member(s) in class %s, before %s, ~%i byte(s)",
                      className.c_str(),
                      nextMember->name,
                      holeSize);
                } else if (prevMember) {
                    LOG_L(
                      L_WARNING,
                      "  Missing member(s) in class %s, after %s, ~%i byte(s)",
                      className.c_str(),
                      prevMember->name,
                      holeSize);
                } else {
                    LOG_L(
                      L_WARNING,
                      "  Missing member(s) in class %s: none member is creg'ed",
                      className.c_str());
                }
            }
            ...
            for (int i = 0; i < classSize; ++i) {
                    ...
                    } else
                    {
                            if (memberMap[i] != lastMember) {
                                    ...
                                    memberMapLegend += IntToString(n) + ": " +
            lastMember->name + " (type: " + lastMember->type->GetName() + "
            alignment: " + IntToString(lastMember->alignment) + ")\n";
                            }
...
{
    PreCregTest(
      "CREG: Test4 (Incorrect Usage of CR_DECLARE vs. CR_DECLARE_STRUCT)");
    LOG("  Note, CR_DECLARE_STRUCT is for plain structs only without a vTable "
        "and/or inheritance.");
    LOG("  While CR_DECLARE should be used for derived classes (both sub & "
        "base!).");
    LOG("  It adds a virtual method! And so a vTable if there isn't one "
        "already.");
    LOG("  This vTable has the size of a pointer and changes the size of the "
        "class.");
    LOG("  AND SO CR_DECLARE IS NOT USABLE FOR `COMPACT` STRUCTS AS FLOAT3/4, "
        "RECT, ...!");
    LOG("  Instead use CR_DECLARE_STRUCT for all inherited classes of those "
        "and register the members of their baseclass, too.");
    ...
    for (const creg::Class* c : creg::System::GetClasses()) {
        ...
        if (!c->binder->isCregStruct) {
            if (!c->base() && c->GetDerivedClasses().empty()) {
                incorrectUsage = true;
                LOG_L(L_WARNING,
                      "  Class %s has a vTable but isn't derived (should use "
                      "CR_DECLARE_STRUCT)",
                      className.c_str());
            }
        } else {
            if (c->base()) {
                incorrectUsage = true;
                LOG_L(L_WARNING,
                      "  Class %s hasn't a vTable but is derived from %s "
                      "(should use CR_DECLARE)",
                      className.c_str(),
                      c->base()->name.c_str());
            } else if (!c->GetDerivedClasses().empty()) {
                incorrectUsage = true;
                LOG_L(L_WARNING,
                      "  Class %s hasn't a vTable but is derived by %s (should "
                      "use CR_DECLARE)",
                      className.c_str(),
                      c->GetDerivedClasses()[0]->name.c_str());
            }
        }
