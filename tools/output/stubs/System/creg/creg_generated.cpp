...
derivedClasses()
{
    ...
    //   global statics, and those have an arbitrary init order. And when a
    //   Class gets created, it doesn't mean its parent class already has.
    //   (Still its pointer is already valid!) So we cannot access other's
    //   Class' members that early in the loading stage, that's solved with
    //   these local static vars, which get created on access.
...
Class::CalculateChecksum(unsigned int& checksum)
{
    for (Member& m : members) {
        ...
        checksum = HsiehHash(
          m.type->GetName().data(), m.type->GetName().size(), checksum);
