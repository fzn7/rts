...
{
    log_formatter_getSinks().insert(sink);
}
void
log_backend_unregisterSink(log_sink_ptr sink)
{
    log_formatter_getSinks().erase(sink);
}

void
log_backend_registerCleanup(log_cleanup_ptr cleanupFunc)
{
    log_formatter_getCleanupFuncs().insert(cleanupFunc);
}
void
log_backend_unregisterCleanup(log_cleanup_ptr cleanupFunc)
{
    log_formatter_getCleanupFuncs().erase(cleanupFunc);
}
                   ...
                   va_list arguments)
{
    ...
    if (sinks.empty()) {
        ...
        if (!warned) {
            ...
            fprintf(stderr,
                    "\nWARNING: A log message was recorded, but no sink is "
                    "registered."
        ...
        // format the record
        const char* record =
          log_formatter_format(section, level, fmt, arguments);
...
log_backend_cleanup()
{
    const std::set<log_cleanup_ptr>& cleanupFuncs =
      log_formatter_getCleanupFuncs();
