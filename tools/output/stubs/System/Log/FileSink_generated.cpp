...
{
    LogFileDetails(FILE* outStream = NULL,
...
struct LogFilesContainer
{
    ...
    logFiles_t& GetLogFiles() { return logFiles; }
...
struct LogRecord
{
    LogRecord(const std::string& section, int level, const std::string& record)
      : section(section)
      , level(level)
      , record(record)
    {}
...
log_file_writeToFiles(const char* section, int level, const char* record)
{
    ...
    for (auto lfi = logFiles.begin(); lfi != logFiles.end(); ++lfi) {
        if (lfi->second.IsLogging(section, level) &&
            ...
            log_file_writeToFile(lfi->second.GetOutStream(),
                                 record,
                                 lfi->second.FlushOnWrite(level));
        }
    }
}
...
log_file_writeBufferToFiles()
{
    while (!log_file_getRecordBuffer().empty()) {
        ...
        log_file_writeToFiles(
          lri->GetSection().c_str(), lri->GetLevel(), lri->GetRecord().c_str());
                    ...
                    int flushLevel)
{
    ...
    setvbuf(tmpStream,
            NULL,
            _IOFBF,
            (BUFSIZ < 8192) ? BUFSIZ : 8192); // limit buffer to 8kB

    const std::string sectionsStr = (sections == NULL) ? "" : sections;
    logFiles[filePathStr] =
      LogFileDetails(tmpStream, sectionsStr, minLevel, flushLevel);
}
