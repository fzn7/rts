...
#define for_each_source(it)                                                    \
    for (vector<ReadOnlyConfigSource*>::iterator it = sources.begin();         \
         it != sources.end();                                                  \
         ++it)

#define for_each_source_const(it)                                              \
    for (vector<ReadOnlyConfigSource*>::const_iterator it = sources.begin();   \
         it != sources.end();                                                  \
         ++it)
...
ConfigHandlerImpl::~ConfigHandlerImpl()
{
    assert(
      observers
        .empty()); // all observers have to be deregistered by RemoveObserver()
    for_each_source(it) { delete (*it); }
}
...
ConfigHandlerImpl::RemoveDefaults()
{
    ...
    vector<ReadOnlyConfigSource*>::const_reverse_iterator rsource =
      sources.rbegin();
    for (; rsource != sources.rend(); ++rsource) {
        FileConfigSource* source = dynamic_cast<FileConfigSource*>(*rsource);
        if (source != NULL) {
            // Copy the map; we modify the original while iterating over the
            // copy.
            StringMap file = source->GetData();
            for (StringMap::const_iterator it = file.begin(); it != file.end();
                 ++it) {
...
ConfigHandlerImpl::GetDataWithoutDefaults() const
{
    ...
    for (auto rsource = sources.crbegin(); rsource != sources.crend();
         ++rsource) {
        const FileConfigSource* source =
          dynamic_cast<const FileConfigSource*>(*rsource);
        if (source == nullptr)
            continue;
...
ConfigHandlerImpl::GetString(const string& key) const
{
    ...
    throw std::runtime_error(
      "ConfigHandler: Error: Key does not exist: " + key +
...
{
    boost::mutex::scoped_lock lck(observerMutex);
    for (StringMap::const_iterator ut = changedValues.begin();
         ut != changedValues.end();
         ++ut) {
        const string& key = ut->first;
        const string& value = ut->second;
        for (list<NamedConfigNotifyCallback>::const_iterator it =
               observers.begin();
             it != observers.end();
             ++it) {
...
{
    boost::mutex::scoped_lock lck(observerMutex);
    for (list<NamedConfigNotifyCallback>::iterator it = observers.begin();
         it != observers.end();
         ++it) {
        if (it->holder == holder) {
            observers.erase(it);
