...
CLogger::FlushBuffer()
{
    ...
    for (auto it = buffer.begin(); it != buffer.end(); ++it) {
        ...
        if (open != std::string::npos && close != std::string::npos) {
            void* p;
            ss << std::hex << it->substr(open + 1, close - open - 1)
               << std::dec;
            ss >> p;
            ...
            if (CrashHandler::InitImageHlpDll() &&
                ...
                SymGetLineFromAddr64(
                  GetCurrentProcess(), (DWORD64)p, &displacement, &line);

                fprintf(logfile,
                        "%s%s:%d [%p]%s\n",
                        it->substr(0, open).c_str(),
                        pSym->Name,
                        line.LineNumber,
                        p,
    ...
    command << ADDR2LINE << " \"--exe=" << exename
            << "\" --functions --demangle --inline";
