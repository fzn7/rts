  ...
  , lastGroundDetail{ 0, 0 }
{
    ...
    // assert((numPatchesX == smfReadMap->numBigTexX) && (numPatchesY ==
    // smfReadMap->numBigTexY));

    for (unsigned int i = MESH_NORMAL; i <= MESH_SHADOW; i++) {
        patchMeshGrid[i].resize(numPatchesX * numPatchesY);
        const int borderSizeX = 1 + (numPatchesX > 1);
        const int borderSizeY = 1 + (numPatchesY > 1);
        borderPatches[i].resize(borderSizeX * borderSizeY +
                                  (numPatchesY - borderSizeY) * borderSizeX +
    ...
    for (unsigned int i = MESH_NORMAL; i <= MESH_SHADOW; i++) {
        ...
        if (numPatchesY > 1)
            borderPatches[i][patchIdx++] =
              &patches[(numPatchesY - 1) * numPatchesX + (0)];

        if (numPatchesX > 1 && numPatchesY > 1)
            borderPatches[i][patchIdx++] =
              &patches[(numPatchesY - 1) * numPatchesX + (numPatchesX - 1)];

        // gather x-border patches
        for (int py = 1; py < (numPatchesY - 1); ++py) {
            borderPatches[i][patchIdx++] = &patches[py * numPatchesX + (0)];
            borderPatches[i][patchIdx++] =
              &patches[py * numPatchesX + (numPatchesX - 1)];
        }
        ...
        for (int px = 1; px < (numPatchesX - 1); ++px) {
            ...
            borderPatches[i][patchIdx++] =
              &patches[(numPatchesY - 1) * numPatchesX + px];
        }
    }
}
...
#define RETESSELLATE_MODE 1
...
#if (RETESSELLATE_MODE == 2)
    retessellate |=
      ((cam->GetPos() - lastCamPos[shadowPass]).SqLength() > (500.0f * 500.0f));
#endif
    retessellate |=
...

    {
        ...
        Reset(shadowPass);
        forceTessellate[shadowPass] =
          Tessellate(patchMeshGrid[shadowPass],
                     cam,
...
CRoamMeshDrawer::DrawMesh(const DrawPass::e& drawPass)
{
    ...
    switch (drawPass) {
        ...
        default: {
            Patch::UpdateVisibility(CCamera::GetActiveCamera(),
                                    patchMeshGrid[MESH_NORMAL],
                                    numPatchesX);
        } break;
    }
...
CRoamMeshDrawer::DrawInMiniMap()
{
    ...
    for (const Patch& p : patchMeshGrid[MESH_NORMAL]) {
        if (!p.IsVisible(CCamera::GetActiveCamera())) {
            glRectf(p.coors.x,
                    p.coors.y,
                    p.coors.x + PATCH_SIZE,
                    p.coors.y + PATCH_SIZE);
        }
    }
...
CRoamMeshDrawer::Reset(bool shadowPass)
{
    ...
    for (int y = 0; y < numPatchesY; ++y) {
        for (int x = 0; x < numPatchesX; ++x) {
            ...
            if (x > (0)) {
                pbl->LeftNeighbor =
                  patches[y * numPatchesX + x - 1].GetBaseRight();
            }
            if (x < (numPatchesX - 1)) {
                pbr->LeftNeighbor =
                  patches[y * numPatchesX + x + 1].GetBaseLeft();
            }

            if (y > (0)) {
                pbl->RightNeighbor =
                  patches[(y - 1) * numPatchesX + x].GetBaseRight();
            }
            if (y < (numPatchesY - 1)) {
                pbr->RightNeighbor =
                  patches[(y + 1) * numPatchesX + x].GetBaseLeft();
            }
        }
    }
}
                            ...
                            bool shadowPass)
{
    ...
    // hint: threading just helps a little with huge cpu usage in
    // retessellation, still better than nothing
    ...
    for (int idx = 0; idx < 9; ++idx) {
        for_mt(0, patches.size(), [&](const int i) {
            ...
            forceTess |=
              (!p->Tessellate(cam->GetPos(), viewRadius, shadowPass));
        });
...
CRoamMeshDrawer::UnsyncedHeightMapUpdate(const SRectangle& rect)
{
    ...
    const int xstart =
      std::max(0, (int)math::floor((rect.x1 - margin) * INV_PATCH_SIZE));
    const int xend = std::min(
      numPatchesX, (int)math::ceil((rect.x2 + margin) * INV_PATCH_SIZE));
    const int zstart =
      std::max(0, (int)math::floor((rect.z1 - margin) * INV_PATCH_SIZE));
    const int zend = std::min(
      numPatchesY, (int)math::ceil((rect.z2 + margin) * INV_PATCH_SIZE));
