...
#define sh shaderHandler
    arbShaders[ARB_SHADER_DEFAULT] =
      sh->CreateProgramObject("[SMFGroundDrawer]", "SMFShaderBaseARB", true);
    arbShaders[ARB_SHADER_REFLECT] =
      sh->CreateProgramObject("[SMFGroundDrawer]", "SMFShaderReflARB", true);
    arbShaders[ARB_SHADER_REFRACT] =
      sh->CreateProgramObject("[SMFGroundDrawer]", "SMFShaderRefrARB", true);
    arbShaders[ARB_SHADER_CURRENT] = arbShaders[ARB_SHADER_DEFAULT];

    arbShaders[ARB_SHADER_DEFAULT]->AttachShaderObject(
      sh->CreateShaderObject("ARB/ground.vp", "", GL_VERTEX_PROGRAM_ARB));
    arbShaders[ARB_SHADER_DEFAULT]->AttachShaderObject(sh->CreateShaderObject(
      "ARB/groundFPshadow.fp", "", GL_FRAGMENT_PROGRAM_ARB));
    arbShaders[ARB_SHADER_DEFAULT]->Link();

    arbShaders[ARB_SHADER_REFLECT]->AttachShaderObject(sh->CreateShaderObject(
      "ARB/dwgroundreflectinverted.vp", "", GL_VERTEX_PROGRAM_ARB));
    arbShaders[ARB_SHADER_REFLECT]->AttachShaderObject(sh->CreateShaderObject(
      "ARB/groundFPshadow.fp", "", GL_FRAGMENT_PROGRAM_ARB));
    arbShaders[ARB_SHADER_REFLECT]->Link();

    arbShaders[ARB_SHADER_REFRACT]->AttachShaderObject(sh->CreateShaderObject(
      "ARB/dwgroundrefract.vp", "", GL_VERTEX_PROGRAM_ARB));
    arbShaders[ARB_SHADER_REFRACT]->AttachShaderObject(sh->CreateShaderObject(
      "ARB/groundFPshadow.fp", "", GL_FRAGMENT_PROGRAM_ARB));
...
SMFRenderStateARB::HasValidShader(const DrawPass::e& drawPass) const
{
    ...
    switch (drawPass) {
        case DrawPass::Normal: {
            shader = arbShaders[ARB_SHADER_CURRENT];
        } break;
        case DrawPass::WaterReflection: {
            shader = arbShaders[ARB_SHADER_REFLECT];
        } break;
        case DrawPass::WaterRefraction: {
            shader = arbShaders[ARB_SHADER_REFRACT];
        } break;
        default: {
        } break;
    }
...
SMFRenderStateGLSL::Init(const CSMFGroundDrawer* smfGroundDrawer)
{
    ...
    const std::string defs =
      ("#define SMF_TEXSQUARE_SIZE " + FloatToString(SMF_TEXSQUARE_SIZE) +
       "\n") +
      ("#define SMF_INTENSITY_MULT " +
       FloatToString(CGlobalRendering::SMF_INTENSITY_MULT) + "\n");

    if (useLuaShaders) {
        for (unsigned int n = GLSL_SHADER_STANDARD; n <= GLSL_SHADER_DEFERRED;
             n++) {
            glslShaders[n] = shaderHandler->CreateProgramObject(
              "[SMFGroundDrawer::Lua]", names[n] + "-Lua", false);
        ...
        for (unsigned int n = GLSL_SHADER_STANDARD; n <= GLSL_SHADER_DEFERRED;
            ...
            // load from VFS files
            glslShaders[n] = shaderHandler->CreateProgramObject(
              "[SMFGroundDrawer::VFS]", names[n], false);
            glslShaders[n]->AttachShaderObject(
              shaderHandler->CreateShaderObject("GLSL/SMFVertProg.glsl",
                                                "#version 120\n" + defs,
                                                GL_VERTEX_SHADER));
            glslShaders[n]->AttachShaderObject(
              shaderHandler->CreateShaderObject("GLSL/SMFFragProg.glsl",
                                                "#version 120\n" + defs,
                                                GL_FRAGMENT_SHADER));
        }
    }
...
{
    if (useLuaShaders) {
        // make sure SH deletes only the wrapper objects; programs are managed
        // by LuaShaders
        for (unsigned int n = GLSL_SHADER_STANDARD; n <= GLSL_SHADER_DEFERRED;
             n++) {
                           ...
                           const LuaMapShaderData* luaMapShaderData)
{
    if (useLuaShaders) {
        ...
        const GL::LightHandler* lightHandler =
          smfGroundDrawer->GetLightHandler();

        const int2 normTexSize = smfMap->GetTextureSize(MAP_BASE_NORMALS_TEX);
        // const int2 specTexSize =
        // smfMap->GetTextureSize(MAP_SSMF_SPECULAR_TEX);

        for (unsigned int n = GLSL_SHADER_STANDARD; n <= GLSL_SHADER_DEFERRED;
             n++) {
            glslShaders[n]->SetFlag("SMF_VOID_WATER", mapInfo->map.voidWater);
            glslShaders[n]->SetFlag("SMF_VOID_GROUND", mapInfo->map.voidGround);
            glslShaders[n]->SetFlag("SMF_SPECULAR_LIGHTING",
                                    (smfMap->GetSpecularTexture() != 0));
            glslShaders[n]->SetFlag("SMF_DETAIL_TEXTURE_SPLATTING",
                                    (smfMap->GetSplatDistrTexture() != 0 &&
                                     smfMap->GetSplatDetailTexture() != 0));
            glslShaders[n]->SetFlag("SMF_DETAIL_NORMAL_TEXTURE_SPLATTING",
                                    (smfMap->GetSplatDistrTexture() != 0 &&
                                     smfMap->HaveSplatNormalTexture()));
            glslShaders[n]->SetFlag("SMF_DETAIL_NORMAL_DIFFUSE_ALPHA",
                                    smfMap->HaveDetailNormalDiffuseAlpha());
            glslShaders[n]->SetFlag("SMF_WATER_ABSORPTION",
                                    smfMap->HasVisibleWater());
            glslShaders[n]->SetFlag("SMF_SKY_REFLECTIONS",
                                    (smfMap->GetSkyReflectModTexture() != 0));
            glslShaders[n]->SetFlag("SMF_BLEND_NORMALS",
                                    (smfMap->GetBlendNormalsTexture() != 0));
            glslShaders[n]->SetFlag("SMF_LIGHT_EMISSION",
                                    (smfMap->GetLightEmissionTexture() != 0));
            glslShaders[n]->SetFlag("SMF_PARALLAX_MAPPING",
                                    (smfMap->GetParallaxHeightTexture() != 0));

            glslShaders[n]->SetFlag("BASE_DYNAMIC_MAP_LIGHT",
                                    lightHandler->GetBaseLight());
            glslShaders[n]->SetFlag("MAX_DYNAMIC_MAP_LIGHTS",
                                    lightHandler->GetMaxLights());

            // both are runtime set in ::Enable, but ATI drivers need values
            // from the beginning
            ...
            glslShaders[n]->SetFlag("DEFERRED_MODE",
                                    (n != GLSL_SHADER_STANDARD));
            glslShaders[n]->SetFlag("GBUFFER_NORMTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_NORMTEX);
            glslShaders[n]->SetFlag("GBUFFER_DIFFTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_DIFFTEX);
            glslShaders[n]->SetFlag("GBUFFER_SPECTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_SPECTEX);
            glslShaders[n]->SetFlag("GBUFFER_EMITTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_EMITTEX);
            glslShaders[n]->SetFlag("GBUFFER_MISCTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_MISCTEX);
            glslShaders[n]->SetFlag("GBUFFER_ZVALTEX_IDX",
                                    GL::GeometryBuffer::ATTACHMENT_ZVALTEX);
            ...
            glslShaders[n]->SetUniform("mapSizePO2",
                                       mapDims.pwr2mapx * SQUARE_SIZE * 1.0f,
                                       mapDims.pwr2mapy * SQUARE_SIZE * 1.0f);
            glslShaders[n]->SetUniform("mapSize",
                                       mapDims.mapx * SQUARE_SIZE * 1.0f,
                                       mapDims.mapy * SQUARE_SIZE * 1.0f);

            glslShaders[n]->SetUniform4v("lightDir",
                                         &sky->GetLight()->GetLightDir()[0]);
            glslShaders[n]->SetUniform3v("cameraPos", &FwdVector[0]);

            glslShaders[n]->SetUniform3v("groundAmbientColor",
                                         &sunLighting->groundAmbientColor[0]);
            glslShaders[n]->SetUniform3v("groundDiffuseColor",
                                         &sunLighting->groundDiffuseColor[0]);
            glslShaders[n]->SetUniform3v("groundSpecularColor",
                                         &sunLighting->groundSpecularColor[0]);
            glslShaders[n]->SetUniform("groundSpecularExponent",
                                       sunLighting->specularExponent);
            glslShaders[n]->SetUniform(
              "groundShadowDensity", sky->GetLight()->GetGroundShadowDensity());

            glslShaders[n]->SetUniform3v("waterMinColor",
                                         &mapInfo->water.minColor[0]);
            glslShaders[n]->SetUniform3v("waterBaseColor",
                                         &mapInfo->water.baseColor[0]);
            glslShaders[n]->SetUniform3v("waterAbsorbColor",
                                         &mapInfo->water.absorb[0]);

            glslShaders[n]->SetUniform4v("splatTexScales",
                                         &mapInfo->splats.texScales[0]);
            glslShaders[n]->SetUniform4v("splatTexMults",
                                         &mapInfo->splats.texMults[0]);

            glslShaders[n]->SetUniform("infoTexIntensityMul", 1.0f);

            glslShaders[n]->SetUniform(
              "normalTexGen",
              1.0f / ((normTexSize.x - 1) * SQUARE_SIZE),
              1.0f / ((normTexSize.y - 1) * SQUARE_SIZE));
            glslShaders[n]->SetUniform("specularTexGen",
                                       1.0f / (mapDims.mapx * SQUARE_SIZE),
                                       1.0f / (mapDims.mapy * SQUARE_SIZE));
            glslShaders[n]->SetUniform("infoTexGen",
...
SMFRenderStateGLSL::HasValidShader(const DrawPass::e& drawPass) const
{
    ...
    switch (drawPass) {
        case DrawPass::TerrainDeferred: {
            shader = glslShaders[GLSL_SHADER_DEFERRED];
        } break;
        default: {
            shader = glslShaders[GLSL_SHADER_CURRENT];
        } break;
    }
...
SMFRenderStateARB::CanEnable(const CSMFGroundDrawer* smfGroundDrawer) const
{
    ...
    return (smfGroundDrawer->UseAdvShading() &&
            !infoTextureHandler->IsEnabled() && shadowHandler->ShadowsLoaded());
}
                          ...
                          const DrawPass::e&)
{
    ...
    if (infoTextureHandler->IsEnabled()) {
        ...
        glMultiTexCoord4f(GL_TEXTURE1_ARB,
                          1.0f,
                          1.0f,
                          1.0f,
                          1.0f); // fix nvidia bug with gltexgen
        glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_RGB_ARB, GL_ADD_SIGNED_ARB);
        glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE_ARB);
        SetTexGen(1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
                  1.0f / (mapDims.pwr2mapy * SQUARE_SIZE),
                  0,
                  0);
        ...
        glMultiTexCoord4f(GL_TEXTURE2_ARB,
                          1.0f,
                          1.0f,
                          1.0f,
                          1.0f); // fix nvidia bug with gltexgen
        ...
        if (smfMap->GetDetailTexture() != 0) {
            ...
            glMultiTexCoord4f(GL_TEXTURE3_ARB,
                              1.0f,
                              1.0f,
                              1.0f,
                              1.0f); // fix nvidia bug with gltexgen
        ...
        SetTexGen(1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
                  1.0f / (mapDims.pwr2mapy * SQUARE_SIZE),
                  -0.5f / mapDims.pwr2mapx,
                  -0.5f / mapDims.pwr2mapy);

        // bind the detail texture a 2nd time to increase the details (->
        // GL_ADD_SIGNED_ARB is limited -0.5 to +0.5) (also do this after the
        // shading texture cause of color clamping issues)
        if (smfMap->GetDetailTexture()) {
            glActiveTexture(GL_TEXTURE3);
            glEnable(GL_TEXTURE_2D);
            glBindTexture(GL_TEXTURE_2D, smfMap->GetDetailTexture());
                          ...
                          const DrawPass::e& drawPass)
{
    ...
    const float3 ambientColor =
      sunLighting->groundAmbientColor * CGlobalRendering::SMF_INTENSITY_MULT;

#ifdef DYNWATER_OVERRIDE_VERTEX_PROGRAM
    // CDynamicWater overrides smfShaderBaseARB during the reflection /
    // refraction pass to distort underwater geometry, but because it's hard to
    // maintain only a vertex shader when working with texture combiners we
    // don't enable this note: we would also want to disable culling for these
    // passes
    ...
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      10,
      1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
      1.0f / (mapDims.pwr2mapy * SQUARE_SIZE),
      0,
      1);
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      12, 1.0f / smfMap->bigTexSize, 1.0f / smfMap->bigTexSize, 0, 1);
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      13,
      -math::floor(camera->GetPos().x * 0.02f),
      -math::floor(camera->GetPos().z * 0.02f),
      0,
      0);
    ...
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      10, ambientColor.x, ambientColor.y, ambientColor.z, 1);
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      11, 0, 0, 0, sky->GetLight()->GetGroundShadowDensity());
                           ...
                           const DrawPass::e&)
{
    ...
    GL::LightHandler* mLightHandler =
      const_cast<GL::LightHandler*>(cLightHandler); // XXX

    glslShaders[GLSL_SHADER_CURRENT]->SetFlag("HAVE_SHADOWS",
                                              shadowHandler->ShadowsLoaded());
    glslShaders[GLSL_SHADER_CURRENT]->SetFlag("HAVE_INFOTEX",
                                              infoTextureHandler->IsEnabled());

    glslShaders[GLSL_SHADER_CURRENT]->Enable();
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform(
      "mapHeights", readMap->GetCurrMinHeight(), readMap->GetCurrMaxHeight());
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform3v("cameraPos",
                                                   &camera->GetPos()[0]);
    glslShaders[GLSL_SHADER_CURRENT]->SetUniformMatrix4x4(
      "shadowMat", false, shadowHandler->GetShadowMatrixRaw());
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform4v(
      "shadowParams", &(shadowHandler->GetShadowParams().x));
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform(
      "infoTexIntensityMul",
      float(infoTextureHandler->GetMode() == "metal") + 1.0f);
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP_ARB,
                  cubeMapHandler->GetSkyReflectionTextureID());
...
SMFRenderStateARB::SetCurrentShader(const DrawPass::e& drawPass)
{
    switch (drawPass) {
        ...
        case DrawPass::WaterReflection: {
            arbShaders[ARB_SHADER_CURRENT] = arbShaders[ARB_SHADER_REFLECT];
        } break;
        case DrawPass::WaterRefraction: {
            arbShaders[ARB_SHADER_CURRENT] = arbShaders[ARB_SHADER_REFRACT];
        } break;
        default: {
        } break;
    }
...
SMFRenderStateGLSL::SetCurrentShader(const DrawPass::e& drawPass)
{
    switch (drawPass) {
        case DrawPass::TerrainDeferred: {
            glslShaders[GLSL_SHADER_CURRENT] =
              glslShaders[GLSL_SHADER_DEFERRED];
        } break;
        default: {
            glslShaders[GLSL_SHADER_CURRENT] =
              glslShaders[GLSL_SHADER_STANDARD];
        } break;
    }
}
...
SMFRenderStateARB::UpdateCurrentShaderSky(const ISkyLight* skyLight) const
{
    arbShaders[ARB_SHADER_CURRENT]->Enable();
    arbShaders[ARB_SHADER_CURRENT]->SetUniform4f(
      11, 0, 0, 0, skyLight->GetGroundShadowDensity());
    arbShaders[ARB_SHADER_CURRENT]->Disable();
}
...
{
    glslShaders[GLSL_SHADER_CURRENT]->Enable();
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform4v("lightDir",
                                                   &skyLight->GetLightDir().x);
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform(
      "groundShadowDensity", skyLight->GetGroundShadowDensity());
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform3v(
      "groundAmbientColor", &sunLighting->groundAmbientColor[0]);
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform3v(
      "groundDiffuseColor", &sunLighting->groundDiffuseColor[0]);
    glslShaders[GLSL_SHADER_CURRENT]->SetUniform3v(
      "groundSpecularColor", &sunLighting->groundSpecularColor[0]);
    glslShaders[GLSL_SHADER_CURRENT]->Disable();
}
