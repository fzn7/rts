  ...
  , groundDrawer(NULL)
{
    ...
    haveSplatDetailDistribTexture = (!mapInfo->smf.splatDetailTexName.empty() &&
                                     !mapInfo->smf.splatDistrTexName.empty());
    haveSplatNormalDistribTexture = false;

    memset(&splatNormalTextures[0],
           0,
           NUM_SPLAT_DETAIL_NORMALS * sizeof(splatNormalTextures[0]));

    for (const std::string& texName : mapInfo->smf.splatDetailNormalTexNames) {
        haveSplatNormalDistribTexture |= !texName.empty();
    }

    // Detail Normal Splatting requires at least one splatDetailNormalTexture
    // and a distribution texture
...
CSMFReadMap::LoadHeightMap()
{
    ...
    const float minHgt = mapInfo->smf.minHeightOverride ? mapInfo->smf.minHeight
                                                        : header.minHeight;
    const float maxHgt = mapInfo->smf.maxHeightOverride ? mapInfo->smf.maxHeight
                                                        : header.maxHeight;
    float* cornerHeightMapSyncedData =
      (cornerHeightMapSynced.empty()) ? NULL : &cornerHeightMapSynced[0];
    float* cornerHeightMapUnsyncedData =
      (cornerHeightMapUnsynced.empty()) ? NULL : &cornerHeightMapUnsynced[0];

    // FIXME:
    //     callchain CReadMap::Initialize --> CReadMap::UpdateHeightMapSynced(0,
    //     0, mapDims.mapx, mapDims.mapy) --> PushVisibleHeightMapUpdate -->
    //     (next UpdateDraw) UpdateHeightMapUnsynced(0, 0, mapDims.mapx,
    //     mapDims.mapy) initializes the UHM a second time merge them some way
    //     so UHM & shadingtex is available from the time readMap got created
    file.ReadHeightmap(cornerHeightMapSyncedData,
                       cornerHeightMapUnsyncedData,
                       minHgt,
                       (maxHgt - minHgt) / 65536.0f);
}
...
CSMFReadMap::LoadMinimap()
{
    ...
    glTexParameteri(
      GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, MINIMAP_NUM_MIPMAP - 1);
    int offset = 0;
...
CSMFReadMap::InitializeWaterHeightColors()
{
    ...
    for (int a = 0; a < 1024; ++a) {
        for (int b = 0; b < 3; ++b) {
            const float absorbColor =
              mapInfo->water.baseColor[b] - mapInfo->water.absorb[b] * a;
            const float clampedColor =
              std::max(mapInfo->water.minColor[b], absorbColor);
            waterHeightColors[a * 4 + b] =
              std::min(255.0f, clampedColor * 255.0f);
        }
...
CSMFReadMap::CreateSpecularTex()
{
    ...
    if (skyReflectModTexBM.Load(mapInfo->smf.skyReflectModTexName)) {
        ...
        skyReflectModTex.SetRawSize(
          int2(skyReflectModTexBM.xsize, skyReflectModTexBM.ysize));
    }

    if (blendNormalsTexBM.Load(mapInfo->smf.blendNormalsTexName)) {
        blendNormalsTex.SetRawTexID(blendNormalsTexBM.CreateTexture());
        blendNormalsTex.SetRawSize(
          int2(blendNormalsTexBM.xsize, blendNormalsTexBM.ysize));
    }

    if (lightEmissionTexBM.Load(mapInfo->smf.lightEmissionTexName)) {
        lightEmissionTex.SetRawTexID(lightEmissionTexBM.CreateTexture());
        lightEmissionTex.SetRawSize(
          int2(lightEmissionTexBM.xsize, lightEmissionTexBM.ysize));
    }
    ...
    if (parallaxHeightTexBM.Load(mapInfo->smf.parallaxHeightTexName)) {
        ...
        parallaxHeightTex.SetRawSize(
          int2(parallaxHeightTexBM.xsize, parallaxHeightTexBM.ysize));
    }
}
...
CSMFReadMap::CreateSplatDetailTextures()
{
    ...
    splatDetailTex.SetRawTexID(
      splatDetailTexBM.CreateTexture(texAnisotropyLevels[true], true));
    splatDetailTex.SetRawSize(
      int2(splatDetailTexBM.xsize, splatDetailTexBM.ysize));

    splatDistrTex.SetRawTexID(
      splatDistrTexBM.CreateTexture(texAnisotropyLevels[true], true));
    splatDistrTex.SetRawSize(
    ...
    for (size_t i = 0; i < mapInfo->smf.splatDetailNormalTexNames.size(); i++) {
        ...
        if (!splatDetailNormalTextureBM.Load(
              mapInfo->smf.splatDetailNormalTexNames[i])) {
            splatDetailNormalTextureBM.channels = 4;
            splatDetailNormalTextureBM.Alloc(1, 1);
            splatDetailNormalTextureBM.mem[0] =
              127; // RGB is packed standard normal map
            splatDetailNormalTextureBM.mem[1] = 127;
            splatDetailNormalTextureBM.mem[2] =
              255; // With a single upward (+Z) pointing vector
            splatDetailNormalTextureBM.mem[3] =
              127; // Alpha is diffuse as in old-style detail textures
        }

        splatNormalTextures[i].SetRawTexID(
          splatDetailNormalTextureBM.CreateTexture(texAnisotropyLevels[true],
                                                   true));
        splatNormalTextures[i].SetRawSize(int2(
          splatDetailNormalTextureBM.xsize, splatDetailNormalTextureBM.ysize));
    }
}
...
CSMFReadMap::CreateGrassTex()
{
    ...
    if (grassShadingTexBM.Load(mapInfo->smf.grassShadingTexName)) {
        ...
        grassShadingTex.SetRawSize(
          int2(grassShadingTexBM.xsize, grassShadingTexBM.ysize));
    }
}
...
CSMFReadMap::CreateDetailTex()
{
    ...
    if (!detailTexBM.Load(mapInfo->smf.detailTexName)) {
        throw content_error("Could not load detail texture from file " +
                            mapInfo->smf.detailTexName);
    }

    detailTex.SetRawTexID(
      detailTexBM.CreateTexture(texAnisotropyLevels[false], true));
...
CSMFReadMap::CreateNormalTex()
{
    ...
    if (!globalRendering->supportNPOTs) {
        normalsTex.SetRawSize(int2(next_power_of_2(mapDims.mapxp1),
                                   next_power_of_2(mapDims.mapyp1)));
    }
...
CSMFReadMap::UpdateVertexNormalsUnsynced(const SRectangle& update)
{
    ...
    for_mt(minz, maxz + 1, [&](const int z) {
        for (int x = minx; x <= maxx; x++) {
            ...
            vn += vtm.cross(vtl) * (zOffT & xOffL);
            assert(vtm.cross(vtl).y >= 0.0f);
            vn += vtr.cross(vtm) * (zOffT);
            assert(vtr.cross(vtm).y >= 0.0f);
            vn += vmr.cross(vtr) * (zOffT & xOffR);
            assert(vmr.cross(vtr).y >= 0.0f);
            vn += vbr.cross(vmr) * (xOffR);
            assert(vbr.cross(vmr).y >= 0.0f);
            vn += vtl.cross(vml) * (xOffL);
            assert(vtl.cross(vml).y >= 0.0f);
            vn += vbm.cross(vbr) * (zOffB & xOffR);
            assert(vbm.cross(vbr).y >= 0.0f);
            vn += vbl.cross(vbm) * (zOffB);
            assert(vbl.cross(vbm).y >= 0.0f);
            vn += vml.cross(vbl) * (zOffB & xOffL);
            assert(vml.cross(vbl).y >= 0.0f);
...
CSMFReadMap::UpdateNormalTexture(const SRectangle& update)
{
    ...
    if (globalRendering->haveGLSL) {
        // texture space is [0 .. mapDims.mapx] x [0 .. mapDims.mapy] (NPOT;
        // vertex-aligned)
        ...
        // Cause the upstreamed float32s need to be transformed to float16s,
        // which seems to happen on the CPU!
#if (SSMF_UNCOMPRESSED_NORMALS == 1)
...
#if (SSMF_UNCOMPRESSED_NORMALS == 1)
                pixels[((z - minz) * xsize + (x - minx)) * 4 + 0] =
                  vertNormal.x;
                pixels[((z - minz) * xsize + (x - minx)) * 4 + 1] =
                  vertNormal.y;
                pixels[((z - minz) * xsize + (x - minx)) * 4 + 2] =
                  vertNormal.z;
                pixels[((z - minz) * xsize + (x - minx)) * 4 + 3] = 1.0f;
#else
                // note: y-coord is regenerated in the shader via "sqrt(1 - x*x
                // - z*z)",
                //   this gives us 2 solutions but we know that the y-coord
                //   always points upwards, so we can reconstruct it in the
                //   shader.
                pixels[((z - minz) * xsize + (x - minx)) * 2 + 0] =
                  vertNormal.x;
                pixels[((z - minz) * xsize + (x - minx)) * 2 + 1] =
                  vertNormal.z;
#endif
            }
        }
...
CSMFReadMap::UpdateShadingTexture(const SRectangle& update)
{
    ...
    // this can be done for diffuse lighting only
    {
        ...
        // enlarge rect by 1pixel in all directions (cause we use center normals
        // and not corner ones)
        ...
        // and our updaterect was already updated (buffered, not send to the GPU
        // yet!) if so update it in that buffer, too
        if (shadingTexUpdateProgress > (y1 * mapDims.mapx + x1)) {
            for (int y = 0; y < ysize; ++y) {
                const int idx = (y + y1) * mapDims.mapx + x1;
                memcpy(
                  &shadingTexBuffer[idx * 4], &pixels[y * xsize * 4], xsize);
            }
        }
...
CSMFReadMap::GetCenterHeightUnsynced(const int x, const int y) const
{
    ...
    const float h = hm[(y)*mapDims.mapxp1 + (x)] +
    ...
    for (int idx = idx1; idx <= idx2; ++idx) {
        ...
        if (height < 0.0f) {
            ...
            const int clampedHeight =
              std::min((int)(-height), int(waterHeightColors.size() / 4) - 1);
            float lightIntensity =
              std::min((DiffuseSunCoeff(xi, yi) + 0.2f) * 2.0f, 1.0f);

            if (height > -10.0f) {
                const float wc = -height * 0.1f;
                const float3 lightColor =
                  GetLightValue(xi, yi) * (1.0f - wc) * 255.0f;

                lightIntensity *= wc;

                dst[i * 4 + 0] =
                  (unsigned char)(waterHeightColors[clampedHeight * 4 + 0] *
                                    lightIntensity +
                                  lightColor.x);
                dst[i * 4 + 1] =
                  (unsigned char)(waterHeightColors[clampedHeight * 4 + 1] *
                                    lightIntensity +
                                  lightColor.y);
                dst[i * 4 + 2] =
                  (unsigned char)(waterHeightColors[clampedHeight * 4 + 2] *
                                    lightIntensity +
                                  lightColor.z);
            } else {
                dst[i * 4 + 0] =
...
{
    float3 light = sunLighting->groundAmbientColor +
...
CSMFReadMap::UpdateShadingTexture()
{
    ...
    // with GLSL, the shading texture has very limited use (minimap etc) so we
    // reduce the updaterate
    ...
    if (shadingTexUpdateProgress >= pixels) {
        ...
        // FIXME use FBO and blend slowly new and old? (this way update rate
        // could reduced even more -> saves CPU time)
        glBindTexture(GL_TEXTURE_2D, shadingTex.GetID());
        glTexSubImage2D(GL_TEXTURE_2D,
                        0,
                        0,
                            ...
                            int extraSize)
{
    if (cam == nullptr) {
        ...
        cam->GetFrustumSides(GetCurrMinHeight() - 100.0f,
                             GetCurrMaxHeight() + 100.0f,
                             SQUARE_SIZE);
    }
    ...
    const std::vector<CCamera::FrustumLine>& negSides =
      cam->GetNegFrustumSides();
    const std::vector<CCamera::FrustumLine>& posSides =
      cam->GetPosFrustumSides();
...
CSMFReadMap::GetInfoMap(const std::string& name, MapBitmapInfo* bmInfo)
{
    ...
    if (!texName.empty() && !infomapBM.LoadGrayscale(texName)) {
        throw content_error("[CSMFReadMap::GetInfoMap] cannot load: " +
                            texName);
    }

    if (!infomapBM.mem.empty()) {
        if (infomapBM.xsize == bmInfo->width &&
            infomapBM.ysize == bmInfo->height) {
            memcpy(data, &infomapBM.mem[0], bmInfo->width * bmInfo->height);
            return data;
        }
        sprintf(failMsg,
                "[CSMFReadMap::GetInfoMap] Invalid image dimensions: %s %ix%i "
                "!= %ix%i",
                texName.c_str(),
                infomapBM.xsize,
                infomapBM.ysize,
...
CSMFReadMap::ConfigureTexAnisotropyLevels()
{
    ...
    for (unsigned int i = 0; i < 2; i++) {
        texAnisotropyLevels[i] =
          std::min(configHandler->GetFloat(cfgKeys[i]), maxAnisotropyLevel);
...
CSMFReadMap::SetLuaTexture(const MapTextureData& td)
{
    const unsigned int num =
      Clamp(int(td.num), 0, NUM_SPLAT_DETAIL_NORMALS - 1);
