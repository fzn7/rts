  ...
  , meshDrawer(NULL)
{
    ...
    meshDrawer = SwitchMeshDrawer((configHandler->GetInt("ROAM") != 0)
                                    ? SMF_MESHDRAWER_ROAM
                                    : SMF_MESHDRAWER_LEGACY);

    smfRenderStates.resize(RENDER_STATE_CNT, nullptr);
    smfRenderStates[RENDER_STATE_SSP] = ISMFRenderState::GetInstance(
      globalRendering->haveARB, globalRendering->haveGLSL, false);
    smfRenderStates[RENDER_STATE_FFP] =
      ISMFRenderState::GetInstance(false, false, false);
    smfRenderStates[RENDER_STATE_LUA] =
      ISMFRenderState::GetInstance(false, true, true);
...
CSMFGroundDrawer::~CSMFGroundDrawer()
{
    ...
    smfRenderStates[RENDER_STATE_FFP]->Kill();
    ISMFRenderState::FreeInstance(smfRenderStates[RENDER_STATE_FFP]);
...
{
    const int curMode = ((dynamic_cast<CRoamMeshDrawer*>(meshDrawer) != nullptr)
                           ? SMF_MESHDRAWER_ROAM
                           : SMF_MESHDRAWER_LEGACY);
...
CSMFGroundDrawer::CreateWaterPlanes(bool camOufOfMap)
{
    ...
    const unsigned char fogColor[4] = { (unsigned char)(255 * sky->fogColor[0]),
    ...
    for (int n = (camOufOfMap) ? 0 : 1; n < 4; ++n) {
        ...
        for (alpha = 0.0f; (alpha - fastmath::PI2) < alphainc;
             alpha += alphainc) {
...
CSMFGroundDrawer::DrawWaterPlane(bool drawWaterReflection)
{
    ...
    glPushMatrix();
    glTranslatef(
      0.f, std::min(-200.0f, smfMap->GetCurrMinHeight() - 400.0f), 0.f);
    glCallList(waterPlaneDispLists[camera->GetPos().IsInBounds() &&
                                   !mapInfo->map.voidWater]);
    glPopMatrix();
}
...
CSMFGroundDrawer::DrawDeferredPass(const DrawPass::e& drawPass, bool alphaTest)
{
...

    {
        ...
        smfRenderStates[RENDER_STATE_SEL]->SetCurrentShader(
          DrawPass::TerrainDeferred);
        smfRenderStates[RENDER_STATE_SEL]->Enable(this,
                                                  DrawPass::TerrainDeferred);
...
CSMFGroundDrawer::Draw(const DrawPass::e& drawPass)
{
    ...
    if (drawDeferred) {
        ...
        DrawDeferredPass(
          drawPass,
          mapInfo->map.voidGround ||
            (mapInfo->map.voidWater && drawPass != DrawPass::WaterReflection));
    }
...
CSMFGroundDrawer::DrawBorder(const DrawPass::e drawPass)
{
    ...
    // SetTexGen(1.0f / (mapDims.pwr2mapx * SQUARE_SIZE), 1.0f /
    // (mapDims.pwr2mapy * SQUARE_SIZE), -0.5f / mapDims.pwr2mapx, -0.5f /
    // mapDims.pwr2mapy);
...
CSMFGroundDrawer::SunChanged()
{
    ...
    // always update, SSMF shader needs current sundir even when shadows are
    // disabled note: only the active state is notified of a given change
...
{
    static const bool drawDeferredAllowed =
