  ...
  : parser(NULL)
{
    ...
    if (f.FileExists()) {
        parser = new LuaParser(
          "mapinfo.lua", SPRING_VFS_MAP_BASE, SPRING_VFS_MAP_BASE);
    } else {
        parser = new LuaParser(
          "maphelper/mapinfo.lua", SPRING_VFS_MAP_BASE, SPRING_VFS_MAP_BASE);
    }
    parser->GetTable("Map");
...
MapParser::GetStartPos(int team, float3& pos) const
{
    ...
    if (!parser->IsValid()) {
        errorLog = "Map-Parser: Failed to get start position for team " +
                   IntToString(team) + ", reason: " + parser->GetErrorLog();
        return false;
    }
    ...
    if (!posTable.IsValid()) {
        errorLog = "Map-Parser: Failed to get start position for team " +
                   IntToString(team) +
                   ", reason: Not defined in the map's config!";
        return false;
    }
...
MapParser::GetRoot()
{
    if (parser) {
        ...
        errorLog = "Map-Parser: Failed to get parser root node, reason: parser "
                   "not ready || file not found.";
        return LuaTable();
    }
}
