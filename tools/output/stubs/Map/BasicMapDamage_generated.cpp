...
CBasicMapDamage::Explosion(const float3& pos, float strength, float radius)
{
    ...
    for (int y = e.y1; y <= e.y2; ++y) {
        for (int x = e.x1; x <= e.x2; ++x) {
            const CSolidObject* so =
              groundBlockingObjectMap->GroundBlockedUnsafe(y * mapDims.mapx +
                                                           x);
            ...
            const float expDist =
              pos.distance2D(float3(x * SQUARE_SIZE, 0, y * SQUARE_SIZE));
            const float relDist = std::min(1.0f, expDist * invRadius);

            const unsigned int tableIdx = relDist * CRATER_TABLE_SIZE;
            const unsigned int ttypeIdx =
              typeMap[(y / 2) * mapDims.hmapx + x / 2];
            ...
            const float prevDif = curHeightMap[y * mapDims.mapxp1 + x] -
            ...
            if (dif < -0.3f && strength > 200.0f)
                grassDrawer->RemoveGrass(
                  float3(x * SQUARE_SIZE, 0.0f, y * SQUARE_SIZE));
    ...
    for (const CUnit* unit : units) {
        ...
        for (int z = unit->mapPos.y; z < unit->mapPos.y + unit->zsize; z++) {
            for (int x = unit->mapPos.x; x < unit->mapPos.x + unit->xsize;
                 x++) {
                // calculate the distance and normalize it
                const float expDist = pos.distance2D(
                  float3(x * SQUARE_SIZE, 0.0f, z * SQUARE_SIZE));
                const float relDist = std::min(1.0f, expDist * invRadius);

                const unsigned int tableIdx = relDist * CRATER_TABLE_SIZE;
                const unsigned int ttypeIdx =
                  typeMap[(z / 2) * mapDims.hmapx + x / 2];

                float dif = baseStrength;
                dif *= craterTable[tableIdx];
                dif *= invHardness[ttypeIdx];

                const float prevDif = curHeightMap[z * mapDims.mapxp1 + x] -
...
CBasicMapDamage::RecalcArea(int x1, int x2, int y1, int y2)
{
    ...
    pathManager->TerrainChange(
      x1, y1, x2, y2, TERRAINCHANGE_DAMAGE_RECALCULATION);
}
