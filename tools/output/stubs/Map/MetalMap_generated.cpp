...
CMetalMap::CMetalMap(const unsigned char* map,
                     int _sizeX,
                     int _sizeZ,
                     float _metalScale)
...
static inline void
ClampInt(int& var, int min, int maxPlusOne)
{
    if (var < min) {
        var = min;
...
float
CMetalMap::GetMetalAmount(int x1, int z1, int x2, int z2)
{
    ClampInt(x1, 0, sizeX);
    ClampInt(x2, 0, sizeX);
    ClampInt(z1, 0, sizeZ);
...
float
CMetalMap::GetMetalAmount(int x, int z)
{
    ClampInt(x, 0, sizeX);
    ClampInt(z, 0, sizeZ);
...
void
CMetalMap::SetMetalAmount(int x, int z, float m)
{
    ClampInt(x, 0, sizeX);
    ClampInt(z, 0, sizeZ);

    distributionMap[(z * sizeX) + x] =
      (metalScale == 0.0f) ? 0 : Clamp((int)(m / metalScale), 0, 255);

    eventHandler.MetalMapChanged(x, z);
}

float
CMetalMap::RequestExtraction(int x, int z, float toDepth)
{
    ClampInt(x, 0, sizeX);
    ClampInt(z, 0, sizeZ);
...
void
CMetalMap::RemoveExtraction(int x, int z, float depth)
{
    ClampInt(x, 0, sizeX);
    ClampInt(z, 0, sizeZ);
...
int
CMetalMap::GetMetalExtraction(int x, int z)
{
    ClampInt(x, 0, sizeX);
    ClampInt(z, 0, sizeZ);
