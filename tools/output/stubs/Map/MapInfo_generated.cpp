...
static void
FIND_MAP_TEXTURE(std::string* filePath, const std::string& defaultDir = "maps/")
{
    if (filePath->empty())
        return;
    if (CFileHandler::FileExists(*filePath, SPRING_VFS_ZIP)) // no RawFS, cause
                                                             // it's also used
                                                             // for synced
                                                             // textures
                                                             // (typemap,
...
CMapInfo::CMapInfo(const std::string& mapInfoFile, const string& mapName)
{
    ...
    // FIXME save all data in an array, so we can destroy the lua context (to
    // save mem)?  delete parser;
}
...
std::string
CMapInfo::GetStringValue(const std::string& key) const
{
...
CMapInfo::ReadAtmosphere()
{
    ...
    atmo.cloudColor =
      atmoTable.GetFloat3("cloudColor", float3(1.0f, 1.0f, 1.0f));
...
CMapInfo::ReadSplats()
{
    ...
    splats.texScales =
      splatsTable.GetFloat4("texScales", float4(0.02f, 0.02f, 0.02f, 0.02f));
    splats.texMults =
      splatsTable.GetFloat4("texMults", float4(1.0f, 1.0f, 1.0f, 1.0f));
}
...
CMapInfo::ReadGrass()
{
    ...
    grass.color =
      grassTable.GetFloat3("bladeColor", float3(0.10f, 0.40f, 0.10f));
...
CMapInfo::ReadLight()
{
    ...
    light.sunDir =
      lightTable.GetFloat4("sunDir", float4(0.0f, 1.0f, 2.0f, FLT_MAX));

    if (light.sunDir.w == FLT_MAX) {
        // if four params are not specified for sundir, fallback to the old
        // three param format
    ...
    light.groundDiffuseColor =
      lightTable.GetFloat3("groundDiffuseColor", float3(0.5f, 0.5f, 0.5f));
    light.groundSpecularColor =
      lightTable.GetFloat3("groundSpecularColor", float3(0.1f, 0.1f, 0.1f));
    light.groundShadowDensity =
      lightTable.GetFloat("groundShadowDensity", 0.8f);

    light.unitAmbientColor =
      lightTable.GetFloat3("unitAmbientColor", float3(0.4f, 0.4f, 0.4f));
    light.unitDiffuseColor =
      lightTable.GetFloat3("unitDiffuseColor", float3(0.7f, 0.7f, 0.7f));
    light.unitSpecularColor =
      lightTable.GetFloat3("unitSpecularColor", light.unitDiffuseColor);
...
CMapInfo::ReadWater()
{
    ...
    water.damage = wt.GetFloat("damage", 0.0f) *
                   ((float)UNIT_SLOWUPDATE_RATE / (float)GAME_SPEED);
    ...
    if (water.normalTexture.empty()) {
        water.normalTexture =
          resGfxMaps.GetString("waternormaltex", "waterbump.png");
    ...
    if (caustics.IsValid()) {
        ...
        for (int i = 0; i < 32; i++) {
            water.causticTextures.push_back(
              IntToString(i, "bitmaps/caustics/caustic%02i.jpg"));
        }
    }
}
...
template<typename T>
static bool
ParseSplatDetailNormalTexture(const LuaTable& table,
                              const T key,
...
CMapInfo::ReadSMF()
{
    ...
    if (smf.detailTexName.empty()) {
        const LuaTable& resGfxMaps =
          resRoot->SubTable("graphics").SubTable("maps");
    ...
    if (sdnTexTable.IsValid()) {
        ...
        for (int i = 0; true; i++) {
            if (!ParseSplatDetailNormalTexture(
                  sdnTexTable, i + 1, smf.splatDetailNormalTexNames)) {
                break;
            }
        }
    } else {
        smf.splatDetailNormalDiffuseAlpha =
          mapResTable.GetBool("splatDetailNormalDiffuseAlpha", false);

        for (int i = 0; true; i++) {
            if (!ParseSplatDetailNormalTexture(mapResTable,
                                               "splatDetailNormalTex" +
                                                 IntToString(i + 1),
                                               smf.splatDetailNormalTexNames)) {
                break;
            }
        }
    }
...
CMapInfo::ReadPFSConstants()
{
    ...
    qtpfsConsts.minSpeedModVal =
      std::max(0.0f, qtpfsTable.GetFloat("minSpeedModVal", 0.0f));
    qtpfsConsts.maxSpeedModVal = std::max(
      qtpfsConsts.minSpeedModVal, qtpfsTable.GetFloat("maxSpeedModVal", 2.0f));
}
...
CMapInfo::ReadSound()
{
    ...
    std::map<std::string, EAXSfxProps>::const_iterator et =
      eaxPresets.find(presetname);
    ...
    for (it = nameToALFilterParam.begin(); it != nameToALFilterParam.end();
         ++it) {
        ...
        switch (type) {
            case EFXParamTypes::FLOAT:
                if (luaType == LuaTable::NUMBER)
                    efxprops->filter_properties_f[param] =
                      filterTable.GetFloat(name, 0.f);
                break;
        }
    }
        ...
        switch (type) {
            case EFXParamTypes::VECTOR:
                if (luaType == LuaTable::TABLE)
                    efxprops->properties_v[param] =
                      filterTable.GetFloat3(name, ZeroVector);
                break;
            case EFXParamTypes::FLOAT:
                if (luaType == LuaTable::NUMBER)
                    efxprops->properties_f[param] =
                      filterTable.GetFloat(name, 0.f);
                break;
            case EFXParamTypes::BOOL:
                if (luaType == LuaTable::BOOLEAN)
                    efxprops->properties_i[param] =
                      filterTable.GetBool(name, false);
                break;
        }
    }
#endif
