...
void
CMapGenerator::Generate()
{
    // loadscreen->SetLoadMessage("Generating map");

    // Create archive for map
    std::string mapArchivePath =
      setup->mapName + "." + virtualArchiveFactory->GetDefaultExtension();
    CVirtualArchive* archive =
      virtualArchiveFactory->AddArchive(setup->mapName);
...
{
    file->buffer.insert(
      file->buffer.end(), (boost::uint8_t*)data, (boost::uint8_t*)data + size);
}
...
CMapGenerator::SetToBuffer(CVirtualFile* file,
                           ...
                           int position)
{
    std::copy((boost::uint8_t*)data,
              (boost::uint8_t*)data + size,
              file->buffer.begin() + position);
}
...
CMapGenerator::GenerateSMF(CVirtualArchive* archive)
{
    ...
    const int numSmallTiles =
      1; // 2087; //32 * 32 * (mapSize.x  / 2) * (mapSize.y / 2);
    ...
    int tilemapTotalSize = sizeof(MapTileHeader) + sizeof(numSmallTiles) +
                           sizeof(smtFileName) + tilemapSize;
    ...
    smfHeader.metalmapPtr =
      smfHeader.tilesPtr +
      tilemapTotalSize; // smfHeader.minimapPtr + minimapSize;
    ...
    float heightMul =
      (float)0xFFFF / (smfHeader.maxHeight - smfHeader.minHeight);
...
CMapGenerator::GenerateMapInfo(CVirtualArchive* archive)
{
    ...
    if (!fh.FileExists()) {
        throw content_error("Error generating map: " + luaTemplate +
                            " not found");
    }
    ...
    for (size_t x = 0; x < startPositions.size(); x++) {
        ss << "[" << x << "] = {startPos = {x = " << startPositions[x].x
           << ", z = " << startPositions[x].y << "}},";
    }
...
CMapGenerator::GenerateSMT(CVirtualArchive* archive)
{
    ...
    smtHeader.version = 1;
    smtHeader.numTiles = 1; // 32 * 32 * (generator->GetMapSize().x * 32) *
                            // (generator->GetMapSize().y * 32);
    ...
    if (errorcode != GL_NO_ERROR) {
        throw content_error(
          "Error generating map - texture generation not supported");
    }
    ...
    fileSMT->buffer.resize(fileSMT->buffer.size() +
                           smtHeader.numTiles * SMALL_TILE_SIZE);
