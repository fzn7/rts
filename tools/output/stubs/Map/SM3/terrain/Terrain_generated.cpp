             ...
             int d)
{
    ...
    if (hm->highDetail) {
        for (int a = 0; a < 4; a++) {
            ...
            int2 sqPos(sqStart.x + (a & 1) * w / 2,
                       sqStart.y + (a & 2) * w / 4); // square pos
            int2 hmPos(hmStart.x * 2 + (a & 1) * QUAD_W,
                       hmStart.y * 2 + (a & 2) * QUAD_W / 2); // heightmap pos
            int2 cqPos(quadPos.x * 2 + (a & 1),
                       quadPos.y * 2 + (a & 2) / 2); // child quad pos

            children[a]->Build(
              hm->highDetail, sqPos, hmPos, cqPos, w / 2, d + 1);
        }
    }
...
TQuad::Draw(IndexTable* indexTable, bool onlyPositions, int lodState)
{
    ...
    glDrawElements(GL_TRIANGLES,
                   indexTable->size[lodState],
                   indexTable->IndexType(),
                   ibuf.Bind());
    ibuf.Unbind();
    ...
    if (!onlyPositions) {
        ...
        if (vda & VRT_TangentSpaceMatrix)
            textureSetup->currentShaderSetup->UnbindTSM();
    }
                                    ...
                                    int maxdepth)
{
    if (depth < maxdepth) {
        for (int a = 0; a < 4; a++) {
            TQuad* r =
              children[a]->FindSmallestContainingQuad2D(pos, range, maxdepth);
            if (r)
                return r;
        }
    }
...
Terrain::CheckNabourLod(TQuad* q, int xOfs, int yOfs)
{
    ...
    // Parent: a child node of the nabour is drawn, which will take care of LOD
    // gaps fixing Queued: drawn itself, so the index buffer can fix gaps for
    // that Culled: no gap fixing required
}
...
Terrain::UpdateLodFix(TQuad* q)
{
    ...
    if (q->drawState == TQuad::Parent) {
        ...
        // find the nabours, and make sure at least them or their parents are
        // drawn
        TQuad* parent = q->parent;
        QuadMap* qm = qmaps[parent->depth];
        if (parent->qmPos.x > 0)
            CheckNabourLod(parent, -1, 0);
        if (parent->qmPos.y > 0)
            CheckNabourLod(parent, 0, -1);
        if (parent->qmPos.x < qm->w - 1)
            CheckNabourLod(parent, 1, 0);
        if (parent->qmPos.y < qm->w - 1)
            CheckNabourLod(parent, 0, 1);
    }
}
...
Terrain::QuadVisLod(TQuad* q)
{
    if (q->InFrustum(&frustum)) {
        // Node is visible, now determine if this LOD is suitable, or that a
        // higher LOD should be used.
        float lod = config.detailMod * q->CalcLod(curRC->cam->pos);
        if (q->depth < quadTreeDepth - config.maxLodLevel ||
            (lod > 1.0f && !q->isLeaf())) {
            q->drawState = TQuad::Parent;
            for (int a = 0; a < 4; a++)
                QuadVisLod(q->children[a]);
        } else
            q->drawState = TQuad::Queued;
...
Terrain::Update()
{
    ...
    for (size_t ctx = 0; ctx < contexts.size(); ctx++) {
        ...
        //	frustum.CalcCameraPlanes(&camera->GetPos(), &camera->right,
        //&camera->up, &camera->front, camera->fov, camera->aspect);
        // assert(camera->GetPos().x == 0.0f ||
        // frustum.IsPointVisible(camera->GetPos() + camera->front *
        // 20)==Frustum::Inside);

        // determine the new set of quads to draw
        QuadVisLod(quadtree);

        // go through nabours to make sure there is a maximum of one LOD
        // difference between nabours
        ...
        for (size_t a = 0; a < updatequads.size(); a++) {
            if (updatequads[a]->drawState == TQuad::Queued) {
                ...
                int ls = 0;
                if (q->qmPos.x > 0 &&
                    qm->At(q->qmPos.x - 1, q->qmPos.y)->drawState ==
                      TQuad::NoDraw)
                    ls |= left_bit;
                if (q->qmPos.y > 0 &&
                    qm->At(q->qmPos.x, q->qmPos.y - 1)->drawState ==
                      TQuad::NoDraw)
                    ls |= up_bit;
                if (q->qmPos.x < qm->w - 1 &&
                    qm->At(q->qmPos.x + 1, q->qmPos.y)->drawState ==
        ...
        // the active set of quads is determined, so their draw-states can be
        ...
        for (size_t a = 0; a < rc->quads.size(); a++) {
            ...
            if (!q->renderData) {
                ...
                if (config.terrainNormalMaps && rc->needsNormalMap)
                    renderDataManager->InitializeNodeNormalMap(
                      q, config.normalMapLevel);
...
DrawNormals(TQuad* q, const Heightmap* hm)
{
    ...
    for (int y = q->hmPos.y; y < q->hmPos.y + QUAD_W; y++)
        for (int x = q->hmPos.x; x < q->hmPos.x + QUAD_W; x++) {
            Vector3 origin(
              x * hm->squareSize, hm->atSynced(x, y), y * hm->squareSize);
    ...
    for (size_t a = 0; a < ctx->quads.size(); a++) {
        ...
        if (q->cacheTexture)
            stats.cacheTextureSize +=
              config.cacheTextureSize * config.cacheTextureSize * 3;
    ...
    if (fr != NULL) {
        ...
        if (debugQuad != NULL) {
            fr->printf(0,
                       30,
                       s,
                       "Selected quad: (%d,%d) on depth %d. Lod=%3.3f",
                       debugQuad->qmPos.x,
                       debugQuad->qmPos.y,
                       debugQuad->depth,
                       config.detailMod *
                         debugQuad->CalcLod(activeRC->cam->pos));
        }
        ...
        fr->printf(0,
                   46,
                   s,
                   "Rendered nodes: %d, tris: %d, VBufSize: %d(kb), "
                   "TotalRenderData(kb): %d, DetailMod: %g, "
                   "CacheTextureMemory: %d",
                   activeRC->quads.size(),
                   stats.tris,
                   VertexBuffer::TotalSize() / 1024,
                   stats.renderDataSize / 1024,
                   config.detailMod,
                   stats.cacheTextureSize);
        fr->printf(0,
                   60,
                   s,
                   "NodeUpdateCount: %d, RenderDataAlloc: %d, #RenderData: %d",
                   nodeUpdateCount,
                   renderDataManager->normalDataAllocates,
...
{
    if (cpos.x >= q->start.x && cpos.z >= q->start.z && cpos.x < q->end.x &&
        cpos.z < q->end.z) {
        if (q->isLeaf())
            return q;
...
Terrain::RenderNode(TQuad* q)
{
    ...
    for (size_t p = 0; p < q->textureSetup->renderSetup[0]->passes.size();
         p++) {
...
Terrain::Load(const TdfParser& tdf, LightingInfo* li, ILoadCallback* cb)
{
    ...
    // prev is now the lowest detail heightmap, and will be used for the root
    // quad tree node
    quadtree = new TQuad;
...
Terrain::LoadHeightMap(const TdfParser& parser, ILoadCallback* cb)
{
    ...
    const float hmScale =
      atof(parser.SGetValueDef("1000", "MAP\\TERRAIN\\HeightScale").c_str());
    const float hmOffset =
      atof(parser.SGetValueDef("0", "MAP\\TERRAIN\\HeightOffset").c_str());

    // apply scaling and offset to the heightmap ASAP
    for (int y = 0; y < heightmap->w * heightmap->h; y++) {
        heightmap->dataSynced[y] =
          heightmap->dataSynced[y] * hmScale + hmOffset;
        heightmap->dataUnsynced[y] = heightmap->dataSynced[y];
    }

    if ((GetHeightmapWidth() - 1) !=
          atoi(parser.SGetValueDef("", "MAP\\GameAreaW").c_str()) ||
        (GetHeightmapHeight() - 1) !=
          atoi(parser.SGetValueDef("", "MAP\\GameAreaH").c_str())) {
        char hmdims[32];
        SNPRINTF(
          hmdims, 32, "%dx%d", GetHeightmapWidth(), GetHeightmapHeight());
        throw content_error("Map size (" + std::string(hmdims) +
                            ") should be equal to GameAreaW and GameAreaH");
    }
}
...
FindRAWProps(int len, int& width, int& bytespp, ILoadCallback* cb)
{
    ...
    if (w * w * 2 != len) {
        ...
        if (w * w != len) {
            cb->PrintMsg("Incorrect raw file size: %d, last comparing size: %d",
                         len,
                         w * w * 2);
            return;
        } else
                                ...
                                ILoadCallback* cb)
{
    ...
    if ((testw > hmWidth) || (hmWidth != hmHeight)) {
        cb->PrintMsg(
          "Heightmap %s has wrong dimensions (should be 129x129,257x257...)",
          hmfile);
        ilDeleteImages(1, &ilheightmap);
        return NULL;
    }
    ...
    if (!ilConvertImage(IL_LUMINANCE, IL_UNSIGNED_SHORT)) {
        cb->PrintMsg(
          "Failed to convert heightmap image (%s) to grayscale image.", hmfile);
        ilDeleteImages(1, &ilheightmap);
        return NULL;
    }
...
{
    return heightmap->dataSynced;
}
std::vector<float>&
Terrain::GetCornerHeightMapUnsynced()
{
    return heightmap->dataUnsynced;
}
