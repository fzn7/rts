...
ShowInfoLog(GLhandleARB handle)
{
    ...
    glGetObjectParameterivARB(
      handle, GL_OBJECT_INFO_LOG_LENGTH_ARB, &infoLogLen);
...
struct Shader
{
    ...
    void Build(GLenum shaderType)
    {
        ...
        int index = 0;
        for (std::list<std::string>::iterator i = texts.begin();
             i != texts.end();
             ++i, index++) {
        ...
        glShaderSourceARB(
          handle, strings.size(), &strings.front(), &lengths.front());
        glCompileShaderARB(handle);

        // Check compile status and show info log if failed
        GLint isCompiled;
        glGetObjectParameterivARB(
          handle, GL_OBJECT_COMPILE_STATUS_ARB, &isCompiled);
        if (!isCompiled) {
...
class BufferTexture : public BaseTexture
{
  public:
    BufferTexture()
    {
        // ATI has GL_EXT_texture_rectangle, but that has no support for GLSL
        // texture2DRect nVidia: Use RECT,  ati: use POT
        ...
        glTexImage2D(
          target, 0, 4, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);

        framebuffer.Bind();
        framebuffer.CreateRenderBuffer(
          GL_DEPTH_ATTACHMENT_EXT, GL_DEPTH_COMPONENT24, width, height);
...
struct ShaderBuilder
{
    ...
    {
        return renderSetup->shaderDef.useShadowMapping;
    }
    ...
    NodeGLSLShader* EndPass(ShaderDef* sd,
                            const std::string& operations,
                            uint passIndex = 0);
    void BuildFragmentShader(NodeGLSLShader* ns,
                             uint passIndex,
                             const std::string& operations,
                             ShaderDef* sd);
    void BuildVertexShader(NodeGLSLShader* ns, uint passIndex, ShaderDef* sd);
    bool ProcessStage(std::vector<ShaderDef::Stage>& stages,
                      uint& index,
                      std::string& opstr);
    ...
    TexReq CalcStagesTexReq(const std::vector<ShaderDef::Stage>& stages,
...
// coordinates
ShaderBuilder::ShadingMethod
ShaderBuilder::CalculateShadingMethod(ShaderDef* sd) const
{
    ...
    LOG(
      "\t    hwmax.units=%2d,     hwmax.coords=%2d", hwmax.units, hwmax.coords);
    LOG("\tdiffuseRQ.units=%2d, diffuseRQ.coords=%2d",
        diffuseRQ.units,
        diffuseRQ.coords);
    LOG("\tbumpmapRQ.units=%2d, bumpmapRQ.coords=%2d",
        bumpmapRQ.units,
        bumpmapRQ.coords);
    LOG("\t  special.units=%2d,   special.coords=%2d",
        special.units,
        special.coords);
    ...
    LOG("\t  special.units=%2d,   special.coords=%2d",
        ...
        special.coords);
    LOG(
      "\t    total.units=%2d,     total.coords=%2d", total.units, total.coords);

    // diffuse + bumpmap in one pass?
    if (total.Fits(hwmax)) {
        LOG("\tnormalMapStages.size()=" _STPF_ ", SM_DiffuseBumpmapSP",
            sd->normalMapStages.size());
    ...
    if (diffuseRQ.Fits(hwmax) && (bumpmapRQ + special).Fits(hwmax)) {
        LOG("\tnormalMapStages.size()=" _STPF_ ", SM_DiffuseBumpmapMP",
            sd->normalMapStages.size());
    ...
    while (index < stages.size()) {
        ...
        if (stage.operation == ShaderDef::Alpha) {
            ...
            assert(index < stages.size() - 1 &&
                   stages[index + 1].operation == ShaderDef::Blend);
...
ShaderBuilder::GenTextureRead(int tu, int tc)
{
    char tcstr[6];
    sprintf(tcstr, "%d", tc);
    return "texture2D(" + texUsage.texUnits[tu]->name + ", gl_TexCoord[" +
           tcstr + "].st)";
}
                       ...
                       uint passIndex)
{
    ...
    GLint isLinked;
    glGetObjectParameterivARB(
      nodeShader->program, GL_OBJECT_LINK_STATUS_ARB, &isLinked);
    ...
    for (size_t a = 0; a < nodeShader->texUnits.size(); a++) {
        ...
        GLint location =
          glGetUniformLocationARB(nodeShader->program, tex->name.c_str());
    ...
    if (shadingMethod != SM_DiffuseSP &&
        ...
        nodeShader->tsmAttrib =
          glGetAttribLocationARB(nodeShader->program, "TangentSpaceMatrix");
        nodeShader->wsLightDirLocation =
          glGetUniformLocationARB(nodeShader->program, "wsLightDir");
        nodeShader->wsEyePosLocation =
          glGetUniformLocationARB(nodeShader->program, "wsEyePos");
    }

    if (ShadowMapping()) {
        nodeShader->shadowMapLocation =
          glGetUniformLocationARB(nodeShader->program, "shadowMap");
        nodeShader->shadowParamsLocation =
          glGetUniformLocationARB(nodeShader->program, "shadowParams");
        nodeShader->shadowMatrixLocation =
          glGetUniformLocationARB(nodeShader->program, "shadowMatrix");
    }

    if (passIndex == 1) {
        // set up uniform to read bumpmap
        GLint invScreenDim =
          glGetUniformLocationARB(nodeShader->program, "invScreenDim");
        glUniform2fARB(invScreenDim,
...
ShaderBuilder::AddPPDefines(ShaderDef* sd, Shader& shader, uint passIndex)
{
    ...
    shader.texts.push_back(std::string("const float specularExponent = ") +
                           specularExponentStr + ";\n");
}
                                   ...
                                   ShaderDef* sd)
{
    ...
    std::string gentxt =
      "vec4 CalculateColor()  { vec4 color; float curalpha; \n" + operations;
                                 ...
                                 ShaderDef* sd)
{
    ...
    for (size_t a = 0; a < ns->texCoordGen.size(); a++) {
        ...
        SNPRINTF(buf,
                 buf_sizeMax,
                 "gl_TexCoord[" _STPF_
                 "].st = vec2(dot(gl_Vertex, gl_ObjectPlaneS[" _STPF_
                 "]), dot(gl_Vertex,gl_ObjectPlaneT[" _STPF_ "]));\n",
                 a,
                 a,
                 a);
        tcgen += buf;
                            ...
                            std::string& opstr)
{
    ...
    if (index == 0) { // replace
        ...
        int blendTC =
          tmpUsage.AddTextureCoordRead(hwmax.coords, blendStage.source);
        ...
        opstr += "color = mix(color, " + GenTextureRead(blendTU, blendTC) +
                 ", curalpha);\n";
...
ShaderBuilder::Build(ShaderDef* shaderDef)
{
    ...
    switch (shadingMethod) {
        ...
        case SM_DiffuseBumpmapSP: {
            ...
            EndPass(shaderDef,
                    diffusecode + "vec4 diffuse = color;\n" + bumpmapcode);
            break;
        }
        ...
        case SM_Impossible:
            throw content_error(
              "Map has too many layers for bumpmapping on this hardware");
    }
}
    ...
    if (tsmAttrib >= 0) {
        for (int a = 0; a < 3; a++) {
            ...
            glVertexAttribPointerARB(
              tsmAttrib + a, 3, GL_FLOAT, 0, vertexSize, buf + a);
        }
    }
}
...
NodeGLSLShader::Setup(NodeSetupParams& params)
{
    ...
    */
    ...
    if (wsLightDirLocation >= 0 && params.wsLightDir)
        glUniform3fARB(wsLightDirLocation,
                       params.wsLightDir->x,
                       params.wsLightDir->y,
                       params.wsLightDir->z);
    if (wsEyePosLocation >= 0 && params.wsEyePos)
        glUniform3fARB(wsEyePosLocation,
                       params.wsEyePos->x,
                       params.wsEyePos->y,
                       params.wsEyePos->z);
    ...
    if (params.shadowMapParams) {
        if (shadowMapLocation >= 0) {
            ...
            shadowHandler->SetupShadowTexSampler(
              GL_TEXTURE0 +
              texUnits.size() /*, params.shadowMapParams->shadowMap*/);
        }

        ShadowMapParams& smp = *params.shadowMapParams;
        if (shadowMatrixLocation >= 0)
            glUniformMatrix4fvARB(
              shadowMatrixLocation, 1, GL_TRUE, smp.shadowMatrix);
        if (shadowParamsLocation >= 0)
            glUniform4fARB(
              shadowParamsLocation, smp.f_a, smp.f_b, smp.mid[0], smp.mid[1]);
    }
}
...
NodeGLSLShader::Cleanup()
{
    for (size_t a = 0; a < texUnits.size(); a++) {
        glActiveTextureARB(GL_TEXTURE0_ARB + a);
        glBindTexture(
          texUnits[a]->IsRect() ? GL_TEXTURE_RECTANGLE_ARB : GL_TEXTURE_2D, 0);
        glDisable(texUnits[a]->IsRect() ? GL_TEXTURE_RECTANGLE_ARB
                                        : GL_TEXTURE_2D);
    }
...
void
GLSLShaderHandler::BeginPass(const std::vector<Blendmap*>& blendmaps,
                             const std::vector<TiledTexture*>& textures,
                             int pass)
{
    if (buffer) {
        if ((buffer->width != globalRendering->viewSizeX) ||
            (buffer->height != globalRendering->viewSizeY)) {
            delete buffer;
            buffer = NULL; // to prevent a dead-pointer in case of an
                           // out-of-memory exception on the next line
            buffer = new BufferTexture;
        }
    }
    if (buffer) {
