...
RenderSetup::DebugOutput()
{
    for (size_t a = 0; a < passes.size(); a++) {
        ...
        std::string shaderstr =
          p.shaderSetup ? p.shaderSetup->GetDebugDesc() : std::string("none");
        LOG_L(L_DEBUG,
              "Pass (" _STPF_ "): %s, %s. Shader: %s",
              a,
              str,
...
BaseTexture*
TerrainTexture::LoadImageSource(const std::string& name,
                                ...
                                bool isBumpmap)
{
    ...
    if (!!atoi(tdfParser->SGetValueDef("0", basepath + name + "\\Blendmap")
                 .c_str())) {
        if (isBumpmap)
                     ...
                     LightingInfo* li)
{
    ...
    if (cb)
        cb->PrintMsg("  parsing texture stages...");
    ...
    optimizeEpsilon =
      atof(tdf->SGetValueDef("0.04", basepath + "LayerOptimizeConst").c_str());
    if (optimizeEpsilon < 0.0f)
        optimizeEpsilon = 0.0f;
    ...
    if (cb)
        cb->PrintMsg("  loading textures and blendmaps...");
    ...
    for (size_t a = 0; a < shaderDef.stages.size(); a++) {
        ...
        if (nametbl.find(st->sourceName) == nametbl.end())
            nametbl[st->sourceName] =
              LoadImageSource(st->sourceName, basepath, heightmap, cb, cfg);
    ...
    if (cfg->useBumpMaps && hasBumpmaps) {
        for (size_t a = 0; a < shaderDef.normalMapStages.size(); a++) {
            ...
            if (st->operation != ShaderDef::Alpha)
                name += "_BM"; // make it unique, this whole naming thing is
                               // very hackish though..

            // Already loaded?
            if (nametbl.find(name) == nametbl.end()) {
                // load a bumpmap unless it's an alpha operation
                st->source = LoadImageSource(st->sourceName,
                                             basepath,
                                             heightmap,
                                             cb,
                                             cfg,
                                             st->operation != ShaderDef::Alpha);
                if (!st->source) {
                    if (!flatBumpmap) {
        ...
        cfg->useBumpMaps = false;

    // Generate blendmap mipmaps
    std::deque<AlphaImage*>* bmMipmaps =
      new std::deque<AlphaImage*>[blendMaps.size()];
    GenerateInfo gi;
    gi.bmMipmaps = bmMipmaps;

    if (cb) {
        cb->PrintMsg("  generating blendmap mipmaps...");
    }
    ...
    if (cb)
        cb->PrintMsg("  loading blendmaps into OpenGL...");
    ...
    if (!cfg->forceFallbackTexturing && GLEW_ARB_fragment_shader &&
    ...
    if (cfg->useStaticShadow) {
        if (cb)
            cb->PrintMsg("  calculating lightmap");
        lightmap = new Lightmap(heightmap, 2, 1, li);
    }

    // see how lighting should be implemented, based on config and available
    // textures
    InstantiateShaders(cfg, cb);

    if (cb) {
        cb->PrintMsg("  initializing terrain node shaders...");
    }
    ...
    // count passes
    maxPasses = 0;
    for (std::map<uint, RenderSetupCollection*>::iterator mi =
           gi.nodesetup.begin();
         mi != gi.nodesetup.end();
         ++mi) {
    ...
    if (cb)
        cb->PrintMsg("  deleting temporary blendmap data...");

    // Free blendmap mipmap images
    for (size_t a = 0; a < blendMaps.size(); a++) {
        for (std::deque<AlphaImage*>::iterator i = bmMipmaps[a].begin();
             i != bmMipmaps[a].end();
             ++i) {
            delete *i;
        }
...
TerrainTexture::CreateTexProg(TQuad* node, TerrainTexture::GenerateInfo* gi)
{
    ...
    for (size_t a = 0; a < blendMaps.size(); a++) {
        ...
        // scaling divisor, needed because the blendmap maybe lower res than the
        // corresponding part of heightmap
        int d = ((1 << node->depth) * QUAD_W) / mipmaps[mipIndex]->w;

        // Calculate "constants" representing the blendmap in the area that the
        // node covers
        blendMaps[a]->curAreaResult =
          mipmaps[mipIndex]->TestArea((node->qmPos.x * QUAD_W) / d,
                                      (node->qmPos.y * QUAD_W) / d,
                                      ((node->qmPos.x + 1) * QUAD_W) / d,
                                      ((node->qmPos.y + 1) * QUAD_W) / d,
                                      optimizeEpsilon);
    }

    uint key = CalcBlendmapSortKey();
    if (gi->nodesetup.find(key) == gi->nodesetup.end()) {
        RenderSetupCollection* tns = gi->nodesetup[key] =
          new RenderSetupCollection;
        ...
        for (size_t a = 0; a < shaders.size(); a++) {
            ...
            for (size_t p = 0; p < tns->renderSetup[a]->passes.size(); p++) {
                if (tns->renderSetup[a]->passes[p].shaderSetup)
                    vda |= tns->renderSetup[a]
                             ->passes[p]
                             .shaderSetup->GetVertexDataRequirements();
            }
        }

        // The unlit shader and far shader have to be supported, near shader is
        // optional
        if (!tns->renderSetup[unlitShader.index] ||
            !tns->renderSetup[farShader.index])
...
TerrainTexture::SetupNode(TQuad* q, int pass)
{
    ...
    if (q->renderData->normalMap) {
        ...
        ns->currentShaderSetup->GetTextureUnits(
          &detailBumpmap, imageUnit, coordUnit);
...
TerrainTexture::DebugEvent(const std::string& event)
{
    if (event == "t_prev_shader") {
        debugShader--;
        if (debugShader < -1)
            debugShader = texNodeSetup.size() - 1;
    }
...
TerrainTexture::DebugPrint(IFontRenderer* fontRenderer)
{
    if (fontRenderer != NULL) {
        fontRenderer->printf(0,
                             75,
                             16.0f,
                             "Numpasses: %d, curshader=%d, texture units: %d, "
                             "sundir(%1.3f,%1.3f,%1.3f)",
                             maxPasses,
                             debugShader,
                             shaderHandler->MaxTextureUnits(),
                             wsLightDir.x,
                             wsLightDir.y,
                             wsLightDir.z);
    }
                      ...
                      std::vector<ShaderDef::Stage>& stages)
{
    for (int a = 0; a < numStages; a++) {
        ...
        if (operation == Blend) {
            ...
            stages.back().sourceName =
              tdf.SGetValueDef(std::string(), ts + "blender");
            stages.back().operation = Alpha;
        }

        stages.push_back(Stage());
        stages.back().sourceName =
          tdf.SGetValueDef(std::string(), ts + "source");
...
ShaderDef::Parse(const TdfParser& tdf, bool needNormalMap)
{
    ...
    int numStages =
      atoi(tdf.SGetValueDef("0", path + "NumTextureStages").c_str());
    bool autoBumpMap =
      !!atoi(tdf.SGetValueDef("1", path + "AutoBumpmapStages").c_str());
    specularExponent =
      atof(tdf.SGetValueDef("8", path + "SpecularExponent").c_str());
    ...
    if (needNormalMap) {
        ...
        if (autoBumpMap) {
            ...
            numStages =
              atoi(tdf.SGetValueDef("0", path + "NumBumpmapStages").c_str());
...
ShaderDef::OptimizeStages(std::vector<Stage>& src, std::vector<Stage>& dst)
{
    ...
    // create an optimized shader definition based on replacing the blendmap
    // with a constant
    for (size_t a = 0; a < src.size(); a++) {
        Stage& st = src[a];
        if (a == 0)
            atr = AlphaImage::AREA_MIXED;
        switch (st.operation) {
            case Blend: // previous * (1-alpha) + source * alpha
                if (atr == AlphaImage::AREA_MIXED)
                    ...
                    for (int i = dst.size() - 1;
                         i >= 0 && dst[i].operation == Alpha;
                         i--)
                        dst.pop_back();
                }
                break;
