                   ...
                   LightingInfo* li)
{
    ...
    for (int y = 0; y < w; y++)
        for (int x = 0; x < w; x++) {
            centerhm[y * w + x] =
              /* hm->scale * */ 0.25f *
              ((int)hm->atSynced(x, y) + (int)hm->atSynced(x + 1, y) +
               (int)hm->atSynced(x, y + 1) +
               (int)hm->atSynced(x + 1, y + 1)); //+ hm->offset;
            shading[y * w + x] = li->ambient;
        }

    uchar* lightMap = new uchar[shadowW * shadowW];
    for (std::vector<StaticLight>::const_iterator l = li->staticLights.begin();
         l != li->staticLights.end();
         ++l) {
        float lightx;
        float lighty;
        ...
        for (int y = 0; y < w; y++) {
            for (int x = 0; x < w; x++) {
                ...
                if (l->directional)
                    ...
                    wp = l->position - Vector3((x + 0.5f) * hm->squareSize,
                                               centerhm[y * w + x],
                                               (y + 0.5f) * hm->squareSize);

                const uchar* normal = hm->GetNormal(x, y);
                Vector3 normv((2 * (int)normal[0] - 256) / 255.0f,
                              (2 * (int)normal[1] - 256) / 255.0f,
                              (2 * (int)normal[2] - 256) / 255.0f);
                ...
                if (dot > 1.0f)
                    dot = 1.0f;
                dot *=
                  lightMap[(y * shadowW + x) / shadowScale] * (1.0f / 255.0f);
    ...
    for (int y = 0; y < w; y++) {
        for (int x = 0; x < w; x++) {
            shadingTexData[(y * w + x) * 4 + 0] =
              (uchar)(min(1.0f, shading[y * w + x].x) * 255);
            shadingTexData[(y * w + x) * 4 + 1] =
              (uchar)(min(1.0f, shading[y * w + x].y) * 255);
            shadingTexData[(y * w + x) * 4 + 2] =
              (uchar)(min(1.0f, shading[y * w + x].z) * 255);
            shadingTexData[(y * w + x) * 4 + 3] =
              CReadMap::EncodeHeight(centerhm[w * y + x]);
        }
    }
    ...
    glBuildMipmaps(
      GL_TEXTURE_2D, 4, w, w, GL_RGBA, GL_UNSIGNED_BYTE, shadingTexData);
    ...
    LOG("Lightmap generation: %2.3f seconds",
                           ...
                           bool directional)
{
    ...
    for (int y = 0; y < dstw; y++) {
        for (int x = 0; x < dstw; x++) {
            ...
            h += dh;
            while ((px >= 0.0f) && (px < hmw) && (py >= 0.0f) && (py < hmw) &&
                   (len >= 0.0f)) {
