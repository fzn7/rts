  ...
  , numFeatures(0)
{
...

    {
        ...
        mapDims.mapy = renderer->GetHeightmapWidth() -
                       1; // note: not height (SM3 only supports square maps!)
    }
    ...
    if (GetMapDefParser().SectionExist("map\\featuretypes")) {
        const int numTypes =
          atoi(GetMapDefParser()
                 .SGetValueDef("0", "map\\featuretypes\\numtypes")
                 .c_str());
        for (int a = 0; a < numTypes; a++) {
            char loc[100];
            SNPRINTF(loc, 100, "map\\featuretypes\\type%d", a);
            featureTypes.push_back(new std::string(
              GetMapDefParser().SGetValueDef("TreeType0", loc)));
        }
    }
...
CSM3ReadMap::~CSM3ReadMap()
{
    ...
    for (std::vector<std::string*>::iterator fti = featureTypes.begin();
         fti != featureTypes.end();
         ++fti)
        delete *fti;
    featureTypes.clear();
...
CSM3ReadMap::InitGroundDrawer()
{
    ...
    renderer->config.forceFallbackTexturing =
      configHandler->GetBool("SM3ForceFallbackTex");

    if (!renderer->config.forceFallbackTexturing && GLEW_ARB_fragment_shader &&
    ...
    /*
    int numStages = atoi(mapDefParser.SGetValueDef("0",
    "map\\terrain\\numtexturestages").c_str()); int maxStages =
    configHandler->GetInt("SM3MaxTextureStages"); if (numStages > maxStages) {
...
CSM3ReadMap::LoadFeatureData()
{
    ...
    std::string fd =
      GetMapDefParser().SGetValueDef(std::string(), "map\\featuredata");
    if (!fd.empty()) {
        CFileHandler fh(fd);
        if (!fh.FileExists())
        ...
        if (version > 0)
            throw content_error("Map feature data has incorrect version, you "
                                "are probably using an outdated spring "
                                "version.");
...
static void
DrawGrid(terrain::TQuad* tq, DrawGridParms* param)
{
                            ...
                            int extraSize)
{
    ...
    tmpFrustum.CalcCameraPlanes(&cam->SetPos(),
                                &cam->right,
                                &cam->up,
                                &cam->forward,
                                cam->GetTanHalfFov(),
