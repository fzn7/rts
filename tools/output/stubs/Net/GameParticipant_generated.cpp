...
GameParticipant::Kill(const std::string& reason, const bool flush)
{
    if (link) {
        ...
        // make sure the Flush() performed by Close() has effect (forced flushes
        // are undesirable) it will cause a slight lag in the game server during
        // kick, but not a big deal
        if (flush)
