...
{
    PackPacket* packet =
      new PackPacket(1 + 1 + sizeof(boost::uint32_t), NETMSG_PATH_CHECKSUM);
    *packet << myPlayerNum;
    *packet << checksum;
    return PacketType(packet);
}
                              ...
                              const std::vector<float>& params)
{
    ...
    *packet << static_cast<unsigned short>(size) << myPlayerNum << id << options
            << params;
    return PacketType(packet);
}
                             ...
                             const std::vector<short>& selectedUnitIDs)
{
    ...
    *packet << static_cast<unsigned short>(size) << myPlayerNum
            << selectedUnitIDs;
    return PacketType(packet);
}
                                ...
                                const std::vector<float>& params)
{
    ...
    *packet << static_cast<unsigned short>(size) << myPlayerNum << aiID
            << unitID << id << options;
...
{
    boost::uint16_t totalNumBytes = 1 + sizeof(boost::uint16_t) + 1 + 1 + 1 +
                                    1 + (2 * sizeof(float)) +
                                    (unitIDs.size() * sizeof(short));

    PackPacket* packet = new PackPacket(totalNumBytes, NETMSG_AISHARE);
    *packet << totalNumBytes << myPlayerNum << aiID << sourceTeam << destTeam
            << metal << energy << unitIDs;
    return PacketType(packet);
}
                                     ...
                                     bool reconnect)
{
    ...
    *packet << size << NETWORK_VERSION << name << passwd << version
            << uchar(reconnect) << uchar(netloss);
    return PacketType(packet);
}
                            ...
                            float shareEnergy)
{
    ...
    *packet << myPlayerNum << shareTeam << bShareUnits << shareMetal
            << shareEnergy;
    return PacketType(packet);
}
                               ...
                               float energyShareFraction)
{
    ...
    *packet << myPlayerNum << myTeam << metalShareFraction
            << energyShareFraction;
    return PacketType(packet);
}
...
{
    PackPacket* packet =
      new PackPacket(2 + sizeof(PlayerStatistics), NETMSG_PLAYERSTAT);
...
{
    PackPacket* packet =
      new netcode::PackPacket(2 + sizeof(TeamStatistics), NETMSG_TEAMSTAT);
...
{
    const unsigned size =
      (3 * sizeof(uchar)) + (winningAllyTeams.size() * sizeof(uchar));
...
CBaseNetProtocol::SendMapErase(uchar myPlayerNum, short x, short z)
{
    ...
    *packet << static_cast<uchar>(8) << myPlayerNum
            << static_cast<uchar>(MAPDRAW_ERASE) << x << z;
    return PacketType(packet);
}
                                   ...
                                   bool fromLua)
{
    ...
    *packet << static_cast<uchar>(size) << myPlayerNum
            << static_cast<uchar>(MAPDRAW_POINT) << x << z << uchar(fromLua)
            << label;
    return PacketType(packet);
}
                                  ...
                                  bool fromLua)
{
    ...
    *packet << static_cast<uchar>(13) << myPlayerNum
            << static_cast<uchar>(MAPDRAW_LINE) << x1 << z1 << x2 << z2
            << uchar(fromLua);
    return PacketType(packet);
}
                                         ...
                                         uchar takeFromTeam)
{
    ...
    *packet << myPlayerNum << static_cast<uchar>(TEAMMSG_GIVEAWAY) << giveToTeam
            << takeFromTeam;
    return PacketType(packet);
}
...
{
    PackPacket* packet = new PackPacket(5, NETMSG_TEAM);
    *packet << myPlayerNum << static_cast<uchar>(TEAMMSG_RESIGN)
...
{
    PackPacket* packet = new PackPacket(5, NETMSG_TEAM);
    *packet << myPlayerNum << static_cast<uchar>(TEAMMSG_JOIN_TEAM)
            << wantedTeamNum << static_cast<uchar>(0);
    return PacketType(packet);
}
                                ...
                                const std::string& name)
{
    ...
    *packet << static_cast<uchar>(size) << myPlayerNum << whichSkirmishAI
            << team << name;
    return PacketType(packet);
}
...
{
    unsigned size = 1 + sizeof(uchar) + sizeof(uchar) + sizeof(uchar) +
                    sizeof(boost::uint16_t) + playerName.size() + 1;
    PackPacket* packet = new PackPacket(size, NETMSG_CREATE_NEWPLAYER);
    *packet << static_cast<boost::uint16_t>(size) << playerNum
            << (uchar)spectator << teamNum << playerName;
    return PacketType(packet);
}
                                      ...
                                      std::vector<unsigned> checksums)
{
    ...
    *packet << static_cast<boost::uint16_t>(size) << myPlayerNum << flop
            << checksums;
    return PacketType(packet);
}
