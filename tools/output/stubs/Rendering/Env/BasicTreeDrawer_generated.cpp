...
{
    CBasicTreeSquareDrawer(CBasicTreeDrawer* td,
                           int /*cx*/,
                           int /*cy*/,
                           float treeDistance)
    ...
    {
        ITreeDrawer::TreeSquareStruct* tss =
          &td->treeSquares[(y * td->treesX) + x];
        ...
        if (distFactor < FAR_TREE_DIST_FACTOR) { // far trees
            ...
            if ((tss->farDispList == 0) || (dif.dot(tss->viewVector) < 0.97f)) {
                ...
                for (auto ti = tss->trees.cbegin(); ti != tss->trees.cend();
                    ...
                    if (ts->type < 8) {
                        DrawTreeVertexFar1(
                          ts->pos, side * MAX_TREE_HEIGHT_3, false);
                    } else {
                        DrawTreeVertexFar2(
                          ts->pos, side * MAX_TREE_HEIGHT_3, false);
                    }
  ...
  private:
    void DrawTreeVertexFar1(const float3& pos,
                            const float3& swd,
                            bool enlarge = true)
    {
        if (enlarge)
    ...
    void DrawTreeVertexFar2(const float3& pos,
                            const float3& swd,
                            bool enlarge = true)
    {
        if (enlarge)
...
void
CBasicTreeDrawer::Draw(float treeDistance, bool drawReflection)
{
    ...
    CBasicTreeSquareDrawer drawer(
      this, cx, cy, treeDistance * SQUARE_SIZE * TREE_SQUARE_SIZE);
