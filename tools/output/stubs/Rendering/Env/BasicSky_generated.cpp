...
CBasicSky::CreateSkyDomeList()
{
    ...
    glDisable(
      GL_TEXTURE_2D); //! texture0 uses a texture matrix for cloud movement!
    ...
    for (y = 0; y < Y_PART; y++) {
        for (int x = 0; x < X_PART; x++) {
            ...
            glMultiTexCoord2f(
              GL_TEXTURE1, c.x / domeWidth + 0.5f, c.z / domeWidth + 0.5f);
...
CBasicSky::UpdatePart(int ast, int aed, int a3cstart, int a4cstart)
{
    ...
    for (int a = ast; a < aed; ++rc, ++ct) {
        int yap1 = ydif[ap1] += (int)cloudThickness[a3c += 4] -
                                cloudThickness[a += 4] * 2 +
                                cloudThickness[a4c += 4];
...
CBasicSky::Update()
{
    ...
    switch (updatepart) { // smoothen out the workload across draw frames
        ...
        case CLOUD_DETAIL + 1: {
            for (int a = 0; a < CLOUD_SIZE * CLOUD_SIZE; a++)
                cloudThickness[a * 4 + 3] =
                  alphaTransform[rawClouds[0][a] >> 7];

            cloudThickness[CLOUD_SIZE * CLOUD_SIZE * 4 + 3] =
              alphaTransform[rawClouds[0][0] >> 7];
            // next line unused
            ...
            ydif[0] +=
              cloudThickness[(0 + CLOUD_SIZE * (CLOUD_SIZE - 3)) * 4 + 3] -
              cloudThickness[0 * 4 + 3] * 2 +
              cloudThickness[(0 + 4 * CLOUD_SIZE) * 4 + 3];
            break;
        }
        case CLOUD_DETAIL + 2: {
            UpdatePart(0,
...
CBasicSky::DrawSun()
{
    ...
    glTexSubImage2D(
      GL_TEXTURE_2D, 0, 0, 0, 32, 1, GL_LUMINANCE, GL_UNSIGNED_BYTE, buf);
...
CBasicSky::CreateCover(int baseX, int baseY, float* buf)
{
    ...
    for (int l = 0; l < 8; ++l) {
        ...
        for (int x = 0; x < num; ++x) {
            ...
            cover2 +=
              255 - GetCloudThickness(baseX - dy, baseY - dx); //*(num-x);
            cover3 +=
              255 - GetCloudThickness(baseX + dx, baseY - dy); //*(num-x);
            cover4 +=
              255 - GetCloudThickness(baseX + dy, baseY + dx); //*(num-x);
...
CBasicSky::UpdateSkyDir()
{
    ...
    skyAngle =
      GetRadFromXY(skydir2.x, skydir2.z) + PI / 2.0f; // FIXME Why the +PI/2???
}
