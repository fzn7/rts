...
CSkyBox::CSkyBox(const std::string& texture)
{
    ...
    if (!btex.Load(texture) || btex.textype != GL_TEXTURE_CUBE_MAP) {
        LOG_L(L_WARNING,
              "could not load skybox texture from file %s",
              texture.c_str());
    } else {
...
CSkyBox::Draw()
{
    ...
    GLfloat verts[] = { 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 0.0f };
    float3 texcoords[] = { -camera->CalcPixelDir(0, 0),
                           -camera->CalcPixelDir(globalRendering->viewSizeX, 0),
                           -camera->CalcPixelDir(globalRendering->viewSizeX,
                                                 globalRendering->viewSizeY),
                           -camera->CalcPixelDir(0,
                                                 globalRendering->viewSizeY) };
