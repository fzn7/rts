...
{
    throw opengl_error(LOG_SECTION_DECALS_GL4
                       ": Compiled without OpenGL4 support!");
}
#else
...
static std::string
GetExtraTextureName(const std::string& s)
{
  ...
  , atlasTex(0)
{
    ...
    if (!GLEW_ARB_depth_clamp)
        throw opengl_error(LOG_SECTION_DECALS_GL4
                           ": missing GL_ARB_depth_clamp");

    if (!GLEW_ARB_vertex_array_object)
        throw opengl_error(LOG_SECTION_DECALS_GL4
                           ": missing GL_ARB_vertex_array_object");
    ...
    if (static_cast<CSMFReadMap*>(readMap)->GetNormalsTexture() <= 0)
        throw unsupported_error(LOG_SECTION_DECALS_GL4
                                ": advanced map shading must be enabled");
    ...
    fboOverlap.Bind();
    // fboOverlap.CreateRenderBuffer(GL_COLOR_ATTACHMENT0, GL_RGBA8,
    // OVERLAP_TEST_TEXTURE_SIZE, OVERLAP_TEST_TEXTURE_SIZE);
    fboOverlap.CreateRenderBuffer(GL_STENCIL_ATTACHMENT,
                                  GL_STENCIL_INDEX8,
                                  OVERLAP_TEST_TEXTURE_SIZE,
                                  OVERLAP_TEST_TEXTURE_SIZE);
...
{
    // detect hw limits
    const int maxDecalsUBO =
      globalRendering->glslMaxUniformBufferSize / sizeof(SGLSLDecal);
    const int maxDecalsSSBO = 0xFFFFFF; // SSBO min supported size is 16MB
    const int maxDecalGroupsUBO =
      globalRendering->glslMaxUniformBufferSize / sizeof(SDecalGroup);
    const int maxDecalGroupsSSBO =
      maxDecalGroupsUBO; // groups are always saved in a UBO
    ...
    // FIXME
    LOG_L(L_ERROR,
          "MaxDecals=%i[UBO=%i SSBO=%i] MaxDecalGroups=%i[UBO=%i SSBO=%i] "
          "useSSBO=%i",
          maxDecals,
          maxDecalsUBO,
          maxDecalsSSBO,
...
{
    decalShader = shaderHandler->CreateProgramObject(
      "[DecalsDrawerGL4]", "DecalShader", false);

    decalShader->AttachShaderObject(shaderHandler->CreateShaderObject(
      "GLSL/DecalsVertGL4.glsl", "", GL_VERTEX_SHADER));
    decalShader->AttachShaderObject(shaderHandler->CreateShaderObject(
      "GLSL/DecalsFragGL4.glsl", "", GL_FRAGMENT_SHADER));
    ...
    decalShader->SetFlag("USE_PARALLAX",
                         configHandler->GetBool("GroundDecalsParallaxMapping"));
    decalShader->Link();

    decalShader->Enable();
    decalShader->SetUniform("invMapSizePO2",
                            1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
                            1.0f / (mapDims.pwr2mapy * SQUARE_SIZE));
    decalShader->SetUniform("invMapSize",
                            1.0f / (mapDims.mapx * SQUARE_SIZE),
                            1.0f / (mapDims.mapy * SQUARE_SIZE));
    decalShader->SetUniform("invScreenSize",
...
GetBuildingDecals(std::unordered_map<std::string, STex>& textures)
{
    for (UnitDef& unitDef : unitDefHandler->unitDefs) {
        ...
        try {
            textures[decalDef.groundDecalTypeName] =
              LoadTexture(decalDef.groundDecalTypeName);
            textures[GetExtraTextureName(decalDef.groundDecalTypeName)] =
              LoadTexture(decalDef.groundDecalTypeName + ".dds"); // FIXME
...
GetGroundScars(std::unordered_map<std::string, STex>& textures)
{
    LuaParser resourcesParser(
      "gamedata/resources.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    ...
    const LuaTable scarsTable =
      resourcesParser.GetRoot().SubTable("graphics").SubTable("scars");

    for (int i = 1; i <= scarsTable.GetLength(); ++i) {
        std::string texName =
          scarsTable.GetString(i, IntToString(i, "scars/scar%i.bmp"));
...
GetFallbacks(std::unordered_map<std::string, STex>& textures)
{
    ...
    textures["%FALLBACK_TEXTURE%"] =
      CREATE_SINGLE_COLOR(SColor(255, 0, 0, 255));
    textures["%FALLBACK_TEXTURE_NORMAL%"] =
      CREATE_SINGLE_COLOR(SColor(0, 255, 0, 255));
}
...
CDecalsDrawerGL4::GenerateAtlasTexture()
{
    ...
    atlas.SetMaxSize(globalRendering->maxTextureSize,
                     globalRendering->maxTextureSize);
    ...
    glSpringTexStorage2D(GL_TEXTURE_2D,
                         atlas.GetMaxMipMaps(),
                         GL_RGBA8,
                         atlas.GetAtlasSize().x,
                         atlas.GetAtlasSize().y);
    ...
    for (auto& p : textures) {
        ...
        va.AddVertex2dT(
          absCoords.z + 1, absCoords.y, 1.0f, 0.0f); // FIXME why +1?
        va.AddVertex2dT(absCoords.x, absCoords.w + 1, 0.0f, 1.0f);
        va.AddVertex2dT(
          absCoords.z + 1, absCoords.w + 1, 1.0f, 1.0f); // FIXME why +1?
...
{
    {
        ...
        GLuint uniformBlockIndex =
          glGetUniformBlockIndex(decalShader->GetObjID(), "decalGroupsUBO");
        glGetActiveUniformBlockiv(decalShader->GetObjID(),
                                  uniformBlockIndex,
...

    {
        ...
        if (useSSBO) {
            uniformBlockIndex = glGetProgramResourceIndex(
              decalShader->GetObjID(), GL_SHADER_STORAGE_BLOCK, "decalsUBO");
            ...
            uniformBlockIndex =
              glGetUniformBlockIndex(decalShader->GetObjID(), "decalsUBO");
            glGetActiveUniformBlockiv(decalShader->GetObjID(),
                                      uniformBlockIndex,
                                      GL_UNIFORM_BLOCK_DATA_SIZE,
                                      &uniformBlockSize);
        ...
        if (useSSBO) {
            glShaderStorageBlockBinding(
              decalShader->GetObjID(), uniformBlockIndex, 3);
            glBindBufferBase(
              GL_SHADER_STORAGE_BUFFER, 3, uboDecalsStructures.GetId());
        } else {
...
CDecalsDrawerGL4::ViewResize()
{
    decalShader->Enable();
    decalShader->SetUniform("invScreenSize",
                            1.f / globalRendering->viewSizeX,
                            1.f / globalRendering->viewSizeY);
    decalShader->Disable();
...
CDecalsDrawerGL4::SunChanged()
{
    // Ground Lighting UBO
    GLuint uniformBlockIndex =
      glGetUniformBlockIndex(decalShader->GetObjID(), "SGroundLighting");
    ...
    if (uniformBlockSize != sizeof(SGLSLGroundLighting))
        LOG("uniformBlockSize sizeof(SGLSLGroundLighting) %u " _STPF_,
            uniformBlockSize,
            sizeof(SGLSLGroundLighting));
    ...
    SGLSLGroundLighting* uboGroundLightingData =
      (SGLSLGroundLighting*)uboGroundLighting.MapBuffer(
        0, sizeof(SGLSLGroundLighting));
    uboGroundLightingData->ambientColor =
      sunLighting->groundAmbientColor * CGlobalRendering::SMF_INTENSITY_MULT;
    uboGroundLightingData->diffuseColor =
      sunLighting->groundDiffuseColor * CGlobalRendering::SMF_INTENSITY_MULT;
    uboGroundLightingData->specularColor =
      sunLighting->groundSpecularColor * CGlobalRendering::SMF_INTENSITY_MULT;
    ...
    uboGroundLightingData->fogScale =
      1.0f / (globalRendering->viewRange * (sky->fogEnd - sky->fogStart));
...
CDecalsDrawerGL4::Draw()
{
    ...
    if (!AnyDecalsInView())
        ...
        if (setTimerQuery) {
                ...
                LOG_L(L_ERROR, "CDecalsDrawerGL4::Update %fms %fms", avgfoo[0],
        avgfoo[1]); if (lastDrawTime > 5.f) {
                        ++laggedFrames;
                        if (laggedFrames > 30) {
    ...
    if (GLEW_EXT_direct_state_access) {
        glCopyTextureSubImage2D(depthTex,
                                0,
                                0,
                                0,
    ...
    decalShader->SetUniformMatrix4x4(
      "viewProjMatrix", false, camera->GetViewProjectionMatrix().m);
    ...
    if (shadowHandler->ShadowsLoaded()) {
        ...
        decalShader->SetUniform("shadowDensity",
...
CDecalsDrawerGL4::UpdateDecalsVBO()
{
    ...
    uboDecalGroups.Bind(GL_UNIFORM_BUFFER);
    GLubyte* uboData =
      uboDecalGroups.MapBuffer(0, sizeof(SDecalGroup) * groups.size());
    ...
    for (int idx : decalsToUpdate) {
        Decal& d = decals[idx];
        GLubyte* uboData = uboDecalsStructures.MapBuffer(
          sizeof(SGLSLDecal) * idx, sizeof(SGLSLDecal) * 1);
        ...
        uboScarsData[0].rotMatElements =
          float2(std::cos(d.rot), std::sin(d.rot));
...
GetEdgePoinsOfDecal(const CDecalsDrawerGL4::Decal& d)
{
    ...
    return { f3tof2(d.pos + m * float3(-d.size.x, 0.f, -d.size.y)),
...
DRAW_DECAL(CVertexArray* va, const CDecalsDrawerGL4::Decal* d)
{
    ...
    dsize.x = std::max(dsize.x,
                       std::ceil(float(mapDims.mapx * SQUARE_SIZE) /
                                 CDecalsDrawerGL4::OVERLAP_TEST_TEXTURE_SIZE));
    dsize.y = std::max(dsize.y,
                       std::ceil(float(mapDims.mapy * SQUARE_SIZE) /
                                 CDecalsDrawerGL4::OVERLAP_TEST_TEXTURE_SIZE));
...
CDecalsDrawerGL4::UpdateOverlap()
{
    ...
    glSpringMatrix2dProj(mapDims.mapx * SQUARE_SIZE,
                         mapDims.mapy * SQUARE_SIZE);
...
CDecalsDrawerGL4::UpdateOverlap_PreCheck()
{
    ...
    // we are in a test already, check if one of the tested decals changed, if
    // so we need to restart the test
    ...
    if ((overlapStage == 0) &&
        (waitingDecalsForOverlapTest.size() <= (MAX_OVERLAP * 4)))
        return candidatesForOverlap;

    // only test those decals which overlap with others and add them to the
    // vector
    if (overlapStage == 0) {
...
CDecalsDrawerGL4::UpdateOverlap_Initialize()
{
    ...
    /*const size_t datasize = 4 * OVERLAP_TEST_TEXTURE_SIZE *
    OVERLAP_TEST_TEXTURE_SIZE; unsigned char* img = new unsigned char[datasize];
    memset(img, 0, datasize);
    glReadPixels(0, 0, OVERLAP_TEST_TEXTURE_SIZE, OVERLAP_TEST_TEXTURE_SIZE,
    GL_RGBA, GL_UNSIGNED_BYTE, img); CBitmap bitmap(img,
    OVERLAP_TEST_TEXTURE_SIZE, OVERLAP_TEST_TEXTURE_SIZE);
...
{
    // query the results of the last issued test (if any pixels were rendered
    // for the decal)
    ...
    while (sortBeginIt != sortEndIt) {
        std::sort(
          sortBeginIt, sortEndIt, [&](const int& idx1, const int& idx2) {
              return decals[idx1].generation < decals[idx2].generation;
          });
        sortEndIt =
          std::partition(sortBeginIt + 1, sortEndIt, [&](const int& idx) {
    ...
    // FIXME
    LOG_L(L_ERROR,
          "Query freed: %i of %i (decals:%i groups:%i)",
          freed,
          int(candidatesForRemoval.size()),
          int(decals.size() - freeIds.size()),
          int(groups.size()));
...
CDecalsDrawerGL4::CreateLuaDecal()
{
    ...
    if (freeIds.empty()) { // try to make space for new one
        // current worst decal is inview, try to find a better one (at best
        // outside of view)
...
{
    const auto proj =
      projectileHandler->GetProjectileBySyncedID(event.projectileID);
    if (proj) {
        if (teamHandler->ValidAllyTeam(proj->GetAllyteamID()) &&
            teamHandler->AlliedAllyTeams(gu->myAllyTeam, proj->GetAllyteamID()))
            return true;
    }
    ...
    if (event.craterAreaOfEffect > 50) {
        ...
        for (int i = r * 0.3f; i >= 0; --i) {
            if (losHandler->InAirLos(p + (gu->rng.RandVector2D() * r),
                                     gu->myAllyTeam))
                return true;
        }
    }
...
CDecalsDrawerGL4::AddExplosion(float3 pos, float damage, float radius)
{
    ...
    d.SetTexture(
      IntToString((gu->RandInt() & 3) + 1)); // pick one of 4 scar textures
...
CDecalsDrawerGL4::CreateBuildingDecal(const CSolidObject* object)
{
    ...
    d.pos = object->pos;
    d.rot =
      GetRadFromXY(object->frontdir.x, object->frontdir.z) - fastmath::HALFPI;
