  ...
  : CEventClient("[CGroundDecalHandler]", 314159, false)
{
    ...
    LoadScar("bitmaps/" + scarsTable.GetString(2, "scars/scar2.bmp"),
             buf.data(),
             0,
             0);
    LoadScar("bitmaps/" + scarsTable.GetString(3, "scars/scar3.bmp"),
             buf.data(),
             256,
             0);
    LoadScar("bitmaps/" + scarsTable.GetString(1, "scars/scar1.bmp"),
             buf.data(),
             0,
             256);
    LoadScar("bitmaps/" + scarsTable.GetString(4, "scars/scar4.bmp"),
             buf.data(),
             256,
             256);
    ...
    glBuildMipmaps(
      GL_TEXTURE_2D, GL_RGBA8, 512, 512, GL_RGBA, GL_UNSIGNED_BYTE, buf.data());
...
#define sh shaderHandler
    ...
    const char* fragmentProgramNameARB =
      haveShadingTexture ? "ARB/GroundDecalsSMF.fp" : "ARB/GroundDecalsSM3.fp";
    const std::string extraDef = haveShadingTexture
                                   ? "#define HAVE_SHADING_TEX 1\n"
                                   : "#define HAVE_SHADING_TEX 0\n";

    decalShaders[DECAL_SHADER_ARB] =
      sh->CreateProgramObject("[GroundDecalHandler]", "DecalShaderARB", true);
    decalShaders[DECAL_SHADER_GLSL] =
      sh->CreateProgramObject("[GroundDecalHandler]", "DecalShaderGLSL", false);
    decalShaders[DECAL_SHADER_CURR] = decalShaders[DECAL_SHADER_ARB];

    if (globalRendering->haveARB && !globalRendering->haveGLSL) {
        decalShaders[DECAL_SHADER_ARB]->AttachShaderObject(
          sh->CreateShaderObject(
            "ARB/GroundDecals.vp", "", GL_VERTEX_PROGRAM_ARB));
        decalShaders[DECAL_SHADER_ARB]->AttachShaderObject(
          sh->CreateShaderObject(
            fragmentProgramNameARB, "", GL_FRAGMENT_PROGRAM_ARB));
        decalShaders[DECAL_SHADER_ARB]->Link();
    } else {
        if (globalRendering->haveGLSL) {
            decalShaders[DECAL_SHADER_GLSL]->AttachShaderObject(
              sh->CreateShaderObject(
                "GLSL/GroundDecalsVertProg.glsl", "", GL_VERTEX_SHADER));
            decalShaders[DECAL_SHADER_GLSL]->AttachShaderObject(
              sh->CreateShaderObject("GLSL/GroundDecalsFragProg.glsl",
                                     extraDef,
                                     GL_FRAGMENT_SHADER));
            decalShaders[DECAL_SHADER_GLSL]->Link();

            decalShaders[DECAL_SHADER_GLSL]->SetUniformLocation(
              "decalTex"); // idx 0
            decalShaders[DECAL_SHADER_GLSL]->SetUniformLocation(
              "shadeTex"); // idx 1
            decalShaders[DECAL_SHADER_GLSL]->SetUniformLocation(
            ...
            decalShaders[DECAL_SHADER_GLSL]->SetUniform1i(
              0, 0); // decalTex  (idx 0, texunit 0)
            decalShaders[DECAL_SHADER_GLSL]->SetUniform1i(
              1, 1); // shadeTex  (idx 1, texunit 1)
            decalShaders[DECAL_SHADER_GLSL]->SetUniform1i(
              2, 2); // shadowTex (idx 2, texunit 2)
            decalShaders[DECAL_SHADER_GLSL]->SetUniform2f(
              3,
              1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
              1.0f / (mapDims.pwr2mapy * SQUARE_SIZE));
            decalShaders[DECAL_SHADER_GLSL]->SetUniform1f(
              7, sky->GetLight()->GetGroundShadowDensity());
...
#else
#define HEIGHT(z, x)                                                           \
    (assert((z) <= gsmy), assert((x) <= gsmx), (hm[((z)*gsmx1) + (x)]))
#endif
    ...
    if (va == nullptr) {
        ...
        const int dxsize = decal->xsize, dzsize = decal->ysize,
        ...
        float yv[4] = {
            0.0f
        }; // heights at each sub-quad vertex (tl, tr, br, bl)
        ...
        if ((dxpos + dxsize) > gsmx) {
            cxsize -= ((dxpos + dxsize) - gsmx);
        }
        if ((dzpos + dzsize) > gsmy) {
            czsize -= ((dzpos + dzsize) - gsmy);
        }
            ...
            for (int vz = 0; vz < czsize; vz++) {
                ...
                switch (decal->facing) {
                    case FACING_SOUTH: {
                        ...
                        uv[3] = (rz)*zts; // uv = (1, 0)
                        uv[4] = (rx + 1) * xts;
                        uv[5] = (rz + 1) * zts; // uv = (1, 1)
                        uv[6] = (rx)*xts;
                        uv[7] = (rz + 1) * zts; // uv = (0, 1)
                    } break;
                    case FACING_NORTH: {
                        uv[0] = (dxsize - rx) * xts;
                        uv[1] = (dzsize - rz) * zts; // uv = (1, 1)
                        uv[2] = (dxsize - rx - 1) * xts;
                        uv[3] = (dzsize - rz) * zts; // uv = (0, 1)
                        uv[4] = (dxsize - rx - 1) * xts;
                        uv[5] = (dzsize - rz - 1) * zts; // uv = (0, 0)
                        uv[6] = (dxsize - rx) * xts;
                        uv[7] = (dzsize - rz - 1) * zts; // uv = (1, 0)
                    } break;
                    ...
                    case FACING_EAST: {
                        ...
                        uv[1] = (rx)*xts; // uv = (1, 0)
                        uv[2] = 1.0f - (rz)*zts;
                        uv[3] = (rx + 1) * xts; // uv = (1, 1)
                        uv[4] = 1.0f - (rz + 1) * zts;
                        uv[5] = (rx + 1) * xts; // uv = (0, 1)
                        uv[6] = 1.0f - (rz + 1) * zts;
                        uv[7] = (rx)*xts; // uv = (0, 0)
                    } break;
                    case FACING_WEST: {
                        uv[0] = (rz)*zts;
                        uv[1] = 1.0f - (rx)*xts; // uv = (0, 1)
                        uv[2] = (rz)*zts;
                        uv[3] = 1.0f - (rx + 1) * xts; // uv = (0, 0)
                        uv[4] = (rz + 1) * zts;
                        uv[5] = 1.0f - (rx + 1) * xts; // uv = (1, 0)
                        uv[6] = (rz + 1) * zts;
                        uv[7] = 1.0f - (rx)*xts; // uv = (1, 1)
                    } break;
                }
        ...
        glPushMatrix();
        glTranslatef(int(decal->pos.x) % SQUARE_SIZE,
                     0.0f,
                     int(decal->pos.z) % SQUARE_SIZE);
    ...
    if (scar->va == NULL) {
        ...
        for (int x = sx; x <= ex; ++x) {
            ...
            for (int z = sz; z <= ez; ++z) {
                ...
                scar->va->AddVertexTC(
                  float3(px1, h1, pz1), tx1 + tx, tz1 + ty, color);
                scar->va->AddVertexTC(
                  float3(px2, h2, pz1), tx2 + tx, tz1 + ty, color);
                scar->va->AddVertexTC(
                  float3(px2, h3, pz2), tx2 + tx, tz2 + ty, color);
                scar->va->AddVertexTC(
                  float3(px1, h4, pz2), tx1 + tx, tz2 + ty, color);
                pz1 = pz2;
            }
        ...
        if (fade) {
            if ((scar->creationTime + 10) > gs->frameNum) {
                color[3] = (int)(scar->startAlpha *
                                 (gs->frameNum - scar->creationTime) * 0.1f);
            } else {
                color[3] =
                  (int)(scar->startAlpha - (gs->frameNum - scar->creationTime) *
                                             scar->alphaFalloff);
            }
    ...
    for (int i = 0; i < objectDecals.size();) {
        ...
        if (decalOwner == NULL) {
            if (decal->gbOwner == NULL) {
                decal->alpha -=
                  (decal->alphaFalloff * globalRendering->lastFrameTime *
                   0.001f * gs->speedFactor);
            }
            if (decal->alpha < 0.0f) {
            ...
            if (decalOwner->GetBlockingMapID() < unitHandler->MaxUnits()) {
                const CUnit* decalOwnerUnit =
                  static_cast<const CUnit*>(decalOwner);
                if (decalOwnerUnit->isIcon)
                    continue;
                if (!gu->spectatingFullView &&
                    (decalOwnerUnit->losStatus[gu->myAllyTeam] & LOS_INLOS) ==
                      0 &&
                    (!gameSetup->ghostedBuildings ||
                     (decalOwnerUnit->losStatus[gu->myAllyTeam] &
                      LOS_PREVLOS) == 0))
                    continue;

                decal->alpha = std::max(0.0f, decalOwnerUnit->buildProgress);
            } else {
                const CFeature* decalOwnerFeature =
                  static_cast<const CFeature*>(decalOwner);
...
CGroundDecalHandler::Draw()
{
    ...
    BindTextures();
    BindShader(sunLighting->groundAmbientColor *
               CGlobalRendering::SMF_INTENSITY_MULT);
    DrawDecals();
    KillTextures();
...
CGroundDecalHandler::BindShader(const float3& ambientColor)
{
    ...
    if (decalShaders[DECAL_SHADER_CURR] == decalShaders[DECAL_SHADER_ARB]) {
        decalShaders[DECAL_SHADER_CURR]->SetUniformTarget(
          GL_VERTEX_PROGRAM_ARB);
        decalShaders[DECAL_SHADER_CURR]->SetUniform4f(
          10,
          1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
          1.0f / (mapDims.pwr2mapy * SQUARE_SIZE),
          0.0f,
        ...
        decalShaders[DECAL_SHADER_CURR]->SetUniform4f(
          10, ambientColor.x, ambientColor.y, ambientColor.z, 1.0f);
        decalShaders[DECAL_SHADER_CURR]->SetUniform4f(
          11, 0.0f, 0.0f, 0.0f, sky->GetLight()->GetGroundShadowDensity());
        ...
        decalShaders[DECAL_SHADER_CURR]->SetUniform4f(
          4, ambientColor.x, ambientColor.y, ambientColor.z, 1.0f);
        decalShaders[DECAL_SHADER_CURR]->SetUniformMatrix4fv(
          5, false, shadowHandler->GetShadowMatrixRaw());
        decalShaders[DECAL_SHADER_CURR]->SetUniform4fv(
          6, &(shadowHandler->GetShadowParams().x));
    }
}
...
CGroundDecalHandler::AddExplosion(float3 pos, float damage, float radius)
{
    ...
    s->x2 = int(std::min(float(mapDims.hmapx - 1),
                         (s->pos.x + radius) / (SQUARE_SIZE * 2) + 1));
    s->y1 = int(std::max(0.f, (s->pos.z - radius) / (SQUARE_SIZE * 2)));
    s->y2 = int(std::min(float(mapDims.hmapy - 1),
                         (s->pos.z + radius) / (SQUARE_SIZE * 2) + 1));
                              ...
                              int yoffset)
{
    ...
    if (FileSystem::GetExtension(file) == "bmp") {
        ...
        for (int y = 0; y < bm.ysize; ++y) {
            ...
            memcpy(
              &buf[bufIndex], &bm.mem[memIndex], bm.xsize * sizeof(SColor));
        }
    }
}
...
CGroundDecalHandler::GetSolidObjectDecalType(const std::string& name)
{
    ...
    if (!bm.Load(fullName)) {
        LOG_L(L_ERROR,
              "[%s] Could not load object-decal from file \"%s\"",
              __FUNCTION__,
              fullName.c_str());
        return -2;
    }
...
CGroundDecalHandler::MoveSolidObject(CSolidObject* object, const float3& pos)
{
    ...
    if (decalDef.groundDecalType < 0) {
        const_cast<SolidObjectDecalDef&>(decalDef).groundDecalType =
          GetSolidObjectDecalType(decalDef.groundDecalTypeName);
    ...
    decal->pos = pos;
    decal->radius =
      std::sqrt(float(sizex * sizex + sizey * sizey)) * SQUARE_SIZE + 20.0f;
    decal->facing = object->buildFacing;
    // convert to heightmap coors
    decal->xsize = sizex << 1;
    decal->ysize = sizey << 1;

    if (object->buildFacing == FACING_EAST ||
        object->buildFacing == FACING_WEST) {
...
{
    AddDecal(const_cast<CUnit*>(unit), unit->pos);
}
...
{
    RemoveSolidObject(const_cast<CFeature*>(feature), nullptr);
}
...
{
    ForceRemoveSolidObject(const_cast<CUnit*>(unit));
}
void
