...
CRefractWater::LoadGfx()
{
    ...
    if (target == GL_TEXTURE_RECTANGLE_ARB) {
        ...
        glTexImage2D(target,
                     0,
                     3,
                     next_power_of_2(globalRendering->viewSizeX),
                     next_power_of_2(globalRendering->viewSizeY),
                     0,
                     GL_RGB,
                     GL_INT,
                     0);
...
CRefractWater::Draw()
{
    ...
    // GL_TEXTURE_RECTANGLE uses texcoord range 0 to width, whereas
    // GL_TEXTURE_2D uses 0 to 1
    if (target == GL_TEXTURE_RECTANGLE_ARB) {
        float v[] = { 10.0f * globalRendering->viewSizeX,
                      10.0f * globalRendering->viewSizeY,
                      0.0f,
                      0.0f };
...
CRefractWater::SetupWaterDepthTex()
{
    ...
    glBindTexture(GL_TEXTURE_2D,
                  readMap->GetShadingTexture()); // the shading texture has
                                                 // water depth encoded in alpha
    glEnable(GL_TEXTURE_GEN_S);
    float splane[] = {
        1.0f / (mapDims.pwr2mapx * SQUARE_SIZE), 0.0f, 0.0f, 0.0f
    };
    ...
    float tplane[] = {
        0.0f, 0.0f, 1.0f / (mapDims.pwr2mapy * SQUARE_SIZE), 0.0f
    };
