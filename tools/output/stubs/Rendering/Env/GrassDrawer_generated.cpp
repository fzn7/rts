...
{
    const float distA =
      GetCamDistOfGrassBlock((a->posX + 0.5f) * grassBlockSize,
                             (a->posZ + 0.5f) * grassBlockSize,
                             true);
    const float distB =
      GetCamDistOfGrassBlock((b->posX + 0.5f) * grassBlockSize,
                             (b->posZ + 0.5f) * grassBlockSize,
                             true);
    return (distA > distB);
}
...
class CGrassBlockDrawer : public CReadMap::IQuadDrawer
{
  public:
    ...
    {
        const float distSq = GetCamDistOfGrassBlock(
          (x + 0.5f) * grassBlockSize, (y + 0.5f) * grassBlockSize, true);

        if (distSq > Square(gd->maxGrassDist))
            return;

        if (abs(x - cx) <= gd->detailedBlocks &&
            abs(y - cy) <= gd->detailedBlocks) {
  ...
  private:
    void DrawDetailQuad(const int x, const int y)
    {
        ...
        for (int y2 = y * grassBlockSize; y2 < (y + 1) * grassBlockSize; ++y2) {
            for (int x2 = x * grassBlockSize; x2 < (x + 1) * grassBlockSize;
                 ++x2) {
                ...
                // TODO instead of adding grass turfs depending on their
                // distance to the camera,
                //     there should be a fixed sized pool for mesh & billboard
                //     turfs and then we fill these pools with _preference_ for
                //     close distance turfs. So when a map has only less turfs,
                //     render them independent of the cam distance as mesh.
                //     -> see Ravaged_2
  ...
  , grassMap(nullptr)
{
    ...
    // load grass density from map
    {
        ...
        if (grassbm.width != mapDims.mapx / grassSquareSize ||
            grassbm.height != mapDims.mapy / grassSquareSize) {
            char b[128];
            SNPRINTF(b,
                     sizeof(b),
                     "grass-map has wrong size (%dx%d, should be %dx%d)\n",
                     grassbm.width,
                     grassbm.height,
                     mapDims.mapx / 4,
                     mapDims.mapy / 4);
        ...
        const int grassMapSize =
          mapDims.mapx * mapDims.mapy / (grassSquareSize * grassSquareSize);
...
CGrassDrawer::ChangeDetail(int detail)
{
    ...
    strawPerTurf = std::min(50 + int(std::sqrt((float)detail_lim) * 10),
                            mapInfo->grass.maxStrawsPerTurf);
...
#define sh shaderHandler
    ...
    for (int i = 0; i < GRASS_PROGRAM_LAST; i++) {
        grassShaders[i] = sh->CreateProgramObject(
          "[GrassDrawer]", shaderNames[i] + "GLSL", false);
        grassShaders[i]->AttachShaderObject(sh->CreateShaderObject(
          "GLSL/GrassVertProg.glsl", shaderDefines[i], GL_VERTEX_SHADER));
        grassShaders[i]->AttachShaderObject(sh->CreateShaderObject(
          "GLSL/GrassFragProg.glsl", shaderDefines[i], GL_FRAGMENT_SHADER));
        grassShaders[i]->Link();

        grassShaders[i]->Enable();
        grassShaders[i]->SetUniform("mapSizePO2",
                                    1.0f / (mapDims.pwr2mapx * SQUARE_SIZE),
                                    1.0f / (mapDims.pwr2mapy * SQUARE_SIZE));
        grassShaders[i]->SetUniform("mapSize",
...
{
    const float3 windSpeed = wind.GetCurrentDirection() *
    ...
    grassShader->SetUniform("frame",
                            gs->frameNum + globalRendering->timeOffset);
    ...
    grassShader->SetUniform("groundShadowDensity",
                            mapInfo->light.groundShadowDensity);
    grassShader->SetUniformMatrix4x4(
      "shadowMatrix", false, shadowHandler->GetShadowMatrixRaw());
    grassShader->SetUniform4v("shadowParams",
                              &shadowHandler->GetShadowParams().x);

    grassShader->SetUniform3v("ambientLightColor",
                              &sunLighting->unitAmbientColor.x);
    grassShader->SetUniform3v("diffuseLightColor",
                              &sunLighting->unitDiffuseColor.x);
    grassShader->SetUniform3v("specularLightColor",
                              &sunLighting->unitSpecularColor.x);
...
static STurfParams
GetTurfParams(UnsyncedRNG& rng, const int x, const int y)
{
    STurfParams result;
...
CGrassDrawer::DrawNear(const std::vector<InviewNearGrass>& inviewGrass)
{
    for (const InviewNearGrass& g : inviewGrass) {
        ...
        //		const float distSq = GetCamDistOfGrassBlock(g.x, g.y,
        //true);
        const float rdist = 1.0f + rng.RandFloat() * 0.5f;
        const float alpha =
          linearstep(maxDetailedDist, maxDetailedDist + 128.f * rdist, g.dist);
                            ...
                            VA_TYPE_TN* va_tn)
{
    ...
    const float rdist = 1.0f + rng.RandFloat() * 0.5f;
    float alpha =
      1.0f - linearstep(maxGrassDist, maxGrassDist + 127.f, dist + 128.f);
    alpha = std::min(
      alpha,
      linearstep(maxDetailedDist, maxDetailedDist + 128.f * rdist, dist));
    ...
    for (int a = 0; a < numTurfs; a++) {
        ...
        va_tn[a * 4 + 0] = {
            pos, 0.0f, 1.0f, float3(-partTurfSize, -partTurfSize, alpha)
        };
        va_tn[a * 4 + 1] = {
            pos, 1.0f / 16.0f, 1.0f, float3(partTurfSize, -partTurfSize, alpha)
        };
        va_tn[a * 4 + 2] = {
            pos, 1.0f / 16.0f, 0.0f, float3(partTurfSize, partTurfSize, alpha)
        };
        va_tn[a * 4 + 3] = {
            pos, 0.0f, 0.0f, float3(-partTurfSize, partTurfSize, alpha)
        };
    }
}
...
CGrassDrawer::DrawFarBillboards(const std::vector<GrassStruct*>& inviewFarGrass)
{
    ...
    if (updateBillboards) {
        ...
        for_mt(0, inviewFarGrass.size(), [&](const int i) {
            ...
            if (!g.va) {
                // TODO vertex arrays need to be send each frame to the gpu,
                // that's slow. switch to VBOs.
            ...
            const float distSq =
              GetCamDistOfGrassBlock((g.posX + 0.5f) * grassBlockSize,
                                     (g.posZ + 0.5f) * grassBlockSize,
                                     true);
            if (distSq == g.lastDist)
                return;

            bool inAlphaRange1 =
              (distSq < Square(maxDetailedDist + 128.f * 1.5f)) ||
              (distSq > Square(maxGrassDist - 128.f));
            bool inAlphaRange2 =
              (g.lastDist < Square(maxDetailedDist + 128.f * 1.5f)) ||
              (g.lastDist > Square(maxGrassDist - 128.f));
            ...
            for (int y2 = g.posZ * grassBlockSize;
                 y2 < (g.posZ + 1) * grassBlockSize;
                 ++y2) {
                for (int x2 = g.posX * grassBlockSize;
                     x2 < (g.posX + 1) * grassBlockSize;
                     ++x2) {
                    ...
                    auto* va_tn =
                      va->GetTypedVertexArray<VA_TYPE_TN>(numTurfs * 4);
  ...
  const std::vector<InviewNearGrass>& inviewNearGrass)
{
    if (farnearVA->drawIndex() == 0) {
        auto* va_tn = farnearVA->GetTypedVertexArray<VA_TYPE_TN>(
          inviewNearGrass.size() * numTurfs * 4);
...
CGrassDrawer::Update()
{
    ...
    if (oldCamPos != cam->GetPos() || oldCamDir != cam->GetDir()) {
        ...
        blockDrawer.gd = this;
        readMap->GridVisibility(
          nullptr, &blockDrawer, maxGrassDist, blockMapSize);

        if (globalRendering->haveGLSL &&
            (!shadowHandler->ShadowsLoaded() ||
             !globalRendering->atiHacks) // Ati crashes w/o an error when
                                         // shadows are enabled!?
        ) {
            std::sort(blockDrawer.inviewFarGrass.begin(),
                      blockDrawer.inviewFarGrass.end(),
                      GrassSort);
            std::sort(blockDrawer.inviewNearGrass.begin(),
                      blockDrawer.inviewNearGrass.end(),
                      GrassSortNear);
    ...
    for (GrassStruct& pGS : grass) {
        if ((pGS.lastSeen != lastVisibilityUpdate) &&
            (pGS.lastSeen < globalRendering->drawFrame - 50) && pGS.va) {
            ResetPos(pGS.posX, pGS.posZ);
        }
    }
}
...
CGrassDrawer::Draw()
{
    ...
    if (globalRendering->haveGLSL &&
        (!shadowHandler->ShadowsLoaded() ||
         !globalRendering
            ->atiHacks) // Ati crashes w/o an error when shadows are enabled!?
        && !(blockDrawer.inviewFarGrass.empty() &&
             blockDrawer.inviewNearGrass.empty())) {
        SetupGlStateFar();
...
CGrassDrawer::DrawShadow()
{
    ...
    /*	if (grassOff || !readMap->GetGrassShadingTexture())
            ...
            //TODO either enable it on high density only, or wait for alpha
       transparent shadows and use those then
            ...
            readMap->GridVisibility(nullptr, &drawer, maxGrassDist,
       blockMapSize); DrawNear(blockDrawer.inviewGrass);
...
CGrassDrawer::SetupGlStateNear()
{
    ...
    if (globalRendering->haveGLSL) {
        ...
        glMultiTexCoord4f(GL_TEXTURE1_ARB,
                          ...
                          1.0f,
                          1.0f,
                          1.0f); // workaround a nvidia bug with TexGen
        SetTexGen(1.0f / (mapDims.mapx * SQUARE_SIZE),
                  1.0f / (mapDims.mapy * SQUARE_SIZE),
                  0.0f,
                  0.0f);
        glActiveTextureARB(GL_TEXTURE2_ARB);
...
CGrassDrawer::ResetGlStateNear()
{
    ...
    if (globalRendering->haveGLSL) {
        ...
        if (shadowHandler->ShadowsLoaded()) {
            ...
            glTexParameteri(
              GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE_ARB, GL_NONE);
            glTexParameteri(
              GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE_ARB, GL_LUMINANCE);
            glActiveTextureARB(GL_TEXTURE0_ARB);
        }
...
CGrassDrawer::CreateGrassDispList(int listNum)
{
    ...
    for (int a = 0; a < strawPerTurf; ++a) {
        ...
        const float maxAng =
          mapInfo->grass.bladeAngle *
          std::max(rng.RandFloat(), 1.f - smoothstep(0.f, 1.f, lngRnd));
        ...
        const float3 basePos =
          rng.RandVector2D() *
          (turfSize - (bendVect * std::sin(maxAng) * length).Length2D());
        ...
        // start btm
        va->AddVertexTN(basePos + sideVect - float3(0.0f, 3.0f, 0.0f),
                        xtexCoord,
                        0.f,
                        normalBend);
        va->AddVertexTN(basePos - sideVect - float3(0.0f, 3.0f, 0.0f),
        ...
        for (float h = 0.0f; h < 1.0f; h += (1.0f / numSections)) {
            ...
            const float3 n =
              (normalBend * std::cos(ang) + UpVector * std::sin(ang))
                .ANormalize();
            const float3 edgePos =
              (UpVector * std::cos(ang) + bendVect * std::sin(ang)) * length *
              h;
            const float3 edgePosL = edgePos - sideVect * (1.0f - h);
            const float3 edgePosR = edgePos + sideVect * (1.0f - h);

            va->AddVertexTN(basePos + edgePosR,
                            xtexCoord + (1.0f / 32) * h,
                            h,
                            (n + sideVect * 0.04f).ANormalize());
            va->AddVertexTN(basePos + edgePosL,
                            xtexCoord - (1.0f / 32) * h + (1.0f / 16),
                            h,
                            (n - sideVect * 0.04f).ANormalize());
        }
        ...
        const float3 edgePos =
          (UpVector * std::cos(maxAng) + bendVect * std::sin(maxAng)) * length;
        const float3 n =
          (normalBend * std::cos(maxAng) + UpVector * std::sin(maxAng))
            .ANormalize();
...
CGrassDrawer::CreateGrassBladeTex(unsigned char* buf)
{
    ...
    for (int y = 0; y < 64; ++y) {
        for (int x = 0; x < 16; ++x) {
            const float brightness = smoothstep(-0.8f, 0.5f, y / 63.0f) +
                                     ((x % 2) == 0 ? 0.035f : 0.0f);
...
CGrassDrawer::CreateFarTex()
{
    ...
    FBO fbo;
    fbo.Bind();
    fbo.CreateRenderBuffer(GL_DEPTH_ATTACHMENT_EXT,
                           GL_DEPTH_COMPONENT16,
                           texSizeX * sizeMod,
                           texSizeY * sizeMod);
    fbo.CreateRenderBuffer(GL_COLOR_ATTACHMENT0_EXT,
                           GL_RGBA8,
                           texSizeX * sizeMod,
                           texSizeY * sizeMod);
    ...
    glClearColor(mapInfo->grass.color.r,
                 mapInfo->grass.color.g,
                 mapInfo->grass.color.b,
                 0.f);
    ...
    for (int a = 0; a < numAngles; ++a) {
        glViewport(a * billboardSize * sizeMod,
                   0,
                   billboardSize * sizeMod,
                   billboardSize * sizeMod);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
    ...
    // blur non-rendered areas, so in mipmaps color data isn't blurred with
    // background color
    {
        const int mipLevels =
          std::ceil(std::log((float)(std::max(texSizeX, texSizeY) + 1)));
        ...
        glEnable(GL_BLEND);
        glBlendFuncSeparate(
          GL_ONE_MINUS_DST_ALPHA, GL_DST_ALPHA, GL_ZERO, GL_DST_ALPHA);
        ...
        for (int mipLevel = mipLevels - 2; mipLevel >= 0; --mipLevel) {
            fboTex.AttachTexture(
              farTex, GL_TEXTURE_2D, GL_COLOR_ATTACHMENT0_EXT, mipLevel);
