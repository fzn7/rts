...
CAdvTreeDrawer::LoadTreeShaders()
{
    ...
    if (globalRendering->haveGLSL) {
        treeShaders[TREE_PROGRAM_NEAR_BASIC] =
          shaderHandler->CreateProgramObject(
            "[TreeDrawer]",
            shaderNames[TREE_PROGRAM_NEAR_BASIC] + "GLSL",
            false);
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->AttachShaderObject(
          shaderHandler->CreateShaderObject(
            "GLSL/TreeVertProg.glsl",
            shaderDefines[TREE_PROGRAM_NEAR_BASIC],
            GL_VERTEX_SHADER));
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->Link();

        treeShaders[TREE_PROGRAM_NEAR_SHADOW] =
          shaderHandler->CreateProgramObject(
            "[TreeDrawer]",
            shaderNames[TREE_PROGRAM_NEAR_SHADOW] + "GLSL",
            false);
        treeShaders[TREE_PROGRAM_DIST_SHADOW] =
          shaderHandler->CreateProgramObject(
            "[TreeDrawer]",
            shaderNames[TREE_PROGRAM_DIST_SHADOW] + "GLSL",
            false);
        ...
        if (CShadowHandler::ShadowsSupported()) {
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "GLSL/TreeVertProg.glsl",
                shaderDefines[TREE_PROGRAM_NEAR_SHADOW],
                GL_VERTEX_SHADER));
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "GLSL/TreeFragProg.glsl",
                shaderDefines[TREE_PROGRAM_NEAR_SHADOW],
                GL_FRAGMENT_SHADER));
            ...
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "GLSL/TreeVertProg.glsl",
                shaderDefines[TREE_PROGRAM_DIST_SHADOW],
                GL_VERTEX_SHADER));
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "GLSL/TreeFragProg.glsl",
        ...
        for (int i = 0; i < numUniformNamesNDNA; i++) {
            treeShaders[TREE_PROGRAM_NEAR_BASIC]->SetUniformLocation(
              uniformNamesNDNA[i]);
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->SetUniformLocation(
              uniformNamesNDNA[i]);
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->SetUniformLocation(
              (i != 3) ? "$UNUSED$" : uniformNamesNDNA[i]);
        }
        ...
        // NA, DA: indices [numUniformNamesNDNA + 1, numUniformNamesNDNA +
        // numUniformNamesNADA]
        ...
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->SetUniform3fv(
          3, &sunLighting->groundAmbientColor[0]);
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->SetUniform3fv(
          4, &sunLighting->groundDiffuseColor[0]);
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->SetUniform4f(
          6,
        ...
        treeShaders[TREE_PROGRAM_NEAR_SHADOW]->SetUniform3fv(
          3, &sunLighting->groundAmbientColor[0]);
        treeShaders[TREE_PROGRAM_NEAR_SHADOW]->SetUniform3fv(
          4, &sunLighting->groundDiffuseColor[0]);
        treeShaders[TREE_PROGRAM_NEAR_SHADOW]->SetUniform1f(
          9, 1.0f - (sky->GetLight()->GetGroundShadowDensity() * 0.5f));
        ...
        treeShaders[TREE_PROGRAM_DIST_SHADOW]->SetUniform3fv(
          3, &sunLighting->groundAmbientColor[0]);
        treeShaders[TREE_PROGRAM_DIST_SHADOW]->SetUniform1f(
          9, 1.0f - (sky->GetLight()->GetGroundShadowDensity() * 0.5f));
        ...
        treeShaders[TREE_PROGRAM_NEAR_BASIC] =
          shaderHandler->CreateProgramObject(
            "[TreeDrawer]", shaderNames[TREE_PROGRAM_NEAR_BASIC] + "ARB", true);
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->AttachShaderObject(
          shaderHandler->CreateShaderObject(
            "ARB/treeNS.vp", "", GL_VERTEX_PROGRAM_ARB));
        treeShaders[TREE_PROGRAM_NEAR_BASIC]->Link();
        ...
        if (CShadowHandler::ShadowsSupported()) {
            treeShaders[TREE_PROGRAM_NEAR_SHADOW] =
              shaderHandler->CreateProgramObject(
                "[TreeDrawer]",
                shaderNames[TREE_PROGRAM_NEAR_SHADOW] + "ARB",
                true);
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "ARB/tree.vp", "", GL_VERTEX_PROGRAM_ARB));
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "ARB/treeFPshadow.fp", "", GL_FRAGMENT_PROGRAM_ARB));
            treeShaders[TREE_PROGRAM_NEAR_SHADOW]->Link();
            treeShaders[TREE_PROGRAM_DIST_SHADOW] =
              shaderHandler->CreateProgramObject(
                "[TreeDrawer]",
                shaderNames[TREE_PROGRAM_DIST_SHADOW] + "ARB",
                true);
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "ARB/treeFar.vp", "", GL_VERTEX_PROGRAM_ARB));
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->AttachShaderObject(
              shaderHandler->CreateShaderObject(
                "ARB/treeFPshadow.fp", "", GL_FRAGMENT_PROGRAM_ARB));
            treeShaders[TREE_PROGRAM_DIST_SHADOW]->Link();
...
CAdvTreeDrawer::DrawTreeVertexA(CVertexArray* va,
                                ...
                                float dy)
{
    SetArrayQ(va, TEX_LEAF_START_X1 + dx, TEX_LEAF_START_Y1 + dy, ftpos);
    ftpos.y += MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_START_X1 + dx, TEX_LEAF_END_Y1 + dy, ftpos);
    ftpos.x -= MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_END_X1 + dx, TEX_LEAF_END_Y1 + dy, ftpos);
    ftpos.y -= MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_END_X1 + dx, TEX_LEAF_START_Y1 + dy, ftpos);
    ftpos.x += HALF_MAX_TREE_HEIGHT;

    ftpos.z += HALF_MAX_TREE_HEIGHT;

    SetArrayQ(va, TEX_LEAF_START_X2 + dx, TEX_LEAF_START_Y2 + dy, ftpos);
    ftpos.y += MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_START_X2 + dx, TEX_LEAF_END_Y2 + dy, ftpos);
    ftpos.z -= MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_END_X2 + dx, TEX_LEAF_END_Y2 + dy, ftpos);
    ftpos.y -= MAX_TREE_HEIGHT;
                                  ...
                                  bool enlarge)
{
    ...
    SetArrayQ(va, TEX_LEAF_START_X1 + dx, TEX_LEAF_START_Y4 + dy, base);
    base.y += MAX_TREE_HEIGHT;
    SetArrayQ(va, TEX_LEAF_START_X1 + dx, TEX_LEAF_END_Y4 + dy, base);
    base -= (swd * 2.0f);
    SetArrayQ(va, TEX_LEAF_END_X1 + dx, TEX_LEAF_END_Y4 + dy, base);
    base.y -= MAX_TREE_HEIGHT;
...
{
    CAdvTreeSquareDrawer(CAdvTreeDrawer* td,
                         int cx,
                         int cy,
                         float treeDistance,
                         bool drawDetailed)
...
void
CAdvTreeSquareDrawer::DrawQuad(int x, int y)
{
    ...
    float3 dif;
    dif.x = camera->GetPos().x - ((x * SQUARE_SIZE * TREE_SQUARE_SIZE) +
                                  (SQUARE_SIZE * TREE_SQUARE_SIZE / 2));
    dif.y = 0.0f;
    dif.z = camera->GetPos().z - ((y * SQUARE_SIZE * TREE_SQUARE_SIZE) +
                                  (SQUARE_SIZE * TREE_SQUARE_SIZE / 2));
    ...
    if (distFactor < MID_TREE_DIST_FACTOR) {
        ...
        if (tss->dispList == 0) {
            ...
            va->Initialize();
            va->EnlargeArrays(12 * tss->trees.size(),
                              0,
                              VA_SIZE_T); //! alloc room for all tree vertexes
            ...
            for (auto ti = tss->trees.cbegin(); ti != tss->trees.cend(); ++ti) {
                ...
                if (ts->type < 8) {
                    CAdvTreeDrawer::DrawTreeVertexMid(
                      va, ts->pos, (ts->type) * 0.125f, 0.5f, false);
                } else {
                    CAdvTreeDrawer::DrawTreeVertexMid(
                      va, ts->pos, (ts->type - 8) * 0.125f, 0.0f, false);
                }
            }
    ...
    if (distFactor < FAR_TREE_DIST_FACTOR) {
        ...
        if ((tss->farDispList == 0) || (dif.dot(tss->viewVector) < 0.97f)) {
            ...
            CVertexArray* va = GetVertexArray();
            va->Initialize();
            va->EnlargeArrays(4 * tss->trees.size(),
                              0,
                              VA_SIZE_T); //! alloc room for all tree vertexes
            tss->viewVector = dif;
            ...
            for (auto ti = tss->trees.cbegin(); ti != tss->trees.cend(); ++ti) {
                ...
                if (f == NULL)
                    continue;
                // note: will cause some trees to be invisible if list is not
                // refreshed
                if (!f->IsInLosForAllyTeam(gu->myAllyTeam))
                    continue;

                if (ts->type < 8) {
                    CAdvTreeDrawer::DrawTreeVertexFar(va,
                                                      ts->pos,
                                                      side *
                                                        HALF_MAX_TREE_HEIGHT,
                                                      (ts->type) * 0.125f,
                                                      0.5f,
                                                      false);
                } else {
        ...
        if (distFactor > FADE_TREE_DIST_FACTOR) {
            ...
            if (!blendEnabled) {
                const float alpha =
                  1.0f - ((distFactor - FADE_TREE_DIST_FACTOR) /
                          (FAR_TREE_DIST_FACTOR - FADE_TREE_DIST_FACTOR));
                glEnable(GL_BLEND);
...
CAdvTreeDrawer::Draw(float treeDistance, bool drawReflection)
{
    ...
    CAdvTreeSquareDrawer drawer(this,
                                cx,
                                cy,
                                treeDistance * SQUARE_SIZE * TREE_SQUARE_SIZE,
                                drawDetailed);
    ...
    readMap->GridVisibility(
      nullptr, &drawer, drawer.treeDistance * 2.0f, TREE_SQUARE_SIZE);

    if (drawDetailed) {
        // draw near-trees
        const int xstart =
          Clamp(cx - 2, 0, mapDims.mapx / TREE_SQUARE_SIZE - 1);
        const int xend = Clamp(cx + 2, 0, mapDims.mapx / TREE_SQUARE_SIZE - 1);
        const int ystart =
          Clamp(cy - 2, 0, mapDims.mapy / TREE_SQUARE_SIZE - 1);
        ...
        if (shadowHandler->ShadowsLoaded()) {
            ...
            if (globalRendering->haveGLSL) {
                treeShader->SetUniformMatrix4fv(
                  7, false, shadowHandler->GetShadowMatrixRaw());
                treeShader->SetUniform4fv(
                  8, &(shadowHandler->GetShadowParams().x));
            }
            ...
            if (!globalRendering->haveGLSL) {
                ...
                treeShader->SetUniformTarget(GL_VERTEX_PROGRAM_ARB);
                treeShader->SetUniform4f(
                  15, 1.0f / mx, 1.0f / my, 1.0f / mx, 1.0f);
            }
        }
        ...
        if (globalRendering->haveGLSL) {
            ...
            treeShader->SetUniform2f(
              5, 0.20f * (1.0f / MAX_TREE_HEIGHT), 0.85f);
        } else {
            treeShader->SetUniformTarget(GL_VERTEX_PROGRAM_ARB);
            treeShader->SetUniform3f(
              13, cam->GetRight().x, cam->GetRight().y, cam->GetRight().z);
            treeShader->SetUniform3f(
              9, cam->GetUp().x, cam->GetUp().y, cam->GetUp().z);
            treeShader->SetUniform4f(11,
                                     sunLighting->groundDiffuseColor.x,
                                     sunLighting->groundDiffuseColor.y,
                                     sunLighting->groundDiffuseColor.z,
                                     0.85f);
            treeShader->SetUniform4f(14,
                                     sunLighting->groundAmbientColor.x,
                                     sunLighting->groundAmbientColor.y,
                                     sunLighting->groundAmbientColor.z,
                                     0.85f);
            treeShader->SetUniform4f(
              ...
              0.0f,
              0.0f,
              0.20f * (1.0f / MAX_TREE_HEIGHT)); // w = alpha/height modifier
        }
        ...
        for (TreeSquareStruct* pTSS = &treeSquares[ystart * treesX];
             pTSS <= &treeSquares[yend * treesX];
             pTSS += treesX) {
            for (TreeSquareStruct* tss = pTSS + xstart; tss <= (pTSS + xend);
                 ++tss) {
                tss->lastSeen = gs->frameNum;
                va->EnlargeArrays(
                  12 * tss->trees.size(),
                ...
                for (auto ti = tss->trees.cbegin(); ti != tss->trees.cend();
                     ++ti) {
                    ...
                    if (!f->IsInLosForAllyTeam(gu->myAllyTeam))
                        continue;
                    if (!cam->InView(ts->pos +
                                       (UpVector * (MAX_TREE_HEIGHT / 2.0f)),
                                     MAX_TREE_HEIGHT / 2.0f))
                        continue;
                    ...
                    if (camDist < (SQUARE_SIZE * SQUARE_SIZE * 110 * 110)) {
                        // draw detailed near-distance tree (same as
                        // mid-distance trees without alpha)
                        treeShader->SetUniform3f(
                          ((globalRendering->haveGLSL) ? 2 : 10),
                          ts->pos.x,
                          ts->pos.y,
                          ts->pos.z);
                        glCallList(dispList);
                    } else if (camDist <
                               (SQUARE_SIZE * SQUARE_SIZE * 125 * 125)) {
                        // draw mid-distance tree
                        const float relDist = (ts->pos.distance(cam->GetPos()) -
                                               SQUARE_SIZE * 110) /
                                              (SQUARE_SIZE * 15);
                        ...
                        CAdvTreeDrawer::DrawTreeVertex(
                          va, ts->pos, type * 0.125f, dy, false);
                    }
                }
            }
        }
        ...
        treeShader->SetUniform3f(
          ((globalRendering->haveGLSL) ? 2 : 10), 0.0f, 0.0f, 0.0f);

        // draw trees that have been marked as falling
        for (auto fti = fallingTrees.cbegin(); fti != fallingTrees.cend();
             ++fti) {
            ...
            if (!losHandler->InLos(pos, gu->myAllyTeam))
                continue;
            if (!cam->InView(pos + (UpVector * (MAX_TREE_HEIGHT / 2.0f)),
                             MAX_TREE_HEIGHT / 2.0f))
                continue;

            const float ang = fti->fallPos * PI;

            const float3 yvec(fti->dir.x * std::sin(ang),
                              std::cos(ang),
                              fti->dir.z * std::sin(ang));
        ...
        for (const FadeTree& fTree : fadeTrees) {
            ...
            CAdvTreeDrawer::DrawTreeVertex(
              va, fTree.pos, fTree.type * 0.125f, fTree.deltaY, false);
...
void
CAdvTreeSquareShadowPassDrawer::DrawQuad(int x, int y)
{
    const int treesX = td->treesX;
    ITreeDrawer::TreeSquareStruct* tss = &td->treeSquares[(y * treesX) + x];
...
CAdvTreeDrawer::DrawShadowPass()
{
    ...
    const int activeFarTex =
      (cam->GetDir().z < 0.0f) ? treeGen->farTex[0] : treeGen->farTex[1];
    ...
    const int cx = drawer.cx =
      (int)(cam->GetPos().x / (SQUARE_SIZE * TREE_SQUARE_SIZE));
    const int cy = drawer.cy =
      (int)(cam->GetPos().z / (SQUARE_SIZE * TREE_SQUARE_SIZE));
    ...
    readMap->GridVisibility(
      nullptr, &drawer, drawer.treeDistance * 2.0f, TREE_SQUARE_SIZE, 1);
    ...
    if (drawDetailed) {
        ...
        po = shadowHandler->GetShadowGenProg(
          CShadowHandler::SHADOWGEN_PROGRAM_TREE_NEAR);
        po->Enable();
        ...
        if (globalRendering->haveGLSL) {
            ...
            po->SetUniform4f(13,
                             ...
                             0.0f);
            po->SetUniform4f(
              9, cam->GetUp().x, cam->GetUp().y, cam->GetUp().z, 0.0f);
            ...
            for (TreeSquareStruct* tss = pTSS + xstart; tss <= pTSS + xend;
                ...
                for (auto ti = tss->trees.cbegin(); ti != tss->trees.cend();
                    ...
                    if (!f->IsInLosForAllyTeam(gu->myAllyTeam))
                        continue;
                    if (!cam->InView(ts->pos +
                                       float3(0, MAX_TREE_HEIGHT / 2, 0),
                                     MAX_TREE_HEIGHT / 2 + 150))
                        continue;
                    ...
                    if (camDist < SQUARE_SIZE * SQUARE_SIZE * 110 * 110) {
                        po->SetUniform3f((globalRendering->haveGLSL ? 3 : 10),
                                         ts->pos.x,
                                         ts->pos.y,
                                         ts->pos.z);
                        glCallList(dispList);
        ...
        po->SetUniform3f(
          (globalRendering->haveGLSL ? 3 : 10), 0.0f, 0.0f, 0.0f);
        ...
        po->Disable();
        po = shadowHandler->GetShadowGenProg(
          CShadowHandler::SHADOWGEN_PROGRAM_TREE_FAR);
        po->Enable();
