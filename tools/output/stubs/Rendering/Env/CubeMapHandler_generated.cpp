...
CubeMapHandler::Init()
{
...

    {
        ...
        glTexParameteri(
          GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(
          GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(
          GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(
          GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB,
                           specTexSize,
                           float3(1, 1, 1),
                           float3(0, 0, -2),
                           float3(0, -2, 0));
        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB,
                           specTexSize,
                           float3(-1, 1, -1),
                           float3(0, 0, 2),
                           float3(0, -2, 0));
        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB,
                           specTexSize,
                           float3(-1, 1, -1),
                           float3(2, 0, 0),
                           float3(0, 0, 2));
        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB,
                           specTexSize,
                           float3(-1, -1, 1),
                           float3(2, 0, 0),
                           float3(0, 0, -2));
        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB,
                           specTexSize,
                           float3(-1, 1, 1),
                           float3(2, 0, 0),
                           float3(0, -2, 0));
        CreateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB,
                           specTexSize,
                           float3(1, 1, -1),
                           float3(-2, 0, 0),
                           float3(0, -2, 0));
    ...
    if (reflectionCubeFBO.IsValid()) {
        ...
        reflectionCubeFBO.CreateRenderBuffer(GL_DEPTH_ATTACHMENT_EXT,
                                             GL_DEPTH_COMPONENT,
                                             reflTexSize,
                                             reflTexSize);
...
CubeMapHandler::UpdateReflectionTexture()
{
    ...
    switch (currReflectionFace) {
        case 0: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB, RgtVector, false);
        } break;
        case 1: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB, -RgtVector, false);
        } break;
        case 2: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB, UpVector, false);
        } break;
        case 3: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB, -UpVector, false);
        } break;
        case 4: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB, FwdVector, false);
        } break;
        case 5: {
            CreateReflectionFace(
              GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB, -FwdVector, false);
        } break;
        default: {
        } break;
    }
    ...
    if (mapSkyReflections) {
        ...
        switch (currReflectionFace) {
            case 6: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB, RgtVector, true);
            } break;
            case 7: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB, -RgtVector, true);
            } break;
            case 8: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB, UpVector, true);
            } break;
            case 9: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB, -UpVector, true);
            } break;
            case 10: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB, FwdVector, true);
            } break;
            case 11: {
                CreateReflectionFace(
                  GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB, -FwdVector, true);
            } break;
            default: {
            } break;
        }
...
CubeMapHandler::CreateReflectionFace(unsigned int glType,
                                     ...
                                     bool skyOnly)
{
    reflectionCubeFBO.AttachTexture(
      (skyOnly ? skyReflectionTexID : envReflectionTexID), glType);
...

    {
        ...
        switch (glType) {
            case GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB: { /*fwd =  Rgt*/
                curCam->right = FwdVector;
                curCam->up = UpVector;
            } break;
            case GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB: { /*fwd = -Rgt*/
                curCam->right = -FwdVector;
                curCam->up = UpVector;
            } break;
            case GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB: { /*fwd =   Up*/
                curCam->right = RgtVector;
                curCam->up = -FwdVector;
            } break;
            case GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB: { /*fwd =  -Up*/
                curCam->right = RgtVector;
                curCam->up = FwdVector;
            } break;
            case GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB: { /*fwd =  Fwd*/
                curCam->right = -RgtVector;
                curCam->up = UpVector;
            } break;
            case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB: { /*fwd = -Fwd*/
                curCam->right = RgtVector;
                curCam->up = UpVector;
            } break;
            default: {
...
CubeMapHandler::UpdateSpecularTexture()
{
    ...
    switch (specularTexIter % 3) {
        case 0: {
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB,
                               specTexSize,
                               float3(1, 1, 1),
                               float3(0, 0, -2),
                               float3(0, -2, 0),
                               specularTexRow,
                               &specTexBuf[0]);
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB,
                               specTexSize,
                               float3(-1, 1, -1),
                               float3(0, 0, 2),
                               float3(0, -2, 0),
        ...
        case 1: {
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB,
                               specTexSize,
                               float3(-1, 1, -1),
                               float3(2, 0, 0),
                               float3(0, 0, 2),
                               specularTexRow,
                               &specTexBuf[0]);
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB,
                               specTexSize,
                               float3(-1, -1, 1),
                               float3(2, 0, 0),
                               float3(0, 0, -2),
        ...
        case 2: {
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB,
                               specTexSize,
                               float3(-1, 1, 1),
                               float3(2, 0, 0),
                               float3(0, -2, 0),
                               specularTexRow,
                               &specTexBuf[0]);
            UpdateSpecularFace(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB,
                               specTexSize,
                               float3(1, 1, -1),
                               float3(-2, 0, 0),
                               float3(0, -2, 0),
                                       ...
                                       unsigned char* buf)
{
    ...
    for (int x = 0; x < size; ++x) {
        const float3 dir =
          (cdir + (xdif * (x + 0.5f)) / size + (ydif * (y + 0.5f)) / size)
            .Normalize();
        const float dot =
          std::max(0.0f, dir.dot(sky->GetLight()->GetLightDir()));
        const float spec =
          std::min(1.0f,
                   std::pow(dot, sunLighting->specularExponent) +
                     std::pow(dot, 3.0f) * 0.25f);
                                   ...
                                   const float3& ydif)
{
    ...
    for (int y = 0; y < size; ++y) {
        CreateSpecularFacePart(
          texType, size, cdir, xdif, ydif, y, &buf[y * size * 4]);
    }

    //! note: no mipmaps, cubemap linear filtering is broken
    glTexImage2D(
      texType, 0, GL_RGBA8, size, size, 0, GL_RGBA, GL_UNSIGNED_BYTE, &buf[0]);
}
