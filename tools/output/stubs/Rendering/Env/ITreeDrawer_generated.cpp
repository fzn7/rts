...
ITreeDrawer::AddTrees()
{
    ...
    for (CFeatureSet::const_iterator it = features.begin();
         it != features.end();
         ++it) {
        const CFeature* f = *it;
...
ITreeDrawer::AddTree(int treeID, int treeType, const float3& pos, float size)
{
    ...
    const int treeSquareIdx = (((int)pos.x) / (treeSquareSize)) +
...
ITreeDrawer::DeleteTree(int treeID, const float3& pos)
{
    ...
    VectorEraseIf(treeSquares[treeSquareIdx].trees,
                  [treeID](const TreeStruct& ts) { return (treeID == ts.id); });
    ResetPos(pos);
}
...
void
ITreeDrawer::Draw(bool drawReflection)
{
    const float maxDistance =
      CGlobalRendering::MAX_VIEW_RANGE / (SQUARE_SIZE * TREE_SQUARE_SIZE);
...
ITreeDrawer::RenderFeatureDestroyed(const CFeature* feature)
{
    if (feature->def->drawType >= DRAWTYPE_TREE) {
        ...
        if (feature->speed.SqLength2D() > 0.25f) {
            AddFallingTree(feature->id,
                           feature->def->drawType - 1,
                           feature->pos,
                           feature->speed * XZVector);
        }
    }
}
