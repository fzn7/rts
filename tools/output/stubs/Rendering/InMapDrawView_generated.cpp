...
CInMapDrawView::CInMapDrawView()
{
    ...
    for (int y = 0; y < 64; y++) {
        ...
        for (int x = 0; x < 64; x++) {
            float dist =
              std::sqrt((float)(x - 32) * (x - 32) + (y - 32) * (y - 32));
            if (dist > 31.0f) {
    ...
    glBuildMipmaps(
      GL_TEXTURE_2D, GL_RGBA8, 128, 64, GL_RGBA, GL_UNSIGNED_BYTE, tex[0]);
}
...
void
InMapDraw_QuadDrawer::DrawPoint(const CInMapDrawModel::MapPoint* point) const
{
    ...
    const unsigned char* color =
      point->IsBySpectator() ? color4::white
                             : teamHandler->Team(point->GetTeamID())->color;
    const unsigned char col[4] = { color[0], color[1], color[2], 200 };
...
{
    const unsigned char* color =
      line->IsBySpectator() ? color4::white
                            : teamHandler->Team(line->GetTeamID())->color;
    linesVa->AddVertexQC(
      line->GetPos1() - (line->GetPos1() - camera->GetPos()).ANormalize() * 26,
      color);
    linesVa->AddVertexQC(
      line->GetPos2() - (line->GetPos2() - camera->GetPos()).ANormalize() * 26,
      color);
}
...
InMapDraw_QuadDrawer::DrawQuad(int x, int y)
{
    ...
    //! draw point markers
    for (std::list<CInMapDrawModel::MapPoint>::const_iterator pi =
           dq->points.begin();
         pi != dq->points.end();
         ++pi) {
    ...
    //! draw line markers
    for (std::list<CInMapDrawModel::MapLine>::const_iterator li =
           dq->lines.begin();
         li != dq->lines.end();
         ++li) {
...
CInMapDrawView::Draw()
{
    ...
    readMap->GridVisibility(
      nullptr, &drawer, 1e9, CInMapDrawModel::DRAW_QUAD_SIZE);
    ...
    // width greater than 2 causes GUI flicker on ATI hardware as of driver
    // version 9.3 so redraw lines with width 1
    ...
    if (!visibleLabels.empty()) {
        ...
        //! draw point labels
        for (std::vector<const CInMapDrawModel::MapPoint*>::const_iterator pi =
               visibleLabels.begin();
             pi != visibleLabels.end();
             ++pi) {
            ...
            const unsigned char* color =
              (*pi)->IsBySpectator()
                ? color4::white
                : teamHandler->Team((*pi)->GetTeamID())->color;
            font->SetTextColor(color[0] / 255.0f,
                               color[1] / 255.0f,
                               color[2] / 255.0f,
                               1.0f); // FIXME (overload!)
