...
{ // wrapper to allow usage as shared_ptr
    SP_Byte(size_t size) { vec.resize(size); }
    FT_Byte* data() { return vec.data(); }
...
struct FontFace
{
    FontFace(FT_Face f, std::shared_ptr<SP_Byte>& mem)
      : face(f)
      , memory(mem)
    {}
    ~FontFace()
...
class FtLibraryHandler
{
  public:
    ...
    // singleton
#ifndef WIN32
        std::call_once(flag, []() { singleton.reset(new FtLibraryHandler()); });
#else
...
GetKerningHash(char32_t lchar, char32_t rchar)
{
...
GetFontFace(const std::string& fontfile, const int size)
{
    ...
    // load the font
    FT_Face face = NULL;
    FT_Error error = FT_New_Memory_Face(FtLibraryHandler::GetLibrary(),
                                        fontMem.get()->data(),
                                        filesize,
                                        0,
                                        &face);
                     ...
                     const int origSize)
{
    ...
    FcPattern* pattern = FcPatternCreate();
    {
        ...
        {
            const FcChar8* ftname =
              reinterpret_cast<const FcChar8*>("not used");
            FcBlanks* blanks = FcBlanksCreate();
            FcPattern* origPattern =
  ...
  , face(NULL)
{
    ...
    fontDescender =
      normScale * FT_MulFix(face->descender, face->size->metrics.y_scale);
    // lineHeight = FT_MulFix(face->height, face->size->metrics.y_scale); // bad
    // results
    ...
    // precache ASCII glyphs & kernings (save them in an array for better lvl2
    // cpu cache hitrate)
    ...
    for (char32_t i = 32; i < 127; ++i) {
        ...
        for (char32_t j = 32; j < 127; ++j) {
            ...
            FT_Vector kerning;
            FT_Get_Kerning(
              face, lgl.index, rgl.index, FT_KERNING_DEFAULT, &kerning);
...
CFontTexture::GetKerning(const GlyphInfo& lgl, const GlyphInfo& rgl)
{
    ...
    FT_Vector kerning;
    FT_Get_Kerning(
      lgl.face, lgl.index, rgl.index, FT_KERNING_DEFAULT, &kerning);
...
CFontTexture::LoadBlock(char32_t start, char32_t end)
{
    ...
    do {
        ...
        usedFallbackFonts.insert(f);
             ...
             (alreadyCheckedFonts.find(f) == alreadyCheckedFonts.end()));
#endif
    // load fail glyph for all remaining ones (they will all share the same fail
    // glyph)
    for (auto c : map) {
    ...
    // readback textureatlas allocator data
    {
        ...
        if (!success)
            LOG_L(L_WARNING,
                  "Texture limit reached! (try to reduce the font size and/or "
                  "outlinewidth)");

        wantedTexWidth = atlasAlloc.GetAtlasSize().x;
        wantedTexHeight = atlasAlloc.GetAtlasSize().y;
        if ((atlasUpdate->xsize != wantedTexWidth) ||
            (atlasUpdate->ysize != wantedTexHeight)) {
            (*atlasUpdate) =
              atlasUpdate->CanvasResize(wantedTexWidth, wantedTexHeight, false);
        }
        ...
        if ((atlasUpdateShadow->xsize != wantedTexWidth) ||
            (atlasUpdateShadow->ysize != wantedTexHeight)) {
            (*atlasUpdateShadow) = atlasUpdateShadow->CanvasResize(
              wantedTexWidth, wantedTexHeight, false);
        }
        ...
        for (char32_t i = start; i < end; ++i) {
            ...
            glyphs[i].texCord = IGlyphRect(texpos[0],
                                           texpos[1],
                                           texpos[2] - texpos[0],
                                           texpos[3] - texpos[1]);
            glyphs[i].shadowTexCord = IGlyphRect(texpos2[0],
                                                 texpos2[1],
                                                 texpos2[2] - texpos2[0],
                                                 texpos2[3] - texpos2[1]);
            ...
            if (texpos2[2] != 0)
                atlasUpdateShadow->CopySubImage(
                  *glyphbm, texpos2.x + outlineSize, texpos2.y + outlineSize);
            delete glyphbm;
            glyphbm = NULL;
        }
        atlasAlloc.clear();
                        ...
                        unsigned index)
{
    ...
    atlasAlloc.AddEntry(
      IntToString(ch) + "sh",
      int2(width + 2 * outlineSize, height + 2 * outlineSize));
#endif
}
...
CFontTexture::CreateTexture(const int width, const int height)
{
    ...
    glTexImage2D(
      GL_TEXTURE_2D, 0, GL_ALPHA, 1, 1, 0, GL_ALPHA, GL_UNSIGNED_BYTE, NULL);
