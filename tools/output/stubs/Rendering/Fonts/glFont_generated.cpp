                  ...
                  float outlineweight)
{
    try {
        CglFont* newFont =
          new CglFont(fontFile, size, outlinewidth, outlineweight);
        return newFont;
...
{
    CglFont::ColorMap::iterator& sci =
      *reinterpret_cast<CglFont::ColorMap::iterator*>(data);
    glColor4fv(*sci++);
}
    ...
    while (pos < text.length()) {
        ...
        switch (u) {
            ...
            default:
                ...
                if (g.descender < d)
                    d = g.descender;
                if (multiLine < 2 && g.height > h)
                    h = g.height; // only calc height for the first line
        }
    }

    if (multiLine > 1)
        d -= (multiLine - 1) * GetLineHeight();
    if (descender)
        *descender = d;
    if (numLines)
        *numLines = multiLine;
...
{
    const float luminosity = 0.05 + 0.2126f * std::pow(textColor[0], 2.2) +
                             0.7152f * std::pow(textColor[1], 2.2) +
                             0.0722f * std::pow(textColor[2], 2.2);

    const float lumdiff = std::max(luminosity, darkLuminosity) /
                          std::min(luminosity, darkLuminosity);
    if (lumdiff > 5.0f) {
        return &darkOutline;
    } else {
        return &lightOutline;
...
CglFont::End()
{
    ...
    // Because texture size can change, texture coordinats are absolute in
    // texels. We could use also just use GL_TEXTURE_RECTANGLE but then all
    // shaders would need to detect so and use different funcs & types if
    // supported -> more work
    ...
    if (stripTextColors.size() > 1) {
        ...
        va.DrawArray2dT(
          GL_QUADS, TextStripCallback, &sci); // FIXME calls a 0 length strip!
    } else {
        if (setColor)
                      ...
                      const std::string& str)
{
     ...
     *
     * 1. When using DLs, we can not group multiple glyphs into one glBegin/End
     * pair because glTranslatef can not go between such a pair.
     * 2. We can now eliminate all glPushMatrix/PopMatrix pairs related to font
     * rendering because the transformations are calculated on the fly. These
     * are just a couple of floating point multiplications and shouldn't be
     * too expensive.
     */
    ...
    do {
        const bool endOfString = SkipColorCodesAndNewLines(
          ustr, &i, &newColor, &colorChanged, &skippedLines, &baseTextColor);
        ...
        const float dx0 = (scaleX * g->size.x0()) + x,
                    dy0 = (scaleY * g->size.y0()) + y;
        const float dx1 = (scaleX * g->size.x1()) + x,
                    dy1 = (scaleY * g->size.y1()) + y;
                            ...
                            const std::string& str)
{
    ...
    do {
        ...
        // draw shadow
        va2.AddVertexQ2dT(
          dx0 + shiftX - ssX, dy1 - shiftY - ssY, stc.x0(), stc.y1());
        va2.AddVertexQ2dT(
          dx0 + shiftX - ssX, dy0 - shiftY + ssY, stc.x0(), stc.y0());
        va2.AddVertexQ2dT(
          dx1 + shiftX + ssX, dy0 - shiftY + ssY, stc.x1(), stc.y0());
        va2.AddVertexQ2dT(
          dx1 + shiftX + ssX, dy1 - shiftY - ssY, stc.x1(), stc.y1());
                 ...
                 const std::string& text)
{
    ...
    // vertical alignment
    y +=
      sizeY * GetDescender(); // move to baseline (note: descender is negative)
    ...
    // backup text & outline colors (also ::ColorResetIndicator will reset to
    // those)
                      ...
                      const std::string& text)
{
    ...
    for (int i = 0; i < coltext.size(); ++i) {
        glPrint(x,
                y,
                s,
                (options | FONT_BASELINE) & ~(FONT_RIGHT | FONT_CENTER),
                coltext[i]);
        x += sizeX * colWidths[i];
    }
}
