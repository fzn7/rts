...
CShadowHandler::Reload(const char* argv)
{
    ...
    configHandler->Set("ShadowMapSize",
                       Clamp(nextShadowMapSize,
                             int(MIN_SHADOWMAP_SIZE),
                             int(MAX_SHADOWMAP_SIZE)));
    configHandler->Set("ShadowProjectionMode",
                       Clamp(nextShadowProMode,
                             int(SHADOWPROMODE_MAP_CENTER),
                             int(SHADOWPROMODE_MIX_CAMMAP)));
...
CShadowHandler::Init()
{
    ...
    if (shadowConfig < 0) {
        LOG("[%s] shadow rendering is disabled (config-value %d)",
            __FUNCTION__,
            shadowConfig);
        return;
    }
    ...
    if (shadowConfig > 0)
        shadowGenBits = SHADOWGEN_BIT_MODEL | SHADOWGEN_BIT_MAP |
                        SHADOWGEN_BIT_PROJ | SHADOWGEN_BIT_TREE;
    ...
    if (!globalRendering->haveARB && !globalRendering->haveGLSL) {
        LOG_L(L_WARNING,
              "[%s] GPU does not support either ARB or GLSL shaders for shadow "
              "rendering",
              __FUNCTION__);
        return;
    }
    ...
    if (!globalRendering->haveGLSL) {
        if (!GLEW_ARB_shadow || !GLEW_ARB_depth_texture ||
            ...
            LOG_L(L_WARNING,
                  "[%s] required OpenGL ARB-extensions missing for shadow "
                  "rendering",
                  __FUNCTION__);
        ...
        if (!GLEW_ARB_shadow_ambient) {
            // can't use arbitrary texvals in case the depth comparison op fails
            // (only 0)
            LOG_L(L_WARNING,
                  "[%s] \"ARB_shadow_ambient\" extension missing (will "
                  "probably make shadows darker than they should be)",
                  __FUNCTION__);
        }
    }
    ...
    if (!InitDepthTarget()) {
        ...
        LOG_L(
          L_ERROR, "[%s] failed to initialize depth-texture FBO", __FUNCTION__);
        return;
    }
...
#define sh shaderHandler
    ...
    static const std::string shadowGenProgNames[SHADOWGEN_PROGRAM_LAST] = {
        "ARB/unit_genshadow.vp", "ARB/groundshadow.vp",     "ARB/treeShadow.vp",
        "ARB/treeFarShadow.vp",  "ARB/projectileshadow.vp",
    };
    static const std::string shadowGenProgHandles[SHADOWGEN_PROGRAM_LAST] = {
        "ShadowGenShaderProgModel",      "ShadowGenshaderProgMap",
        "ShadowGenshaderProgTreeNear",   "ShadowGenshaderProgTreeDist",
    ...
    if (globalRendering->haveGLSL) {
        for (int i = 0; i < SHADOWGEN_PROGRAM_LAST; i++) {
            Shader::IProgramObject* po = sh->CreateProgramObject(
              "[ShadowHandler]", shadowGenProgHandles[i] + "GLSL", false);
            Shader::IShaderObject* so =
              sh->CreateShaderObject("GLSL/ShadowGenVertProg.glsl",
                                     shadowGenProgDefines[i] + extraDef,
                                     GL_VERTEX_SHADER);

            po->AttachShaderObject(so);
            po->Link();
            po->SetUniformLocation("shadowParams");
            po->SetUniformLocation(
              "cameraDirX"); // used by SHADOWGEN_PROGRAM_TREE_NEAR
            po->SetUniformLocation(
              "cameraDirY"); // used by SHADOWGEN_PROGRAM_TREE_NEAR
            po->SetUniformLocation(
              "treeOffset"); // used by SHADOWGEN_PROGRAM_TREE_NEAR
            po->Validate();
        ...
        for (int i = 0; i < SHADOWGEN_PROGRAM_LAST; i++) {
            Shader::IProgramObject* po = sh->CreateProgramObject(
              "[ShadowHandler]", shadowGenProgHandles[i] + "ARB", true);
            Shader::IShaderObject* so = sh->CreateShaderObject(
              shadowGenProgNames[i], "", GL_VERTEX_PROGRAM_ARB);
...
CShadowHandler::InitDepthTarget()
{
    ...
    glTexParameteri(
      ...
      GL_LINEAR); // shadowtex linear sampling is for-free on NVidias
...
CShadowHandler::WorkaroundUnsupportedFboRenderTargets()
{
    ...
    // some drivers/GPUs fail to render to GL_CLAMP_TO_BORDER (and GL_LINEAR may
    // cause a drop in performance for them, too)
    {
        ...
        const GLint texFormat = globalRendering->support24bitDepthBuffers
    ...
    // ATI sometimes fails without an attached color texture, so check a few
    // formats (not all supported texture formats are renderable)
    {
        glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT);
        glReadBuffer(GL_COLOR_ATTACHMENT0_EXT);
        ...
        fb.CreateRenderBuffer(
          GL_COLOR_ATTACHMENT0_EXT, GL_RED, shadowMapSize, shadowMapSize);
...
CShadowHandler::SetShadowMapSizeFactors()
{
    ...
    const float shadowMapX =
      std::sqrt(std::fabs(shadowProjMinMax.y)); // math::sqrt( |x2| )
    const float shadowMapY =
      std::sqrt(std::fabs(shadowProjMinMax.w)); // math::sqrt( |y2| )
    const float shadowMapW =
      shadowMapX +
      std::sqrt(std::fabs(
        shadowProjMinMax.x)); // math::sqrt( |x2| ) + math::sqrt( |x1| )
    const float shadowMapH =
      shadowMapY +
      std::sqrt(std::fabs(
        shadowProjMinMax.z)); // math::sqrt( |y2| ) + math::sqrt( |y1| )
...
ComposeLightMatrix(const ISkyLight* light)
{
    ...
    lightMatrix.SetY(
      ((lightMatrix.GetX()).cross(lightMatrix.GetZ())).ANormalize());
...
ComposeScaleMatrix(const float4 scales)
{
    ...
    return (CMatrix44f(FwdVector * 0.5f,
                       RgtVector / scales.x,
                       UpVector / scales.y,
                       FwdVector / scales.w));
}
...
{
    const CMatrix44f lightMatrix =
      std::move(ComposeLightMatrix(sky->GetLight()));
    const CMatrix44f scaleMatrix = std::move(ComposeScaleMatrix(
      GetShadowProjectionScales(playerCam, -lightMatrix.GetZ())));

    // convert xy-diameter to radius
    shadowCam->SetFrustumScales(shadowProjScales *
...
#else
    // KISS; define only the world-to-light transform (P[CULLING] is unused
    // anyway)
    //
    // we have two options: either place the camera such that it *looks at*
    // projMidPos (along lightMatrix.GetZ()) or such that it is *at or behind*
    // projMidPos looking in the inverse direction (the latter is chosen here)
    viewMatrix[SHADOWMAT_TYPE_CULLING].LoadIdentity();
    viewMatrix[SHADOWMAT_TYPE_CULLING].SetX(-std::move(lightMatrix.GetX()));
    viewMatrix[SHADOWMAT_TYPE_CULLING].SetY(std::move(lightMatrix.GetY()));
    viewMatrix[SHADOWMAT_TYPE_CULLING].SetZ(-std::move(lightMatrix.GetZ()));
    ...
    // shaders need this form, projection into SM-space is done by
    // shadow2DProj() note: ShadowGenVertProg is a special case because it does
    // not use uniforms
    ...
    viewMatrix[SHADOWMAT_TYPE_DRAWING].Scale(
      float3(scaleMatrix[0],
             scaleMatrix[5],
             scaleMatrix[10])); // extract (X.x, Y.y, Z.z)
    viewMatrix[SHADOWMAT_TYPE_DRAWING].Transpose();
    viewMatrix[SHADOWMAT_TYPE_DRAWING].SetPos(
      viewMatrix[SHADOWMAT_TYPE_DRAWING] * -projMidPos[2]);
    viewMatrix[SHADOWMAT_TYPE_DRAWING].SetPos(
      viewMatrix[SHADOWMAT_TYPE_DRAWING].GetPos() +
      (FwdVector * 0.5f)); // add z-bias
...
CShadowHandler::CreateShadows()
{
    ...
    if (globalRendering->haveARB) {
        ...
        glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB,
                                   16,
                                   shadowTexProjCenter.x,
                                   shadowTexProjCenter.y,
                                   0.0f,
                                   0.0f);
        glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB,
                                   17,
                                   shadowTexProjCenter.z,
                                   shadowTexProjCenter.z,
                                   0.0f,
                                   0.0f);
        glProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB,
                                   18,
                                   shadowTexProjCenter.w,
                                   shadowTexProjCenter.w,
                                   0.0f,
                                   0.0f);
    }
    ...
    if (sunProjDir != sunDir) {
        ...
        if (sunDirXZ.x >= 0.0f) {
            if (sunDirXZ.z >= 0.0f) {
                ...
                mapVerts[1] = float3(
                  mapDims.mapx * SQUARE_SIZE, 0.0f, mapDims.mapy * SQUARE_SIZE);
            }
        } else {
            if (sunDirXZ.z >= 0.0f) {
...
CShadowHandler::GetOrthoProjectedFrustumRadius(CCamera* cam, float3& projPos)
{
    ...
    const unsigned int planes[] = { CCamera::FRUSTUM_PLANE_LFT,
                                    CCamera::FRUSTUM_PLANE_RGT };
    ...
    for (unsigned int n = 0; n < 2; n++) {
        ...
        frustumPoints[n * 2 + 0] = float3(
          cx0, CGround::GetHeightReal(cx0, cz0, false), cz0); // far-point
        frustumPoints[n * 2 + 1] = float3(
          cx1, CGround::GetHeightReal(cx1, cz1, false), cz1); // near-point
    ...
    // calculate the radius of the minimally-bounding sphere around the
    // projected frustum
    for (unsigned int n = 0; n < 4; n++) {
        frustumCenter.w =
          std::max(frustumCenter.w, (frustumPoints[n] - projPos).SqLength());
    }
