...
CVerticalSync::SetInterval(int i)
{
    ...
    if (GLXEW_EXT_swap_control) {
        // System 1
        // This is the prefered way cause glXSwapIntervalEXT won't lock the
        // thread until the OGL cmd queue is full! And so we can process
        // SimFrames while waiting for VSync.
        Display* dpy = glXGetCurrentDisplay();
        GLXDrawable drawable = glXGetCurrentDrawable();
#ifdef GLXEW_EXT_swap_control_tear
        // this enables so called `adaptive vsync` or also called late syncing
        // (~ it won't vsync if FPS < monitor refresh rate)
