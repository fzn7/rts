                    ...
                    float sqFadeDistMax = -1.0f)
{
    ...
    if (sqDist < sqFadeDistE) {
        ...
        f->drawAlpha =
          1.0f - ((sqDist - sqFadeDistB) / (sqFadeDistE - sqFadeDistB));
        return true;
    }
  ...
  : CEventClient("[CFeatureDrawer]", 313373, false)
{
    ...
    featureFadeDistance = std::min(
      configHandler->GetFloat("FeatureFadeDistance"), featureDrawDistance);
...
CFeatureDrawer::UpdateDrawQuad(CFeature* feature)
{
    ...
    const int newDrawQuadX = Clamp(
      int(feature->pos.x / SQUARE_SIZE / DRAW_QUAD_SIZE), 0, drawQuadsX - 1);
    const int newDrawQuadY = Clamp(
      int(feature->pos.z / SQUARE_SIZE / DRAW_QUAD_SIZE), 0, drawQuadsY - 1);
    const int newDrawQuad = newDrawQuadY * drawQuadsX + newDrawQuadX;

    if (oldDrawQuad == newDrawQuad)
        return;

    // TODO check if out of map features get drawn, when the camera is outside
    // of the map
    ...
    if (feature->model) {
        if (oldDrawQuad >= 0) {
            modelRenderers[oldDrawQuad]
              .GetRenderer(MDL_TYPE(feature))
              ->DelFeature(feature);
        }

        modelRenderers[newDrawQuad]
          .GetRenderer(MDL_TYPE(feature))
          ->AddFeature(feature);
    }
...
CFeatureDrawer::Draw()
{
    ...
    if (drawDeferred && !water->DrawReflectionPass() &&
    ...
    if (drawForward) {
        DrawOpaquePass(
          false, water->DrawReflectionPass(), water->DrawRefractionPass());
    }
...
{
    const auto& quads =
      camVisibleQuads[(CCamera::GetActiveCamera())->GetCamType()];
    ...
    for (int quad : quads) {
        ...
        for (const auto& binElem : featureBin) {
            ...
            for (CFeature* f : binElem.second) {
                ...
                switch (f->drawFlag) {
                    case CFeature::FD_NODRAW_FLAG: {
                        continue;
                    } break;
                    case CFeature::FD_ALPHAF_FLAG: {
                        continue;
                    } break;
                    case CFeature::FD_FARTEX_FLAG: {
                ...
                if (inShadowPass &&
                    LuaObjectDrawer::AddShadowMaterialObject(f, LUAOBJ_FEATURE))
                    continue;
                if (!inShadowPass &&
                    LuaObjectDrawer::AddOpaqueMaterialObject(f, LUAOBJ_FEATURE))
                    continue;
...
CFeatureDrawer::DrawIndividual(const CFeature* feature, bool noLuaCall)
{
    ...
    if (!LuaObjectDrawer::DrawSingleObject(feature,
                                           LUAOBJ_FEATURE /*, noLuaCall*/)) {
...
CFeatureDrawer::DrawIndividualNoTrans(const CFeature* feature, bool noLuaCall)
{
    ...
    if (!LuaObjectDrawer::DrawSingleObjectNoTrans(
          feature, LUAOBJ_FEATURE /*, noLuaCall*/)) {
        PushIndividualState(feature, false);
        ...
        for (const auto& binElem : featureBin) {
            ...
            for (CFeature* f : binElem.second) {
                ...
                switch (f->drawFlag) {
                    ...
                    case CFeature::FD_OPAQUE_FLAG: {
                        continue;
                    } break;
                    case CFeature::FD_SHADOW_FLAG: {
                        continue;
                    } break;
                    case CFeature::FD_FARTEX_FLAG: {
                        continue;
...
CFeatureDrawer::DrawShadowPass()
{
...

    {
        ...
        // needed for 3do models (else they will use any currently bound
        // texture) note: texture0 is by default a 1x1 texture with
        // rgba(0,0,0,255) (we are just interested in the 255 alpha here)
...
{
  public:
    CFeatureQuadDrawer(int _numQuadsX)
    ...
    void DrawQuad(int x, int y)
    {
        ...
        rdrProxies[y * numQuadsX + x].SetLastDrawFrame(
          globalRendering->drawFrame);
    }

    std::vector<int>& GetCamQuads() { return camQuads; }
    std::vector<CFeatureDrawer::RdrContProxy>& GetRdrProxies()
    {
        return rdrProxies;
    }
...
void
CFeatureDrawer::FlagVisibleFeatures(const CCamera* cam,
                                    bool drawShadowPass,
                                    bool drawReflection,
                                    bool drawRefraction,
        ...
        for (int i = 0; i < MODELTYPE_OTHER; ++i) {
            ...
            for (auto& binElem : featureBin) {
                for (CFeature* f : binElem.second) {
                    ...
                    if (drawShadowPass) {
                        if (SetFeatureDrawAlpha(
                              f, playerCam, sqFadeDistBegin, sqFadeDistEnd)) {
                            // no shadows for fully alpha-faded features from
                            // player's POV
                    ...
                    if (drawReflection &&
                        !CUnitDrawer::ObjectVisibleReflection(
                          f->drawMidPos, cam->GetPos(), f->GetDrawRadius()))
                        continue;

                    if (SetFeatureDrawAlpha(
                          f, cam, sqFadeDistBegin, sqFadeDistEnd)) {
                        f->UpdateTransform(f->drawPos, false);
                        f->drawFlag +=
                    ...
                    // non-fading features
                    f->drawFlag = CFeature::FD_FARTEX_FLAG * drawFarFeatures *
                                  (!f->alphaFade);
                }
            }
        }
...
CFeatureDrawer::GetVisibleFeatures(CCamera* cam, int extraSize, bool drawFar)
{
...

    {
        ...
        readMap->GridVisibility(
          cam, &drawer, featureDrawDistance, DRAW_QUAD_SIZE, extraSize);
