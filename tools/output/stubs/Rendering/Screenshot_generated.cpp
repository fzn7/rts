  ...
  public:
    ...
    void AddTask(FunctionArgs arg)
    {
        ...
        if (!myThread) {
            finished = false;
            myThread =
              new boost::thread(boost::bind(&SaverThread::SaveStuff, this));
        }
    };
...
void
TakeScreenshot(std::string type)
{
    if (type.empty())
        type = "jpg";
    ...
    if (FileSystem::CreateDirectory("screenshots")) {
        FunctionArgs args;
        args.x = globalRendering->dualScreenMode
        ...
        for (int a = configHandler->GetInt("ScreenshotCounter"); a <= 99999;
             ++a) {
            std::ostringstream fname;
            fname << "screenshots/screen" << std::setfill('0') << std::setw(5)
                  << a << '.' << type;
