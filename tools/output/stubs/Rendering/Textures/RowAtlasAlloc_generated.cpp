...
CRowAtlasAlloc::Allocate()
{
    ...
    if (npot) {
        ...
        // else for the case when Allocate() is called multiple times, the width
        // would grew faster than height also AddRow() only works with
        // PowerOfTwo values.
        atlasSize.y = next_power_of_2(atlasSize.y);
    }

    // it gives much better results when we resize the available space before
    // starting allocation esp. allocation is more horizontal and so we can clip
    // more free space at bottom
    EstimateNeededSize();
    ...
    for (auto& curtex : memtextures) {
        Row* row = FindRow(curtex->size.x + ATLAS_PADDING,
                           curtex->size.y + ATLAS_PADDING);
        if (!row) {
            success = false;
            continue;
        }
...
CRowAtlasAlloc::FindRow(int glyphWidth, int glyphHeight)
{
    ...
    for (auto& row : imageRows) {
        ...
        if ((ratio < best_ratio) ||
            ((ratio == best_ratio) && (row.width < best_width))) {
