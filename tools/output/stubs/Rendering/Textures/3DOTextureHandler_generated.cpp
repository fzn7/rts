...
C3DOTextureHandler::C3DOTextureHandler()
{
    ...
    atlasAlloc->SetMaxSize(std::min(globalRendering->maxTextureSize, 2048),
                           std::min(globalRendering->maxTextureSize, 2048));

    // default for 3DO primitives that point to non-existing textures
    textures["___dummy___"] = UnitTexture(0.0f, 0.0f, 1.0f, 1.0f);

    for (std::vector<TexFile*>::iterator it = texfiles.begin();
         it != texfiles.end();
         ++it) {
        atlasAlloc->AddEntry((*it)->name,
                             int2((*it)->tex.xsize, (*it)->tex.ysize));
    }
    ...
    if (!allocated) {
        ...
        LOG_L(L_WARNING,
              "[%s] failed to allocate 3DO texture-atlas memory (size=%dx%d "
              "max=%dx%d)",
              __FUNCTION__,
              curAtlasSize.x,
              curAtlasSize.y,
    ...
    unsigned char* bigtex1 =
      new unsigned char[curAtlasSize.x * curAtlasSize.y * 4];
    unsigned char* bigtex2 =
      new unsigned char[curAtlasSize.x * curAtlasSize.y * 4];
        ...
        for (int y = 0; y < curtex1.ysize; ++y) {
            for (int x = 0; x < curtex1.xsize; ++x) {
                for (int col = 0; col < 4; ++col) {
                    bigtex1[(((foundy + y) * curAtlasSize.x + (foundx + x)) *
                             4) +
                            col] =
                      curtex1.mem[(((y * curtex1.xsize) + x) * 4) + col];
                    bigtex2[(((foundy + y) * curAtlasSize.x + (foundx + x)) *
                             4) +
                            col] =
                      curtex2.mem[(((y * curtex1.xsize) + x) * 4) + col];
                }
            }
        }
...

    {
        ...
        glTexParameteri(GL_TEXTURE_2D,
                        ...
                        (maxMipMaps > 0) ? GL_LINEAR_MIPMAP_NEAREST
                                         : GL_LINEAR);
        ...
        if (maxMipMaps > 0) {
            glBuildMipmaps(GL_TEXTURE_2D,
                           ...
                           GL_RGBA,
                           GL_UNSIGNED_BYTE,
                           bigtex1); // FIXME disable texcompression
        } else {
    ...
    }
    {
        ...
        glTexParameteri(GL_TEXTURE_2D,
                        GL_TEXTURE_MIN_FILTER,
                        (maxMipMaps > 0) ? GL_NEAREST_MIPMAP_NEAREST
                                         : GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, maxMipMaps);
        if (maxMipMaps > 0) {
            glBuildMipmaps(GL_TEXTURE_2D,
                           ...
                           GL_RGBA,
                           GL_UNSIGNED_BYTE,
                           bigtex2); // FIXME disable texcompression
        } else {
            glTexImage2D(GL_TEXTURE_2D,
                         0,
                         GL_RGBA8,
    ...
    if (CTextureAtlas::GetDebug()) {
        ...
        tex1.Save(atlas.GetName() + "-1-" + IntToString(curAtlasSize.x) + "x" +
                  IntToString(curAtlasSize.y) + ".png");
        tex2.Save(atlas.GetName() + "-2-" + IntToString(curAtlasSize.x) + "x" +
                  IntToString(curAtlasSize.y) + ".png");
    }
...
C3DOTextureHandler::LoadTexFiles()
{
    ...
    const std::vector<std::string>& filesBMP =
      CFileHandler::FindFiles("unittextures/tatex/", "*.bmp");
    std::vector<std::string> files =
      CFileHandler::FindFiles("unittextures/tatex/", "*.tga");
                              ...
                              bool teamcolor)
{
    ...
    for (int a = 0; a < (tex1->ysize * tex1->xsize); ++a) {
        tex2->mem[a * 4 + 0] = 0;
        tex2->mem[a * 4 + 1] =
          tex1->mem[a * 4 + 3]; // move reflectivity to texture2
        tex2->mem[a * 4 + 2] = 0;
        tex2->mem[a * 4 + 3] = 255;
        ...
        if (teamcolor) {
            // purple = teamcolor
            if ((tex1->mem[a * 4] == tex1->mem[a * 4 + 2]) &&
                (tex1->mem[a * 4 + 1] == 0)) {
                ...
                tex1->mem[a * 4 + 2] = 0;
                tex1->mem[a * 4 + 3] =
                  (unsigned char)(std::min(255.0f, lum * 1.5f));
            }
        }
    }
