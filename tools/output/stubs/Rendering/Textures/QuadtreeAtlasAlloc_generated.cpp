...
QuadTreeNode*
CQuadtreeAtlasAlloc::FindPosInQuadTree(int xsize, int ysize)
{
...
int
CQuadtreeAtlasAlloc::CompareTex(SAtlasEntry* tex1, SAtlasEntry* tex2)
{
...
CQuadtreeAtlasAlloc::Allocate()
{
    ...
    for (std::map<std::string, SAtlasEntry>::iterator it = entries.begin();
         it != entries.end();
         ++it) {
        sortedEntries.push_back(&it->second);
    }
    sort(sortedEntries.begin(),
         sortedEntries.end(),
         CQuadtreeAtlasAlloc::CompareTex);

    for (std::vector<SAtlasEntry*>::iterator it = sortedEntries.begin();
         it != sortedEntries.end();
         ++it) {
        SAtlasEntry& entry = **it;
        QuadTreeNode* node = FindPosInQuadTree(entry.size.x, entry.size.y);

        if (!node) {
            for (std::map<std::string, SAtlasEntry>::iterator jt =
                   entries.begin();
                 jt != entries.end();
                 ++jt) {
                if (&entry == &(jt->second)) {
                    LOG_L(L_ERROR,
                          "CQuadtreeAtlasAlloc full: failed to add %s",
                          jt->first.c_str());
                    break;
                }
            }
        ...
        entry.texCoords.x2 = node->posx + entry.size.x -
                             1; // FIXME stretch if image is smaller than node!
...
int
CQuadtreeAtlasAlloc::GetMaxMipMaps()
{
    if (!root)
        return 0;
