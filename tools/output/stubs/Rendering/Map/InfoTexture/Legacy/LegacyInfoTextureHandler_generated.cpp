...
CONFIG(bool, HighResLos)
  .defaultValue(false)
  .description("Controls whether LOS (\"L view\") edges are rendered in high "
               "resolution. Resource heavy!");
CONFIG(int, ExtraTextureUpdateRate)
  .defaultValue(45)
  .description("EXTREME CPU-HEAVY ON MEDIUM/BIG MAPS! DON'T CHANGE DEFAULT!");

static CLegacyInfoTextureHandler::BaseGroundDrawMode
NameToDrawMode(const std::string& name)
{
    if (name == "los") {
...
static std::string
DrawModeToName(const CLegacyInfoTextureHandler::BaseGroundDrawMode mode)
{
    switch (mode) {
        case CLegacyInfoTextureHandler::drawNormal:
            return "";
        case CLegacyInfoTextureHandler::drawLos:
            return "los";
        case CLegacyInfoTextureHandler::drawMetal:
            return "metal";
        case CLegacyInfoTextureHandler::drawHeight:
            return "height";
  ...
  , highResInfoTexWanted(false)
{
    ...
    extraTextureUpdateRate =
      std::max(4, configHandler->GetInt("ExtraTextureUpdateRate") - 1);
    ...
    for (unsigned int i = 1; i < NUM_INFOTEXTURES; i++) {
        ...
        infoTextures.emplace_back(
          DrawModeToName(e), infoTextureIDs[i], texSize);
    }
...
bool
CLegacyInfoTextureHandler::IsEnabled() const
{
    return (drawMode != drawNormal);
}

void
CLegacyInfoTextureHandler::DisableCurrentMode()
{
...
void
CLegacyInfoTextureHandler::SetMode(const std::string& name)
{
...
void
CLegacyInfoTextureHandler::ToggleMode(const std::string& name)
{
...
const std::string&
CLegacyInfoTextureHandler::GetMode() const
{
    // return DrawModeToName(drawMode); (warning: returning reference to
    // temporary)
    return infoTextures[drawMode].GetName();
}

GLuint
CLegacyInfoTextureHandler::GetCurrentInfoTexture() const
{
    return infoTextureIDs[drawMode];
}

int2
CLegacyInfoTextureHandler::GetCurrentInfoTextureSize() const
{
...
const CInfoTexture*
CLegacyInfoTextureHandler::GetInfoTextureConst(const std::string& name) const
{
    return &infoTextures[NameToDrawMode(name)];
}

CInfoTexture*
CLegacyInfoTextureHandler::GetInfoTexture(const std::string& name)
{
    return &infoTextures[NameToDrawMode(name)];
}
...
static inline int
InterpolateLos(const unsigned short* p,
               int2 size,
               int mip,
               int factor,
               int2 pos)
...
void
CLegacyInfoTextureHandler::Update()
{
    UpdateExtraTexture(drawMode);
}

// Gradually calculate the extra texture based on updateTextureState:
//   updateTextureState < extraTextureUpdateRate:   Calculate the texture color
//   values and copy them into buffer updateTextureState =
//   extraTextureUpdateRate:   Copy the buffer into a texture
// NOTE:
//   when switching TO an info-texture drawing mode
//   the texture is calculated in its entirety, not
//   over multiple frames
bool
CLegacyInfoTextureHandler::UpdateExtraTexture(BaseGroundDrawMode texDrawMode)
{
    ...
    if (updateTextureState < extraTextureUpdateRate) {
        ...
        if (highResInfoTexWanted) {
            ...
            endy =
              (updateTextureState + 1) * mapDims.mapy / extraTextureUpdateRate;

            offset = starty * mapDims.pwr2mapx * 4;
            infoTexMem = reinterpret_cast<unsigned char*>(infoTexPBO.MapBuffer(
              offset, (endy - starty) * mapDims.pwr2mapx * 4));
        } else {
            starty =
              updateTextureState * mapDims.hmapy / extraTextureUpdateRate;
            endy =
              (updateTextureState + 1) * mapDims.hmapy / extraTextureUpdateRate;

            offset = starty * pwr2mapx_half * 4;
            infoTexMem = reinterpret_cast<unsigned char*>(infoTexPBO.MapBuffer(
              offset, (endy - starty) * pwr2mapx_half * 4));
        }
        ...
        switch (texDrawMode) {
            ...
            case drawPathFlow:
            case drawPathCost: {
                pathDrawer->UpdateExtraTexture(
                  texDrawMode, starty, endy, offset, infoTexMem);
            } break;

            case drawMetal: {
                const CMetalMap* metalMap = readMap->metalMap;

                const unsigned short* myLos =
                  &losHandler->los.losMaps[gu->myAllyTeam].front();
                const unsigned char* extraTex = metalMap->GetDistributionMap();
                const unsigned char* extraTexPal =
                  metalMap->GetTexturePalette();
                    ...
                    for (int x = 0; x < mapDims.hmapx; ++x) {
                        ...
                        if (myLos[alx + (aly * losHandler->los.size.x)]) {
                            infoTexMem[a + COLOR_R] = (unsigned char)std::min(
                              255.0f,
                              900.0f * fastmath::apxsqrt(fastmath::apxsqrt(
                                         extractDepthMap[y_hmapx + x])));
                        } else {
                            infoTexMem[a + COLOR_R] = 0;
                        }

                        infoTexMem[a + COLOR_G] =
                          (extraTexPal[extraTex[y_hmapx + x] * 3 + 1]);
                        infoTexMem[a + COLOR_B] =
                          (extraTexPal[extraTex[y_hmapx + x] * 3 + 2]);
            ...
            case drawHeight: {
                ...
                // NOTE:
                //   PBO/ExtraTexture resolution is always mapDims.pwr2mapx *
                //   mapDims.pwr2mapy (we don't use it fully) while the CORNER
                //   heightmap has dimensions (mapDims.mapx + 1) * (mapDims.mapy
                //   + 1). In case POT(mapDims.mapx) == mapDims.mapx we would
                //   therefore get a buffer overrun in our PBO when iterating
                //   over column (mapDims.mapx + 1) or row (mapDims.mapy + 1) so
                //   we select the easiest solution and just skip those squares.
            ...
            case drawLos: {
                const int jammerAllyTeam =
                  modInfo.separateJammers ? gu->myAllyTeam : 0;
                const unsigned short* myLos =
                  &losHandler->los.losMaps[gu->myAllyTeam].front();
                const unsigned short* myAirLos =
                  &losHandler->airLos.losMaps[gu->myAllyTeam].front();
                const unsigned short* myRadar =
                  &losHandler->radar.losMaps[gu->myAllyTeam].front();
                const unsigned short* myJammer =
                  &losHandler->jammer.losMaps[jammerAllyTeam].front();

                const int lowRes = highResInfoTexWanted ? 0 : -1;
                const int endx =
                  highResInfoTexWanted ? mapDims.mapx : mapDims.hmapx;
                ...
                for (int y = starty; y < endy; ++y) {
                    for (int x = 0; x < endx; ++x) {
                        ...
                        if (!losHandler->globalLOS[gu->myAllyTeam]) {
                            const int inLos =
                              InterpolateLos(myLos,
                                             losHandler->los.size,
                                             losMipLevel,
                                             128,
                                             pos);
                            const int inAir =
                              InterpolateLos(myAirLos,
                                             losHandler->airLos.size,
                                             airMipLevel,
                                             127,
                                             pos);
                        ...
                        const int inRadar =
                          InterpolateLos(myRadar,
                                         losHandler->radar.size,
                                         radarMipLevel,
                                         255,
                                         pos);
                        const int inJam =
                          (totalLos == 255)
                            ? InterpolateLos(myJammer,
                                             losHandler->jammer.size,
                                             jammerMipLevel,
                                             255,
                                             pos)
                            : 0;
        ...
        if (highResInfoTexWanted) {
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, starty, mapDims.pwr2mapx,
        (endy-starty), GL_BGRA, GL_UNSIGNED_BYTE, infoTexPBO.GetPtr(offset)); }
        else { glTexSubImage2D(GL_TEXTURE_2D, 0, 0, starty, mapDims.pwr2mapx>>1,
        (endy-starty), GL_BGRA, GL_UNSIGNED_BYTE, infoTexPBO.GetPtr(offset));
        }
        */
