...
GetShaderSource(const std::string& fileName)
{
    ...
    if (soFile.FileExists()) {
        ...
        LOG_L(
          L_ERROR, "[%s] file not found \"%s\"", __FUNCTION__, soPath.c_str());
    }
...
IShaderObject::GetHash() const
{
    ...
    hash =
      HsiehHash((const void*)curShaderSrc.data(), curShaderSrc.size(), hash);
...
{
    assert(
      globalRendering->haveARB); // non-debug check is done in ShaderHandler
...
{
    assert(
      globalRendering->haveGLSL); // non-debug check is done in ShaderHandler
}

GLSLShaderObject::CompiledShaderObjectUniquePtr
GLSLShaderObject::CompileShaderObject()
{
    CompiledShaderObjectUniquePtr res(new CompiledShaderObject(),
                                      [](CompiledShaderObject* so) {
    ...
    if (!res->valid) {
        const std::string& name =
          srcFile.find("void main()") ? "unknown" : srcFile;
        LOG_L(L_WARNING,
              "[GLSL-SO::%s] shader-object name: %s, compile-log:\n%s\n",
              __FUNCTION__,
              name.c_str(),
              res->log.c_str());
        LOG_L(L_WARNING,
...
IProgramObject::PrintDebugInfo()
{
    ...
    for (const auto& p : uniformStates) {
        ...
        if (p.second.IsUninit()) {
            LOG_L(L_DEBUG,
                  "\t%s: uninitialized used=%i",
                  (p.second.GetName()).c_str(),
                  int(curUsed));
        } else {
            LOG_L(L_DEBUG,
                  "\t%s: x=float:%f;int:%i y=%f z=%f used=%i",
                  (p.second.GetName()).c_str(),
                  p.second.GetFltValues()[0],
                  p.second.GetIntValues()[0],
                  p.second.GetFltValues()[1],
                  p.second.GetFltValues()[2],
                  int(curUsed));
        }
    }
#endif
...
ARBProgramObject::SetUniform1i(int idx, int v0)
{
    glProgramEnvParameter4fARB(
      uniformTarget, idx, float(v0), float(0), float(0), float(0));
}
void
ARBProgramObject::SetUniform2i(int idx, int v0, int v1)
{
    glProgramEnvParameter4fARB(
      uniformTarget, idx, float(v0), float(v1), float(0), float(0));
}
void
ARBProgramObject::SetUniform3i(int idx, int v0, int v1, int v2)
{
    glProgramEnvParameter4fARB(
      uniformTarget, idx, float(v0), float(v1), float(v2), float(0));
}
void
ARBProgramObject::SetUniform4i(int idx, int v0, int v1, int v2, int v3)
{
    glProgramEnvParameter4fARB(
      uniformTarget, idx, float(v0), float(v1), float(v2), float(v3));
}
void
...
GLSLProgramObject::Validate()
{
    ...
    for (int i = 0; i < numUniforms; ++i) {
        ...
        LOG_L(L_WARNING,
              "[GLSL-PO::%s] program-object name: %s, unset uniform: %s",
              __FUNCTION__,
              name.c_str(),
              &bufname[0]);
        // assert(false);
...
GLSLProgramObject::Reload(bool reloadFromDisk, bool validate)
{
    ...
    const bool useShaderCache =
      oldValid && configHandler->GetBool("UseShaderCache");
    bool deleteOldShader = true;
    objID = 0;
    if (useShaderCache) {
        CShaderHandler::ShaderCache& shadersCache =
          shaderHandler->GetShaderCache();
    ...
    if (objID == 0) {
        ...
        if (!IsValid()) {
            LOG_L(L_WARNING,
                  "[GLSL-PO::%s] program-object name: %s, link-log:\n%s\n",
                  __FUNCTION__,
                  name.c_str(),
                  log.c_str());
        }
...
GLSLProgramObject::SetUniform2v(UniformState* uState, const int* v)
{
    assert(IsBound());
    if (uState->Set2v(v))
        glUniform2iv(uState->GetLocation(), 1, v);
}
void
GLSLProgramObject::SetUniform2v(UniformState* uState, const float* v)
{
    assert(IsBound());
    if (uState->Set2v(v))
        glUniform2fv(uState->GetLocation(), 1, v);
}
void
GLSLProgramObject::SetUniform3v(UniformState* uState, const int* v)
{
    assert(IsBound());
    if (uState->Set3v(v))
        glUniform3iv(uState->GetLocation(), 1, v);
}
void
GLSLProgramObject::SetUniform3v(UniformState* uState, const float* v)
{
    assert(IsBound());
    if (uState->Set3v(v))
        glUniform3fv(uState->GetLocation(), 1, v);
}
void
GLSLProgramObject::SetUniform4v(UniformState* uState, const int* v)
{
    assert(IsBound());
    if (uState->Set4v(v))
        glUniform4iv(uState->GetLocation(), 1, v);
}
void
GLSLProgramObject::SetUniform4v(UniformState* uState, const float* v)
{
    assert(IsBound());
    if (uState->Set4v(v))
        glUniform4fv(uState->GetLocation(), 1, v);
}
