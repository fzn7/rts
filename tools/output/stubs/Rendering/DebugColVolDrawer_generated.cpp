...
DrawCollisionVolume(const CollisionVolume* vol)
{
    ...
    switch (vol->GetVolumeType()) {
        ...
        case CollisionVolume::COLVOL_TYPE_SPHERE: {
            // scaled sphere is special case of ellipsoid: radius, slices,
            // stacks
            glTranslatef(
              vol->GetOffset(0), vol->GetOffset(1), vol->GetOffset(2));
            ...
            switch (vol->GetPrimaryAxis()) {
                case CollisionVolume::COLVOL_AXIS_X: {
                    ...
                    glTranslatef(
                      vol->GetOffset(0), vol->GetOffset(1), vol->GetOffset(2));
                    glScalef(
                      vol->GetScale(0), vol->GetHScale(1), vol->GetHScale(2));
                ...
                case CollisionVolume::COLVOL_AXIS_Y: {
                    ...
                    glTranslatef(
                      vol->GetOffset(0), vol->GetOffset(1), vol->GetOffset(2));
                    glScalef(
                      vol->GetHScale(0), vol->GetScale(1), vol->GetHScale(2));
                ...
                case CollisionVolume::COLVOL_AXIS_Z: {
                    ...
                    glTranslatef(
                      vol->GetOffset(0), vol->GetOffset(1), vol->GetOffset(2));
                    glScalef(
                      vol->GetHScale(0), vol->GetHScale(1), vol->GetScale(2));
                } break;
            }
...
LocalModelPiece* lap, int lapf)
{
        glPushMatrix();
                ...
                if (lmp->scriptSetVisible &&
!lmp->GetCollisionVolume()->IgnoreHits()) { if ((lmp == lap) && (lapf > 0 &&
((gs->frameNum - lapf) < 150))) { glColor3f((1.0f - ((gs->frameNum - lapf) /
150.0f)), 0.0f, 0.0f);
                        }
                        ...
                        if ((lmp == lap) && (lapf > 0 && ((gs->frameNum - lapf)
< 150))) { glColorf3(DEFAULT_VOLUME_COLOR);
                        }
                }
        glPopMatrix();
...
DrawFeatureColVol(const CFeature* f)
{
    ...
    const bool vCustomType =
      (v->GetVolumeType() < CollisionVolume::COLVOL_TYPE_SPHERE);
    const bool vCustomDims =
      ((v->GetOffsets()).SqLength() >= 1.0f ||
       std::fabs(v->GetBoundingRadius() - f->radius) >= 1.0f);
...
DrawUnitColVol(const CUnit* u)
{
    ...
    const bool vCustomDims =
      ((v->GetOffsets()).SqLength() >= 1.0f ||
       std::fabs(v->GetBoundingRadius() - u->radius) >= 1.0f);
    ...
    if (v->DefaultToPieceTree()) {
        ...
        if (!v->IgnoreHits()) {
            ...
            if (u->lastAttackFrame > 0 &&
                ((gs->frameNum - u->lastAttackFrame) < 150)) {
                glColor3f(
                  (1.0f - ((gs->frameNum - u->lastAttackFrame) / 150.0f)),
                  0.0f,
                  0.0f);
            }
    ...
    if (u->shieldWeapon != nullptr) {
        const CPlasmaRepulser* shield =
          static_cast<const CPlasmaRepulser*>(u->shieldWeapon);
  ...
  public:
    ...
    void DrawQuad(int x, int y)
    {
        ...
        for (const CFeature* f : q.features) {
            if (alreadyDrawnIds.find(MAX_UNITS + f->id) ==
                alreadyDrawnIds.end()) {
...
Draw()
{
    ...
    readMap->GridVisibility(
      nullptr, &drawer, 1e9, CQuadField::BASE_QUAD_SIZE / SQUARE_SIZE);
