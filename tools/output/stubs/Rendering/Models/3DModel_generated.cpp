...
S3DModelPiece::CreateShatterPieces()
{
    ...
    vboShatterIndices.Resize(S3DModelPiecePart::SHATTER_VARIATIONS *
                             GetVertexIndices().size() * sizeof(unsigned int));
...
S3DModelPiece::CreateShatterPiecesVariation(const int num)
{
    ...
    auto* memVBO = reinterpret_cast<unsigned char*>(
      vboShatterIndices.MapBuffer(num * isize, isize, GL_WRITE_ONLY));
    size_t curVboPos = 0;
    for (auto& cp : shatterPartsBuf) {
        cp.indexCount = cp.indices.size();
        cp.vboOffset = num * isize + curVboPos;
        if (cp.indexCount > 0) {
            memcpy(memVBO + curVboPos,
                   &cp.indices[0],
                   cp.indexCount * sizeof(unsigned int));
                       ...
                       const CMatrix44f& m) const
{
    const float2 pieceParams = {
        float3::max(float3::fabs(maxs), float3::fabs(mins)).Length(),
        pieceChance
    };
    const int2 renderParams = { texType, team };

    projectileHandler->AddFlyingPiece(
      modelType, this, m, pos, speed, pieceParams, renderParams);
}
...
LocalModel::CreateLocalModelPieces(const S3DModelPiece* mpParent)
{
    ...
    assert(lmpParent->GetLModelPieceIndex() ==
           lmpParent->GetScriptPieceIndex());
...
{
    original->ComposeTransform(
      pieceSpaceMat.LoadIdentity(), pos, rot, original->scales);
}
