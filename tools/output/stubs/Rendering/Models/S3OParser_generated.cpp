...
CS3OParser::Load(const std::string& name)
{
    ...
    SS3OPiece* rootPiece =
      LoadPiece(model, NULL, &fileBuf[0], header.rootPiece);
    model->SetRootPiece(rootPiece);

    // set after the extrema are known
    model->radius = (header.radius <= 0.01f)
                      ? (model->maxs - model->mins).Length() * 0.5f
                      : header.radius;
    model->height = (header.height <= 0.01f) ? (model->maxs.y - model->mins.y)
                                             : header.height;
                      ...
                      int offset)
{
    ...
    fp->swap(); // Does it matter we mess with the original buffer here? Don't
                // hope so.
...
SS3OPiece::UploadGeometryVBOs()
{
    ...
    vboAttributes.New(
      vertices.size() * sizeof(SS3OVertex), GL_STATIC_DRAW, &vertices[0]);
...
SS3OPiece::BindVertexAttribVBOs() const
{
    ...
    glVertexPointer(3,
                    GL_FLOAT,
                    sizeof(SS3OVertex),
                    vboAttributes.GetPtr(offsetof(SS3OVertex, pos)));

    glEnableClientState(GL_NORMAL_ARRAY);
    glNormalPointer(GL_FLOAT,
                    sizeof(SS3OVertex),
                    vboAttributes.GetPtr(offsetof(SS3OVertex, normal)));
    ...
    glTexCoordPointer(2,
                      GL_FLOAT,
                      sizeof(SS3OVertex),
                      vboAttributes.GetPtr(offsetof(SS3OVertex, texCoords[0])));
    ...
    glTexCoordPointer(2,
                      GL_FLOAT,
                      sizeof(SS3OVertex),
                      vboAttributes.GetPtr(offsetof(SS3OVertex, texCoords[1])));
    ...
    glTexCoordPointer(3,
                      GL_FLOAT,
                      sizeof(SS3OVertex),
                      vboAttributes.GetPtr(offsetof(SS3OVertex, sTangent)));
    ...
    glTexCoordPointer(3,
                      GL_FLOAT,
                      sizeof(SS3OVertex),
                      vboAttributes.GetPtr(offsetof(SS3OVertex, tTangent)));
    vboAttributes.Unbind();
}
    ...
    switch (primType) {
        ...
        case S3O_PRIMTYPE_TRIANGLE_STRIP: {
            ...
            if (globalRendering->supportRestartPrimitive) {
                // this is not compiled into display lists, but executed
                // immediately
    ...
    switch (primType) {
        ...
        case S3O_PRIMTYPE_TRIANGLE_STRIP: {
            ...
            newIndices.resize(indices.size() *
                              3); // each index (can) create a new triangle
...
SS3OPiece::SetVertexTangents()
{
    ...
    // for v in [0, n - 2]
    const unsigned vrtMaxNr =
      (stride == 1) ? indices.size() - 2 : indices.size();
