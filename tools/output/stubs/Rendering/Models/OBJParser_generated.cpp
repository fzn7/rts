...
COBJParser::Load(const std::string& modelFileName)
{
    std::string metaFileName =
      modelFileName.substr(0, modelFileName.find_last_of('.')) + ".lua";

    CFileHandler modelFile(modelFileName);
    CFileHandler metaFile(metaFileName);

    if (!modelFile.FileExists())
        throw content_error("[OBJParser] could not find model-file \"" +
                            modelFileName + "\"");

    if (!metaFile.FileExists())
        throw content_error("[OBJParser] could not find meta-file \"" +
                            metaFileName + "\"");
    ...
    if (!metaFileParser.IsValid())
        throw content_error("[OBJParser] failed to parse meta-file \"" +
                            metaFileName + "\"");
    ...
    if (ParseModelData(model, modelData, modelTable)) {
        ...
        delete model;
        throw content_error("[OBJParser] failed to parse model-data \"" +
                            modelFileName + "\"");
    }
                           ...
                           const LuaTable& metaData)
{
    ...
    texcoors.reserve(2048);
    std::vector<float3> vnormals;
    vnormals.reserve(2048);
    ...
    while ((currReadIdx = modelData.find_first_of('\n', prevReadIdx)) !=
        ...
        if (!line.empty()) {
            ...
            if (!regexMatch) {
                regexMatch = (boost::regex_match(line, objectPattern));
            }
            if (!regexMatch) {
            ...
            if (!regexMatch) {
                regexMatch = (boost::regex_match(line, txcoorPattern));
            }
            if (!regexMatch) {
                regexMatch = (boost::regex_match(line, polygonPattern));
            }
            ...
            if (!regexMatch) {
                ...
                LOG_L(L_WARNING,
                      "Failed to parse line \"%s\" for model \"%s\"",
                      line.c_str(),
                      model->name.c_str());
            ...
            switch (lineHeader[0]) {
                ...
                case 'f': {
                    ...
                    SOBJTriangle triangle = { { 0, 0, 0 },
                                              { 0, 0, 0 },
                                              { 0, 0, 0 } };

                    while (lineStream.good() && n < 3) {
                        std::string vtnIndices;
                        lineStream >> vtnIndices;

                        // vIdx/tcIdx/nIdx triplet of indices into the global
                        // lists
                        i = vtnIndices.find('/', 0);
                        assert(i != std::string::npos);
                        j = vtnIndices.find('/', i + 1);
                        assert(j != std::string::npos);

                        vIdx = std::atoi(vtnIndices.substr(0, i).c_str());
                        tIdx =
                          std::atoi(vtnIndices.substr(i + 1, j - i).c_str());
                    ...
                    const bool b0 =
                      (triangle.vIndices[0] >= 0 && triangle.vIndices[1] >= 0 &&
                       triangle.vIndices[2] >= 0) &&
                      (triangle.tIndices[0] >= 0 && triangle.tIndices[1] >= 0 &&
                       triangle.tIndices[2] >= 0) &&
                      (triangle.nIndices[0] >= 0 && triangle.nIndices[1] >= 0 &&
                       triangle.nIndices[2] >= 0);
                    const bool b1 = (triangle.vIndices[0] < vertices.size() &&
                                     triangle.vIndices[1] < vertices.size() &&
                                     triangle.vIndices[2] < vertices.size()) &&
                                    (triangle.tIndices[0] < texcoors.size() &&
                                     triangle.tIndices[1] < texcoors.size() &&
                                     triangle.tIndices[2] < texcoors.size()) &&
                                    (triangle.nIndices[0] < vnormals.size() &&
                                     triangle.nIndices[1] < vnormals.size() &&
                                     triangle.nIndices[2] < vnormals.size());

                    assert(n == 3);
                    assert(b0 && b1);

                    if (b0 && b1) {
                        // note: this assumes face elements may not reference
                        // indices ahead of the current {vertices, texcoors,
                        // vnormals}.size() if this *is* allowed, the entire
                        // file must be parsed first
                        float3 &v0 = vertices[triangle.vIndices[0]],
                               &v1 = vertices[triangle.vIndices[1]],
                               &v2 = vertices[triangle.vIndices[2]];
                        float2 &t0 = texcoors[triangle.tIndices[0]],
                               &t1 = texcoors[triangle.tIndices[1]],
                               &t2 = texcoors[triangle.tIndices[2]];
                        float3 &n0 = vnormals[triangle.nIndices[0]],
                               &n1 = vnormals[triangle.nIndices[1]],
                               &n2 = vnormals[triangle.nIndices[2]];

                        const int idx0 = (piece->GetTriangleCount() * 3) + 0,
                                  idx1 = (piece->GetTriangleCount() * 3) + 1,
                                  idx2 = (piece->GetTriangleCount() * 3) + 2;

                        // convert to piece-local vtn indices
                        triangle.vIndices[0] = idx0;
                        piece->AddVertex(v0);
                        triangle.nIndices[0] = idx0;
                        piece->AddNormal(n0.ANormalize());
                        triangle.tIndices[0] = idx0;
                        piece->AddTxCoor(t0);

                        triangle.vIndices[1] = idx1;
                        piece->AddVertex(v1);
                        triangle.nIndices[1] = idx1;
                        piece->AddNormal(n1.ANormalize());
                        triangle.tIndices[1] = idx1;
                        piece->AddTxCoor(t1);

                        triangle.vIndices[2] = idx2;
                        piece->AddVertex(v2);
                        triangle.nIndices[2] = idx2;
                        piece->AddNormal(n2.ANormalize());
                        triangle.tIndices[2] = idx2;
                        piece->AddTxCoor(t2);

                        piece->AddTriangle(triangle);
                    } else {
                        LOG_L(L_WARNING,
                              "Illegal face-element indices on line \"%s\" for "
                              "model \"%s\"",
                              line.c_str(),
                              model->name.c_str());
                    }
    ...
    const bool globalVertexOffsets =
      metaData.GetBool("globalvertexoffsets", false);
    const bool localPieceOffsets = metaData.GetBool("localpieceoffsets", false);

    if (BuildModelPieceTree(
          model, pieceMap, piecesTable, globalVertexOffsets, localPieceOffsets))
        return true;

    for (PieceMap::iterator it = pieceMap.begin(); it != pieceMap.end(); ++it)
        delete (it->second);

    throw content_error("[OBJParser] model " + model->name +
                        " has no uniquely defined root-piece");
    return false;
}
                                ...
                                bool localPieceOffsets)
{
    ...
    if (!rootPieceNames.empty()) {
        ...
        const PieceMap::const_iterator rootPieceIt =
          pieceMap.find(rootPieceName);

        if (rootPieceIt != pieceMap.end()) {
            rootPiece = rootPieceIt->second;
            model->SetRootPiece(rootPiece);

            BuildModelPieceTreeRec(model,
                                   rootPiece,
                                   pieceMap,
                                   rootPieceTable,
                                   globalVertexOffsets,
    ...
    if (!rootPieceNumbers.empty()) {
        const LuaTable& rootPieceTable =
          piecesTable.SubTable(rootPieceNumbers[0]);
                                   ...
                                   bool localPieceOffsets)
{
    ...
    piece->goffset =
      (localPieceOffsets)
        ? (piece->offset +
           ((parentPiece != nullptr) ? parentPiece->goffset : ZeroVector))
        : (piece->offset);
    piece->offset =
      (localPieceOffsets)
        ? (piece->offset)
        : (piece->offset -
           ((parentPiece != nullptr) ? parentPiece->offset : ZeroVector));
    ...
    if (!childPieceNames.empty()) {
        for (std::vector<std::string>::const_iterator it =
               childPieceNames.begin();
             it != childPieceNames.end();
             ++it) {
            ...
            const LuaTable& childPieceTable =
              pieceTable.SubTable(childPieceName);

            PieceMap::const_iterator pieceIt = pieceMap.find(childPieceName);

            if (pieceIt == pieceMap.end()) {
                throw content_error("[OBJParser] meta-data piece named \"" +
                                    childPieceName + "\" not defined in model");
            } else {
    ...
    if (!childPieceNumbers.empty()) {
        for (std::vector<int>::const_iterator it = childPieceNumbers.begin();
             it != childPieceNumbers.end();
             ++it) {
            const LuaTable& childPieceTable = pieceTable.SubTable(*it);
            const std::string& childPieceName =
              childPieceTable.GetString("name", "");
...
SOBJPiece::UploadGeometryVBOs()
{
    ...
    vboPositions.New(
      vertices.size() * sizeof(float3), GL_STATIC_DRAW, &vertices[0]);
    vboPositions.Unbind();

    vboNormals.Bind(GL_ARRAY_BUFFER);
    vboNormals.New(
      vnormals.size() * sizeof(float3), GL_STATIC_DRAW, &vnormals[0]);
    vboNormals.Unbind();

    vboTexcoords.Bind(GL_ARRAY_BUFFER);
    vboTexcoords.New(
      texcoors.size() * sizeof(float2), GL_STATIC_DRAW, &texcoors[0]);
    vboTexcoords.Unbind();

    vbosTangents.Bind(GL_ARRAY_BUFFER);
    vbosTangents.New(
      sTangents.size() * sizeof(float3), GL_STATIC_DRAW, &sTangents[0]);
    vbosTangents.Unbind();

    vbotTangents.Bind(GL_ARRAY_BUFFER);
    vbotTangents.New(
      tTangents.size() * sizeof(float3), GL_STATIC_DRAW, &tTangents[0]);
    vbotTangents.Unbind();
    ...
    // FIXME:
    //   assumes vIndices, nIndices and tIndices are identical in layout for all
    //   vertices (not a big problem because OBJ models must have a normal and
    //   texcoord per vertex)
...
SOBJPiece::SetMinMaxExtends(bool globalVertexOffsets)
{
    ...
    for (int i = GetVertexCount() - 1; i >= 0; i--) {
        ...
        // all pieces
        if (overrideMins) {
            mins = float3::min(mins, vertexLocalPos);
        }
        if (overrideMaxs) {
            maxs = float3::max(maxs, vertexLocalPos);
        }
...
SOBJPiece::SetVertexTangents()
{
    ...
    for (int i = GetTriangleCount() - 1; i >= 0; i--) {
        ...
        const float3 &p0 = GetVertexPos(tri.vIndices[0]),
                     p1 = GetVertexPos(tri.vIndices[1]),
                     p2 = GetVertexPos(tri.vIndices[2]);
        const float2 &tc0 = GetTxCoor(tri.tIndices[0]),
        ...
        const float3 sdir((t2 * x1x0 - t1 * x2x0) * r,
                          (t2 * y1y0 - t1 * y2y0) * r,
                          (t2 * z1z0 - t1 * z2z0) * r);
        const float3 tdir((s1 * x2x0 - s2 * x1x0) * r,
                          (s1 * y2y0 - s2 * y1y0) * r,
                          (s1 * z2z0 - s2 * z1z0) * r);
