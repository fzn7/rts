...
static float3 aiQuaternionToRadianAngles(const aiQuaternion q1)
{
        ...
        if (test > (0.499f * unit)) {
                ...
                angles.x = math::atan2(2.0f * q1.y * q1.w - 2.0f * q1.x * q1.z,
sqx - sqy - sqz + sqw); angles.y = math::asin((2.0f * test) / unit); angles.z =
math::atan2(2.0f * q1.x * q1.w - 2.0f * q1.y * q1.z, -sqx + sqy - sqz + sqw);
        }
...
{
#ifndef BITMAP_NO_OPENGL
    // FIXME returns non-optimal data, at best compute it ourselves (pre-TL
    // cache size!)
    ...
    Assimp::DefaultLogger::get()->attachStream(new AssLogStream(),
                                               ASS_LOGGING_OPTIONS);
}
...
{
    LOG_SL(
      LOG_SECTION_MODEL, L_INFO, "Loading model: %s", modelFilePath.c_str());
    ...
    if (!CFileHandler::FileExists(metaFileName, SPRING_VFS_ZIP)) {
        LOG_SL(LOG_SECTION_MODEL,
               L_INFO,
               "No meta-file '%s'. Using defaults.",
               metaFileName.c_str());
    }
    ...
    if (!modelTable.IsValid()) {
        LOG_SL(LOG_SECTION_MODEL,
               L_INFO,
               "No valid model metadata in '%s' or no meta-file",
               metaFileName.c_str());
    }
...
#ifndef BITMAP_NO_OPENGL
    {
        ...
        importer.SetPropertyInteger(AI_CONFIG_PP_SLM_TRIANGLE_LIMIT,
                                    maxIndices / 3);
    }
#endif
    ...
    if (scene != nullptr) {
        LOG_SL(LOG_SECTION_MODEL,
               L_INFO,
               "Processing scene for model: %s (%d meshes / %d materials / %d "
               "textures)",
               modelFilePath.c_str(),
               scene->mNumMeshes,
               scene->mNumMaterials,
        ...
        throw content_error("[AssimpParser] Model Import: " +
    ...
    LOG_SL(LOG_SECTION_MODEL,
           L_INFO,
           "Loading textures. Tex1: '%s' Tex2: '%s'",
           model->texs[0].c_str(),
           model->texs[1].c_str());

    texturehandlerS3O->PreloadTexture(
      model,
      modelTable.GetBool("fliptextures", true),
      modelTable.GetBool("invertteamcolor", true));

    // Load all pieces in the model
    LOG_SL(LOG_SECTION_MODEL,
           L_INFO,
           "Loading pieces from root node '%s'",
    ...
    LOG_SL(
      LOG_SECTION_MODEL, L_DEBUG, "model->numobjects: %d", model->numPieces);
    ...
    LOG_SL(LOG_SECTION_MODEL,
           ...
           model->maxs[2]);
    LOG_SL(
      LOG_SECTION_MODEL, L_INFO, "Model %s Imported.", model->name.c_str());
    return model;
}
...
vertexIndex++) { const aiVector3D& aiVertex = mesh->mVertices[vertexIndex]; mins
= std::min(mins, aiVectorToFloat3(aiVertex)); maxs = std::max(maxs,
aiVectorToFloat3(aiVertex));
                }
                                     ...
                                     const LuaTable& pieceTable)
{
    ...
    if (piece->scales.x != piece->scales.y ||
        piece->scales.y != piece->scales.z) {
        // LOG_SL(LOG_SECTION_MODEL, L_WARNING, "Spring doesn't support
        // non-uniform scaling");
    ...
    piece->offset =
      pieceTable.GetFloat3("offset", aiVectorToFloat3(aiTransVec));
    ...
    //
    // float3 pieceRotAngles = pieceTable.GetFloat3("rotate",
    // aiQuaternionToRadianAngles(aiRotateQuat) * RADTODEG);
    ...
    LOG_SL(LOG_SECTION_PIECE,
           L_INFO,
           "(%d:%s) Assimp offset (%f,%f,%f), rotate (%f,%f,%f,%f), scale "
           "(%f,%f,%f)",
           model->numPieces,
           piece->name.c_str(),
           aiTransVec.x,
    ...
    CMatrix44f bakedMatrix =
      aiMatrixToMatrix(aiMatrix4x4t<float>(aiRotateQuat.GetMatrix()));
    ...
    if (piece == model->GetRootPiece()) {
        ...
        if (math::fabs(xaxis.SqLength() - yaxis.SqLength()) < 0.01f &&
            math::fabs(yaxis.SqLength() - zaxis.SqLength()) < 0.01f) {
            bakedMatrix = CMatrix44f(ZeroVector, xaxis, yaxis, zaxis);
        }
    }

    piece->rotAxisSigns =
      pieceTable.GetFloat3("rotAxisSigns", float3(-OnesVector));
    piece->axisMapType =
      AxisMappingType(pieceTable.GetInt("rotAxisMap", AXIS_MAPPING_XYZ));
                         ...
                         ModelPieceMap& pieceMap)
{
    ...
    // find a new name if none given or if a piece with the same name already
    // exists
                              ...
                              const aiScene* scene)
{
    ...
    for (unsigned meshListIndex = 0; meshListIndex < pieceNode->mNumMeshes;
         ++meshListIndex) {
        const unsigned int meshIndex = pieceNode->mMeshes[meshListIndex];
        const aiMesh* mesh = scene->mMeshes[meshIndex];

        LOG_SL(
          LOG_SECTION_PIECE, L_DEBUG, "Fetching mesh %d from scene", meshIndex);
        LOG_SL(LOG_SECTION_PIECE,
               L_DEBUG,
        ...
        for (unsigned vertexIndex = 0; vertexIndex < mesh->mNumVertices;
            ...
            if (mesh->HasTangentsAndBitangents()) {
                ...
                vertex.sTangent =
                  (aiVectorToFloat3(aiTangent)).SafeANormalize();
                vertex.tTangent =
                  (aiVectorToFloat3(aiBitangent)).SafeANormalize();
            }

            // vertex tex-coords per channel
            for (unsigned int uvChanIndex = 0; uvChanIndex < NUM_MODEL_UVCHANNS;
                 uvChanIndex++) {
                ...
                vertex.texCoords[uvChanIndex].x =
                  mesh->mTextureCoords[uvChanIndex][vertexIndex].x;
                vertex.texCoords[uvChanIndex].y =
                  mesh->mTextureCoords[uvChanIndex][vertexIndex].y;
            }
        ...
        for (unsigned faceIndex = 0; faceIndex < mesh->mNumFaces; ++faceIndex) {
            ...
            for (unsigned vertexListID = 0; vertexListID < face.mNumIndices;
                 ++vertexListID) {
                const unsigned int vertexFaceIdx = face.mIndices[vertexListID];
                const unsigned int vertexDrawIdx =
                  mesh_vertex_mapping[vertexFaceIdx];
                      ...
                      ParentNameMap& parentMap)
{
    ...
    LOG_SL(LOG_SECTION_PIECE,
           L_INFO,
           "Converting node '%s' to piece '%s' (%d meshes).",
           pieceNode->mName.data,
           piece->name.c_str(),
           pieceNode->mNumMeshes);

    // Load additional piece properties from metadata
    const LuaTable& pieceTable =
      modelTable.SubTable("pieces").SubTable(piece->name);

    if (pieceTable.IsValid()) {
        LOG_SL(LOG_SECTION_PIECE,
               L_INFO,
               "Found metadata for piece '%s'",
               piece->name.c_str());
    }
...

    {
        ...
        const std::string& parentName = (parentNameIt != parentMap.end())
                                          ? (parentNameIt->second).c_str()
                                          : "[null]";

        // Verbose logging of piece properties
        LOG_SL(LOG_SECTION_PIECE,
               L_INFO,
               "Loaded model piece: %s with %d meshes",
               piece->name.c_str(),
               pieceNode->mNumMeshes);
        LOG_SL(
          LOG_SECTION_PIECE, L_INFO, "piece->name: %s", piece->name.c_str());
        LOG_SL(
          LOG_SECTION_PIECE, L_INFO, "piece->parent: %s", parentName.c_str());
    }

    // Recursively process all child pieces
    for (unsigned int i = 0; i < pieceNode->mNumChildren; ++i) {
        LoadPiece(model,
                  pieceNode->mChildren[i],
                  scene,
                  modelTable,
                  pieceMap,
                  parentMap);
...
CAssParser::CalculateModelDimensions(S3DModel* model, S3DModelPiece* piece)
{
    ...
    // cannot set this until parent relations are known, so either here or in
    // BuildPieceHierarchy()
    piece->goffset =
      scaleRotMat.Mul(piece->offset) +
      ((piece->parent != nullptr) ? piece->parent->goffset : ZeroVector);
    ...
    piece->SetCollisionVolume(CollisionVolume(
      "box", piece->maxs - piece->mins, (piece->maxs + piece->mins) * 0.5f));
                                     ...
                                     const LuaTable& modelTable)
{
    ...
    model->radius = modelTable.GetFloat(
      "radius", (model->maxs - model->mins).Length() * 0.5f);
    model->height =
      modelTable.GetFloat("height", (model->maxs.y - model->mins.y));
            ...
            const std::string& fallback)
{
    ...
    if (CFileHandler::FileExists("unittextures/" + testTextureFile,
                                 SPRING_VFS_ZIP_FIRST))
        return "unittextures/" + testTextureFile;

    if (CFileHandler::FileExists(modelPath + testTextureFile,
                                 SPRING_VFS_ZIP_FIRST))
...
FindTextureByRegex(const std::string& regex_path, const std::string& regex)
{
    ...
    const std::vector<std::string>& files =
      CFileHandler::FindFiles(regex_path, regex + ".*");
                         ...
                         const std::string& modelName)
{
    ...
    if (model->texs[1].empty())
        model->texs[1] =
          FindTextureByRegex(modelPath, "glow"); // lowest-priority name
    ...
    if (scene->mNumMaterials > 0) {
        constexpr unsigned int texTypes[] = {
            ...
            /*
            // TODO: support these too (we need to allow constructing tex1 &
            tex2 from several sources) aiTextureType_EMISSIVE,
        ...
        for (unsigned int texType : texTypes) {
            ...
            if (scene->mMaterials[0]->Get(AI_MATKEY_TEXTURE(texType, 0),
                                          textureFile) != aiReturn_SUCCESS)
                continue;

            assert(textureFile.length > 0);
            model->texs[0] =
              FindTexture(textureFile.data, modelPath, model->texs[0]);
        }
    }

    // 3. try to load from metafile (highest priority)
    model->texs[0] =
      FindTexture(modelTable.GetString("tex1", ""), modelPath, model->texs[0]);
    model->texs[1] =
      FindTexture(modelTable.GetString("tex2", ""), modelPath, model->texs[1]);
}
...
SAssPiece::UploadGeometryVBOs()
{
    ...
    vboAttributes.New(
      vertices.size() * sizeof(SAssVertex), GL_STATIC_DRAW, &vertices[0]);
    vboAttributes.Unbind();

    vboIndices.Bind(GL_ELEMENT_ARRAY_BUFFER);
    vboIndices.New(
      indices.size() * sizeof(unsigned int), GL_STATIC_DRAW, &indices[0]);
    vboIndices.Unbind();
...
SAssPiece::BindVertexAttribVBOs() const
{
    ...
    glVertexPointer(3,
                    GL_FLOAT,
                    sizeof(SAssVertex),
                    vboAttributes.GetPtr(offsetof(SAssVertex, pos)));

    glEnableClientState(GL_NORMAL_ARRAY);
    glNormalPointer(GL_FLOAT,
                    sizeof(SAssVertex),
                    vboAttributes.GetPtr(offsetof(SAssVertex, normal)));
    ...
    for (unsigned int n = 0; n < NUM_MODEL_TEXTURES; n++) {
        ...
        glTexCoordPointer(2,
                          GL_FLOAT,
                          sizeof(SAssVertex),
                          vboAttributes.GetPtr(offsetof(SAssVertex, texCoords) +
                                               (0 * sizeof(float2))));
    }
    ...
    glTexCoordPointer(3,
                      GL_FLOAT,
                      sizeof(SAssVertex),
                      vboAttributes.GetPtr(offsetof(SAssVertex, sTangent)));
    ...
    glTexCoordPointer(3,
                      GL_FLOAT,
                      sizeof(SAssVertex),
                      vboAttributes.GetPtr(offsetof(SAssVertex, tTangent)));
    vboAttributes.Unbind();
}
