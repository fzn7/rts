...
C3DOParser::Load(const std::string& name)
{
    ...
    S3DOPiece* rootPiece =
      LoadPiece(model, 0, NULL, &model->numPieces, fileBuf);
...
C3DOParser::IsBasePlate(S3DOPiece* obj, S3DOPrimitive* face)
{
    ...
    const float3 s1 =
      obj->vertexPos[face->indices[0]] - obj->vertexPos[face->indices[1]];
    const float3 s2 =
      obj->vertexPos[face->indices[1]] - obj->vertexPos[face->indices[2]];
                       ...
                       const std::vector<unsigned char>& fileBuf) const
{
    ...
    LOG_L(L_WARNING,
          "[%s] unknown 3DO texture \"%s\" for piece \"%s\"",
          __FUNCTION__,
          texName.c_str(),
          obj->name.c_str());
                          ...
                          const std::vector<unsigned char>& fileBuf)
{
    ...
    for (int a = 0; a < num; a++) {
        ...
        const float3 v0v1 =
          (obj->vertexPos[sp.indices[1]] - obj->vertexPos[sp.indices[0]]);
        const float3 v0v2 =
          (obj->vertexPos[sp.indices[2]] - obj->vertexPos[sp.indices[0]]);
        ...
        const int vertHash = HsiehHash(
          &orderVert[0], orderVert.size() * sizeof(orderVert[0]), 0x123456);
                      ...
                      const std::vector<unsigned char>& fileBuf)
{
    ...
    piece->goffset =
      piece->offset + ((parent != NULL) ? parent->goffset : ZeroVector);

    GetVertexes(&me, piece, fileBuf);
    GetPrimitives(piece,
                  me.OffsetToPrimitiveArray,
                  me.NumberOfPrimitives,
                  ((pos == 0) ? me.SelectionPrimitive : -1),
                  fileBuf);
    piece->CalcNormals();
    ...
    if (me.OffsetToChildObject > 0) {
        piece->children.push_back(
          LoadPiece(model, me.OffsetToChildObject, piece, numobj, fileBuf));
    }

    if (me.OffsetToSiblingObject > 0) {
        parent->children.push_back(
          LoadPiece(model, me.OffsetToSiblingObject, parent, numobj, fileBuf));
    }
...
S3DOPiece::UploadGeometryVBOs()
{
    ...
    for (const S3DOPrimitive& ps : prims) {
        ...
        if (ps.indices.size() == 4) {
            ...
            vertices.emplace_back(vertexPos[ps.indices[0]],
                                  ps.vnormals[0],
                                  float2(tex->xstart, tex->ystart));
            vertices.emplace_back(vertexPos[ps.indices[1]],
                                  ps.vnormals[1],
                                  float2(tex->xend, tex->ystart));
            vertices.emplace_back(vertexPos[ps.indices[2]],
                                  ps.vnormals[2],
                                  float2(tex->xend, tex->yend));
            vertices.emplace_back(vertexPos[ps.indices[3]],
                                  ps.vnormals[3],
                                  float2(tex->xstart, tex->yend));
            ...
            for (int i = 0; i < ps.indices.size(); ++i) {
                vertices.emplace_back(vertexPos[ps.indices[i]],
                                      ps.vnormals[i],
                                      float2(tex->xstart, tex->ystart));
            }
        }
    ...
    vboAttributes.New(
      vertices.size() * sizeof(S3DOVertex), GL_STATIC_DRAW, &vertices[0]);
    vboAttributes.Unbind();

    vboIndices.Bind(GL_ELEMENT_ARRAY_BUFFER);
    vboIndices.New(
      indices.size() * sizeof(unsigned), GL_STATIC_DRAW, &indices[0]);
    vboIndices.Unbind();
...
S3DOPiece::BindVertexAttribVBOs() const
{
    ...
    glVertexPointer(3,
                    GL_FLOAT,
                    sizeof(S3DOVertex),
                    vboAttributes.GetPtr(offsetof(S3DOVertex, pos)));

    glEnableClientState(GL_NORMAL_ARRAY);
    glNormalPointer(GL_FLOAT,
                    sizeof(S3DOVertex),
                    vboAttributes.GetPtr(offsetof(S3DOVertex, normal)));
    ...
    glTexCoordPointer(2,
                      GL_FLOAT,
                      sizeof(S3DOVertex),
                      vboAttributes.GetPtr(offsetof(S3DOVertex, texCoord)));
...
S3DOPiece::DrawForList() const
{
    ...
    glDrawRangeElements(GL_TRIANGLES,
                        0,
                        (vboAttributes.GetSize() - 1) / sizeof(S3DOVertex),
                        vboIndices.GetSize() / sizeof(unsigned),
                        GL_UNSIGNED_INT,
                        vboIndices.GetPtr());
    vboIndices.Unbind();
