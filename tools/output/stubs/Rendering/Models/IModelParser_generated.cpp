...
RegisterAssimpModelFormats(CModelLoader::FormatMap& formats)
{
    ...
    // get a ";" separated list of format extensions
    // ("*.3ds;*.lwo;*.mesh.xml;...")
...
CreateDummyModel()
{
    ...
    model->GetRootPiece()->SetCollisionVolume(
      CollisionVolume("box", -UpVector, ZeroVector));
    return model;
}
    ...
    if (!name.empty()) {
        ...
        if (fileExt.empty()) {
            for (auto it = formats.cbegin(); it != formats.cend(); ++it) {
                ...
                if (CFileHandler::FileExists(name + "." + formatExt,
                                             SPRING_VFS_ZIP)) {
                    name += ("." + formatExt);
                    break;
                }
...
CModelLoader::PreloadModel(const std::string& modelName)
{
    ...
    ThreadPool::enqueue(
      [modelName]() { modelLoader.LoadModel(modelName, true); });
}
...
CModelLoader::ParseModel(const std::string& name, const std::string& path)
{
    ...
    if (parser != nullptr) {
        try {
            ...
            LOG_L(L_WARNING,
                  "could not load model \"%s\" (reason: %s)",
                  name.c_str(),
                  ex.what());
        }
    } else {
        LOG_L(L_ERROR,
              "could not find a parser for model \"%s\" (unknown format?)",
              name.c_str());
    }
