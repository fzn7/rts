...
CStandardGroundFlash::InitCommon(const float3& _pos, const float3& _color)
{
    ...
    const float3 normal =
      CalcNormal(_pos, camera->GetDir() * -1000.0f, flashSize);
...
{
    float iAlpha = Clamp(
      circleAlpha - (circleAlphaDec * globalRendering->timeOffset), 0.0f, 1.0f);
    ...
    if (iAlpha > 0.0f) {
        ...
        va->AddVertexQTC(p1,
                         projectileDrawer->groundringtex->xstart,
                         projectileDrawer->groundringtex->ystart,
                         color);
        va->AddVertexQTC(p2,
                         projectileDrawer->groundringtex->xend,
                         projectileDrawer->groundringtex->ystart,
                         color);
        va->AddVertexQTC(p3,
                         projectileDrawer->groundringtex->xend,
                         projectileDrawer->groundringtex->yend,
                         color);
        va->AddVertexQTC(p4,
                         projectileDrawer->groundringtex->xstart,
                         projectileDrawer->groundringtex->yend,
    ...
    if (iAge < 1.0f) {
        ...
        va->AddVertexQTC(p1,
                         projectileDrawer->groundflashtex->xstart,
                         projectileDrawer->groundflashtex->yend,
                         color);
        va->AddVertexQTC(p2,
                         projectileDrawer->groundflashtex->xend,
                         projectileDrawer->groundflashtex->yend,
                         color);
        va->AddVertexQTC(p3,
                         projectileDrawer->groundflashtex->xend,
                         projectileDrawer->groundflashtex->ystart,
                         color);
        va->AddVertexQTC(p4,
                         projectileDrawer->groundflashtex->xstart,
                         projectileDrawer->groundflashtex->ystart,
...
CSimpleGroundFlash::Init(const CUnit* owner, const float3& offset)
{
    ...
    const float3 normal =
      CalcNormal(pos, camera->GetDir() * -1000.0f, size + (sizeGrowth * ttl));
...
CSimpleGroundFlash::GetMemberInfo(SExpGenSpawnableMemberInfo& memberInfo)
{
    ...
    CHECK_MEMBER_INFO_PTR(
      CSimpleGroundFlash, colorMap, CColorMap::LoadFromDefString)
    CHECK_MEMBER_INFO_PTR(CSimpleGroundFlash,
                          texture,
                          projectileDrawer->groundFXAtlas->GetTexturePtr)
  ...
  , ttl(_ttl)
{
    ...
    const float3 normal =
      CalcNormal(_pos, camera->GetDir() * -1000.0f, size + (sizeGrowth * ttl));
