...
GetTrackableUnit(const CUnit* caiOwner, const CUnit* cmdUnit)
{
    if (cmdUnit == nullptr)
        return nullptr;
    if ((cmdUnit->losStatus[caiOwner->allyteam] & (LOS_INLOS | LOS_INRADAR)) ==
        0)
        return nullptr;
...
CommandDrawer::Draw(const CCommandAI* cai) const
{
    ...
    if ((dynamic_cast<const CAirCAI*>(cai)) != nullptr) {
        DrawAirCAICommands(static_cast<const CAirCAI*>(cai));
        return;
    }
    if ((dynamic_cast<const CBuilderCAI*>(cai)) != nullptr) {
        DrawBuilderCAICommands(static_cast<const CBuilderCAI*>(cai));
        return;
    }
    if ((dynamic_cast<const CFactoryCAI*>(cai)) != nullptr) {
        DrawFactoryCAICommands(static_cast<const CFactoryCAI*>(cai));
        return;
    }
    if ((dynamic_cast<const CMobileCAI*>(cai)) != nullptr) {
        DrawMobileCAICommands(static_cast<const CMobileCAI*>(cai));
        return;
    }
...
CommandDrawer::DrawLuaQueuedUnitSetCommands() const
{
    ...
    for (auto ui = luaQueuedUnitSet.cbegin(); ui != luaQueuedUnitSet.cend();
         ++ui) {
...
CommandDrawer::DrawCommands(const CCommandAI* cai) const
{
    ...
    for (auto ci = commandQue.begin(); ci != commandQue.end(); ++ci) {
        ...
        switch (cmdID) {
            ...
            case CMD_MANUALFIRE: {
                if (ci->params.size() == 1) {
                    const CUnit* unit = GetTrackableUnit(
                      owner, unitHandler->GetUnit(ci->params[0]));

                    if (unit != nullptr) {
                        lineDrawer.DrawLineAndIcon(
                          cmdID,
                          unit->GetObjDrawErrorPos(owner->allyteam),
                          cmdColors.attack);
                    }
                } else {
                    ...
                    lineDrawer.DrawLineAndIcon(
                      cmdID, float3(x, y, z), cmdColors.attack);
                }
                break;
            }
            case CMD_WAIT: {
...
CommandDrawer::DrawAirCAICommands(const CAirCAI* cai) const
{
    ...
    for (auto ci = commandQue.begin(); ci != commandQue.end(); ++ci) {
        ...
        switch (cmdID) {
            case CMD_MOVE: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0), cmdColors.move);
                break;
            }
            case CMD_FIGHT: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0), cmdColors.fight);
                break;
            }
            case CMD_PATROL: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0), cmdColors.patrol);
                break;
            }
            case CMD_ATTACK: {
                if (ci->params.size() == 1) {
            ...
            case CMD_GUARD: {
                const CUnit* unit =
                  GetTrackableUnit(owner, unitHandler->GetUnit(ci->params[0]));
    ...
    for (auto ci = commandQue.begin(); ci != commandQue.end(); ++ci) {
        ...
        if (cmdID < 0) {
            if (cai->buildOptions.find(cmdID) != cai->buildOptions.end()) {
                ...
                cursorIcons.AddBuildIcon(
                  cmdID, bi.pos, owner->team, bi.buildFacing);
        ...
        switch (cmdID) {
            ...
            case CMD_RECLAIM:
            case CMD_RESURRECT: {
                const float* color = (cmdID == CMD_RECLAIM)
                                       ? cmdColors.reclaim
                                       : cmdColors.resurrect;
                if (ci->params.size() == 4) {
                    const float3& endPos = ci->GetPos(0);
                    ...
                    if (id >= unitHandler->MaxUnits()) {
                        const CFeature* feature = featureHandler->GetFeature(
                          id - unitHandler->MaxUnits());

                        if (feature != nullptr) {
                            lineDrawer.DrawLineAndIcon(
                              cmdID, feature->GetObjDrawMidPos(), color);
                        }
                    } else {
                        const CUnit* unit = GetTrackableUnit(
                          owner, unitHandler->GetUnit(ci->params[0]));
            ...
            case CMD_REPAIR:
            case CMD_CAPTURE: {
                const float* color = (ci->GetID() == CMD_REPAIR)
                                       ? cmdColors.repair
                                       : cmdColors.capture;
                if (ci->params.size() == 4) {
                    const float3& endPos = ci->GetPos(0);
...
CommandDrawer::DrawFactoryCAICommands(const CFactoryCAI* cai) const
{
    ...
    for (auto ci = newUnitCommands.begin(); ci != newUnitCommands.end(); ++ci) {
        ...
        switch (cmdID) {
            case CMD_MOVE: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0) + UpVector * 3.0f, cmdColors.move);
                break;
            }
            case CMD_FIGHT: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0) + UpVector * 3.0f, cmdColors.fight);
                break;
            }
            case CMD_PATROL: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0) + UpVector * 3.0f, cmdColors.patrol);
                break;
            }
            case CMD_ATTACK: {
                if (ci->params.size() == 1) {
    ...
    for (auto ci = commandQue.begin(); ci != commandQue.end(); ++ci) {
        ...
        switch (cmdID) {
            ...
            case CMD_UNLOAD_UNIT: {
                lineDrawer.DrawLineAndIcon(
                  cmdID, ci->GetPos(0), cmdColors.unload);
                break;
            }
            case CMD_WAIT: {
                DrawWaitIcon(*ci);
...
CommandDrawer::DrawDefaultCommand(const Command& c, const CUnit* owner) const
{
    ...
    if (paramsCount >= 1) {
        ...
        const CUnit* unit =
          GetTrackableUnit(owner, unitHandler->GetUnit(c.params[0]));
...
CommandDrawer::DrawQuedBuildingSquares(const CBuilderCAI* cai) const
{
    ...
    // worst case - 2 squares per building (when underwater) - 8 vertices * 3
    // floats
    ...
    for (const Command& c : commandQue) {
        ...
        for (int i = 0; i < 4; ++i) {
            std::copy(
              col, col + 8, lineColors.begin() + linecounter * 2 + i * 8);
        }
