                  ...
                  float slope)
{
    ...
    for_mt(0, resolution, [&](const int i) {
        ...
        float adjRadius =
          weapon ? weapon->GetRange2D(heightDiff * heightMod) : rad;
        float adjustment = rad * 0.5f;
        float ydiff = 0;
        for (int j = 0;
             j < rdiv && std::fabs(adjRadius - rad) + ydiff > .01 * rad;
             j++) {
            if (adjRadius > rad) {
                rad += adjustment;
            } else {
...
glWireCylinder(unsigned int* listID, unsigned int numDivs, float zSize)
{
    ...
    for (unsigned int n = 0; n <= numDivs; n++) {
        ...
        glVertex3f(vertices[i + numDivs].x,
                   vertices[i + numDivs].y,
                   vertices[i + numDivs].z);
    }
    glEnd();
    ...
    for (unsigned int n = 0; n < numDivs; n++) {
        ...
        glVertex3f(vertices[n + numDivs].x,
                   vertices[n + numDivs].y,
                   vertices[n + numDivs].z);
    }

    glVertex3f(vertices[0].x, vertices[0].y, vertices[0].z);
    glVertex3f(vertices[0 + numDivs].x,
               vertices[0 + numDivs].y,
               vertices[0 + numDivs].z);
    glEnd();
