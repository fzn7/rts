                           ...
                           const GLvoid* userParam)
{
    if (id == 131185) {
        ...
        // example: "Buffer detailed info: Buffer object 260 (bound to
        // GL_PIXEL_UNPACK_BUFFER_ARB, usage hint is GL_STREAM_DRAW) has been
        // mapped in DMA CACHED memory."
        return;
    }
    ...
    LOG_L(L_ERROR,
          "OpenGL: source<%s> type<%s> id<%u> severity<%s>:\n%s",
          sourceStr.c_str(),
          typeStr.c_str(),
          id,
          severityStr.c_str(),
    ...
    if (configHandler->GetBool("DebugGLStacktraces")) {
        CrashHandler::Stacktrace(
          Threading::GetCurrentThread(), "rendering", LOG_LEVEL_WARNING);
    }
}
...
{
#if defined(HEADLESS) ||                                                       \
  !defined(GL_GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX) ||                   \
  !defined(GL_TEXTURE_FREE_MEMORY_ATI)
    return false;
#else
    ...
    if (!configHandler->GetBool("DisableCrappyGPUWarning")) {
        if (gfxCardIsCrap) {
            const std::string msg =
              ...
              "If the game crashes, looks ugly or runs slow, buy a better "
              "card!\n"
              "You may try \"spring --safemode\" to test if some of your "
              "issues are related to wrong settings.\n"
              "\nHint: You can disable this MessageBox by appending "
              "\"DisableCrappyGPUWarning = 1\" to \"" +
              configHandler->GetConfigFile() + "\".";
            LOG_L(L_WARNING, "%s", msg.c_str());
            Platform::MsgBox(
              msg, "Warning: Your GPU is not supported", MBF_EXCL);
        } else if (globalRendering->haveMesa) {
            const std::string mesa_msg =
              "Warning!\n"
              "OpenSource graphics card drivers detected.\n"
              "MesaGL/Gallium drivers don't work well with Spring. Try to "
            ...
            Platform::MsgBox(
              mesa_msg, "Warning: Your GPU driver is not supported", MBF_EXCL);
        } else if (msDrivers) {
            const std::string mesa_msg =
              "Warning!\n"
              "No OpenGL drivers installed.\n"
              "Please go to your GPU vendor's website and download their "
              "drivers:\n"
            ...
            LOG_L(L_WARNING, "%s", mesa_msg.c_str());
            Platform::MsgBox(
              mesa_msg, "Warning: No OpenGL drivers found", MBF_EXCL);
        }
    }
}
...
LoadExtensions()
{
    ...
    LOG("SDL version:  linked %d.%d.%d; compiled %d.%d.%d",
        sdlVersionLinked.major,
        sdlVersionLinked.minor,
        sdlVersionLinked.patch,
        sdlVersionCompiled.major,
        sdlVersionCompiled.minor,
        sdlVersionCompiled.patch);
    ...
    if (GLEW_ARB_debug_output && configHandler->GetBool("DebugGL")) {
        ...
        // typecast is a workarround for #4510, signature of the callback
        // message changed :-|
        glDebugMessageCallbackARB((GLDEBUGPROCARB)&OpenGLDebugMessageCallback,
                                  NULL);
...
glSaveTexture(const GLuint textureID, const std::string& filename)
{
    ...
    int bits = 0;
    {
        ...
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_INTERNAL_FORMAT, (GLint*)&format);

        GLint _cbits;
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_RED_SIZE, &_cbits);
        bits += _cbits;
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_GREEN_SIZE, &_cbits);
        bits += _cbits;
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_BLUE_SIZE, &_cbits);
        bits += _cbits;
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_ALPHA_SIZE, &_cbits);
        bits += _cbits;
        glGetTexLevelParameteriv(
          GL_TEXTURE_2D, 0, GL_TEXTURE_DEPTH_SIZE, &_cbits);
        bits += _cbits;
    }
    assert(bits == 32);
...
#endif
    {
        ...
        switch (internalFormat) {
            ...
            case GL_RGB8:
                ...
                break;
            default
              : /*LOG_L(L_ERROR, "[%s] Couldn't detect format type for %i",
                   __FUNCTION__, internalFormat);*/
                break;
        }
        glTexImage2D(
          target, 0, internalFormat, width, height, 0, format, type, nullptr);
    }
}
...
CheckParseErrors(GLenum target, const char* filename, const char* program)
{
    ...
    if (isNative != 1) {
        ...
        glGetProgramivARB(
          GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_ALU_INSTRUCTIONS_ARB, &aluInstrs);
        glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB,
                          GL_MAX_PROGRAM_ALU_INSTRUCTIONS_ARB,
                          &maxAluInstrs);
        glGetProgramivARB(
          GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_TEX_INSTRUCTIONS_ARB, &texInstrs);
        glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB,
                          GL_MAX_PROGRAM_TEX_INSTRUCTIONS_ARB,
                          &maxTexInstrs);
        glGetProgramivARB(
          GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_TEX_INDIRECTIONS_ARB, &texIndirs);
        glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB,
        ...
        if (aluInstrs > maxAluInstrs) {
            LOG_L(L_ERROR,
                  "[%s] too many ALU instructions in %s (%d, max. %d)\n",
                  __FUNCTION__,
                  filename,
                  aluInstrs,
                  maxAluInstrs);
        ...
        if (texInstrs > maxTexInstrs) {
            LOG_L(L_ERROR,
                  "[%s] too many texture instructions in %s (%d, max. %d)\n",
                  __FUNCTION__,
                  filename,
                  texInstrs,
                  maxTexInstrs);
        ...
        if (texIndirs > maxTexIndirs) {
            LOG_L(L_ERROR,
                  "[%s] too many texture indirections in %s (%d, max. %d)\n",
                  __FUNCTION__,
                  filename,
                  texIndirs,
                  maxTexIndirs);
        ...
        if (nativeTexIndirs > maxNativeTexIndirs) {
            LOG_L(
              L_ERROR,
              "[%s] too many native texture indirections in %s (%d, max. %d)\n",
              __FUNCTION__,
              filename,
              nativeTexIndirs,
              maxNativeTexIndirs);
        ...
        if (nativeAluInstrs > maxNativeAluInstrs) {
            LOG_L(L_ERROR,
                  "[%s] too many native ALU instructions in %s (%d, max. %d)\n",
                  __FUNCTION__,
                  filename,
                  nativeAluInstrs,
                  maxNativeAluInstrs);
...
LoadProgram(GLenum target, const char* filename, const char* program_type)
{
    ...
    if (!file.FileExists()) {
        ...
        SNPRINTF(c,
                 512,
                 "[myGL::LoadProgram] Cannot find %s-program file '%s'",
                 program_type,
                 filename);
          ...
          const float& offsetZ)
{
    ...
    // BUG: Nvidia drivers take the current texcoord into account when TexGen is
    // used!
