...
CGuiHandler::LoadConfig(const std::string& cfg)
{
    ...
    if (deadIconSlot >= 0) {
        ...
        xBpos =
          buttonBox.x1 + (fullBorder + (0.5 * xIconSize) + (fx * xIconStep));
        yBpos =
          buttonBox.y2 - (fullBorder + (0.5 * yIconSize) + (fy * yIconStep));
        ...
        if ((delta == 1) || (delta == xIcons)) {
            ...
            xBpos = buttonBox.x1 +
                    (fullBorder + (0.5 * xIconSize) + (fx * xIconStep));
            yBpos = buttonBox.y2 -
                    (fullBorder + (0.5 * yIconSize) + (fy * yIconStep));
        } else {
...
CGuiHandler::LayoutIcons(bool useSelectionPage)
{
    ...
    {
        defCmd = (mouse->buttons[SDL_BUTTON_RIGHT].pressed &&
        ...
        validInCommand =
          (activeCmd >= 0) && ((size_t)activeCmd < commands.size());
        if (validInCommand) {
    ...
    const int pageCount =
      ((cmdCount + (cmdIconsPerPage - 1)) / cmdIconsPerPage);
...
CGuiHandler::LayoutCustomIcons(bool useSelectionPage)
{
    ...
    if (!luaUI->LayoutButtons(tmpXicons,
                              tmpYicons,
                              cmds,
                              removeCmds,
                              customCmds,
    ...
    if ((tmpXicons < 2) || (tmpYicons < 2)) {
        LOG_L(L_ERROR,
              "LayoutCustomIcons() bad xIcons or yIcons (%i, %i)",
              tmpXicons,
              tmpYicons);
        return false;
    }
    ...
    for (unsigned int i = 0; i < removeCmds.size(); i++) {
        ...
        if ((index >= 0) || ((size_t)index < cmds.size())) {
            ...
            LOG_L(L_ERROR,
                  "LayoutCustomIcons() skipping bad removeCmd (%i)",
                  index);
        }
    }
    ...
    for (unsigned int i = 0; i < onlyTextureCmds.size(); i++) {
        ...
        if ((index >= 0) && (index < cmdCount)) {
            ...
            LOG_L(L_ERROR,
                  "LayoutCustomIcons() skipping bad onlyTexture (%i)",
                  index);
        }
    }
    ...
    for (unsigned int i = 0; i < reTextureCmds.size(); i++) {
        ...
        if ((index >= 0) && (index < cmdCount)) {
            ...
            LOG_L(L_ERROR,
                  "LayoutCustomIcons() skipping bad reTexture (%i)",
                  index);
        }
    }
    ...
    for (unsigned int i = 0; i < reNamedCmds.size(); i++) {
        ...
        if ((index >= 0) && (index < cmdCount)) {
            ...
            LOG_L(
              L_ERROR, "LayoutCustomIcons() skipping bad reNamed (%i)", index);
        }
    }
    ...
    for (unsigned int i = 0; i < reParamsCmds.size(); i++) {
        ...
        if ((index >= 0) && (index < cmdCount)) {
            ...
            LOG_L(
              L_ERROR, "LayoutCustomIcons() skipping bad reParams (%i)", index);
        }
    }
    ...
    const int pageCount =
      ((iconListCount + (tmpIconsPerPage - 1)) / tmpIconsPerPage);
...
CGuiHandler::GiveCommandsNow()
{
    ...
    for (std::vector<std::pair<Command, bool>>::iterator i =
           commandsToGiveTemp.begin();
         i != commandsToGiveTemp.end();
         ++i) {
...
CGuiHandler::Update()
{
    ...
    {
        if (!invertQueueKey &&
            (needShift && !KeyInput::GetKeyModState(KMOD_SHIFT))) {
            SetShowingMetal(false);
            inCommand = -1;
            needShift = false;
        }
    ...
    if ((inCommand >= 0) && ((size_t)inCommand < commands.size())) {
        ...
        if (useMinimap && (cmdDesc.id < 0)) {
            ...
            if (!CGameHelper::TestUnitBuildSquare(
                  bi, feature, gu->myAllyTeam, false)) {
                newCursor = "BuildBad";
            } else {
                newCursor = "BuildGood";
            }
...
CGuiHandler::TryTarget(const SCommandDescription& cmdDesc) const
{
    ...
    const float dist = TraceRay::GuiTraceRay(camera->GetPos(),
                                             mouse->dir,
                                             viewRange,
                                             NULL,
                                             targetUnit,
    ...
    for (const CUnit* u : selectedUnitsHandler.selectedUnits) {
        ...
        for (const CWeapon* w : u->weapons) {
            ...
            if (u->immobile) {
                ...
                // only check if we got a weapon that can shot the target (i.e.
                // anti-air/anti-sub)
...
{
    {
        if (button != SDL_BUTTON_LEFT && button != SDL_BUTTON_RIGHT &&
            button != -SDL_BUTTON_RIGHT && button != -SDL_BUTTON_LEFT)
            return false;
        ...
        if (inCommand >= 0) {
            if (invertQueueKey && (button == SDL_BUTTON_RIGHT) &&
                !mouse->buttons[SDL_BUTTON_LEFT]
                   .pressed) { // for rocker gestures
                SetShowingMetal(false);
                inCommand = -1;
                needShift = false;
                return false;
                          ...
                          const float3& mouseDir)
{
        ...
        if (!game->hideInterface) {
            ...
            if (iconPos >= 0) {
                ...
                if (iconCmd != lastIconCmd)
                    iconCmd = -1; // mouse was pressed on one button and
                                  // released on another one --> ignore the
                                  // command
            }
        }
    }
    ...
    if (c.GetID() ==
        CMD_FAILED) { // indicates we should not finish the current command
...
CGuiHandler::IconAtPos(int x, int y) // GetToolTip --> IconAtPos
{
    ...
    if ((fx < buttonBox.x1) || (fx > buttonBox.x2) || (fy < buttonBox.y1) ||
        (fy > buttonBox.y2)) {
        return -1;
    }
...
static bool
ParseCustomCmdMods(std::string& cmd, ModGroup& in, ModGroup& out)
{
        ...
        if (ParseCustomCmdMods(copy, inMods, outMods)) {
            if (CheckCustomCmdMods(rightMouseButton, inMods)) {
                ...
                if (outMods.alt != DontCare) {
                    KeyInput::SetKeyModState(KMOD_ALT,
                                             int(outMods.alt == Required));
                }
                if (outMods.ctrl != DontCare) {
                    KeyInput::SetKeyModState(KMOD_CTRL,
                                             int(outMods.ctrl == Required));
                }
                if (outMods.meta != DontCare) {
                    KeyInput::SetKeyModState(KMOD_GUI,
                                             int(outMods.meta == Required));
                }
                if (outMods.shift != DontCare) {
                    KeyInput::SetKeyModState(KMOD_SHIFT,
                                             int(outMods.shift == Required));
                }
...
CGuiHandler::CreateOptions(bool rightMouseButton)
{
    ...
    if (GetQueueKeystate()) {
        ...
        if (!invertQueueKey || (!mouse->buttons[SDL_BUTTON_LEFT].pressed &&
...

    {
        ...
        if ((ir == minimap) && (minimap->FullProxy())) {
            ...
            const float dist = TraceRay::GuiTraceRay(
              cameraPos, mouseDir, viewRange, NULL, unit, feature, true);
...
CGuiHandler::ProcessBuildActions(const Action& action)
{
    ...
    if (action.command == "buildspacing") {
        ...
        static const char* buildFaceDirs[] = {
                              ...
                              int actionIndex)
{
    ...
    for (size_t a = 0; a < commands.size(); ++a) {
        ...
        if ((static_cast<int>(a) != iconCmd) &&
            (cmdDesc.action != action.command)) {
        ...
        if (!cmdDesc.hidden &&
            (((cmdType == CMDTYPE_ICON) &&
              ((cmdDesc.id < 0) || (cmdDesc.id == CMD_STOCKPILE))) ||
        ...
        switch (cmdType) {
            ...
            case CMDTYPE_ICON_MODE: {
                ...
                if ((newMode < 0) ||
                    ((size_t)newMode > (cmdDesc.params.size() - 2))) {
                    newMode = 0;
                }
...
CGuiHandler::FinishCommand(int button)
{
    if ((button == SDL_BUTTON_LEFT) &&
        (KeyInput::GetKeyModState(KMOD_SHIFT) || invertQueueKey)) {
        needShift = true;
    } else {
        SetShowingMetal(false);
        inCommand = -1;
...
CheckCommand(Command c)
{
    if (selectedUnitsHandler.selectedUnits.empty() || (c.options & SHIFT_KEY))
        return c; // always allow queued commands, since conditions may change
                  // so the command becomes valid
    for (CUnitSet::iterator ui = selectedUnitsHandler.selectedUnits.begin();
         ui != selectedUnitsHandler.selectedUnits.end();
         ++ui) {
                        ...
                        const float3& mouseDir)
{
    ...
    if (button == SDL_BUTTON_RIGHT && preview) {
        ...
        if (mouse->buttons[SDL_BUTTON_RIGHT].pressed) {
            ...
            tempInCommand =
              GetDefaultCommand(mouseX, mouseY, cameraPos, mouseDir);
        }
    }
    ...
    switch (commands[tempInCommand].type) {
        ...
        case CMDTYPE_ICON: {
            ...
            if (button == SDL_BUTTON_LEFT && !preview) {
                LOG_L(L_WARNING,
                      "CMDTYPE_ICON left button press in incommand test? This "
                      "should not happen.");
            }
            return CheckCommand(c);
        }
        ...
        case CMDTYPE_ICON_MAP: {
            const float dist = CGround::LineGroundCol(
              cameraPos,
              cameraPos + (mouseDir * globalRendering->viewRange * 1.4f),
              false);
        ...
        case CMDTYPE_ICON_BUILDING: {
            ...
            const UnitDef* unitdef =
              unitDefHandler->GetUnitDefByID(-commands[inCommand].id);
            ...
            if (GetQueueKeystate() && (button == SDL_BUTTON_LEFT)) {
                const float dist2 = CGround::LineGroundCol(
                  ...
                  false);
                const float3 pos2 = mouse->buttons[SDL_BUTTON_LEFT].camPos +
                                    mouse->buttons[SDL_BUTTON_LEFT].dir * dist2;
            ...
            if (buildPos.size() == 1) {
                ...
                if (!CGameHelper::TestUnitBuildSquare(
                      buildPos[0], feature, gu->myAllyTeam, false)) {
                    return defaultRet;
                }
            }

            if (!preview) {
                for (auto bpi = buildPos.cbegin(); bpi != --buildPos.cend();
                     ++bpi) {
        ...
        case CMDTYPE_ICON_UNIT: {
            ...
            TraceRay::GuiTraceRay(cameraPos,
                                  mouseDir,
                                  globalRendering->viewRange * 1.4f,
                                  NULL,
                                  unit,
        ...
        case CMDTYPE_ICON_FRONT: {
            ...
            float3 pos = mouse->buttons[button].camPos +
                         (mouse->buttons[button].dir * dist);

            Command c(commands[tempInCommand].id, CreateOptions(button), pos);

            if (mouse->buttons[button].movement >
                30) { // only create the front if the mouse has moved enough
                const float dist2 = CGround::LineGroundCol(
                  cameraPos,
                  cameraPos + mouseDir * globalRendering->viewRange * 1.4f,
                  false);
                ...
                if (!commands[tempInCommand].params.empty() &&
                    pos.SqDistance2D(pos2) >
                      Square(atof(commands[tempInCommand].params[0].c_str()))) {
                    const float3 dif = (pos2 - pos).ANormalize();
                    pos2 =
                      pos +
                      dif * atoi(commands[tempInCommand].params[0].c_str());
                }
        ...
        case CMDTYPE_ICON_AREA: {
            ...
            if (mouse->buttons[button].movement < 4) {
                ...
                if (dist2 > (globalRendering->viewRange * 1.4f - 300) &&
                    (commands[tempInCommand].type !=
                     CMDTYPE_ICON_UNIT_FEATURE_OR_AREA))
                    return defaultRet;

                if (feature &&
                    commands[tempInCommand].type ==
                      CMDTYPE_ICON_UNIT_FEATURE_OR_AREA) { // clicked on feature
                    c.PushParam(unitHandler->MaxUnits() + feature->id);
                } else if (unit && commands[tempInCommand].type !=
                                     CMDTYPE_ICON_AREA) { // clicked on unit
                    if (c.GetID() == CMD_RESURRECT)
                        return defaultRet; // cannot resurrect units!
                    c.PushParam(unit->id);
                } else { // clicked in map
                    if (explicitCommand < 0 ||
                        !ZeroRadiusAllowed(c)) // only attack ground if
                ...
                const float dist =
                  CGround::LineGroundCol(mouse->buttons[button].camPos,
                                         mouse->buttons[button].camPos +
                                           mouse->buttons[button].dir *
                                             globalRendering->viewRange * 1.4f,
                                         false);
                ...
                const float3 pos = mouse->buttons[button].camPos +
                                   mouse->buttons[button].dir * dist;
        ...
        case CMDTYPE_ICON_UNIT_OR_RECTANGLE: {
            ...
            if (mouse->buttons[button].movement < 16) {
                ...
                if (unit != nullptr) {
                    ...
                    // clicked in map; only attack ground if explicitly set the
                    // command
                ...
                const float3 startPos = mouse->buttons[button].camPos +
                                        mouse->buttons[button].dir * dist;
...
{
    for (int i = 0; i < n; ++i) {
        BuildInfo bi(startInfo.def,
                     float3(x, 0.0f, z),
                     (startInfo.buildFacing + facing) % NUM_FACINGS);
        bi.pos = CGameHelper::Pos2BuildPos(bi, false);
                         ...
                         const float3& mouseDir)
{
    ...
    if (other.def && GetQueueKeystate() &&
        ...
        const float xsize =
          SQUARE_SIZE * (startInfo.GetXSize() + buildSpacing * 2);
        const int xnum = (int)((math::fabs(delta.x) + xsize * 1.4f) / xsize);
        float xstep = (int)((0 < delta.x) ? xsize : -xsize);

        const float zsize =
          SQUARE_SIZE * (startInfo.GetZSize() + buildSpacing * 2);
        ...
        if (KeyInput::GetKeyModState(KMOD_ALT)) {      // build a rectangle
            if (KeyInput::GetKeyModState(KMOD_CTRL)) { // hollow rectangle
                if ((1 < xnum) && (1 < znum)) {
                    ...
                    FillRowOfBuildPos(startInfo,
                                      start.x + (xnum - 1) * xstep,
                                      start.z + (znum - 2) * zstep,
                                      0,
                                      -zstep,
                                      znum - 1,
                                      0,
                ...
                for (float z = start.z; zn < znum; ++zn) {
                    if (zn & 1) {
                        ...
                        FillRowOfBuildPos(
                          startInfo, start.x, z, xstep, 0, xnum, 0, false, ret);
                    }
                    z += zstep;
                }
            }
            ...
            if (xDominatesZ) {
                zstep = KeyInput::GetKeyModState(KMOD_CTRL)
                          ? 0
                          : xstep * delta.z / (delta.x ? delta.x : 1);
            } else {
                xstep = KeyInput::GetKeyModState(KMOD_CTRL)
                          ? 0
                          : zstep * delta.x / (delta.z ? delta.z : 1);
            }
            FillRowOfBuildPos(startInfo,
                              start.x,
                              start.z,
                      ...
                      bool offsetForLEDs)
{
    ...
    const float tWidth = std::max(0.01f,
                                  font->GetSize() * font->GetTextWidth(text) *
                                    globalRendering->pixelX); // FIXME
    const float tHeight =
      std::max(0.01f,
               font->GetSize() * font->GetTextHeight(text) *
                 globalRendering->pixelY); // FIXME merge in 1 function?
    ...
    font->glPrint(xCenter,
                  yCenter,
                  fontScale,
                  (dropShadows ? FONT_SHADOW : 0) | FONT_CENTER | FONT_VCENTER |
...
CGuiHandler::DrawNWtext(const IconInfo& icon, const std::string& text)
{
    ...
    const Box& b = icon.visual;
    const float tHeight =
      font->GetSize() * font->GetTextHeight(text) * globalRendering->pixelY;
    ...
    font->glPrint(
      xPos, yPos, fontScale, FONT_TOP | FONT_SCALE | FONT_NORM, text);
}
...
CGuiHandler::DrawNEtext(const IconInfo& icon, const std::string& text)
{
    ...
    font->glPrint(xPos,
                  yPos,
                  fontScale,
                  FONT_TOP | FONT_RIGHT | FONT_SCALE | FONT_NORM,
                  text);
}
...
CGuiHandler::DrawSEtext(const IconInfo& icon, const std::string& text)
{
    ...
    font->glPrint(
      xPos, yPos, fontScale, FONT_RIGHT | FONT_SCALE | FONT_NORM, text);
}
...
CGuiHandler::DrawButtons() // Only called by Draw
{
    ...
    for (int ii = buttonStart; ii < buttonEnd; ii++) {
        ...
        const SCommandDescription& cmdDesc = commands[icon.commandsID];
        const bool customCommand =
          (cmdDesc.id == CMD_INTERNAL) && (cmdDesc.type == CMDTYPE_CUSTOM);
        const bool highlight =
          ((mouseIcon == ii) || (icon.commandsID == inCommand)) &&
        ...
        if (customCommand) {
            ...
            const bool useLEDs =
              useOptionLEDs && (cmdDesc.type == CMDTYPE_ICON_MODE);
            ...
            if (!usedTexture) {
                if (cmdDesc.id < 0) {
                    const UnitDef* ud =
                      unitDefHandler->GetUnitDefByID(-cmdDesc.id);
                    if (ud != NULL) {
            ...
            if (!usedTexture || !onlyTexture) {
                ...
                if (cmdDesc.type == CMDTYPE_ICON_MODE &&
                    ...
                    if (opt < 0 ||
                        static_cast<size_t>(opt) < cmdDesc.params.size()) {
    ...
    if (luaUI == NULL) {
        ...
        font->glFormat(xBpos,
                       yBpos,
                       textSize,
                       FONT_CENTER | FONT_VCENTER | FONT_SCALE | FONT_NORM,
                       "%i",
                       activePage + 1);
    }
...
CGuiHandler::DrawMenuName() // Only called by drawbuttons
{
    if (!menuName.empty() && (iconsCount > 0)) {
        ...
        if (!outlineFonts) {
            const float textHeight = fontScale * font->GetTextHeight(menuName) *
                                     globalRendering->pixelY;
            glDisable(GL_TEXTURE_2D);
            ...
            font->glPrint(xp,
                          yp,
                          fontScale,
                          FONT_CENTER | FONT_SCALE | FONT_NORM,
                          menuName);
        } else {
            ...
            font->glPrint(xp,
                          yp,
                          fontScale,
                          FONT_CENTER | FONT_OUTLINE | FONT_SCALE | FONT_NORM,
                          menuName);
        }
    }
}
...
CGuiHandler::DrawSelectionInfo()
{
    if (!selectedUnitsHandler.selectedUnits.empty()) {
        ...
        if (selectedUnitsHandler.GetSelectedGroup() != -1) {
            buf << "Selected units "
                << selectedUnitsHandler.selectedUnits.size() << " [Group "
                << selectedUnitsHandler.GetSelectedGroup() << "]";
        } else {
            buf << "Selected units "
                << selectedUnitsHandler.selectedUnits.size();
        }
        ...
        if (!outlineFonts) {
            float descender;
            const float textWidth = fontSize *
                                    smallFont->GetTextWidth(buf.str()) *
                                    globalRendering->pixelX;
            float textHeight = fontSize *
                               smallFont->GetTextHeight(buf.str(), &descender) *
                               globalRendering->pixelY;
            const float textDescender =
              fontSize * descender *
              globalRendering->pixelY; //! descender is always negative
            ...
            smallFont->glPrint(xSelectionPos,
                               ySelectionPos - textDescender,
                               fontSize,
                               FONT_BASELINE | FONT_NORM,
                               buf.str());
        } else {
...
CGuiHandler::DrawNumberInput() // Only called by drawbuttons
{
    ...
    if ((inCommand >= 0) && ((size_t)inCommand < commands.size())) {
        ...
        if (cd.type == CMDTYPE_NUMBER) {
            ...
            glColor4f(1.0f, 1.0f, 1.0f, 0.8f);
            const float mouseX =
              (float)mouse->lastx / (float)globalRendering->viewSizeX;
            const float slideX = std::min(std::max(mouseX, 0.25f), 0.75f);
            // const float mouseY = 1.0f - (float)(mouse->lasty - 16) /
            // (float)globalRendering->viewSizeY;
...
DrawUnitDefRanges(const CUnit* unit, const UnitDef* unitdef, const float3 pos)
{
    ...
    if (unitdef->onoffable || unitdef->activateWhenBuilt) {
        if (unit != nullptr &&
            unitdef == unit->unitDef) { // test if it's a decoy
...
CGuiHandler::DrawMapStuff(bool onMinimap)
{
    ...
    if (minimapCoords) {
        ...
        mouseDir = -UpVector;
        if (miniMapMarker && minimap->FullProxy() && !onMinimap &&
            !minimap->GetMinimized()) {
    ...
    if (activeMousePress) {
        ...
        if (mouse->buttons[button].pressed && (cmdIndex >= 0) &&
            ((size_t)cmdIndex < commands.size())) {
            ...
            switch (cmdDesc.type) {
                ...
                case CMDTYPE_ICON_AREA: {
                    ...
                    if (mouse->buttons[button].movement > 4) {
                        ...
                        const float dist2 = CGround::LineGroundCol(
                          cameraPos,
                          cameraPos +
                            mouseDir * globalRendering->viewRange * 1.4f,
                          false);
                        ...
                        const float radius =
                          std::min(maxRadius, pos.distance2D(pos2));
                ...
                case CMDTYPE_ICON_UNIT_OR_RECTANGLE: {
                    if (mouse->buttons[button].movement >= 16) {
                        ...
                        const float3 pos1 = mouse->buttons[button].camPos +
                                            mouse->buttons[button].dir * dist1;
                        const float3 pos2 = cameraPos + mouseDir * dist2;
    ...
    if (GetQueueKeystate()) {
        ...
        if (unit && ((unit->losStatus[gu->myAllyTeam] & LOS_INLOS) ||
            ...
            if (unitdef->maxWeaponRange > 0) {
                ...
                glBallisticCircle(
                  unit->pos, unitdef->maxWeaponRange, unit->weapons[0], 40);
                glEnable(GL_DEPTH_TEST);
            }
            // draw decloak distance
            if (unit->decloakDistance > 0.0f) {
                glColor4fv(cmdColors.rangeDecloak);
                if (unit->unitDef->decloakSpherical &&
                    globalRendering->drawdebug) {
                    glPushMatrix();
                    glTranslatef(
                      unit->midPos.x, unit->midPos.y, unit->midPos.z);
            ...
            if (unitdef->maxCoverage > 0.0f) {
                const CWeapon* w =
                  NULL; // will be checked if any missiles are ready
                if (!enemyUnit) {
                    w = unit->stockpileWeapon;
                    if (w != NULL && !w->weaponDef->interceptor) {
                        w = NULL; // if this isn't the interceptor, then don't
                                  // use it
                    }
                } // shows as on if enemy, a non-stockpiled weapon, or if the
                  // stockpile has a missile
    ...
    if ((inCommand >= 0) && ((size_t)inCommand < commands.size()) &&
        ...
        { // limit the locking scope to avoid deadlock
            // draw build distance for all immobile builders during build
            // commands
            for (const auto bi : unitHandler->GetBuilderCAIs()) {
                ...
                if (!builderDef->canmove ||
                    selectedUnitsHandler.IsUnitSelected(builder)) {
                    const float radius = builderDef->buildDistance;
                    if (radius > 0.0f) {
                        glDisable(GL_TEXTURE_2D);
                        const float* color = cmdColors.rangeBuild;
                        glColor4f(
                          color[0], color[1], color[2], color[3] * 0.333f);
        ...
        if (dist > 0.0f) {
            ...
            if (unitdef != nullptr) {
                const CMouseHandler::ButtonPressEvt& bp =
                  mouse->buttons[SDL_BUTTON_LEFT];
                const float bpDist = CGround::LineGroundCol(
                  bp.camPos,
                  bp.camPos + bp.dir * globalRendering->viewRange * 1.4f,
                  false);
                ...
                if (GetQueueKeystate() && bp.pressed) {
                    const BuildInfo cInfo =
                      BuildInfo(unitdef, cPos, buildFacing);
                    const BuildInfo bInfo =
                      BuildInfo(unitdef, bPos, buildFacing);

                    buildInfos =
                      std::move(GetBuildPos(bInfo, cInfo, cameraPos, mouseDir));
                } else {
                    const BuildInfo bi(unitdef, cPos, buildFacing);

                    buildInfos =
                      std::move(GetBuildPos(bi, bi, cameraPos, mouseDir));
                }

                for (auto bpi = buildInfos.cbegin(); bpi != buildInfos.cend();
                     ++bpi) {
                    ...
                    if (!unitdef->weapons.empty()) {
                        glDisable(GL_DEPTH_TEST);
                        glColor4fv(cmdColors.rangeAttack);
                        glBallisticCircle(buildPos,
                                          unitdef->weapons[0].def->range,
                                          NULL,
                                          40,
                                          unitdef->weapons[0].def->heightmod);
                        glEnable(GL_DEPTH_TEST);
                    }
                    ...
                    if (GetQueueKeystate()) {
                        ...
                        for (const CUnit* su :
                             selectedUnitsHandler.selectedUnits) {
                            for (Command& cmd :
                                 su->commandAI->GetOverlapQueued(c)) {
                                cv.push_back(cmd);
                            }
                        }
                    }
    ...
    int defcmd =
      GetDefaultCommand(mouse->lastx, mouse->lasty, cameraPos, mouseDir);
    if ((inCommand >= 0 && (size_t)inCommand < commands.size() &&
         commands[inCommand].id == CMD_ATTACK) ||
        (inCommand == -1 && defcmd > 0 && commands[defcmd].id == CMD_ATTACK)) {
        for (CUnitSet::iterator si = selectedUnitsHandler.selectedUnits.begin();
             si != selectedUnitsHandler.selectedUnits.end();
             ++si) {
            CUnit* unit = *si;
            if (unit == pointedAt) {
                continue;
            ...
            if (unit->maxRange > 0 &&
                ...
                glColor4fv(cmdColors.rangeAttack);
                glBallisticCircle(
                  unit->pos, unit->maxRange, unit->weapons.front(), 40);
                glEnable(GL_DEPTH_TEST);
...
CGuiHandler::DrawMiniMapMarker(const float3& cameraPos)
{
    ...
    const float groundLevel =
      CGround::GetHeightAboveWater(cameraPos.x, cameraPos.z, false);

    static float spinTime = 0.0f;
    spinTime =
      math::fmod(spinTime + globalRendering->lastFrameTime * 0.001f, 60.0f);
...
CGuiHandler::DrawCentroidCursor()
{
    ...
    if (winPos.z <= 1.0f) {
        ...
        if (mc != NULL) {
            glDisable(GL_DEPTH_TEST);
            mc->Draw(
              (int)winPos.x, globalRendering->viewSizeY - (int)winPos.y, 1.0f);
            glEnable(GL_DEPTH_TEST);
        }
    }
}
...
CGuiHandler::DrawArea(float3 pos, float radius, const float* color)
{
    ...
    glVertexf3(pos);
    for (int a = 0; a <= 40; ++a) {
        float3 p(fastmath::cos(a * 2 * PI / 40) * radius,
                 0,
                 fastmath::sin(a * 2 * PI / 40) * radius);
        p += pos;
                       ...
                       const float3& mouseDir)
{
    ...
    float dist = CGround::LineGroundCol(
      bp.camPos, bp.camPos + bp.dir * globalRendering->viewRange * 1.4f, false);

    if (dist < 0.0f)
        return;

    dist = CGround::LineGroundCol(
      cameraPos,
      cameraPos + mouseDir * globalRendering->viewRange * 1.4f,
      false);
...
DrawCornerPosts(const float3& pos0, const float3& pos1)
{
    ...
    const float3 corner0(
      pos0.x, CGround::GetHeightAboveWater(pos0.x, pos0.z, false), pos0.z);
    const float3 corner1(
      pos1.x, CGround::GetHeightAboveWater(pos1.x, pos1.z, false), pos1.z);
    const float3 corner2(
      pos0.x, CGround::GetHeightAboveWater(pos0.x, pos1.z, false), pos1.z);
    const float3 corner3(
      pos1.x, CGround::GetHeightAboveWater(pos1.x, pos0.z, false), pos0.z);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glLineWidth(2.0f);
    glBegin(GL_LINES);
                     ...
                     bool invColorSelect)
{
    ...
    boxData.mins = float3(std::min(pos0.x, pos1.x),
                          readMap->GetCurrMinHeight() - 250.0f,
                          std::min(pos0.z, pos1.z));
    boxData.maxs = float3(std::max(pos0.x, pos1.x),
                          readMap->GetCurrMaxHeight() + 10000.0f,
                           ...
                           const float3& cameraPos)
{
    ...
    const float3 mins(std::min(pos0.x, pos1.x),
                      readMap->GetCurrMinHeight() - 250.0f,
                      std::min(pos0.z, pos1.z));
    const float3 maxs(std::max(pos0.x, pos1.x),
                      readMap->GetCurrMaxHeight() + 10000.0f,
                      std::max(pos0.z, pos1.z));
...
static void
DrawCylinderShape(const void* data)
{
                              ...
                              const float* color)
{
    ...
    glLineWidth(2.0f);
    const float3 base(
      pos.x, CGround::GetHeightAboveWater(pos.x, pos.z, false), pos.z);
    glBegin(GL_LINES);
    glVertexf3(base);
