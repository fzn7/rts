...
CHwWinCursor::buildIco(unsigned char* dst, ImageData& image)
{
    ...
    cursize[0] = 3 * sizeof(WORD) + sizeof(CursorDirectoryHeader) +
                 sizeof(CursorInfoHeader) + xsize * ysize * 4 +
                 xsize * ysize / 8;
    dst += 4;

    ////////////////////////////////////////////////////////////////////////////////////////////////
    // the following code writes a fully working .cur file in the memory (in a
    // .ani container)
    ...
    curHeader.size =
      40 + xsize * ysize * 4 +
      xsize * ysize /
        8; // size of the bmp data (infoheader 40byte, 32bit color, 1bit mask)
...
CHwWinCursor::Finish()
{
    ...
    // resize images
    for (std::vector<ImageData>::iterator it = icons.begin(); it < icons.end();
         ++it)
        resizeImage(&*it, squaresize, squaresize);

    const int riffsize =
      32 + sizeof(AnihStructure) + (frames.size() + 2) * 2 * sizeof(DWORD);
    const int iconssize =
      icons.size() *
      (2 * sizeof(DWORD) + 3 * sizeof(WORD) + sizeof(CursorDirectoryHeader) +
       sizeof(CursorInfoHeader) + squaresize * squaresize * 4 +
    ...
    anih.flags = 0x3L; // using seq structure and .cur format for saving bmp
                       // data
    ...
    for (std::vector<ImageData>::iterator it = icons.begin(); it < icons.end();
        ...
        curmem += 2 * sizeof(DWORD) + 3 * sizeof(WORD) +
                  sizeof(CursorDirectoryHeader) + sizeof(CursorInfoHeader) +
                  squaresize * squaresize * 4 + squaresize * squaresize / 8;
    }
    ...
    cursor = (HCURSOR)CreateIconFromResourceEx(
      (PBYTE)mem, totalsize, FALSE, 0x00030000, squaresize, squaresize, 0);
    ...
    // if (cursor==NULL) LOG_L(L_ERROR, "HW cursor failed: x%d y%d", squaresize,
    // squaresize);
}
...
CHwWinCursor::Bind()
{
	...
	SetClassLong(info.window,GCL_HCURSOR,(LONG)cursor);*/ // SDL doesn't let
                                                              // us use it :<
...
CHwX11Cursor::Finish()
{
    ...
    // resize images
    for (std::vector<XcursorImage*>::iterator it = cimages.begin();
         it < cimages.end();
         ++it)
        resizeImage(*it, squaresize, squaresize);
