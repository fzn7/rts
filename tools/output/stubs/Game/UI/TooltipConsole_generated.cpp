...
CTooltipConsole::Draw()
{
    ...
    const float fontSize =
      (h * globalRendering->viewSizeY) * (smallFont->GetLineHeight() / 5.75f);

    float curX = x + 0.01f;
    float curY =
      y + h -
      0.5f * fontSize * smallFont->GetLineHeight() / globalRendering->viewSizeY;
    ...
    if (outFont) {
        smallFont->glPrint(
          curX, curY, fontSize, FONT_ASCENDER | FONT_OUTLINE | FONT_NORM, s);
    } else {
...
CTooltipConsole::IsAbove(int x, int y)
{
    ...
    return ((mx > (x + 0.01f)) && (mx < (x + w)) && (my > (y + 0.01f)) &&
            (my < (y + h)));
}
    ...
    if (active) {
        ...
        if (ud->extractsMetal > 0.0f) {
            if (rd->extractsMetal > 0.0f) {
                mMake +=
                  unit->metalExtract * (ud->extractsMetal / rd->extractsMetal);
            }
        }
        mUse += ud->metalUpkeep;
...
CTooltipConsole::MakeUnitString(const CUnit* unit)
{
    ...
    const bool enemyUnit =
      (teamHandler->AllyTeam(unit->team) != gu->myAllyTeam) &&
    ...
    if (enemyUnit && !(losStatus & LOS_INLOS) &&
...
CTooltipConsole::MakeUnitStatsString(const SUnitStats& stats)
{
    ...
    charsPrinted += SNPRINTF(&s[charsPrinted],
                             512 - charsPrinted,
                             "\nHealth %.0f/%.0f",
                             stats.health,
                             stats.maxHealth);
    ...
    if (stats.harvestMetalMax > 0.0f || stats.harvestEnergyMax > 0.0f) {
        charsPrinted += SNPRINTF(
          ...
          "%.1f" GREY "/" RED "-%.1f" GREY " (" GREEN "%.1f" GREY "/" BLUE
          "%.1f" GREY ") " BLUE "Energy: " GREEN "%.1f" GREY "/" RED
          "-%.1f" GREY " (" GREEN "%.1f" GREY "/" BLUE "%.1f" GREY ")\x08",
          stats.experience,
          stats.cost,
          stats.maxRange,
          stats.metalMake,
          stats.metalUse,
          stats.harvestMetal,
          stats.harvestMetalMax,
          stats.energyMake,
          stats.energyUse,
          stats.harvestEnergy,
          stats.harvestEnergyMax);
        charsPrinted = std::min<size_t>(512, charsPrinted);
    } else {
        charsPrinted +=
          SNPRINTF(&s[charsPrinted],
                   512 - charsPrinted,
                   "\nExperience %.2f Cost %.0f Range %.0f\n" BLUE
                   "Metal: " GREEN "%.1f" GREY "/" RED "-%.1f " BLUE
                   "Energy: " GREEN "%.1f" GREY "/" RED "-%.1f\x08",
                   stats.experience,
                   stats.cost,
                   stats.maxRange,
                   stats.metalMake,
...
CTooltipConsole::MakeGroundString(const float3& pos)
{
    ...
    const int typeMapIdx = std::min(mapDims.hmapx * mapDims.hmapy - 1,
                                    std::max(0, pz * mapDims.hmapx + px));
    const unsigned char* typeMap = readMap->GetTypeMapSynced();
    const CMapInfo::TerrainType* tt =
      &mapInfo->terrainTypes[typeMap[typeMapIdx]];
...
void
SUnitStats::AddUnit(const CUnit* unit, bool enemy)
{
    ...
    if (!decoyDef) {
        ...
        experience =
          (experience * (count - 1) + unit->experience) / count; // average xp
