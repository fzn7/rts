...
{
    return (CKeyCodes::IsModifier(Key()) ||
            (Key() == keyBindings->GetFakeMetaKey()));
}
                          ...
                          const bool isRepeat)
{
    ...
    // When you want to press c,alt+b you will press the
    // c(down),c(up),alt(down),b(down)
    // -> the chain will be: c,Alt+alt,Alt+b
    // this should now match "c,alt+b", so we need to get rid of the redundant
    // Alt+alt . Still we want to support "Alt+alt,Alt+alt" (double click alt),
    // so only override e.g. the last in chain when the new keypress is _not_ a
    // modifier.
    if (!empty() && !ks.IsPureModifier() && back().IsPureModifier() &&
        (back().Mod() == ks.Mod())) {
        times.back() = t;
        back() = ks;
        return;
