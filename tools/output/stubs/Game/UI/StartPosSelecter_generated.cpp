...
CStartPosSelecter::DrawStartBox() const
{
    ...
    const float dy = (myStartData.startRectBottom - myStartData.startRectTop) *
                     mapDims.mapy * SQUARE_SIZE / 10;
    const float dx = (myStartData.startRectRight - myStartData.startRectLeft) *
                     mapDims.mapx * SQUARE_SIZE / 10;
...
CStartPosSelecter::Draw()
{
    ...
    const float my = (globalRendering->viewSizeY - float(mouse->lasty)) /
    ...
    const float unitWidth =
      font->GetSize() * font->GetTextWidth("Ready") * globalRendering->pixelX;
    const float unitHeight =
      font->GetSize() * font->GetLineHeight() * globalRendering->pixelY;

    const float ySize = (readyBox.y2 - readyBox.y1);
    const float xSize = (readyBox.x2 - readyBox.x1);

    const float fontScale =
      0.9f * std::min(xSize / unitWidth, ySize / unitHeight);
    ...
    font->glPrint(xPos,
                  yPos,
                  fontScale,
                  FONT_OUTLINE | FONT_CENTER | FONT_VCENTER | FONT_SCALE |
                    FONT_NORM,
                  "Ready");
    font->End();
}
