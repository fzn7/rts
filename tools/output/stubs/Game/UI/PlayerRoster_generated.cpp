...
PlayerRoster::SetSortTypeByName(const std::string& name)
{
    ...
    if (lower == "ally") {
        ...
        compareType = PlayerName;
    } else if (lower == "cpu") {
        compareType = PlayerCPU;
    } else if (lower == "ping") {
        compareType = PlayerPing;
    } else {
...
{
    if ((type == Allies) || (type == TeamID) || (type == PlayerName) ||
        (type == PlayerCPU) || (type == PlayerPing)) {
        compareType = type;
        SetCompareFunc();
        return true;
    }
...
CompareBasics(const CPlayer* aP, const CPlayer* bP)
{
    ...
    if (gs->PreSimFrame()) {
        if (aP->ping == PATHING_FLAG && bP->ping != PATHING_FLAG) {
            return -1;
        }
        if (aP->ping != PATHING_FLAG && bP->ping == PATHING_FLAG) {
            return +1;
