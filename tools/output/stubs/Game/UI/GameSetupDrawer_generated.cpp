...
GameSetupDrawer::Draw()
{
    ...
    if (readyCountdown > spring_nulltime) {
        startState =
          "Starting in " + IntToString(readyCountdown.toSecsi(), "%i");
        ...
        const CKeyBindings::HotkeyList& fsKeys =
          keyBindings->GetHotkeys("forcestart");
        const std::string fsKey = fsKeys.empty() ? "<none>" : *fsKeys.begin();
        startState = std::string("Waiting for players, press ") + fsKey +
                     " to force start";
    } else {
    ...
    font->glPrint(
      0.3f, 0.7f, 1.0f, FONT_OUTLINE | FONT_SCALE | FONT_NORM, startState);
