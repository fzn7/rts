...
DrawThreadBarcode()
{
    ...
    CVertexArray* va = GetVertexArray();
    va->Initialize();
    va->AddVertex0(drawArea[0] - 10 * globalRendering->pixelX,
                   drawArea[1] - 10 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(drawArea[0] - 10 * globalRendering->pixelX,
                   drawArea[3] + 10 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(drawArea[2] + 10 * globalRendering->pixelX,
                   drawArea[3] + 10 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(drawArea[2] + 10 * globalRendering->pixelX,
                   drawArea[1] - 10 * globalRendering->pixelY,
    ...
    // title
    font->glFormat(drawArea[0],
                   drawArea[3],
                   0.7f,
                   FONT_TOP | DBG_FONT_FLAGS,
                   "ThreadPool (%.0fsec)",
                   maxHist_f);
    ...
    for (auto& frames : coreProf) {
        ...
        drawArea2[1] =
          drawArea[1] + ((drawArea[3] - drawArea[1]) / numThreads) * i++;
        drawArea2[3] = drawArea[1] +
                       ((drawArea[3] - drawArea[1]) / numThreads) * i -
                       4 * globalRendering->pixelY;
...
DrawFrameBarcode()
{
    ...
    va->AddVertex0(drawArea[0] - 10 * globalRendering->pixelX,
                   ...
                   0.0f);
    va->AddVertex0(drawArea[0] - 10 * globalRendering->pixelX,
                   drawArea[3] + 20 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(drawArea[2] + 10 * globalRendering->pixelX,
                   drawArea[3] + 20 * globalRendering->pixelY,
                   0.0f);
    ...
    const float xs1 =
      drawArea[2] - 1.f / (30.f * maxHist_f) * (drawArea[2] - drawArea[0]);
...
DrawProfiler()
{
    ...
    // draw the background of the window
    {
        ...
        va->AddVertex0(
          start_x, start_y - profiler.profile.size() * lineHeight - 0.01f, 0);
        va->AddVertex0(
          end_x, start_y - profiler.profile.size() * lineHeight - 0.01f, 0);
    ...
    // table header
    {
        ...
        fStartX += 0.04f;
        font->glPrint(fStartX,
                      fStartY,
                      textSize,
                      FONT_SHADOW | FONT_DESCENDER | FONT_SCALE | FONT_NORM |
                        FONT_RIGHT,
                      "totaltime");
    ...
    // draw the textual info (total-time, short-time percentual time,
    // timer-name)
    int y = 1;
    for (auto pi = profiler.profile.begin(); pi != profiler.profile.end();
         ++pi, ++y) {
        ...
        // print total-time running since application start
        fStartX += 0.04f;
        font->glFormat(fStartX,
                       fStartY,
                       textSize,
                       FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT,
                       "%.2fs",
                       profileData.total.toSecsf());
        ...
        font->glFormat(fStartX,
                       ...
                       FONT_DESCENDER | FONT_SCALE | FONT_NORM | FONT_RIGHT,
                       "\xff\xff%c%c%.2f%%",
                       profileData.newPeak ? 1 : 255,
                       profileData.newPeak ? 1 : 255,
                       profileData.peak * 100);
        fStartX += 0.04f;
        font->glFormat(fStartX,
                       fStartY,
                       textSize,
        ...
        font->glPrint(fStartX,
                      fStartY,
                      textSize,
                      FONT_DESCENDER | FONT_SCALE | FONT_NORM,
                      pi->first);
    }
    ...
    glPushMatrix();
    glTranslatef(start_x + 0.005f,
                 start_y + boxSize,
                 0); // we are now at upper left of first box
    CVertexArray* va = GetVertexArray();
    ...
    for (auto pi = profiler.profile.begin(); pi != profiler.profile.end();
        ...
        va->AddVertexC(float3(0, -i * lineHeight - boxSize, 0),
                       c); // lower left
        va->AddVertexC(float3(boxSize, -i * lineHeight - boxSize, 0),
                       c);                                      // lower right
        va->AddVertexC(float3(boxSize, -i * lineHeight, 0), c); // upper right

        if (pi->second.showGraph) {
            va2->AddVertex0(lineHeight + selOffset,
                            -i * lineHeight - selOffset,
                            0); // upper left
            va2->AddVertex0(lineHeight + selOffset,
                            -i * lineHeight - boxSize + selOffset,
                            0); // lower left
            va2->AddVertex0(lineHeight + boxSize - selOffset,
                            -i * lineHeight - boxSize + selOffset,
                            0); // lower right
            va2->AddVertex0(lineHeight + boxSize - selOffset,
                            -i * lineHeight - selOffset,
    ...
    for (auto pi = profiler.profile.begin(); pi != profiler.profile.end();
        ...
        va->Initialize();
        const float steps_x =
          (end_x - start_x) / CTimeProfiler::TimeRecord::frames_size;
        for (size_t a = 0; a < CTimeProfiler::TimeRecord::frames_size; ++a) {
            // profile runtime; eg 0.5f means: uses 50% of a CPU (during that
            // frame) This may be more then 1.0f, in case an operation which ran
            // over many frames, ended in this one.
...
DrawInfoText()
{
    ...
    CVertexArray* va = GetVertexArray();
    va->Initialize();
    va->AddVertex0(0.01f - 10 * globalRendering->pixelX,
                   0.02f - 10 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(0.01f - 10 * globalRendering->pixelX,
                   0.16f + 20 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(start_x - 0.05f + 10 * globalRendering->pixelX,
                   0.16f + 20 * globalRendering->pixelY,
                   0.0f);
    va->AddVertex0(start_x - 0.05f + 10 * globalRendering->pixelX,
                   0.02f - 10 * globalRendering->pixelY,
    ...
    font->glFormat(0.01f,
                   ...
                   1.0f,
                   DBG_FONT_FLAGS,
                   "FPS: %0.1f SimFPS: %0.1f SimFrame: %d Speed: %2.2f (%2.2f) "
                   "Particles: %d (%d : %0.1f)",
                   globalRendering->FPS,
                   gu->simFPS,
                   gs->frameNum,
                   gs->speedFactor,
                   gs->wantedSpeedFactor,
                   projectileHandler->syncedProjectiles.size() +
                     projectileHandler->unsyncedProjectiles.size(),
                   projectileHandler->GetCurrentParticles(),
                   projectileHandler->GetParticleSaturation(true));
    ...
    font->glFormat(0.01f,
                   ...
                   0.7f,
                   DBG_FONT_FLAGS,
                   "avgFrame: %s%2.1fms\b avgDrawFrame: %s%2.1fms\b "
                   "avgSimFrame: %s%2.1fms\b",
                   (gu->avgFrameTime > 30) ? "\xff\xff\x01\x01" : "",
                   gu->avgFrameTime,
                   (gu->avgDrawFrameTime > 16) ? "\xff\xff\x01\x01" : "",
                   gu->avgDrawFrameTime,
                   (gu->avgSimFrameTime > 16) ? "\xff\xff\x01\x01" : "",
                   gu->avgSimFrameTime);
...
ProfileDrawer::IsAbove(int x, int y)
{
    ...
    if (mx < start_x || mx > end_x ||
        my < start_y - profiler.profile.size() * lineHeight || my > start_y) {
        return false;
    }
