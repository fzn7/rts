...
CPlayerRosterDrawer::Draw()
{
    ...
    for (int a = 0; a < count; ++a) {
        ...
        if (p->ping != PATHING_FLAG || !gs->PreSimFrame()) {
            ...
            float4 cpucolor(
              !p->spectator && p->cpuUsage > 0.75f &&
                  gs->speedFactor < gs->wantedSpeedFactor * 0.99f &&
                  (currentTime & 128)
                ? 0.5f
                : std::max(0.01f, std::min(1.0f, p->cpuUsage * 2.0f / 0.75f)),
              std::min(1.0f,
                       std::max(0.01f, (1.0f - p->cpuUsage / 0.75f) * 2.0f)),
              0.01f,
              1.0f);
            float4 pingcolor(
              !p->spectator && globalConfig->reconnectTimeout > 0 &&
                  p->ping > 1000 * globalConfig->reconnectTimeout &&
                  (currentTime & 128)
                ? 0.5f
                : std::max(0.01f, std::min(1.0f, (p->ping - 250) / 375.0f)),
              std::min(1.0f, std::max(0.01f, (1000 - p->ping) / 375.0f)),
              0.01f,
              1.0f);
            SNPRINTF(
              buf,
              sizeof(buf),
              "\xff%c%c%c%c \t%i \t%s   \t\xff%c%c%c%s   \t\xff%c%c%c%.0f%%  "
              "\t\xff%c%c%c%dms",
              allycolor[0],
              allycolor[1],
              allycolor[2],
    ...
    smallFont->SetColors();
    smallFont->glPrintTable(1.0f - 5 * globalRendering->pixelX,
                            0.00f + 5 * globalRendering->pixelY,
                            1.0f,
                            font_options,
                            chart);
}
