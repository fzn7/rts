...
inline void
CSelectedUnitsHandlerAI::AddUnitSetMaxSpeedCommand(CUnit* unit,
                                                   unsigned char options)
{
    ...
    if (cai->CanSetMaxSpeed()) {
        Command c(
          CMD_SET_WANTED_MAX_SPEED, options, unit->moveType->GetMaxSpeed());
...
{
    const std::vector<int>& netSelected =
    ...
    //
    ...
    else if (((cmd_id == CMD_MOVE) || (cmd_id == CMD_FIGHT)) &&
             (c.GetParamsCount() == 6)) {
        ...
        for (ui = netSelected.begin(); ui != netSelected.end(); ++ui) {
            ...
            if (unit) {
               ...
               ((cmd_id == CMD_MOVE) || (cmd_id == CMD_PATROL) ||
                (cmd_id == CMD_FIGHT))) {
        CalculateGroupData(player, !!(c.options & SHIFT_KEY));
...
{
    // Finding the highest, lowest and weighted central positional coordinates
    ...
    int mobileUnits = 0;
    minMaxSpeed = 1e9f;
    for (std::vector<int>::iterator ui =
           selectedUnitsHandler.netSelected[player].begin();
         ui != selectedUnitsHandler.netSelected[player].end();
         ++ui) {
        CUnit* unit = unitHandler->units[*ui];
        if (unit) {
...
CSelectedUnitsHandlerAI::MakeFrontMove(Command* c, int player)
{
    ...
    if (centerPos.distance(rightPos) <
        selectedUnitsHandler.netSelected[player].size() +
          33) { // Strange line! treat this as a standard move if the front isnt
                // long enough
        for (std::vector<int>::iterator ui =
               selectedUnitsHandler.netSelected[player].begin();
             ui != selectedUnitsHandler.netSelected[player].end();
    ...
    if (frontLength > sumLength * 2 * 8) {
        addSpace = (frontLength - sumLength * 2 * 8) /
                   (selectedUnitsHandler.netSelected[player].size() - 1);
    }
    ...
    for (std::multimap<float, int>::iterator oi = orderedUnits.begin();
         oi != orderedUnits.end();) {
        bool newline;
        nextPos = MoveToPos(oi->second, nextPos, sd, c, &frontcmds, &newline);
        // mix units in each row to avoid weak flanks consisting solely of e.g.
        // artillery units
        std::multimap<float, std::vector<int>>::iterator si =
          sortUnits.find(oi->first);
        if (si == sortUnits.end())
            si = sortUnits.insert(std::pair<float, std::vector<int>>(
              oi->first, std::vector<int>()));
        ...
        if (oi == orderedUnits.end() || newline) {
            ...
            for (std::multimap<float, std::vector<int>>::iterator ui =
                   sortUnits.begin();
                 ui != sortUnits.end();
                 ++ui) {
            ...
            for (std::vector<std::pair<int, Command>>::iterator fi =
                   frontcmds.begin();
                 fi != frontcmds.end();
                 ++fi) {
                int bestpos = 0;
            ...
            for (std::vector<std::pair<int, Command>>::iterator fi =
                 ...
                 ++fi) {
                unitHandler->units[randunits[i++]]->commandAI->GiveCommand(
                  fi->second, false);
            }
            frontcmds.clear();
        }
                                         ...
                                         int player)
{
    ...
    for (auto ui = netUnits.cbegin(); ui != netUnits.cend(); ++ui) {
        ...
        if (unit) {
            ...
            if (range < 1) {
                // give weaponless units a long range to make them go to the
                // back
                range = 2000;
            }
            const float value =
              ((ud->metal * 60) + ud->energy) / unit->unitDef->health * range;
  ...
  bool* newline)
{
    ...
    *newline = false;
    ...
    float3 retPos(
      nextCornerPos.x + unitSize * 8 * 2 + addSpace, 0, nextCornerPos.z);
    float3 movePos(nextCornerPos.x + unitSize * 8 + addSpace,
                   0,
    ...
    float3 pos;
    pos.x = rightPos.x + (movePos.x * (dir.x / dir.y)) -
            (movePos.z * (dir.z / dir.y));
    pos.z = rightPos.z + (movePos.x * (dir.z / dir.y)) +
            (movePos.z * (dir.x / dir.y));
...
void
CSelectedUnitsHandlerAI::SelectAttack(const Command& cmd, int player)
{
    ...
    for (unsigned int t = 0; t < targetsCount; t++) {
        ...
        const float3 unitPos =
          queueing ? LastQueuePosition(unit) : float3(unit->midPos);
                                           ...
                                           std::vector<int>& units)
{
    ...
    const std::vector<CUnit*>& tmpUnits =
