...
FPSUnitController::Update()
{
    ...
    float hitDist = TraceRay::TraceRay(pos,
                                       viewDir,
                                       controllee->maxRange,
                                       Collision::NOCLOAKED,
                                       controllee,
    ...
    if (hitUnit != NULL) {
        ...
        if (!mouse2) {
            ...
            if ((targetPos.y - CGround::GetHeightReal(
                                 targetPos.x, targetPos.z)) <= SQUARE_SIZE) {
...
FPSUnitController::SendStateUpdate()
{
    ...
    if (hp.x != oldHeading || hp.y != oldPitch || state != oldState) {
        ...
        clientNet->Send(CBaseNetProtocol::Get().SendDirectControlUpdate(
          gu->myPlayerNum, state, hp.x, hp.y));
    }
}
