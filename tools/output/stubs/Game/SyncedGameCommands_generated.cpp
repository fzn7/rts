...
class NoSpecDrawActionExecutor : public ISyncedActionExecutor
{
  public:
    NoSpecDrawActionExecutor()
      : ISyncedActionExecutor("NoSpecDraw",
  ...
  public:
    ...
    bool Execute(const SyncedAction& action) const
    {
        ...
        if (argAllyTeam < maxAllyTeam) {
            losHandler->globalLOS[argAllyTeam] =
              !losHandler->globalLOS[argAllyTeam];

            LOG("[GlobalLosActionExecutor] global LOS toggled for allyteam %u",
                argAllyTeam);
            return true;
        }
  ...
  public:
    NoCostActionExecutor()
      : ISyncedActionExecutor(
          ...
          " to build everything for zero resource costs",
          true)
    {}
  ...
  public:
    ...
    bool Execute(const SyncedAction& action) const
    {
        ...
        unitLoader->ParseAndExecuteGiveUnitsCommand(
          ...
          playerHandler->Player(action.GetPlayerID())->team);
        return true;
    }
};
...
class NoSpectatorChatActionExecutor : public ISyncedActionExecutor
{
  public:
    NoSpectatorChatActionExecutor()
      : ISyncedActionExecutor("NoSpectatorChat",
...
static void
ExecuteSyncedLuaAction(CLuaHandleSynced* handler,
...
class LuaRulesActionExecutor : public ISyncedActionExecutor
{
  public:
    ...
    {
        // NOTE:
        //   previously only the host player (ID == 0) was allowed to issue
        //   these actions prior to some server changes they worked even in
        //   demos with that restriction, but this is no longer the case so we
        //   now let any player execute them (for MP it does not matter who does
        //   so since they are not meant to be used there ITFP and no less
        //   sync-safe)
  ...
  public:
    DesyncActionExecutor()
      : ISyncedActionExecutor(
          ...
          " the rest of the participating hosts",
          true)
    {}
    ...
    bool Execute(const SyncedAction& action) const
    {
        ...
        // ASSERT_SYNCED(float3(gu->myPlayerNum, gu->myPlayerNum,
        // gu->myPlayerNum));
...
class TakeActionExecutor : public ISyncedActionExecutor
{
  public:
    ...
    bool Execute(const SyncedAction& action) const
    {
        const CPlayer* actionPlayer =
          playerHandler->Player(action.GetPlayerID());
...
SyncedGameCommands::CreateInstance()
{
    if (singleton == NULL) {
        ...
        throw std::logic_error(
          "SyncedGameCommands singleton is already initialized");
    }
}
...
SyncedGameCommands::DestroyInstance()
{
    if (singleton != NULL) {
        ...
        LOG_L(L_WARNING,
              "SyncedGameCommands singleton was not initialized or is already "
              "destroyed");
    }
}
