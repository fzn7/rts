...
bool
AviVideoCapturing::IsCapturingSupported() const
{
    return true;
}
...
AviVideoCapturing::StartCapturing()
{
    ...
    if (vi == MAX_NUM_VIDEOS) {
        LOG_L(L_ERROR,
              "You have too many videos on disc already, please move, rename "
              "or delete some.");
        ...
        // aviGenerator->InitEngine() (avicap32.dll)? modifies the FPU control
        // word.  Setting it back to default state.
...
AviVideoCapturing::RenderFrame()
{
    if (IsCapturing()) {
        ...
        //		LOG("Saved avi frame size %i %i", ih->biWidth,
        //ih->biHeight);
    }
}
