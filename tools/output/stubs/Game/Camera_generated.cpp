...
CCamera::Update(bool updateDirs, bool updateMats, bool updatePort)
{
    ...
    if (updateMats)
        UpdateMatrices(globalRendering->viewSizeX,
                       globalRendering->viewSizeY,
                       globalRendering->aspectRatio);
    if (updatePort)
        UpdateViewPort(globalRendering->viewPosX,
                       0,
...
CCamera::UpdateFrustum()
{
    switch (projType) {
        case PROJTYPE_PERSP: {
            ...
            const float3 forwardx =
              (-forward * math::tan(globalRendering->aspectRatio * halfFov));

            frustumPlanes[FRUSTUM_PLANE_TOP] =
              (forwardy + up).UnsafeANormalize();
            frustumPlanes[FRUSTUM_PLANE_BOT] =
              (forwardy - up).UnsafeANormalize();
            frustumPlanes[FRUSTUM_PLANE_RGT] =
              (forwardx + right).UnsafeANormalize();
            frustumPlanes[FRUSTUM_PLANE_LFT] =
              (forwardx - right).UnsafeANormalize();
        } break;
        case PROJTYPE_ORTHO: {
...
CCamera::UpdateMatrices(unsigned int vsx, unsigned int vsy, float var)
{
    ...
    const float3 fShake = ((forward * (1.0f + tiltOffset.z)) +
                           (right * tiltOffset.x) + (up * tiltOffset.y))
                            .ANormalize();
...
CCamera::UpdateViewRange()
{
    ...
    const float maxDistToBorder =
      math::sqrt(Square(maxDistToBorderX) + Square(maxDistToBorderZ));

    const float angleViewRange =
      (1.0f - forward.dot(UpVector)) * maxDistToBorder;
    const float heightViewRange =
      (pos.y - std::max(0.0f, readMap->GetCurrMinHeight())) * 2.4f;
...
{
    // orthographic plane offsets along each respective normal; [0] = LFT&RGT,
    // [1] = TOP&BOT
    ...
    for (unsigned int i = FRUSTUM_PLANE_LFT; i < FRUSTUM_PLANE_FRN; i++) {
        if (!AABBInOriginPlane(
              pos, mins, maxs, frustumPlanes[i], planeOffsets[i >> 1])) {
            return false;
        }
    }
...
CCamera::InView(const float3& p, float radius) const
{
    ...
    for (unsigned int i = FRUSTUM_PLANE_LFT; i < FRUSTUM_PLANE_FRN; i++) {
        if (objectVector.dot(frustumPlanes[i]) >
            (xyPlaneOffsets[i >> 1] + radius)) {
            return false;
        }
    }

    // test if <p> is behind the far-plane
    return (objectVector.dot(frustumPlanes[FRUSTUM_PLANE_BCK]) <=
            (zPlaneOffsets[1] + radius));
}
...
CCamera::GetRgtFromRot(const float3 r)
{
    ...
    //
    //   fwd=(0,+1,0) -> rot=GetRotFromDir(fwd)=(0.0, PI, 0.0) ->
    //   GetRgtFromRot(rot)=(-1.0, 0.0, 0.0) fwd=(0,-1,0) ->
    //   rot=GetRotFromDir(fwd)=( PI, PI, 0.0) -> GetRgtFromRot(rot)=(+1.0, 0.0,
    //   0.0)
    //
    float3 rgt;
...
CCamera::CalcPixelDir(int x, int y) const
{
    ...
    const float dx = float(x - globalRendering->viewPosX - (vsx >> 1)) / vsy *
                     (tanHalfFov * 2.0f);
...
CCamera::GetFrustumSides(float miny, float maxy, float scale, bool negOnly)
{
    ...
    for (unsigned int i = 0; i < FRUSTUM_PLANE_FRN; i++) {
        GetFrustumSide(frustumPlanes[i],
                       posOffsets[i],
                       miny,
                       maxy,
                       scale,
                       negOnly ? FRUSTUM_SIDE_NEG : FRUSTUM_SIDE_POS);
    }
}
                        ...
                        unsigned int side)
{
    ...
    if (ydir.y != 0.0f) {
        ...
        if (normal.y > 0.0f) {
            pInt =
              (pos + offset) - ydir * (((pos.y + offset.y) - miny) / ydir.y);
        } else {
            pInt =
              (pos + offset) - ydir * (((pos.y + offset.y) - maxy) / ydir.y);
        }
    }

    // <line.dir> is the direction coefficient (0 ==> parallel to z-axis, inf
    // ==> parallel to x-axis) in the xz-plane; <line.base> is the x-coordinate
    // at which line intersects x-axis; <line.sign> indicates line direction,
    // ie. left-to-right (whenever <xdir.z> is negative) or right-to-left NOTE:
    //   (b.x / b.z) is actually the reciprocal of the DC (ie. the number of
    //   steps along +x for one step along +y); the world z-axis is inverted
    //   wrt. a regular Carthesian grid, so the DC is also inverted
    FrustumLine line;
...
CCamera::GetMoveVectorFromState(bool fromKeyState) const
{
    ...
    if (fromKeyState) {
        ...
        float2 distToEdge; // must be float, ints don't save the sign in case of
                           // 0 and we need it for copysign()
        distToEdge.x = Clamp(mouse->lastx, 0, screenW);
        distToEdge.y = Clamp(mouse->lasty, 0, screenH);
        if (((screenW - 1) - distToEdge.x) < distToEdge.x)
            distToEdge.x = -((screenW - 1) - distToEdge.x);
        if (((screenH - 1) - distToEdge.y) < distToEdge.y)
            distToEdge.y = -((screenH - 1) - distToEdge.y);
        distToEdge.x = -distToEdge.x;

        float2 move;
        if (dynamic) {
            move.x = Clamp(
              float(border.x - std::abs(distToEdge.x)) / border.x, 0.f, 1.f);
            move.y = Clamp(
              float(border.y - std::abs(distToEdge.y)) / border.y, 0.f, 1.f);
        } else {
