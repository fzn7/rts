...
CGameSetup::LoadUnitRestrictions(const TdfParser& file)
{
    ...
    for (int i = 0; i < numRestrictions; ++i) {
        const std::string resUnitName =
          "GAME\\RESTRICT\\Unit" + IntToString(i, "%d");
        const std::string resUnitLimit =
          "GAME\\RESTRICT\\Limit" + IntToString(i, "%d");
...
{
    if (withoutMap &&
        (startPosType == StartPos_Random || startPosType == StartPos_Fixed))
        throw content_error(
          "You need the map to use the map's start-positions");
                         ...
                         std::vector<std::string>& mutatorsList)
{
    for (int a = 0; a < 10; ++a) {
        const std::string s =
          file.SGetValueDef("", IntToString(a, "GAME\\MUTATOR%i"));
        if (s.empty())
            break;
...
CGameSetup::LoadPlayers(const TdfParser& file, std::set<std::string>& nameList)
{
    ...
    for (int a = 0; a < MAX_PLAYERS; ++a) {
        ...
        if (playerBase.name.empty())
            throw content_error(
              str(boost::format("GameSetup: No name given for Player %i") % a));
        if (nameList.find(playerBase.name) != nameList.end())
            throw content_error(
              str(boost::format(
                    "GameSetup: Player %i has name %s which is already taken") %
                  a % playerBase.name.c_str()));
                            ...
                            std::set<std::string>& nameList)
{
    ...
    for (int a = 0; a < MAX_PLAYERS; ++a) {
        ...
        data.hostPlayer =
          atoi(file.SGetValueDef("-1", section + "Host").c_str());
        ...
        std::string visibleName =
          file.SGetValueDef(data.shortName, section + "Name");
...
CGameSetup::LoadAllyTeams(const TdfParser& file)
{
...

    {
        ...
        for (size_t a = 0; a < numAllyTeams; ++a) {
            ...
            allyStartingData[a].allies[a] =
              true; // each team is allied with itself

            const std::string section =
              "GAME\\ALLYTEAM" + IntToString(a) + "\\";

            const size_t numAllies =
              atoi(file.SGetValueDef("0", section + "NumAllies").c_str());

            for (size_t b = 0; b < numAllies; ++b) {
                const std::string key =
                  "GAME\\ALLYTEAM" + IntToString(a) + "\\Ally" + IntToString(b);
...
CGameSetup::RemapPlayers()
{
    ...
    for (size_t a = 0; a < teamStartingData.size(); ++a) {
        if (playerRemap.find(teamStartingData[a].GetLeader()) ==
            playerRemap.end()) {
            std::ostringstream buf;
            buf << "GameSetup: Team " << a
                << " has invalid leader: " << teamStartingData[a].GetLeader();
            throw content_error(buf.str());
        }
        teamStartingData[a].SetLeader(
          playerRemap[teamStartingData[a].GetLeader()]);
    }

    // relocate AI.hostPlayer field
    for (size_t a = 0; a < skirmishAIStartingData.size(); ++a) {
        if (playerRemap.find(skirmishAIStartingData[a].hostPlayer) ==
            playerRemap.end())
            throw content_error("invalid AI.Host in GameSetup script");

        skirmishAIStartingData[a].hostPlayer =
          playerRemap[skirmishAIStartingData[a].hostPlayer];
    }
}
...
CGameSetup::RemapTeams()
{
    ...
    for (size_t a = 0; a < playerStartingData.size(); ++a) {
        if (playerStartingData[a].spectator) {
            ...
            if (teamRemap.find(playerStartingData[a].team) == teamRemap.end())
                throw content_error(
                  str(boost::format(
                        "GameSetup: Player %i belong to wrong team: %i") %
                      a % playerStartingData[a].team));
    ...
    for (size_t a = 0; a < skirmishAIStartingData.size(); ++a) {
        ...
        skirmishAIStartingData[a].team =
          teamRemap[skirmishAIStartingData[a].team];
        // unused (also seems redundant)
        // team_skirmishAI[skirmishAIStartingData[a].team] =
        // &(skirmishAIStartingData[a]);
    }
}
...
CGameSetup::RemapAllyteams()
{
    ...
    for (size_t a = 0; a < teamStartingData.size(); ++a) {
        if (allyteamRemap.find(teamStartingData[a].teamAllyteam) ==
            allyteamRemap.end())
            throw content_error("invalid Team.Allyteam in GameSetup script");

        teamStartingData[a].teamAllyteam =
          allyteamRemap[teamStartingData[a].teamAllyteam];
    }
}
...
CGameSetup::Init(const std::string& buf)
{
    ...
    file.GetDef((std::underlying_type<StartPosType>::type&)startPosType,
                IntToString(StartPos_Fixed),
                "GAME\\StartPosType");
