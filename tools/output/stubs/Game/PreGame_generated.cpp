...
CPreGame::Draw()
{
    ...
    if (!clientNet->Connected()) {
        if (clientSetup->isHost)
            ...
            font->glFormat(0.5f,
                           ...
                           2.0f,
                           FONT_CENTER | FONT_SCALE | FONT_NORM,
                           "Connecting to server (%ds)",
                           (spring_gettime() - timer).toSecsi());
    } else {
        font->glPrint(0.5f,
                      0.48f,
                      2.0f,
    ...
    font->glFormat(0.60f,
                   ...
                   FONT_SCALE | FONT_NORM,
                   "User name: %s",
                   clientSetup->myPlayerName.c_str());
    ...
    font->glPrint(0.5f,
                  ...
                  "License, see doc/LICENSE for more info");
...
CPreGame::StartServer(const std::string& setupscript)
{
    ...
    // We must map the map into VFS this early, because server needs the start
    // positions. Take care that MapInfo isn't loaded here, as map options
    // aren't available to it yet.
    ...
    const std::string& modArchive =
      archiveScanner->ArchiveFromName(startGameSetup->modName);
    const std::string& mapArchive =
      archiveScanner->ArchiveFromName(startGameSetup->mapName);
    const auto modChecksum =
      archiveScanner->GetArchiveCompleteChecksum(modArchive);
    const auto mapChecksum =
      archiveScanner->GetArchiveCompleteChecksum(mapArchive);
    ...
    gameServer->AddLocalClient(clientSetup->myPlayerName,
                               SpringVersion::GetFull());
...
CPreGame::UpdateClientNet()
{
    ...
    while ((packet = clientNet->GetData(gs->frameNum))) {
        ...
        if (packet->length <= 0) {
            LOG_L(L_WARNING,
                  "[PreGame::%s] zero-length packet (header: %i)",
                  __FUNCTION__,
                  inbuf[0]);
            continue;
        }
        ...
        switch (inbuf[0]) {
            ...
            case NETMSG_CREATE_NEWPLAYER: {
                ...
                try {
                    ...
                    LOG("[PreGame::%s] added new player %s with number %d to "
                        "team %d",
                        __FUNCTION__,
                        name.c_str(),
                        player.playerNum,
                    ...
                    LOG_L(
                      L_ERROR,
                      "[PreGame::%s] got invalid NETMSG_CREATE_NEWPLAYER: %s",
                      __FUNCTION__,
                      ex.what());
                }
                break;
            ...
            case NETMSG_SETPLAYERNUM: {
                ...
                if (!playerHandler->IsValidPlayer(playerNum))
                    throw content_error(
                      "Invalid player number received from server");

                gu->SetMyPlayer(playerNum);

                LOG("[PreGame::%s] user number %i (team %i, allyteam %i)",
                    __FUNCTION__,
                    gu->myPlayerNum,
                    gu->myTeam,
                    gu->myAllyTeam);

                CLoadScreen::CreateInstance(
                  gameSetup->MapFile(), modArchive, savefile);
            ...
            default: {
                LOG_L(L_WARNING,
                      "[PreGame::%s] unknown packet type (header: %i)",
                      __FUNCTION__,
                      inbuf[0]);
                break;
            }
...
{
    TdfParser script((gameData->GetSetupText()).c_str(),
                     (gameData->GetSetupText()).size());
...

    {
        ...
        for (std::map<std::string, TdfParser::TdfSection*>::iterator it =
               tgame->sections.begin();
             it != tgame->sections.end();
             ++it) {
        ...
        // is this needed?
        TdfParser::TdfSection* modopts =
          tgame->construct_subsection("MODOPTIONS");
...
CPreGame::GameDataReceived(boost::shared_ptr<const netcode::RawPacket> packet)
{
    ...
    try {
        ...
        throw content_error(std::string("Server sent us invalid GameData: ") +
                            ex.what());
    }
    ...
    // modified version (cf StartServerForDemo) which the server already has
    // that contains an extra player
    if (gameSetup != NULL)
        SafeDelete(gameSetup);

    if (CGameSetup::LoadReceivedScript(gameData->GetSetupText(),
                                       clientSetup->isHost)) {
        assert(gameSetup != NULL);
    ...
    for (int p = 0; p < playerHandler->ActivePlayers(); ++p) {
        ...
        if (!teamHandler->IsValidAllyTeam(teamHandler->AllyTeam(
              player->team))) { // TODO: seems not to make sense really
    ...
    try {
        archiveScanner->CheckArchive(gameSetup->mapName,
                                     gameData->GetMapChecksum());
    ...
    if (clientSetup->isHost &&
        !gameSetup->recordDemo) { // script.txt allows to disable demo file
                                  // recording (host only, used for menu)
        wantDemo = false;
    }
    ...
    if (clientNet != NULL && wantDemo) {
        ...
        CDemoRecorder* recorder =
          new CDemoRecorder(gameSetup->mapName, gameSetup->modName, false);
        recorder->WriteSetupText(gameData->GetSetupText());
        recorder->SaveToDemo(
          packet->data, packet->length, clientNet->GetPacketTime(gs->frameNum));
