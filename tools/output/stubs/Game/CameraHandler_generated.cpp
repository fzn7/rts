...
CreateGlobalCams()
{
    ...
    for (unsigned int i = CCamera::CAMTYPE_PLAYER; i < CCamera::CAMTYPE_ACTIVE;
         i++) {
        CCamera::SetCamera(i, new CCamera(i));
    }

    CCamera::SetCamera(CCamera::CAMTYPE_ACTIVE,
                       CCamera::GetCamera(CCamera::CAMTYPE_PLAYER));
}
...
RemoveGlobalCams()
{
    ...
    for (unsigned int i = CCamera::CAMTYPE_PLAYER; i < CCamera::CAMTYPE_ACTIVE;
         i++) {
        delete CCamera::GetCamera(i);
        CCamera::SetCamera(i, nullptr);
    }
                                 ...
                                 bool wnEdgeMove)
{
    ...
    // We have to update transition both before and after updating the
    // controller: before: if the controller makes a new begincam, it needs to
    // take into account previous transitions after: apply changes made by the
    // controller with 0 time transition.
    UpdateTransition();
                                 ...
                                 bool edgeMove)
{
    ...
    if (wheelMove) {
        ...
        mouseWheelDist +=
          -float(camera->GetMovState()[CCamera::MOVE_STATE_DWN]);
...
CCameraHandler::UpdateTransition()
{
    ...
    const float timeRatio = (camTransState.timeEnd - transTime) /
                            (camTransState.timeEnd - camTransState.timeStart);
    const float tweenFact =
      1.0f - math::pow(timeRatio, camTransState.timeExponent);

    camera->SetPos(
      mix(camTransState.startPos, camTransState.tweenPos, tweenFact));
    camera->SetRot(
      mix(camTransState.startRot, camTransState.tweenRot, tweenFact));
    camera->SetVFOV(
      mix(camTransState.startFOV, camTransState.tweenFOV, tweenFact));
    camera->Update();
}
...
CCameraHandler::PushAction(const Action& action)
{
    ...
    if (cmd == "viewfps") {
        ...
        COverheadController* taCam = dynamic_cast<COverheadController*>(
          camControllers[CAMERA_MODE_OVERHEAD]);
...
CCameraHandler::LoadViewData(const ViewData& vd)
{
    ...
    if (it != vd.end()) {
        ...
        if (camMode != currCamCtrlNum) {
            CameraTransition(1.0f);
            camControllers[currCamCtrlNum = camMode]->SwitchTo(
              curMode, camMode != curMode);
        }
    }
