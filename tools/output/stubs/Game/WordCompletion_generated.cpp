...
CWordCompletion::CreateInstance()
{
    if (singleton == NULL) {
        ...
        throw std::logic_error(
          "WordCompletion singleton is already initialized");
    }
}
...
CWordCompletion::DestroyInstance()
{
    if (singleton != NULL) {
        ...
        throw std::logic_error("WordCompletion singleton was not initialized "
                         ...
                         bool miniMap)
{
    if (!word.empty()) {
        if (words.find(word) != words.end()) {
            LOG_SL("WordCompletion",
                   L_DEBUG,
                   "Tried to add already present word: %s",
                   word.c_str());
            return;
        }
