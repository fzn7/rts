...
CLoadScreen::Init()
{
    ...
    // user override
    mtLoading = (mtCfg > 0);
    // runtime detect. disable for intel/mesa drivers, they crash at
    // multithreaded OpenGL (date: Nov. 2011)
    mtLoading |=
      (mtCfg < 0) && !globalRendering->haveMesa && !globalRendering->haveIntel;
    showMessages = configHandler->GetBool("ShowLoadMessages");
#endif

    //! Create a thread during the loading that pings the host/server, so it
    //! knows that this client is still alive/loading
    clientNet->KeepUpdating(true);

    netHeartbeatThread = new boost::thread();
    *netHeartbeatThread =
      Threading::CreateNewThread(boost::bind<void, CNetProtocol, CNetProtocol*>(
        &CNetProtocol::UpdateLoop, clientNet));
    ...
    try {
        ...
        if (mtLoading) {
            ...
            gameLoadThread = new COffscreenGLThread(
              boost::bind(&CGame::LoadGame, game, mapName, true));
        }
...
CLoadScreen::~CLoadScreen()
{
    ...
    if (!gu->globalQuit) {
        //! sending your playername to the server indicates that you are
        //! finished loading
        const CPlayer* p = playerHandler->Player(gu->myPlayerNum);
        clientNet->Send(
          CBaseNetProtocol::Get().SendPlayerName(gu->myPlayerNum, p->name));
#ifdef SYNCCHECK
        clientNet->Send(CBaseNetProtocol::Get().SendPathCheckSum(
          gu->myPlayerNum, pathManager->GetPathCheckSum()));
#endif
        mouse->ShowMouse();
...
CLoadScreen::Update()
{
    ...
    if (!mtLoading) {
        // without this call the window manager would think the window is
        // unresponsive and thus asks for hard kill
...
CLoadScreen::Draw()
{
    //! Limit the Frames Per Second to not lock a singlethreaded CPU from
    //! loading the game
    if (mtLoading) {
    ...
    if (LuaIntro != nullptr) {
        ...
        if (showMessages) {
            ...
            font->glPrint(0.1f,
                          0.9f,
                          globalRendering->viewSizeY / 35.0f,
                          FONT_NORM,
                          oldLoadMessages);

            font->SetTextColor(0.9f, 0.9f, 0.9f, 0.9f);
            float posy = font->GetTextNumLines(oldLoadMessages) *
                         font->GetLineHeight() * globalRendering->viewSizeY /
                         35.0f;
            font->glPrint(0.1f,
                          0.9f - posy * globalRendering->pixelY,
                          globalRendering->viewSizeY / 35.0f,
                          FONT_NORM,
                          curLoadMessage);
            font->End();
    ...
    font->glFormat(0.5f,
                   0.06f,
                   globalRendering->viewSizeY / 35.0f,
                   FONT_OUTLINE | FONT_CENTER | FONT_NORM,
                   "Spring %s",
                   SpringVersion::GetFull().c_str());
    font->glFormat(0.5f,
                   0.02f,
                   globalRendering->viewSizeY / 50.0f,
                   FONT_OUTLINE | FONT_CENTER | FONT_NORM,
                   "This program is distributed under the GNU General Public "
                   "License, see license.html for more info");
    font->End();
...
CLoadScreen::SetLoadMessage(const std::string& text, bool replace_lastline)
{
    ...
    //! some external libraries which get linked during loading might reset
    //! those. Here it is done for the loading thread, for the mainthread it is
    //! done in CLoadScreen::Update()
    good_fpu_control_registers(curLoadMessage.c_str());
...
{
    std::vector<std::string> prefPics =
      std::move(CFileHandler::FindFiles(dir, prefix + "*"));
...
CLoadScreen::LoadStartPicture(const std::string& name)
{
    ...
    if (!bm.Load(name)) {
        LOG_L(L_WARNING,
              "[%s] could not load \"%s\" (wrong format?)",
              __FUNCTION__,
              name.c_str());
        return;
    }
    ...
    if ((bm.xsize > globalRendering->viewSizeX) ||
        (bm.ysize > globalRendering->viewSizeY)) {
