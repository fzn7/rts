...
CWaitCommandsAI::Update()
{
    ...
    while (it != waitMap.end()) {
        // grab an incremented iterator in case the active iterator deletes
        // itself
...
void
CWaitCommandsAI::DrawCommands() const
{
...
void
CWaitCommandsAI::AddTimeWait(const Command& cmd)
{
...
void
CWaitCommandsAI::AddDeathWait(const Command& cmd)
{
    InsertWaitObject(DeathWait::New(cmd));
}

void
CWaitCommandsAI::AddSquadWait(const Command& cmd)
{
    InsertWaitObject(SquadWait::New(cmd));
}

void
CWaitCommandsAI::AddGatherWait(const Command& cmd)
{
    InsertWaitObject(GatherWait::New(cmd));
}

void
CWaitCommandsAI::AcknowledgeCommand(const Command& cmd)
{
...
void
CWaitCommandsAI::AddLocalUnit(CUnit* unit, const CUnit* builder)
{
...
void
CWaitCommandsAI::RemoveWaitCommand(CUnit* unit, const Command& cmd)
{
    if ((cmd.params.size() != 2) || (unit->team != gu->myTeam)) {
        return;
    }
    const KeyType key = Wait::GetKeyFromFloat(cmd.params[1]);
...
void
CWaitCommandsAI::ClearUnitQueue(CUnit* unit, const CCommandQueue& queue)
{
    if ((unit->team != gu->myTeam) || waitMap.empty()) {
        return;
    }
...
bool
CWaitCommandsAI::InsertWaitObject(Wait* wait)
{
    if (wait == NULL) {
        return false;
    }
...
void
CWaitCommandsAI::RemoveWaitObject(Wait* wait)
{
...
void
CWaitCommandsAI::AddIcon(const Command& cmd, const float3& pos) const
{
...
// static
CWaitCommandsAI::KeyType
CWaitCommandsAI::Wait::GetNewKey()
{
...
// static
CWaitCommandsAI::KeyType
CWaitCommandsAI::Wait::GetKeyFromFloat(float f)
{
    return *((KeyType*)&f);
}

void
CWaitCommandsAI::Wait::PostLoad()
{
...
// static
float
CWaitCommandsAI::Wait::GetFloatFromKey(KeyType k)
{
    return *((float*)&k);
}
...
CWaitCommandsAI::Wait::GetWaitState(const CUnit* unit) const
{
    ...
    if ((cmd.GetID() == CMD_WAIT) && (cmd.params.size() == 2) &&
        (cmd.params[0] == code) && (GetKeyFromFloat(cmd.params[1]) == key)) {
        return Active;
    }
...
bool
CWaitCommandsAI::Wait::IsWaitingOn(const CUnit* unit) const
{
    const CCommandQueue& dq = unit->commandAI->commandQue;
    if (dq.empty()) {
        return false;
...
void
CWaitCommandsAI::Wait::SendCommand(const Command& cmd, const CUnitSet& unitSet)
{
    if (unitSet.empty()) {
        return;
    }
...
void
CWaitCommandsAI::Wait::SendWaitCommand(const CUnitSet& unitSet)
{
...
CUnitSet::iterator
CWaitCommandsAI::Wait::RemoveUnitFromSet(CUnitSet::iterator it,
                                         CUnitSet& unitSet)
{
...
void
CWaitCommandsAI::TimeWait::DependentDied(CObject* object)
{
    unit = NULL;
}

void
CWaitCommandsAI::TimeWait::AddUnit(CUnit* unit)
{
    // do nothing
}

void
CWaitCommandsAI::TimeWait::RemoveUnit(CUnit* _unit)
{
    if (_unit == unit) {
        delete this;
        return;
...
void
CWaitCommandsAI::TimeWait::Update()
{
    if (unit == NULL) {
        delete this;
        return;
...
const string&
CWaitCommandsAI::TimeWait::GetStateText() const
{
    static char buf[32];
    if (enabled) {
        const int remaining =
...
void
CWaitCommandsAI::TimeWait::Draw() const
{
    // do nothing
}
...
void
CWaitCommandsAI::DeathWait::DependentDied(CObject* object)
{
...
void
CWaitCommandsAI::DeathWait::AddUnit(CUnit* unit)
{
...
void
CWaitCommandsAI::DeathWait::RemoveUnit(CUnit* unit)
{
...
void
CWaitCommandsAI::DeathWait::Update()
{
    if (waitUnits.empty()) {
        delete this;
        return;
...
void
CWaitCommandsAI::DeathWait::Draw() const
{
    if (unitPos.empty()) {
        return;
    }
...
void
CWaitCommandsAI::DeathWait::AddUnitPosition(const float3& pos)
{
    unitPos.push_back(pos);
}

void
CWaitCommandsAI::DeathWait::SelectAreaUnits(const float3& pos0,
                                            const float3& pos1,
                                            CUnitSet& units,
                                            bool enemies)
{
...
void
CWaitCommandsAI::SquadWait::DependentDied(CObject* object)
{
...
void
CWaitCommandsAI::SquadWait::AddUnit(CUnit* unit)
{
    if (waitUnits.insert(unit).second)
        AddDeathDependence(unit, DEPENDENCE_WAITCMD);
}
...
void
CWaitCommandsAI::SquadWait::RemoveUnit(CUnit* unit)
{
...
void
CWaitCommandsAI::SquadWait::Update()
{
    if (buildUnits.empty() && ((int)waitUnits.size() < squadCount)) {
...
void
CWaitCommandsAI::SquadWait::UpdateText()
{
    static char buf[64];
...
void
CWaitCommandsAI::SquadWait::Draw() const
{
    // do nothing
}
...
void
CWaitCommandsAI::GatherWait::DependentDied(CObject* object)
{
    waitUnits.erase(static_cast<CUnit*>(object));
}

void
CWaitCommandsAI::GatherWait::AddUnit(CUnit* unit)
{
    // do nothing
}

void
CWaitCommandsAI::GatherWait::RemoveUnit(CUnit* unit)
{
    if (waitUnits.erase(unit))
        DeleteDeathDependence(unit, DEPENDENCE_WAITCMD);
}
...
void
CWaitCommandsAI::GatherWait::Update()
{
    if (waitUnits.size() < 2) {
        delete this;
        return;
