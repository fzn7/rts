                              ...
                              const float expDistanceMod)
{
    ...
    const float rawImpulseScale =
      damages.impulseFactor * expDistanceMod * impulseDmgMult;

    return Clamp(
      rawImpulseScale, -MAX_EXPLOSION_IMPULSE, MAX_EXPLOSION_IMPULSE);
}
                               ...
                               const int projectileID)
{
    ...
    const float3& lhpPos = (lhp != NULL && vol == lhp->GetCollisionVolume())
                             ? lhp->GetAbsolutePos()
                             : ZeroVector;
    const float3& volPos = vol->GetWorldSpacePos(unit, lhpPos);

    // linear damage falloff with distance
    const float expDist = (expRadius != 0.0f)
                            ? vol->GetPointSurfaceDistance(unit, lhp, expPos)
                            : 0.0f;
    ...
    // expEdgeEffect should be in [0, 1], so expRadius >= expDist >=
    // expDist*expEdgeEffect
    ...
    if (expDist < (expSpeed * DIRECT_EXPLOSION_DAMAGE_SPEED_SCALE)) {
        // damage directly
        unit->DoDamage(
          expDamages, expImpulse, owner, weaponDefID, projectileID);
    } else {
        // damage later
        waitingDamageLists[(gs->frameNum + int(expDist / expSpeed) - 3) & 127]
          .emplace_back((owner ? owner->id : -1),
                        unit->id,
                        expDamages,
                        expImpulse,
                        weaponDefID,
                               ...
                               const int projectileID)
{
    ...
    const float expDist =
      (expRadius != 0.0f) ? vol->GetPointSurfaceDistance(feature, NULL, expPos)
                          : 0.0f;
    const float expRim = expDist * expEdgeEffect;
    ...
    feature->DoDamage(
      damages * expDistanceMod, expImpulse, owner, weaponDefID, projectileID);
}
                                            ...
                                            const int weaponDefID)
{
    ...
    quadField->GetUnitsAndFeaturesColVol(
      params.pos, expRad, unitCache, featureCache);
    ...
    for (unsigned int n = oldNumFeatures; n < newNumFeatures; n++)
        DoExplosionDamage(featureCache[n],
                          params.owner,
                          params.pos,
                          expRad,
                          params.edgeEffectiveness,
...
CGameHelper::Explosion(const CExplosionParams& params)
{
    ...
    const int weaponDefID = (weaponDef != NULL)
                              ? weaponDef->id
                              : -CSolidObject::DAMAGE_EXPLOSION_DEBRIS;
    const int explosionID = (weaponDef != NULL)
                              ? weaponDef->impactExplosionGeneratorID
    ...
    const bool noGfx = eventHandler.Explosion(
      weaponDefID, params.projectileID, params.pos, params.owner);
    ...
    if (params.impactOnly) {
        if (params.hitUnit != NULL) {
            DoExplosionDamage(params.hitUnit,
                              params.owner,
                              params.pos,
                              0.0f,
                              params.explosionSpeed,
        ...
        if (params.hitFeature != NULL) {
            DoExplosionDamage(params.hitFeature,
                              params.owner,
                              params.pos,
                              0.0f,
                              params.edgeEffectiveness,
        ...
        if (altitude >= -1.0f) {
            if (params.damageGround && !mapDamage->disabled &&
                (craterAOE > altitude) && (damages.craterMult > 0.0f)) {
                // limit the depth somewhat
                const float craterDepth =
                  damages.GetDefault() * (1.0f - (altitude / craterAOE));
                const float damageDepth =
                  std::min(craterAOE * 10.0f, craterDepth);
                const float craterStrength =
                  (damageDepth + damages.craterBoost) * damages.craterMult;
    ...
    if (!noGfx) {
        explGenHandler->GenExplosion(explosionID,
                                     params.pos,
                                     params.dir,
                                     damages.GetDefault(),
                                     damageAOE,
    ...
    if (weaponDef != NULL) {
        ...
        const unsigned int soundFlags =
          CCustomExplosionGenerator::GetFlagsFromHeight(params.pos.y,
                                                        realHeight);
        const unsigned int soundMask =
          CCustomExplosionGenerator::SPW_WATER |
          CCustomExplosionGenerator::SPW_UNDERWATER;

        const int soundNum = ((soundFlags & soundMask) != 0);
        const int soundID = soundSet.getID(soundNum);

        if (soundID > 0) {
            Channels::Battle->PlaySample(
              soundID, params.pos, soundSet.getVolume(soundNum));
        }
    }
}
...
struct Friendly : public Base
{
  public:
    ...
    {}
    bool Team(int allyTeam)
    {
        return teamHandler->Ally(searchAllyteam, allyTeam);
    }
...
struct Enemy : public Base
{
  public:
    ...
    bool Unit(const CUnit* unit)
    {
        return (unit != excludeUnit && !unit->IsNeutral());
    }
...
struct ClosestUnit_InLos : public Base
{
  ...
  public:
    ...
    void AddUnit(CUnit* u)
    {
        ...
        if (dist <= closeDist && (canBeBlind || (u->losRadius > dist))) {
            closeDist = dist;
            closeUnit = u;
        }
    }
...
struct ClosestUnit_InLos_Cylinder : public ClosestUnit
{
    ...
    ClosestUnit_InLos_Cylinder(const float3& pos,
                               float searchRadius,
                               bool canBeBlind)
      : ClosestUnit(pos, searchRadius)
      , canBeBlind(canBeBlind)
...
void
CGameHelper::GenerateWeaponTargets(const CWeapon* weapon,
                                   ...
                                   std::multimap<float, CUnit*>& targets)
{
    ...
    const CUnit* lastAttacker = ((owner->lastAttackFrame + 200) <= gs->frameNum)
                                  ? owner->lastAttacker
                                  : nullptr;
    ...
    const float secDamage = weapon->damages->GetDefault() * weapon->salvoSize /
                            weapon->reloadTime * GAME_SPEED;
    const bool paralyzer = (weapon->damages->paralyzeDamageTime != 0);

    // copy on purpose since the below calls lua
    const std::vector<int> quads = quadField->GetQuads(
      pos,
      radius +
        (aHeight - std::max(0.0f, readMap->GetInitMinHeight())) * heightMod);
    const int tempNum = gs->GetTempNum();
    ...
    for (int t = 0; t < teamHandler->ActiveAllyTeams(); ++t) {
        ...
        for (const int qi : quads) {
            const std::vector<CUnit*>& allyTeamUnits =
              quadField->GetQuad(qi).teamUnits[t];
            ...
            for (CUnit* targetUnit : allyTeamUnits) {
                ...
                float3 targPos;
                const unsigned short targetLOSState =
                  targetUnit->losStatus[owner->allyteam];
                ...
                const float modRange =
                  radius + (aHeight - targPos.y) * heightMod;

                if (pos.SqDistance2D(targPos) > modRange * modRange)
                    continue;

                const float dist2D = (pos - targPos).Length2D();
                const float rangeMul = (dist2D * weaponDef->proximityPriority +
                                        modRange * 0.4f + 100.0f);
                const float damageMul =
                  weapon->damages->Get(targetUnit->armorType) *
                  targetUnit->curArmorMultiple;
                ...
                if (targetLOSState & LOS_INLOS) {
                    ...
                    if (paralyzer &&
                        targetUnit->paralyzeDamage >
                          (modInfo.paralyzeOnMaxHealth ? targetUnit->maxHealth
                                                       : targetUnit->health))
                        targetPriority *= 4.0f;
                ...
                const bool allow =
                  eventHandler.AllowWeaponTarget(owner->id,
                                                 targetUnit->id,
                                                 weapon->weaponNum,
                                                 weaponDef->id,
                                                 &targetPriority);
                ...
                targets.insert(
                  std::pair<float, CUnit*>(targetPriority, targetUnit));
            }
        }
    }
...
#ifdef TRACE_SYNC
    {
        tracefile << "[GenerateWeaponTargets] ownerID, attackRadius: "
                  << owner->id << ", " << radius << " ";

        for (std::multimap<float, CUnit*>::const_iterator ti = targets.begin();
             ti != targets.end();
             ++ti)
            tracefile << "\tpriority: " << (ti->first)
                      << ", targetID: " << (ti->second)->id << " ";
                       ...
                       CUnit* excludeUnit)
{
    ...
    const std::vector<CUnit*> units =
      quadField->GetUnitsExact(pos, radius + SQUARE_SIZE, spherical);
    ...
    for (std::vector<CUnit*>::const_iterator ui = units.begin();
        ...
        const bool allied = (teamHandler->Ally(uAllyTeamId, allyTeamId) ||
                             teamHandler->Ally(allyTeamId, uAllyTeamId));

        if ((u != excludeUnit) && allied &&
            ((!u->unitDef->pushResistant && !u->UsingScriptMoveType()) ||
             forced)) {
...
CGameHelper::Pos2BuildPos(const BuildInfo& buildInfo, bool synced)
{
    ...
    if (buildInfo.GetXSize() & 2)
        pos.x = math::floor((buildInfo.pos.x) / (HALFMAP_SQ)) * HALFMAP_SQ +
                SQUARE_SIZE;
    else
        pos.x = math::floor((buildInfo.pos.x + SQUARE_SIZE) / (HALFMAP_SQ)) *
                HALFMAP_SQ;

    if (buildInfo.GetZSize() & 2)
        pos.z = math::floor((buildInfo.pos.z) / (HALFMAP_SQ)) * HALFMAP_SQ +
                SQUARE_SIZE;
    else
        pos.z = math::floor((buildInfo.pos.z + SQUARE_SIZE) / (HALFMAP_SQ)) *
                HALFMAP_SQ;
...
static bool
SearchOffsetComparator(const SearchOffset& a, const SearchOffset& b)
{
...
GetSearchOffsetTable(int radius)
{
    ...
    if (size > searchOffsets.size()) {
        ...
        std::sort(
          searchOffsets.begin(), searchOffsets.end(), SearchOffsetComparator);
    }
                              ...
                              int facing)
{
    ...
    for (int so = 0; so < endr * endr * 4; so++) {
        ...
        if (CGameHelper::TestUnitBuildSquare(bi, feature, allyTeam, false) &&
            (!feature || feature->allyteam != allyTeam)) {
            ...
            for (int z2 = z2Min; z2 < z2Max; ++z2) {
                for (int x2 = x2Min; x2 < x2Max; ++x2) {
                    CSolidObject* solObj =
                      groundBlockingObjectMap->GroundBlockedUnsafe(
                        z2 * mapDims.mapx + x2);

                    if (solObj && solObj->immobile &&
                        !dynamic_cast<CFeature*>(solObj)) {
                        good = false;
                        break;
                    }
                }
            ...
            if (good) {
                ...
                z2Max =
                  std::min(mapDims.mapy, zs + (zsize + 1) / 2 + minDist + 2);
                x2Min = std::max(0, xs - (xsize) / 2 - minDist - 2);
                x2Max =
                  std::min(mapDims.mapx, xs + (xsize + 1) / 2 + minDist + 2);
                            ...
                            bool synced)
{
    ...
    for (int x = x1; x <= x2; x++) {
        for (int z = z1; z <= z2; z++) {
            ...
            if (minHgt < (sqMinHgt - maxDifHgt)) {
                minHgt = sqMinHgt - maxDifHgt;
            }
            if (maxHgt > (sqMaxHgt + maxDifHgt)) {
                maxHgt = sqMaxHgt + maxDifHgt;
            }
        }
    }
                                 ...
                                 const std::vector<Command>* commands)
{
    ...
    const MoveDef* moveDef =
      (buildInfo.def->pathType != -1U)
        ? moveDefHandler->GetMoveDefByPathType(buildInfo.def->pathType)
        : NULL;
    /*const S3DModel* model =*/buildInfo.def->LoadModel();

    const float buildHeight = GetBuildHeight(pos, buildInfo.def, synced);
    // const float modelHeight = (model != NULL) ? math::fabs(model->height)
    // : 10.0f;
    ...
    if (buildInfo.def->needGeo) {
        ...
        const std::vector<CFeature*>& features =
          quadField->GetFeaturesExact(pos, std::max(xsize, zsize) * 6);
    ...
    if (commands != NULL) {
        ...
        for (int z = z1; z < z2; z++) {
            for (int x = x1; x < x2; x++) {
                const float3 bpos(
                  x * SQUARE_SIZE, buildHeight, z * SQUARE_SIZE);
                BuildSquareStatus tbs =
                  (bpos.IsInBounds())
                    ? TestBuildSquare(bpos,
                                      xrange,
                                      zrange,
                                      buildInfo.def,
                                      moveDef,
                                      feature,
                                      gu->myAllyTeam,
                                      buildInfo.def->buildingMask,
                                      synced)
                    : BUILDSQUARE_BLOCKED;

                if (tbs != BUILDSQUARE_BLOCKED) {
                    // test if build-position overlaps a queued command
                    for (std::vector<Command>::const_iterator ci =
                           commands->begin();
                         ci != commands->end();
                         ++ci) {
                        BuildInfo bc(*ci);
                        if (std::max(bc.pos.x - bpos.x - SQUARE_SIZE,
                                     bpos.x - bc.pos.x) *
                                2 <
                              bc.GetXSize() * SQUARE_SIZE &&
                            std::max(bc.pos.z - bpos.z - SQUARE_SIZE,
                                     bpos.z - bc.pos.z) *
                                2 <
                              bc.GetZSize() * SQUARE_SIZE) {
        ...
        // out of map?
        if (unsigned(x1) > mapDims.mapx || unsigned(x2) > mapDims.mapx ||
            unsigned(z1) > mapDims.mapy || unsigned(z2) > mapDims.mapy)
        ...
        for (int z = z1; z < z2; z++) {
            for (int x = x1; x < x2; x++) {
                canBuild = std::min(
                  canBuild,
                  TestBuildSquare(
                    float3(x * SQUARE_SIZE, buildHeight, z * SQUARE_SIZE),
                    xrange,
                    zrange,
                    buildInfo.def,
                    moveDef,
                             ...
                             bool synced)
{
    ...
    const float groundHeight =
      CGround::GetApproximateHeightUnsafe(sqx, sqz, synced);
    ...
    CSolidObject* so =
      groundBlockingObjectMap->GroundBlocked(yardxpos, yardypos);
    if (so != NULL) {
...
//   fixed: no longer true for mobile units
            ...
            if (moveDef != NULL &&
                CMoveMath::IsNonBlocking(*moveDef, so, NULL)) {
    ...
    for (CUnit* unit : unitHandler->activeUnits) {
        ...
        for (; ci != unit->commandAI->commandQue.end(); ++ci) {
            ...
            if (cmd.GetID() < 0 && cmd.params.size() >= 3) {
                ...
                if (bi.def &&
                    (bi.GetXSize() / 2) * SQUARE_SIZE > math::fabs(tempF1.x) &&
                    (bi.GetZSize() / 2) * SQUARE_SIZE > math::fabs(tempF1.z)) {
                    return cmd;
                }
            }
        }
...
CGameHelper::Update()
{
    ...
    for (const WaitingDamage& wd : wdList) {
        ...
        CUnit* attacker =
          unitHandler->GetUnit(wd.attacker); // null if wd.attacker is -1

        if (attackee != nullptr) {
            attackee->DoDamage(
              wd.damage, wd.impulse, attacker, wd.weaponID, wd.projectileID);
        }
    }
