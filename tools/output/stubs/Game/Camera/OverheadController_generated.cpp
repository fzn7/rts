...
void
COverheadController::KeyMove(float3 move)
{
    if (flipped) {
        move.x = -move.x;
        move.y = -move.y;
...
void
COverheadController::MouseMove(float3 move)
{
    if (flipped) {
        move.x = -move.x;
        move.y = -move.y;
    ...
    pos.x += move.x * pixelSize *
             (1 + KeyInput::GetKeyModState(KMOD_SHIFT) * 3) * scrollSpeed;
    pos.z += move.y * pixelSize *
             (1 + KeyInput::GetKeyModState(KMOD_SHIFT) * 3) * scrollSpeed;
    Update();
}

void
COverheadController::ScreenEdgeMove(float3 move)
{
    KeyMove(move);
}

void
COverheadController::MouseWheelMove(float move)
{
    if (move == 0.0f)
        return;
    ...
    const float shiftSpeed =
      (KeyInput::GetKeyModState(KMOD_SHIFT) ? 3.0f : 1.0f);
    ...
    if (KeyInput::GetKeyModState(KMOD_CTRL)) {
        ...
        if (move < 0.0f) {
            ...
            float newHeight =
              CGround::LineGroundCol(wantedPos, wantedPos + dir * 15000, false);
            ...
            if (KeyInput::GetKeyModState(KMOD_ALT)) {
                ...
                pos.z = mapDims.mapy * SQUARE_SIZE *
                        0.55f; // somewhat longer toward bottom
            } else {
...
COverheadController::Update()
{
    ...
    dir = float3(0.0f,
                 -fastmath::cos(angle),
                 flipped ? fastmath::sin(angle) : -fastmath::sin(angle));
    pixelSize = (camera->GetTanHalfFov() * 2.0f) / globalRendering->viewSizeY *
                height * 2.0f;
}

float3
COverheadController::GetPos() const
{
    float3 cpos = pos - dir * height;
    return cpos;
}
...
void
COverheadController::SetPos(const float3& newPos)
{
    pos = newPos;
    Update();
}
...
float3
COverheadController::SwitchFrom() const
{
    return pos;
}

void
COverheadController::SwitchTo(const int oldCam, const bool showText)
{
    if (showText) {
...
void
COverheadController::GetState(StateMap& sm) const
{
...
bool
COverheadController::SetState(const StateMap& sm)
{
