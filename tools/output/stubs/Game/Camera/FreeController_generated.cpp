...
CFreeController::Update()
{
    ...
    if (gndLock && (autoTilt > 0.0f)) {
        ...
        if (pos.y < (gndHeight + gndOffset + 1.0f)) {
            ...
            const float3 gndNormal =
              CGround::GetSmoothNormal(pos.x, pos.z, false);
    ...
    if (!tracking) {
        if (goForward) {
            ...
            const float3 tmpVel((forwardNoY * vel.x) + (UpVector * vel.y) +
                                (camera->GetRight() * vel.z));
            vel = tmpVel;
        }
    }
    ...
    if (gndOffset < 0.0f) {
        ...
        if (pos.y < minHeight) {
            ...
            if (gndLock) {
                vel.y =
                  min(math::fabs(scrollSpeed), ((minHeight - prevPos.y) / ft));
            } else {
                vel.y = 0.0f;
            }
        }
...
CFreeController::KeyMove(float3 move)
{
    ...
    const float speed =
      (KeyInput::GetKeyModState(KMOD_GUI)) ? 4.0f * scrollSpeed : scrollSpeed;
    const float aspeed =
      (KeyInput::GetKeyModState(KMOD_GUI)) ? 2.0f * tiltSpeed : tiltSpeed;
