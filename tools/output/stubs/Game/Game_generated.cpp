  ...
  , gameOver(false)
{
    ...
    playerRoster.SetSortTypeByCode(
      (PlayerRoster::SortType)configHandler->GetInt("ShowPlayerInfo"));
...
CGame::~CGame()
{
    ...
    SafeDelete(
      saveFile); // ILoadSaveHandler, depends on vfsHandler via ~IArchive
...
{
    {
        ...
        // static initialization is done BEFORE Spring's time-epoch is set, so
        // use notime
        jobDispatcher->AddTimedJob(
          j,
          spring_notime + spring_time(j.startDirect ? 0 : (1000.0f / j.freq)));
    }
...
CGame::LoadMap(const std::string& mapName)
{
...

    {
        ...
        groundBlockingObjectMap =
          new CGroundBlockingObjectMap(mapDims.mapSquares);
...
CGame::LoadDefs()
{
...

    {
        ...
        defsParser = new LuaParser(
          "gamedata/defs.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
...
CGame::PreLoadSimulation()
{
    ...
    smoothGround = new SmoothHeightMesh(
      float3::maxxpos, float3::maxzpos, SQUARE_SIZE * 2, SQUARE_SIZE * 40);

    loadscreen->SetLoadMessage("Creating QuadField & CEGs");
    moveDefHandler = new MoveDefHandler(defsParser);
    quadField = new CQuadField(int2(mapDims.mapx, mapDims.mapy),
                               CQuadField::BASE_QUAD_SIZE);
...

    {
        ...
        for (int pp = 0; pp < playerHandler->ActivePlayers(); pp++) {
            wordCompletion->AddWord(
              playerHandler->Player(pp)->name, false, false, false);
        }
        ...
        // add the available Skirmish AI libraries to word completion, for
        // /aicontrol
        const IAILibraryManager::T_skirmishAIKeys& aiLibs =
          aiLibManager->GetSkirmishAIKeys();
        // add the available Lua AI implementations to word completion, for
        // /aicontrol
        const std::set<std::string>& luaAIShortNames =
        ...
        for (const auto& aiLib : aiLibs) {
            wordCompletion->AddWord(aiLib.GetShortName() + " " +
                                      aiLib.GetVersion() + " ",
                                    false,
                                    false,
                                    false);
        ...
        for (auto fit = featureDefs.cbegin(); fit != featureDefs.cend();
             ++fit) {
...
CGame::LoadSkirmishAIs()
{
    ...
    const CSkirmishAIHandler::ids_t& localAIs =
      skirmishAIHandler.GetSkirmishAIsByPlayer(gu->myPlayerNum);
...
CGame::LoadFinalize()
{
    ...
    {
        loadscreen->SetLoadMessage("[" + std::string(__FUNCTION__) +
                                   "] finalizing PFS");
...
CGame::KillMisc()
{
    ...
    LOG("[%s][2]", __FUNCTION__);
    // delete this first since AI's might call back into sim-components in their
    // dtors this means the simulation *should not* assume the EOH still exists
    // on game exit
    CEngineOutHandler::Destroy();

    // TODO move these to the end of this dtor, once all action-executors are
    // registered by their respective engine sub-parts
...
CGame::KeyPressed(int key, bool isRepeat)
{
    ...
    const CKeyBindings::ActionList& actionList =
      keyBindings->GetActionList(curKeyChain);
...
{
    if (userWriting) {
        if (keyCodes->IsPrintable(k) || k == SDLK_RETURN ||
            k == SDLK_BACKSPACE || k == SDLK_DELETE || k == SDLK_HOME ||
            k == SDLK_END || k == SDLK_RIGHT || k == SDLK_LEFT) {
            return 0;
        }
    }
...
CGame::Update()
{
    ...
    // When video recording do step by step simulation, so each simframe gets a
    // corresponding videoframe
...
CGame::UpdateUnsynced(const spring_time currentTime)
{
    ...
    const float modGameDeltaTimeSecs =
      mix(deltaDrawFrameTime.toMilliSecsf() * 0.001f, 0.01f, skipping);

    lastDrawFrameTime = currentTime;

    globalRendering->lastFrameTime = deltaDrawFrameTime.toMilliSecsf();
    gu->avgFrameTime =
      mix(gu->avgFrameTime, deltaDrawFrameTime.toMilliSecsf(), 0.05f);

    {
        // update game timings
        gu->gameTime += modGameDeltaTimeSecs;
        gu->modGameTime +=
          (modGameDeltaTimeSecs * gs->speedFactor * (1 - gs->paused));
    ...
    if (!gs->paused && !IsLagging() && !gs->PreSimFrame() &&
        !videoCapturing->IsCapturing()) {
        globalRendering->weightedSpeedFactor = 0.001f * gu->simFPS;
        globalRendering->timeOffset =
          (currentTime - lastFrameTime).toMilliSecsf() *
          globalRendering->weightedSpeedFactor;
    } else {
    ...
    if ((currentTime - frameStartTime).toMilliSecsf() >= 1000.0f) {
        globalRendering->FPS =
          (numDrawFrames * 1000.0f) /
          std::max(0.01f, (currentTime - frameStartTime).toMilliSecsf());
    ...
    // set camera
    camHandler->UpdateController(playerHandler->Player(gu->myPlayerNum),
                                 gu->fpsMode,
                                 fullscreenEdgeMove,
                                 windowedEdgeMove);
    ...
    const float unsyncedUpdateDeltaTime =
      (currentTime - lastUnsyncedUpdateTime).toSecsf();

    // always update ExtraTexture & SoundListener with <= 30Hz (even when
    // paused) garbage collection event must also run regularly because of
    // unsynced code
    if (newSimFrame || unsyncedUpdateDeltaTime >= (1.0f / GAME_SPEED)) {
        ...
        sound->UpdateListener(
          camera->GetPos(), camera->GetDir(), camera->GetUp());
    }
...
CGame::Draw()
{
    ...
    if (!globalRendering->active) {
        ...
        // return early if and only if less than 30K milliseconds have passed
        // since last draw-frame so we force render two frames per minute when
        // minimized to clear batches and free memory don't need to mess with
        // globalRendering->active since only mouse-input code depends on it
        if ((currentTimePreDraw - lastDrawFrameTime).toSecsi() < 30)
            return true;
    }

    if (globalRendering->drawdebug) {
        const float deltaFrameTime =
          (currentTimePreUpdate - lastSimFrameTime).toMilliSecsf();
        const float deltaNetPacketProcTime =
          (currentTimePreUpdate - lastNetPacketProcessTime).toMilliSecsf();
        const float deltaReceivedPacketTime =
          (currentTimePreUpdate - lastReceivedNetPacketTime).toMilliSecsf();
        const float deltaSimFramePacketTime =
          (currentTimePreUpdate - lastSimFrameNetPacketTime).toMilliSecsf();
        ...
        static const char* minFmtStr =
          "assert(CTO >= 0.0f) failed (SF=%u : DF=%u : CTO=%f : WSF=%f : "
          "DT=%fms : DLNPPT=%fms | DLRPT=%fms | DSFPT=%fms : NP=%u)";
        static const char* maxFmtStr =
          "assert(CTO <= 1.3f) failed (SF=%u : DF=%u : CTO=%f : WSF=%f : "
          "DT=%fms : DLNPPT=%fms | DLRPT=%fms | DSFPT=%fms : NP=%u)";

        // CTO = MILLISECSF(CT - LSFT) * WSF = MILLISECSF(CT - LSFT) * (SFPS *
        // 0.001) AT 30Hz LHS (MILLISECSF(CT - LSFT)) SHOULD BE ~33ms, RHS
        // SHOULD BE ~0.03
        ...
        if (currTimeOffset < 0.0f)
            LOG_L(L_ERROR,
                  ...
                  globalRendering->weightedSpeedFactor,
                  deltaFrameTime,
                  deltaNetPacketProcTime,
                  deltaReceivedPacketTime,
                  deltaSimFramePacketTime,
                  clientNet->GetNumWaitingServerPackets());
        ...
        if (lastGameFrame == gs->frameNum && currTimeOffset < lastTimeOffset)
            LOG_L(L_ERROR,
                  "assert(CTO >= LTO) failed (SF=%u : DF=%u : CTO=%f : LTO=%f "
                  ": WSF=%f : DT=%fms)",
                  gs->frameNum,
                  globalRendering->drawFrame,
                  currTimeOffset,
    ...
    const spring_time currentFrameDrawTime =
      currentTimePostDraw - currentTimePreDraw;
    gu->avgDrawFrameTime =
      mix(gu->avgDrawFrameTime, currentFrameDrawTime.toMilliSecsf(), 0.05f);

    eventHandler.DbgTimingInfo(
      TIMING_VIDEO, currentTimePreDraw, currentTimePostDraw);
...
CGame::DrawInputReceivers()
{
    ...
    if (!hideInterface) {
        ...
        for (auto ri = inputReceivers.rbegin(); ri != inputReceivers.rend();
             ++ri) {
...
#define INF_FONT_FLAGS                                                         \
    (FONT_RIGHT | FONT_SCALE | FONT_NORM |                                     \
     (FONT_OUTLINE * guihandler->GetOutlineFonts()))

    if (showClock) {
        const int seconds = (gs->frameNum / GAME_SPEED);
        if (seconds < 3600) {
            smallFont->glFormat(0.99f,
                                0.94f,
                                1.0f,
                                INF_FONT_FLAGS,
                                "%02i:%02i",
    ...
    if (showFPS) {
        ...
        smallFont->glFormat(
          0.99f, 0.92f, 1.0f, INF_FONT_FLAGS, "%.0f", globalRendering->FPS);
    }

    if (showSpeed) {
        const float4 speedcol(
          1.0f,
          gs->speedFactor < gs->wantedSpeedFactor * 0.99f ? 0.25f : 1.0f,
          0.25f,
          1.0f);
        smallFont->SetColors(&speedcol, NULL);
        smallFont->glFormat(
          0.99f, 0.90f, 1.0f, INF_FONT_FLAGS, "%2.2f", gs->speedFactor);
    }
...
CGame::DrawInputText()
{
    ...
    // draw the caret
    {
        ...
        const float caretPos =
          fontSize * font->GetTextWidth(caretStr) * globalRendering->pixelX;
        const float caretHeight =
          fontSize * font->GetLineHeight() * globalRendering->pixelY;
        int cpos = writingPos;
        char32_t c = Utf8GetNextChar(userInput, cpos);
        if (c == 0)
            c = ' '; // make caret always visible
        const float cw =
          fontSize * font->GetCharacterWidth(c) * globalRendering->pixelX;
        const float csx = inputTextPosX + caretPos;

        glDisable(GL_TEXTURE_2D);
        const float f =
          0.5f * (1.0f + fastmath::sin(spring_now().toMilliSecsf() * 0.015f));
    ...
    if (!guihandler->GetOutlineFonts()) {
        font->glPrint(inputTextPosX,
                      inputTextPosY,
                      fontSize,
                      FONT_DESCENDER | FONT_NORM,
                      tempstring);
    } else {
        font->glPrint(inputTextPosX,
                      inputTextPosY,
                      fontSize,
                      FONT_DESCENDER | FONT_OUTLINE | FONT_NORM,
                      tempstring);
    }
    font->End();
}
...
CGame::StartPlaying()
{
    ...
    for (int a = 0; a < teamHandler->ActiveTeams(); ++a) {
        ...
        if (!team->HasValidStartPos() &&
            gameSetup->startPosType == CGameSetup::StartPos_ChooseInGame) {
...
CGame::SimFrame()
{
    ...
    if (!skipping) {
        // everything here is unsynced and should ideally moved to
        // Game::Update()
        ...
        (playerHandler->Player(gu->myPlayerNum)->fpsController)
          .SendStateUpdate(/*camera->GetMovState(), mouse->buttons*/);
    ...
    lastSimFrameTime = spring_gettime();
    gu->avgSimFrameTime = mix(gu->avgSimFrameTime,
                              (lastSimFrameTime - lastFrameTime).toMilliSecsf(),
                              0.05f);
...
#ifdef HEADLESS
    {
        const float msecMaxSimFrameTime =
          1000.0f / (GAME_SPEED * gs->wantedSpeedFactor);
        const float msecDifSimFrameTime =
          (lastSimFrameTime - lastFrameTime).toMilliSecsf();
        // multiply by 0.5 to give unsynced code some execution time (50% of our
        // sleep-budget)
        const float msecSleepTime =
          (msecMaxSimFrameTime - msecDifSimFrameTime) * 0.5f;
    ...
    // useful for desync-debugging (enter instead of -1 start & end frame of the
    // range you want to debug)
    DumpState(-1, -1, 1);
...
CGame::GameEnd(const std::vector<unsigned char>& winningAllyTeams, bool timeout)
{
    ...
    if (record != NULL) {
        ...
        clientNet->Send(CBaseNetProtocol::Get().SendPlayerStat(
          gu->myPlayerNum,
          playerHandler->Player(gu->myPlayerNum)->currentStats));
        ...
        for (int i = 0; i < numTeams; ++i) {
            ...
            clientNet->Send(CBaseNetProtocol::Get().SendTeamStat(
              team->teamNum, team->GetCurrentStats()));
        }
    }

    if (!timeout) {
        // pass the winner info to the host in the case it's a dedicated server
        clientNet->Send(CBaseNetProtocol::Get().SendGameOver(gu->myPlayerNum,
                                                             winningAllyTeams));
    } else {
    ...
    if (!s.empty()) {
        ...
        */
        ...
        if (msg.destination == ChatMessage::TO_ALLIES && player) {
            ...
            const bool specsOnly =
              noSpectatorChat && (player && player->spectator);
            if (gu->spectating || !specsOnly) {
                ...
                Channels::UserInterface->PlaySample(chatSound, 5);
                   ...
                   player) { // player -> spectators and spectator -> player PMs
                             // should be forbidden
            // player <-> player and spectator <-> spectator are allowed
            if (msg.destination == gu->myPlayerNum &&
                player->spectator == gu->spectating) {
                LOG("%sPrivate: %s", label.c_str(), s.c_str());
                Channels::UserInterface->PlaySample(chatSound, 5);
            } else if (player->playerNum == gu->myPlayerNum) {
                LOG("You whispered %s: %s",
                    playerHandler->Player(msg.destination)->name.c_str(),
                    s.c_str());
            }
        }
    }
...
CGame::ReloadCOB(const string& msg, int player)
{
    ...
    if (oldScript == NULL) {
        LOG_L(L_WARNING,
              "Unknown COB script for unit \"%s\": %s",
              unitName.c_str(),
    ...
    if (newScript == NULL) {
        LOG_L(L_WARNING,
              "Could not load COB script for unit \"%s\" from: %s",
              unitName.c_str(),
              udef->scriptName.c_str());
        return;
    }
...
CGame::SaveGame(const std::string& filename, bool overwrite, bool usecreg)
{
    if (FileSystem::CreateDirectory("Saves")) {
        if (overwrite || !FileSystem::FileExists(filename)) {
            ...
            LOG_L(L_WARNING,
                  "File %s already exists(use /save -y to override)",
                  filename.c_str());
        }
    }
}
...
CGame::ReloadGame()
{
    if (saveFile) {
        ...
        // Inside the Load call-in, Lua can ensure old units are wiped before
        // new ones are placed.
        saveFile->LoadGame();
    } else {
        LOG_L(L_WARNING,
              "We can only reload the game when it has been started from a "
              "savegame");
    }
}
...
CGame::ProcessKeyPressAction(unsigned int key, const Action& action)
{
    ...
    if (action.command == "pastetext") {
        ...
        if (action.rawline.length() > pastecommand.length()) {
            userInput.insert(writingPos,
                             action.rawline.substr(pastecommand.length(),
                                                   action.rawline.length() -
                                                     pastecommand.length()));
    ...
    if (action.command.find("edit_") == 0) {
        if (action.command == "edit_return") {
            ...
            if (chatting) {
                ...
                if ((userInput.find_first_of("aAsS") == 0) &&
                    (userInput[1] == ':')) {
        ...
        if (action.command ==
            "edit_prev_word") { // TODO don't seems to work correctly with utf-8
            // prev word
        ...
        if (action.command ==
            "edit_next_word") { // TODO don't seems to work correctly with utf-8
    ...
    if (action.command.find("chatswitch") == 0) {
        ...
        if (action.command == "chatswitchally") {
            if ((userInput.find_first_of("aA") == 0) && (userInput[1] == ':')) {
                ...
                writingPos = std::max(0, writingPos - 2);
                userInputPrefix = "";
            } else if ((userInput.find_first_of("sS") == 0) &&
                       (userInput[1] == ':')) {
        ...
        if (action.command == "chatswitchspec") {
            if ((userInput.find_first_of("sS") == 0) && (userInput[1] == ':')) {
                ...
                writingPos = std::max(0, writingPos - 2);
                userInputPrefix = "";
            } else if ((userInput.find_first_of("aA") == 0) &&
                       (userInput[1] == ':')) {
...
{
    const ISyncedActionExecutor* executor =
      syncedGameCommands->GetActionExecutor(action.command);
...
{
    const IUnsyncedActionExecutor* executor =
    ...
    const std::set<std::string>& serverCommands =
      CGameServer::GetCommandBlackList();
