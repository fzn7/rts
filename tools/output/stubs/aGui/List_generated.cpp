...
List::AddItem(const std::string& name, const std::string& description)
{
    ...
    float w = itemFontScale * font->GetSize() * font->GetTextWidth(name) *
                screensize[0] +
              2 * itemSpacing;
    if (w > (size[0])) {
        // box.x1 = 0.5f - 0.5f * w;
...
{
    return std::max(
      1.0f, (size[1] - 2.0f * borderSpacing) / (itemHeight + itemSpacing));
}
...
List::ScaleFactor()
{
    return (float)std::max(1, globalRendering->winSizeY) *
           (size[1] - 2.0f * borderSpacing);
}
...
List::MouseUpdate(int x, int y)
{
    ...
    GuiElement b;
    b.SetPos(pos[0] + borderSpacing,
             pos[1] + size[1] - borderSpacing - itemHeight);
    b.SetSize(size[0] - 2.0f * borderSpacing -
                ((scrollbar.GetSize()[0] < 0) ? 0 : (itemHeight + itemSpacing)),
              itemHeight);
    ...
    for (/*ii = items.begin()*/;
         ii != filteredItems->end() && nDrawOffset < numDisplay;
         ++ii) {
        if (b.MouseOver(mx, my)) {
            if (nCurIndex == place &&
                (clickedTime + spring_msecs(250)) > spring_now()) {
                FinishSelection();
            }
            clickedTime = spring_now();
            place = nCurIndex;
    ...
    if (nDrawOffset < filteredItems->size()) {
        if (scrollbar.MouseOver(mx, my)) {
            ...
            b.SetPos(sbX + (size[0] - 2.0f * borderSpacing) -
...
List::DrawSelf()
{
    ...
    for (/*ii = items.begin()*/;
        ...
        font->glPrint(pos[0] + borderSpacing + 0.002f,
                      b.GetMidY() - hf * 0.15f,
                      itemFontScale,
                      FONT_BASELINE | FONT_SHADOW | FONT_SCALE | FONT_NORM,
                      *ii);
    ...
    if (nDrawOffset < filteredItems->size()) {
        ...
        float sbHeight = sbY1 - sbY2;
        float sbSize =
          ((float)nDrawOffset / (float)filteredItems->size()) * sbHeight;

        if (activeScrollbar) {
            topIndex = std::max(
              0,
              std::min((int)(((float)filteredItems->size() *
                              ((sbY1 - sbSize) -
                               (my - std::min(scrollbarGrabPos, sbSize))) /
                              sbHeight) +
                             0.5f),
                       (int)filteredItems->size() - numDisplay));
        }
        ...
        scrollbar.SetPos(
          ...
          sbY1 - sbSize -
            ((float)topIndex / (float)filteredItems->size()) * sbHeight);
...
{
    place = std::max(
      0, std::min(place + NumDisplay(), int(filteredItems->size()) - 1));
}
...
List::Filter(bool reset)
{
    ...
    std::vector<std::string>* destination =
      filteredItems == &temp1 ? &temp2 : &temp1;
    destination->clear();

    if (reset)
        filteredItems = &items; // reset filter

    for (std::vector<std::string>::const_iterator it = filteredItems->begin();
         it != filteredItems->end();
         ++it) {
