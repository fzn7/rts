...
HorizontalLayout::GeometryChangeSelf()
{
    ...
    for (ChildList::const_iterator it = children.begin(); it != children.end();
         ++it) {
        if ((*it)->SizeFixed()) {
            numFixed++;
    ...
    const float hspacePerObject =
      (size[0] - float(weightedObjects - 1) * itemSpacing - 2 * borderSpacing -
       totalFixedSize) /
      float(weightedObjects - numFixed);
    float startX = pos[0] + borderSpacing;
    for (ChildList::iterator i = children.begin(); i != children.end(); ++i) {
        (*i)->SetPos(startX, pos[1] + borderSpacing);
        if ((*i)->SizeFixed()) {
            (*i)->SetSize(
              (*i)->GetSize()[0], size[1] - 2.0f * borderSpacing, true);
            startX += (*i)->GetSize()[0] + itemSpacing;
        } else {
            (*i)->SetSize(hspacePerObject * float((*i)->Weight()),
