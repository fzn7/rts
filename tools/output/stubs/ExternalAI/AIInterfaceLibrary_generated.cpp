  ...
  int engineVersionNumber) const
{
    ...
    //   return
    //   sAIInterfaceLibrary.getLevelOfSupportFor(engineVersionString.c_str(),
    //   engineVersionNumber);
...
CAIInterfaceLibrary::GetSkirmishAICount() const
{
    ...
    if (sAIInterfaceLibrary.listSkirmishAILibraries != nullptr)
        numLookupSkirmishAIs =
          sAIInterfaceLibrary.listSkirmishAILibraries(interfaceId);
...
CAIInterfaceLibrary::GetSkirmishAIInfos(int aiIndex) const
{
    ...
    const int numLookupInfos =
      sAIInterfaceLibrary.listSkirmishAILibraryInfos(interfaceId, aiIndex);

    for (int i = 0; i < numLookupInfos; ++i) {
        const char* key = sAIInterfaceLibrary.listSkirmishAILibraryInfoKey(
          interfaceId, aiIndex, i);
        const char* value = sAIInterfaceLibrary.listSkirmishAILibraryInfoValue(
          interfaceId, aiIndex, i);
...
CAIInterfaceLibrary::GetSkirmishAIOptions(int aiIndex) const
{
    ...
    const char* rawLuaOptions =
      sAIInterfaceLibrary.listSkirmishAILibraryOptions(interfaceId, aiIndex);
...
CAIInterfaceLibrary::GetLoadCount() const
{
    ...
    for (auto salc = skirmishAILoadCount.cbegin();
         salc != skirmishAILoadCount.cend();
         ++salc) {
  ...
  const CSkirmishAILibraryInfo& aiInfo)
{
    ...
    if (skirmishAILoadCount[skirmishAIKey] == 0) {
        ...
        if (sLib == nullptr) {
            ...
            loadedSkirmishAILibraries[skirmishAIKey] =
              CSkirmishAILibrary(EmptyInterfaceLib(), skirmishAIKey);
        } else {
            loadedSkirmishAILibraries[skirmishAIKey] =
              CSkirmishAILibrary(*sLib, skirmishAIKey);
        }
    }
...
CAIInterfaceLibrary::InitializeFromLib(const std::string& libFilePath)
{
    ...
    sAIInterfaceLibrary.initStatic = (int(CALLING_CONV_FUNC_POINTER*)(
      int interfaceId, const SAIInterfaceCallback* callback))funcAddr;
    ...
    sAIInterfaceLibrary.releaseStatic =
      (int(CALLING_CONV_FUNC_POINTER*)())funcAddr;
    ...
    /*
            ...
            sAIInterfaceLibrary.getLevelOfSupportFor = (LevelOfSupport
       (CALLING_CONV_FUNC_POINTER *)( const char*, int
            )) funcAddr;

            if (sAIInterfaceLibrary.getLevelOfSupportFor == nullptr) {
                    // do nothing: it is permitted that an AI does not export
       this function
                    //reportInterfaceFunctionError(libFilePath, funcName);
                    //return -3;
            }
    */
    ...
    sAIInterfaceLibrary.loadSkirmishAILibrary = (const SSkirmishAILibrary*(
      CALLING_CONV_FUNC_POINTER*)(const char* const shortName,
                                  const char* const version))funcAddr;
    ...
    sAIInterfaceLibrary.unloadSkirmishAILibrary =
      (int(CALLING_CONV_FUNC_POINTER*)(const char* const shortName,
                                       const char* const version))funcAddr;
    ...
    funcAddr = nullptr;
    sAIInterfaceLibrary.listSkirmishAILibraryOptions = (const char*(
      CALLING_CONV_FUNC_POINTER*)(int interfaceId, int aiIndex))funcAddr;
