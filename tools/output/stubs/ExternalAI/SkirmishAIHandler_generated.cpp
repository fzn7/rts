...
CSkirmishAIHandler::LoadPreGame()
{
    ...
    const std::vector<std::vector<InfoItem>>& luaAIImpls =
...
CSkirmishAIHandler::GetSkirmishAI(const std::string& name) const
{
    ...
    bool found = false;
    for (id_ai_t::const_iterator ai = id_ai.begin();
         ai != id_ai.end() && !found;
         ++ai) {
                                         ...
                                         const int hostPlayerId)
{
    ...
    for (auto ai = id_ai.cbegin(); ai != id_ai.cend(); ++ai) {
        if ((ai->second.team == teamId) &&
            ((hostPlayerId < 0) || (ai->second.hostPlayer == hostPlayerId))) {
...
CSkirmishAIHandler::CreateLocalSkirmishAI(const size_t skirmishAIId)
{
    ...
    assert(team_localAIsInCreation.find(aiData->team) ==
           team_localAIsInCreation.end());
...
CSkirmishAIHandler::CreateLocalSkirmishAI(const SkirmishAIData& aiData)
{
    ...
    clientNet->Send(CBaseNetProtocol::Get().SendAICreated(
      aiData.hostPlayer, unspecified_skirmishAIId, aiData.team, aiData.name));
}

const SkirmishAIData*
CSkirmishAIHandler::GetLocalSkirmishAIInCreation(const int teamId) const
{
    return (team_localAIsInCreation.find(teamId) !=
            team_localAIsInCreation.end())
             ? &(team_localAIsInCreation.find(teamId)->second)
             : nullptr;
}
                                             ...
                                             const int reason)
{
    ...
    clientNet->Send(CBaseNetProtocol::Get().SendAIStateChanged(
      gu->myPlayerNum, skirmishAIId, SKIRMAISTATE_DIEING));
}
...
CSkirmishAIHandler::GetLocalSkirmishAILibraryKey(const size_t skirmishAIId)
{
    ...
    const SkirmishAIKey& resKey = aiLibManager->ResolveSkirmishAIKey(
...
CSkirmishAIHandler::CompleteWithDefaultOptionValues(const size_t skirmishAIId)
{
    ...
    const IAILibraryManager::T_skirmishAIInfos& aiInfos =
      aiLibMan->GetSkirmishAIInfos();
