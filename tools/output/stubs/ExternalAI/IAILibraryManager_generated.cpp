...
IAILibraryManager::OutputAIInterfacesInfo()
{
    ...
    const T_interfaceSpecs& keys = myLibManager->GetInterfaceKeys();
  ...
  const SkirmishAIKey& skirmishAIKey) const
{
    ...
    if (!fittingKeys.empty()) {
        ...
        for (size_t k = 1; k < fittingKeys.size(); ++k) {
            if (IAILibraryManager::VersionCompare(fittingKeys[k].GetVersion(),
                                                  *bestVersion) > 0) {
                bestIndex = k;
...
IAILibraryManager::OutputSkirmishAIInfo()
{
    ...
    for (T_dupSkirm::const_iterator info = duplicateSkirmishAIInfos.begin();
         info != duplicateSkirmishAIInfos.end();
         ++info) {
...
{
    const std::vector<std::string>& parts1 =
      CSimpleParser::Split(version1, ".");
    const std::vector<std::string>& parts2 =
      CSimpleParser::Split(version2, ".");
    unsigned int maxParts =
      parts1.size() > parts2.size() ? parts1.size() : parts2.size();
    ...
    int sign = (diff != 0) | -(int)((unsigned int)((int)diff) >>
