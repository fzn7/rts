  ...
  , dieing(false)
{
    ...
    teamId = aiData->team;
    key = aiLibManager->ResolveSkirmishAIKey(
      SkirmishAIKey(aiData->shortName, aiData->version));
...
CSkirmishAIWrapper::CreateCallback()
{
    ...
    sCallback = skirmishAiCallback_getInstanceFor(
      skirmishAIId, teamId, callback.get(), cheats.get());
}
...
{
    {
        ...
        if (library == nullptr) {
            ...
            skirmishAIHandler.SetLocalSkirmishAIDieing(
              skirmishAIId, 5 /* = AI failed to init */);
        } else {
    ...
    for (size_t a = 0; a < unitHandler->MaxUnits(); a++) {
        ...
        if (unit->team == teamId) {
            ...
            if (teamHandler->Ally(teamHandler->AllyTeam(teamId),
                                  unit->allyteam))
                continue;

            if (unit->losStatus[teamHandler->AllyTeam(teamId)] &
                (LOS_INRADAR | LOS_INLOS)) {
                EnemyEnterRadar(a);
            }
...
CSkirmishAIWrapper::Init()
{
    ...
    // init failed
    LOG_L(L_ERROR,
          "Failed to handle init event: AI for team %d, error %d",
          teamId,
          error);
...
createTempFileName(const char* action, int teamId, int skirmishAIId)
{
    ...
    return (dataDirsAccess.LocateFile(
      tmpFileName, FileQueryFlags::WRITE | FileQueryFlags::CREATE_DIRS));
}
...
CSkirmishAIWrapper::Load(std::istream* loadStream)
{
    const std::string tmpFile =
      createTempFileName("load", teamId, skirmishAIId);
...
{
    const std::string tmpFile =
      createTempFileName("save", teamId, skirmishAIId);
...
{
    float3 cpyDir = dir;
    const SUnitDamagedEvent evtData = { unitId,     attackerUnitId, damage,
                                        &cpyDir[0], weaponDefId,    paralyzer };
...
{
    float3 cpyDir = dir;
    const SEnemyDamagedEvent evtData = {
        enemyUnitId, attackerUnitId, damage, &cpyDir[0], weaponDefId, paralyzer
    };
  ...
  int playerId)
{
    ...
    const SPlayerCommandEvent evtData = { &unitIds[0],
                                          static_cast<int>(
                                            playerSelectedUnits.size()),
                                          cCommandId,
                                          playerId };
