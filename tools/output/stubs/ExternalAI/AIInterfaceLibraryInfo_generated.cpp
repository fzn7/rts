  ...
  const std::string& interfaceInfoFile)
{
    ...
    for (ii = tmpInfo.begin(); ii != tmpInfo.end(); ++ii) {
        // TODO remove this, once we support non-string value types for AI
        // Interface info
...
CAIInterfaceLibraryInfo::IsLookupSupported() const
{
    ...
    const std::string& lookupSupportedStr =
      GetInfo(AI_INTERFACE_PROPERTY_SUPPORTS_LOOKUP);
...
CAIInterfaceLibraryInfo::GetInfo(const std::string& key) const
{
    ...
    if (!found) {
        LOG_L(L_WARNING,
              "AI Interface property '%s' could not be found.",
              key.c_str());
        return DEFAULT_VALUE;
    } else {
                                 ...
                                 const std::string& description)
{
    ...
    static const std::string snKey =
      StringToLower(AI_INTERFACE_PROPERTY_SHORT_NAME);
    static const std::string vKey =
      StringToLower(AI_INTERFACE_PROPERTY_VERSION);
