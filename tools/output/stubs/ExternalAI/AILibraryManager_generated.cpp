...
CAILibraryManager::GatherInterfaceLibrariesInfos()
{
    ...
    for (auto dir = aiInterfaceDataDirs.cbegin();
         dir != aiInterfaceDataDirs.cend();
         ++dir) {
        const std::string& possibleDataDir = *dir;
        const T_dirs infoFiles =
        ...
        CAIInterfaceLibraryInfo interfaceInfo =
          CAIInterfaceLibraryInfo(infoFile);

        interfaceInfo.SetDataDir(
          FileSystem::EnsureNoPathSepAtEnd(possibleDataDir));
        interfaceInfo.SetDataDirCommon(FileSystem::GetParent(possibleDataDir) +
                                       "common");
    ...
    for (auto info = duplicateInterfaceInfoCheck.cbegin();
         info != duplicateInterfaceInfoCheck.cend();
         ++info) {
...
CAILibraryManager::GatherSkirmishAIsLibrariesInfos()
{
    ...
    GatherSkirmishAIsLibrariesInfosFromInterfaceLibrary(
      duplicateSkirmishAIInfoCheck);
  ...
  const std::string& sourceDesc)
{
    ...
    AIInterfaceKey interfaceKey =
      FindFittingInterfaceSpecifier(skirmishAIInfo.GetInterfaceShortName(),
  ...
  T_dupSkirm duplicateSkirmishAIInfoCheck)
{
    ...
    T_dirs skirmishAIDataDirs =
      dataDirsAccess.FindDirsInDirectSubDirs(SKIRMISH_AI_DATA_DIR);

    for (auto dir = skirmishAIDataDirs.cbegin();
         dir != skirmishAIDataDirs.cend();
         ++dir) {
        const std::string& possibleDataDir = *dir;
        const T_dirs infoFiles =
          CFileHandler::FindFiles(possibleDataDir, "AIInfo.lua");
        ...
        const T_dirs optionFile =
          CFileHandler::FindFiles(possibleDataDir, "AIOptions.lua");
        ...
        CSkirmishAILibraryInfo skirmishAIInfo =
          CSkirmishAILibraryInfo(infoFile, optionFileName);

        skirmishAIInfo.SetDataDir(
          FileSystem::EnsureNoPathSepAtEnd(possibleDataDir));
        skirmishAIInfo.SetDataDirCommon(FileSystem::GetParent(possibleDataDir) +
                                        "common");

        StoreSkirmishAILibraryInfos(
          duplicateSkirmishAIInfoCheck, skirmishAIInfo, infoFile);
    }
}
  ...
  T_dupSkirm duplicateSkirmishAIInfoCheck)
{
    ...
    for (auto intInfIt = intInfs.begin(); intInfIt != intInfs.end();
         ++intInfIt) {
        ...
        const CAIInterfaceLibrary* intLib =
          FetchInterface((intInfIt->second).GetKey());
        const int aiCount = intLib->GetSkirmishAICount();

        for (int aii = 0; aii < aiCount; ++aii) {
            const std::map<std::string, std::string>& rawInfos =
              intLib->GetSkirmishAIInfos(aii);
            const std::string& rawLuaOptions =
              intLib->GetSkirmishAIOptions(aii);
            ...
            CSkirmishAILibraryInfo skirmishAIInfo =
              CSkirmishAILibraryInfo(rawInfos, rawLuaOptions);

            StoreSkirmishAILibraryInfos(duplicateSkirmishAIInfoCheck,
                                        skirmishAIInfo,
                                        intInfIt->first.ToString());
        }
    }
}
  ...
  T_dupSkirm duplicateSkirmishAIInfoCheck)
{
    ...
    for (auto info = duplicateSkirmishAIInfoCheck.cbegin();
         info != duplicateSkirmishAIInfoCheck.cend();
         ++info) {
        if (info->second.size() < 2)
            continue;
  ...
  const SkirmishAIKey& skirmishAIKey) const
{
    ...
    for (auto sasi = skirmishAIKeys.cbegin(); sasi != skirmishAIKeys.cend();
         ++sasi) {
...
CAILibraryManager::FetchSkirmishAILibrary(const SkirmishAIKey& skirmishAIKey)
{
    ...
    if (aiInfo == skirmishAIInfos.end()) {
        ...
        CAIInterfaceLibrary* interfaceLib =
          FetchInterface(skirmishAIKey.GetInterface());
...
{
    for (auto lil = loadedAIInterfaceLibraries.cbegin();
         lil != loadedAIInterfaceLibraries.cend();
         ++lil) {
...
CAILibraryManager::FetchInterface(const AIInterfaceKey& interfaceKey)
{
    ...
    std::unique_ptr<CAIInterfaceLibrary>& ptr =
      loadedAIInterfaceLibraries[interfaceKey];
  ...
  const IAILibraryManager::T_interfaceSpecs& keys)
{
    ...
    for (key = keys.begin(); key != keys.end(); ++key) {
        if (shortName == key->GetShortName()) {
            int diff =
              IAILibraryManager::VersionCompare(key->GetVersion(), minVersion);
