...
#endif
{
    ...
    std::random_shuffle(
      freeUnsyncedIDs.begin(), freeUnsyncedIDs.end(), gu->rng);
...
CProjectileHandler::UpdateProjectileContainer(ProjectileContainer& pc,
                                              bool synced)
{
    // WARNING: we can't use iters here cause ProjectileCreated and
    // ProjectileDestroyed events may add new projectiles to the container!
    ...
    // WARNING: we can't use iters here cause p->Update() may add new
    // projectiles to the container!
...
{
    {
        ...
        // particles
        CheckCollisions(); // before :Update() to check if the particles move
                           // into stuff
...
CProjectileHandler::AddProjectile(CProjectile* p)
{
    ...
    if ((p->id) > (1 << 24)) {
        LOG_L(L_WARNING,
              "Lua %s projectile IDs are now out of range",
    ...
    if (p->synced) {
        ASSERT_SYNCED(p->id);
#ifdef TRACE_SYNC
        tracefile << "New projectile id: " << p->id
                  << ", ownerID: " << p->GetOwnerID();
        tracefile << ", type: " << p->GetProjectileType() << " pos: <"
    ...
    for (CUnit* unit : tempUnits) {
        ...
        if (teamHandler->IsValidAllyTeam(p->GetAllyteamID())) {
            if (p->GetCollisionFlags() & Collision::NOFRIENDLIES) {
                if (teamHandler->AlliedAllyTeams(p->GetAllyteamID(),
                                                 unit->allyteam)) {
                    continue;
                }
            }
        ...
        if (CCollisionHandler::DetectHit(
              unit, unit->GetTransformMatrix(true), ppos0, ppos1, &cq)) {
                                           ...
                                           const float3 ppos1)
{
    ...
    for (CFeature* feature : tempFeatures) {
        ...
        if (!feature->HasCollidableStateBit(
              CSolidObject::CSTATE_BIT_PROJECTILES))
            continue;

        if (CCollisionHandler::DetectHit(
              feature, feature->GetTransformMatrix(true), ppos0, ppos1, &cq)) {
            if (cq.GetHitPiece() != NULL) {
  ...
  const float3 ppos1)
{
    ...
    for (CPlasmaRepulser* repulser : tempRepulsers) {
        ...
        if (!repulser->CanIntercept(wdef->interceptedByShieldType,
                                    p->GetAllyteamID()))
            continue;

        if (CCollisionHandler::DetectHit(
              repulser->owner,
              &repulser->collisionVolume,
              repulser->owner->GetTransformMatrix(true),
              ppos0,
              ppos1,
              &cq)) {
            if (!cq.InsideHit() || !repulser->weaponDef->exteriorShield ||
                repulser->IsRepulsing(wpro)) {
...
CProjectileHandler::CheckUnitFeatureCollisions(ProjectileContainer& pc)
{
    ...
    for (size_t i = 0; i < pc.size(); ++i) {
        ...
        quadField->GetUnitsAndFeaturesColVol(p->pos,
                                             p->radius + p->speed.w,
                                             tempUnits,
                                             tempFeatures,
                                             &tempRepulsers);
...
CProjectileHandler::CheckCollisions()
{
    ...
    CheckUnitFeatureCollisions(
      unsyncedProjectiles); //! does not change simulation state
...
{
    flyingPieces[modelType].emplace_back(
      piece, m, pos, speed, pieceParams, renderParams);
                                    ...
                                    bool highPriority)
{
    ...
    new CNanoProjectile(
      startPos, dif, int(l), colors[globalRendering->teamNanospray]);
}
                                    ...
                                    bool highPriority)
{
    ...
    if (!inverse) {
        new CNanoProjectile(startPos,
                            (dif + error) * 3,
                            int(l / 3),
                            colors[globalRendering->teamNanospray]);
    } else {
        new CNanoProjectile(startPos + (dif + error) * l,
                            -(dif + error) * 3,
                            int(l / 3),
                            colors[globalRendering->teamNanospray]);
    }
...
{
    if (UNSYNCED_PROJ_NOEVENT)
        return nullptr; // unsynced projectiles have no IDs if
                        // UNSYNCED_PROJ_NOEVENT
...
CProjectileHandler::GetParticleSaturation(const bool withRandomization) const
{
    ...
    // i.e. when there are rockets that spam CEGs this gives smaller CEGs still
    // a chance
    return (GetCurrentParticles() / float(maxParticles)) *
           (1.f + int(withRandomization) * 0.3f * gu->RandFloat());
}

int
CProjectileHandler::GetCurrentParticles() const
{
    // use precached part of particles count calculation that else becomes very
    // heavy example where it matters: (in ZK) /cheat /give 20 armraven -> shoot
    // ground
    int partCount = lastCurrentParticles;
    for (size_t i = lastSyncedProjectilesCount, e = syncedProjectiles.size();
         i < e;
         ++i) {
        partCount += syncedProjectiles[i]->GetProjectilesCount();
    }
    for (size_t i = lastUnsyncedProjectilesCount,
                e = unsyncedProjectiles.size();
         i < e;
         ++i) {
