...
CFlareProjectile::Update()
{
    ...
    if (gs->frameNum >= activateFrame) {
        ...
        if (owner &&
            lastSub < (gs->frameNum - owner->unitDef->flareSalvoDelay) &&
            numSub < owner->unitDef->flareSalvoSize) {
...
CFlareProjectile::Draw()
{
    ...
    unsigned char col[4];
    float alpha =
      std::max(0.0f, 1 - (gs->frameNum - activateFrame) * alphaFalloff);
    ...
    for (int a = 0; a < numSub; ++a) {
        ...
        const float3 interPos =
          subPos[a] + subSpeed[a] * globalRendering->timeOffset;

#define fpt projectileDrawer->flareprojectiletex
        va->AddVertexQTC(interPos - camera->GetRight() * rad -
                           camera->GetUp() * rad,
                         fpt->xstart,
                         fpt->ystart,
                         col);
