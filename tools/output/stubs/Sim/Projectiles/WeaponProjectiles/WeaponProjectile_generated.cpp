  ...
  , targetPos(params.end)
{
    ...
    if ((po = dynamic_cast<CWeaponProjectile*>(target)) != NULL) {
        po->SetBeingIntercepted(po->IsBeingIntercepted() ||
                                weaponDef->interceptSolo);
    ...
    if (params.owner == NULL) {
        ...
        allyteamID =
          teamHandler->IsValidTeam(teamID) ? teamHandler->AllyTeam(teamID) : -1;
    }
    ...
    if (ownerID != -1u && weaponNum != -1u) {
        ...
        if (owner != nullptr && weaponNum < owner->weapons.size()) {
            damages =
              DynDamageArray::IncRef(owner->weapons[weaponNum]->damages);
        }
    }
    if (damages == nullptr)
                           ...
                           float3 impactDir)
{
    const DamageArray& damageArray =
      damages->GetDynamicDamages(startPos, impactPos);
...
CWeaponProjectile::UpdateInterception()
{
    ...
    if (hitscan) {
        if (ClosestPointOnLine(startPos, targetPos, po->pos)
              .SqDistance(po->pos) < Square(weaponDef->collisionSize)) {
            po->Collision();
            Collision();
        }
    } else {
        // FIXME: if (pos.SqDistance(po->pos) <
        // Square(weaponDef->collisionSize)) {
...
CWeaponProjectile::UpdateGroundBounce()
{
    ...
    float3 normal;
    bool bounced = false;
    const float distWaterHit =
      (pos.y > 0.0f && speed.y < 0.0f) ? (pos.y / -speed.y) : -1.0f;
    const bool intersectWater =
      (distWaterHit >= 0.0f) && (distWaterHit <= 1.0f);
    if (intersectWater && weaponDef->waterBounce) {
        ...
        const float distGroundHit = CGround::LineGroundCol(
          pos, pos + speed); // TODO use traj one for traj weapons?
    ...
    explGenHandler->GenExplosion(weaponDef->bounceExplosionGeneratorID,
                                 pos,
                                 normal,
                                 speed.w,
                                 1.0f,
    ...
    CWorldObject::SetVelocity(speed - (speed + normal * dot) *
                                        (1 - weaponDef->bounceSlip));
    CWorldObject::SetVelocity(speed +
                              normal * dot * (1 + weaponDef->bounceRebound));
...
{
    return ((pos - startPos).SqLength() >
            (weaponDef->range * weaponDef->range));
}
