  ...
  , curTracerPart(0)
{
    ...
    for (unsigned int a = 0; a < NUM_TRACER_PARTS; ++a) {
        ...
        tracerParts[a].numAgeMods = std::min(
          MAX_NUM_AGEMODS,
          static_cast<unsigned int>((speed.w + 0.6f) / TRACER_PARTS_STEP));
    }
    castShadow = true;
...
CStarburstProjectile::Collision()
{
    if (weaponDef->visuals.smokeTrail) {
        new CSmokeTrailProjectile(owner(),
                                  pos,
                                  oldSmoke,
                                  dir,
                                  oldSmokeDir,
...
CStarburstProjectile::Update()
{
    ...
    if (ttl > 0) {
        explGenHandler->GenExplosion(
          cegID, pos, dir, ttl, damages->damageAreaOfEffect, 0.0f, NULL, NULL);
    }

    {
        const unsigned int newTracerPart =
          (curTracerPart + 1) % NUM_TRACER_PARTS;
        ...
        for (float aa = 0; aa < speed.w + 0.6f && newsize < MAX_NUM_AGEMODS;
             aa += TRACER_PARTS_STEP, ++newsize) {
            const float ageMod =
              (missileAge < 20) ? 1.0f : (0.6f + (rand() * 0.8f) / RAND_MAX);
    ...
    if (weaponDef->visuals.smokeTrail) {
        ...
        if ((age % 8) == 0) {
            smokeTrail = new CSmokeTrailProjectile(owner(),
                                                   pos,
                                                   oldSmoke,
                                                   dir,
                                                   oldSmokeDir,
...
CStarburstProjectile::UpdateTrajectory()
{
    if (uptime > 0) {
        ...
        CWorldObject::SetVelocity(
          dir * speed.w); // do not need to update dir or speed.w here
        ...
        if (targetErrorVec.dot(dir) > 0.99f) {
            ...
            if (weaponDef->turnrate != 0) {
                dir =
                  (dir + (targetErrorVec * weaponDef->turnrate)).Normalize();
            } else {
        ...
        SetDirectionAndSpeed((dir + (UpVector * mygravity)).Normalize(),
                             speed.w - mygravity);
    }
...
CStarburstProjectile::Draw()
{
    ...
    for (unsigned int a = 0; a < NUM_TRACER_PARTS; ++a) {
        ...
        for (int num = 0; num < numAgeMods; aa += TRACER_PARTS_STEP, ++num) {
            ...
            va->AddVertexTC(interPos - camera->GetRight() * drawsize -
                              camera->GetUp() * drawsize,
                            wt3->xstart,
                            wt3->ystart,
                            col);
            va->AddVertexTC(interPos + camera->GetRight() * drawsize -
    ...
    va->AddVertexTC(drawPos - camera->GetRight() * fsize -
                      camera->GetUp() * fsize,
                    wt1->xstart,
                    wt1->ystart,
                    lightRed);
                                    ...
                                    float shieldMaxSpeed)
{
    ...
    if (ttl > 0) {
        ...
        const float tracking =
          std::max(shieldForce * 0.05f, weaponDef->turnrate * 2.0f);
