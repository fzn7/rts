...
CSolidObject::CSolidObject()
  ...
  , collidableState(CollidableState(CSTATE_BIT_SOLIDOBJECTS |
                                    CSTATE_BIT_PROJECTILES |
                                    CSTATE_BIT_QUADMAPRAYS))
  ,
...
void
CSolidObject::UpdatePhysicalState(float eps)
{
...
#define MASK_NOAIR                                                             \
    (PSTATE_BIT_ONGROUND | PSTATE_BIT_INWATER | PSTATE_BIT_UNDERWATER |        \
     PSTATE_BIT_UNDERGROUND)
...
CSolidObject::SetVoidState()
{
    ...
    ClearCollidableStateBit(CSTATE_BIT_SOLIDOBJECTS | CSTATE_BIT_PROJECTILES |
                            CSTATE_BIT_QUADMAPRAYS);
...
void
CSolidObject::UpdateVoidState(bool set)
{
    if (set) {
        SetVoidState();
    } else {
...
void
CSolidObject::SetMass(float newMass)
{
    mass = Clamp(newMass, MINIMUM_MASS, MAXIMUM_MASS);
}

void
CSolidObject::UnBlock()
{
    if (!IsBlocking())
        return;
...
YardMapStatus
CSolidObject::GetGroundBlockingMaskAtPos(float3 gpos) const
{
    if (!blockMap)
        return YARDMAP_OPEN;
    ...
    gpos -= pos;
    gpos.x += SQUARE_SIZE / 2; //??? needed to move to SQUARE-center? (possibly
                               //current input is wrong)
    gpos.z += SQUARE_SIZE / 2;

    frontv = frontdir;
    rightv =
      -rightdir; //??? spring's unit-rightdir is in real the LEFT vector :x
#else
    // use old fixed space (4 facing dirs & ints for unit positions)

    // form the rotated axis vectors
    static const float3 fronts[] = {
        FwdVector, RgtVector, -FwdVector, -RgtVector
    };
    static const float3 rights[] = {
        RgtVector, -FwdVector, -RgtVector, FwdVector
    };
    ...
    // transform worldspace pos to unit rotation dependent `centered blockmap
    // space` [-hxsize .. +hxsize] x [-hzsize .. +hzsize]
    ...
    // transform: [(-hxsize + eps) .. (+hxsize - eps)] x [(-hzsize + eps) ..
    // (+hzsize - eps)] -> [0 .. (xsize - 1)] x [0 .. (zsize - 1)]
    bx += hxsize;
    by += hzsize;
...
int2
CSolidObject::GetMapPos(const float3& position) const
{
    int2 mp;
...
float3
CSolidObject::GetDragAccelerationVec(const float4& params) const
{
    ...
    // other units as normal: mass in kg, speed in elmos/frame, density in
    // kg/m^3
    //
    // params.xyzw map: {{atmosphere, water}Density, {drag,
    // friction}Coefficient}
    //
    const float3 speedSignVec =
      float3(Sign(speed.x), Sign(speed.y), Sign(speed.z));
    const float3 dragScaleVec = float3(
      IsInAir() * dragScales.x *
        (0.5f * params.x * params.z * (M_PI * sqRadius * 0.01f * 0.01f)), // air
      IsInWater() * dragScales.y *
        (0.5f * params.y * params.z *
         (M_PI * sqRadius * 0.01f * 0.01f)),            // water
    ...
    dragAccelVec.x =
      Clamp(dragAccelVec.x, -math::fabs(speed.x), math::fabs(speed.x));
    dragAccelVec.y =
      Clamp(dragAccelVec.y, -math::fabs(speed.y), math::fabs(speed.y));
    dragAccelVec.z =
      Clamp(dragAccelVec.z, -math::fabs(speed.z), math::fabs(speed.z));

    return dragAccelVec;
}

float3
CSolidObject::GetWantedUpDir(bool useGroundNormal) const
{
    // NOTE:
    ...
    //
    const float3 gn =
      CGround::GetSmoothNormal(pos.x, pos.z) * (useGroundNormal);
    ...
    if (!IsInAir()) {
        switch (moveDef->speedModClass) {
            case MoveDef::Tank: {
                return ((gn + wn) * IsOnGround() + updir * (1 - IsOnGround()));
            } break;
            case MoveDef::KBot: {
                return ((gn + wn) * IsOnGround() + updir * (1 - IsOnGround()));
            } break;
...
void
CSolidObject::SetDirVectorsEuler(const float3 angles)
{
    CMatrix44f matrix;
...
void
CSolidObject::SetHeadingFromDirection()
{
    heading = GetHeadingFromVector(frontdir.x, frontdir.z);
}

void
CSolidObject::UpdateDirVectors(bool useGroundNormal)
{
...
void
CSolidObject::ForcedSpin(const float3& newDir)
{
    ...
    const float3 udir = mix(UpVector,
                            (frontdir * Sign(-zdir.y)),
                            (math::fabs(zdir.dot(UpVector)) >= 0.99f));
...
void
CSolidObject::Kill(CUnit* killer, const float3& impulse, bool crushed)
{
    UpdateVoidState(false);

    if (crushed) {
        DoDamage(DamageArray(health + 1.0f),
                 impulse,
                 killer,
                 -DAMAGE_EXTSOURCE_CRUSHED,
                 -1);
    } else {
        DoDamage(DamageArray(health + 1.0f),
                 impulse,
