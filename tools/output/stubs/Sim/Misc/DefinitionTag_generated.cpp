...
DefType::AddMetaData(const DefTagMetaData* data)
{
    ...
    for (const DefTagMetaData* md : tags) {
        if (key == md->GetInternalName()) {
            LOG_VAR(data,
                    "Duplicate config variable declaration \"%s\"",
                    key.c_str());
...
DefType::CheckType(const DefTagMetaData* meta, const std::type_info& want)
{
    ...
    if (meta->GetTypeInfo() != want)
        LOG_L(
          L_ERROR,
          "DEFTAG \"%s\" defined with wrong typevalue \"%s\" should be \"%s\"",
          meta->GetKey().c_str(),
          DefTagMetaData::GetTypeName(meta->GetTypeInfo()).c_str(),
          DefTagMetaData::GetTypeName(want).c_str());
                           ...
                           const std::string pre)
{
    ...
    for (const std::string& tag : keys) {
        ...
        if (luaTable.GetType(tag) == LuaTable::TABLE) {
            ReportUnknownTags(
              instanceName, luaTable.SubTable(tag), pre + tag + ".");
            continue;
        }

        LOG_L(L_WARNING,
              "%s: Unknown tag \"%s%s\" in \"%s\"",
              name.c_str(),
              pre.c_str(),
              tag.c_str(),
              instanceName.c_str());
