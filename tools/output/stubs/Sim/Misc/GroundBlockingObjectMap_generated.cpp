...
CGroundBlockingObjectMap::AddGroundBlockingObject(CSolidObject* object)
{
    ...
    for (int zSqr = minZSqr; zSqr < maxZSqr; zSqr++) {
        for (int xSqr = minXSqr; xSqr < maxXSqr; xSqr++) {
            VectorInsertUnique(
              GetCellUnsafe(xSqr + zSqr * mapDims.mapx), object, true);
        }
    }
    ...
    if (object->moveDef == nullptr && pathManager != nullptr) {
        pathManager->TerrainChange(
          minXSqr, minZSqr, maxXSqr, maxZSqr, TERRAINCHANGE_OBJECT_INSERTED);
    }
}
                                                  ...
                                                  const YardMapStatus& mask)
{
    ...
    for (int z = minZSqr; z < maxZSqr; z++) {
        for (int x = minXSqr; x < maxXSqr; x++) {
            // unit yardmaps always contain sx=UnitDef::xsize *
            // sz=UnitDef::zsize cells (the unit->moveDef footprint can have
            // different dimensions)
            const float3 testPos = float3(x, 0.0f, z) * SQUARE_SIZE;

            if (object->GetGroundBlockingMaskAtPos(testPos) & mask) {
                VectorInsertUnique(
                  GetCellUnsafe(x + (z)*mapDims.mapx), object, true);
            }
        }
    }
    ...
    if (object->moveDef == nullptr && pathManager != nullptr) {
        pathManager->TerrainChange(
          minXSqr, minZSqr, maxXSqr, maxZSqr, TERRAINCHANGE_OBJECT_INSERTED_YM);
    }
}
...
CGroundBlockingObjectMap::RemoveGroundBlockingObject(CSolidObject* object)
{
    ...
    if (object->moveDef == nullptr && pathManager != nullptr) {
        pathManager->TerrainChange(
          bx, bz, bx + sx, bz + sz, TERRAINCHANGE_OBJECT_DELETED);
    }
}
...
{
    for (int z = yardUnit->mapPos.y; z < yardUnit->mapPos.y + yardUnit->zsize;
         ++z) {
        for (int x = yardUnit->mapPos.x;
             x < yardUnit->mapPos.x + yardUnit->xsize;
             ++x) {
            if (yardUnit->GetGroundBlockingMaskAtPos(
                  float3(x * SQUARE_SIZE, 0.0f, z * SQUARE_SIZE)) &
                mask)
