...
CModInfo::Init(const char* modArchive)
{
    ...
    const CArchiveScanner::ArchiveData md =
      archiveScanner->GetArchiveData(humanNameVersioned);
    ...
    LuaParser parser(
      "gamedata/modrules.lua", SPRING_VFS_MOD_BASE, SPRING_VFS_ZIP);
    ...
    if (!parser.IsValid()) {
        LOG_L(L_ERROR,
              "Failed loading mod-rules, using defaults; error: %s",
...

    {
        ...
        pathFinderSystem =
          system.GetInt("pathFinderSystem", PFS_TYPE_DEFAULT) % PFS_NUM_TYPES;
...

    {
        ...
        allowDirectionalPathing =
          movementTbl.GetBool("allowDirectionalPathing", true);
        allowAircraftToLeaveMap =
          movementTbl.GetBool("allowAirPlanesToLeaveMap", true);
        allowAircraftToHitGround =
          movementTbl.GetBool("allowAircraftToHitGround", true);
        allowPushingEnemyUnits =
          movementTbl.GetBool("allowPushingEnemyUnits", false);
        allowCrushingAlliedUnits =
          movementTbl.GetBool("allowCrushingAlliedUnits", false);
        allowUnitCollisionDamage =
          movementTbl.GetBool("allowUnitCollisionDamage", false);
        allowUnitCollisionOverlap =
          movementTbl.GetBool("allowUnitCollisionOverlap", true);
        allowGroundUnitGravity =
          movementTbl.GetBool("allowGroundUnitGravity", true);
        allowHoverUnitStrafing = movementTbl.GetBool(
          "allowHoverUnitStrafing", (pathFinderSystem == PFS_TYPE_QTPFS));
    }

    {
        ...
        constructionDecayTime =
          (int)(constructionTbl.GetFloat("constructionDecayTime", 6.66) *
                GAME_SPEED);
        constructionDecaySpeed = std::max(
          constructionTbl.GetFloat("constructionDecaySpeed", 0.03), 0.01f);
    }

    {
        ...
        reclaimUnitEnergyCostFactor =
          reclaimTbl.GetFloat("unitEnergyCostFactor", 0.0);
        reclaimUnitEfficiency = reclaimTbl.GetFloat("unitEfficiency", 1.0);
        reclaimFeatureEnergyCostFactor =
          reclaimTbl.GetFloat("featureEnergyCostFactor", 0.0);
...

    {
        ...
        resurrectEnergyCostFactor =
          resurrectTbl.GetFloat("energyCostFactor", 0.5);
    }
...

    {
        ...
        targetableTransportedUnits =
          transportTbl.GetBool("targetableTransportedUnits", false);
    }
...

    {
        ...
        featureVisibility =
          featureLOS.GetInt("featureVisibility", FEATURELOS_ALL);
        featureVisibility =
          Clamp(featureVisibility, int(FEATURELOS_NONE), int(FEATURELOS_ALL));
    }

    {
        ...
        requireSonarUnderWater =
          sensors.GetBool("requireSonarUnderWater", true);
        alwaysVisibleOverridesCloaked =
          sensors.GetBool("alwaysVisibleOverridesCloaked", false);
        ...
        // airLosMipLevel doesn't have such restrictions, it's just used in
        // various bitshifts with signed integers
        ...
        if ((losMipLevel < 0) || (losMipLevel > 6)) {
            throw content_error(
              "Sensors\\Los\\LosMipLevel out of bounds. "
              "The minimum value is 0. The maximum value is 6.");
        }

        if ((radarMipLevel < 0) || (radarMipLevel > 6)) {
            throw content_error(
              "Sensors\\Los\\RadarMipLevel out of bounds. "
              "The minimum value is 0. The maximum value is 6.");
        }

        if ((airMipLevel < 0) || (airMipLevel > 30)) {
            throw content_error(
              "Sensors\\Los\\AirLosMipLevel out of bounds. "
