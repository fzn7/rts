                  ...
                  std::vector<float>& mesh)
{
    ...
    for (int x = 0; x <= maxx; ++x) {
        ...
        for (int i = startx; i <= endx; ++i) {
            assert(i >= 0);
            assert(i <= maxx);
            assert(CGround::GetHeightReal(i * resolution, cury) <=
                   colsMaxima[i]);
        ...
        for (float y1 = cury - smoothRadius; y1 <= cury + smoothRadius;
             y1 += resolution) {
            for (float x1 = curx - smoothRadius; x1 <= curx + smoothRadius;
                 x1 += resolution) {
                maxRowHeightAlt = std::max(
                  maxRowHeightAlt, CGround::GetHeightAboveWater(x1, y1));
            }
        }
                   ...
                   std::vector<int>& maximaRows)
{
    ...
    for (int x = 0; x <= maxx; ++x) {
#ifdef _DEBUG
        for (int y1 = std::max(0, y - intrad); y1 <= std::min(maxy, y + intrad);
             ++y1) {
            assert(CGround::GetHeightReal(x * resolution, y1 * resolution) <=
                   colsMaxima[x]);
        }
#endif
        const float curx = x * resolution;
        ...
        if (maximaRows[x] <= (y - intrad)) {
            ...
            for (int y1 = std::max(0, y - intrad + 1);
                 y1 <= std::min(maxy, nextrow);
                 ++y1) {
                const float h =
                  CGround::GetHeightAboveWater(curx, y1 * resolution);
...
#ifdef _DEBUG
        for (int y1 = std::max(0, y - intrad + 1);
             y1 <= std::min(maxy, y + intrad + 1);
             ++y1) {
            assert(colsMaxima[x] >=
                   CGround::GetHeightReal(curx, y1 * resolution));
        }
#endif
    }
}
    ...
    for_mt(0, maxy + 1, [&](const int y) {
        ...
        for (int x = 0; x <= maxx; ++x) {
            ...
            if (x <= smoothrad || x > (maxx - smoothrad)) {
                ...
                const float gh =
                  CGround::GetHeightAboveWater(x * resolution, y * resolution);
                const float sh = smoothed[idx] / (xend - xstart + 1);

                smoothed[idx] =
                  std::min(readMap->GetCurrMaxHeight(), std::max(gh, sh));
            } else {
                // non-border case
    ...
    for_mt(0, maxx + 1, [&](const int x) {
        ...
        for (int y = 0; y <= maxy; ++y) {
            ...
            if (y <= smoothrad || y > (maxy - smoothrad)) {
                ...
                // non-border case
                avg += mesh[x + (y + smoothrad) * lineSize] -
                       mesh[x + (y - smoothrad - 1) * lineSize];
                ...
                const std::vector<int>& maximaRows)
{
    ...
    for (int y1 = std::max(0, y - intrad + 1);
        ...
        for (int x1 = 0; x1 <= maxx; ++x1) {
            assert(CGround::GetHeightReal(x1 * resolution, y1 * resolution) <=
                   colsMaxima[x1]);
        }
    }
}
...
SmoothHeightMesh::MakeSmoothMesh()
{
    ...
    FindMaximumColumnHeights(
      maxx, maxy, intrad, resolution, colsMaxima, maximaRows);

    for (int y = 0; y <= maxy; ++y) {
        AdvanceMaximaRows(y, maxx, resolution, colsMaxima, maximaRows);
        FindRadialMaximum(y, maxx, intrad, resolution, colsMaxima, mesh);
        FixRemainingMaxima(
          y, maxx, maxy, intrad, resolution, colsMaxima, maximaRows);
