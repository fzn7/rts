...
CTeam::SetDefaultStartPos()
{
    ...
    const std::vector<AllyTeam>& allyStartData =
    ...
    const float xmin =
      (mapDims.mapx * SQUARE_SIZE) * allyTeamData.startRectLeft;
    const float zmin = (mapDims.mapy * SQUARE_SIZE) * allyTeamData.startRectTop;
    const float xmax =
      (mapDims.mapx * SQUARE_SIZE) * allyTeamData.startRectRight;
    const float zmax =
      (mapDims.mapy * SQUARE_SIZE) * allyTeamData.startRectBottom;
    ...
    startPos.x =
      (teamNum - teamHandler->ActiveTeams()) * 4 * SQUARE_SIZE + xcenter;
    startPos.z =
      (teamNum - teamHandler->ActiveTeams()) * 4 * SQUARE_SIZE + zcenter;
}
...
CTeam::Died(bool normalDeath)
{
    ...
    if (normalDeath) {
        ...
        clientNet->Send(
          CBaseNetProtocol::Get().SendTeamDied(gu->myPlayerNum, teamNum));
...
{
    for (const auto& id :
         skirmishAIHandler.GetSkirmishAIsInTeam(teamNum, gu->myPlayerNum)) {
...
CTeam::SlowUpdate()
{
    ...
    for (int a = 0; a < teamHandler->ActiveTeams(); ++a) {
        ...
        if ((a != teamNum) &&
            (teamHandler->AllyTeam(teamNum) == teamHandler->AllyTeam(a))) {
            if (team->isDead)
                continue;

            eShare += std::max(
              0.0f, (team->resStorage.energy * 0.99f) - team->res.energy);
            mShare += std::max(
              0.0f, (team->resStorage.metal * 0.99f) - team->res.metal);
        }
    }
    ...
    const float eExcess =
      std::max(0.0f, res.energy - (resStorage.energy * resShare.energy));
    const float mExcess =
      std::max(0.0f, res.metal - (resStorage.metal * resShare.metal));
    ...
    for (int a = 0; a < teamHandler->ActiveTeams(); ++a) {
        if ((a != teamNum) &&
            ...
            const float edif =
              std::max(0.0f,
                       (team->resStorage.energy * 0.99f) - team->res.energy) *
              de;
            const float mdif =
              std::max(0.0f,
                       (team->resStorage.metal * 0.99f) - team->res.metal) *
              dm;
    ...
    assert(
      ((TeamStatistics::statsPeriod * GAME_SPEED) % TEAM_SLOWUPDATE_RATE) == 0);
    ...
    if (nextHistoryEntry <= gs->frameNum) {
        ...
        nextHistoryEntry =
          gs->frameNum + (TeamStatistics::statsPeriod * GAME_SPEED);
...
CTeam::GetControllerName() const
{
    ...
    if (HasLeader()) {
        ...
        if (leadPlayer->team != this->teamNum) {
            const CTeam* realLeadPlayerTeam =
              teamHandler->Team(leadPlayer->team);
            const CPlayer* realLeadPlayer = NULL;

            if (realLeadPlayerTeam->HasLeader()) {
                realLeadPlayer =
                  playerHandler->Player(realLeadPlayerTeam->GetLeader());
        ...
        for (const auto& aiId :
             skirmishAIHandler.GetSkirmishAIsInTeam(this->teamNum)) {
            const SkirmishAIData* aiData =
              skirmishAIHandler.GetSkirmishAI(aiId);
            const std::string prefix =
              "AI: " + aiData->shortName + " " + aiData->version + " ";
