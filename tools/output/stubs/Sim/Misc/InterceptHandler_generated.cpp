    ...
    for (CWeapon* w : interceptors) {
        ...
        for (CWeaponProjectile* p : interceptables) {
            if (!p->CanBeInterceptedBy(wDef))
                continue;
            if (w->incomingProjectiles.find(p->id) !=
                w->incomingProjectiles.end())
                continue;

            const int pAllyTeam = p->GetAllyteamID();

            if (teamHandler->IsValidAllyTeam(pAllyTeam) &&
                teamHandler->Ally(wOwner->allyteam, pAllyTeam))
                continue;

            // note: will be called every Update so long as gadget does not
            // return true
            if (!eventHandler.AllowWeaponInterceptTarget(wOwner, w, p))
                continue;

            // there are four cases when an interceptor <w> should fire at a
            // projectile <p>:
            //     1. p's target position inside w's interception circle (w's
            //     owner can move!)
            //     2. p's current position inside w's interception circle
            //     3. p's projected impact position inside w's interception
            //     circle
            ...
            const float impactDist =
              CGround::LineGroundCol(p->pos, p->pos + p->dir * weaponDist);
            ...
            if (w->aimFromPos.SqDistance2D(pTargetPos) <
                Square(wDef->coverageRange)) {
            ...
            if (false /*wDef->noFlyThroughIntercept*/) {
                // <w> is just a static interceptor and fires only at
                // projectiles TARGETED within its current interception area;
                // any projectiles CROSSING its interception area aren't
                // targeted
            ...
            if (w->aimFromPos.SqDistance2D(pImpactPos) <
                Square(wDef->coverageRange)) {
                const float3 pTargetDir = (pTargetPos - p->pos).SafeNormalize();
                const float3 pImpactDir = (pImpactPos - p->pos).SafeNormalize();

                // the projected impact position can briefly shift into the
                // covered area during transition from vertical to horizontal
                // flight, so we perform an extra test (NOTE: assumes
            ...
            const float3 pMinSepPos =
              p->pos +
              p->dir * Clamp(-(pWeaponVec.dot(p->dir)), 0.0f, impactDist);
...
{
    auto it = std::find(interceptables.begin(),
                        interceptables.end(),
                        static_cast<CWeaponProjectile*>(o));
