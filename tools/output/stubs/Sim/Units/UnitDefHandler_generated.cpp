...
{
    if (std::find_if(unitName.begin(), unitName.end(), isblank) !=
        unitName.end()) {
        LOG_L(L_WARNING,
...
CUnitDefHandler::ProcessDecoys()
{
    ...
    for (mit = decoyNameMap.begin(); mit != decoyNameMap.end(); ++mit) {
        ...
        if ((fakeIt != unitDefIDsByName.end()) &&
            (realIt != unitDefIDsByName.end())) {
    ...
    for (unsigned int i = 0; i < sideParser.GetCount(); i++) {
        ...
        if (!startUnit.empty()) {
            std::map<std::string, int>::iterator it =
              unitDefIDsByName.find(startUnit);
...
CUnitDefHandler::UnitDefLoadSounds(UnitDef* ud, const LuaTable& udTable)
{
    ...
    LoadSounds(soundsTable,
               ud->sounds.select,
               "select"); // eg. "select1", "select2", ...
    LoadSounds(soundsTable,
               ud->sounds.arrived,
               "arrived"); // eg. "arrived1", "arrived2", ...
...
CUnitDefHandler::AssignTechLevel(UnitDef& ud, int level)
{
    ...
    for (bo_it = ud.buildOptions.begin(); bo_it != ud.buildOptions.end();
         ++bo_it) {
        std::map<std::string, int>::const_iterator ud_it =
          unitDefIDsByName.find(bo_it->second);
