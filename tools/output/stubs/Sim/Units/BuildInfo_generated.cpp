...
int
BuildInfo::CreateCommandID() const
{
    return -def->id;
}

void
BuildInfo::AddCommandParams(Command& cmd) const
{
...
bool
BuildInfo::Parse(const Command& c)
{
    if (c.params.size() < 3)
        return false;
...
int
BuildInfo::GetXSize() const
{
    return ((buildFacing & 1) == 0) ? def->xsize : def->zsize;
}

int
BuildInfo::GetZSize() const
{
