...
{
    // Free all cobfiles
    for (std::map<std::string, CCobFile*>::iterator i = cobFiles.begin();
         i != cobFiles.end();
         ++i) {
        delete i->second;
    }
...
CCobEngine::AddThread(CCobThread* thread)
{
    switch (thread->state) {
        ...
        default:
            LOG_L(L_ERROR,
                  "thread added to scheduler with unknown state (%d)",
                  thread->state);
            break;
    }
}
...
CCobEngine::Tick(int deltaTime)
{
    ...
    for (CCobThread* t : running) {
        // LOG_L(L_DEBUG, "Now 1running %d: %s", currentTime,
        // (*i)->GetName().c_str());
        TickThread(t);
    }
    ...
    // The threads that just ran may have added new threads that should run next
    // tick
    ...
    if (!sleeping.empty()) {
        ...
        while ((cur != NULL) && (cur->GetWakeTime() < currentTime)) {
            ...
            // Run forward again. This can quite possibly readd the thread to
            // the sleeping array again  But it will not interfere since it is
            // guaranteed to sleep > 0 ms  LOG_L(L_DEBUG, "Now 2running %d: %s",
            // currentTime, cur->GetName().c_str());
