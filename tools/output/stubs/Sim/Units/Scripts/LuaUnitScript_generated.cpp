...
static inline LocalModelPiece*
ParseLocalModelPiece(lua_State* L, CUnitScript* script, const char* caller)
{
...
static inline int
ToLua(lua_State* L, const float3& v)
{
    ...
    some notes:
    - all piece numbers are 1 based, to be consistent with other parts of
    interface
    ...
    - Because in current design CBCobThreadFinish can impossibly be called,
    certain state changes which normally happen immediately when script returns
    should be triggered through a call to a callOut when using Lua scripts. This
    applies to:
      * Spring.SetUnitShieldState(unitID, false|true) replaces return value 0|1
    of COB's AimWeaponX function for plasma repulsers.
    ...
    - HitByWeaponId takes x,z,weaponDefID,damage instead of
    500z,500x,tdfID,damage
    - HitByWeaponId returns the new damage, instead of a percentage of old
    damage
    - QueryLandingPadCount doesn't exist
    - QueryLandingPad should return an array (table) of all pieces
    - BeginTransport and QueryTransport take unitID instead of
    unit->height*65536, use 'local height = Spring.GetUnitHeight(unitID)' to get
    the height.
    ...
    TurnFinished(number piece, number axis)
            Called after a turn finished for this unit/piece/axis (not a
    turn-now!) Should resume coroutine of the particular thread which called the
    Lua WaitForTurn function (see below).

    MoveFinished(number piece, number axis)
            Called after a move finished for this unit/piece/axis (not a
    move-now!) Should resume coroutine of the particular thread which called the
    Lua WaitForMove function (see below).
    ...
    Spring.UnitScript.Spin(number piece, number axis, number speed[, number
    accel]) -> nil Same as COB's spin.  If accel isn't given spinning starts at
    the desired speed.

    Spring.UnitScript.StopSpin(number piece, number axis[, number decel]) -> nil
            Same as COB's stop-spin.  If decel isn't given spinning stops
    immediately.

    Spring.UnitScript.Turn(number piece, number axis, number destination[,
    number speed]) -> nil Same as COB's turn iff speed is given and not zero,
    and turn-now otherwise.

    Spring.UnitScript.Move(number piece, number axis, number destination[,
    number speed]) -> nil Same as COB's move iff speed is given and not zero,
    and move-now otherwise.
    ...
    Spring.UnitScript.WaitForTurn(number piece, number axis) -> boolean
            Returns true iff such an animation exists, false otherwise.  Iff it
    returns true, the TurnFinished callIn will be called once the turn
    completes.

    Spring.UnitScript.WaitForMove(number piece, number axis) -> boolean
            Returns true iff such an animation exists, false otherwise.  Iff it
    returns true, the MoveFinished callIn will be called once the move
    completes.

    Spring.UnitScript.SetDeathScriptFinished(number wreckLevel])
            Tells Spring the Killed script finished, and which wreckLevel to
    use. If wreckLevel is not given no wreck is created.

    Spring.UnitScript.CreateScript(number unitID, table callIns) -> nil
            Replaces the current unit script (independent of type, also replaces
    COB) with the unit script given by a table of callins for the unit. Callins
    are similar to COB functions, e.g. a number of predefined names are called
    by the engine if they exist in the table.

    Spring.UnitScript.UpdateCallIn(number unitID, string fname[, function
    callIn]) -> number|boolean Iff callIn is a function, a single callIn is
    replaced or added, and the new functionID is returned.  If callIn isn't
    given or is nil, the callIn is nilled, returns true if it was removed, or
    false if the callin didn't exist. See also Spring.UnitScript.CreateScript.
    */
...
CLuaUnitScript::~CLuaUnitScript()
{
    ...
    if (L != NULL) {
        ...
        Destroy();
        for (map<string, int>::iterator it = scriptNames.begin();
             it != scriptNames.end();
             ++it) {
...
void
CLuaUnitScript::HandleFreed(CLuaHandle* handle)
{
    for (auto ui = unitHandler->activeUnits.begin();
         ui != unitHandler->activeUnits.end();
         ++ui) {
...
void
CLuaUnitScript::UpdateCallIn(const string& fname, int ref)
{
    // Map common function names to indices
    int num = CLuaUnitScriptNames::GetScriptNumber(fname);

    // Check upper bound too in case user calls UpdateCallIn with nonexisting
    // weapon. (we only allocate slots in scriptIndex for the number of weapons
    // the unit has)
    ...
    switch (num) {
        case LUAFN_SetSFXOccupy:
            hasSetSFXOccupy = (ref != LUA_NOREF);
            break;
        case LUAFN_RockUnit:
            hasRockUnit = (ref != LUA_NOREF);
            break;
        case LUAFN_StartBuilding:
            hasStartBuilding = (ref != LUA_NOREF);
            break;
    }
...
void
CLuaUnitScript::RemoveCallIn(const string& fname)
{
    map<string, int>::iterator it = scriptNames.find(fname);
...
void
CLuaUnitScript::ShowScriptError(const string& msg)
{
...
bool
CLuaUnitScript::HasBlockShot(int weaponNum) const
{
    return HasFunction(LUAFN_BlockShot);
}

bool
CLuaUnitScript::HasTargetWeight(int weaponNum) const
{
...
inline float
CLuaUnitScript::PopNumber(int fn, float def)
{
...
inline bool
CLuaUnitScript::PopBoolean(int fn, bool def)
{
...
inline void
CLuaUnitScript::RawPushFunction(int functionId)
{
...
inline void
CLuaUnitScript::PushFunction(int id)
{
    RawPushFunction(scriptIndex[id]);
}

inline void
CLuaUnitScript::PushUnit(const CUnit* targetUnit)
{
    if (targetUnit) {
...
inline bool
CLuaUnitScript::RunCallIn(int id, int inArgs, int outArgs)
{
    return RawRunCallIn(scriptIndex[id], inArgs, outArgs);
}

int
CLuaUnitScript::RunQueryCallIn(int fn)
{
    if (!HasFunction(fn))
        return -1;
...
int
CLuaUnitScript::RunQueryCallIn(int fn, float arg1)
{
    if (!HasFunction(fn))
        return -1;
...
void
CLuaUnitScript::Call(int fn, float arg1)
{
    if (!HasFunction(fn))
        return;
...
void
CLuaUnitScript::Call(int fn, float arg1, float arg2)
{
    if (!HasFunction(fn))
        return;
...
void
CLuaUnitScript::Call(int fn, float arg1, float arg2, float arg3)
{
    if (!HasFunction(fn))
        return;
...
CLuaUnitScript::Killed()
{
    ...
    if (lua_israwnumber(L, -1)) {
        ...
        LOG_L(L_ERROR,
              "%s: bad return value, expected number or nil",
              fname.c_str());
        RemoveCallIn(fname);
...
void
CLuaUnitScript::WindChanged(float heading, float speed)
{
    Call(LUAFN_WindChanged, heading, speed);
}

void
CLuaUnitScript::ExtractionRateChanged(float speed)
{
    Call(LUAFN_ExtractionRateChanged, speed);
}

void
CLuaUnitScript::RockUnit(const float3& rockDir)
{
    Call(LUAFN_RockUnit, rockDir.x, rockDir.z);
}

void
CLuaUnitScript::HitByWeapon(const float3& hitDir,
                            int weaponDefId,
                            float& inoutDamage)
{
...
void
CLuaUnitScript::SetSFXOccupy(int curTerrainType)
{
...
void
CLuaUnitScript::QueryLandingPads(std::vector<int>& out_pieces)
{
...
void
CLuaUnitScript::BeginTransport(const CUnit* unit)
{
    Call(LUAFN_BeginTransport, unit->id);
}

int
CLuaUnitScript::QueryTransport(const CUnit* unit)
{
    return RunQueryCallIn(LUAFN_QueryTransport, unit->id);
}

void
CLuaUnitScript::TransportPickup(const CUnit* unit)
{
    Call(LUAFN_TransportPickup, unit->id);
}

void
CLuaUnitScript::TransportDrop(const CUnit* unit, const float3& pos)
{
...
void
CLuaUnitScript::StartBuilding(float heading, float pitch)
{
    Call(LUAFN_StartBuilding, heading, pitch);
}

int
CLuaUnitScript::QueryNanoPiece()
{
    return RunQueryCallIn(LUAFN_QueryNanoPiece);
}

int
CLuaUnitScript::QueryBuildInfo()
{
    return RunQueryCallIn(LUAFN_QueryBuildInfo);
}

int
CLuaUnitScript::QueryWeapon(int weaponNum)
{
    return RunQueryCallIn(LUAFN_QueryWeapon, weaponNum + LUA_WEAPON_BASE_INDEX);
}

void
CLuaUnitScript::AimWeapon(int weaponNum, float heading, float pitch)
{
    Call(LUAFN_AimWeapon, weaponNum + LUA_WEAPON_BASE_INDEX, heading, pitch);
}

void
CLuaUnitScript::AimShieldWeapon(CPlasmaRepulser* weapon)
{
    Call(LUAFN_AimShield, weapon->weaponNum + LUA_WEAPON_BASE_INDEX);
}

int
CLuaUnitScript::AimFromWeapon(int weaponNum)
{
    return RunQueryCallIn(LUAFN_AimFromWeapon,
                          weaponNum + LUA_WEAPON_BASE_INDEX);
}

void
CLuaUnitScript::Shot(int weaponNum)
{
...
bool
CLuaUnitScript::BlockShot(int weaponNum,
...
float
CLuaUnitScript::TargetWeight(int weaponNum, const CUnit* targetUnit)
{
...
void
CLuaUnitScript::AnimFinished(AnimType type, int piece, int axis)
{
...
void
CLuaUnitScript::RawCall(int functionId)
{
    if (functionId < 0) {
        return;
    }
...
string
CLuaUnitScript::GetScriptName(int functionId) const
{
...
bool
CLuaUnitScript::RawRunCallIn(int functionId, int inArgs, int outArgs)
{
...
void
CLuaUnitScript::Destroy()
{
    Call(LUAFN_Destroy);
}
void
CLuaUnitScript::StartMoving(bool reversing)
{
    Call(LUAFN_StartMoving, reversing * 1.0f);
}
void
CLuaUnitScript::StopMoving()
{
    Call(LUAFN_StopMoving);
}
void
CLuaUnitScript::StartUnload()
{
    Call(LUAFN_StartUnload);
}
void
CLuaUnitScript::EndTransport()
{
    Call(LUAFN_EndTransport);
}
void
CLuaUnitScript::StartBuilding()
{
    Call(LUAFN_StartBuilding);
}
void
CLuaUnitScript::StopBuilding()
{
    Call(LUAFN_StopBuilding);
}
void
CLuaUnitScript::Falling()
{
    Call(LUAFN_Falling);
}
void
CLuaUnitScript::Landed()
{
    Call(LUAFN_Landed);
}
void
CLuaUnitScript::Activate()
{
    Call(LUAFN_Activate);
}
void
CLuaUnitScript::Deactivate()
{
    Call(LUAFN_Deactivate);
}
void
CLuaUnitScript::MoveRate(int curRate)
{
    Call(LUAFN_MoveRate, curRate);
}
void
CLuaUnitScript::FireWeapon(int weaponNum)
{
    Call(LUAFN_FireWeapon, weaponNum + LUA_WEAPON_BASE_INDEX);
}
void
CLuaUnitScript::EndBurst(int weaponNum)
{
...
static void
PushEntry(lua_State* L, const char* name, lua_CFunction fun)
{
...
bool
CLuaUnitScript::PushEntries(lua_State* L)
{
    {
...
#define REGISTER_LUA_CFUNC(x) PushEntry(L, #x, x)
...
static inline int
ParseAxis(lua_State* L, const char* caller, int index)
{
    if (!lua_israwnumber(L, index)) {
...
int
CLuaUnitScript::CreateScript(lua_State* L)
{
...
int
CLuaUnitScript::UpdateCallIn(lua_State* L)
{
    CUnit* unit = ParseUnit(L, __FUNCTION__, 1);
    if (unit == NULL) {
        return 0;
    ...
    if (!lua_israwstring(L, 2) ||
        (!lua_isfunction(L, 3) && !lua_isnoneornil(L, 3))) {
...
int
CLuaUnitScript::CallAsUnit(lua_State* L)
{
    CUnit* unit = ParseUnit(L, __FUNCTION__, 1);
    if (unit == NULL) {
        return 0;
...
int
CLuaUnitScript::GetUnitValue(lua_State* L, CUnitScript* script, int arg)
{
...
int
CLuaUnitScript::GetUnitCOBValue(lua_State* L)
{
    CUnit* unit = ParseUnit(L, __FUNCTION__, 1);
...
int
CLuaUnitScript::GetUnitValue(lua_State* L)
{
...
int
CLuaUnitScript::SetUnitValue(lua_State* L, CUnitScript* script, int arg)
{
    const int args = lua_gettop(L) - arg; // number of arguments
    const int val = luaL_checkint(L, arg++);
    int param;
    if (args == 1) {
        param = lua_isboolean(L, arg) ? int(lua_toboolean(L, arg++))
                                      : luaL_checkint(L, arg++);
    } else {
...
int
CLuaUnitScript::SetUnitCOBValue(lua_State* L)
{
    CUnit* unit = ParseUnit(L, __FUNCTION__, 1);
...
int
CLuaUnitScript::SetUnitValue(lua_State* L)
{
    if (activeScript == NULL)
        return 0;
...
int
CLuaUnitScript::SetPieceVisibility(lua_State* L)
{
...
int
CLuaUnitScript::EmitSfx(lua_State* L)
{
...
int
CLuaUnitScript::AttachUnit(lua_State* L)
{
...
int
CLuaUnitScript::DropUnit(lua_State* L)
{
...
int
CLuaUnitScript::Explode(lua_State* L)
{
...
int
CLuaUnitScript::ShowFlare(lua_State* L)
{
...
int
CLuaUnitScript::Spin(lua_State* L)
{
    ...
    const float accel = luaL_optfloat(
      L, 4, 0.0f); // accel == 0 -> start at desired speed immediately
...
int
CLuaUnitScript::StopSpin(lua_State* L)
{
    ...
    const int axis = ParseAxis(L, __FUNCTION__, 2);
    const float decel =
      luaL_optfloat(L, 3, 0.0f); // decel == 0 -> stop immediately
...
int
CLuaUnitScript::Turn(lua_State* L)
{
...
int
CLuaUnitScript::Move(lua_State* L)
{
...
int
CLuaUnitScript::IsInAnimation(lua_State* L, const char* caller, AnimType type)
{
    if (activeScript == NULL)
        return 0;
...
int
CLuaUnitScript::IsInTurn(lua_State* L)
{
    return IsInAnimation(L, __FUNCTION__, ATurn);
}

int
CLuaUnitScript::IsInMove(lua_State* L)
{
    return IsInAnimation(L, __FUNCTION__, AMove);
}

int
CLuaUnitScript::IsInSpin(lua_State* L)
{
    return IsInAnimation(L, __FUNCTION__, ASpin);
}

int
CLuaUnitScript::WaitForAnimation(lua_State* L,
                                 const char* caller,
                                 AnimType type)
{
    if (activeScript == NULL)
...
int
CLuaUnitScript::WaitForTurn(lua_State* L)
{
    return WaitForAnimation(L, __FUNCTION__, ATurn);
}

int
CLuaUnitScript::WaitForMove(lua_State* L)
{
    return WaitForAnimation(L, __FUNCTION__, AMove);
}

int
CLuaUnitScript::SetDeathScriptFinished(lua_State* L)
{
    ...
    if (script == NULL || !script->inKilled) {
        luaL_error(L,
                   "%s(): not a Lua unit script or 'Killed' not called",
                   __FUNCTION__);
    }
...
int
CLuaUnitScript::GetPieceTranslation(lua_State* L)
{
    if (activeScript == NULL)
        return 0;

    LocalModelPiece* piece =
      ParseLocalModelPiece(L, activeScript, __FUNCTION__);
    return ToLua(L, piece->GetPosition() - piece->original->offset);
}

int
CLuaUnitScript::GetPieceRotation(lua_State* L)
{
    if (activeScript == NULL)
        return 0;
...
int
CLuaUnitScript::GetPiecePosDir(lua_State* L)
{
    if (activeScript == NULL)
        return 0;
...
int
CLuaUnitScript::GetActiveUnitID(lua_State* L)
{
    if (activeScript == NULL)
        return 0;
    if (activeUnit == NULL)
