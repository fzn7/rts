...
CCobThread::~CCobThread()
{
    if (owner != nullptr) {
        if (cbType != CCobInstance::CBNone) {
            // LOG_L(L_DEBUG, "%s callback with %d",
            // script.scriptNames[callStack.back().functionId].c_str(), retCode);
...
CCobThread::Tick()
{
    ...
    // LOG_L(L_DEBUG, "Executing in %s (from %s)",
    ...
    while (state == Run) {
        ...
        // LOG_L(L_DEBUG, "PC: %x opcode: %x (%s)", PC - 1, opcode,
        ...
        switch (opcode) {
            ...
            case SLEEP:
                ...
                state = Sleep;
                cobEngine->AddThread(this);
                // LOG_L(L_DEBUG, "%s sleeping for %d ms",
                // script.scriptNames[callStack.back().functionId].c_str(), r1);
                return true;
            case SPIN:
                r1 = GET_LONG_PC();
                r2 = GET_LONG_PC();
            ...
            case STOP_SPIN:
                ...
                r2 = GET_LONG_PC();
                r3 = POP(); // decel
                // LOG_L(L_DEBUG, "Stop spin of %s around %d",
                // script.pieceNames[r1].c_str(), r2);
                owner->StopSpin(r1, r2, r3);
                break;
            case RETURN:
                retCode = POP();
                if (callStack.back().returnAddr == -1) {
                    // LOG_L(L_DEBUG, "%s returned %d",
                    // script.scriptNames[callStack.back().functionId].c_str(),
                    // retCode);
                    state = Dead;
                    // callStack.pop_back();
                    // Leave values intact on stack in case caller wants to
                    // check them
                    return false;
                }
            ...
            case REAL_CALL:
                ...
                if (owner->script->scriptLengths[r1] == 0) {
                    // LOG_L(L_DEBUG, "Preventing call to zero-len script %s",
                    // owner->script->scriptNames[r1].c_str());
                    break;
                }
            ...
            case START: {
                ...
                if (owner->script->scriptLengths[r1] == 0) {
                    // LOG_L(L_DEBUG, "Preventing start of zero-len script %s",
                    // owner->script->scriptNames[r1].c_str());
                    break;
                }
                ...
                // LOG_L(L_DEBUG, "Starting %s %d",
                // owner->script->scriptNames[r1].c_str(), signalMask);
            } break;
            ...
            case JUMP:
                ...
                // r2 =
                // owner->script->scriptOffsets[callStack.back().functionId] +
                // r1;
                PC = r1;
                break;
            case POP_LOCAL_VAR:
            ...
            case BITWISE_AND:
                ...
                break;
            case BITWISE_OR: // seems to want stack contents or'd, result places
                             // on stack
                r1 = POP();
                r2 = POP();
                stack.push_back(r1 | r2);
            ...
            case PUSH_STATIC:
                ...
                // LOG_L(L_DEBUG, "Push static %d val %d", r1,
                // owner->staticVars[r1]);
                break;
            case SET_NOT_EQUAL:
                r1 = POP();
            ...
            case TURN:
                ...
                r3 = GET_LONG_PC();
                r4 = GET_LONG_PC();
                // LOG_L(L_DEBUG, "Turning piece %s axis %d to %d speed %d",
                // owner->script->pieceNames[r3].c_str(), r4, r2, r1);
            ...
            case WAIT_TURN:
                r1 = GET_LONG_PC();
                r2 = GET_LONG_PC();
                // LOG_L(L_DEBUG, "Waiting for turn on piece %s around axis %d",
                // owner->script->pieceNames[r1].c_str(), r2);
            ...
            case WAIT_MOVE:
                r1 = GET_LONG_PC();
                r2 = GET_LONG_PC();
                // LOG_L(L_DEBUG, "Waiting for move on piece %s on axis %d",
                // owner->script->pieceNames[r1].c_str(), r2);
            ...
            case SHOW: {
                ...
                for (i = 0; i < MAX_WEAPONS_PER_UNIT; ++i)
                    if (callStack.back().functionId ==
                        owner->script->scriptIndex[COBFN_FirePrimary +
                                                   COBFN_Weapon_Funcs * i])
                        break;

                // If true, we are in a Fire-script and should show a special
                // flare effect
            ...
            default:
                LOG_L(L_ERROR,
                      "Unknown opcode %x (in %s:%s at %x)",
                      opcode,
                      owner->script->name.c_str(),
                      owner->script->scriptNames[callStack.back().functionId]
                        .c_str(),
                      PC - 1);
                LOG_L(L_ERROR, "Exec trace:");
                // ei = execTrace.begin();
                // while (ei != execTrace.end()) {
                // LOG_L(L_ERROR, "PC: %3x  opcode: %s", *ei,
                // GetOpcodeName(owner->script->code[*ei]).c_str());
                // ++ei;
                // }
                state = Dead;
                return false;
    ...
    return (state !=
            Dead); // can arrive here as dead, through CCobInstance::Signal()
}
...
CCobThread::LuaCall()
{
    ...
    if ((r1 < 0) ||
        (static_cast<size_t>(r1) >= owner->script->luaScripts.size())) {
        luaArgs[0] = 0; // failure
        return;
    }
