  ...
  : CCommandAI(owner)
{
    ...
    if (owner->unitDef->canPatrol) {
        ...
        c.tooltip =
          c.name +
          ": Order ready built units to patrol to one or more waypoints";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canFight) {
        ...
        c.tooltip =
          c.name +
          ": Order ready built units to take action while moving to a position";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canGuard) {
        ...
        c.action = "guard";
        c.name = "Guard";
        c.tooltip = c.name + ": Order ready built units to guard another unit "
                             "and attack units attacking it";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
...
CFactoryCAI::GiveCommandReal(const Command& c, bool fromSynced)
{
    ...
    if (boi == buildOptions.end()) {
        ...
        if (!(c.options & SHIFT_KEY) &&
            (cmdID == CMD_WAIT || cmdID == CMD_SELFD)) {
            CCommandAI::GiveAllowedCommand(c);
            return;
        }
        ...
        if (cmdID != CMD_STOP) {
            if ((cmdID == CMD_WAIT) || (cmdID == CMD_SELFD)) {
                if (!newUnitCommands.empty() &&
                    (newUnitCommands.back().GetID() == cmdID)) {
                    if (cmdID == CMD_WAIT) {
    ...
    if (c.options & RIGHT_MOUSE_KEY) {
        ...
        if (c.options & ALT_KEY) {
            for (unsigned int cmdNum = 0;
                 cmdNum < commandQue.size() && numToErase;
                 ++cmdNum) {
            ...
            for (int cmdNum = commandQue.size() - 1; cmdNum != -1 && numToErase;
                 --cmdNum) {
                if (commandQue[cmdNum].GetID() == cmdID) {
                    commandQue[cmdNum] = Command(CMD_STOP);
                    numToErase--;
...
CFactoryCAI::DecreaseQueueCount(const Command& buildCommand, int& numQueued)
{
    ...
    const Command frontCommand =
      commandQue.empty() ? Command(CMD_STOP) : commandQue.front();
...
CFactoryCAI::SlowUpdate()
{
    ...
    while (!commandQue.empty()) {
        ...
        if (buildOptions.find(c.GetID()) != buildOptions.end()) {
            // build-order
            switch (
              fac->QueueBuild(unitDefHandler->GetUnitDefByID(-c.GetID()), c)) {
...
{
    for (auto pci = possibleCommands.begin(); pci != possibleCommands.end();
         ++pci) {
