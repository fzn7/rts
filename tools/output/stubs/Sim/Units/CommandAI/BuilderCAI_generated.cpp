  ...
  , range3D(owner->unitDef->buildRange3D)
{
    ...
    if (owner->unitDef->canReclaim) {
        ...
        c.action = "reclaim";
        c.name = "Reclaim";
        c.tooltip = c.name + ": Sucks in the metal/energy content of a "
                             "unit/feature and add it to your storage";
        c.mouseicon = c.name;
        possibleCommands.push_back(commandDescriptionCache->GetPtr(c));
    }
    ...
    if (owner->unitDef->canRestore && !mapDamage->disabled) {
        ...
        c.name = "Restore";
        c.tooltip =
          c.name + ": Restores an area of the map to its original height";
        c.mouseicon = c.name;
    ...
    for (const auto& bi : ownerBuilder->unitDef->buildOptions) {
...

        {
            ...
            c.name = name;
            c.mouseicon = c.name;
            c.tooltip = GetUnitDefBuildOptionToolTip(
              ud, c.disabled = (ud->maxThisUnit <= 0));
...
CBuilderCAI::GetBuildRange(const float targetRadius) const
{
    ...
    // and so it would be impossible to get in buildrange (collision detection
    // with units/features)
    //
    // what does this even mean?? IMMOBILE units cannot "get in range" of
    // anything
    if (owner->immobile) {
        return (ownerBuilder->buildDistance +
                std::max(targetRadius - ownerBuilder->buildDistance, 0.0f));
    }
                              ...
                              const bool checkMoveTypeForFailed)
{
    if (!IsInBuildRange(objPos, objRadius)) {
        ...
        const bool checkFailed =
          (checkMoveTypeForFailed && !owner->unitDef->IsAirUnit());
        ...
        const bool haveFailed =
          (owner->moveType->progressState == AMoveType::Failed &&
           f3SqDist(owner->moveType->goalPos, objPos) > 1.0f);
    ...
    if (owner->unitDef->IsAirUnit()) {
        ...
        StopMoveAndKeepPointing(
          owner->moveType->goalPos, GetBuildRange(objRadius) * 0.9f, false);
    }
                               ...
                               const CUnit** nanoFrame) const
{
    ...
    CGameHelper::BuildSquareStatus status =
      CGameHelper::TestUnitBuildSquare(build, feature, owner->allyteam, true);

    if (feature != NULL && build.def->isFeature &&
        build.def->wreckName == feature->def->name) {
    ...
    if (u->beingBuilt) {
        if (!ownerBuilder->CanAssistUnit(u, build.def)) {
            if (u->immobile) {
                ...
                // mobile unit blocks the position, wait till it is finished &
                // moved
                return false;
            }
        }
        ...
        if (nanoFrame != NULL &&
            teamHandler->Ally(owner->allyteam, u->allyteam))
            *nanoFrame = u;
...
CBuilderCAI::CancelRestrictedUnit()
{
    if (owner->team == gu->myTeam) {
        LOG_L(L_WARNING,
              "%s: Build failed, unit type limit reached",
...
CBuilderCAI::GiveCommandReal(const Command& c, bool fromSynced)
{
    ...
    if ((c.GetID() == CMD_GUARD) && (c.params.size() == 1) &&
        ((int)c.params[0] == owner->id)) {
        return;
    }

    // stop building/reclaiming/... if the new command is not queued, i.e.
    // replaces our current activity
    // FIXME should happen just before CMobileCAI::GiveCommandReal? (the new cmd
    // can still be skipped!)
    if ((c.GetID() != CMD_WAIT && c.GetID() != CMD_SET_WANTED_MAX_SPEED) &&
        !(c.options & SHIFT_KEY)) {
    ...
    if (buildOptions.find(c.GetID()) != buildOptions.end()) {
        ...
        if (!owner->unitDef->canmove) {
            if (!IsInBuildRange(bi.pos,
                                GetBuildOptionRadius(bi.def, c.GetID()))) {
                return;
            }
        }
...
CBuilderCAI::ExecuteBuildCmd(Command& c)
{
    ...
    if (building) {
        ...
        else if (unitHandler->unitsByDefs[owner->team][build.def->id].size() >
                 build.def->maxThisUnit) {
            // unit restricted
            building = false;
            ownerBuilder->StopBuild();
        ...
        if (unitHandler->unitsByDefs[owner->team][build.def->id].size() >=
            build.def->maxThisUnit) {
        ...
        if (MoveInBuildRange(build.pos, 0.0f, true)) {
            ...
            if (ownerBuilder->StartBuild(build, f, waitstance) ||
                (++buildRetries > 30)) {
                building = true;
            } else if (f != NULL && (!build.def->isFeature ||
                                     build.def->wreckName != f->def->name)) {
                inCommand = false;
                ReclaimFeature(f);
            } else if (!waitstance) {
                const float fpSqRadius = (build.def->xsize * build.def->xsize +
                                          build.def->zsize * build.def->zsize);
                ...
                CGameHelper::BuggerOff(build.pos,
                                       std::max(buildeeRadius, fpRadius),
                                       false,
                                       true,
                                       owner->team,
                                       NULL);
...
{
    // if the target is moving away at a higher speed than we can manage, there
    // is little point in chasing it
    ...
    if (c.params.size() == 1 || c.params.size() == 5) {
        ...
        if (tempOrder && owner->moveState <= MOVESTATE_MANEUVER) {
            ...
            if (LinePointDist(commandPos1, commandPos2, unit->pos) >
                std::max(500.0f, GetBuildRange(unit->radius))) {
                StopMoveAndFinishCommand();
                return;
            }
        }
        ...
        if (c.params.size() == 5) {
            ...
            const float radius =
              c.params[4] + 100.0f; // do not walk too far outside repair area

            if ((pos - unit->pos).SqLength2D() > radius * radius ||
                (unit->IsMoving() &&
                 (((c.options & INTERNAL_ORDER) &&
                   !TargetInterceptable(unit, unit->speed.Length2D())) ||
        ...
        canRepairUnit &=
          ((unit->beingBuilt) ||
           (unit->unitDef->repairable && (unit->health < unit->maxHealth)));
        canRepairUnit &= ((unit != owner) || owner->unitDef->canSelfRepair);
        canRepairUnit &= (!unit->soloBuilder || (unit->soloBuilder == owner));
        canRepairUnit &=
          (!(c.options & INTERNAL_ORDER) || (c.options & CONTROL_KEY) ||
           !IsUnitBeingReclaimed(unit, owner));
        ...
        ownerBuilder->StopBuild();
        if (FindRepairTargetAndRepair(
              pos, radius, c.options, false, (c.options & META_KEY))) {
            inCommand = false;
            SlowUpdate();
            return;
        }
...
CBuilderCAI::ExecuteCapture(Command& c)
{
    ...
    if (c.params.size() == 1 || c.params.size() == 5) {
        ...
        if (c.params.size() == 5) {
            const float3& pos = c.GetPos(1);
            const float radius =
              c.params[4] + 100; // do not walk too far outside capture area

            if (((pos - unit->pos).SqLength2D() > (radius * radius) ||
                 (ownerBuilder->curCapture == unit && unit->IsMoving() &&
                  !IsInBuildRange(unit)))) {
                StopMoveAndFinishCommand();
                return;
            }
        ...
        if (unit->unitDef->capturable && unit->team != owner->team &&
        ...
        if (FindCaptureTargetAndCapture(
              pos, radius, c.options, (c.options & META_KEY))) {
            inCommand = false;
            SlowUpdate();
            return;
        }
...
CBuilderCAI::ExecuteGuard(Command& c)
{
    ...
    if (CBuilder* b = dynamic_cast<CBuilder*>(guardee)) {
        if (b->terraforming) {
            if (MoveInBuildRange(b->terraformCenter,
                                 b->terraformRadius * 0.7f)) {
        ...
        const bool pushRepairCommand =
          (b->curBuild != NULL) &&
          (b->curBuild->soloBuilder == NULL ||
           b->curBuild->soloBuilder == owner) &&
    ...
    if (CFactory* fac = dynamic_cast<CFactory*>(guardee)) {
        const bool pushRepairCommand =
          (fac->curBuild != NULL) &&
          (fac->curBuild->soloBuilder == NULL ||
           fac->curBuild->soloBuilder == owner) &&
    ...
    const float radius =
      (guardee->immobile)
        ? guardee->radius
        : guardee->radius * 0.8f; // in case of mobile units reduce radius a bit
...
CBuilderCAI::ExecuteReclaim(Command& c)
{
    ...
    if (c.params.size() == 1 || c.params.size() == 5) {
        ...
        if (signedId < 0) {
            LOG_L(
              L_WARNING,
              "Trying to reclaim unit or feature with id < 0 (%i), aborting.",
              signedId);
            return;
        }
        ...
        if (checkForBetterTarget && (c.options & INTERNAL_ORDER) &&
            (c.params.size() >= 5)) {
            ...
            if (obj) {
                ...
                const bool recEnemyOnly =
                  (c.options & META_KEY) && (c.options & CONTROL_KEY);
                ...
                const int rid =
                  FindReclaimTarget(pos, radius, c.options, recopt, curdist);
                if ((rid > 0) && (rid != uid)) {
                    StopMoveAndFinishCommand();
                    RemoveUnitFromReclaimers(owner);
                    RemoveUnitFromFeatureReclaimers(owner);
        ...
        if (uid >= unitHandler->MaxUnits()) { // reclaim feature
            CFeature* feature =
              featureHandler->GetFeature(uid - unitHandler->MaxUnits());

            if (feature != NULL) {
                bool featureBeingResurrected =
                  IsFeatureBeingResurrected(feature->id, owner);
                featureBeingResurrected &=
                  (c.options & INTERNAL_ORDER) && !(c.options & CONTROL_KEY);
            ...
            if (unit != NULL && c.params.size() == 5) {
                ...
                const float radius =
                  c.params[4] +
                  100.0f; // do not walk too far outside reclaim area

                const bool outOfReclaimRange =
                  (pos.SqDistance2D(unit->pos) > radius * radius) ||
                  (ownerBuilder->curReclaim == unit && unit->IsMoving() &&
                   !IsInBuildRange(unit));
            ...
            if (unit != NULL && unit != owner && unit->unitDef->reclaimable &&
                UpdateTargetLostTimer(unit->id) &&
...
CBuilderCAI::ExecuteResurrect(Command& c)
{
    ...
    if (c.params.size() == 1) {
        ...
        if (id >= unitHandler->MaxUnits()) { // resurrect feature
            CFeature* feature =
              featureHandler->GetFeature(id - unitHandler->MaxUnits());

            if (feature && feature->udef != NULL) {
                if (((c.options & INTERNAL_ORDER) &&
                     !(c.options & CONTROL_KEY) &&
                     IsFeatureBeingReclaimed(feature->id, owner)) ||
                ...
                if (ownerBuilder->lastResurrected &&
                    unitHandler->GetUnitUnsafe(ownerBuilder->lastResurrected) !=
                      NULL &&
                    owner->unitDef->canRepair) {
                    // resurrection finished, start repair (by overwriting the
                    // current order)
                    c = Command(CMD_REPAIR,
                                c.options | INTERNAL_ORDER,
                                ownerBuilder->lastResurrected);
        ...
        if (FindResurrectableFeatureAndResurrect(
              pos, radius, c.options, (c.options & META_KEY))) {
            inCommand = false;
            SlowUpdate();
            return;
...
CBuilderCAI::ExecuteFight(Command& c)
{
    ...
    if (c.params.size() < 3) { // this shouldnt happen but anyway ...
        LOG_L(L_ERROR,
              "Received a Fight command with less than 3 params on %s in "
              "BuilderCAI",
              owner->unitDef->humanName.c_str());
        return;
    }
    ...
    const bool reclaimEnemyOnlyMode =
      (c.options & CONTROL_KEY) && (c.options & META_KEY);
    ...
    const float searchRadius =
      (owner->immobile ? 0 : (300 * owner->moveState)) +
      ownerBuilder->buildDistance;

    if (!reclaimEnemyOnlyMode &&
        (owner->unitDef->canRepair ||
         owner->unitDef->canAssist) && // Priority 1: Repair
        FindRepairTargetAndRepair(
          curPosOnLine, searchRadius, c.options, true, resurrectMode)) {
        tempOrder = true;
        inCommand = false;
    ...
    if (!reclaimEnemyOnlyMode && resurrectMode &&
        owner->unitDef->canResurrect && // Priority 2: Resurrect (optional)
        FindResurrectableFeatureAndResurrect(
          curPosOnLine, searchRadius, c.options, false)) {
        tempOrder = true;
        inCommand = false;
    ...
    if (owner->unitDef->canReclaim && // Priority 3: Reclaim / reclaim non
                                      // resurrectable (optional) / reclaim
                                      // enemy units (optional)
        FindReclaimTargetAndReclaim(
          curPosOnLine, searchRadius, c.options, recopt)) {
        tempOrder = true;
        inCommand = false;
    ...
    if (owner->HaveTarget() &&
        owner->moveType->progressState != AMoveType::Done) {
        StopMove();
    } else {
...
CBuilderCAI::ExecuteRestore(Command& c)
{
    ...
    if (inCommand) {
        ...
        const float3 pos(c.params[0],
                         CGround::GetHeightReal(c.params[0], c.params[2]),
                         c.params[2]);
    ...
    if (pointed) {
        if (!teamHandler->Ally(gu->myAllyTeam, pointed->allyteam)) {
            if (owner->unitDef->canAttack && (owner->maxRange > 0)) {
                return CMD_ATTACK;
            } else if (owner->unitDef->canReclaim &&
                       pointed->unitDef->reclaimable) {
                return CMD_RECLAIM;
            }
        } else {
...
CBuilderCAI::IsUnitBeingReclaimed(const CUnit* unit, CUnit* friendUnit)
{
    ...
    for (CUnitSet::iterator it = reclaimers.begin(); it != reclaimers.end();
         ++it) {
        ...
        if (c.GetID() != CMD_RECLAIM ||
            (c.params.size() != 1 && c.params.size() != 5)) {
            rm.push_back(*it);
            continue;
        }
        ...
        if (cmdUnitId == unit->id &&
            (!friendUnit ||
             teamHandler->Ally(friendUnit->allyteam, (*it)->allyteam))) {
            retval = true;
            break;
        }
    }
...
CBuilderCAI::IsFeatureBeingReclaimed(int featureId, CUnit* friendUnit)
{
    ...
    for (CUnitSet::iterator it = featureReclaimers.begin();
         it != featureReclaimers.end();
         ++it) {
        if ((*it)->commandAI->commandQue.empty()) {
            rm.push_back(*it);
            continue;
...
CBuilderCAI::IsFeatureBeingResurrected(int featureId, CUnit* friendUnit)
{
    ...
    for (CUnitSet::iterator it = resurrecters.begin(); it != resurrecters.end();
         ++it) {
        if ((*it)->commandAI->commandQue.empty()) {
            rm.push_back(*it);
            continue;
                               ...
                               float bestStartDist) const
{
    ...
    if (recUnits || recEnemy || recEnemyOnly) {
        const std::vector<CUnit*>& units =
          quadField->GetUnitsExact(pos, radius, false);
    ...
    if ((!best || !stationary) && !recEnemyOnly) {
        ...
        const std::vector<CFeature*>& features =
          quadField->GetFeaturesExact(pos, radius, false);
        bool metal = false;
        for (std::vector<CFeature*>::const_iterator fi = features.begin();
             fi != features.end();
             ++fi) {
            const CFeature* f = *fi;
            if (f->def->reclaimable && (recSpecial || f->def->autoreclaim) &&
                ...
                if ((dist < bestDist ||
                     (recSpecial && !metal && f->def->metal > 0.0)) &&
                    (noResCheck ||
                     ((f->def->metal > 0.0f) &&
                    ...
                    if (!(cmdopt & CONTROL_KEY) &&
                        IsFeatureBeingResurrected(f->id, owner)) {
                        continue;
                    }
                                                  ...
                                                  bool freshOnly)
{
    ...
    for (std::vector<CFeature*>::const_iterator fi = features.begin();
        ...
        if (f->def->destructable && f->udef != NULL) {
            ...
            if (freshOnly && f->reclaimLeft < 1.0f &&
                f->resurrectProgress <= 0.0f) {
                continue;
            }
            float dist = f3SqDist(f->pos, owner->pos);
            if (dist < bestDist) {
                // dont lock-on to units outside of our reach (for immobile
                // builders)
                if (owner->immobile && !IsInBuildRange(f)) {
                    continue;
                }
                if (!(options & CONTROL_KEY) &&
                    IsFeatureBeingReclaimed(f->id, owner)) {
                    continue;
                }
                bestDist = dist;
                best = f;
    ...
    if (best) {
        Command c2(CMD_RESURRECT,
                   options | INTERNAL_ORDER,
                   unitHandler->MaxUnits() + best->id);
        commandQue.push_front(c2);
        return true;
    }
                                         ...
                                         bool healthyOnly)
{
    ...
    for (ui = cu.begin(); ui != cu.end(); ++ui) {
        ...
        if ((((options & CONTROL_KEY) && owner->team != unit->team) ||
            ...
            if (unit->IsMoving() &&
                stationary) { // capture stationary targets first
                continue;
            }
            if (healthyOnly && unit->health < unit->maxHealth &&
                unit->captureProgress <= 0.0f) {
                continue;
            }
                                       ...
                                       bool builtOnly)
{
    ...
    for (std::vector<CUnit*>::const_iterator ui = cu.begin(); ui != cu.end();
         ++ui) {
        const CUnit* unit = *ui;
        ...
        if (teamHandler->Ally(owner->allyteam, unit->allyteam)) {
            if (!haveEnemy && (unit->health < unit->maxHealth)) {
                ...
                if (unit->beingBuilt && owner->team != unit->team &&
                    (owner->moveState != MOVESTATE_ROAM)) {
                    continue;
                }
                // don't help factories produce units when set on hold pos
                if (unit->beingBuilt && unit->moveDef != NULL &&
                    (owner->moveState == MOVESTATE_HOLDPOS)) {
                    continue;
                }
                // don't assist or repair if can't assist or repair
                if (!ownerBuilder->CanAssistUnit(unit) &&
                    !ownerBuilder->CanRepairUnit(unit)) {
                    continue;
                }
                if (unit == owner) {
                    trySelfRepair = true;
                ...
                if (dist < bestDist || (!stationary && !unit->IsMoving())) {
                    ...
                    if ((owner->immobile ||
                         (unit->IsMoving() &&
                          !TargetInterceptable(unit, unitSpeed))) &&
                        !IsInBuildRange(unit)) {
                        continue;
                    }
            ...
            if (!attackEnemy || !owner->unitDef->canAttack ||
                (owner->maxRange <= 0))
                continue;
    ...
    if (bestUnit == NULL) {
        if (trySelfRepair && owner->unitDef->canSelfRepair &&
