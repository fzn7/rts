...
{
    return id != cd.id || type != cd.type || name != cd.name ||
           action != cd.action || iconname != cd.iconname ||
           mouseicon != cd.mouseicon || tooltip != cd.tooltip ||
           queueing != cd.queueing || hidden != cd.hidden ||
           disabled != cd.disabled || showUnique != cd.showUnique ||
           onlyTexture != cd.onlyTexture || params != cd.params;
}
