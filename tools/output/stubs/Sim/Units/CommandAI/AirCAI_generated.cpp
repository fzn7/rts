  ...
  , lastPC2(-1)
{
    ...
    if (owner->unitDef->canAttack) {
        ...
        c.name = "Area attack";
        c.tooltip =
          c.name + ": Sets the aircraft to attack enemy units within a circle";
        c.mouseicon = c.name;
...
CAirCAI::AirAutoGenerateTarget(AAirMoveType* myPlane)
{
    ...
    const bool autoAttack = ((owner->fireState >= FIRESTATE_FIREATWILL) &&
    ...
    if (ownerDef->canAttack && autoAttack && owner->maxRange > 0) {
        if (ownerDef->IsFighterAirUnit()) {
            ...
            const CUnit* enemy =
              CGameHelper::GetClosestEnemyAircraft(NULL, P, R, owner->allyteam);
            ...
            const CUnit* enemy =
              CGameHelper::GetClosestValidTarget(P, R, owner->allyteam, this);
...
CAirCAI::ExecuteMove(Command& c)
{
    ...
    const CStrafeAirMoveType* airMT =
      (!owner->UsingScriptMoveType())
        ? static_cast<const CStrafeAirMoveType*>(myPlane)
        : NULL;
    const float radius =
      (airMT != NULL) ? std::max(airMT->turnRadius + 2 * SQUARE_SIZE, 128.f)
                      : 127.f;
...
CAirCAI::ExecuteFight(Command& c)
{
    ...
    if (c.params.size() < 3) {
        LOG_L(L_ERROR,
              "[ACAI::%s][f=%d][id=%d] CMD_FIGHT #params < 3",
              __FUNCTION__,
              gs->frameNum,
              owner->id);
        return;
    ...
    if (c.params.size() >= 6) {
        ...
        // 'if((curPos-pos).SqLength2D()<(127*127)){' condition, but is actually
    ...
    // CMD_FIGHT is pretty useless if !canAttack, but we try to honour the
    // modders wishes anyway...
    if (owner->unitDef->canAttack &&
        (owner->fireState >= FIRESTATE_FIREATWILL) &&
        (owner->moveState != MOVESTATE_HOLDPOS) && (owner->maxRange > 0)) {
        CUnit* enemy = NULL;

        if (owner->unitDef->IsFighterAirUnit()) {
            const float3 P = ClosestPointOnLine(
        ...
        if (IsValidTarget(enemy) &&
            (owner->moveState != MOVESTATE_MANEUVER ||
...
CAirCAI::ExecuteAttack(Command& c)
{
    ...
    if (tempOrder && owner->moveState == MOVESTATE_MANEUVER) {
        ...
        if (orderTarget &&
            LinePointDist(commandPos1, commandPos2, orderTarget->pos) > 1500) {
    ...
    if (inCommand) {
        ...
        if (c.params.size() == 1) {
            ...
            owner->AttackUnit(
              targetUnit, (c.options & INTERNAL_ORDER) == 0, false);

            inCommand = true;
        } else {
            SetGoal(c.GetPos(0), owner->pos, cancelDistance);
            owner->AttackGround(
              c.GetPos(0), (c.options & INTERNAL_ORDER) == 0, false);
...
CAirCAI::ExecuteAreaAttack(Command& c)
{
    ...
    if (inCommand) {
        ...
        if (orderTarget &&
            orderTarget->pos.SqDistance2D(pos) > Square(radius)) {
...
CAirCAI::ExecuteGuard(Command& c)
{
    ...
    const bool pushAttackCommand =
      (owner->maxRange > 0.0f) && owner->unitDef->canAttack &&
      ((guardee->lastAttackFrame + 40) < gs->frameNum) &&
      IsValidTarget(guardee->lastAttacker);

    if (pushAttackCommand) {
        Command nc(
          CMD_ATTACK, c.options | INTERNAL_ORDER, guardee->lastAttacker->id);
        commandQue.push_front(nc);
        SlowUpdate();
    } else {
...
{
    if (!CMobileCAI::IsValidTarget(enemy))
        return false;
    if (enemy->IsCrashing())
        return false;
...
CAirCAI::SelectNewAreaAttackTargetOrPos(const Command& ac)
{
    ...
    if (enemyUnitIDs.empty()) {
        ...
        owner->AttackGround(
          attackPos, (ac.options & INTERNAL_ORDER) == 0, false);
        SetGoal(attackPos, owner->pos);
    } else {
        // note: the range of randFloat() is inclusive of 1.0f
        const unsigned int unitIdx = std::min<int>(
          gs->randFloat() * enemyUnitIDs.size(), enemyUnitIDs.size() - 1);
        ...
        SetOrderTarget(targetUnit);
        owner->AttackUnit(
          targetUnit, (ac.options & INTERNAL_ORDER) == 0, false);
