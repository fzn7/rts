...
void
CFactory::KillUnit(CUnit* attacker,
                   bool selfDestruct,
                   bool reclaimed,
...
{
    const float3 relBuildPos = script->GetPiecePos(
      (buildPiece < 0) ? script->QueryBuildInfo() : buildPiece);
...
CFactory::Update()
{
    ...
    const bool wantClose =
      (!IsStunned() && yardOpen &&
       (gs->frameNum >= (lastBuildUpdateFrame + GAME_SPEED * 7)));
    const bool closeYard = (wantClose && curBuild == nullptr &&
...
{
        // this (ancient) code was intended to keep vicinity around factories
clear
        // so units could exit more easily among crowds of assisting builders,
etc.
        // it is unneeded now that units can flow / push through a non-moving
crowd
        // (so we no longer have to override CBuilding::SlowUpdate either)
        if (transporter == NULL) {
                CGameHelper::BuggerOff(pos - float3(0.01f, 0, 0.02f), radius,
true, true, team, NULL);
        }
...
CFactory::StartBuild(const UnitDef* buildeeDef)
{
    ...
    UnitLoadParams buildeeParams = { buildeeDef,  this, buildPos,
                                     ZeroVector,  -1,   team,
                                     buildFacing, true, false };
...
CFactory::UpdateBuild(CUnit* buildee)
{
    ...
    const int buildPieceHeading =
      GetHeadingFromVector(buildPieceMat[2], buildPieceMat[10]); //! x.z, z.z

    float3 buildeePos = buildPos;

    // rotate unit nanoframe with platform
    buildee->heading =
      (-buildPieceHeading + GetHeadingFromFacing(buildFacing)) &
      (SPRING_CIRCLE_DIVS - 1);
...
CFactory::FinishBuild(CUnit* buildee)
{
    ...
    const bool buildeeMobile =
      (dynamic_cast<const CMobileCAI*>(bcai) != nullptr);
...
CFactory::QueueBuild(const UnitDef* buildeeDef, const Command& buildCmd)
{
    ...
    if (unitHandler->unitsByDefs[team][buildeeDef->id].size() >=
        buildeeDef->maxThisUnit)
...
CFactory::SendToEmptySpot(CUnit* unit)
{
    ...
    for (int x = 0; x < numSteps; ++x) {
        ...
        if (quadField->NoSolidsExact(testPos,
                                     unit->radius * 1.5f,
                                     0xFFFFFFFF,
    ...
    if (foundPos == tempPos) {
        // no empty spot found, pick one randomly so units do not pile up even
        // more also make sure not to loop forever if we happen to be facing a
        // map border
        foundPos.y = 0.0f;
        ...
        do {
            ...
            const float a =
              searchRadius * math::cos(x * PI / (numSteps * 0.5f));
            const float b =
              searchRadius * math::sin(x * PI / (numSteps * 0.5f));

            foundPos.x = pos.x + frontdir.x * a + rightdir.x * b;
            foundPos.z = pos.z + frontdir.z * a + rightdir.z * b;
            foundPos.y += 1.0f;
        } while (
          (!foundPos.IsInBounds() || (foundPos - pos).dot(frontdir) < 0.0f) &&
          (foundPos.y < 100.0f));
...
CFactory::AssignBuildeeOrders(CUnit* unit)
{
    ...
    if (!unit->unitDef->canfly) {
        ...
        if (buildFacing == FACING_NORTH || buildFacing == FACING_SOUTH) {
            while (
              (tmpPos.z >= unit->pos.z - zs && tmpPos.z <= unit->pos.z + zs)) {
                tmpDst += 0.5f;
                tmpPos = unit->pos + (frontdir * this->radius * tmpDst);
            }
        } else {
            while (
              (tmpPos.x >= unit->pos.x - xs && tmpPos.x <= unit->pos.x + xs)) {
                tmpDst += 0.5f;
                tmpPos = unit->pos + (frontdir * this->radius * tmpDst);
            }
        }
    ...
    if (unitCmdQue.empty()) {
        ...
        for (CCommandQueue::const_iterator ci = factoryCmdQue.begin();
             ci != factoryCmdQue.end();
             ++ci) {
            Command c = *ci;
            c.options |= SHIFT_KEY;
...
CFactory::CreateNanoParticle(bool highPriority)
{
    ...
    projectileHandler->AddNanoParticle(
      nanoPos, curBuild->midPos, unitDef, team, highPriority);
}
