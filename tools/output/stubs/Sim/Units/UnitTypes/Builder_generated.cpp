...
void
CBuilder::PreInit(const UnitLoadParams& params)
{
...
{
    return unitDef->canAssist && (!def || (u->unitDef == def)) &&
...
CBuilder::Update()
{
    ...
    const Command& fCommand =
      (!cQueue.empty()) ? cQueue.front() : Command(CMD_STOP);

    nanoPieceCache.Update();

    if (!beingBuilt && !IsStunned()) {
        if (terraforming && inBuildStance) {
            assert(!mapDamage->disabled); // The map should not be deformed in
                                          // the first place.
            ...
            switch (terraformType) {
                case Terraform_Building:
                    if (curBuild != NULL) {
                        if (curBuild->terraformLeft <= 0)
                            ...
                            terraformScale = (terraformSpeed + terraformHelp) /
                                             curBuild->terraformLeft;

                        curBuild->terraformLeft -=
                          (terraformSpeed + terraformHelp);
                        terraformHelp = 0;
                        terraformScale = std::min(terraformScale, 1.0f);

                        // prevent building from timing out while terraforming
                        // for it
                        ...
                        for (int z = tz1; z <= tz2; z++) {
                            for (int x = tx1; x <= tx2; x++) {
                                ...
                                readMap->AddHeight(
                                  idx, (curBuild->pos.y - ch) * terraformScale);
                            }
                        }
                        ...
                        if (curBuild->terraformLeft <= 0.0f) {
                            ...
                            if (eventHandler.TerraformComplete(this,
                                                               curBuild)) {
                                StopBuild();
                            }
                        }
                ...
                case Terraform_Restore:
                    if (myTerraformLeft <= 0.0f)
                        ...
                        terraformScale =
                          (terraformSpeed + terraformHelp) / myTerraformLeft;
                    ...
                    for (int z = tz1; z <= tz2; z++) {
                        for (int x = tx1; x <= tx2; x++) {
                            ...
                            float ch = heightmap[idx];
                            float oh =
                              readMap->GetOriginalHeightMapSynced()[idx];
            ...
            for (int z = tz1; z <= tz2; z++) {
                ...
                for (int x = 1; x <= 3; x++) {
                    if (tx1 - 3 >= 0) {
                        ...
                        const float ch =
                          heightmap[z * mapDims.mapxp1 + tx1 - x];
                        const float ch2 =
                          heightmap[z * mapDims.mapxp1 + tx1 - 3];
                        const float amount =
                          ((ch3 * (3 - x) + ch2 * x) / 3 - ch) * terraformScale;

                        readMap->AddHeight(z * mapDims.mapxp1 + tx1 - x,
                                           amount);
                    }
                    if (tx2 + 3 < mapDims.mapx) {
                        const float ch3 = heightmap[z * mapDims.mapxp1 + tx2];
                        const float ch =
                          heightmap[z * mapDims.mapxp1 + tx2 + x];
                        const float ch2 =
                          heightmap[z * mapDims.mapxp1 + tx2 + 3];
                        const float amount =
                          ((ch3 * (3 - x) + ch2 * x) / 3 - ch) * terraformScale;

                        readMap->AddHeight(z * mapDims.mapxp1 + tx2 + x,
                                           amount);
                    }
                }
            }
            for (int z = 1; z <= 3; z++) {
                ...
                for (int x = tx1; x <= tx2; x++) {
                    if (tz1 - 3 >= 0) {
                        ...
                        const float ch =
                          heightmap[(tz1 - z) * mapDims.mapxp1 + x];
                        const float ch2 =
                          heightmap[(tz1 - 3) * mapDims.mapxp1 + x];
                        const float adjust =
                          ((ch3 * (3 - z) + ch2 * z) / 3 - ch) * terraformScale;

                        readMap->AddHeight((tz1 - z) * mapDims.mapxp1 + x,
                                           adjust);
                    }
                    if (tz2 + 3 < mapDims.mapy) {
                        const float ch3 = heightmap[(tz2)*mapDims.mapxp1 + x];
                        const float ch =
                          heightmap[(tz2 + z) * mapDims.mapxp1 + x];
                        const float ch2 =
                          heightmap[(tz2 + 3) * mapDims.mapxp1 + x];
                        const float adjust =
                          ((ch3 * (3 - z) + ch2 * z) / 3 - ch) * terraformScale;

                        readMap->AddHeight((tz2 + z) * mapDims.mapxp1 + x,
                                           adjust);
                    }
                }
            }
            ...
            if (helpTerraform->terraforming) {
                ...
                CreateNanoParticle(helpTerraform->terraformCenter,
                                   helpTerraform->terraformRadius * 0.5f,
                                   false);
            } else {
            ...
            if (fCommand.GetID() == CMD_WAIT) {
                if (curBuild->buildProgress < 1.0f) {
                    // prevent buildee from decaying (we cannot call StopBuild
                    // here)
                    curBuild->AddBuildPower(this, 0.0f);
                } else {
                    // stop repairing (FIXME: should be much cleaner to let
                    // BuilderCAI call this instead when a wait command is
                    // given?)
                    StopBuild();
                }
            } else {
                if (curBuild->soloBuilder != NULL &&
                    (curBuild->soloBuilder != this)) {
                    StopBuild();
                } else {
                    if (inBuildStance || true) {
                        // NOTE:
                        //     technically this block of code should be guarded
                        //     by "if (inBuildStance)", but doing so can create
                        //     zombie guarders because scripts might not set
                        //     inBuildStance to true when guard or repair orders
                        //     are executed and SetRepairTarget does not check
                        //     for it
                        //
                        //     StartBuild *does* ensure construction will not
                        //     start until inBuildStance is set to true by the
                        //     builder's script, and there are no cases during
                        //     construction when inBuildStance can become false
                        //     yet the buildee should be kept from decaying, so
                        //     this is free from serious side-effects (when
                        //     repairing, a builder might start adding
                        //     build-power before having fully finished its
                        //     opening animation)
                        //
                        ScriptDecloak(true);
                        ...
                        if (curBuild->buildProgress >= 1.0f) {
                            adjBuildSpeed =
                              std::min(repairSpeed,
                                       unitDef->maxRepairSpeed * 0.5f -
                                         curBuild->repairAmount); // repair
                        } else {
                            adjBuildSpeed = buildSpeed; // build
                        }

                        if (adjBuildSpeed > 0.0f &&
                            curBuild->AddBuildPower(this, adjBuildSpeed)) {
                            CreateNanoParticle(
                              curBuild->midPos, curBuild->radius * 0.5f, false);
                        } else {
        ...
        } else if (curReclaim != NULL &&
                   f3SqDist(curReclaim->pos, pos) <
                     Square(buildDistance + curReclaim->radius) &&
                   inBuildStance) {
            if (fCommand.GetID() == CMD_WAIT) {
                StopBuild();
            } else {
                ...
                if (curReclaim->AddBuildPower(this, -reclaimSpeed)) {
                    CreateNanoParticle(
                      curReclaim->midPos,
                      curReclaim->radius * 0.7f,
                      true,
                      (reclaimingUnit && curReclaim->team != team));
                }
            }
        } else if (curResurrect != NULL &&
                   f3SqDist(curResurrect->pos, pos) <
                     Square(buildDistance + curResurrect->radius) &&
                   inBuildStance) {
            ...
            if (fCommand.GetID() == CMD_WAIT) {
                ...
                if (ud != NULL) {
                    if ((modInfo.reclaimMethod != 1) &&
                        (curResurrect->reclaimLeft < 1)) {
                        // This corpse has been reclaimed a little, need to
                        // restore the resources before we can let the player
                        // resurrect it.
                        ...
                        const bool resurrectAllowed =
                          eventHandler.AllowFeatureBuildStep(
                            this, curResurrect, step);
                        const bool canExecResurrect =
                          (resurrectAllowed &&
                           UseEnergy(ud->energy * step *
                                     modInfo.resurrectEnergyCostFactor));

                        if (canExecResurrect) {
                            curResurrect->resurrectProgress += step;
                            curResurrect->resurrectProgress =
                              std::min(curResurrect->resurrectProgress, 1.0f);

                            CreateNanoParticle(curResurrect->midPos,
                                               curResurrect->radius * 0.7f,
                                               (gs->randInt() & 1));
                        }
                        ...
                        if (curResurrect->resurrectProgress >= 1.0f) {
                            if (!curResurrect->deleteMe) {
                                ...
                                UnitLoadParams resurrecteeParams = {
                                    ...
                                    -1,
                                    team,
                                    curResurrect->buildFacing,
                                    false,
                                    false
                                };
                                CUnit* resurrectee =
                                  unitLoader->LoadUnit(resurrecteeParams);

                                assert(ud == resurrectee->unitDef);

                                if (!ud->canBeAssisted) {
                                    resurrectee->soloBuilder = this;
                                    resurrectee->AddDeathDependence(
                                      this, DEPENDENCE_BUILDER);
                                }

                                // TODO: make configurable if this should happen
                                resurrectee->health *= 0.05f;

                                for (CUnitSet::iterator it =
                                       cai->resurrecters.begin();
                                     it != cai->resurrecters.end();
                                     ++it) {
                                    ...
                                    if ((c.params[0] -
                                         unitHandler->MaxUnits()) !=
                                        curResurrect->id)
                                        continue;

                                    if (!teamHandler->Ally(allyteam,
                                                           bld->allyteam))
                                        continue;

                                    // all units that were rezzing shall assist
                                    // the repair too
                                    bld->lastResurrected = resurrectee->id;
                                    // prevent FinishCommand from removing this
                                    // command when the feature is deleted,
                                    // since it is needed to start the repair
                                    // (WTF!)
        ...
        } else if (curCapture != NULL &&
                   f3SqDist(curCapture->pos, pos) <
                     Square(buildDistance + curCapture->radius) &&
                   inBuildStance) {
            if (fCommand.GetID() == CMD_WAIT) {
                StopBuild();
            } else {
                if (curCapture->team != team) {
                    const float captureMagicNumber =
                      (150.0f + (curCapture->buildTime / captureSpeed) *
                                  (curCapture->health + curCapture->maxHealth) /
                                  curCapture->maxHealth * 0.4f);
                    const float captureProgressStep = 1.0f / captureMagicNumber;
                    const float captureProgressTemp = std::min(
                      curCapture->captureProgress + captureProgressStep, 1.0f);

                    const float captureFraction =
                      captureProgressTemp - curCapture->captureProgress;
                    const float energyUseScaled =
                      curCapture->cost.energy * captureFraction *
                      modInfo.captureEnergyCostFactor;

                    const bool captureAllowed =
                      (eventHandler.AllowUnitBuildStep(
                        this, curCapture, captureProgressStep));
                    const bool canExecCapture =
                      (captureAllowed && UseEnergy(energyUseScaled));

                    if (canExecCapture) {
                        curCapture->captureProgress += captureProgressStep;
                        curCapture->captureProgress =
                          std::min(curCapture->captureProgress, 1.0f);

                        CreateNanoParticle(curCapture->midPos,
                                           curCapture->radius * 0.7f,
                                           false,
                                           true);

                        if (curCapture->captureProgress >= 1.0f) {
                            if (!curCapture->ChangeTeam(
                                  team, CUnit::ChangeCaptured)) {
                                // capture failed
                                if (team == gu->myTeam) {
                                    LOG_L(L_WARNING,
                                          "%s: Capture failed, unit type limit "
                                          "reached",
...
CBuilder::SetRepairTarget(CUnit* target)
{
    ...
    if (!target->groundLevelled) {
        ...
        tx1 = (int)max(
          (float)0,
          (target->pos.x - (target->unitDef->xsize * 0.5f * SQUARE_SIZE)) /
            SQUARE_SIZE);
        tx2 = min(mapDims.mapx, tx1 + target->unitDef->xsize);
        tz1 = (int)max(
          (float)0,
          (target->pos.z - (target->unitDef->zsize * 0.5f * SQUARE_SIZE)) /
            SQUARE_SIZE);
...
{
    if (dynamic_cast<CFeature*>(target) != NULL &&
        !static_cast<CFeature*>(target)->def->reclaimable) {
        return;
    }
...
CBuilder::StartRestore(float3 centerPos, float radius)
{
    ...
    for (int z = tz1; z <= tz2; z++) {
        for (int x = tx1; x <= tx2; x++) {
            float delta = orgHeightMap[z * mapDims.mapxp1 + x] -
...
CBuilder::StartBuild(BuildInfo& buildInfo, CFeature*& feature, bool& waitStance)
{
    ...
    const CGameHelper::BuildSquareStatus tbs =
      CGameHelper::TestUnitBuildSquare(buildInfo, feature, -1, true);
    ...
    switch (tbs) {
        ...
        case CGameHelper::BUILDSQUARE_OCCUPIED: {
            ...
            // closest to us
            CSolidObject* o =
              groundBlockingObjectMap->GroundBlocked(buildInfo.pos);
            CUnit* u = NULL;
            ...
            if (o != NULL) {
                ...
                u = CGameHelper::GetClosestFriendlyUnit(
                  NULL, buildInfo.pos, buildDistance, allyteam);
            }
    ...
    const UnitLoadParams buildeeParams = {
        buildeeDef, this, buildInfo.pos,         ZeroVector,
        -1,         team, buildInfo.buildFacing, true,
        false
    };
    ...
    const float groundheight =
      CGround::GetHeightReal(buildee->pos.x, buildee->pos.z);
    ...
    if (mapDamage->disabled || !buildeeDef->levelGround || onWater ||
        ...
        tx1 =
          (int)max(0.0f,
                   (buildee->pos.x - (buildeeDef->xsize * 0.5f * SQUARE_SIZE)) /
                     SQUARE_SIZE);
        tx2 = min(mapDims.mapx, tx1 + buildeeDef->xsize);
        tz1 =
          (int)max(0.0f,
                   (buildee->pos.z - (buildeeDef->zsize * 0.5f * SQUARE_SIZE)) /
                     SQUARE_SIZE);
...
CBuilder::CalculateBuildTerraformCost(BuildInfo& buildInfo)
{
    ...
    for (int z = tz1; z <= tz2; z++) {
        for (int x = tx1; x <= tx2; x++) {
            ...
            if (delta > 0) {
                cost = max(
                  3.0f, curHeightMap[idx] - orgHeightMap[idx] + delta * 0.5f);
            } else {
                cost = max(
                  3.0f, orgHeightMap[idx] - curHeightMap[idx] - delta * 0.5f);
            }
                             ...
                             bool highPriority)
{
    ...
    projectileHandler->AddNanoParticle(
      nanoPos, goal, unitDef, team, radius, inverse, highPriority);
}
