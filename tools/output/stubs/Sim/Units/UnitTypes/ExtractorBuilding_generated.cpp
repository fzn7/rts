...
CExtractorBuilding::ResetExtraction()
{
    ...
    for (std::vector<MetalSquareOfControl>::iterator si =
           metalAreaOfControl.begin();
         si != metalAreaOfControl.end();
         ++si) {
    ...
    for (std::list<CExtractorBuilding*>::iterator ei = neighbours.begin();
         ei != neighbours.end();
         ++ei) {
...
{
    // circle vs. circle
    return (this->pos.SqDistance2D(other->pos) <
            Square(this->extractionRange + other->extractionRange));
}
...
CExtractorBuilding::SetExtractionRangeAndDepth(float range, float depth)
{
    ...
    const std::vector<CUnit*>& cu =
      quadField->GetUnits(pos, extractionRange + maxExtractionRange);
    ...
    metalExtract = 0;
    int xBegin =
      std::max(0, (int)((pos.x - extractionRange) / METAL_MAP_SQUARE_SIZE));
    int xEnd =
      std::min(mapDims.mapx / 2 - 1,
               (int)((pos.x + extractionRange) / METAL_MAP_SQUARE_SIZE));
    int zBegin =
      std::max(0, (int)((pos.z - extractionRange) / METAL_MAP_SQUARE_SIZE));
    int zEnd =
      std::min(mapDims.mapy / 2 - 1,
               (int)((pos.z + extractionRange) / METAL_MAP_SQUARE_SIZE));
    ...
    for (int x = xBegin; x <= xEnd; x++) {
        for (int z = zBegin; z <= zEnd; z++) {
            ...
            const float3 msqrPos((x + 0.5f) * METAL_MAP_SQUARE_SIZE,
                                 pos.y,
            ...
            if (sqrCenterDistance < Square(extractionRange)) {
                ...
                msqr.extractionDepth =
                  readMap->metalMap->RequestExtraction(x, z, depth);
                metalAreaOfControl.push_back(msqr);
                metalExtract +=
                  msqr.extractionDepth *
                  readMap->metalMap->GetMetalAmount(msqr.x, msqr.z);
            }
        }
    }
...
CExtractorBuilding::ReCalculateMetalExtraction()
{
    ...
    for (std::vector<MetalSquareOfControl>::iterator si =
        ...
        readMap->metalMap->RemoveExtraction(
          msqr.x, msqr.z, msqr.extractionDepth);

        // extraction is done in a cylinder
        msqr.extractionDepth =
          readMap->metalMap->RequestExtraction(msqr.x, msqr.z, extractionDepth);
        metalExtract += msqr.extractionDepth *
                        readMap->metalMap->GetMetalAmount(msqr.x, msqr.z);
    }
