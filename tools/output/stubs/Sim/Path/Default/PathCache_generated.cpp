...
CPathCache::~CPathCache()
{
    const char* fmt =
#ifdef _WIN32
      "[%s(%ux%u)] cacheHits=%u hitPercentage=%.0f%% numHashColls=%u "
      "maxCacheSize=%I64u";
#else
      "[%s(%ux%u)] cacheHits=%u hitPercentage=%.0f%% numHashColls=%u "
      "maxCacheSize=%lu";
    ...
    for (CachedPathConstIter iter = cachedPaths.begin();
         iter != cachedPaths.end();
         ++iter)
        delete (iter->second);
}
                    ...
                    int pathType)
{
    ...
    if (iter != cachedPaths.end()) {
        return ((numHashCollisions += HashCollision(
                   iter->second, strtBlock, goalBlock, goalRadius, pathType)) !=
                cols);
    }
    ...
    const int lifeTime = (result == IPath::Ok)
                           ? GAME_SPEED * MAX_PATH_LIFETIME_SECS
                           : GAME_SPEED * (MAX_PATH_LIFETIME_SECS / 2);
...
#define NZ numBlocksZ
    ...
    const boost::uint32_t index =
      ((goalBlk.y * NX + goalBlk.x) * NZ + strtBlk.y) * NX;
    const boost::uint32_t offset =
      strtBlk.x * (pathType + 1) * std::max(1.0f, goalRadius);
    return (index + offset);
#else
    // map into linear space, cannot collide unless given non-integer radii
    const boost::uint64_t index =
      (strtBlk.y * NX + strtBlk.x) + (goalBlk.y * NX + goalBlk.x) * N;
    const boost::uint64_t offset = pathType * N * N + goalRadius * N * N * N;
    return (index + offset);
#endif
                          ...
                          int pathType) const
{
    ...
    const char* fmt =
#ifdef _WIN32
      "[%s][f=%d][hash=%I64u] Hash(sb=<%d,%d> gb=<%d,%d> gr=%.2f "
      "pt=%d)==Hash(sb=<%d,%d> gb=<%d,%d> gr=%.2f pt=%d)";
#else
      "[%s][f=%d][hash=%lu] Hash(sb=<%d,%d> gb=<%d,%d> gr=%.2f "
      "pt=%d)==Hash(sb=<%d,%d> gb=<%d,%d> gr=%.2f pt=%d)";
#endif
    if (hashColl) {
        LOG_L(L_DEBUG,
