...
GetNumThreads()
{
    const size_t numThreads =
      std::max(0, configHandler->GetInt("PathingThreadCount"));
...
{
    vertexCosts.resize(moveDefHandler->GetNumMoveDefs() *
                         blockStates.GetSize() * PATH_DIRECTION_VERTICES,
                       PATHCOST_INFINITY);
    ...
    // precalc for FindOffset()
    {
        ...
        std::stable_sort(offsetBlocksSortedByCost.begin(),
                         offsetBlocksSortedByCost.end(),
                         [](const SOffsetBlock a, const SOffsetBlock b) {
                              ...
                              const std::string& map)
{
    ...
    if (!ReadFile(cacheFileName, map)) {
        ...
        const unsigned int minMemFootPrint =
          sizeof(CPathFinder) + pathFinder->GetMemFootPrint();
        const unsigned int maxMemFootPrint =
          configHandler->GetInt("MaxPathCostsMemoryFootPrint") * 1024 * 1024;
        const unsigned int numExtraThreads = Clamp(
          int(maxMemFootPrint / minMemFootPrint) - 1, 0, int(numThreads) - 1);
        const unsigned int reqMemFootPrint =
          minMemFootPrint * (numExtraThreads + 1);
        ...
        for (unsigned int i = 1; i <= numExtraThreads; i++) {
            ...
            threads[i] = new boost::thread(
              boost::bind(&CPathEstimator::CalcOffsetsAndPathCosts, this, i));
        }
...
CPathEstimator::CalcOffsetsAndPathCosts(unsigned int threadNum)
{
    ...
    // NOTE: EstimatePathCosts() [B] is temporally dependent on
    // CalculateBlockOffsets() [A], A must be completely finished before B_i can
    // be safely called. This means we cannot let thread i execute (A_i, B_i),
    // but instead have to split the work such that every thread finishes its
    // part of A before any starts B_i.
                                      ...
                                      unsigned int threadNum)
{
    ...
    if (threadNum == 0 && blockIdx >= nextOffsetMessageIdx) {
        ...
        clientNet->Send(
          CBaseNetProtocol::Get().SendCPUUsage(BLOCK_SIZE | (blockIdx << 8)));
    }
    ...
    for (unsigned int i = 0; i < moveDefHandler->GetNumMoveDefs(); i++) {
        ...
        if (md->udRefCount > 0) {
            blockStates.peNodeOffsets[md->pathType][blockIdx] =
              FindOffset(*md, blockPos.x, blockPos.y);
        }
    }
}
...
CPathEstimator::EstimatePathCosts(unsigned int blockIdx, unsigned int threadNum)
{
    ...
    if (threadNum == 0 && blockIdx >= nextCostMessageIdx) {
        ...
        char calcMsg[128];
        sprintf(calcMsg,
                "PathCosts: precached %d of %d blocks",
                blockIdx,
                blockStates.GetSize());

        clientNet->Send(CBaseNetProtocol::Get().SendCPUUsage(0x1 | BLOCK_SIZE |
                                                             (blockIdx << 8)));
                           ...
                           unsigned int blockZ) const
{
    ...
    /*for (unsigned int z = 0; z < BLOCK_SIZE; ++z) {
            for (unsigned int x = 0; x < BLOCK_SIZE; ++x) {
                    float speedMod = CMoveMath::GetPosSpeedMod(moveDef, lowerX +
    x, lowerZ + z); bool curblock = (speedMod == 0.0f) ||
    CMoveMath::IsBlockedStructure(moveDef, lowerX + x, lowerZ + z, NULL);

                    if (!curblock) {
                            const float dx = x - (float)(BLOCK_SIZE - 1) * 0.5f;
                            const float dz = z - (float)(BLOCK_SIZE - 1) * 0.5f;
                            const float cost = (dx * dx + dz * dz) + (blockArea
    / (0.001f + speedMod));
    ...
    for (const SOffsetBlock& ob : offsetBlocksSortedByCost) {
        ...
        const float speedMod =
          CMoveMath::GetPosSpeedMod(moveDef, blockPos.x, blockPos.y);

        // assert((blockArea / (0.001f + speedMod) >= 0.0f);
        const float cost = ob.cost + (blockArea / (0.001f + speedMod));
        if (cost < bestCost) {
            if (!CMoveMath::IsBlockedStructure(
                  moveDef, blockPos.x, blockPos.y, NULL)) {
                bestCost = cost;
                bestPos = blockPos;
            }
        }
...
CPathEstimator::CalculateVertices(const MoveDef& moveDef,
                                  ...
                                  unsigned int thread)
{
    // see code comment of GetBlockVertexOffset() for more info why those
    // directions are choosen
                                ...
                                unsigned int threadNum)
{
    ...
    const unsigned int vertexNbr =
      ...
      parentBlockNbr * PATH_DIRECTION_VERTICES + direction;

    // outside map?
    if ((unsigned)childBlock.x >= nbrOfBlocks.x ||
        (unsigned)childBlock.y >= nbrOfBlocks.y) {
        vertexCosts[vertexNbr] = PATHCOST_INFINITY;
        return;
    }

    // start position within parent block
    const int2 parentSquare =
      blockStates.peNodeOffsets[moveDef.pathType][parentBlockNbr];

    // goal position within child block
    const int2 childSquare =
      blockStates.peNodeOffsets[moveDef.pathType][childBlockNbr];
    ...
    const bool strtBlocked =
      ((CMoveMath::IsBlocked(moveDef, startPos, nullptr) &
        CMoveMath::BLOCK_STRUCTURE) != 0);
    const bool goalBlocked =
      pfDef.IsGoalBlocked(moveDef, CMoveMath::BLOCK_STRUCTURE, nullptr);
    ...
    IPath::Path path;
    IPath::SearchResult result = pathFinders[threadNum]->GetPath(
      moveDef, pfDef, nullptr, startPos, path, MAX_SEARCHED_NODES_PF >> 2);
                           ...
                           unsigned z2)
{
    ...
    const int lowerX =
      Clamp(int(mmx.first / BLOCK_SIZE) - 1, 0, int(nbrOfBlocks.x - 1));
    const int upperX =
      Clamp(int(mmx.second / BLOCK_SIZE) + 1, 0, int(nbrOfBlocks.x - 1));
    const int lowerZ =
      Clamp(int(mmz.first / BLOCK_SIZE) - 1, 0, int(nbrOfBlocks.y - 1));
    const int upperZ =
      Clamp(int(mmz.second / BLOCK_SIZE) + 1, 0, int(nbrOfBlocks.y - 1));
...
CPathEstimator::Update()
{
    ...
    int consumeBlocks = 0;
    {
        const int progressiveUpdates =
          updatedBlocks.size() * numMoveDefs * modInfo.pfUpdateRate;
        const int MIN_BLOCKS_TO_UPDATE =
          std::max<int>(BLOCKS_TO_UPDATE >> 1, 4U);
        const int MAX_BLOCKS_TO_UPDATE =
          std::max<int>(BLOCKS_TO_UPDATE << 1, MIN_BLOCKS_TO_UPDATE);
        blocksToUpdate =
          Clamp(progressiveUpdates, MIN_BLOCKS_TO_UPDATE, MAX_BLOCKS_TO_UPDATE);
        ...
        // we have to update blocks for all movedefs (cause PATHOPT_OBSOLETE is
        // per block and not movedef)
        consumeBlocks = int(progressiveUpdates != 0) *
                        int(ceil(float(blocksToUpdate) / numMoveDefs)) *
                        numMoveDefs;
    ...
    while (!updatedBlocks.empty()) {
        ...
        // inform dependent pathEstimator that we change vertex cost of those
        // blocks
        if (nextPathEstimator)
            nextPathEstimator->MapChanged(pos.x * BLOCK_SIZE,
                                          pos.y * BLOCK_SIZE,
                                          pos.x * BLOCK_SIZE,
                                          pos.y * BLOCK_SIZE);

        updatedBlocks.pop_front(); // must happen _after_ last usage of the
                                   // `pos` reference!
        blockStates.nodeMask[idx] &= ~PATHOPT_OBSOLETE;
    }
    ...
    // FindOffset (threadsafe)
    {
        ...
        for_mt(0, consumedBlocks.size(), [&](const int n) {
            ...
            blockStates.peNodeOffsets[currBlockMD->pathType][blockN] =
              FindOffset(*currBlockMD, sb.blockPos.x, sb.blockPos.y);
        });
    }
...
{
    return pathCache[synced]->GetCachedPath(
      strtBlock, goalBlock, goalRadius, pathType);
}
...
CPathEstimator::AddCache(const IPath::Path* path,
                         ...
                         const bool synced)
{
    pathCache[synced]->AddPath(
      path, result, strtBlock, goalBlock, goalRadius, pathType);
}
                         ...
                         const CSolidObject* owner)
{
    ...
    while (!openBlocks.empty() &&
           (openBlockBuffer.GetSize() < maxBlocksToBeSearched)) {
        // get the open block with lowest cost
        PathNode* ob = const_cast<PathNode*>(openBlocks.top());
        openBlocks.pop();

        // check if the block has been marked as unaccessible during its time in
        // the queue
        if (blockStates.nodeMask[ob->nodeNum] &
            (PATHOPT_BLOCKED | PATHOPT_CLOSED))
            continue;

        // no, check if the goal is already reached
        const int2 bSquare =
          blockStates.peNodeOffsets[moveDef.pathType][ob->nodeNum];
        const int2 gSquare = ob->nodePos * BLOCK_SIZE + goalSqrOffset;

        if (peDef.IsGoal(bSquare.x, bSquare.y) ||
            peDef.IsGoal(gSquare.x, gSquare.y)) {
        ...
        TestBlock(
          moveDef, peDef, ob, owner, PATHDIR_LEFT_UP, PATHOPT_OPEN, 1.f);
        TestBlock(moveDef, peDef, ob, owner, PATHDIR_UP, PATHOPT_OPEN, 1.f);
        TestBlock(
          moveDef, peDef, ob, owner, PATHDIR_RIGHT_UP, PATHOPT_OPEN, 1.f);
        TestBlock(moveDef, peDef, ob, owner, PATHDIR_RIGHT, PATHOPT_OPEN, 1.f);
        TestBlock(
          moveDef, peDef, ob, owner, PATHDIR_RIGHT_DOWN, PATHOPT_OPEN, 1.f);
        TestBlock(moveDef, peDef, ob, owner, PATHDIR_DOWN, PATHOPT_OPEN, 1.f);
        TestBlock(
          moveDef, peDef, ob, owner, PATHDIR_LEFT_DOWN, PATHOPT_OPEN, 1.f);
...
)
{
    ...
    const int2 testBlockPos =
      parentOpenBlock->nodePos + PE_DIRECTION_VECTORS[pathDir];
    const int2 goalBlockPos = { int(peDef.goalSquareX / BLOCK_SIZE),
                                int(peDef.goalSquareZ / BLOCK_SIZE) };

    const unsigned int testBlockIdx = BlockPosToIdx(testBlockPos);

    // bounds-check
    if ((unsigned)testBlockPos.x >= nbrOfBlocks.x)
        return false;
    if ((unsigned)testBlockPos.y >= nbrOfBlocks.y)
        return false;
    ...
    const unsigned int pathTypeBaseIdx =
      moveDef.pathType * blockStates.GetSize() * PATH_DIRECTION_VERTICES;
    const unsigned int blockNumBaseIdx =
      parentOpenBlock->nodeNum * PATH_DIRECTION_VERTICES;
    const unsigned int vertexIdx = pathTypeBaseIdx + blockNumBaseIdx +
    ...
    if (vertexCosts[vertexIdx] >= PATHCOST_INFINITY) {
        ...
        //
        // blockStates.nodeMask[testBlockIdx] |= (PathDir2PathOpt(pathDir) |
        // PATHOPT_BLOCKED); dirtyBlocks.push_back(testBlockIdx);
        return false;
    }

    // check if the block is out of constraints
    const int2 square =
      blockStates.peNodeOffsets[moveDef.pathType][testBlockIdx];
    ...
    // constraintDisabled is a hackish way to make sure we don't check this in
    // CalculateVertices
    if (testBlockPos == goalBlockPos && peDef.needPath) {
        ...
        //
        // const float3 gWorldPos = {            testBlockPos.x *
        // BLOCK_PIXEL_SIZE * 1.0f, 0.0f,             testBlockPos.y *
        // BLOCK_PIXEL_SIZE * 1.0f}; const float3 sWorldPos =
        // {parentOpenBlock->nodePos.x * BLOCK_PIXEL_SIZE * 1.0f, 0.0f,
        // parentOpenBlock->nodePos.y * BLOCK_PIXEL_SIZE * 1.0f};
        const int idx = BlockPosToIdx(testBlockPos);
        const int2 testSquare =
          blockStates.peNodeOffsets[moveDef.pathType][idx];
        ...
        if (sWorldPos.SqDistance2D(gWorldPos) > peDef.sqGoalRadius) {
            CRectangularSearchConstraint pfDef =
              CRectangularSearchConstraint(sWorldPos,
                                           gWorldPos,
                                           peDef.sqGoalRadius,
                                           BLOCK_SIZE); // sets goalSquare{X,Z}
            pfDef.testMobile = false;
            pfDef.needPath = false;
            pfDef.exactPath = true;
            pfDef.dirIndependent = true;
            const IPath::SearchResult searchRes =
              pathFinder->GetPath(moveDef,
                                  pfDef,
                                  owner,
                                  sWorldPos,
                                  path,
                                  MAX_SEARCHED_NODES_PF >> 3);
    ...
    // evaluate this node (NOTE the max-resolution indexing for
    // {flow,extra}Cost)
    // const float flowCost  = (peDef.testMobile) ?
    // (PathFlowMap::GetInstance())->GetFlowCost(square.x, square.y, moveDef,
    // PathDir2PathOpt(pathDir)) : 0.0f;
    const float extraCost =
      blockStates.GetNodeExtraCost(square.x, square.y, peDef.synced);
    ...
    blockStates.SetMaxCost(
      NODE_COST_F, std::max(blockStates.GetMaxCost(NODE_COST_F), fCost));
    blockStates.SetMaxCost(
      NODE_COST_G, std::max(blockStates.GetMaxCost(NODE_COST_G), gCost));

    // mark this block as open
    blockStates.fCost[testBlockIdx] = fCost;
    blockStates.gCost[testBlockIdx] = gCost;
    blockStates.nodeMask[testBlockIdx] |=
      (PathDir2PathOpt(pathDir) | PATHOPT_OPEN);
                             ...
                             IPath::Path& foundPath) const
{
    ...
    if (pfDef.needPath) {
        ...
        while (true) {
            ...
            const int2 square =
              blockStates.peNodeOffsets[moveDef.pathType][blockIdx];
            ...
            // next step backwards
            auto pathDir = PathOpt2PathDir(blockStates.nodeMask[blockIdx] &
                                           PATHOPT_CARDINALS);
            int2 blockPos =
              BlockIdxToPos(blockIdx) - PE_DIRECTION_VECTORS[pathDir];
                         ...
                         const std::string& map)
{
    ...
    std::string filename =
      GetPathCacheDir() + map + hashString + "." + cacheFileName + ".zip";
    if (!FileSystem::FileExists(filename))
        return false;
    // open file for reading from a suitable location (where the file exists)
    IArchive* pfile =
      archiveLoader.OpenArchive(dataDirsAccess.LocateFile(filename), "sdz");
    ...
    for (int pathType = 0; pathType < moveDefHandler->GetNumMoveDefs();
         ++pathType) {
        std::memcpy(
          &blockStates.peNodeOffsets[pathType][0], &buffer[pos], blockSize);
        pos += blockSize;
    }
    ...
    if (buffer.size() < pos + vertexCosts.size() * sizeof(float))
        return false;
    std::memcpy(
      &vertexCosts[0], &buffer[pos], vertexCosts.size() * sizeof(float));
                          ...
                          const std::string& map)
{
    ...
    zipFile file = zipOpen(
      dataDirsAccess.LocateFile(filename, FileQueryFlags::WRITE).c_str(),
    ...
    for (int pathType = 0; pathType < moveDefHandler->GetNumMoveDefs();
        ...
        zipWriteInFileInZip(file,
                            (void*)&blockStates.peNodeOffsets[pathType][0],
                            blockStates.peNodeOffsets[pathType].size() *
                              sizeof(short2));
    }

    // Write vertices.
    zipWriteInFileInZip(
      file, &vertexCosts[0], vertexCosts.size() * sizeof(float));
...
CPathEstimator::CalcChecksum() const
{
    ...
    for (auto& pathTypeOffsets : blockStates.peNodeOffsets) {
        pathChecksum = HsiehHash(&pathTypeOffsets[0],
                                 pathTypeOffsets.size() * sizeof(short2),
                                 pathChecksum);
    }

    pathChecksum = HsiehHash(
      &vertexCosts[0], vertexCosts.size() * sizeof(float), pathChecksum);
...
CPathEstimator::Hash() const
{
    ...
    LOG(
      "[PathEstimator::%s] typeMapChecksum=%u", __FUNCTION__, typeMapChecksum);
    unsigned int moveDefChecksum = moveDefHandler->GetCheckSum();
    LOG(
      "[PathEstimator::%s] moveDefChecksum=%u", __FUNCTION__, moveDefChecksum);
    unsigned int blockingChecksum = groundBlockingObjectMap->CalcChecksum();
    LOG("[PathEstimator::%s] blockingChecksum=%u",
        __FUNCTION__,
        blockingChecksum);
    LOG("[PathEstimator::%s] BLOCK_SIZE=%u", __FUNCTION__, BLOCK_SIZE);
    LOG("[PathEstimator::%s] PATHESTIMATOR_VERSION=%u",
        __FUNCTION__,
        PATHESTIMATOR_VERSION);
    return mapChecksum + typeMapChecksum + moveDefChecksum + blockingChecksum +
