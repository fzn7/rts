...
CPathManager::Finalize()
{
...

    {
        ...
        medResPE = new CPathEstimator(
          maxResPF, MEDRES_PE_BLOCKSIZE, "pe", mapInfo->map.name);
        lowResPE = new CPathEstimator(
          medResPE, LOWRES_PE_BLOCKSIZE, "pe2", mapInfo->map.name);
                           ...
                           const bool cantGetCloser)
{
    ...
    if (!ep->path.empty()) {
        ...
        ep->path.front().y =
          CMoveMath::yLevel(*path->moveDef, ep->path.front());
    }
}
                          ...
                          CSolidObject* caller) const
{
    ...
    const unsigned int nodeLimits[] = { MAX_SEARCHED_NODES_PE >> 3,
                                        MAX_SEARCHED_NODES_PE >> 3,
                                        MAX_SEARCHED_NODES_PF >> 3 };
    const bool useConstraints[] = { false, false, false };

    IPathFinder* pathFinders[] = { lowResPE, medResPE, maxResPF };
    IPath::Path* pathObjects[] = { &newPath->lowResPath,
                                   &newPath->medResPath,
                                   &newPath->maxResPath };
    ...
    if (heurGoalDist2D > searchDistances[PATH_MED_RES]) {
        ...
        bestResult = std::min(
          bestResult,
          pathFinders[PATH_MED_RES]->GetPath(*moveDef,
                                             *pfDef,
                                             caller,
                                             startPos,
                                             *pathObjects[PATH_MED_RES],
                                             nodeLimits[PATH_MED_RES]));
    }
    ...
    // first attempt - use ideal pathfinder (performance-wise)
    {
        ...
        switch (origPathRes) {
            case PATH_MAX_RES:
                bestResult = maxResPF->GetPath(*moveDef,
                                               *pfDef,
                                               caller,
                                               startPos,
                                               newPath->maxResPath,
                ...
                break;
            case PATH_MED_RES:
                bestResult = medResPE->GetPath(*moveDef,
                                               *pfDef,
                                               caller,
                                               startPos,
                                               newPath->medResPath,
                ...
                break;
            case PATH_LOW_RES:
                bestResult = lowResPE->GetPath(*moveDef,
                                               *pfDef,
                                               caller,
                                               startPos,
                                               newPath->lowResPath,
    ...
    /*{
            CCircularSearchConstraint reversedPfDef(goalPos, startPos,
    pfDef->sqGoalRadius, 7.0f, 8000); switch (pathres) { case PATH_MAX_RES:
    bestResult = maxResPF->GetPath(*moveDef, reversedPfDef, caller, goalPos,
    newPath->maxResPath, nodeLimits[2]); break; case PATH_MED_RES: bestResult =
    medResPE->GetPath(*moveDef, reversedPfDef, caller, goalPos,
    newPath->medResPath, nodeLimits[1]); break; case PATH_LOW_RES: bestResult =
    lowResPE->GetPath(*moveDef, reversedPfDef, caller, goalPos,
    newPath->lowResPath, nodeLimits[0]); break;
            }
            ...
            if (bestResult == IPath::Ok) {
                    ...
                    float3 midPos;
                    switch (pathres) {
                            case PATH_MAX_RES: midPos =
    newPath->maxResPath.path.back(); break; case PATH_MED_RES: midPos =
    newPath->medResPath.path.back(); break; case PATH_LOW_RES: midPos =
    newPath->lowResPath.path.back(); break;
                    }

                    CCircularSearchConstraint midPfDef(startPos, midPos,
    pfDef->sqGoalRadius, 3.0f, 8000); bestResult = maxResPF->GetPath(*moveDef,
    midPfDef, caller, startPos, newPath->maxResPath, MAX_SEARCHED_NODES_PF >>
    3);

                    CCircularSearchConstraint restPfDef(midPos, goalPos,
    pfDef->sqGoalRadius, 7.0f, 8000); switch (pathres) { case PATH_MAX_RES: case
    PATH_MED_RES: bestResult = medResPE->GetPath(*moveDef, restPfDef, caller,
    startPos, newPath->medResPath, nodeLimits[1]); break; case PATH_LOW_RES:
    bestResult = lowResPE->GetPath(*moveDef, restPfDef, caller, startPos,
    newPath->lowResPath, nodeLimits[0]); break;
                    }
    ...
    // third attempt - use better pathfinder
    {
        ...
        int maxRes = (heurGoalDist2D < (MAXRES_SEARCH_DISTANCE * 2.0f))
                       ? PATH_MAX_RES
                       : PATH_MED_RES;
    ...
    // fourth attempt - unconstrained search radius (performance heavy, esp. on
    // max_res)
    pfDef->DisableConstraint(true);
                          ...
                          bool synced)
{
    ...
    goalRadius = std::max<float>(
      goalRadius,
      PATH_NODE_SPACING * SQUARE_SIZE); // FIXME do on a per PE & PF level?
    CCircularSearchConstraint* pfDef =
      new CCircularSearchConstraint(startPos, goalPos, goalRadius, 3.0f, 2000);
    assert(moveDef == moveDefHandler->GetMoveDefByPathType(moveDef->pathType));

    // Creates a new multipath.
    MultiPath* newPath =
      new MultiPath(startPos, pfDef, moveDef); // deletes pfDef in dtor
    ...
    const IPath::SearchResult result =
      ArrangePath(newPath, moveDef, startPos, goalPos, pfDef, caller);
    ...
    if (result != IPath::Error) {
        if (newPath->maxResPath.path.empty()) {
            if (result != IPath::CantGetCloser) {
                ...
                newPath->maxResPath.squares.push_back(
                  int2(startPos.x / SQUARE_SIZE, startPos.z / SQUARE_SIZE));
            }
        }

        FinalizePath(
          newPath, startPos, goalPos, result == IPath::CantGetCloser);
                            ...
                            bool synced) const
{
    ...
    while (!medResPath.path.empty() &&
           startPos.SqDistance2D(medResPath.path.back()) <
    ...
    CCircularSearchConstraint rangedGoalDef(
      startPos, goalPos, 0.0f, 2.0f, Square(MAXRES_SEARCH_DISTANCE));
    rangedGoalDef.synced = synced;

    // Perform the search.
    // If this is the final improvement of the path, then use the original goal.
    const auto& pfd = (medResPath.path.empty() && lowResPath.path.empty())
                        ? *multiPath.peDef
                        : rangedGoalDef;
    const IPath::SearchResult result =
      maxResPF->GetPath(*multiPath.moveDef,
                        pfd,
                        owner,
                        startPos,
                        maxResPath,
                        MAX_SEARCHED_NODES_ON_REFINE);
                            ...
                            bool synced) const
{
    ...
    while (!lowResPath.path.empty() &&
           startPos.SqDistance2D(lowResPath.path.back()) <
             Square(MEDRES_SEARCH_DISTANCE_EXT)) {
        lowResPath.path.pop_back();
    }
    ...
    // define the search
    CCircularSearchConstraint rangedGoalDef(
      startPos, goalPos, 0.0f, 2.0f, Square(MEDRES_SEARCH_DISTANCE));
    rangedGoalDef.synced = synced;

    // Perform the search.
    // If there is no low-res path left, use original goal.
    const auto& pfd =
      (lowResPath.path.empty()) ? *multiPath.peDef : rangedGoalDef;
    const IPath::SearchResult result =
      medResPE->GetPath(*multiPath.moveDef,
                        pfd,
                        owner,
                        startPos,
                        medResPath,
...
#define EXTEND_PATH_POINTS(curResPts, nxtResPts, dist)                         \
    ((!curResPts.empty() &&                                                    \
      (curResPts.back()).SqDistance2D(callerPos) < Square((dist))) ||          \
     nxtResPts.size() <= 2)
    const bool extendMaxResPath = EXTEND_PATH_POINTS(
      medResPath.path, maxResPath.path, MAXRES_SEARCH_DISTANCE_EXT);
    const bool extendMedResPath = EXTEND_PATH_POINTS(
      lowResPath.path, medResPath.path, MEDRES_SEARCH_DISTANCE_EXT);
#undef EXTEND_PATH_POINTS
    ...
    if (extendMaxResPath) {
        ...
        FinalizePath(multiPath,
                     callerPos,
                     multiPath->finalGoal,
                     multiPath->searchResult == IPath::CantGetCloser);
    }
        ...
        if (maxResPath.path.empty()) {
            if (lowResPath.path.empty() && medResPath.path.empty()) {
                if (multiPath->searchResult == IPath::Ok) {
                    ...
                    // reached in the CantGetCloser case for any max-res
                    // searches that start within their goal radius (ie. have no
                    // waypoints) RequestPath always puts startPos into
                    // maxResPath to handle this so waypoint will have been set
                    // to it (during previous iteration) if we end up here
                    break;
                }
            } else {
                waypoint = NextWayPoint(
                  owner, pathID, numRetries + 1, callerPos, radius, synced);
                break;
            }
        } else {
    ...
    } while ((callerPos.SqDistance2D(waypoint) < Square(radius)) &&
             (waypoint != maxResPath.pathGoal));
                                     ...
                                     std::vector<int2>& points) const
{
    ...
    for (auto pvi = maxResSquares.rbegin(); pvi != maxResSquares.rend();
         ++pvi) {
        points.push_back(*pvi);
    }
}
                               ...
                               std::vector<int>& starts) const
{
    ...
    points.reserve(maxResPoints.size() + medResPoints.size() +
                   lowResPoints.size());
    starts.reserve(3);
    starts.push_back(points.size());

    for (IPath::path_list_type::const_reverse_iterator pvi =
           maxResPoints.rbegin();
         pvi != maxResPoints.rend();
         ++pvi) {
        points.push_back(*pvi);
    }
