...
{
    return (SquareToFloat3(xSquare, zSquare).SqDistance2D(goal) <=
            sqGoalRadius);
}
...
{
    const float r0 =
      SQUARE_SIZE * SQUARE_SIZE * 4.0f; // same as (SQUARE_SIZE*2)^2
    const float r1 = ((moveDef.xsize * SQUARE_SIZE) >> 1) *
                     ((moveDef.zsize * SQUARE_SIZE) >> 1) * 1.5f;
