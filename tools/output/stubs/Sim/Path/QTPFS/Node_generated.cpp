...
QTPFS::INode::SetPathCost(unsigned int type, float cost)
{
    ...
    switch (type) {
        case NODE_PATH_COST_F: {
            fCost = cost;
            return;
        } break;
        case NODE_PATH_COST_G: {
            gCost = cost;
            return;
        } break;
        case NODE_PATH_COST_H: {
            hCost = cost;
            return;
        } break;
    }
...
{
    const float dx =
      float(xmid() * SQUARE_SIZE) - float(n->xmid() * SQUARE_SIZE);
    const float dz =
      float(zmid() * SQUARE_SIZE) - float(n->zmid() * SQUARE_SIZE);
...
QTPFS::INode::GetRectangleRelation(const SRectangle& r) const
{
    ...
    //
    if ((r.x1 >= xmin() && r.x2 < xmax()) &&
        (r.z1 >= zmin() && r.z2 < zmax())) {
        return REL_RECT_INTERIOR_NODE;
    }
    if ((r.x1 >= xmax() || r.x2 < xmin()) ||
        (r.z1 >= zmax() || r.z2 < zmin())) {
                                             ...
                                             float alpha) const
{
    ...
    const unsigned int minx = std::max(xmin(), ngb->xmin()),
                       maxx = std::min(xmax(), ngb->xmax());
    const unsigned int minz = std::max(zmin(), ngb->zmin()),
...
#define CAST static_cast<unsigned int>
        ...
        case REL_NGB_EDGE_T: {
            p.x = Clamp(CAST(pos.x / SQUARE_SIZE), minx, maxx) * SQUARE_SIZE;
            p.z = minz * SQUARE_SIZE;
        } break;
        case REL_NGB_EDGE_B: {
            p.x = Clamp(CAST(pos.x / SQUARE_SIZE), minx, maxx) * SQUARE_SIZE;
            p.z = maxz * SQUARE_SIZE;
        } break;
        case REL_NGB_EDGE_R: {
            p.z = Clamp(CAST(pos.z / SQUARE_SIZE), minz, maxz) * SQUARE_SIZE;
...
QTPFS::QTNode::GetCheckSum() const
{
    ...
    {
        const unsigned char* minByte =
          reinterpret_cast<const unsigned char*>(&nodeNumber);
        const unsigned char* maxByte =
          reinterpret_cast<const unsigned char*>(&hCost) + sizeof(hCost);
    ...
    {
        const unsigned char* minByte =
          reinterpret_cast<const unsigned char*>(&_depth);
        const unsigned char* maxByte =
          reinterpret_cast<const unsigned char*>(&prevMagicNum) +
          sizeof(prevMagicNum);
...
QTPFS::QTNode::IsLeaf() const
{
    ...
    assert((children[0] == NULL && children[1] == NULL && children[2] == NULL &&
            children[3] == NULL) ||
...
QTPFS::QTNode::Split(NodeLayer& nl, bool forced)
{
    ...
    children[NODE_IDX_TL] =
      new QTNode(this, GetChildID(NODE_IDX_TL), xmin(), zmin(), xmid(), zmid());
    children[NODE_IDX_TR] =
      new QTNode(this, GetChildID(NODE_IDX_TR), xmid(), zmin(), xmax(), zmid());
    children[NODE_IDX_BR] =
      new QTNode(this, GetChildID(NODE_IDX_BR), xmid(), zmid(), xmax(), zmax());
    children[NODE_IDX_BL] =
      new QTNode(this, GetChildID(NODE_IDX_BL), xmin(), zmid(), xmid(), zmax());
...
QTPFS::QTNode::PreTesselate(NodeLayer& nl, const SRectangle& r, SRectangle& ur)
{
    ...
    if (!IsLeaf()) {
        for (unsigned int i = 0; i < children.size(); i++) {
            if ((cont |= (children[i]->GetRectangleRelation(r) ==
                          REL_RECT_INTERIOR_NODE))) {
...
QTPFS::QTNode::PreTesselate(NodeLayer& nl, const SRectangle& r, SRectangle& ur)
{
    ...
    const SRectangle& cr =
      (rel != REL_RECT_INTERIOR_NODE) ? ClipRectangle(r) : r;

    if ((cr.x2 - cr.x1) <= 0 || (cr.z2 - cr.z1) <= 0) {
        return;
    }

    // continue recursion while our CHILDREN are still larger than the clipped
    // rectangle
    //
    // NOTE: this is a trade-off between minimizing the number of leaf-nodes
    // (achieved by re-tesselating in its entirety the deepest node that fully
    // contains <r>) and cost of re-tesselating (which grows as the node-count
    // decreases, kept under control by breaking <r> up into pieces while
    // descending further)
    //
    const bool leaf = IsLeaf();
    const bool cont =
      (rel == REL_RECT_INTERIOR_NODE) || (((xsize() >> 1) > (cr.x2 - cr.x1)) &&
                                          ((zsize() >> 1) > (cr.z2 - cr.z1)));
...
QTPFS::QTNode::Tesselate(NodeLayer& nl, const SRectangle& r)
{
    unsigned int numNewBinSquares =
      0; // nr. of squares in <r> that changed bin after deformation
    unsigned int numDifBinSquares =
      0; // nr. of different bin-types across all squares within <r>
...
QTPFS::QTNode::UpdateMoveCost(const NodeLayer& nl,
                              ...
                              bool& needSplit)
{
    const std::vector<NodeLayer::SpeedBinType>& oldSpeedBins =
      nl.GetOldSpeedBins();
    const std::vector<NodeLayer::SpeedBinType>& curSpeedBins =
      nl.GetCurSpeedBins();
    const std::vector<NodeLayer::SpeedModType>& oldSpeedMods =
      nl.GetOldSpeedMods();
    const std::vector<NodeLayer::SpeedModType>& curSpeedMods =
      nl.GetCurSpeedMods();

    const NodeLayer::SpeedBinType refSpeedBin =
      curSpeedBins[zmin() * mapDims.mapx + xmin()];
    ...
    if (false && moveCostAvg > 0.0f) {
        ...
        for (unsigned int hmz = minz; hmz < maxz; hmz++) {
            for (unsigned int hmx = minx; hmx < maxx; hmx++) {
                ...
                const NodeLayer::SpeedBinType oldSpeedBin =
                  oldSpeedBins[sqrIdx];
                const NodeLayer::SpeedBinType curSpeedBin =
                  curSpeedBins[sqrIdx];
                ...
                speedModSum -= (oldSpeedMods[sqrIdx] /
                                float(NodeLayer::MaxSpeedModTypeValue()));
    ...
    moveCostAvg =
      (speedModAvg <= 0.001f) ? QTPFS_POSITIVE_INFINITY : (1.0f / speedModAvg);
    ...
    if (numClosedSquares > 0) {
        if (numClosedSquares < area()) {
            moveCostAvg = QTPFS_CLOSED_NODE_COST *
                          (numClosedSquares / float(xsize() * xsize()));
        } else {
                         ...
                         bool readMode)
{
    ...
    if (readMode) {
        fStream.read(reinterpret_cast<char*>(&nodeNumber),
                     sizeof(unsigned int));
        fStream.read(reinterpret_cast<char*>(&numChildren),
                     sizeof(unsigned int));
        ...
        fStream.write(reinterpret_cast<const char*>(&nodeNumber),
                      sizeof(unsigned int));
        fStream.write(reinterpret_cast<const char*>(&numChildren),
                      sizeof(unsigned int));

        fStream.write(reinterpret_cast<const char*>(&speedModAvg),
                      sizeof(float));
        fStream.write(reinterpret_cast<const char*>(&speedModSum),
                      sizeof(float));
        fStream.write(reinterpret_cast<const char*>(&moveCostAvg),
                      sizeof(float));
    }
        ...
        if (maxNgbs > 0) {
            ...
            if (xmin() > 0) {
                ...
                for (unsigned int hmz = zmin(); hmz < zmax();) {
                    ...
                    for (unsigned int i = 0;
                         i < QTPFS_MAX_NETPOINTS_PER_NODE_EDGE;
                         i++) {
                        netpoints.push_back(
                          INode::GetNeighborEdgeTransitionPoint(
                            ngb,
                            float3(),
                            QTPFS_NETPOINT_EDGE_SPACING_SCALE * (i + 1)));
                    }
                }
            ...
            if ((ngbRels & REL_NGB_EDGE_L) != 0) {
                if ((ngbRels & REL_NGB_EDGE_T) != 0) {
                    const INode* ngbL =
                      nodes[(zmin() + 0) * mapDims.mapx + (xmin() - 1)];
                    const INode* ngbT =
                      nodes[(zmin() - 1) * mapDims.mapx + (xmin() + 0)];
                    INode* ngbC =
                      nodes[(zmin() - 1) * mapDims.mapx + (xmin() - 1)];

                    // VERT_TL ngb must be distinct from EDGE_L and EDGE_T ngbs
                    if (ngbC != ngbL && ngbC != ngbT) {
                        if (ngbL->AllSquaresAccessible() &&
                            ngbT->AllSquaresAccessible()) {
                            ...
                            for (unsigned int i = 0;
                                ...
                                netpoints.push_back(
                                  INode::GetNeighborEdgeTransitionPoint(
                                    ngbC,
                                    float3(),
                                    QTPFS_NETPOINT_EDGE_SPACING_SCALE *
                                      (i + 1)));
                            }
                        }
                    }
                ...
                if ((ngbRels & REL_NGB_EDGE_B) != 0) {
                    const INode* ngbL =
                      nodes[(zmax() - 1) * mapDims.mapx + (xmin() - 1)];
                    const INode* ngbB =
                      nodes[(zmax() + 0) * mapDims.mapx + (xmin() + 0)];
                    INode* ngbC =
                      nodes[(zmax() + 0) * mapDims.mapx + (xmin() - 1)];

                    // VERT_BL ngb must be distinct from EDGE_L and EDGE_B ngbs
                    if (ngbC != ngbL && ngbC != ngbB) {
                        if (ngbL->AllSquaresAccessible() &&
                            ngbB->AllSquaresAccessible()) {
                            neighbors.push_back(ngbC);
            ...
            if ((ngbRels & REL_NGB_EDGE_R) != 0) {
                if ((ngbRels & REL_NGB_EDGE_T) != 0) {
                    const INode* ngbR =
                      nodes[(zmin() + 0) * mapDims.mapx + (xmax() + 0)];
                    const INode* ngbT =
                      nodes[(zmin() - 1) * mapDims.mapx + (xmax() - 1)];
                    INode* ngbC =
                      nodes[(zmin() - 1) * mapDims.mapx + (xmax() + 0)];

                    // VERT_TR ngb must be distinct from EDGE_R and EDGE_T ngbs
                    if (ngbC != ngbR && ngbC != ngbT) {
                        if (ngbR->AllSquaresAccessible() &&
                            ngbT->AllSquaresAccessible()) {
                            neighbors.push_back(ngbC);
                ...
                if ((ngbRels & REL_NGB_EDGE_B) != 0) {
                    const INode* ngbR =
                      nodes[(zmax() - 1) * mapDims.mapx + (xmax() + 0)];
                    const INode* ngbB =
                      nodes[(zmax() + 0) * mapDims.mapx + (xmax() - 1)];
                    INode* ngbC =
                      nodes[(zmax() + 0) * mapDims.mapx + (xmax() + 0)];
...
// NOTE: gcc 4.7 should FINALLY define __cplusplus properly (and accept
// -std=c++11)
