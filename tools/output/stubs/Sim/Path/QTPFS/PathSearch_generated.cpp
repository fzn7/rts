                              ...
                              const SRectangle& searchArea)
{
    ...
    srcPoint.ClampInBounds();
    tgtPoint = targetPoint;
    tgtPoint.ClampInBounds();
    ...
    srcNode =
      nodeLayer->GetNode(srcPoint.x / SQUARE_SIZE, srcPoint.z / SQUARE_SIZE);
    tgtNode =
      nodeLayer->GetNode(tgtPoint.x / SQUARE_SIZE, tgtPoint.z / SQUARE_SIZE);
    curNode = NULL;
    nxtNode = NULL;
    minNode = srcNode;
}
                           ...
                           unsigned int searchMagicNumber)
{
    ...
    switch (searchType) {
        case PATH_SEARCH_ASTAR: {
            hCostMult = 1.0f / nodeLayer->GetMaxRelSpeedMod();
        } break;
...
QTPFS::PathSearch::IterateNodes(const std::vector<INode*>& allNodes)
{
...
#ifdef QTPFS_TRACE_PATH_SEARCHES
    searchIter.SetPoppedNodeIdx(curNode->zmin() * mapDims.mapx +
                                curNode->xmin());
#endif
    ...
    if (curNode->xmid() < searchRect.x1)
        return;
    if (curNode->zmid() < searchRect.z1)
        return;
    if (curNode->xmid() > searchRect.x2)
        return;
    if (curNode->zmid() > searchRect.z2)
        return;

#ifdef QTPFS_SUPPORT_PARTIAL_SEARCHES
    // remember the node with lowest h-cost in case the search fails to reach
    // tgtNode
    if (curNode->GetPathCost(NODE_PATH_COST_H) <
        minNode->GetPathCost(NODE_PATH_COST_H))
        minNode = curNode;
#endif
...
QTPFS::PathSearch::IterateNodeNeighbors(const std::vector<INode*>& nxtNodes)
{
    ...
    for (unsigned int i = 0; i < nxtNodes.size(); i++) {
        // NOTE:
        //   this uses the actual distance that edges of the final path will
        //   cover, from <curPoint> (initialized to sourcePoint) to a position
        //   on the edge shared between <curNode> and <nxtNode> (each individual
        //   path-segment is weighted by the average move-cost of the node it
        //   crosses, which is the reciprocal of the average speed-mod)
        // NOTE:
        //   short paths that should have 3 points (2 nodes) can contain 4 (3
        //   nodes); this happens when a path takes a "detour" through a corner
        //   neighbor of srcNode if the shared corner vertex is closer to the
        //   goal position than any transition-point on the edge between srcNode
        //   and tgtNode
        // NOTE:
        //   H needs to be of the same order as G, otherwise the search reduces
        //   to Dijkstra (if G dominates H) or becomes inadmissable (if H
        //   dominates G) in the first case we would explore many more nodes
        //   than necessary (CPU nightmare), while in the second we would get
        //   low-quality paths (player nightmare)
        nxtNode = nxtNodes[i];
        ...
        const bool isClosed =
          ((nxtNode->GetSearchState() & 1) == NODE_STATE_CLOSED);
        ...
        /*if (!IntersectEdge(curNode, nxtNode, tgtPoint - curPoint))*/ {
            ...
            gCosts[0] = curNode->GetPathCost(NODE_PATH_COST_G) +
...
#ifdef QTPFS_TRACE_PATH_SEARCHES
            searchIter.AddPushedNodeIdx(nxtNode->zmin() * mapDims.mapx +
                                        nxtNode->xmin());
#endif
...
QTPFS::PathSearch::SmoothPathIter(IPath* path) const
{
    ...
    while (n1 != srcNode) {
        ...
        const bool hEdge = (((ngbRel & REL_NGB_EDGE_T) != 0) ||
                            ((ngbRel & REL_NGB_EDGE_B) != 0));
        const bool vEdge = (((ngbRel & REL_NGB_EDGE_L) != 0) ||
                            ((ngbRel & REL_NGB_EDGE_R) != 0));
...

        {
            ...
            const float dfx =
              (p2p0.x > 0.0f) ? ((n0->xmax() * SQUARE_SIZE) - p0.x) : // A(x)
                ((n0->xmin() * SQUARE_SIZE) - p0.x);                  // B(x)
            const float dfz =
              (p2p0.z > 0.0f) ? ((n0->zmax() * SQUARE_SIZE) - p0.z) : // A(z)
            ...
            ok = ok &&
                 (pi.x >= (xmin * SQUARE_SIZE) && pi.x <= (xmax * SQUARE_SIZE));
            ok = ok &&
                 (pi.z >= (zmin * SQUARE_SIZE) && pi.z <= (zmax * SQUARE_SIZE));
...
{
    return (srcNode->GetNodeNumber() + (tgtNode->GetNodeNumber() * N) +
            (k * N * N));
}
