...
CPlasmaRepulser::CPlasmaRepulser(CUnit* owner, const WeaponDef* def)
  : CWeapon(owner, def)
  , tempNum(0)
  , curPower(0.0f)
  , hitFrames(0)
...
bool
CPlasmaRepulser::IsRepulsing(CWeaponProjectile* p) const
{
    return weaponDef->shieldRepulser &&
           std::find(repulsedProjectiles.begin(),
                     repulsedProjectiles.end(),
                     p) != repulsedProjectiles.end();
}

bool
CPlasmaRepulser::IsActive() const
{
    return isEnabled && !owner->IsStunned() && !owner->beingBuilt;
}

bool
CPlasmaRepulser::HaveFreeLineOfFire(const float3 pos,
...
bool
CPlasmaRepulser::CanIntercept(unsigned interceptedType, int allyTeam) const
{
...
void
CPlasmaRepulser::Update()
{
    rechargeDelay -= (rechargeDelay > 0) ? 1 : 0;

    if (IsActive() && (curPower < weaponDef->shieldPower) &&
        rechargeDelay <= 0) {
        if (owner->UseEnergy(weaponDef->shieldPowerRegenEnergy *
                             (1.0f / GAME_SPEED))) {
...
bool
CPlasmaRepulser::IncomingProjectile(CWeaponProjectile* p)
{
    ...
    if (eventHandler.ShieldPreDamaged(
          p, this, owner, weaponDef->shieldRepulser, nullptr, nullptr))
        return false;

    const DamageArray& damageArray =
      p->damages->GetDynamicDamages(p->GetStartPos(), p->pos);
    ...
    if (weaponDef->shieldRepulser) {
        ...
        const int type = p->ShieldRepulse(
          weaponMuzzlePos, weaponDef->shieldForce, weaponDef->shieldMaxSpeed);
        ...
        if (type == 0) {
            ...
            // FIXME why do all weapons except LASERs do only (1 / GAME_SPEED)
            // damage???
        ...
        const bool newRepulsed =
          VectorInsertUnique(repulsedProjectiles, p, true);
        if (newRepulsed) {
            ...
            if (weaponDef->visibleShieldRepulse) {
                ...
                const float colorMix = std::min(
                  1.0f, curPower / std::max(1.0f, weaponDef->shieldPower));
                const float3 color =
...
bool
CPlasmaRepulser::IncomingBeam(const CWeapon* emitter,
                              const float3& start,
                              float damageMultiplier)
{
    // gadget handles the collision event, don't touch the projectile
    if (eventHandler.ShieldPreDamaged(nullptr,
                                      this,
                                      owner,
                                      weaponDef->shieldRepulser,
                                      emitter,
                                      emitter->owner))
        return false;

    const DamageArray& damageArray =
      emitter->damages->GetDynamicDamages(start, weaponMuzzlePos);
    const float shieldDamage = damageArray.Get(weaponDef->shieldArmorType);
...
void
CPlasmaRepulser::DependentDied(CObject* o)
{
