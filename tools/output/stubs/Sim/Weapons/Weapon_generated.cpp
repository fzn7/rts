...
CWeapon::CobBlockShot() const
{
    ...
    return owner->script->BlockShot(
      weaponNum, currentTarget.unit, currentTarget.isUserTarget);
}
...
CWeapon::UpdateWeaponPieces(const bool updateAimFrom)
{
    ...
    if (aimExists && muzExists) {
        ...
        if (!alreadyWarnedAboutMissingPieces &&
            (owner->script != &CNullUnitScript::value) &&
            !weaponDef->isShield &&
            (dynamic_cast<CNoWeapon*>(this) == nullptr)) {
            LOG_L(L_WARNING,
                  "%s: weapon%i: Neither AimFromWeapon nor QueryWeapon defined "
                  "or returned invalid pieceids",
...
CWeapon::UpdateWeaponVectors()
{
    ...
    // hope that we are underground because we are a popup weapon and will come
    // above ground later
...
CWeapon::Update()
{
    // update conditional cause last SlowUpdate maybe longer away than
    // UNIT_SLOWUPDATE_RATE i.e. when the unit got stunned (neither is
    // SlowUpdate exactly called at UNIT_SLOWUPDATE_RATE, it's only called
    // `close` to that)
    ...
    // esp. for bombs this is often too late (SlowUpdate gets only called twice
    // per second) so check unit's target this check every frame (unit target
    // has highest priority even in SlowUpdate!)
    ...
    tracefile << aimFromPos.x << " " << aimFromPos.y << " " << aimFromPos.z
              << " " << currentTargetPos.x << " " << currentTargetPos.y << " "
              << currentTargetPos.z << "\n";
#endif
}
...
CWeapon::UpdateAim()
{
    ...
    const float3 worldTargetDir =
      (currentTargetPos - owner->pos).SafeNormalize();
    const float3 worldMainDir = owner->GetObjectSpaceVec(mainDir);
    const bool targetAngleConstraint =
      CheckTargetAngleConstraint(worldTargetDir, worldMainDir);
...
CWeapon::ReAimWeapon()
{
    ...
    const float pitch =
      math::asin(Clamp(wantedDir.dot(owner->updir), -1.0f, 1.0f));

    // for COB, this sets <angleGood> to return value of AimWeapon when
    // finished, for LUS, there exists a callout to set the <angleGood> member
    // directly.
    // FIXME: convert CSolidObject::heading to radians too.
    owner->script->AimWeapon(
      weaponNum, ClampRad(heading - owner->heading * TAANG2RAD), pitch);
}
                 ...
                 bool ignoreRequestedDir) const
{
    ...
    if (weaponDef->maxFireAngle > -1.0f) {
        if (!ignoreRequestedDir &&
            wantedDir.dot(lastRequestedDir) <= weaponDef->maxFireAngle)
            return false;
    }

    // if in FPS mode, player must be pressing at least one button to fire
    const CPlayer* fpsPlayer = owner->fpsControlPlayer;
    if (fpsPlayer != NULL && !fpsPlayer->fpsController.mouse1 &&
...
CWeapon::UpdateFire()
{
    ...
    // pre-check if we got enough resources (so CobBlockShot gets only called
    // when really possible to shoot)
    ...
    if (!weaponDef->stockpile) {
        ...
        if (!owner->UseResources(shotRes)) {
            // not enough resource, update pull (needs factor cause called each
            // ::Update() and not at reloadtime!)
            const int minPeriod =
              std::max(1, (int)(reloadTime / owner->reloadSpeed));
            const float averageFactor = 1.0f / minPeriod;
            ownerTeam->resPull.energy +=
              (averageFactor * weaponDef->energycost);
    ...
    salvoError =
      gs->randVector() *
      (owner->IsMoving() ? weaponDef->movingAccuracy : accuracyError);

    if (currentTarget.type == Target_Pos ||
        (currentTarget.type == Target_Unit &&
         !(currentTarget.unit->losStatus[owner->allyteam] & LOS_INLOS))) {
...
CWeapon::UpdateStockpile()
{
    ...
    if (numStockpileQued > 0) {
        ...
        auto res =
          SResourcePack(weaponDef->metalcost * p, weaponDef->energycost * p);
...
CWeapon::UpdateSalvo()
{
    ...
    const bool searchForNewTarget =
      (salvoLeft == 0) && (currentTarget == owner->curTarget);
...
{
    const int checkAllowed =
      eventHandler.AllowWeaponTargetCheck(owner->id, weaponNum, weaponDef->id);
    if (checkAllowed >= 0) {
        return checkAllowed;
    }
    ...
    if (currentTarget.type == Target_Unit) {
        if (!TryTarget(
              SWeaponTarget(currentTarget.unit, currentTarget.isUserTarget))) {
...
CWeapon::AutoTarget()
{
    ...
    const CUnit* avoidUnit = (avoidTarget && currentTarget.type == Target_Unit)
                               ? currentTarget.unit
                               : nullptr;

    // NOTE:
    //   sorts by INCREASING order of priority, so lower equals better
    //   <targets> is normally sorted such that all bad TargetCategory units are
    //   at the end, but Lua can mess with the ordering arbitrarily
...
{
    errorVectorAdd =
      (gs->randVector() - errorVector) * (1.0f / UNIT_SLOWUPDATE_RATE);
    predictSpeedMod =
      1.0f + (gs->randFloat() - 0.5f) * 2 * (1.0f - owner->limExperience);
    ...
    if (slavedTo != NULL) {
        ...
        // If unit got an attack target, clone the job (independent of
        // AutoTarget!) Also do this unconditionally (owner's target always has
        // priority over weapon one!)
        Attack(owner->curTarget);
    } else if (!HaveTarget() && owner->lastAttacker != nullptr &&
               owner->fireState == FIRESTATE_RETURNFIRE) {
        // Try to return fire
...
CWeapon::DependentDied(CObject* o)
{
    ...
    // NOTE: DependentDied is called from ~CObject-->Detach, object is just
    // barely valid
...
CWeapon::TargetUnderWater(const float3 tgtPos, const SWeaponTarget& target)
{
    switch (target.type) {
        case Target_None:
            return false;
        case Target_Unit:
            return target.unit->IsUnderWater();
        case Target_Pos:
            return (tgtPos.y <
                    0.0f); // consistent with CSolidObject::IsUnderWater (LT)
        case Target_Intercept:
            return (target.intercept->pos.y < 0.0f);
        default:
            return false;
    }
}
...
CWeapon::TargetInWater(const float3 tgtPos, const SWeaponTarget& target)
{
    switch (target.type) {
        case Target_None:
            return false;
        case Target_Unit:
            return target.unit->IsInWater();
        case Target_Pos:
            return (tgtPos.y <=
                    0.0f); // consistent with CSolidObject::IsInWater (LE)
        case Target_Intercept:
            return (target.intercept->pos.y <= 0.0f);
        default:
            return false;
                            ...
                            const float3 rawTargetDir) const
{
    ...
    if (CCollisionHandler::DetectHit(targetUnit,
                                     &tmpColVol,
                                     targetUnit->GetTransformMatrix(true),
                                     weaponMuzzlePos,
                                     ZeroVector,
                                     NULL)) {
        ...
        // aimpoint
        const float3 targetOffset =
          rawTargetDir * (tmpColVol.GetBoundingRadius() * 2.0f);
        ...
        if (CCollisionHandler::DetectHit(targetUnit,
                                         ...
                                         targetRayPos,
                                         &tmpColQry)) {
            targetBorderPos = (weaponDef->targetBorder > 0.0f)
                                ? tmpColQry.GetIngressPos()
                                : tmpColQry.GetEgressPos();
        }
    }
                   ...
                   bool preFire) const
{
    ...
    // FIXME add a forcedUserTarget (a forced fire mode enabled with ctrl key or
    // something) and skip the tests below then
...
{
    if ((trg.isManualFire != weaponDef->manualfire) &&
        owner->unitDef->canManualFire)
        return false;
    ...
    switch (trg.type) {
        ...
        case Target_Unit: {
            ...
            if (trg.unit->IsCrashing() && modInfo.fireAtCrashing == 0)
                return false;
            if ((trg.unit->losStatus[owner->allyteam] &
                 (LOS_INLOS | LOS_INRADAR)) == 0)
                return false;
            if (!trg.isUserTarget && trg.unit->IsNeutral() &&
                owner->fireState < FIRESTATE_FIREATNEUTRAL)
                return false;

            // don't fire at allied targets
            if (!trg.isUserTarget &&
                teamHandler->Ally(owner->allyteam, trg.unit->allyteam))
                return false;

            if (trg.unit->GetTransporter() != NULL) {
                if (!modInfo.targetableTransportedUnits)
                    return false;
                // the transportee might be "hidden" below terrain, in which
                // case we can't target it
                if (trg.unit->pos.y <
                    CGround::GetHeightReal(trg.unit->pos.x, trg.unit->pos.z))
                    return false;
            }
        } break;
        case Target_Pos: {
    ...
    if (!weaponDef->waterweapon) {
        // we cannot pick targets underwater, check where target is in relation
        // to us
        if (!owner->IsUnderWater() && TargetUnderWater(tgtPos, trg))
            return false;
        // if we are underwater but target is *not* in water, fireSubmersed gets
        // checked
...
CWeapon::TestRange(const float3 tgtPos, const SWeaponTarget& trg) const
{
    ...
    float weaponRange =
      0.0f; // range modified by heightDiff and cylinderTargeting
    ...
    if (trg.type == Target_Pos || weaponDef->cylinderTargeting < 0.01f) {
        ...
        if ((weaponDef->cylinderTargeting * range) >
            (math::fabsf(heightDiff) * weaponDef->heightmod)) {
                            ...
                            bool useMuzzle) const
{
    ...
    if ((avoidFlags & Collision::NOGROUND) == 0) {
        // NOTE:
        //     ballistic weapons (Cannon / Missile icw. trajectoryHeight) do not
        //     call this, they use TrajectoryGroundCol with an external check
        //     for the NOGROUND flag
        CUnit* unit = nullptr;
        CFeature* feature = nullptr;

        const float gdst = TraceRay::TraceRay(
          testPos, dir, length, ~Collision::NOGROUND, owner, unit, feature);
        const float3 gpos = testPos + dir * gdst;

        // true iff ground does not block the ray of length <length> from <pos>
        // along <dir>
        if ((gdst > 0.0f) &&
            (gpos.SqDistance(pos) > Square(damages->damageAreaOfEffect)))
            return false;
    }

    // friendly, neutral & feature check
    if (TraceRay::TestCone(
          testPos, dir, length, spread, owner->allyteam, avoidFlags, owner))
        return false;
...
CWeapon::TryTargetRotate(const CUnit* unit, bool userTarget, bool manualFire)
{
    ...
    const short enemyHeading = GetHeadingFromVector(
      tempTargetPos.x - aimFromPos.x, tempTargetPos.z - aimFromPos.z);
...
CWeapon::TryTargetRotate(float3 pos, bool userTarget, bool manualFire)
{
    ...
    const short weaponHeading = GetHeadingFromVector(mainDir.x, mainDir.z);
    const short enemyHeading =
      GetHeadingFromVector(pos.x - aimFromPos.x, pos.z - aimFromPos.z);
...
CWeapon::Init()
{
    ...
    muzzleFlareSize =
      std::min(damages->damageAreaOfEffect * 0.2f,
               std::min(1500.f, damages->GetDefault()) * 0.003f);
    ...
    if (weaponDef->isShield) {
        if ((owner->shieldWeapon == NULL) ||
            (owner->shieldWeapon->weaponDef->shieldRadius <
             weaponDef->shieldRadius)) {
...
CWeapon::Fire(bool scriptCall)
{
    ...
    tracefile << owner->pos.x << " " << owner->frontdir.x << " "
              << currentTargetPos.x << " " << currentTargetPos.y << " "
              << currentTargetPos.z;
    tracefile << sprayAngle << " "
              << " " << salvoError.x << " " << salvoError.z << " "
              << owner->limExperience << " " << projectileSpeed << "\n";
#endif
    ...
    if (fireSoundId > 0 &&
        (!weaponDef->soundTrigger || salvoLeft == salvoSize - 1)) {
...
CWeapon::UpdateInterceptTarget()
{
    ...
    if (currentTarget.type == Target_Intercept) {
        minInterceptTargetDistSq =
          aimFromPos.SqDistance(currentTarget.intercept->pos);
    }

    for (std::map<int, CWeaponProjectile*>::iterator pi =
           incomingProjectiles.begin();
         pi != incomingProjectiles.end();
         ++pi) {
        ...
        if (weaponDef->interceptSolo &&
            p->IsBeingIntercepted()) // FIXME add bad target?
            continue;
...
{
    if ((currentTarget.type == Target_Unit) &&
        currentTarget.unit->allyteam == ally) {
        DropCurrentTarget();
    }
}
...
CWeapon::AdjustTargetPosToWater(float3& tgtPos, bool attackGround) const
{
    ...
    if (owner->UnderFirstPersonControl() &&
        dynamic_cast<const CCannon*>(this)) {
...
CWeapon::GetUnitPositionWithError(const CUnit* unit) const
{
    ...
    const float errorScale =
      (MoveErrorExperience() * GAME_SPEED * unit->speed.w);
...
{
    const float3 tmpTargetPos =
      GetUnitPositionWithError(unit) + GetLeadVec(unit);
    ...
    aimPos.y = std::max(
      aimPos.y, CGround::GetApproximateHeight(aimPos.x, aimPos.z) + 2.0f);
...
CWeapon::GetLeadVec(const CUnit* unit) const
{
    ...
    float3 lead = unit->speed * predict *
                  mix(predictSpeedMod, 1.0f, weaponDef->predictBoost);
    if (weaponDef->leadLimit >= 0.0f) {
        const float leadBonus =
          weaponDef->leadLimit + weaponDef->leadBonus * owner->experience;
...
CWeapon::ExperienceErrorScale() const
{
    ...
    // (lower is better)
    //
    //   for accWeight=0.00 and {0.25, 0.50, 0.75, 1.0} exp, scale=(1.0 -
    //   {0.25*0.00, 0.5*0.00, 0.75*0.00, 1.0*0.00}) = {1.0000, 1.000,
    //   1.0000, 1.00} for accWeight=0.25 and {0.25, 0.50, 0.75, 1.0} exp,
    //   scale=(1.0 - {0.25*0.25, 0.5*0.25, 0.75*0.25, 1.0*0.25}) = {0.9375,
    //   0.875, 0.8125, 0.75} for accWeight=0.50 and {0.25, 0.50, 0.75, 1.0}
    //   exp, scale=(1.0 - {0.25*0.50, 0.5*0.50, 0.75*0.50, 1.0*0.50}) =
    //   {0.8750, 0.750, 0.6250, 0.50} for accWeight=1.00 and {0.25, 0.50,
    //   0.75, 1.0} exp, scale=(1.0 - {0.25*1.00, 0.5*1.00,
    //   0.75*1.00, 1.0*0.75}) = {0.7500, 0.500, 0.2500, 0.25}
    return (CUnit::ExperienceScale(owner->limExperience,
...
CWeapon::GetLeadTargetPos(const SWeaponTarget& target) const
{
    switch (target.type) {
        ...
        case Target_Unit:
            return GetUnitLeadTargetPos(target.unit);
        case Target_Pos: {
        ...
        case Target_Intercept:
            return target.intercept->pos + target.intercept->speed;
    }
