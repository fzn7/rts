...
float
CRifle::GetPredictedImpactTime(float3 p) const
{
    return 0;
}
...
CRifle::FireImpl(const bool scriptCall)
{
    ...
    const float length = TraceRay::TraceRay(
      weaponMuzzlePos, dir, range, 0, owner, hitUnit, hitFeature);
    const float impulse = CGameHelper::CalcImpulseScale(*damages, 1.0f);

    if (hitUnit != NULL) {
        hitUnit->DoDamage(*damages, dir * impulse, owner, weaponDef->id, -1);
        new CHeatCloudProjectile(
          owner, weaponMuzzlePos + dir * length, hitUnit->speed * 0.9f, 30, 1);
        ...
        new CHeatCloudProjectile(owner,
                                 ...
                                 hitFeature->speed * 0.9f,
                                 30,
                                 1);
    }

    new CTracerProjectile(
      owner, weaponMuzzlePos, dir * projectileSpeed, length);
    new CSmokeProjectile(
      owner, weaponMuzzlePos, ZeroVector, 70, 0.1f, 0.02f, 0.6f);
}
