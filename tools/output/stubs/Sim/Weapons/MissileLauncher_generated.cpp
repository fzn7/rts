...
void
CMissileLauncher::UpdateWantedDir()
{
...
CMissileLauncher::FireImpl(const bool scriptCall)
{
    ...
    params.ttl = weaponDef->flighttime == 0
                   ? std::ceil(std::max(dist, range) / projectileSpeed +
                               25 * weaponDef->selfExplode)
    ...
    const float groundDist =
      ((avoidFlags & Collision::NOGROUND) == 0)
        ? CGround::TrajectoryGroundCol(
            weaponMuzzlePos, flatDir, flatLength, linear, quadratic)
        : -1.0f;
