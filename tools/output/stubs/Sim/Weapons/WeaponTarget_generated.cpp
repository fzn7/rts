...
SWeaponTarget::SWeaponTarget(const CUnit* u, bool userTarget)
  ...
  , isManualFire(false)
  , unit(const_cast<CUnit*>(u)) // XXX evil but better than having two structs
                                // for const & non-const version
  , intercept(nullptr)
{}
...
bool
SWeaponTarget::operator!=(const SWeaponTarget& other) const
{
    if (type != other.type)
        return true;
    ...
    switch (type) {
        ...
        case Target_Pos:
            return (groundPos != other.groundPos);
