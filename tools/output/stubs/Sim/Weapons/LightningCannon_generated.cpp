...
CLightningCannon::FireImpl(const bool scriptCall)
{
    ...
    float boltLength = TraceRay::TraceRay(curPos,
                                          curDir,
                                          range,
                                          collisionFlags,
                                          owner,
    ...
    const DamageArray& damageArray =
      damages->GetDynamicDamages(weaponMuzzlePos, currentTargetPos);
    const CExplosionParams params = {
        curPos + curDir * boltLength, // hitPos (same as hitColQuery.GetHitPos()
                                      // if no water or shield in way)
        curDir,
        damageArray,
        weaponDef,
        owner,
