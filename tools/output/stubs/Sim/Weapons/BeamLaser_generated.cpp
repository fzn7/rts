                                 ...
                                 const float3& muzzlePos)
{
    ...
    sweepStartAngle =
      math::acosf(Clamp(sweepInitDir.dot(sweepGoalDir), -1.0f, 1.0f));
    sweepFiring = true;
}
...
CBeamLaser::SweepFireState::GetTargetDist2D() const
{
    ...
    const float sweepCurAngleRad =
      math::acosf(Clamp(sweepCurAngleCos, -1.0f, 1.0f));

    // goes from 1 to 0 as the angular difference decreases during the sweep
    const float sweepAngleAlpha =
      (Clamp(sweepCurAngleRad / sweepStartAngle, 0.0f, 1.0f));
  ...
  , salvoDamageMult(1.0f)
{
    ...
    sweepFireState.SetDamageAllies((collisionFlags & Collision::NOFRIENDLIES) ==
                                   0);
}
...
CBeamLaser::Init()
{
    if (!weaponDef->beamburst) {
        ...
        // multiply damage with this on each shot so the total damage done is
        // correct
...
CBeamLaser::UpdatePosAndMuzzlePos()
{
    if (sweepFireState.IsSweepFiring()) {
        ...
        const float3 newWeaponDir = owner->GetObjectSpaceVec(
          float3(weaponMat[2], weaponMat[6], weaponMat[10]));
        ...
        if (weaponDef->sweepFire) {
            // needed for first call to GetFireDir() when new sweep starts after
            // inactivity
            sweepFireState.SetSweepTempDir(
              (weaponMuzzlePos - aimFromPos).SafeNormalize());
        }
    }
}
...
CBeamLaser::GetPredictedImpactTime(float3 p) const
{
    if (!weaponDef->beamburst) {
        ...
        // beamburst tracks the target during the burst so there's no need to
        // lead
        return 0;
    }
}
...
CBeamLaser::UpdateSweep()
{
    ...
    if (teamHandler->Team(owner->team)->res.metal < weaponDef->metalcost) {
        return;
    }
    if (teamHandler->Team(owner->team)->res.energy < weaponDef->energycost) {
        return;
    }
    ...
    if (!sweepFire) {
        if (scriptCall) {
            ...
            if (onlyForward && owner->unitDef->IsStrafingAirUnit()) {
                // [?] StrafeAirMovetype cannot align itself properly, change
            ...
            if ((currentTargetPos - weaponMuzzlePos)
                  .dot(currentTargetPos - owner->aimPos) < 0.0f) {
                dir = -dir;
            }
        }
    } else {
        ...
        const float3 tgtPos = float3(dir.x * sweepFireState.GetTargetDist2D(),
                                     0.0f,
                                     dir.z * sweepFireState.GetTargetDist2D());
        const float tgtHgt = CGround::GetHeightReal(
          weaponMuzzlePos.x + tgtPos.x, weaponMuzzlePos.z + tgtPos.z);
...
CBeamLaser::FireInternal(float3 curDir)
{
    ...
    if (!sweepFireState.IsSweepFiring()) {
        ...
        if (weaponDef->cylinderTargeting > 0.01f) {
            const float verticalDist =
              owner->radius * weaponDef->cylinderTargeting * curDir.y;
            const float maxLengthModSq =
              maxLength * maxLength + verticalDist * verticalDist;
    ...
    for (int tries = 0; tries < 5 && tryAgain; ++tries) {
        ...
        if (hitUnit != NULL &&
            teamHandler->AlliedTeams(hitUnit->team, owner->team)) {
            if (sweepFireState.IsSweepFiring() &&
                !sweepFireState.DamageAllies()) {
                doDamage = false;
                break;
            }
        }
        ...
        if (!weaponDef->waterweapon) {
            ...
            if ((curDir.y < 0.0f) &&
                ((curPos.y + curDir.y * beamLength) <= 0.0f)) {
        ...
        for (const TraceRay::SShieldDist& sd : hitShields) {
            if (sd.dist < beamLength &&
                sd.rep->IncomingBeam(this, curPos, salvoDamageMult)) {
                beamLength = sd.dist;
                hitUnit = NULL;
                hitFeature = NULL;
                hitShield = sd.rep;
        ...
        if (hitShield != nullptr && hitShield->weaponDef->shieldRepulser) {
            const float3 normal =
              (hitPos - hitShield->weaponMuzzlePos).Normalize();
...

        {
            ...
            const float endAlpha =
              (1.0f - (curLength + beamLength) / maxLength);
    ...
    if (curLength < maxLength) {
        // make it possible to always hit with some minimal intensity (melee
        // weapons have use for that)
        const float hitIntensity = std::max(
          weaponDef->minIntensity, 1.0f - curLength / (actualRange * 2.0f));

        const DamageArray& baseDamages =
          damages->GetDynamicDamages(weaponMuzzlePos, curPos);
