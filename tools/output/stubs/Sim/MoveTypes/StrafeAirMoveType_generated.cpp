                     ...
                     bool attacking)
{
    ...
    if (attacking) {
        ...
        const float minPredictedHeight =
          pos.y + spd.y * 60.0f * math::fabs(frontdir.y) +
          std::min(0.0f, updir.y * 1.0f) * (GAME_SPEED * 5);
        const float maxPredictedHeight =
          groundHeight + 60.0f + math::fabs(rightdir.y) * (GAME_SPEED * 5);
        ...
        if (spd.w > 1.5f &&
            ((pos.y + spd.y * 10.0f) > (groundHeight + wantedHeight * 0.6f))) {
            ...
            if (goalBankDif > maxAileronSpeedf && rightdir.y > -maxBank) {
                aileron = 1.0f;
            } else if (goalBankDif < -maxAileronSpeedf &&
                       rightdir.y < maxBank) {
                aileron = -1.0f;
            } else {
                if (math::fabs(rightdir.y) < maxBank &&
                    maxAileronSpeedf > 0.0f) {
            ...
            if (goalDotRight < -maxRudderSpeedf) {
                ...
                if (maxRudderSpeedf > 0.0f) {
                    ...
                    if (math::fabs(rudder) < 0.01f && aGoalDotFront < 0.0f) {
                        // target almost straight behind us, we have to choose a
                        // direction
                      ...
                      bool attacking)
{
    ...
    if (attacking) {
        if (spd.w < 1.5f) {
            ...
            const float gHeightR = CGround::GetHeightAboveWater(
              pos.x + spd.x * 40.0f, pos.z + spd.z * 40.0f);
            const float hdif = std::max(gHeightR, groundHeight) + 60 - pos.y -
                               frontdir.y * spd.w * 20.0f;

            const float maxElevatorSpeedf = maxElevator * spd.w;
            const float maxElevatorSpeedf2 = maxElevatorSpeedf * spd.w * 20.0f;

            float minPitch =
              1.0f; // min(1.0f, hdif / (maxElevator * spd.w * spd.w * 20));
            ...
            if (collidee != NULL && avoidCollision &&
                frontdir.dot(collidee->pos + collidee->speed * 20.0f - pos -
                             spd * 20.0f) < 0.0f) {
                elevator =
                  (updir.dot(collidee->midPos - owner->midPos) > 0.0f) ? -1 : 1;
            } else {
        ...
        if (spd.w > 0.8f) {
            ...
            if (notColliding) {
                const float maxElevatorSpeedf =
                  maxElevator * 20.0f * spd.w * spd.w;
                const float gHeightAW = CGround::GetHeightAboveWater(
                  pos.x + spd.x * 40.0f, pos.z + spd.z * 40.0f);
                const float hdif = std::max(groundHeight, gHeightAW) +
                                   wantedHeight - pos.y -
                                   frontdir.y * spd.w * 20.0f;
...
CStrafeAirMoveType::Update()
{
    ...
    if ((owner->IsStunned() && !owner->IsCrashing()) || owner->beingBuilt) {
        UpdateAirPhysics(0.0f * lastRudderPos,
                         lastAileronPos,
                         lastElevatorPos,
                         0.0f,
                         ZeroVector);
        return (HandleCollisions(collide && !owner->beingBuilt &&
    ...
    if (aircraftState != AIRCRAFT_CRASHING) {
        if (owner->UnderFirstPersonControl()) {
            ...
            const FPSUnitController& con =
              owner->fpsControlPlayer->fpsController;
    ...
    switch (aircraftState) {
        case AIRCRAFT_FLYING: {
            ...
            const bool isAttacking =
              (!cmdQue.empty() && (cmdQue.front()).GetID() == CMD_ATTACK);
            const bool keepAttacking = ((owner->curTarget.type == Target_Unit &&
                                         !owner->curTarget.unit->isDead) ||
                                        owner->curTarget.type == Target_Pos);

            /*
            const float brakeDistSq = Square(0.5f * lastSpd.SqLength2D() /
            decRate); const float goalDistSq = (goalPos - lastPos).SqLength2D();

            if (brakeDistSq >= goalDistSq &&
            !owner->commandAI->HasMoreMoveCommands()) {
            ...
            */
            {
                if (isAttacking && keepAttacking) {
                    ...
                    const bool goalInFront =
                      ((goalPos - lastPos).dot(owner->frontdir) > 0.0f);
                    const bool goalInRange = (goalPos.SqDistance(lastPos) <
                                              Square(owner->maxRange * 4.0f));
                    ...
                    if (maneuverState != MANEUVER_FLY_STRAIGHT) {
                        ...
                        if (UpdateFlying(wantedHeight, 1.0f) && !goalInFront &&
                            loopbackAttack) {
                            // once yaw and roll are unblocked, semi-randomly
                            // decide to turn or loop
                            const SyncedFloat3& rightdir = owner->rightdir;
                            const SyncedFloat3& frontdir = owner->frontdir;

                            const float altitude =
                              CGround::GetHeightAboveWater(owner->pos.x,
                                                           owner->pos.z) -
                              lastPos.y;
                            ...
                            if ((maneuverState = SelectLoopBackManeuver(
                                   ...
                                   lastSpd,
                                   turnRadius,
                                   altitude)) == MANEUVER_IMMELMAN_INV) {
                                maneuverSubState = 0;
                            }
                        }
                    }
        ...
        case AIRCRAFT_CRASHING: {
            ...
            UpdateAirPhysics(
              crashRudder, crashAileron, crashElevator, 0.0f, owner->frontdir);

            if ((CGround::GetHeightAboveWater(owner->pos.x, owner->pos.z) +
                 5.0f + owner->radius) > owner->pos.y) {
                owner->ClearPhysicalStateBit(CSolidObject::PSTATE_BIT_CRASHING);
                owner->KillUnit(NULL, true, false);
            }

            new CSmokeProjectile(owner,
                                 owner->midPos,
                                 gs->randVector() * 0.08f,
                                 100 + gs->randFloat() * 50,
                                 5,
...
CStrafeAirMoveType::HandleCollisions(bool checkCollisions)
{
    ...
    if (lastColWarningType == 1) {
        const int g =
          geometricObjects->AddLine(pos, lastColWarning->pos, 10, 1, 1);
        geometricObjects->SetColor(g, 0.2f, 1, 0.2f, 0.6f);
    } else if (lastColWarningType == 2) {
        const int g =
          geometricObjects->AddLine(pos, lastColWarning->pos, 10, 1, 1);
        if (owner->frontdir.dot(lastColWarning->midPos +
                                lastColWarning->speed * 20.0f - owner->midPos -
                                spd * 20.0f) < 0) {
    ...
    if (pos != oldPos) {
        ...
        if (checkCollisions) {
            ...
            const std::vector<CUnit*> nearUnits =
              quadField->GetUnitsExact(pos, owner->radius + 6);

            for (CUnit* unit : nearUnits) {
                const bool unloadingUnit =
                  (unit->unloadingTransportId == owner->id);
                const bool unloadingOwner =
                ...
                if (unit->id == owner->loadingTransportId ||
                    owner->id == unit->loadingTransportId ||
...
CStrafeAirMoveType::UpdateManeuver()
{
    ...
    switch (maneuverState) {
        case MANEUVER_IMMELMAN: {
            ...
            if (math::fabs(owner->rightdir.y) < maxAileron * 3.0f * speedf ||
                owner->updir.y < 0.0f)
                elevator = 1.0f;

            UpdateAirPhysics(0.0f, aileron, elevator, 1.0f, owner->frontdir);

            if ((owner->updir.y < 0.0f && owner->frontdir.y < 0.0f) ||
                speedf < 0.8f) {
                maneuverState = MANEUVER_FLY_STRAIGHT;
            }
            // [?] some seem to report that the "unlimited altitude" thing is
            // because of these maneuvers
            if ((owner->pos.y -
                 CGround::GetApproximateHeight(owner->pos.x, owner->pos.z)) >
                (wantedHeight * 4.0f)) {
                maneuverState = MANEUVER_FLY_STRAIGHT;
            }
        } break;
...
CStrafeAirMoveType::UpdateAttack()
{
    ...
    const float3 goalDir =
      (goalDist > 0.0f) ? (goalPos - pos) / goalDist : ZeroVector;

    // if goal too close, stop dive and resume flying at normal desired height
    // to avoid colliding with target, evade blast, friendly and enemy fire,
    // etc.
...

    {
        ...
        const float aileron = GetAileronDeflection(owner,
                                                   lastColWarning,
                                                   pos,
                                                   spd,
                                                   rightdir,
                                                   ...
                                                   true); // roll
        const float rudder = GetRudderDeflection(owner,
                                                 lastColWarning,
                                                 pos,
                                                 spd,
                                                 rightdir,
                                                 ...
                                                 lastColWarningType == 2,
                                                 true); // yaw
        const float elevator = GetElevatorDeflection(owner,
                                                     lastColWarning,
                                                     pos,
                                                     spd,
                                                     rightdir,
                                                     ...
                                                     true); // pitch
        const float engine =
          ((attackee == NULL) || attackee->unitDef->IsGroundUnit())
            ? 1.0f
            : std::min(1.0f,
                       (goalDist / owner->maxRange + 1.0f -
                        goalDir.dot(frontdir) * 0.7f));
...
CStrafeAirMoveType::UpdateFlying(float wantedHeight, float engine)
{
    ...
    const bool allowUnlockYawRoll =
      (goalDist2D >= TurnRadius(turnRadius, spd.w) ||
       goalVec.dot(owner->frontdir) > 0.0f);
    const bool forceUnlockYawRoll =
      ((gs->frameNum - owner->lastFireWeapon) >= GAME_SPEED * 3);
    ...
    float3 rightDir2D = rightdir;
    float3 yprMults =
      (XZVector * float(allowUnlockYawRoll || forceUnlockYawRoll)) + UpVector;
    ...
    if ((goalDist2D < turnRadius * 0.5f && goalDir2D.dot(frontdir) < 0.7f) ||
    ...
    if (lastColWarning != NULL) {
        ...
        const float otherThreat =
          (otherLength > 0.0f)
            ? std::max(1200.0f, goalDist2D) / otherLength * 0.036f
            : 0.0f;

        const float3 otherDir =
          (otherLength > 0.0f) ? (otherDif / otherLength) : ZeroVector;
    ...
    UpdateAirPhysics(rudder * yprMults.x,
                     aileron * yprMults.z,
                     elevator * yprMults.y,
                     engine,
                     owner->frontdir);
...
CStrafeAirMoveType::UpdateTakeOff()
{
    ...
    const float currentHeight =
      pos.y - (owner->unitDef->canSubmerge
    ...
    owner->SetVelocity(spd + (UpVector * accRate *
                              GetVTOLAccelerationSign(
                                currentHeight, wantedHeight, spd.y, true)));
...
CStrafeAirMoveType::UpdateLanding()
{
    ...
    const float3 brakeSpot =
      pos + frontdir * BrakingDistance(owner->speed.Length2D(), decRate);
    ...
    if (rightdir.dot(reservedLandingPosDir) > 0.01f) {
        frontdir += (rightdir * 0.02f);
    } else if (rightdir.dot(reservedLandingPosDir) < -0.01f) {
        frontdir -= (rightdir * 0.02f);
    }

    {
        ...
        if (landPosDistXZ > curSpeedXZ && curSpeedXZ > 0.1f) {
            owner->SetVelocity(
              spd +
              UpVector * std::max(-altitudeRate,
                                  landPosDistY * curSpeedXZ / landPosDistXZ));
        } else {
            const float localAltitude =
              pos.y -
              (owner->unitDef->canSubmerge
            ...
            if (localAltitude > wantedHeight) {
                owner->SetVelocity(
                  spd + UpVector * std::max(-altitudeRate,
                                            wantedHeight - localAltitude));
            }
        }
                                     ...
                                     const float3& engineThrustVector)
{
    ...
    if (owner->UnderFirstPersonControl()) {
        if ((pos.y - gHeight) > wantedHeight * 1.2f) {
            engine =
              std::max(0.0f,
                       std::min(engine,
                                1 - (pos.y - gHeight - wantedHeight * 1.2f) /
                                      wantedHeight));
        }
    ...
    owner->SetVelocity(spd +
                       UpVector * (mapInfo->map.gravity * myGravity) *
                         int((owner->midPos.y - owner->radius) > gHeight));
    ...
    if (!owner->IsStunned()) {
        ...
        owner->SetVelocity(spd * (1 - int(owner->beingBuilt &&
    ...
    owner->Move(UpVector * (Clamp(pos.y,
                                  gHeight,
                                  readMap->GetCurrMaxHeight() +
                                    owner->unitDef->wantedHeight * 5.0f) -
                            pos.y),
                true);
    ...
    if (modInfo.allowAircraftToHitGround) {
        const bool groundContact =
          (gHeight > (owner->midPos.y - owner->radius));
        const bool handleContact = (aircraftState != AIRCRAFT_LANDED &&
                                    aircraftState != AIRCRAFT_TAKEOFF);

        if (groundContact && handleContact) {
            owner->Move(UpVector *
                          (gHeight - (owner->midPos.y - owner->radius) + 0.01f),
                        true);

            const float3& gNormal = CGround::GetNormal(pos.x, pos.z);
            const float impactSpeed =
              -spd.dot(gNormal) * int(1 - owner->IsStunned());

            if (impactSpeed > 0.0f) {
                // fix for mantis #1355
                // aircraft could get stuck in the ground and never recover (on
                // takeoff near map edges) where their forward speed wasn't
                // allowed to build up therefore add a vertical component help
                // get off the ground if |speed| is below a certain threshold
...
CStrafeAirMoveType::BrakingDistance(float speed, float rate) const
{
    ...
    //		s_i = a_0 * r * (r^i - 1) / (r - 1) - d * (r * (r^i - 1) / (r - 1) -
    //i) / (r - 1)
    ...
    const int n =
      math::floor(math::log(d / (d - speed * (r - 1))) / math::log(r));
    const float r_n = math::pow(r, n);

    const float dist = speed * r * (r_n - 1) / (r - 1) -
                       d * (r * (r_n - 1) / (r - 1) - n) / (r - 1);
...
CStrafeAirMoveType::SetMaxSpeed(float speed)
{
    ...
    if (accRate != 0.0f && maxSpeed != 0.0f) {
        // meant to set the drag such that the maxspeed becomes what it should
        // be
...
CStrafeAirMoveType::StopMoving(bool callScript, bool hardStop)
{
    ...
    if (aircraftState != AAirMoveType::AIRCRAFT_FLYING &&
        aircraftState != AAirMoveType::AIRCRAFT_LANDING)
        return;
