...
ParseSpeedModClass(const std::string& moveDefName, const LuaTable& moveDefTable)
{
    ...
    if (speedModClass != -1)
        return Clamp(
          MoveDef::SpeedModClass(speedModClass), MoveDef::Tank, MoveDef::Ship);
...
MoveDefHandler::MoveDefHandler(LuaParser* defsParser)
{
    ...
    CMoveMath::noHoverWaterMove =
      (mapInfo->water.damage >= MAX_ALLOWED_WATER_DAMAGE_HMM);
    CMoveMath::waterDamageCost =
      (mapInfo->water.damage >= MAX_ALLOWED_WATER_DAMAGE_GMM)
        ? 0.0f
        : (1.0f / (1.0f + mapInfo->water.damage * 0.1f));
...
MoveDef::MoveDef(const LuaTable& moveDefTable, int moveDefID)
{
    *this = MoveDef();
    ...
    const float minWaterDepth =
      moveDefTable.GetFloat("minWaterDepth", GetDefaultMinWaterDepth());
    const float maxWaterDepth =
      moveDefTable.GetFloat("maxWaterDepth", GetDefaultMaxWaterDepth());
    ...
    switch ((speedModClass = ParseSpeedModClass(name, moveDefTable))) {
        ...
        case MoveDef::KBot: {
            depthModParams[DEPTHMOD_MIN_HEIGHT] =
              std::max(0.00f, depthModTable.GetFloat("minHeight", 0.0f));
            depthModParams[DEPTHMOD_MAX_HEIGHT] = (depthModTable.GetFloat(
              "maxHeight", std::numeric_limits<float>::max()));
            depthModParams[DEPTHMOD_MAX_SCALE] =
              std::max(0.01f,
                       depthModTable.GetFloat(
                         "maxScale", std::numeric_limits<float>::max()));
            depthModParams[DEPTHMOD_QUA_COEFF] =
              std::max(0.00f, depthModTable.GetFloat("quadraticCoeff", 0.0f));
            depthModParams[DEPTHMOD_LIN_COEFF] = std::max(
              0.00f,
              depthModTable.GetFloat("linearCoeff",
                                     moveDefTable.GetFloat("depthMod", 0.1f)));
            depthModParams[DEPTHMOD_CON_COEFF] =
              std::max(0.00f, depthModTable.GetFloat("constantCoeff", 1.0f));

            // ensure [depthModMinHeight, depthModMaxHeight] is a valid range
            depthModParams[DEPTHMOD_MAX_HEIGHT] =
              std::max(depthModParams[DEPTHMOD_MIN_HEIGHT],
                       depthModParams[DEPTHMOD_MAX_HEIGHT]);

            depth = maxWaterDepth;
            maxSlope =
              DegreesToMaxSlope(moveDefTable.GetFloat("maxSlope", 60.0f));
        } break;

        case MoveDef::Hover: {
            depth = maxWaterDepth;
            maxSlope =
              DegreesToMaxSlope(moveDefTable.GetFloat("maxSlope", 15.0f));
        } break;
    ...
    speedModMults[SPEEDMOD_MOBILE_BUSY_MULT] = std::max(
      0.01f, speedModMultsTable.GetFloat("mobileBusyMult", 1.0f /*0.10f*/));
    speedModMults[SPEEDMOD_MOBILE_IDLE_MULT] = std::max(
      0.01f, speedModMultsTable.GetFloat("mobileIdleMult", 1.0f /*0.35f*/));
    speedModMults[SPEEDMOD_MOBILE_MOVE_MULT] = std::max(
      0.01f, speedModMultsTable.GetFloat("mobileMoveMult", 1.0f /*0.65f*/));

    avoidMobilesOnPath = moveDefTable.GetBool("avoidMobilesOnPath", true);
    allowTerrainCollisions =
      moveDefTable.GetBool("allowTerrainCollisions", true);

    heatMapping = moveDefTable.GetBool("heatMapping", false);
    flowMapping = moveDefTable.GetBool("flowMapping", true);

    heatMod =
      moveDefTable.GetFloat("heatMod", (1.0f / (GAME_SPEED * 2)) * 0.25f);
    ...
    followGround =
      (speedModClass == MoveDef::Tank || speedModClass == MoveDef::KBot);
    ...
    // or water)
    if ((followGround && maxWaterDepth <= 0.0f) ||
        speedModClass == MoveDef::Hover)
                        ...
                        int* maxBlockBitPtr) const
{
    ...
    const int zMin = -zsizeh * (1 - centerOnly),
              zMax = zsizeh * (1 - centerOnly);
    const int xMin = -xsizeh * (1 - centerOnly),
              xMax = xsizeh * (1 - centerOnly);
    ...
    for (int z = zMin; (z <= zMax) && retTestMove; z++) {
        for (int x = xMin; (x <= xMax) && retTestMove; x++) {
            const float speedMod = CMoveMath::GetPosSpeedMod(
              *this, xTestMoveSqr + x, zTestMoveSqr + z, testMoveDir2D);
    ...
    if (retTestMove) {
        ...
        retTestMove &=
          (!testObjects || (blockBits & CMoveMath::BLOCK_STRUCTURE) == 0);
    }
                        ...
                        int* maxBlockBit) const
{
    ...
    return (TestMoveSquare(collider,
                           xTestMoveSqr,
                           zTestMoveSqr,
                           testMoveDir,
                           testTerrain,
...
MoveDef::GetDepthMod(const float height) const
{
    ...
    const float scale =
      Clamp((a * depth * depth + b * depth + c), minScale, maxScale);
...
MoveDef::GetCheckSum() const
{
    ...
    const unsigned char* minByte =
      reinterpret_cast<const unsigned char*>(&speedModClass);
    const unsigned char* maxByte =
      reinterpret_cast<const unsigned char*>(&flowMapping) +
      sizeof(flowMapping);
